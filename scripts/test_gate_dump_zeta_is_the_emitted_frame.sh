...
# callgen.act quads the old dump never printed; (4) the MEASURES line.
...
n="$(timeout 20s ./scrip --dump-zeta scripts/fixtures/frame_r1_witness.pl </dev/null 2>/dev/null | awk "/^; graph [0-9]+ 'r\/1'/{p=1} /^; graph [0-9]+ 'q\/1'/{p=0} p" | grep -c 'callgen.act —')"
