...
# ARMS:
#   1 the census's own selftest: every declared form is counted once or refused as not-storage (typedef, extern), by NAME
...
#   3 FAIL-ONCE BUILT IN: a scratch copy of src/ with ONE planted file-scope table reads RED at baseline+1, naming the growth
# The census's unguarded count (a bound macro never compared in its file) is printed, not graded here: bringing it to zero is the
# row's loud-refusal sweep, a separate criterion.
set -uo pipefail
...
ck "1 the census selftest: every declared form counted once, a typedef and an extern refused ($(grep -c '^SELFTEST: ' <<<"$st") checks)" '[ "$st_rc" = 0 ] && grep -q "^SELFTEST PASS" <<<"$st"'
[ "$st_rc" = 0 ] || printf '%s\n' "$st" | grep FAIL | sed 's/^/      /'
...
echo "  census: $n fixed-bound declarations at file, static or field scope, baseline $b; $u bound by a macro never compared in its file (the loud-refusal sweep's population)"
...
mkdir -p "$WORK/r/scripts/fixtures/dyn_caps" && cp -r "$ROOT/src" "$WORK/r/src" && cp "$C" "$WIT" "$WORK/r/scripts/" && cp "$B" "$WORK/r/scripts/fixtures/dyn_caps/"
printf '#define PLANTED_BY_THE_RATCHET_GATE_MAX 8\nstatic int g_planted_by_the_ratchet_gate[PLANTED_BY_THE_RATCHET_GATE_MAX];\n' > "$WORK/r/src/planted_by_the_ratchet_gate.c"
po=$(cd "$WORK/r" && bash scripts/util_dyn_caps_witness.sh census 2>&1); prc=$?
ck "3 FAIL-ONCE: one planted file-scope table reads RED at baseline+1 ($b -> $((b + 1)))" '[ "$prc" != 0 ] && grep -q "grew from $b to $((b + 1))" <<<"$po"'
if [ "$fails" = 0 ]; then echo "GATE PASS(0) [$GATE_NAME]: 3 arms -- $n fixed tables, exactly the baseline; a planted one reds"; gate_stamp; exit 0; fi
echo "GATE FAIL(1) [$GATE_NAME]: $fails of 3 arms red"; gate_stamp; exit 1
