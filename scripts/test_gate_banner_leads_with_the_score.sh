...
head8="$(timeout 100 bash "$HERE/s4e_msg.sh" banner 2>&1 | head -14)"
[ -n "$head8" ] || { echo "⛔ GATE REFUSE(2) [banner-leads-with-the-score]: the banner printed nothing"; exit 2; }
red=0
printf '%s\n' "$head8" | grep -q '^PROGRESS [0-9][0-9]-[0-9][0-9] |' || { echo "RED: no 'PROGRESS <day> |' line in the banner's first 14 lines:"; printf '%s\n' "$head8" | sed 's/^/    /'; red=1; }
broken="$(S4E_PROGRESS_PROBE_BROKEN=1 timeout 100 bash "$HERE/s4e_msg.sh" banner 2>&1 | head -14)"
printf '%s\n' "$broken" | grep -q '^PROGRESS: UNREADABLE' || { echo "RED: with the probe broken the banner did not print a visible 'PROGRESS: UNREADABLE' line in its first 14 lines"; red=1; }
