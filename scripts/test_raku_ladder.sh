...
MASTER_DIR="$S4E/corpus/tests/raku"; MASTER_EXT=.raku; export MASTER_DIR MASTER_EXT
TO=""; ONLY=""; LIST=0
while [ $# -gt 0 ]; do
  case "$1" in
    --to) shift; TO="${1:-}";;
...
[ -f "$MASTER_DIR/ALL.raku" ] && [ -f "$MASTER_DIR/ALL.ref" ] && [ -f "$MASTER_DIR/ALL.csv" ] || refuse "Raku master suite missing under $MASTER_DIR (ALL.raku / ALL.ref / ALL.csv)"
...
for pair in $(printf '%s\n' "${origins[@]}" | sort -n | tr ' ' ':'); do
  r=${pair%%:*}; o=${pair#*:}; src="$W/$o.raku"; ref="$W/$o.ref"
