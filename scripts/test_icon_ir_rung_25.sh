...
for icn in "$RUNG_DIR"/rung25_global_*.icn; do
  ...
  ${1:-/tmp/scrip-cc} -jvm "$icn" -o /tmp/t25.j 2>/dev/null
  timeout 30 java -jar miscellaneous/jasmin.jar /tmp/t25.j -d /tmp/ 2>/dev/null
  for rj in /tmp/*\$*.j; do [ -f "$rj" ] && timeout 30 java -jar miscellaneous/jasmin.jar "$rj" -d /tmp/ 2>/dev/null; done
