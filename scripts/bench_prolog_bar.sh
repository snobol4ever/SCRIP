...
read -r _k _g _gp _s _m3 _m4 check <<<"$row"
...
read -r _k gnu gplc swi m3 m4 <<<"$wrow"
case "$RIVAL" in swi) rv=$swi;; gnu) rv=$gnu;; gplc) rv=$gplc;; esac; case "$MODE" in m3) mine=$m3;; *) mine=$m4;; esac
