#!/usr/bin/env bash
# test_smoke_compile.sh — gate for GOAL-MODE4-EMIT EM-1..EM-4
#
...
#   - --compile + --target=x86 flag pair selects asm emission to stdout.
...
CEOF
gcc -O0 -g -w -I"$ROOT/src" -I"$ROOT/src/emitter" -I"$ROOT/src/lower" -I"$ROOT/src/machine" -I"$ROOT/src/runtime" -I"$ROOT/src/runtime/snobol4" \
    "$TMP/unh.c" \
...
END
EOF
"$SCRIP" --compile "$TMP/real.sno" > "$TMP/real.s" 2> "$TMP/real_emit.err" || {
...
set +e
"$SCRIP" --compile "$TMP/real.sno" >/dev/null 2>&1
...
# valid only inside the in-process native codegen (mode 3); a follow-up rung
...
END
EOF
"$SCRIP" --compile "$TMP/em7c_inv.sno" > "$TMP/em7c.s" 2> "$TMP/em7c.err" || {
...
# output matches --run (the mode-3 oracle for mode-4).
EM7C_GOT=$("$TMP/em7c_bin" < /dev/null 2>/dev/null)
EM7C_WANT=$("$SCRIP" --run "$TMP/em7c_inv.sno" < /dev/null 2>/dev/null)
[ "$EM7C_GOT" = "$EM7C_WANT" ] || {
...
# regenerated by this test (writes through scrip --compile) so the
...
if [ -d "$DEMO" ]; then
    ...
    (cd "$DEMO" && for f in roman wordcount claws5 treebank-list treebank-array; do
        "$SCRIP" --compile "$f.sno" > "$f.s" 2>/dev/null || {
