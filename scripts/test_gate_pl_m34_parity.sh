...
# EXCISED: both abort (m3 SIGABRT rc=134, m4 link/asm/emit fails rc!=0). Not a failure.
...
VERBOSE=0
[ "${1:-}" = "--verbose" ] && VERBOSE=1
PASS=0; FAIL=0; EXCISED=0; SKIP=0
...
for pl in "$CORPUS"/rung0[1-9]_*.pl \
    ...
    if [ $m3_abort -eq 1 ] && [ $m4_abort -eq 1 ]; then
        [ $VERBOSE -eq 1 ] && echo "EXCISED $name (both abort: m3_rc=$m3_rc m4_rc=$m4_rc)"
        EXCISED=$((EXCISED+1))
...
echo "--- PL-M34-PARITY: PASS=$PASS FAIL=$FAIL EXCISED=$EXCISED SKIP=$SKIP ---"
[ "$FAIL" -eq 0 ]
