...
#   mode 3 = --run                      (stackless native)      — TRACKED: DECLINED until GZ regrows it.
...
# A mode that prints the Stack-Machine-eXcision banner is reported DECLINED (expected mid-Ground-Zero), not FAIL,
...
pl() {
    ...
    # mode 3 — --run (tracked; DECLINED until regrown). PL-GZ-1b(d): a program the native blob does not
    # cover executes via the mode-2 interpreter and says so on stderr — output is STILL verified
    # (mismatch = FAIL); a verified fallback counts DECLINED, NOT a native PASS.
    ...
    if grep -q "MODE-3 INTERP-FALLBACK" "$e3"; then
        if [ "$o3" = "$expected" ]; then r3="m3 DECLINED"; X3=$((X3+1)); else r3="m3 FAIL"; F3=$((F3+1)); fi
    elif echo "$o3" | grep -qE "$SMX_SIG"; then r3="m3 DECLINED"; X3=$((X3+1))
...
echo "mode-3 (--run):               PASS=$P3 FAIL=$F3 DECLINED=$X3   / $N   (tracked)"
echo "mode-4 (--compile x86):       PASS=$P4 FAIL=$F4 DECLINED=$X4   / $N   (tracked)"
[ "$F2" -eq 0 ]
