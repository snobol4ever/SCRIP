#!/usr/bin/env bash
# test_prolog_rung12.sh — atom builtins: atom_length/atom_concat/atom_chars/atom_codes
S4E="${S4E_HOME:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"   # D-17 PORTABLE-HOME: the sibling root (all repos + oracles are siblings under ONE root; /home/claude2-style seat roots work with zero env; S4E_HOME overrides)
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CORPUS=$S4E/corpus/tests/prolog
FAMILY=rung12_atom_builtins
SNO="$CORPUS/$FAMILY.pl"
REF="$CORPUS/$FAMILY.ref"

echo "=== rung12: atom builtins (atom_length/atom_concat/atom_chars/atom_codes) ==="

# consolidated 2026-08-28 (tests-consolidate-prolog): the loose $CORPUS/$FAMILY_*.pl
# files this script used to glob were replaced by one suite pair. A suite file is
