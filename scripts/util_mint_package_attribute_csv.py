...
existing package files already use (rank,entry,origin,package,n_lines,stdin,want_rc,modes) plus the heap_kb column that
...
usage: util_mint_package_attribute_csv.py --package NAME --dir DIR --glob PATTERN [--glob PATTERN ...] [--recursive]
                                           [--origin-root DIR] [--modes 'm3,m4'] [--filter-grep REGEX] [--out PATH] [--refresh]
...
def main():
    ...
    ap.add_argument('--origin-root', default=''); ap.add_argument('--modes', default='m3,m4')
