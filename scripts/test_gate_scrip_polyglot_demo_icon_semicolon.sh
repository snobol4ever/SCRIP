...
for d in demo05 demo07 demo08 demo09 demo10; do
    src="$(ls "$CORPUS/scrip/$d"/*.scrip 2>/dev/null | head -1)"
    exp="$(ls "$CORPUS/scrip/$d"/*.expected 2>/dev/null | head -1)"
    [ -f "$src" ] && [ -f "$exp" ] || fail "$d: witness or .expected not found under $CORPUS/scrip/$d"
