...
run_smoke() {
    ...
    java -jar "$JASMIN" "$HERE/../interpreters/runtime/jvm/SnoRt.j" -d "$class_dir" > /dev/null 2>&1
    java -jar "$JASMIN" "$HERE/../interpreters/runtime/jvm/SnoRtMatchState.j" -d "$class_dir" > /dev/null 2>&1
    # Compile SnoPat.java (pattern matcher) with SnoRt on classpath
    if which javac > /dev/null 2>&1; then
        javac -cp "$class_dir" -d "$class_dir" "$HERE/../interpreters/runtime/jvm/SnoPat.java" > /dev/null 2>&1
    fi
    # Run
