...
[ "$n103" -ge 16 ] && [ "$n101" -ge 5 ] && [ "$n104" -ge 1 ] && [ "$nok" -ge 16 ] || {
  echo "⛔ REFUSE(2): the oracle's own stream no longer carries all four outcomes this gate exists to tell apart"
  echo "   measured 103=$n103 (want >=12)  101=$n101 (want >=3)  104=$n104 (want >=1)  no-error=$nok (want >=16)"
...
if [ "$RC" = 0 ]; then echo "GATE PASS [$(basename "${BASH_SOURCE[0]}" .sh)]: ${#CASES[@]} witness(es) x 2 modes = $PASS gradings byte-identical to icont -- 9 string builtins and the 6 scanning builtins refuse a non-string argument with the oracle's own code (103), keep 101 for an integer slot and 104 for a cset slot, and still convert a number"
