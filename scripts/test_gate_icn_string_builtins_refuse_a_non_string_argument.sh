...
CASES=( 'trim(L)' 'map(L)' 'reverse(L)' 'left(L,3)' 'right(L,3)' 'center(L,3)' 'repl(L,2)' 'detab(L)' 'entab(L)'
...
[ "$n103" -ge 12 ] && [ "$n101" -ge 3 ] && [ "$n104" -ge 1 ] && [ "$nok" -ge 8 ] || {
  echo "⛔ REFUSE(2): the oracle's own stream no longer carries all four outcomes this gate exists to tell apart"
  echo "   measured 103=$n103 (want >=12)  101=$n101 (want >=3)  104=$n104 (want >=1)  no-error=$nok (want >=8)"
  echo "   a gate that cannot see a 101, a 104 and a CLEAN conversion beside its 103s is satisfiable by a builtin that raises 103 unconditionally"; exit 2; }
RC=0; PASS=0; FAIL=0
for x in "${CASES[@]}"; do
  printf 'procedure main()\n  L := [1,2];\n  write(%s);\nend\n' "$x" > "$T/w.icn"
  ( cd "$T" && "$ICONT" -s w.icn -x ) >"$T/w.ref" 2>&1
...
if [ "$RC" = 0 ]; then echo "GATE PASS [$(basename "${BASH_SOURCE[0]}" .sh)]: ${#CASES[@]} witness(es) x 2 modes = $PASS gradings byte-identical to icont -- 9 string builtins refuse a non-string argument with the oracle's own code (103), keep 101 for an integer slot and 104 for a cset slot, and still convert a number"
