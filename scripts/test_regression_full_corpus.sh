...
SUITE="${SUITE:-$CORPUS/packages/snobol4/csnobol4_suite}"
    ...
    case "$MODE" in
        --run)
            if [ -n "$stdin_data" ]; then
                SNO_LIB="$INC" timeout "$TIMEOUT" "$SCRIP" --run "$sno" <<< "$stdin_data" 2>/dev/null || true
            else
...
run_test() {
    ...
    if is_stdin_test "$name" && [ "$MODE" = "--run" ]; then
        local pt st
