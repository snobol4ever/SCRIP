#!/usr/bin/env bash
# test_prolog_rung21.sh — char_type: alpha, digit_val, space_alnum, to_upper_lower, upper_lower
S4E="${S4E_HOME:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"   # D-17 PORTABLE-HOME: the sibling root (all repos + oracles are siblings under ONE root; /home/claude2-style seat roots work with zero env; S4E_HOME overrides)
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CORPUS=$S4E/corpus/tests/prolog
FAMILY=rung21_char_type
SNO="$CORPUS/$FAMILY.pl"
REF="$CORPUS/$FAMILY.ref"

echo "=== rung21: char_type (alpha/digit_val/space_alnum/to_upper_lower/upper_lower) ==="
