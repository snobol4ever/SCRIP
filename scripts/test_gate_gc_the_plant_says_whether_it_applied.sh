...
export SCRIP_HEAP_KB="${SCRIP_HEAP_KB:-64}"
...
echo "  HOLDS: the plant's applied banner is the proof it applied and prints with nothing asked for; its decline spells no literal a count would match; every gate that plants reads the banner (arena SCRIP_HEAP_KB=$SCRIP_HEAP_KB, witness $(basename "$WIT"), apply=$APPLY_SHIFT decline=$DECLINE_SHIFT)"
...
[ "$a_tel" -ge 1 ] || { echo "⛔ GATE REFUSE(2) [$G]: PREMISE UNMET -- at shift $APPLY_SHIFT and SCRIP_HEAP_KB=$SCRIP_HEAP_KB the plant applied $a_tel times under telemetry, so this configuration cannot test a VISIBLE application. That is a statement about the arena, not a defect; raise the arena or lower the shift"; exit 2; }
...
[ "$d_tel" -eq 0 ] && [ "$d_dec" -ge 1 ] || { echo "⛔ GATE REFUSE(2) [$G]: PREMISE UNMET -- at shift $DECLINE_SHIFT and SCRIP_HEAP_KB=$SCRIP_HEAP_KB the plant applied $d_tel times and printed $d_dec decline line(s), so this configuration produces no decline to impersonate an application. Arena headroom exceeds the shift; lower the arena or this gate measures nothing"; exit 2; }
...
SCRIP_GC_PLANT_FLIP|GC-FLIP|test_gate_gc_the_flip_plant_turns_a_stale_pointer_into_a_located_fault.sh test_gate_gc_a_frame_never_inherits_a_dead_siblings_map_cell.sh test_gate_gc_the_scan_subject_base_is_reloaded_after_a_call_inside_a_scan.sh test_gate_gc_the_birth_ledger_names_a_line_not_a_two_thousand_line_symbol.sh test_gate_icn_the_cset_registry_is_weak_so_a_dead_computed_cset_leaves_the_collector.sh
