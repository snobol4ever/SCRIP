...
if [ "$blobs" -le "$BLOB_CEILING" ]; then
    echo "  arm 6 PASS: $blobs stored-pattern BLOB activation frame(s) carry NO map cell, at or under the declared ceiling $BLOB_CEILING (since 2026-09-19 every blob prologue stores its DT_MAP cell on the line after the carve and its map carries a static layout, ARCH-GC section 6.2e; the 6.2b exclusion is retired)"
