...
python3 scripts/audit_fixed_caps_census.py --tsv "$T/c.tsv" > "$T/census.txt" 2>&1 || { echo "REFUSE(2): the census refused"; exit 2; }
red=0
...
census)
  ...
  base=$(cat "$B"); n=$(awk -F'\t' 'NR>1 && ($3=="file"||$3=="static"||$3=="field")' "$T/c.tsv" | wc -l); u=$(awk -F'\t' 'NR>1 && ($3=="file"||$3=="static"||$3=="field") && $8=="UNGUARDED"' "$T/c.tsv" | wc -l)
  echo "fixed-bound declarations at file, static or field scope: $n (baseline $base); bound by a macro never compared in its file: $u"
