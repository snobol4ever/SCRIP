...
if [ ! -f "$PW" ]; then
  ...
  pl_unrep="$(grep '^\[GC-RTCCB\] plant:' "$eon" | grep -o 'unrepaired=[01] swept=[01]' | grep -c '^unrepaired=1')"; pl_swept="$(grep '^\[GC-RTCCB\] plant:' "$eon" | grep -o 'unrepaired=[01] swept=[01]' | grep -c 'swept=1$')"
  ...
  if [ "${s7on:-0}" -ge 1 ] && [ "${s7off:-0}" = 0 ] && [ "${nab:-0}" -ge 1 ] && [ "${cens:-0}" = "${pl_unrep:-x}" ] && cmp -s "$poff" "$pon"; then
    ck ok "(g) FAIL-ONCE HOLDS: with a heap pointer planted in slot 7 the detector NAMES it $s7on time(s) over $nap planted collection(s); with the plant off the same witness reads ZERO at slots 7 and 8; the plant announces itself $nab time(s) with no telemetry asked for; and stdout is BYTE-IDENTICAL either way, so the instrument has no footprint on the answer. ⭐ AND THE PLANT MEASURES THE HAZARD RATHER THAN ASSERTING IT: $pl_unrep of $nap collection(s) left the slot UNREPAIRED after forwarding the block it names (moved, and nothing rewrote the word, because no root walk visits this block), $pl_swept left it pointing at swept ground. ⭐⭐ AND IT IS ARM (i)'s POSITIVE CONTROL, CHECKED BY AGREEMENT RATHER THAN ASSUMED: the staleness census -- a different reader, counting GC-RTCCB-STALE lines over every slot -- reports $cens unrepaired sightings at slot 7 against the plant's own $pl_unrep, and the two agree to the unit. Two readers of one run that disagree would mean one of them is wrong, and this arm would say so"
  else
    ck no "(g) ⛔ THE DETECTOR IS NOT PROVEN TO FIRE: planted slot-7 sightings=$s7on (want >=1), unplanted slot-7/8 sightings=$s7off (want 0), bare applied banners=$nab (want >=1), staleness-census sightings at slot 7=$cens against the plant's own $pl_unrep (want EQUAL -- two readers of one run, and a disagreement means one of them is wrong), stdout identical=$(cmp -s "$poff" "$pon" && echo yes || echo NO). A zero from arm (b) is only a measurement while this arm holds; if the plant applied but the detector said nothing, the reporter is blind and every green reading of this invariant since it landed is a silence"
  fi
  rm -f "$pon" "$poff" "$eon" "$eoff" "$bare"
fi
