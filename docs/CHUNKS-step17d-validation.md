...
```
$ SCRIP_PROC_ENTRY_PCS=1 ./scrip --run /home/claude/corpus/programs/prolog/palindrome.pl
...
--run Prolog crash (consumer still dormant — expected).
...
| Icon ----run all rungs | PASS=186 FAIL=47 XFAIL=30 TOTAL=263 |
...
consumer site.  After CH-17e, `--run` Prolog should work end-to-end for
the first time.
