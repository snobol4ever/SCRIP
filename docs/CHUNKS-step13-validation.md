...
`SCRIP_CHUNKS_AUDIT=1 ./scrip --run test/raku/rk_given.raku`:
...
| `--run` | `sm_interp: stack underflow` (Aborted) | reaches default branch in all 6 calls; outputs "weekday" twice (broken at deeper Raku-SM-mode level — the surrounding `say`/sub infrastructure is broken; this is pre-existing per the Raku full-suite baseline 29/0/0 across IR/SM/JIT) |
| `--run` | `sm_interp: stack underflow` (Aborted) | same as `--run` — no longer underflows |

The CASE wrapper itself no longer underflows.  Full Raku --run /
