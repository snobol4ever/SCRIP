...
`sm_interp.c`, `interp_eval.c`) so `----run`, `--run`, and the
...
consumption patterns differed across modes: `----run` of an Icon
program advanced only `coro_value.c::_rnd_seed`; `--run` of the
...
bash scripts/test_icon_all_rungs.sh       # PASS=177 FAIL=56 XFAIL=30
...
- Pre-rung: `----run` produced `65 / 84`; `--run` produced
...
`rung36_jcon_random.icn` still diverges between `----run` and
`--run`. After A3-seed-fix the divergence is no longer
seed-driven; it is caused by two unrelated SM-mode gaps:

1. `&lcase` / `&ucase` keyword evaluation returns empty under
   `--run` (verified with a minimal `write(&lcase)` probe —
   `----run` prints `abcdefghijklmnopqrstuvwxyz`, `--run`
   prints nothing).
2. `?L` (random element of a list) under `--run` raises
