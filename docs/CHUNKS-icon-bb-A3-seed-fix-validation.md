...
`sm_interp.c`, `interp_eval.c`) so `----interp`, `--interp`, and the
...
consumption patterns differed across modes: `----interp` of an Icon
...
- Pre-rung: `----interp` produced `65 / 84`; `--interp` produced
...
`rung36_jcon_random.icn` still diverges between `----interp` and
...
1. `&lcase` / `&ucase` keyword evaluation returns empty under
   ...
   `----interp` prints `abcdefghijklmnopqrstuvwxyz`, `--interp`
   prints nothing).
