...
|------|----------|-----------|
| `----interp`  | PASS (output: `hello world`) | PASS unchanged |
| `--interp`  | FAIL (rc=134 SIGABRT, "sm_interp: stack underflow") | **PASS** byte-identical to --interp |
| `--run` | FAIL (rc=134 SIGABRT, same stack underflow) | **PASS** byte-identical to --interp |
...
flips --interp / --run FAIL→PASS byte-identical to ----interp output.
...
49/0, scrip_all_modes 2/0, Icon ----interp 177/56/30.
