...
|------|----------|-----------|
| `----run`  | PASS (output: `hello world`) | PASS unchanged |
| `--run`  | FAIL (rc=134 SIGABRT, "sm_interp: stack underflow") | **PASS** byte-identical to --run |
| `--run` | FAIL (rc=134 SIGABRT, same stack underflow) | **PASS** byte-identical to --run |

Audit-counter sweep with `SCRIP_EXPRS_AUDIT=1`:

```
$ SCRIP_EXPRS_AUDIT=1 ./scrip --run rung15_real_swap_lconcat.icn
...
`--run` with `SCRIP_EXPRS_AUDIT=1`: zero programs fire `SM_PUSH_EXPR`
...
| Icon corpus `--run` | 100/163 | 101/162 | **+1 PASS** (rung15_real_swap_lconcat) |
| `--run` rung01–04 | 20/24 | 20/24 | unchanged (the 4 FAILs are pre-existing: rung02_proc_fact, rung02_proc_locals, rung03_suspend_fail, rung03_suspend_return) |

The +1 in the broad `--run` Icon corpus is exactly
...
## What this rung does NOT do
  ...
  - **Does not unblock rung11_bang_augconcat_*.**  Those programs FAIL
    under `--run` via a different mechanism (no `SM_PUSH_EXPR` fire
...
flips --run / --run FAIL→PASS byte-identical to ----run output.
...
49/0, scrip_all_modes 2/0, Icon ----run 177/56/30.
Icon --run corpus: 100/163 → 101/162 (+1 = rung15_real_swap_lconcat).
