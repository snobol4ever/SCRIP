...
5. Diagnostic env var `SCRIP_PROC_ENTRY_PCS=1` prints the
   ...
   ```
   $ SCRIP_PROC_ENTRY_PCS=1 ./scrip --run test/icon/hello.icn
   ...
   $ SCRIP_PROC_ENTRY_PCS=1 ./scrip --run /tmp/simple.pl
   ...
   $ SCRIP_PROC_ENTRY_PCS=1 ./scrip --run test/snobol4/hello/hello.sno
   [CH-17a] resolve entry_pcs (proc_table=0 procs, pl_pred_table=hash)
...
- Does NOT fix the broken `--run` Prolog path documented in
