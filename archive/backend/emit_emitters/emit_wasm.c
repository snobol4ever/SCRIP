...
/* ── Expression type ──────────────────────────────────────────────────────── */
/* Guard against redefinition when emit_wasm.h is included later in this TU  */
...
/* ── Output ────────────────────────────────────────────────────────────── */
/* Prolog WASM emitter uses its own output stream (wpl_out), so redefine W. */
#undef W
...
/* ── prescan ───────────────────────────────────────────────────────────── */
static void pl_prescan_expr(const EXPR_t *g) {
    if (!g) return;
    ...
    for (int i = 0; i < g->nchildren; i++) pl_prescan_expr(g->children[i]);
}

static void pl_prescan_prog(Program *prog) {
    emit_wasm_strlit_intern("");   /* index 0 = empty */
    atom_intern("");               /* atom_id 0 = unbound */
    if (!prog) return;
    for (STMT_t *s = prog->head; s; s = s->next) {
        pl_prescan_expr(s->subject);
        pl_prescan_expr(s->pattern);
        pl_prescan_expr(s->replacement);
    }
}
