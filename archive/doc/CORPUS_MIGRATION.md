...
- [x] **DELETE** `SCRIP/test/frontend/icon/corpus/` from SCRIP — commit `8327311`
...
- [x] Update prolog rung runners to use `CORPUS_REPO` (run_invariants.sh flat .pl glob) — SCRIP `6b5f6a5`
- [x] **DELETE** `SCRIP/test/frontend/prolog/corpus/` from SCRIP — commit `4da8aed`

## SNOBOL4 smoke (`programs/snobol4/smoke/`)

- [x] Copy `test/frontend/snobol4/*.sno` → `corpus/programs/snobol4/smoke/` — corpus `606c141`
- [x] Update any runners (jvm_artifact_check.sh null.sno path fixed — SCRIP `e63d8d6`)
- [x] **DELETE** originals from SCRIP — commit `2e22f6e`
...
- [x] Update any runners (run_sc_corpus_rung.sh comments updated — SCRIP `e63d8d6`)
- [x] **DELETE** originals from SCRIP — commit `edc0ab4`
...
Every box above is checked. `SCRIP/test/` contains no `.icn`, `.pl`, `.sno`,
`.sc`, or `.reb` corpus source files. All runners use `CORPUS_REPO`.

## ✅ MIGRATION COMPLETE — G-9 session 8 (2026-03-29)

All source programs migrated to corpus repo. SCRIP/test/ is clean.
...
- [x] **DELETE** all above from SCRIP — commit `f9fbf15`
...
- [x] **DELETE** all `test/snobol4/` from SCRIP — commit `f9fbf15`
...
`find SCRIP -name "*.sno" -o -name "*.icn" -o -name "*.pl"` → **zero results** ✅
