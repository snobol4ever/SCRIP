...
the four ports (α start / β resume / γ succeed / ω fail). one4all's `lower_pl.c` already aims at
...
| JCON / Icon (irgen.icn)            | Prolog (one4all)                          |
|------------------------------------|-------------------------------------------|
...
the file. one4all's `lower_pl.c` has no equivalent flag (per the existing PL-LOWER-REVAMP note),
...
untrails, restores, and pops `B = BB(cur_B)`. one4all's `pl_choice` already documents this exact
...
fail sentinel. This is reassurance that the CP-record-as-suspended-box design in one4all is the
