...
def leading_literal(node, sym):
    """The candidate's declarative prefix: the first consuming node, if it is a literal. Zero-width nodes
    (:my, {code}, :dba, MARKER, constant assertions) are skipped; a lookahead, call, class or alternation
    ends the prefix with no literal -> None (always entered)."""
    ZW = {'MY', 'CODE', 'MOD', 'EMPTY', 'MARK', 'FAILN', 'NOT'}
    n = node
    while True:
        if n.k == 'SEQ':
            for kid in n.kids:
                if kid.k in ZW: continue
                n = kid; break
            else:
                return None
            if n.k == 'SEQ': continue
...
def emit_all(path, out_c, provided=(), grammar='Perl6::Grammar'):
    ...
    for pn in sorted(protos):
        ...
        for d in cands:
            ...
            # ⛔ LTM's DECLARATIVE PREFIX: NQP enters a candidate only if its leading literal matches at the
            # cursor. Without it every candidate ran in full and a refusal in ANY of them refused the family
            # (`say 42;` refused inside quote:sym<s>, never a quote). The prefix is the candidate's leading LIT
...
static int rk_lit_peek(RkCur *c, const char *s) {           /* zero-width: does the literal start here? */
    int n = (int)strlen(s);
    return c->pos + n <= c->len && !memcmp(c->src + c->pos, s, (size_t)n);
}
