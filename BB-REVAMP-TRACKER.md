...
- [ ] bb_builtin_findall.cpp — (TIER H 2026-06-06: ef 4→0 (emit_fmt→std::string concat), pe 1→0 (PORT_*→literal Greek), lv 2→0 (std::string b inlined, std::string goal_build→static bff_goal() helper); asm-diff EMPTY (label-normalized, corpus does not exercise bb_builtin_findall mode-4); 13→6) — [S] rb=6: MEDIUM_BINARY raw-byte arms = medium-invisible conversion, owner GOAL-PROLOG-BB — [S] IR walk (gn->t/cand->t via fs->gcfg): EMIT-BLIND in spirit, design not pinned
- [ ] bb_builtin_io.cpp — (TIER H 2026-06-06: ef 6→0, pe 4→0 (BINARY ports→literal Greek glyphs, XK_PORT-preserving), lv 8→0 (bio_succ/bio_write_body/bio_fbits_str/bio_bin_write_arg helpers); eb 8→6; asm-diff EMPTY (label-normalized); 51→30) — [S] eb=6: pBB->α admission/operand reads → LOWER _.op_*/ζ-slots, design not pinned — [S] rb=24: MEDIUM_BINARY raw-byte arms, owner GOAL-PROLOG-BB
