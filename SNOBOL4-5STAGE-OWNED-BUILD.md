...
- [ ] **B7 NULLARY protos** — ARB/REM/BAL/FENCE/FAIL/SUCCEED/ABORT (replaces the B0 NULL-head placeholders). **FULL RECIPE (derived 2026-06-08 from bb_pattern_unary_i.cpp skeleton + B2 universal layout; split by mechanism so the trivial set lands first):**
  ...
  **BAL — SEPARATE SUB-RUNG:** balanced-paren scan ( () nesting ), regenerating; needs a depth counter (use +8 scratch for depth, +0 for start) + per-char ( )-balance scan in the α; defer.
  WIRING TOUCHPOINTS (B7a; mirror B4/B5/B6): (1) `bb_pattern_nullary.cpp` (new, ONE op_kind-discriminated template FAIL/REM/SUCCEED). (2) `bb_templates.h` decl. (3) `emit_core.c` — replace the FAIL/REM/SUCCEED stubs (currently lines ~431-434) with `bb_pattern_nullary()`; leave ARB/BAL/FENCE/ABORT stubbed. (4) `emit_bb.c` — FILL cases ×3 (`g_emit.op_kind="FAIL"/"REM"/"SUCCEED"; op_off=bb_slot_alloc24(nd); FILL(...)`), `descr_chain_arity` → 0 ×3. (5) `lower_sno.c` — `lower_pattern_build` arms `case TT_FAIL/TT_REM/TT_SUCCEED: return emit_leaf(cx, nalloc(cx,IR_PATTERN_xxx), γ_in,ω_in,α_out,β_out);` (NB: builder uses IR_PATTERN_*, NOT the legacy IR_PAT_* the old `lower_pattern` arms at 919-922 use); `sno_pattern_buildable` add `if(e->t==TT_FAIL||e->t==TT_REM||e->t==TT_SUCCEED) return 1;`; `sno_seq_is_pattern` add TT_FAIL/TT_SUCCEED (TT_REM already present). (6) `Makefile` RT_PIC_SRCS + per-obj rule (mirror bb_pattern_cat). VERIFY: build → probe `P=REM`/abcd→`abcd` · `P='a' REM`/abcd→`abcd` · `P=FAIL`/abc→fail · `P='a' FAIL`/abc→fail · `P=SUCCEED 'b'`/ab→`b` (anchored-ish) all sbl-MATCH → full m4 battery (smoke 7/7 · pat-rung 19/19 · corpus ≥155 non-decreasing · beauty 1/17 · fence HARD) → commit; do NOT push without "perform hand off".
