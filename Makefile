...
test: scrip  # ⭐ WAS THE FALSE-GREEN TRAP (cured hq_P s268): `test`, `test-ir` and `test-all` were named in .PHONY with NO RECIPE ANYWHERE, so each exited 0 having run NOTHING ("Nothing to be done for 'test'") while reading as a full green suite. `test-ir` and `test-all` are DELETED rather than wired — nothing behind them ever existed. This target now runs THE blocking set named in CLAUDE.md and fails loudly on the first red. ⛔ HQ-27 PRISTINE-BEFORE-VERDICT IS VOID as a per-landing requirement (Lon 2026-09-03 ~15:58, RULES.md FACT RULE: "It's time to loosen this pristine build that keeps preventing forward progress and causes 20 minute wait times."). A landing verdict runs on an INCREMENTAL make -- never on a stale binary, and the stale-binary refusal stays as the guard. `make pristine` survives for the ceo's audits, release points, and whenever that refusal fires; the receipt names the build it graded on. This comment asserted the retired law for hours after it was retired, which is the failure mode the FACT RULE exists to prevent: law that lives in a second unversioned place cannot be retired in one edit.
	...
	-bash scripts/test_gate_icn_genhost_layout_consistent.sh   # ~12s MEASURED (46 Icon demo+benchmark programs compiled, no run, no oracle): THE GENERATOR-HOST REGION LAYOUT IS INTERNALLY CONSISTENT. ⭐ WIRED 2026-09-06 (hq_B, row icon-generator-host-carve-sums-the-whole-component-and-reserves-66mb-on-jtran-main, ceo-372 clause 2 THE LOUD CAP AS THE FLOOR): a generator callee's region is carved by its CALLER (icn_gen_host_slice) and the callee lays out its own callee area as a host (icn_gen_host_reserve) -- THE SAME WALK SEEDED DIFFERENTLY, so the caller cuts a cycle early and hands over a small container while the callee cuts it late and lays out a large one. On jtran: container 1.8 MB, contents 246 MB, 134x, and it is a SILENT WILD WRITE, not a stack overflow -- the program compiles, links, runs, exits 0. ⛔ KEYED ON CONSISTENCY, NEVER ON SIZE, because the carve distribution is BIMODAL (65,544 for every program but three) and any size threshold that catches jtran reds icon_parser and icon_recognizer, which match icont today; the consistency audit fires on exactly the programs whose layout is wrong, including geddump.icn, which carves the SAME 65,544 as every clean program and is still inconsistent -- invisible to any threshold. ⛔ REPORTED, NOT BLOCKING, because it is RED TODAY (4 of 46: geddump 1 site, icon_parser 13, icon_recognizer 23, jtran 80; 3 of the 4 are GREEN against the oracle by not recursing deep enough), the same ramp the xfail census and board_packages took; GENHOST_LAYOUT_GATE_BLOCKING=1 proves it genuinely reds (rc=1), SCRIP_ICN_GENHOST_LAYOUT_STRICT=1 makes the COMPILER refuse rc=2 at the host, and it REFUSES rc=2 when it cannot see its population. The cure that clears it is per-activation, per-path carving on the stack (Lon 2026-09-06 "No activations on the heap"; the cto's SCC depth table) -- when it lands this reads 0 on all 46, which is the arm that proves it.
