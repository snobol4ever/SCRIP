...
.PHONY: preflight all scrip setup hooks pristine pristine-all buildinfo FORCE \
        test test-postoffice test-boards \
...
test-postoffice:  # ⭐ THE HERMETIC POSTOFFICE GATES, WIRED 2026-09-03 (hq_B, ceo ruling on row `postoffice-gates-red-on-origin-because-no-s4e-gate-is-in-make-test`). ⛔ WHY THIS TARGET EXISTS: three of these gates were RED ON ORIGIN and nobody knew, because NO s4e_* gate was in any runner -- a gate in no runner is not measuring, the same false-green shape `test` itself was cured of at s268. Each red was a stale FIXTURE, never a broken tool: the picker grew an owner-column constraint and a baton requirement, and the fixtures kept asserting against the older contract. ⭐ HERMETIC IS THE MEMBERSHIP TEST, AND IT IS MEASURED, NOT ASSUMED: every gate below builds its own scratch postoffice under mktemp, and the set was verified to leave /home/resources/postoffice byte-identical (QUEUE.tsv, QUEUE.done.tsv, claims/, tasks/) across a full run -- a gate that mutated live fleet state would be unrunnable here at any speed. The gates that read the LIVE postoffice (baton_donewhen_runnable{,_live}, baton_next_blocks, baton_one_next_block, baton_state_header_single_record, queue_is_an_index, s4e_release_verbs_mark_last_row) are deliberately OUT: they grade rows sixteen seats are editing right now, so they red on a dirty fleet BY DESIGN -- exactly the reason test_gate_preflight_complete.sh is out too (ceo, same ruling). Keep this target seconds-cheap; anything needing a build or a live read does not belong in it.
	...
	bash scripts/test_gate_banner_leads_with_the_suite_line.sh   # ~1.5s MEASURED, read-only against the live postoffice (banner is idempotent and writes nothing): THE SUITE BANNER LEADS. ⭐ WIRED 2026-09-06 (hq_T, row banner-leads-with-the-suite-banner-line-from-util-suite-banner, rank 0, Lon via ceo under THE FLEET-12 PLAN). ⛔ THE LOAD-BEARING ARM IS THE FAIL-ONCE ONE, not the happy path: a suite line that silently vanishes when .github is unpulled is INDISTINGUISHABLE FROM A FLEET WITH NO SUITES, and that is the state a fresh clone starts in -- so the gate drives the absent case through S4E_SUITE_BANNER_PROBE_BROKEN and requires the banner to SAY SO. It also asserts the computed verdict SURVIVES that refusal, because trading a missing line for a missing verdict is not a fix, and asserts the normal path does NOT print the refusal, or the absent arm would pass against a wiring that always refuses. Mutation-proved both ways: reorder the block below PROGRESS and ARM 2 reds; silence the absent message and ARM 3 reds.
...
test: scrip  # ⭐ WAS THE FALSE-GREEN TRAP (cured hq_P s268): `test`, `test-ir` and `test-all` were named in .PHONY with NO RECIPE ANYWHERE, so each exited 0 having run NOTHING ("Nothing to be done for 'test'") while reading as a full green suite. `test-ir` and `test-all` are DELETED rather than wired — nothing behind them ever existed. This target now runs THE blocking set named in CLAUDE.md and fails loudly on the first red. ⛔ HQ-27 PRISTINE-BEFORE-VERDICT IS VOID as a per-landing requirement (Lon 2026-09-03 ~15:58, RULES.md FACT RULE: "It's time to loosen this pristine build that keeps preventing forward progress and causes 20 minute wait times."). A landing verdict runs on an INCREMENTAL make -- never on a stale binary, and the stale-binary refusal stays as the guard. `make pristine` survives for the ceo's audits, release points, and whenever that refusal fires; the receipt names the build it graded on. This comment asserted the retired law for hours after it was retired, which is the failure mode the FACT RULE exists to prevent: law that lives in a second unversioned place cannot be retired in one edit.
	...
	bash scripts/test_gate_gate_wiring_ratchet.sh   # ~0.3s MEASURED (0.25/0.27/0.34), hermetic and offline (32 arms over scratch fixtures under mktemp; it builds no tree and reads no live state). THE RATCHET OVER scripts/gate_wiring.tsv, THE ONE POPULATION OF GATES. ⭐ WIRED 2026-09-06 (hq_T, row requested by hq_S, routed by the ceo as CEO-326). ⛔ THE DEFECT IT EXISTS FOR, hq_S's own measured case: test_gate_sno_fence_body_dynamic_operand.sh landed on disk IN THE SAME PUSH as the cure it grades and was never added to this recipe, so for a day the one instrument that could grade that cure was run by nothing -- and NOTHING COULD HAVE NOTICED, because the gate was green by hand, ls found it, the commit looked complete, and every board was byte-identical with and without the cure BY CONSTRUCTION. An unwired gate is indistinguishable from a wired one by every check except reading the recipe. ⛔ WHAT IS WIRED HERE IS THE INSTRUMENT'S OWN REFUSAL PROOFS, NOT THE LIVE CENSUS: measured 282 gates on disk and 92 reachable, so UNWIRED IS THE NORM at two in three -- a live census here would fire every seat's make test for everybody else's debt and be muted on day one, and a muted check is worse than no check because it looks like coverage. The live check runs WARN-ONLY in handoff_status.sh, which is the moment a seat asks 'is there anything I owe' (hq_S's placement argument).
	...
	bash scripts/test_gate_banner_reports_ahead_of_origin.sh   # ~3s MEASURED, mktemp fixture only (a bare origin plus a clone), no build needed: the banner NAMES a repo whose HEAD is not on origin. ⭐ WIRED 2026-09-08 (hq_B, ceo row instruments-the-banner-reports-a-seat-that-is-ahead-of-origin-because-finished-invisible-work-is-reported-by-nothing, rank 0). ⛔ THE GAP: the fleet reports a seat that STALLS and a seat that FAILS and reported NOTHING about a seat whose work is finished, correct and unpushed -- no red exists anywhere in that state and the seat's OWN banner says ✅ SUCCESS, because "safe to /clear" is a different question from "can anyone else see this". MEASURED COST 2026-09-08: 45 minutes on the cto's END-statement cure with the coo explicitly waiting, and 20 minutes on hq_B's digest-gate commit. ⛔ IT IS A REPORTING LINE, WHICH IS THE EASIEST THING HERE TO BREAK INVISIBLY -- delete it and every banner still prints, still says SUCCESS, and no suite goes red -- so the gate is PROVEN RED as well as green: with the arm removed from a copy of s4e_msg.sh it exits 1 naming the defect. Being ahead is NOT a failure and the gate asserts the word FAILURE never appears on the line and that the banner's rc is unchanged between its clean and ahead arms.
	...
	bash scripts/test_gate_icon_master_per_entry_identity.sh   # ~90s MEASURED on a built tree (the Icon master runs in 85s; the comparison adds seconds), needs the built binary: THE ICON MASTER IS GRADED BY PER-ENTRY IDENTITY, NOT BY A PASS FLOOR. ⭐ WIRED 2026-09-06 (hq_B, CEO-353 ruling on row icon-master-board-grades-per-entry-identity-not-a-floor; the ceo's words: "a count-neutral swap invisible to every floor is exactly what make test exists to catch; 90 s on a built tree is the trade and it is worth it"). ⛔ THE INCIDENT: board_icon_master.sh's floor stood at 596 while the tree carried 601; two programs regressed, the board fell 601 -> 599, printed "watermarks held" -- 599 is above 596 -- and invited a re-pin BELOW the truth. Every arm behaved as written; the instrument is the defect, because a floor scores A SET BY A SCALAR and cannot tell "the same 599 pass" from "599 pass, but not the same 599". ⛔⭐ AND THE WORST CASE IS A SWAP, NOT A DROP: two entries regress while two are cured in the same window and THE COUNT NEVER MOVES -- measured here at 676 passes on both sides, where this gate reds and NAMES the entry. A floor, a watermark and a delta are all silent on it by construction. Pinned per (origin, mode) in scripts/icon_master_identity_baseline.tsv, keyed on ALL.csv's ORIGIN because the builder renumbers entry names on every rebuild (a name-keyed pin would go mass-VANISHED plus mass-NEW and train its reader to re-pin without looking, which is the floor's own failure with extra steps). RED on a pinned PASS that stops passing or a pinned entry no longer graded; REPORTED and never red on an entry that starts passing, a newly added entry, a kind change between two non-PASS readings, and ast-shape drift. ⛔ Its fail-once arm is not optional and is proven: FAIL_ONCE=1 injects one pinned-PASS entry as FAIL downstream of a real run and upstream of the real comparison, and the gate REFUSES rc=2 if the injection does not register -- an identity gate that cannot fail on an injected red entry is the floor defect wearing a new name.
