...
<!-- SUITE-TABLE:BEGIN generated by .github/scripts/util_suite_banner.py --readme from .github/SUITES.tsv at .github@fdd92db1 -- do not edit by hand; scripts/test_gate_readme_suite_table_matches_suites_tsv.sh holds it -->
...
|:---|:---|---:|:---|
| SNOBOL4 | Gimpel | **129/144** | `2026-09-24` |
| SNOBOL4 | Budne | **71/93** | `2026-09-23` |
| SNOBOL4 | Flake | **118/180** | `2026-09-24` |
| SNOBOL4 | AIS | **5/7** | `2026-09-23` |
| SNOBOL4 | Dotnet | **5/14** | `2026-09-23` |
| SNOBOL4 | TPgm | **1/8** | `2026-09-16` |
| SNOBOL4 | X64T | **28/36** | `2026-09-23` |
| SNOBOL4 | SnoM (master) | **1969/1988** | `2026-09-24` |
| SNOBOL4 | SnoBench | **23/23** | `2026-09-25` |
| Icon | Zona | **88/88** | `2026-09-24` |
| Icon | Jcon | **82/82** | `2026-09-24` |
| Icon | IPL | **194/194** | `2026-09-24` |
| Icon | IcnM (master) | **826/826** | `2026-09-24` |
| Icon | IcnBench | **26/26** | `2026-09-24` |
| Prolog | INRIA | **442/442** | `2026-09-24` |
| Prolog | SWI | **1051/2935** | `2026-09-23` |
| Prolog | GNU | **11/11** | `2026-09-24` |
| Prolog | Logtalk | **3235/3600** | `2026-09-24` |
| Prolog | ProM (master) | **563/563** | `2026-09-24` |
| Prolog | ProBench | **23/23** | `2026-09-23` |
| Pascal | FPC | **156/181** | `2026-09-24` |
| Pascal | PAT | **368/427** | `2026-09-24` |
| Pascal | PasM (master) | **247/247** | `2026-09-24` |
| Pascal | PasBench | **11/11** | `2026-09-24` |
| Raku — IN DEVELOPMENT | Roast | **9/1464** | `2026-09-16` |
| Raku — IN DEVELOPMENT | RakM (master) | **867/929** | `2026-09-23` |
| Raku — IN DEVELOPMENT | RakBench | **5/17** | `2026-09-24` |
| Snocone | SncM (master) | **338/338** | `2026-09-24` |
| Snocone | SncBench | **16/16** | `2026-09-23` |
| Rebus | RebM (master) | **43/43** | `2026-09-23` |
| Rebus | RebBench | **3/3** | `2026-09-23` |
<!-- SUITE-TABLE:END -->
...
*Measured 2026-09-25 14:4x CDT on SCRIP `be0e10b9a` / corpus `4d805e1bc`, **RT_OPT=-O0**, load 1.2–1.5 on 16 cores, **500 ms of
repetitions per point**, five iterations (`BENCH_BUD_MS=500 BENCH_ITER_N=5`, one warm-up repetition first; the 500 ms budget was proven
against 5 s points kernel for kernel on 444e9893d), oracle `spitbol-bench-oracle/sbl -bf` (the clean benchmark oracle, never the
monitor-hooked correctness fork). **Both engines at SPITBOL's own defaults** (`-d128m -i1m -s4m`: SCRIP's shipped defaults since CEO-1261),
SCRIP with every diagnostic off (`SCRIP_DIAG=0`, CEO-1262). 23 of 23 kernels printed their ref on every angle in all three engines.* The
per-repetition figure is the fixed-time twin's (microseconds per repetition of the kernel's own function); the x-factor is SPITBOL time /
SCRIP time on that same twin (1.5x is one and a half times faster, 0.5x half as fast). The last two columns are the mode-4 x-factor of
the grid this one replaces (`3a936a797`, 13:2x the same day) and of the 2026-09-03 tree `380cc4162`, rebuilt and run 2026-09-24, for
the regression story below.

| kernel | SPITBOL µs/rep | m3 µs/rep | m4 µs/rep | m3 | m4 | `3a936a797` m4 | `380cc4162` m4 |
|---|---:|---:|---:|:---:|:---:|:---:|:---:|
| arith_loop | 25.6 | 7.2 | 6.8 | 3.56x | 3.76x | 4.03x | 4.03x |
| arith_loop_twin | 7293.0 | 2151.3 | 2156.1 | 3.39x | 3.38x | 3.36x | 3.47x |
| array_sum | 576.4 | 251.6 | 255.2 | 2.29x | 2.26x | 2.18x | 0.27x |
| eval_fixed | 133.5 | 193.2 | 186.5 | 0.69x | 0.72x | 0.70x | 0.96x |
| fib_recur | 3764.5 | 2062.6 | 2279.6 | 1.83x | 1.65x | 1.61x | 1.78x |
| fibonacci | 236.3 | 134.2 | 141.9 | 1.76x | 1.67x | 1.63x | 1.66x |
| func_call | 36.2 | 15.4 | 16.0 | 2.34x | 2.26x | 2.04x | 2.01x |
| ident_call1 | 29.4 | 10.4 | 10.1 | 2.83x | 2.91x | 2.65x | 2.46x |
| ident_call2 | 29.5 | 10.0 | 9.1 | 2.94x | 3.25x | 2.92x | 2.36x |
| indirect_dispatch | 26.8 | 44.6 | 45.8 | 0.60x | 0.58x | 0.57x | 0.62x |
| mixed_workload | 29.7 | 38.0 | 36.3 | 0.78x | 0.82x | 0.86x | 0.28x |
| name_indirection | 921.0 | 1287.1 | 1332.2 | 0.72x | 0.69x | 0.72x | — |
| op_dispatch | 51.8 | 15.6 | 14.8 | 3.32x | 3.50x | 3.96x | 3.73x |
| pattern_bt | 213.9 | 188.9 | 163.6 | 1.13x | 1.31x | 1.35x | 1.47x |
| roman | 152.2 | 183.2 | 166.9 | 0.83x | 0.91x | 0.94x | 0.78x |
| string_concat | 47.4 | 41.4 | 43.0 | 1.15x | 1.10x | 0.87x | 0.65x |
| string_concat_twin | 202.7 | 157.4 | 169.4 | 1.29x | 1.20x | 1.29x | 0.98x |
| string_manip | 61.3 | 59.3 | 59.0 | 1.03x | 1.04x | 1.07x | 0.66x |
| string_pattern | 65.5 | 74.6 | 74.5 | 0.88x | 0.88x | 0.89x | 0.92x |
| table_access | 746.9 | 571.8 | 547.2 | 1.31x | 1.37x | 1.28x | 1.29x |
| table_variety | 560.0 | 524.0 | 517.6 | 1.07x | 1.08x | 1.06x | 0.68x |
| test_icon | 1.0 | 0.3 | 0.2 | 3.67x | 4.08x | 4.06x | 4.12x |
| var_access | 57.1 | 16.3 | 15.1 | 3.50x | 3.79x | 3.63x | 4.25x |

Geometric mean over the 23 kernels: **1.57x** in mode 3, **1.59x** in mode 4; SCRIP is ahead of SPITBOL on 17 of 23
kernels (2026-09-25 13:2x on 3a936a797: 1.55x / 1.57x, 16 of 23; 09:2x on 80ee6bb5c: 1.16x / 1.26x; 03:2x on 82b4a61a8: 1.13x / 1.23x; 02:5x on bf9613327: 1.14x / 1.23x; 02:2x on 40a7bac3c: 1.12x / 1.21x; 00:4x on 6134f316c: 1.04x / 1.14x; 2026-09-24 22:04 on 74729b18b: 0.94x / 1.00x; 20:16 that day, on 73295fba6: 0.78x / 0.84x; 17:2x, before the by-name landing: 0.70x / 0.75x; that morning
0.19x; the 09-03 tree `380cc4162`: 1.25x / 1.33x). Three landings lie between the two grids, and the moves are attributed by
mechanism, not by a grid per landing. SPITBOL's defaults (`ddfa57a55`, CEO-1261/1262): the window is SPITBOL's -i1m where it was
128 KB, and the benchmarks run with the collector's self-checks and the node-id stores off — table_access 0.36x → 1.28x, table_variety 0.46x → 1.06x, mixed_workload 0.44x → 0.86x, string_manip 0.64x →
1.07x, roman 0.67x → 0.94x, array_sum 1.50x → 2.18x. The by-name paths (`d566528fa`, CEO-1263): `$name` takes the direct table and a
length-bounded lookup and hands a plain variable back as its cell, APPLY finds its procedure once, an EVAL of an expression string
skips the number parses — name_indirection 0.42x → 0.72x, indirect_dispatch 0.47x → 0.57x, eval_fixed 0.65x → 0.70x. The
collector's pacing (`3a936a797`, CEO-1264) moved no kernel; it moved treebank (below). The program's own huge-page stack
(`be0e10b9a`, CEO-1265) moved the demos that keep deep backtracking state (below); the kernels keep shallow stacks, and their small
moves between the two grids are within the SPITBOL arm's own run-to-run spread (its string_concat twin read 34.3 µs, then 47.4). Where SCRIP still trails, the operation is reached
BY NAME at run time (eval_fixed, indirect_dispatch, name_indirection) or is a C string or pattern primitive at -O0 (string_concat
0.87x, string_pattern 0.89x, roman 0.94x, mixed_workload 0.86x); wherever the compiler resolves the operation it runs 2–4x
SPITBOL (arith_loop 4.03x, op_dispatch 3.96x, var_access 3.63x, ident_call2 2.92x, array_sum 2.18x, func_call 2.04x).

**The workhorse demos on their large inputs, whole program, best of 5, mode 4, tree `be0e10b9a`, 2026-09-25 14:4x CDT, load 1.3,
`-d512m -i64m -s256m` given to BOTH engines (`sbl -bf` and the mode-4 binary; RULES.md hard-cap rule clause 8(e)), SCRIP with
`SCRIP_DIAG=0`, outputs byte-identical on both engines:**

| demo | input | SPITBOL ms | m4 ms | m4 |
|---|---:|---:|---:|:---:|
| claws5 (CLAWS5inTASA.dat ×16) | 1,043 KB | 69 | 49 | 1.41x |
| treebank (treebank.input ×1024) | 327 KB | 113 | 160 | 0.71x |
| json (citm_catalog.json) | 1,686 KB | 35 | 42 | 0.83x |
| porter (porter.input ×4) | 742 KB | 216 | 272 | 0.79x |
| calculator-1 (calculator.input ×4) | 127 KB | 29 | 33 | 0.88x |
| calculator-2 (calculator.input ×4) | 127 KB | 32 | 35 | 0.91x |

On `3a936a797` at 13:2x the same six read 1.06x, 0.66x, 0.81x, 0.82x, 0.47x and 0.56x. The landing between, `be0e10b9a` (CEO-1265), runs
...
4 KB page at a time and cannot take a huge page: the calculator's whole-file match keeps every deferred-pattern activation for
backtracking and touched 48 MB of stack in 12,312 page faults, two thirds of its run (58 ms → 31 ms, 13,675 faults → 1,389).
On `d566528fa` at 12:3x, the same switches, the same six read 1.12x, **0.41x**, 0.81x, 0.81x, 0.48x and 0.54x: with `-i64m` SCRIP
collected treebank once at HALF its window, where SPITBOL (`sbl -x`: 0 regenerations, 45 MB used) never collected, and that one
collection cost 110 ms of the run; the pacing landing `3a936a797` spends a window before collecting it and grows one a collection
leaves under 15 percent free, SPITBOL's own rule, and treebank collects zero times. On `80ee6bb5c` at 09:2x, SCRIP given a 512 MB
window (`SCRIP_HEAP_KB=524288`) against SPITBOL's `-i64m`, the six read 1.06x, 0.64x, 0.75x, 0.80x, 0.46x and 0.57x.
On 82b4a61a8 at 03:2x the same six read 1.00x, 0.64x, 0.74x, 0.61x, 0.47x and 0.55x; the capture landing of 80ee6bb5c (every
capture a slice of the subject, nothing allocated; TABLE, CONVERT and SNO$MKPAT on the direct table) moved porter 0.61x → 0.80x.
On bf9613327 at 02:5x the same six read 1.04x, 0.51x, 0.67x, 0.63x, 0.47x and 0.54x; the DATA landing of 82b4a61a8 (a field read
takes one cached lookup, a field used as a variable is its cell, a constructor site constructs directly, PROTOTYPE and ARRAY take
the direct table) moved treebank 0.51x → 0.64x and nothing else. On 40a7bac3c at 02:2x the same six read 1.05x, 0.48x, 0.69x, 0.61x, 0.45x and 0.51x; the landing of bf9613327 (a deferred-call
...
f8c25ae0e, 0.86x, 0.08x, 0.58x, 0.41x, 0.40x and 0.43x. The night's four landings (CEO-1250..1253) moved the pattern-heavy demos:
the immediate assignment stores into its GVA cell, every poll site tests the pending flag in line, IDENT and the fused table
store are asm entries, the stale-read trap is released in 256 KB chunks. What holds the rest back is measured, not guessed:
at this 512 MB arena the calculator spends more cycles in the kernel (158 M, 13,690 first-touch page faults over 72 MB of RSS)
than in user code (149 M), because every deferred function call inside its whole-file match keeps its frame on the spine
until the match ends (row snobol4-a-deferred-function-call-inside-a-pattern-keeps-its-frame-on-the-spine-until-the-match-ends);
SPITBOL runs the same program in 22 MB. A smaller pacing line does not help: a collection here costs 10–16 ms per 32 MB of
garbage, and every line from 4 to 64 MB measured slower than 128.
...
twin. The rest is a creep across 09-05 → 09-24 with no single step. Four landings the same day: `2a81a02db` makes the statement
instrumentation the `--stlimit` switch, off by default and never inferred from the source (an `EVAL` or `CODE` can introduce a
keyword behind the compiler's back), with the correctness graders passing it because the oracle always has the feature;
`d752e30eb` puts the variable tap under the same switch (`--monitor` and `--trace` imply it); and `ea0c115b5` puts every
remaining monitor hook — the DEFINE call/return/fail hooks, the Icon procedure return/fail taps, suspend/resume and
generator-return hooks, the statement mark's &STCOUNT count — behind it too, for every language (Icon's `&trace` rides the same
hooks, so the Icon graders pass the switch as well; the Icon call tap alone stays on by default because `display()` reads the
activation record it writes, which is hq_icon's row to inline); and `444e9893d` makes every runtime trace entry point exit
before its table lookup when no trace is armed — `trace_find`, a linear scan of the TRACE set on every call and return from the
runtime's own call path, was 76% of indirect_dispatch's instructions (9.5 G → 2.2 G; 09-03 1.4 G). What remains over 09-03 on
the call kernels is the collector's 09-19/21 call contract (a runtime-made call opens in C and lands in emitted code: about
1,100 instructions per call against 695), and on the tight loops the two statement marks' GOT stores. corpus `723e5aa08` drops the
