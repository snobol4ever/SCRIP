...
implementation's own published test suite, vendored unmodified. Speed is an x-factor
that always carries its direction: `1.5x faster` means SCRIP took 1.5 times less time than
the reference on that program, `6.3x slower` 6.3 times more (Lon 2026-09-24); every number
carries its date, and every grid names the script that produced it. The suite table:

<!-- SUITE-TABLE:BEGIN generated by .github/scripts/util_suite_banner.py --readme from .github/SUITES.tsv at .github@97d6c1d7 -- do not edit by hand; scripts/test_gate_readme_suite_table_matches_suites_tsv.sh holds it -->
...
| SNOBOL4 | SnoM (master) | **1971/1988** | `2026-09-24` |
...
| workload | × vs SPITBOL |
|---|:---:|
| claws5 — the 3-level table gauntlet, real 66 KB corpus input | 1.4x slower |
| json — the deserializer, real 631 KB document | 2.4x slower |
| claws5 grammar only (pattern engine alone, zero captures) | **1.6x faster** |
...
and beats SPITBOL (claws5 grammar-only at **1.6x faster**; the verified pattern_bt kernel
at **1.4x faster**). Real full programs — the same engine plus captures, tables, and the
runtime services they call out to — still trail (claws5 1.4x slower, json 2.4x slower). The
...
| arm | m3 | m4 | | arm | m3 | m4 |
|---|:---:|:---:|---|---|:---:|:---:|
| claws5 | 3.0x slower | 1.4x slower | | calculator-1 | 1.2x slower | **2.1x faster** |
| claws5-match | 2.7x slower | 1.4x slower | | calculator-1-match | **3.1x faster** | **7.4x faster** |
| claws5-match-fence | 2.6x slower | 1.5x slower | | calculator-1-match-fence | **1.8x faster** | **3.7x faster** |
| json | 4.0x slower | 1.1x slower | | calculator-2 | 1.1x slower | **2.6x faster** |
| json-match | 3.4x slower | 1.4x slower | | calculator-2-match | 2.4x slower | 1.2x slower |
| json-match-fence | 2.9x slower | 1.2x slower | | calculator-2-match-fence | 2.1x slower | 1.1x slower |
| treebank | 7.1x slower | 2.0x slower | | | | |
| treebank-match | 2.1x slower | 1.4x slower | | | | |
| treebank-match-fence | 3.0x slower | 1.8x slower | | | | |

The reading: pure-match arms can crush (calculator-1-match at 7.4x faster), json is within 8%
of SPITBOL on totals, and the worst full program (treebank, allocation-heavy at 2.0x slower)
...
618-line source): **5.6x slower** (19.0 ms vs 3.4 ms per run, re-measured on the current
...
|---|:---:|---:|---:|---:|---:|:---:|:---:|
| `concord` | **PASS** | 30 | 210 | 90 | 90 | QNT:3.0x slower | QNT:1.3x faster |
| `deal` | **PASS** | 20 | 220 | 400 | 400 | QNT:20.0x slower | QNT:3.1x slower |
| `ipxref` | **PASS** | 20 | 230 | 90 | 60 | QNT:3.0x slower | QNT:2.3x faster |
| `queens` | **PASS** | 40 | 170 | 40 | 20 | QNT:2.0x faster | QNT:TOTAL:8.5x faster |
| `rsg` | **PASS** | 10 | 180 | 60 | 30 | QNT:3.0x slower | QNT:TOTAL:6.0x faster |
...
`2.0x faster` really spans roughly 1.5x faster–3x. The figure is real; the precision it *looks*
...
5.6x faster … concat_intvar 1.4x slower), which was whole-program wall clock, carried no overhead term, and
...
|---|:---:|---:|---:|:---:|---:|---:|---:|:---:|
| `concat_dispatch` | **PASS** | 1,606,391,766 | 70,952,791 | **23.6x faster** | 110 | 150 | 10 | 8.0x faster |
| `int_loop` | **PASS** | 1,500,321,544 | 168,953,120 | **9.0x faster** | 90 | 140 | 10 | TOTAL:14.0x faster |
| `mod_isolate` | **PASS** | 1,566,321,374 | 234,953,641 | **6.7x faster** | 110 | 170 | 20 | 5.0x faster |
| `list_dispatch` | **PASS** | 2,634,322,381 | 744,967,306 | **3.5x faster** | 160 | 180 | 60 | 1.8x faster |
| `concat_strvar` | **PASS** | 1,680,393,558 | 594,959,443 | **2.8x faster** | 150 | 240 | 70 | 2.4x faster |
| `table_miss_dispatch` | **PASS** | 1,846,547,237 | 966,966,098 | **1.9x faster** | 110 | 150 | 80 | 1.0x faster |
| `concat_int_dispatch` | **PASS** | 2,313,225,976 | 2,127,396,785 | **1.1x faster** | 130 | 170 | 140 | 1.4x slower |
| `table_miss_semantics` | **PASS** | 27,135,062 | 27,562,633 | **1.1x faster** | 0 | 130 | 0 | <1 tick |
| `concat_intvar` | **PASS** | 2,006,402,358 | 2,106,963,216 | **1.0x slower** | 150 | 260 | 190 | 1.0x faster |
| `concat_table` | **PASS** | 4,385,992,311 | *refused* | — | 290 | 130 | 220 | TOTAL:1.7x slower |

**Nine of the ten carry an `Ir` multiple and eight of those nine are above `1.00x`**, from
`23.6x faster` on `concat_dispatch` down to `1.0x slower` on `concat_intvar` — the one kernel where Arizona
still does less work than we do. `concat_int_dispatch` (`1.1x faster`) and `concat_intvar` (`1.0x slower`)
...
|---|:---:|:---:|
| `interfacegen` | 6.3x slower | **1.5x faster** |
| `jlink` | 47.6x slower | 1.5x slower |

⚠️ These are single-run totals on 5–8 ms programs and they move: an immediately
preceding run of the same script put `interfacegen` m4 at 1.3x faster and `jlink` m4 at
1.3x slower. Read them as the ~±20% band that whole-program totals on millisecond programs
...
of SWI-Prolog on all ten kernels (up to 3.9x faster). On work time it is ahead on **one**.
...
|---|:---:|:---:|---|:---:|:---:|
| tak | 20072 | **1.5x faster** | fib | 5195 | 2.6x slower |
| sendmore | 9278 | 1.8x slower | ops8 | 112 | 25.0x slower |
| cal | 29 | 2.1x slower | log10 | 73 | 25.0x slower |
| deriv | 125 | 25.0x slower | times10 | 207 | 33.3x slower |
| divide10 | 141 | 33.3x slower | derive | 269 | 33.3x slower |
...
never mixed into a WORK column. Each cell is the x-factor with its direction (SCRIP against Rakudo). All three angles run; the cross-proof column is angle 1
(live fixed-time search) against angle 2 (committed fixed N):

| kernel | SCRIP m3 WORK | SCRIP m4 WORK | Rakudo WORK | × (m3) | × (m4) | cross-proof |
|---|---:|---:|---:|:---:|:---:|:---|
| string-escape | 65 µs | 71 µs | 1820 µs | **28.0x faster** | **25.6x faster** | m3/m4 AGREE, Rakudo DISAGREE |
| point_class_add1 | 10.19 s | 8.93 s | 5.70 s | 1.8x slower | 1.6x slower | ✅ all three AGREE |
| point_class_add | 9.57 s | 8.07 s | 1.19 s | 8.0x slower | 6.8x slower | m3/m4 AGREE, Rakudo DISAGREE |
...
method-heavy work is well behind (`point_class_add` 8.0x slower/6.8x slower). Both were always
...
⛔ **RETRACTED 2026-09-16 (ceo, CEO-798, on hq_pascal's CEO-782 report):** the four cells that stood here (queens 1.0x slower/1.1x faster, quick 1.0x slower/1.0x faster, sieve 1.0x slower/1.1x slower, towers 1.0x slower/1.0x slower) were **the instrument's angle-2-over-angle-1 SELF-AGREEMENT ratio, not a multiple against fpc** — eight of eight cells equal, to two decimals, that column of `triangulation-20260904T021323Z.tsv`, whose true WORK-basis multiples read 1481.5x slower to 30.3x slower, and the same instrument on the same kernels reads the same three-orders-from-parity figure on the 09-04 engine rebuilt and on HEAD (hq_pascal, 2026-09-16, both loads stamped). The grid is a ceo signature and the signature is withdrawn: **no Pascal speed number is published until `bench_triangulate_pascal.sh` exits 0 with every angle agreeing and the ceo signs the run**. Speed is hq_pascal's third axis under DECTET, after completeness.
...
|---|---|---:|---:|:---:|
| fib_recur | naive recursive `fib(24)` | 24,131,699 | 47,007,649 | **1.9x faster** |
| arith_loop | 300,000-iteration accumulator | 36,628,696 | 77,206,594 | **2.1x faster** |
| string_concat | 4,000× `&&` concatenation | 3,776,098 | 9,123,253 | **2.4x faster** |

⭐ **Separating startup moved every row, and `string_concat` most of all — 1.4x faster on totals to
2.4x faster on work.** SCRIP's process startup is ~13x SPITBOL's in instructions (2.79M vs 0.21M),
