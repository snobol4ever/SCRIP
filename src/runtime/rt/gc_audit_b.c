...
long gc_audit_b_collect(const gc_audit_b_t *v)
{
    ...
    if (v->run <= 1) fprintf(stderr, "[GC-AUDIT-B] PASS B IS AN AUDITOR AND NEVER A COLLECTOR: pass A (the exact typed walk) has already decided this collection and pass B cannot mark, forward, free or influence it -- it is handed a const block lookup and nothing else. IT NAMES CANDIDATES, NEVER DEFECTS: a dead word that looks like a heap pointer reads identically to a live root, so every line below is a CANDIDATE to be cured or declared. WHAT IT CANNOT SEE, DECLARED: a pointer that at the instant of collection lives only in a register, or on the C stack above the emitted-stack ceiling that SCRIP_GC_CEILING draws (that ground is 98.4%% of mode 3's stack and is dead compiler frames by measurement, CEO-1030). SUCCESS IS SILENCE, and the findings=0 line is PRINTED so its silence is a statement rather than an absence.\n");
