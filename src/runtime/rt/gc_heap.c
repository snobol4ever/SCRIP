...
static long gc_collect_ex(int cons_stack)
{
    extern void core_gc_roots(void); extern void gen_gc_roots(void); extern void pas_gc_roots(void) __attribute__((weak)); extern void rt_gc_root_args(void); extern void rt_gc_ws_roots(void); extern int rt_scan_active(void);
    ...
    core_gc_roots(); gen_gc_roots(); if (pas_gc_roots) pas_gc_roots(); rt_gc_root_args();
