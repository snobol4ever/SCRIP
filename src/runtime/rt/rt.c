...
DESCR_t *gva_register(const char **names, DESCR_t *cells, int n) {
    ...
    { static int _b1cg = -1; if (_b1cg < 0) { const char *_e = getenv("SCRIP_B1C_PARITY"); _b1cg = (_e && *_e == '0') ? 0 : 1; }
      ...
      if (_b1cg && n > 0 && gva_count() == 0) { for (int k = 0; k < n; k++) if (names && names[k]) (void)gva_collect_var(names[k]); g_gva_active = (gva_count() > 0) ? 1 : 0; } }   /* B1c m4 seam (s170, seat6): THE EMIT-SIDE GVA REGISTRY IS EMPTY IN THE m4 RUNTIME IMAGE -- the driver builds gva_collect's name table and g_gva_active at COMPILE time and clears the flag before exit (scrip.c:1573), so when the runtime fragment compiler re-enters the emitter for an EVAL body it asks bb_scc_probe a question the image cannot answer: g_gva_active==0 and gva_index_of()==-1 for every global.  TINY admission (bb_tiny_shim_ok) therefore REFUSES in m4 where it ADMITS in m3, and the fragment falls to the slim/legacy call path whose pushed-landing protocol is the B1c SEGV.  Rebuilt here from __gva_names, the very table the m4 preamble already hands us: gva_collect_var assigns indices sequentially and the preamble emits .Lgvan<k> in gva_name(k) order, so index k is reproduced EXACTLY as the compile-time emitter saw it.  Guard gva_count()==0 is the m4-runtime signature -- the m3 driver only reaches this call with the table already populated (scrip.c:1612), so this block is unreachable there.  DEFAULT FLIPPED ON (s170 seat6, following c6245f60 which flipped the other three SCRIP_B1C_PARITY sites): leaving this one default-OFF made the switch mean "m3 armed, m4 half-armed", the incoherent state this codebase punishes.  Measured at the flip: corpus 326/322 and crosscheck 308/306 DIVERGE=1 IDENTICAL armed vs disarmed -- zero count movement -- while the four m4 b1 witnesses go SEGV(139) -> a clean Error 22, which is c6245f60's own "crash -> better" bar. */
    return cells;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
DESCR_t rt_nret_fix_tiny(DESCR_t r, int unused_edx) { (void)unused_edx; extern int rt_cap_name_strict(void); int wn = rt_g_want_name; DESCR_t o = rt_nret_fix(r, wn); rt_g_want_name = (rt_cap_name_strict() && wn && rt_g_ret_by_name && o.v == DT_N) ? wn : 0; return o; }   /* ⭐ SN4-CAP-NAME-STRICT s178-c (FINDING s178-FW addendum, witness sk6/probe/fw follow-up): want_name RE-ARMS while a real NAME (DT_N, by_name standing) is riding UP a landing chain.  THE DEFECT: a deferred capture target with ARGS (`. *Shift('t', thx)`) lowers as an EXPR$ fragment, so the pump's ONE call has TWO landings -- the inner user fn's consult (wn=1: name preserved, want_name consumed) and the FRAGMENT's own landing (wn now 0: name DEREFFED, by_name cleared) -- and the pump read by_name=0, retreating a capture sbl accepts (measured: 17-capture beauty skeleton, end_ok then nomatch).  A compound expression (`*(F() 'x')`) breaks the DT_N chain at its operator, so its landing still clears -- the not-a-name retreat stays sbl-true.  The pump clears want_name itself after its call, so the re-arm cannot leak past the capture site; every wn==0 caller and the whole non-strict arm are byte-identical (the ternary answers 0 exactly as before). */
 ...
 * rehome vs register ABI — is untouched: only the number of crossings that feed it changes).  Guards precede every copy so a refuse (bad idx / no body / dyn) is side-effect-free per the SCC arm's
