...
/* R12-FREE-1 (Lon 2026-07-29, GOAL-ZETA-FOUR): the r12 environment seed is DELETED from the outer thunk -- the pend top is cell-resident at [RT_CAS_TOP] (pattern_match.c g_dcap_top alias), nothing reads r12, and r12 is vacated for ZC_STORAGE_FRAME_R12.  The old push r12 was ALSO the 16-byte call-alignment adjuster, so its removal is paired with an explicit sub/add rsp,8 bracket. */
 ...
 * from [RT_CAS_TOP] before entering the graph (the graph assumes r12 live on entry) and save/restore the
 * CALLER's r12 across the call (SysV callee-saved contract).  The sub/add $16 alignment is preserved.
 * 0x70000000 == RT_CAS_TOP == RT_PIN_BASE+0 (rtx_init.c _Static_assert locks this). */
