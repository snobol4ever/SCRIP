...
DESCR_t rt_proc_call_gen_h(const char *name, int nargs, void **hout)
{
    ...
    if (p->jmp_entry && p->is_generator) {
        if (p->gen_region_ft > 0 && scrip_co_current) { if (!getenv("SCRIP_N2_APPLY_NESTED_DIAG")) { fprintf(stderr, "[N2-APPLY] REFUSED: apply/value call to generator '%s' from inside a coexpression thread -- a coexpr thread's OWN stack is misaligned for glibc's SSE-using malloc-arena bootstrap (gdb-confirmed: RBP mod 16 == 8 at the crash, SIGSEGV inside alloc_new_heap's movaps on this thread's first-ever calloc, NOT a pthread_create thread-descriptor fault as earlier described; row icon-n2-apply-nested-coexpr, FINDING seat13 2026-08-29). SCRIP_N2_APPLY_NESTED_DIAG=1 bypasses this refusal for diagnosis only -- do not set it in production, the crash is real. Refusing loudly: a silent FAIL here would falsely exhaust the outer generator (wrong answers, not just a crash).\n", p->name ? p->name : "?"); abort(); } }
        uint64_t cregs[5];
