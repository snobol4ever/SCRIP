...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* Raku prints the SHORTEST decimal that round-trips back to the same double; Icon prints 10 significant digits. They are different languages' rules and rk_real_str must not borrow Icon's. */
/* Measured 2026-09-04 (hq_T): say sqrt(2) printed 1.414213562 in both modes where Rakudo prints 1.4142135623730951 -- the witness ladder__/smoke__real_num_precision_not_truncated (raku ALL entry simple_program_37) exists for exactly this and had gone red. */
/* CAUSE was a SHARED-NODE change graded on ONE frontend: a15198201 re-derived icon_real_str to 10 sig figs empirically against icont, which is CORRECT FOR ICON, and this function delegated to it, so Raku silently inherited Icon's truncation. RULES.md SHARED-NODE VERDICT SCOPE names this exact shape. */
/* The cure adds Raku's own rule and leaves icon_real_str untouched, so Icon keeps the 10 sig figs it was just measured to need -- branching on WHAT differs (the language's printing rule), never on a language name inside a shared helper. */
