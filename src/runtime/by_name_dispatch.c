...
void *rt_call_value_spine_prep(DESCR_t callee, DESCR_t *argv, int n) {
    ...
    /* An indirect call to a jmp-entry generator (a procedure VALUE, or ! apply) has no working landing anywhere today -- MEASURED, not assumed, three ways: the native spine transfer reads garbage as the N-2 region pointer at [rsp+16] (bb_call_value.cpp never learned the region protocol; witness `gen ! [10])` SIGSEGV rc=139 3/3 on a pristine build); the SAME spine transfer ALSO SIGSEGVs 3/3 with SCRIP_ICN_GENFRAME2=0, so this is not purely an N-2 regression; and forcing the "one-shot C window" fallback (rt_proc_call_gen_h's jmp_entry&&is_generator arm) SIGSEGVs 3/3 a third way because rt_genp_spine_enter (src/runtime/rt/rt.c) still delivers gamma/omega through rcx/rdx, the pre-N-1 protocol every OTHER caller left behind when THE CROSSING moved to the pushed-{gamma,omega}-pair convention (RULES.md law 0/0a). Indirect dispatch through IR_CALL_VALUE to a generator is UNRULED: FINDING-2026-08-29-ceo-n2-step3-region-resident-generator-frames-landed-and-the-old-landing-carried-three-depth-defects.md, "NOT CLAIMED / CARRIED FORWARD": "Indirect dispatch (IR_CALL_VALUE, 70 sites) stays UNRULED (hq_C's one-shape-test design)". Refuse loudly here (rt_bomb) rather than let any of the three broken landings run -- matching that same FINDING's ruling for the sibling flat_gen-host call site: "REFUSE LOUDLY under arming (rt_bomb, named message)". The rt_genp_spine_enter rcx/rdx defect is real and separate; not fixed here -- it needs its own sitting (see LEDGER). */
    { extern void rt_bomb(const char *); char b[256];
      snprintf(b, sizeof b, "rt_call_value_spine_prep: indirect call to generator '%s' (procedure value or ! apply) is not supported -- IR_CALL_VALUE has no region/wire channel for a jmp-entry generator's activation frame, and the C-window generator fallback is separately broken (rt_genp_spine_enter predates the wire-stack pushed-pair crossing); indirect dispatch to a generator is UNRULED, see FINDING-2026-08-29-ceo-n2-step3-region-resident-generator-frames-landed-and-the-old-landing-carried-three-depth-defects.md", nm);
      rt_bomb(b); }
