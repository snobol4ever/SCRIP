...
static const char *rt_mc_type_name(DESCR_t d) {
    ...
    case DT_DATA: { if (d.slen == DATA_INST_SLEN && d.u && d.u->type && d.u->type->name) return d.u->type->name; return "Any"; }
    default: return "Any";
    }
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int dop_ax(const char *op, DESCR_t *args, int nargs, DESCR_t *out, void **ball) {
    ...
    if (nargs == 1) {
        ...
        if (!strcmp(op, "trunc") || !strcmp(op, "floor") || !strcmp(op, "ceil") || !strcmp(op, "round")) {
            ...
            { double r = !strcmp(op, "trunc") ? trunc(ad) : !strcmp(op, "floor") ? floor(ad) : !strcmp(op, "ceil") ? ceil(ad) : floor(ad) + (ad - floor(ad) >= 0.5);
              if (r >= -9223372036854775808.0 && r < 9223372036854775808.0) *out = INTVAL((long long)r);
              else if (r == r && r - r == 0.0) { extern DESCR_t rt_big_from_str(const char *); char nb[400]; snprintf(nb, sizeof nb, "%.0f", r); *out = rt_big_from_str(nb); }
              else { *out = FAILDESCR; if (ball && !*ball) *ball = rt_pl_ball_eval_error("undefined", op, 1); } }
            return 1; }
...
static int pl_parse_number_radix(const char *s, DESCR_t *out) { char *e = 0; int base = 0;
    ...
    if (!base) return 0;
    { long long v; errno = 0; v = strtoll(s + 2, &e, base); if (e && e != s + 2 && !*e) {
        if (errno == ERANGE) { extern DESCR_t rt_big_from_str_base(const char *, int); DESCR_t bg = rt_big_from_str_base(s + 2, base); if (bg.v == DT_FAIL) return 0; *out = bg; return 1; }
        *out = INTVAL(v); return 1; } }
    return 0; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int pl_parse_number(const char *s, DESCR_t *out) { char *e = 0;
    ...
    { long long iv; errno = 0; iv = strtoll(s, &e, 10); if (e && e != s && !*e) {
...
static int pl_open_leaf(DESCR_t *args, int nargs, pl_tr_ctx_t *cx) {
    ...
    { int idx = fh_alloc(fp); if (idx < 0) { fclose(fp); return 0; }
      g_fh[idx].name = rt_heap_strdup_c(fn); g_fh[idx].mode = (char) fmode[0];
      { int bom_opt = -1;
...
static long sort_struct_serial(DESCR_t v) {
    ...
    if (IS_DATA_INST_fn(v) && v.u) return ((DATINST_t *)v.u)->id;
    return 0;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int sort_data_field(DESCR_t v, int field_i, DESCR_t *out) {
    if (field_i == 0 || !IS_DATA_INST_fn(v) || !v.u) return 0;
...
static int icn_argtype_gate(int bid, DESCR_t *args, int nargs, DESCR_t *out, int strict) {
    ...
    switch (bid) {
        ...
        case BID_sort:
            if (nargs >= 1 && !(icn_arg_is_list(args[0]) || args[0].v == DT_T || (IS_DATA_INST_fn(args[0]) && args[0].u && args[0].u->type))) return icn_argtype_raise(115, args[0], out);
            return 0;
...
#undef TONUM
    ...
    if ((_bid == BID_copy) && nargs == 1) {
        ...
        if (IS_DATA_INST_fn(src) && src.u && src.u->type) {
            DATBLK_t *rt = src.u->type;
            int nf = rt->nfields > 0 ? rt->nfields : 0;
            DATINST_t *nu = (DATINST_t *) rt_gcheap_alloc(HB_DINST, sizeof(DATINST_t));
            nu->type = rt;
    ...
    if ((_bid == BID_push) && nargs >= 1) {
        ...
        { DESCR_t *_a=0; int _n=0; if (rt_lv_is_list(ld,&_a,&_n)) {
            ...
            for (int _pi = 0; _pi < _nv2; _pi++) {
                ...
                _a=_nb; _n++;
                rt_lv_set_elems(ld,DATA_ELEMS(_nb)); rt_lv_set_size(ld,_n); rt_lv_set_cap(ld,_n); }
            *out = ld; return 1; } }
    ...
    if ((_bid == BID_get) && nargs == 1) {
        ...
        { DESCR_t *_a=0; int _n=0; if (rt_lv_is_list(ld,&_a,&_n)) {
            ...
            DESCR_t _r=_a[0]; rt_lv_set_elems(ld,DATA_ELEMS(_a+1)); rt_lv_set_size(ld,_n-1);
        ...
        DESCR_t ret=arr[0];
        FIELD_SET_fn(ld,"frame_elems",DATA_ELEMS(arr+1));
        FIELD_SET_fn(ld,"frame_size",INTVAL(n-1));
    ...
    if ((_bid == BID_serial) && nargs == 1) {
        if (IS_DATA_INST_fn(args[0]) && args[0].u) { *out = INTVAL(args[0].u->id); return 1; }
    ...
    if (nargs == 1) {
        ...
        if (fn[0]=='*' && fn[1]=='\0') {
            ...
            if (IS_DATA_INST_fn(a) && a.u && a.u->type) { *out=INTVAL(a.u->type->nfields); return 1; }
