...
extern int rt_icn_cset_member_n(const char *, long, int);
#include <unistd.h>
#include <sys/stat.h>
#include <setjmp.h>
...
int junction_collapse(DESCR_t scalar, DESCR_t jct, int op, int numeric) {
    ...
    while (*p == SOH) {
        ...
        if (*p == '\x03') {
            ...
            char *mb = rt_pinned_alloc(L + 1); memcpy(mb, start, L); mb[L] = '\0';
...
static void gram_set(const char *qname, const char *body, int flavor) {
    for (int i = 0; i < gram_n; i++) if (!strcmp(gram_reg[i].qname, qname)) { gram_reg[i].body = rt_pinned_strdup(body); gram_reg[i].flavor = flavor; return; }
    if (gram_n < GRAMMAR_MAX) { gram_reg[gram_n].qname = rt_pinned_strdup(qname); gram_reg[gram_n].body = rt_pinned_strdup(body); gram_reg[gram_n].flavor = flavor; gram_n++; }
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int rt_multi_meth_dispatch(const char *cname, const char *mname, DESCR_t *args, int nargs, DESCR_t *out) {
    ...
    if (win < 0) win = 0;
    int total = 1 + nm; DESCR_t *ca = rt_pinned_alloc((size_t)total * sizeof(DESCR_t));
...
static int pl_num_cmp(const char *s, DESCR_t a, DESCR_t b) {
    extern int rt_big_cmp(DESCR_t, DESCR_t); extern char *rt_big_str(DESCR_t);
    if ((a.v == DT_BIG || b.v == DT_BIG) && !IS_REAL_fn(a) && !IS_REAL_fn(b)) {
        int c = rt_big_cmp(a, b);
        if (!strcmp(s, "lt")) return c < 0; if (!strcmp(s, "gt")) return c > 0; if (!strcmp(s, "le")) return c <= 0;
        if (!strcmp(s, "ge")) return c >= 0; if (!strcmp(s, "eq")) return c == 0; if (!strcmp(s, "ne")) return c != 0;
        return 0;
    }
    double av = IS_REAL_fn(a) ? a.r : (a.v == DT_BIG ? strtod(rt_big_str(a), (char **)0) : (double)a.i);
    double bv = IS_REAL_fn(b) ? b.r : (b.v == DT_BIG ? strtod(rt_big_str(b), (char **)0) : (double)b.i);
...
static DESCR_t pl_mk_atom_dup(const char *s, size_t n) { extern int prolog_atom_intern(const char *); char *o = (char *)rt_pinned_alloc(n + 1); if (n) memcpy(o, s, n); o[n] = 0; DESCR_t d; d.v = DT_S; d.slen = (uint32_t)n; d.s = o; (void)prolog_atom_intern(o); return d; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static DESCR_t pl_sink_build(int kind, const char *s, size_t n) {
    ...
    DESCR_t *elems = (DESCR_t *)rt_pinned_alloc((n > 0 ? n : 1) * sizeof(DESCR_t));
    for (size_t i = 0; i < n; i++) {
        if (kind == 3) { elems[i].v = (DTYPE_t)DT_I; elems[i].slen = 0; elems[i].i = (unsigned char)s[i]; }
        else { char *o = (char *)rt_pinned_alloc(2); o[0] = s[i]; o[1] = 0; elems[i] = pl_mk_atom_dup(o, 1); }
    }
...
void rk_sprintf_core(const char *fmt, DESCR_t *args, int nargs, int from, char **outp, size_t *outlen) {
    size_t cap = strlen(fmt) + 64, len = 0; char *buf = (char *)rt_pinned_alloc(cap); int ai = from;
    for (const char *p = fmt; *p; ) {
        if (*p != '%') { if (len + 2 > cap) { cap = cap * 2 + 8; char *nb = (char *)rt_pinned_alloc(cap); memcpy(nb, buf, len); buf = nb; } buf[len++] = *p++; continue; }
        ...
        if (conv == 'd' || conv == 'i' || conv == 'u' || conv == 'x' || conv == 'X' || conv == 'o') {
            ...
            if (need >= (int)sizeof piece) { char *big = (char *)rt_pinned_alloc((size_t)need + 1); snprintf(big, (size_t)need + 1, cspec, sv);
                if (len + (size_t)need + 1 > cap) { cap = len + (size_t)need + 8; char *nb = (char *)rt_pinned_alloc(cap); memcpy(nb, buf, len); buf = nb; }
            ...
            int pad = (int)w - bn; if (pad > 0) { char pc = zero ? '0' : ' '; for (int k = 0; k < pad; k++) { if (len + 1 >= cap) { cap = cap * 2 + 8; char *nb = (char *)rt_pinned_alloc(cap); memcpy(nb, buf, len); buf = nb; } buf[len++] = pc; } }
            for (int k = 0; k < bn; k++) { if (len + 1 >= cap) { cap = cap * 2 + 8; char *nb = (char *)rt_pinned_alloc(cap); memcpy(nb, buf, len); buf = nb; } buf[len++] = bits[k]; } continue;
        } else { for (const char *q = start; q <= start + (p - start) - 1; q++) { if (len + 1 >= cap) { cap = cap * 2 + 8; char *nb = (char *)rt_pinned_alloc(cap); memcpy(nb, buf, len); buf = nb; } buf[len++] = *q; } continue; }
        if (done) { size_t pl = strlen(piece); if (len + pl + 1 > cap) { cap = len + pl + 8; char *nb = (char *)rt_pinned_alloc(cap); memcpy(nb, buf, len); buf = nb; } memcpy(buf + len, piece, pl); len += pl; }
    }
...
static int pl_big_unop(const char *op, DESCR_t a, DESCR_t *out) {
    ...
    if (!strcmp(op, "neg")) { *out = rt_big_neg(a); return 1; }
...
static int pl_big_binop(const char *op, DESCR_t a, DESCR_t b, DESCR_t *out, void **ball) {
    ...
    extern int rt_big_cmp(DESCR_t, DESCR_t); extern void *rt_pl_ball_eval_error(const char *, const char *, int);
    if (!strcmp(op, "add")) { *out = rt_big_add(a, b); return 1; }
    if (!strcmp(op, "sub")) { *out = rt_big_sub(a, b); return 1; }
    if (!strcmp(op, "mul")) { *out = rt_big_mul(a, b); return 1; }
    if (!strcmp(op, "min")) { *out = rt_big_cmp(a, b) <= 0 ? a : b; return 1; }
    if (!strcmp(op, "max")) { *out = rt_big_cmp(a, b) >= 0 ? a : b; return 1; }
    if (!strcmp(op, "idiv") || !strcmp(op, "divf") || !strcmp(op, "mod") || !strcmp(op, "rem")) {
        if (pl_big_sign(b) == 0) { *out = FAILDESCR; if (ball && !*ball) *ball = rt_pl_ball_eval_error("zero_divisor", !strcmp(op, "idiv") ? "//" : !strcmp(op, "divf") ? "div" : op, 2); return 1; }
        { DESCR_t q = rt_big_div(a, b), m = rt_big_mod(a, b);
          if (!strcmp(op, "idiv")) { *out = q; return 1; }
          if (!strcmp(op, "rem")) { *out = m; return 1; }
          if (pl_big_sign(m) != 0 && (pl_big_sign(m) < 0) != (pl_big_sign(b) < 0)) { q = rt_big_sub(q, INTVAL(1)); m = rt_big_add(m, b); }
          *out = !strcmp(op, "mod") ? m : q; return 1; } }
    if ((!strcmp(op, "pow") || !strcmp(op, "fpow")) && b.v == DT_I && b.i >= 0) { *out = rt_big_pow(a, b.i); return 1; }
    { extern DESCR_t rt_big_and(DESCR_t, DESCR_t); extern DESCR_t rt_big_or(DESCR_t, DESCR_t); extern DESCR_t rt_big_xor(DESCR_t, DESCR_t);
...
static int dop_ax(const char *op, DESCR_t *args, int nargs, DESCR_t *out, void **ball) {
    ...
    if (b.v != DT_I && b.v != DT_R && b.v != DT_BIG) { void *bl = (void *)0; DESCR_t ev; if (pl_ax_eval(b, &ev, &bl)) b = ev; else { if (bl && ball && !*ball) *ball = bl; *out = FAILDESCR; return 1; } }
...
static int pl_stream_resolve(DESCR_t s, int out, int textop, void **ball)
{
    ...
    *ball = (void *)0;
    if (pl_val_unbound(d)) { *ball = rt_pl_ball_instantiation(); return -1; }
    nm = pl_atom_str(d);
    if (nm) {
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int pl_stream_idx_ball(DESCR_t s, int out, int textop, pl_tr_ctx_t *cx)
{ void *b = (void *)0; int idx = pl_stream_resolve(s, out, textop, &b); if (b) cx->ball = b; return idx; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
PL_OUT_LEAF(put_code, dop_pl_put_code)
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int pl_enc_norm(const char *s, char *out, size_t n) { size_t i, k = 0;
    if (!s) return 0;
    for (i = 0; s[i]; i++) { char c = s[i]; if (c == '-' || c == '_') continue; if (c >= 'A' && c <= 'Z') c = (char)(c - 'A' + 'a'); if (k + 1 >= n) return 0; out[k++] = c; }
    out[k] = '\0'; return k != 0;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int pl_open_opts(DESCR_t *args, int nargs, int idx, pl_tr_ctx_t *cx, int *bom_opt) {
    ...
    if (pl_val_unbound(o)) { cx->ball = rt_pl_ball_instantiation(); return 0; }
    if (!pl_is_nil(o)) { cx->ball = rt_pl_ball_kind2("type_error", "list", rt_pl_deref_val(args[3])); return 0; }
    return 1;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int pl_open_leaf(DESCR_t *args, int nargs, pl_tr_ctx_t *cx) {
    extern int fh_alloc(FILE *); extern void *rt_pl_ball_instantiation(void); extern void *rt_pl_ball_kind2(const char *, const char *, DESCR_t); extern int fh_is_untranslated(int); extern void fh_set_bom(int, int);
    ...
    { int idx = fh_alloc(fp); if (idx < 0) { fclose(fp); return 0; }
      ...
      { int bom_opt = -1;
        ...
        if (g_fh[idx].type != 'b' && !fh_is_untranslated(idx)) {
            if (fmode[0] == 'r') fh_set_bom(idx, bom_opt != 0 ? pl_skip_bom(fp) : 0);
...
DESCR_t rt_pl_dop_big(DESCR_t *args, int nargs) {
    if (nargs != 1) return FAILDESCR;
...
static int pl_bagof_groups(void *acc, DESCR_t **items_out, int **ord_out, int **gs_out, int **gi_out) {
    ...
    if (n <= 0) return 0;
    it = (DESCR_t *)rt_pinned_alloc((size_t)n * sizeof(DESCR_t));
    ord = (int *)rt_pinned_alloc((size_t)n * sizeof(int));
    gs = (int *)rt_pinned_alloc((size_t)(n + 1) * sizeof(int));
    used = (int *)rt_pinned_alloc((size_t)n * sizeof(int));
    gi = (int *)rt_pinned_alloc((size_t)n * sizeof(int));
...
static int rt_pl_bagof_group_at_cell(DESCR_t *args, pl_tr_ctx_t *cx, int sorted) {
    ...
    cnt = gs[g + 1] - gs[g];
    if (cnt <= 0) return 0;
    el = (DESCR_t *)rt_pinned_alloc((size_t)cnt * sizeof(DESCR_t));
    if (!el) return 0;
...
PL_CX_LEAF_HEAD(unget_char, 1) { extern FILE *fh_cur_in_fp(void); char b[64]; const char *t;
    ok = 0; if (pl_cell_text(args[0], b, sizeof b, &t) && t && t[0]) { ungetc((unsigned char)t[0], fh_cur_in_fp()); ok = 1; } } PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(unget_code, 1) { extern FILE *fh_cur_in_fp(void); DESCR_t v = rt_pl_deref_val(args[0]);
    ok = 0; if (v.v == (DTYPE_t)DT_I && v.i >= 0) { ungetc((int)v.i, fh_cur_in_fp()); ok = 1; } } PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(unget_byte, 1) { extern FILE *fh_cur_in_fp(void); DESCR_t v = rt_pl_deref_val(args[0]);
    ok = 0; if (v.v == (DTYPE_t)DT_I && v.i >= 0 && v.i <= 255) { ungetc((int)v.i, fh_cur_in_fp()); ok = 1; } } PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
DESCR_t dop_pl_at_end_of_stream(DESCR_t *args, int nargs) { extern FILE *fh_cur_in_fp(void); int c;
    (void)args; if (nargs != 0) return FAILDESCR; c = fgetc(fh_cur_in_fp());
    if (c == EOF) return pl_ok(); ungetc(c, fh_cur_in_fp()); return FAILDESCR; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
DESCR_t dop_pl_get0(DESCR_t *args, int nargs) { (void)args; (void)nargs; return FAILDESCR; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(get_edin, 1) { extern FILE *fh_cur_in_fp(void); int c; FILE *f = fh_cur_in_fp();
    do { c = fgetc(f); } while (c != EOF && (unsigned char)c <= ' ');
    ok = plw_unify_vals(args[0], INTVAL(c == EOF ? -1LL : (long long)c), cx); } PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int pl_parse_term_text(const char *txt, DESCR_t *out, pl_vtab_t *vt, PlProgram **pg_out) {
    size_t L = strlen(txt); char *text = (char *)rt_pinned_alloc(L + 20); size_t e;
...
#define PL_OUT_CX_LEAF(nm, ar) PL_CX_LEAF_HEAD(nm##_s, ar) { extern int fh_current_output(void); extern void fh_set_output(int); int idx = pl_stream_idx_ball(args[0], 1, PL_TEXTOP_##nm, cx); ok = 0; \
    if (idx >= 0) { int sv = fh_current_output(); fh_set_output(idx); DESCR_t r = rt_pl_dop_##nm##_c(args + 1, ar - 1, cx); fh_set_output(sv); \
        ok = (r.v == (DTYPE_t)DT_I); } } PL_CX_LEAF_TAIL
#define PL_OUT_BALL_LEAF(nm, body1, ar) PL_CX_LEAF_HEAD(nm##_sb, ar) { extern int fh_current_output(void); extern void fh_set_output(int); \
    int idx = pl_stream_idx_ball(args[0], 1, 1, cx); ok = 0; \
    if (idx >= 0) { int sv = fh_current_output(); fh_set_output(idx); DESCR_t r = body1(args + 1, ar - 1); fh_set_output(sv); ok = (r.v == (DTYPE_t)DT_I); } } PL_CX_LEAF_TAIL
PL_OUT_BALL_LEAF(write, dop_write, 2)
PL_OUT_BALL_LEAF(writeq, dop_pl_writeq, 2)
PL_OUT_BALL_LEAF(write_canonical, dop_pl_write_canonical, 2)
PL_OUT_BALL_LEAF(writeln, dop_pl_writeln, 2)
PL_OUT_BALL_LEAF(nl, dop_nl, 1)
PL_OUT_BALL_LEAF(tab, dop_pl_tab, 2)
PL_OUT_BALL_LEAF(put_char, dop_pl_put_char, 2)
...
PL_CX_LEAF_HEAD(put_char_c, 1) { extern FILE *fh_cur_out_fp(void); extern void *rt_pl_ball_instantiation(void);
    extern void *rt_pl_ball_kind2(const char *, const char *, DESCR_t);
    DESCR_t v = rt_pl_deref_val(args[0]); char b[64]; const char *t; ok = 0;
    if (pl_val_unbound(v)) cx->ball = rt_pl_ball_instantiation();
    else if (!pl_cell_text(args[0], b, sizeof b, &t) || !t || !t[0]) cx->ball = rt_pl_ball_kind2("type_error", "character", v);
    else { fputs(t, fh_cur_out_fp()); ok = 1; } } PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int pl_edin_revert(int is_out) {
    extern FILE *fh_get(int); extern void fh_free(int); extern void fh_set_output(int); extern void fh_set_input(int);
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(seeing, 1) { extern int fh_current_input(void);
    ok = plw_unify_vals(args[0], (fh_current_input() == 0 || !pl_edin_in_name[0]) ? pl_mk_atom("user") : pl_mk_atom_dup(pl_edin_in_name, strlen(pl_edin_in_name)), cx); } PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(tell, 1) ok = pl_edin_switch(args, cx, 1, "w"); PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(append1, 1) ok = pl_edin_switch(args, cx, 1, "a"); PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
PL_CX_LEAF_HEAD(see, 1) ok = pl_edin_switch(args, cx, 0, "r"); PL_CX_LEAF_TAIL
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
DESCR_t dop_pl_told(DESCR_t *args, int nargs) { (void)args; if (nargs != 0) return FAILDESCR; pl_edin_revert(1); return pl_ok(); }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
DESCR_t rt_pl_dop_close_c(DESCR_t *args, int nargs, pl_tr_ctx_t *cx) {
    ...
    rt_pl_tr_gc_sync(cx->tr); return pl_ok();
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
PL_IN_LEAF(peek_code, 2)
PL_IN_LEAF(get_byte, 2)
PL_IN_LEAF(peek_byte, 2)
PL_IN_LEAF(unget_char, 2)
PL_IN_LEAF(unget_code, 2)
PL_IN_LEAF(unget_byte, 2)
...
DESCR_t rt_make_nested_agg(DESCR_t *args, int nargs) {
    if (nargs <= 0 || !args) { char *e = rt_pinned_alloc(1); e[0] = '\0'; return STRVAL(e); }
    size_t total = 0;
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    if (nargs >= 1 && (!strcmp(fn, "__rk_jct_any") || !strcmp(fn, "__rk_jct_all")
        ...
        size_t total = 2;
        char **ms = rt_pinned_alloc((size_t)nargs * sizeof(char *));
        size_t *mlen = rt_pinned_alloc((size_t)nargs * sizeof(size_t));
        for (int i = 0; i < nargs; i++) {
            char scratch[64];
            const char *cs = to_cstring(args[i], scratch, sizeof scratch);
            size_t L = strlen(cs);
            char *cp = rt_pinned_alloc(L + 1); memcpy(cp, cs, L + 1);
            ms[i] = cp; mlen[i] = L; total += 1 + L;
        }
        char *buf = rt_pinned_alloc(total + 2);
    ...
    if (!strcmp(fn, "__rk_named_call") && nargs >= 2) {
        ...
        char *hbuf = rt_pinned_alloc(hcap); size_t hp = 0; hbuf[0] = '\0';
    ...
    if (!strcmp(fn, "__rk_rep") && nargs == 2) {
        ...
        if (n < 0) n = 0;
        size_t L = strlen(src);
        char *buf = rt_pinned_alloc(L * (size_t)(n > 0 ? n : 0) + 1); size_t p = 0;
    ...
    if (!strcmp(fn, "__rk_range_arr") && nargs == 2) {
        ...
        if (hi < lo) { char *e = rt_pinned_alloc(1); e[0] = '\0'; *out = STRVAL(e); return 1; }
        long long cnt = hi - lo + 1;
        char *buf = rt_pinned_alloc((size_t)cnt * 24 + 1); size_t p = 0;
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    if (!strcmp(fn, "__rk_arr_slice") && nargs == 3) {
        ...
        if (lo < 0) lo = 0;
        if (hi < lo) { char *e = rt_pinned_alloc(1); e[0] = '\0'; *out = STRVAL(e); return 1; }
        char *buf = rt_pinned_alloc(strlen(cs) + 1); size_t p = 0; const char *seg = cs; long long k = 0; int wrote = 0;
        for (;;) {
            const char *nx = strchr(seg, SOH); size_t L = nx ? (size_t)(nx - seg) : strlen(seg);
    ...
    if (!strcmp(fn, "__rk_arr_pick") && nargs >= 2) {
        ...
        const char **els = rt_pinned_alloc((tot + 2) * sizeof(const char *)); size_t *lens = rt_pinned_alloc((tot + 2) * sizeof(size_t));
        ...
        char *buf = rt_pinned_alloc((tot + 2) * (size_t)nsel + 2); size_t p = 0;
    ...
    if (!strcmp(fn, "__rk_arr_xx") && nargs == 2) {
        ...
        if (cnt < 1) { char *e = rt_pinned_alloc(1); e[0] = '\0'; *out = STRVAL(e); return 1; }
        const char **els = rt_pinned_alloc((size_t)64 * sizeof(const char *));
        size_t *lens = rt_pinned_alloc((size_t)64 * sizeof(size_t));
        int nel = 0, cap = 64;
        const char *seg = cs;
        for (;;) {
            const char *nx = strchr(seg, SOH);
        ...
        size_t total = one * (size_t)cnt; char *buf = rt_pinned_alloc(total + 1); size_t p = 0;
    ...
    if (!strcmp(fn, "unique") && nargs >= 1) {
        ...
        for (int i = 0; i < nargs; i++) {
            ...
            for (;;) {
                ...
                if (!dup && nel < cap) { char *cp = rt_pinned_alloc(L + 1); memcpy(cp, seg, L); cp[L] = '\0'; els[nel] = cp; lens[nel] = L; nel++; }
                if (!nx) break;
                seg = nx + 1;
            }
        }
    ...
    if ((!strcmp(fn, "__rk_arr_min") || !strcmp(fn, "__rk_arr_max")) && nargs >= 1) {
        ...
        for (int i = 0; i < nargs; i++) {
            ...
            for (;;) {
                ...
                if (take) { char *cp = rt_pinned_alloc(cl + 1); memcpy(cp, eb, cl); cp[cl] = '\0'; best = cp; bestl = cl; bestn = v; best_num = isn; have = 1; }
                if (!nx) break;
                seg = nx + 1;
            }
        }
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    if (!strcmp(fn, "__rk_arr_first") && nargs >= 1) {
        ...
        char *cp = rt_pinned_alloc(L + 1); memcpy(cp, cs, L); cp[L] = '\0';
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    if (!strcmp(fn, "__rk_arr_keys") && nargs >= 1) {
        ...
        char *buf = rt_pinned_alloc((size_t)nel * 24 + 1); int p = 0;
    ...
    if (!strcmp(fn, "__rk_reduce_cat") && nargs >= 1) {
        ...
        char *buf = rt_pinned_strdup(""); size_t blen = 0; const char *seg = cs;
        for (;;) {
            const char *nx = strchr(seg, SOH); size_t L = nx ? (size_t)(nx - seg) : strlen(seg);
            char *no = rt_pinned_alloc(blen + L + 1); memcpy(no, buf, blen); memcpy(no + blen, seg, L); no[blen + L] = '\0';
            buf = no; blen += L;
            if (!nx) break; seg = nx + 1;
        }
        *out = STRVAL(buf); return 1;
    ...
    if ((!strcmp(fn, "__rk_reduce_min") || !strcmp(fn, "__rk_reduce_max")) && nargs >= 1) {
        ...
        for (;;) {
            ...
            if (take) { char *cp = rt_pinned_alloc(cl + 1); memcpy(cp, eb, cl); cp[cl] = '\0'; best = cp; bestn = v; best_num = isn; have = 1; }
            if (!nx) break; seg = nx + 1;
        }
    ...
    if (!strcmp(fn, "join") && nargs >= 1) {
        ...
        char *sepd = rt_pinned_strdup(sep ? sep : ""); size_t seplen = strlen(sepd);
        char *buf = rt_pinned_strdup(""); size_t blen = 0; int first = 1;
        for (int i = 1; i < nargs; i++) {
            char scratch[64];
            const char *cs = to_cstring(args[i], scratch, sizeof scratch);
            const char *seg = cs;
            for (;;) {
                ...
                char *no = rt_pinned_alloc(blen + add + 1);
    ...
    if (!strcmp(fn, "arr_make") && nargs == 1) {
        ...
        ARBLK_t *b = (ARBLK_t *) rt_pinned_alloc_tag(sizeof(ARBLK_t), HB_ARR); b->id = rt_agg_serial_list(); b->dumpno = rt_sno_dumpno_next(); b->lo = 0; b->hi = (int) hi;
        b->ndim = 1;
        b->ndim = 1;
        b->lo2 = 0; b->hi2 = 0; b->proto_bare = 0; b->data = (DESCR_t *) rt_pinned_alloc(sizeof(DESCR_t) * (size_t) n); for (long long k = 0; k < n; k++) b->data[k] = INTVAL(0); DESCR_t d; d.v = DT_A;
    ...
    if (!strcmp(fn, "str_substr") || (!strcmp(fn, "substr") && nargs >= 2)) {
        ...
        char *o = rt_pinned_alloc((size_t)len + 1);
    ...
    if ((!strcmp(fn, "uc") || !strcmp(fn, "str_uc")) && nargs == 1) {
        const char *s = VARVAL_fn(args[0]); if (!s) s = "";
        size_t n = strlen(s); char *o = rt_pinned_alloc(n + 1);
    ...
    if ((!strcmp(fn, "str_trim") || !strcmp(fn, "trim")) && nargs == 1) {
        ...
        char *o = rt_pinned_alloc(n + 1); memcpy(o, s, n); o[n] = '\0';
        *out = STRVAL(o); return 1;
    }
    ...
    if (!strcmp(fn, "array_sort") && nargs == 1) {
        ...
        char **elems = rt_pinned_alloc((size_t)cnt * sizeof(char*));
        int idx = 0; const char *seg = as;
        do {
            const char *nx = strchr(seg, SOH);
            size_t elen = nx ? (size_t)(nx - seg) : strlen(seg);
            char *el = rt_pinned_alloc(elen + 1); memcpy(el, seg, elen); el[elen] = '\0';
            elems[idx++] = el;
            seg = nx ? nx + 1 : NULL;
        } while (seg && idx < cnt);
        int all_int = 1;
        ...
        char *o = rt_pinned_alloc(total + 1); o[0] = '\0';
        for (int i = 0; i < cnt; i++) {
    ...
    if (!strcmp(fn, "array_reverse") && nargs == 1) {
        if (args[0].v == DT_A && args[0].arr) {
            ...
            ARBLK_t *r = (ARBLK_t *) rt_pinned_alloc_tag(sizeof(ARBLK_t), HB_ARR);
            r->id = rt_agg_serial_list(); r->dumpno = rt_sno_dumpno_next(); r->lo = b->lo; r->hi = b->hi; r->ndim = 1; r->lo2 = 0; r->hi2 = 0; r->proto_bare = 0;
            r->data = (DESCR_t *) rt_pinned_alloc(sizeof(DESCR_t) * (size_t) (n ? n : 1));
    ...
    if (!strcmp(fn, "lines") && nargs == 1) {
        ...
        if (!fp) { *out = STRVAL(rt_heap_strdup_c("")); return 1; }
        char *acc = rt_pinned_alloc(65536); acc[0] = '\0'; size_t cap = 65536, used = 0; int first = 1;
        char line[4096];
        while (fgets(line, sizeof line, fp)) {
            size_t ll = strlen(line);
            while (ll > 0 && (line[ll-1] == '\n' || line[ll-1] == '\r')) line[--ll] = '\0';
            size_t need = used + ll + 2;
            if (need > cap) { cap = need * 2; char *nb = rt_pinned_alloc(cap); memcpy(nb, acc, used); acc = nb; }
    ...
    if (!strcmp(fn, "meth_call") && nargs >= 2) {
        ...
        if (args[0].v != DT_DATA && mname0) {
            ...
            if (!is_dat_recv) {
                ...
                if (is_arrm) {
                    ...
                    int total = 1 + (nargs - 2);
                    DESCR_t *fa = rt_pinned_alloc((size_t)total * sizeof(DESCR_t));
        ...
        if (args[0].v != DT_DATA) {
            ...
            if (tname && dat_find_type(tname)) {
                ...
                if (meth_is_user_proc(tproc)) {
                    ...
                    DESCR_t *ca = rt_pinned_alloc((size_t)total * sizeof(DESCR_t));
        ...
        if (!meth_is_user_proc(procname)) {
            ...
            if (dat_handles_field(cname, mname, delegfield, sizeof delegfield)) {
                ...
                DESCR_t *fwd = rt_pinned_alloc((size_t)nargs * sizeof(DESCR_t)); fwd[0] = deleg; fwd[1] = args[1];
        ...
        int nextra = nargs - 2;
        int total = 1 + nextra;
        DESCR_t *callargs = rt_pinned_alloc((size_t)total * sizeof(DESCR_t));
        callargs[0] = args[0];
    ...
    if (!strcmp(fn, "re_match_global") && nargs == 2) {
        ...
        int slen = (int)strlen(subj);
        char *acc = rt_pinned_alloc((size_t)slen * 4 + 4); acc[0] = '\0';
        int pos = 0, count = 0;
        while (pos <= slen) {
    ...
    if (!strcmp(fn, "re_subst") && nargs == 2) {
        ...
        char *pat  = rt_pinned_alloc((size_t)plen + 1); memcpy(pat, tok, (size_t)plen); pat[plen] = '\0';
        char *repl = rt_pinned_alloc((size_t)rlen + 1); memcpy(repl, sep1 + 1, (size_t)rlen); repl[rlen] = '\0';
        int global = (*(sep2 + 1) == 'g');
        Nfa *nfa = nfa_build(pat);
        if (!nfa) { *out = args[0]; return 1; }
        int slen = (int)strlen(subj);
        char *res = rt_pinned_alloc((size_t)slen * 4 + (size_t)rlen * 8 + 4); res[0] = '\0';
        int pos = 0, did_one = 0;
        while (pos <= slen) {
            Match m; nfa_exec(nfa, subj + pos, &m);
    ...
    if (!strcmp(fn, "re_capture") && nargs == 1) {
        ...
        int len = ge - gs; char *o = rt_pinned_alloc((size_t)len + 1);
    ...
    if (!strcmp(fn, "unshift_pure") && nargs >= 2) {
        ...
        char *o = rt_pinned_alloc(al + 1 + cl + 1);
    ...
    if (!strcmp(fn, "arr_set_pure") && nargs >= 3) {
        ...
        if (!strchr(cur, SOH)) {
            ...
            if (idx == 0) { char *e0 = rt_pinned_alloc(rvl + 1); memcpy(e0, rv, rvl); e0[rvl] = '\0'; *out = STRVAL(e0); return 1; }
            if (idx < 1 || (size_t)idx > slen) { *out = FAILDESCR; return 1; }
            char *buf = rt_pinned_alloc(slen * 5 + rvl + 4);
    ...
    if (!strcmp(fn, "arr_init") && nargs == 1) {
        ...
        char *o = rt_pinned_alloc(nl + 1); memcpy(o, cur, nl); o[nl] = '\0';
        *out = STRVAL(o); return 1;
    }
...
char *script_hash_delete_str(const char *h, const char *key) {
    if (!h || !*h) return rt_pinned_strdup("");
    const char *pair_start = NULL;
    hash_find(h, key, &pair_start);
    if (!pair_start) return rt_pinned_strdup(h);
    const char *nx = strchr(pair_start, SOH);
    size_t pre = (size_t)(pair_start - h);
    size_t trim_pre = pre;
    ...
    if (!strcmp(fn, "hash_keys") && nargs >= 1) {
        ...
        while (*seg) {
            ...
            if (stx) {
                ...
                char *k = rt_pinned_alloc(klen + 1); memcpy(k, seg, klen); k[klen] = '\0';
                size_t al = strlen(acc);
                char *na = rt_pinned_alloc(al + (al ? 1 : 0) + klen + 1);
                memcpy(na, acc, al);
                if (al) na[al++] = SOH;
    ...
    if (!strcmp(fn, "hash_values") && nargs >= 1) {
        ...
        while (*seg) {
            ...
            if (stx) {
                ...
                char *v = rt_pinned_alloc(vlen + 1); memcpy(v, vstart, vlen); v[vlen] = '\0';
                size_t al = strlen(acc);
                char *na = rt_pinned_alloc(al + (al ? 1 : 0) + vlen + 1);
                memcpy(na, acc, al);
                if (al) na[al++] = SOH;
    ...
    if (!strcmp(fn, "hash_kv") && nargs >= 1) {
        ...
        while (*seg) {
            ...
            if (stx) {
                ...
                size_t vlen = nx ? (size_t)(nx - vstart) : strlen(vstart);
                size_t al = strlen(acc);
                char *na = rt_pinned_alloc(al + (al ? 1 : 0) + klen + 1 + vlen + 1);
                memcpy(na, acc, al);
                if (al) na[al++] = SOH;
...
DESCR_t rt_make_list(DESCR_t *args, int nargs) {
    ...
    DESCR_t *elems = rt_pinned_alloc((nargs>0?nargs:1)*sizeof(DESCR_t));
...
DESCR_t rt_args_list_from(char **v, int n) {
    if (n < 0 || !v) n = 0;
    DESCR_t *tmp = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t));
...
static int bn_str_anal(DESCR_t *args, int nargs, int si, const char **out_s, int *out_i, int *out_j) {
    ...
    const char *s;
    int i, j, slen; long s_auth = -1;
    if (!have_s) {
        if (!scan_subj) return 0;
        s = scan_subj;
        i = scan_pos;
        { extern long rt_scan_subj_len(void); s_auth = rt_scan_subj_len(); }
    } else {
        if (args[si].v == DT_S && args[si].s && args[si].slen != 0xFFFFFFFFu) { s = args[si].s; s_auth = (long)args[si].slen; }
        else { s = VARVAL_fn(args[si]); if (!s) s = ""; }
        i = 1;
    }
    slen = (s_auth >= 0) ? (int)s_auth : (int)strlen(s);
...
static __attribute__((noinline)) int bn_date(DESCR_t *args, int nargs, DESCR_t *out) {
    ...
    char *buf = (char *)rt_heap_alloc_c(64); strftime(buf, 64, "%m/%d/%Y %H:%M:%S", tm);
    *out = STRVAL(buf); return 1;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static __attribute__((noinline)) int bn_trim(DESCR_t *args, int nargs, DESCR_t *out) {
    ...
    char *buf = (char *)rt_heap_alloc_c(n + 1); memcpy(buf, sv, n); buf[n] = 0;
...
static __attribute__((noinline)) int bn_dupl(DESCR_t *args, int nargs, DESCR_t *out) {
    ...
    size_t sl = sv_len(args[0], sv); char *buf = (char *)rt_heap_alloc_c(sl * (size_t)k + 1);
...
static int bn_sno_name(DESCR_t *args, int nargs, DESCR_t *out)
{
    ...
    { DESCR_t d; memset(&d, 0, sizeof d); d.v = DT_N; d.slen = 0; d.s = rt_pinned_strdup(sv); *out = d; return 1; }
}
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int sortf_field_i(DESCR_t *args, int nargs) { return (nargs < 2 || args[1].v == DT_SNUL || IS_FAIL_fn(args[1])) ? 1 : (int)to_int(args[1]); }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int sort_field_slot(int field_i, int n) { int idx = field_i > 0 ? field_i - 1 : n + field_i; return (field_i == 0 || idx < 0 || idx >= n) ? -1 : idx; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int sort_data_field(DESCR_t v, int field_i, DESCR_t *out) {
    if (field_i == 0 || v.v != DT_DATA || !v.u) return 0;
    DATINST_t *ia = (DATINST_t *)v.u;
    if (!ia->type || !ia->type->name || !ia->fields) return 0;
    if (!strcmp(ia->type->name, "list")) {
        DESCR_t ea = FIELD_GET_fn(v, "frame_elems"); int n = (int)FIELD_GET_fn(v, "frame_size").i, idx = sort_field_slot(field_i, n);
        DESCR_t *arr = (ea.v == DT_DATA) ? (DESCR_t *)ea.ptr : (DESCR_t *)0;
        if (!arr || idx < 0) return 0;
        *out = arr[idx]; return 1;
    }
    { int idx = sort_field_slot(field_i, ia->type->nfields); if (idx < 0) return 0; *out = ia->fields[idx]; return 1; }
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int sort_field_cmp(DESCR_t a, DESCR_t b, int field_i) {
    int ra = sort_type_rank(a), rb = sort_type_rank(b);
    if (ra != rb) return ra < rb ? -1 : 1;
    { DESCR_t fa, fb; int ha = sort_data_field(a, field_i, &fa), hb = sort_data_field(b, field_i, &fb), c;
      if (!ha && !hb) return sort_descr_cmp(a, b);
      if (!ha) return -1;
      if (!hb) return 1;
      c = sort_descr_cmp(fa, fb);
      return c ? c : sort_descr_cmp(a, b); }
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void sort_msort_descr(DESCR_t *a, DESCR_t *tmp, int n, int field_i) {
    if (n < 2) return;
    { int m = n / 2, i = 0, j = n / 2, k = 0, t;
      sort_msort_descr(a, tmp, m, field_i); sort_msort_descr(a + m, tmp + m, n - m, field_i);
      while (i < m && j < n) { if (sort_field_cmp(a[j], a[i], field_i) < 0) tmp[k++] = a[j++]; else tmp[k++] = a[i++]; }
      while (i < m) tmp[k++] = a[i++];
      while (j < n) tmp[k++] = a[j++];
...
static int icn_argtype_gate(int bid, DESCR_t *args, int nargs, DESCR_t *out, int strict) {
    ...
    switch (bid) {
        ...
        case BID_sortf:
            ...
            return sortf_field_i(args, nargs) == 0 ? icn_argtype_raise(205, args[1], out) : 0;
        case BID_bal:
...
int try_call_builtin_by_name_bl_s(const char *fn, DESCR_t *args, int nargs, DESCR_t *out, int bidlen, int strict)
{
    ...
    if ((_bid == BID___pas_chr) && nargs == 1) {
        ...
        char *s = (char *)rt_pinned_alloc(2); s[0] = (char)(unsigned char)cv; s[1] = '\0';
    ...
    if ((_bid == BID___pas_enum_name) && nargs == 2) {
        ...
        size_t L = (size_t)(p - st); char *s = (char *)rt_pinned_alloc(L + 1); memcpy(s, st, L); s[L] = '\0';
        *out = (DESCR_t){ .v = DT_S, .s = s }; return 1;
    }
    L_bidjmp_5121: ;
    ...
    if (((_bid == BID_char) || (_bid == BID_chr)) && nargs == 1) {
        ...
        char *buf = rt_pinned_alloc(2); buf[0]=(char)(n&0xFF); buf[1]='\0';
    ...
    if ((_bid == BID_args) && nargs == 1) {
        ...
        if (IS_PROCVAL_fn(a)) {
            ...
            if (np >= 0 && a.s) { extern int rt_proc_is_variadic(const char *); if (rt_proc_is_variadic(a.s)) np = -np; }
    ...
    if ((_bid == BID_proc) && (nargs == 2 || nargs == 1)) {
        ...
        { static const char *const op2[] = { "+","-","*","/","%","^","||","|||","++","--","**","<","<=",">",">=","=","~=","<<","<<=",">>",">>=","==","~==","===","~===","...","[:]", 0 };
          ...
          if (tbl) for (int oi = 0; tbl[oi]; oi++) if (!strcmp(tbl[oi], pname)) { DESCR_t bv; bv.v = DT_E; bv.slen = 0xFFFFFFFEu; bv.s = rt_pinned_strdup(pname); *out = bv; return 1; } }
        *out = FAILDESCR; return 1;
    }
    ...
    if ((_bid == BID_repl) && nargs == 2) {
        ...
        int sl=icn_true_len(args[0], s); char *buf=rt_pinned_alloc(sl*n+1); buf[0]='\0';
    ...
    if ((_bid == BID_trim) && (nargs == 1 || nargs == 2)) {
        ...
        char *buf=rt_pinned_alloc(sl+1); memcpy(buf,s,sl); buf[sl]='\0';
        *out = BSTRVAL(buf, sl); return 1;
    }
    L_bidjmp_5576: ;
    if ((_bid == BID_getenv) && nargs == 1) {
        ...
        size_t vl = strlen(val); char *buf = rt_pinned_alloc(vl+1); memcpy(buf, val, vl); buf[vl] = '\0';
        *out = STRVAL(buf); return 1;
    }
    L_bidjmp_5584: ;
    ...
    if ((_bid == BID_detab) && nargs >= 1) {
        ...
        if (gap < 1) gap = 1;
        int cap = 4096; char *buf = rt_pinned_alloc(cap); int bi = 0, col = 0; int slen0 = icn_true_len(args[0], s);
        for (int i = 0; i < slen0; i++) {
            if (s[i] == '\t') {
                ...
                int sp = next - (col+1);
                while (sp-- > 0) { if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=' '; col++; }
            } else if (s[i] == '\b') {
                if (col > 0) col--;
                if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=s[i];
            } else if (s[i] == '\n' || s[i] == '\r') {
                col = 0;
                if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=s[i];
            } else {
    ...
    if ((_bid == BID_entab) && nargs >= 1) {
        ...
        int gap = (nstops >= 2) ? stops[nstops-1] - stops[nstops-2] : stops[0] - 1;
        if (gap < 1) gap = 1;
        int cap = 4096; char *buf = rt_pinned_alloc(cap); int bi = 0, col = 1;
        ...
        for (int i = 0; i < slen; ) {
            ...
            if (c == ' ') {
                ...
                if (target - col > 1) {
                    ...
                    if (do_tab) {
                        while (nt <= target) { if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]='\t'; col = nt; nt = icn_nxttab(col, stops, nstops, gap); }
                        while (col++ < target) { if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=' '; }
                    } else {
                        while (col < target) { if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=' '; col++; }
                    }
                } else {
                    if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=' ';
                }
                col = target; i = j;
            } else if (c == '\t') {
                col = icn_nxttab(col, stops, nstops, gap);
                if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]='\t'; i++;
            } else if (c == '\b') {
                if (col > 1) col--;
                if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=c; i++;
            } else if (c == '\n' || c == '\r') {
                col = 1;
                if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=c; i++;
            } else {
                if (bi>=cap-1){cap*=2;buf=rt_pinned_realloc(buf,cap);} buf[bi++]=c;
...
#undef TONUM
    ...
    if ((_bid == BID_copy) && nargs == 1) {
        ...
        if (src.v == DT_DATA) {
            ...
            if (tag.v == DT_S && tag.s && strcmp(tag.s, "list") == 0) {
                ...
                DESCR_t *new_elems = (DESCR_t *)rt_pinned_alloc((size_t)(n > 0 ? n : 1) * sizeof(DESCR_t));
        ...
        if (src.v == DT_DATA && src.u && src.u->type) {
            ...
            DATINST_t *nu = (DATINST_t *) rt_pinned_alloc_tag(sizeof(DATINST_t), HB_DINST);
            nu->type = rt;
            if (rt->serial_next < 1) rt->serial_next = 1;
            nu->id = rt->serial_next++;
            nu->dumpno = rt_sno_dumpno_next();
            nu->fields = (DESCR_t *) rt_pinned_alloc((size_t)(nf > 0 ? nf : 1) * sizeof(DESCR_t));
    ...
    if ((_bid == BID_list) && nargs >= 0) {
        ...
        DESCR_t *elems = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t));
    ...
    if ((_bid == BID_read) && nargs == 0) {
        ...
        char *r = rt_pinned_alloc(len + 1); memcpy(r, ln, len + 1); free(ln);
    ...
    if ((_bid == BID_ICN_SCAN_PUSH) && nargs == 1) {
        const char *s;
        if (IS_REAL_fn(args[0])) { char _rb[64]; icon_real_str(args[0].r,_rb,sizeof _rb); s = rt_pinned_strdup(_rb); }
        else { s = VARVAL_fn(args[0]); if (!s) s = ""; }
        scan_depth++;
        scan_subj = rt_pinned_strdup(s); scan_pos = 1;
        *out = args[0]; return 1;
    }
    L_bidjmp_5912: ;
    ...
    if ((_bid == BID_any) && nargs >= 1 && (scan_pos > 0 || nargs >= 2)) {
        ...
        if (!rt_icn_cset_member_n(cv, cvn, s[i1 - 1])) { *out = FAILDESCR; return 1; }
    ...
    if ((_bid == BID_many) && nargs >= 1 && (scan_pos > 0 || nargs >= 2)) {
        ...
        if (!bn_str_anal(args, nargs, 1, &s, &i1, &i2)) { *out = FAILDESCR; return 1; }
        int p = i1;
        while (p < i2 && rt_icn_cset_member_n(cv, cvn, s[p - 1])) p++;
    ...
    if ((_bid == BID_tab) && nargs == 1 && scan_pos > 0) {
        ...
        int len = hi - lo;
        char *buf = rt_pinned_alloc(len + 1);
    ...
    if ((_bid == BID_push) && nargs >= 1) {
        ...
        { DESCR_t *_a=0; int _n=0; if (rt_lv_is_list(ld,&_a,&_n)) {
            ...
            for (int _pi = 0; _pi < _nv2; _pi++) {
                ...
                DESCR_t *_nb=rt_pinned_alloc((_n+1)*sizeof(DESCR_t));
                _nb[0]=_vd;
        ...
        for (int _pi = 0; _pi < _nv; _pi++) {
            ...
            DESCR_t *nb=rt_pinned_alloc((n+1)*sizeof(DESCR_t));
            nb[0]=vd;
    ...
    if ((_bid == BID_put) && nargs >= 1) {
        ...
        { DESCR_t *_a=0; int _n=0; if (rt_lv_is_list(ld,&_a,&_n)) {
            ...
            for (int _pi = 0; _pi < _nv2; _pi++) {
                ...
                DESCR_t *_nb=rt_pinned_alloc(_ncap*sizeof(DESCR_t));
        ...
        for (int _pi = 0; _pi < _nv; _pi++) {
            ...
            DESCR_t *nb=rt_pinned_alloc(ncap*sizeof(DESCR_t));
    ...
    if ((_bid == BID_sort) && nargs >= 1 && args[0].v == DT_T && args[0].tbl) {
        ...
        TBPAIR_t **ent = rt_pinned_alloc((n>0?n:1)*sizeof(TBPAIR_t*));
        { int _k = 0; TBPAIR_t *e; TBL_FOREACH(tb, e) ent[_k++] = e; }
        int by_val = (i_mode % 2 == 0);
        { TBPAIR_t **_tmp = rt_pinned_alloc((n>0?n:1)*sizeof(TBPAIR_t*)); sort_msort_pairs(ent, _tmp, n, by_val); }
        extern DESCR_t rt_make_list(DESCR_t *a, int nn);
        if (tb->is_set) {
            DESCR_t *mem = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t));
            for (int _k = 0; _k < n; _k++) mem[_k] = ent[_k]->key_descr;
            *out = rt_make_list(mem, n); return 1;
        }
        if (i_mode >= 3) {
            DESCR_t *flat = rt_pinned_alloc((2*n>0?2*n:1)*sizeof(DESCR_t));
        ...
        DESCR_t *pairs = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t));
    ...
    if ((_bid == BID_sortf) && (nargs == 1 || nargs == 2) && args[0].v == DT_T && args[0].tbl && args[0].tbl->is_set) {
        ...
        { DESCR_t *_tmp = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t)); sort_msort_descr(mem, _tmp, n, sortf_field_i(args, nargs)); }
        extern DESCR_t rt_make_list(DESCR_t *a, int nn);
        *out = rt_make_list(mem, n); return 1;
    }
    if (((_bid == BID_sortf) || (_bid == BID_sort)) && (nargs == 1 || nargs == 2) && args[0].v == DT_DATA && args[0].u && sort_is_record(args[0])) {
        ...
        { DESCR_t *_tmp = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t)); sort_msort_descr(mem, _tmp, n, (_bid == BID_sortf) ? sortf_field_i(args, nargs) : 0); }
        extern DESCR_t rt_make_list(DESCR_t *a, int nn);
        *out = rt_make_list(mem, n); return 1;
    }
    if (((_bid == BID_sort)&&(nargs==1||nargs==2))||((_bid == BID_sortf)&&(nargs==1||nargs==2))) {
        DESCR_t ld = args[0];
        if (ld.v != DT_DATA) return 0;
        DESCR_t tag = FIELD_GET_fn(ld,"gen_type");
        if (!(tag.v==DT_S && tag.s && strcmp(tag.s,"list")==0)) return 0;
        ...
        DESCR_t *sorted=rt_pinned_alloc(n*sizeof(DESCR_t));
        memcpy(sorted,arr,n*sizeof(DESCR_t));
        int field_i=(_bid == BID_sortf)?sortf_field_i(args, nargs):0;
        { DESCR_t *_tmp = rt_pinned_alloc((n>0?n:1)*sizeof(DESCR_t)); sort_msort_descr(sorted, _tmp, n, field_i); }
    ...
    L_bidjmp_9997: ;
    if ((_bid == BID_errorclear) && nargs == 0) {
        extern long g_error; extern int g_icn_err_valid;
        g_error = 0; g_icn_err_valid = 0;
        *out = NULVCL; return 1;
    }
    L_bidjmp_9998: ;
    ...
    if ((_bid == BID_read) && nargs == 1) {
        ...
        char *r = rt_pinned_alloc(len + 1); memcpy(r, ln, len); r[len] = '\0'; free(ln);
        DESCR_t rs; rs.v = DT_S; rs.slen = (uint32_t)len; rs.s = r; *out = rs; return 1;
    }
    ...
    if ((_bid == BID_name) && nargs == 1) {
        DESCR_t a = args[0];
        if (a.v == DT_N && a.slen == 0 && a.s && *a.s) { *out = STRVAL(rt_heap_strdup_c(a.s)); return 1; }
        if (IS_NAMETRAP_fn(a) && a.p) {
            VCELL_t *vc = (VCELL_t *)a.p;
            if (vc->key && *vc->key) { *out = STRVAL(rt_heap_strdup_c(vc->key)); return 1; }
            if (vc->tbl) {
    ...
    if ((_bid == BID_ARRAY) && nargs >= 1) {
        ...
        if (r.v == DT_A && r.arr) ((ARBLK_t *)r.arr)->proto = rt_pinned_strdup(proto);
        *out = r; return 1;
    }
    L_bidjmp_6479: ;
    ...
    if ((_bid == BID_SNOx24MKEXPR) && nargs == 1) {
        ...
        DESCR_t xd; xd.v = DT_X; xd.slen = (uint32_t)strlen(nm); xd.s = rt_pinned_strdup(nm);
        *out = xd; return 1;
    }
    L_bidjmp_6545: ;
    ...
    if (nargs == 1) {
        ...
        if (fn[0]=='~' && fn[1]=='\0') {
            ...
            if (IS_INT_fn(a)) { char *nb=rt_pinned_alloc(32); snprintf(nb,32,"%lld",(long long)a.i); s=nb; }
            else if (IS_REAL_fn(a)) { char *nb=rt_pinned_alloc(64); icon_real_str(a.r,nb,64); s=nb; }
            ...
            char *buf=rt_pinned_alloc(256); int n=0;
        ...
        if (fn[0]=='?' && fn[1]=='\0') {
            ...
            if (s&&*s) { int n=(int)strlen(s); char *ch=rt_pinned_alloc(2); ch[0]=s[rand()%n]; ch[1]='\0'; *out=STRVAL(ch); return 1; }
            *out=FAILDESCR; return 1;
        }
    }
    if (nargs == 2) {
...
static DESCR_t sno_array_from_proto_d(const char *proto, DESCR_t init, int depth) {
    ...
    if (!a) return FAILDESCR;
    if (depth > 0) { a->dumpno = 0; rt_sno_dumpno_undo(); } else a->proto = rt_pinned_strdup(proto);
    int n = (int)(hi - lo + 1);
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void * pl_anum_atomics_in_list(DESCR_t lst) {
    extern void *rt_pl_ball_kind2(const char *, const char *, DESCR_t);
    extern void *rt_pl_ball_instantiation(void);
    DESCR_t cur = rt_pl_deref_val(lst);
    while (pl_is_cons(cur)) {
        ...
        if (!pl_is_nil(e)) {
            if (pl_iso_unbound(e)) return rt_pl_ball_instantiation();
            if (!pl_anum_is_text(e) && !pl_anum_is_num(e)) return rt_pl_ball_kind2("type_error", "atomic", e); }
        cur = rt_pl_deref_val(((DESCR_t *)cur.p)[1]); }
    if (!pl_is_nil(cur) && pl_iso_unbound(cur)) return rt_pl_ball_instantiation();
    return (void *)0;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void * pl_anum_check(const char *nm, DESCR_t *a, int n) {
    ...
    if (!strcmp(nm, "number_string") && n == 2) {
        ...
        return (void *)0; }
    if (!strcmp(nm, "atomic_list_concat") && (n == 2 || n == 3)) {
        DESCR_t l = rt_pl_deref_val(a[0]), sep = rt_pl_deref_val(a[1]), r = rt_pl_deref_val(a[n - 1]);
        if (n == 3 && !pl_iso_unbound(sep) && !pl_is_nil(sep) && !pl_anum_is_text(sep) && !pl_anum_is_num(sep)) return rt_pl_ball_kind2("type_error", "atomic", sep);
        if (!pl_iso_unbound(r) && !pl_anum_is_text(r)) return rt_pl_ball_kind2("type_error", "atom", r);
        if (n == 3 && !pl_is_nil(l) && pl_iso_unbound(l) && !pl_iso_unbound(r)) return (void *)0;
        if (n == 3 && pl_iso_unbound(sep)) return rt_pl_ball_instantiation();
        { void *b = pl_anum_atomics_in_list(l); if (b) return b; }
        return (void *)0; }
...
void * rt_pl_dop_stream_guard_c(DESCR_t *args, int nargs) {
    ...
    if (nargs != 2) return (void *)0;
    pl_atoms_ready();
    if (!pl_cell_text(args[1], db, sizeof db, &dop) || !dop || !dop[0]) dop = "output";
    pl_stream_resolve(args[0], !strcmp(dop, "output"), 0, &ball);
    return ball;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
