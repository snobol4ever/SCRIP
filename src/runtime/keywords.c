...
DESCR_t rt_keyword_read_snobol4(const char *sval) {
    ...
    if (!strcmp(lk, "user_declared_constants")) return INTVAL(kwb_own[7]);   /* ⭐ CN-4: answered HERE and not only from the block, because the block is behind SCRIP_KW_STATIC (default OFF) while the declaration must be readable in every arm. kwb_read above already answered it when the block is armed; this is the unarmed twin, and both read the SAME cell so they cannot disagree (ONE AUTHORITY, RULES.md). */
