...
_Static_assert(sizeof(ARBLK_t) == 56 && offsetof(ARBLK_t, lo) == 0 && offsetof(ARBLK_t, hi) == 4 && offsetof(ARBLK_t, ndim) == 8 && offsetof(ARBLK_t, data) == 32, "rtx_icnsub.s RTX-28 hardcodes ARBLK_t.lo/.hi/.ndim/.data; core.h drifted -- the bounds test would read the wrong ints and the cell address would be computed off the wrong member, which links fine and hands back a pointer into arbitrary memory");
_Static_assert(offsetof(DESCR_t, slen) == 4 && DATA_INST_SLEN == 0 && DATA_ELEMS_SLEN == 1, "rtx_icnsub.s hardcodes FIELD0_SLEN 4 and both DT_DATA discriminator values to tell an element vector from a record instance, and rtx_icnagg.s shifts the same field out of rsi; if slen moves or a discriminator is renumbered, those checks read the wrong bytes and the list fast path takes a DESCR_t array for a DATINST_t -- a silent wrong answer");
