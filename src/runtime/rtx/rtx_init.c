...
_Static_assert(DT_T == 5, "rtx_icnsub.S RTX-26 table arm guards on DT_T; descr.h drifted -- the arm would admit the wrong datatype and read a bucket vector out of a non-table, which links fine and corrupts silently");
...
_Static_assert(DT_A == 4, "rtx_icnsub.S RTX-28 array arm guards on DT_A AND branches on it AHEAD of the DT_N varref gate; descr.h drifted -- the arm would admit the wrong datatype and index a non-array, which links fine and corrupts silently");
