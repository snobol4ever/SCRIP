...
_Static_assert(offsetof(TBBLK_t, buckets) == 0 && sizeof(TBBLK_t *) == 8 && offsetof(TBBUCK_t, len) == 0 && offsetof(TBBUCK_t, cap) == 4 && offsetof(TBBUCK_t, ent) == 8,
               "rtx_icnsub.S RTX-26 indexes the bucket vector as [tb + h*16] with no displacement and loads the bucket POINTER from it and reads {len,cap,ent[]} out of the block it names; core.h drifted -- every lookup would start from the wrong word.  ⛔ THE STRIDE IS 8 AND STAYED 8 across the s262 rewrite -- an intermediate cut made a bucket a 16-byte {ent,len,cap} struct and that DOUBLED TBBLK_t, which cost CLAWS5 7.4% in cycles; len/cap moved into the entry block and the vector went back to bare pointers");
_Static_assert(TABLE_BUCKETS == 256 && (TABLE_BUCKETS & (TABLE_BUCKETS - 1)) == 0,
               "rtx_table.S masks the bucket index as `and eax, TABLE_BUCKETS-1`, and TBL_BUCKET_OF in aggregates.c does the same -- both require a POWER OF TWO, and the .S bakes the literal.  "
               "⛔ 256 UNTIL s262, and the old fold was `movzx eax, al`, i.e. a mask of 255 SPELLED AS A TRUNCATION: it was silently correct for 256 and silently WRONG for every other value, which is "
               "exactly the drift this assert exists to catch.  ⭐ 256 was RE-CONFIRMED by sweep at s262 (256/128/64/32 -> 180.2M/184.5M/189.5M/200.3M Ir at fixed work), not merely inherited; if it ever moves, rtx_table.S's TBL_BUCKET_MASK literal moves with it");
