...
_Static_assert(HB_WS == 203, "rtx_alloc.s hardcodes HB_WS 203; gc_heap.h drifted -- rt_pinned_alloc would tag every workspace-island block with the WRONG block type, which links fine, allocates fine, and corrupts any code that switches on title.type silently");
