...
int main(void) {
    ...
    descr_set_src_node(&fg, DESCR_SRC_NODE_UNSTAMPED);
    if (memcmp(&fa,&fg,sizeof fa)) { fails++;
        printf("  MISMATCH faildescr asm{v=%d,src_node=%u,slen=%u,i=%lld} golden{v=%d,src_node=%u,slen=%u,i=%lld}\n",
            fa.v,descr_src_node(fa),fa.slen,(long long)fa.i, fg.v,descr_src_node(fg),fg.slen,(long long)fg.i);
