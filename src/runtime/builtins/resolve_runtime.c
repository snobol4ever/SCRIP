...
Term *resolve_unified_term_from_expr(tree_t *e, Term **env) {
    ...
    switch (e->t) {
        ...
        case TT_ADD: case TT_SUB: case TT_MUL: case TT_DIV: case TT_MOD: {
            ...
            Term *args2[2]; args2[0]=resolve_unified_term_from_expr(e->c[0],env); args2[1]=resolve_unified_term_from_expr(e->c[1],env);
            return term_new_compound(atom, 2, args2);
        }
        case TT_UNIFY: {
            int atom = prolog_atom_intern("=");
            Term *args2[2];
            args2[0] = e->n > 0 ? resolve_unified_term_from_expr(e->c[0], env) : term_new_atom(atom);
            args2[1] = e->n > 1 ? resolve_unified_term_from_expr(e->c[1], env) : term_new_atom(atom);
            return term_new_compound(atom, 2, args2);
        }
        ...
        case TT_FNC: {
            ...
            for (int i = 0; i < arity; i++) args[i] = resolve_unified_term_from_expr(e->c[i], env);
...
static Term *resolve_unified_deep_copy(Term *t) {
    ...
    if (t->tag == TERM_COMPOUND) {
        ...
        for (int i = 0; i < t->compound.arity; i++) args[i] = resolve_unified_deep_copy(t->compound.args[i]);
...
/*--------------------------------------------------------------------------------------------------------------------*/
static inline long resolve_iso_mod(long n, long d) {
    if (!d) return 0;
...
/*--------------------------------------------------------------------------------------------------------------------*/
static Term *resolve_unified_eval_arith_term(tree_t *e, Term **env) {
    if (!e) return term_new_int(0);
#define _EI(x) ({ Term *_t = resolve_unified_eval_arith_term(x,env); (_t&&_t->tag==TERM_INT)?_t->ival:(_t&&_t->tag==TERM_FLOAT)?(long)_t->fval:0L; })
#define _ED(x) ({ Term *_t = resolve_unified_eval_arith_term(x,env); (_t&&_t->tag==TERM_FLOAT)?_t->fval:(_t&&_t->tag==TERM_INT)?(double)_t->ival:0.0; })
#define _EIS(x) (resolve_unified_eval_arith_term(x,env)->tag==TERM_FLOAT)
#define _EF(op,a,b) ({ Term *_la=resolve_unified_eval_arith_term(a,env),*_lb=resolve_unified_eval_arith_term(b,env); \
    ...
    switch (e->t) {
        ...
        case TT_VAR: {
            ...
            if (!t || t->tag == TERM_VAR) { resolve_throw_instantiation_error(); return NULL; }
            return t;
        }
        case TT_ADD: {
            Term *la=resolve_unified_eval_arith_term(e->c[0],env);
            Term *lb=resolve_unified_eval_arith_term(e->c[1],env);
        ...
        case TT_MOD: return term_new_int(resolve_iso_mod(_EI(e->c[0]), _EI(e->c[1])));
        case TT_FNC: {
            ...
            if (strcmp(fn,"mod")==0&&e->n==2) return term_new_int(resolve_iso_mod(_EI(e->c[0]),_EI(e->c[1])));
            ...
            if (strcmp(fn,"max")==0&&e->n==2) {
                Term *la=resolve_unified_eval_arith_term(e->c[0],env),*lb=resolve_unified_eval_arith_term(e->c[1],env); int fl=(la&&la->tag==TERM_FLOAT)||(lb&&lb->tag==TERM_FLOAT);
            ...
            Term *t=term_deref(resolve_unified_term_from_expr(e,env));
            if (!t || t->tag == TERM_VAR) { resolve_throw_instantiation_error(); return NULL; }
            if (t->tag == TERM_INT || t->tag == TERM_FLOAT) return t;
            resolve_throw_type_error_evaluable(fn ? fn : "", e->n);
            return NULL;
        }
...
/*--------------------------------------------------------------------------------------------------------------------*/
static long resolve_unified_eval_arith(tree_t *e, Term **env) {
    Term *t = resolve_unified_eval_arith_term(e, env);
    if (!t) return 0;
...
/*--------------------------------------------------------------------------------------------------------------------*/
static Term *resolve_copy_term(Term *t) {
    CopyVarMap map[COPY_TERM_MAX_VARS];
    int nmap = 0;
    return copy_term_rec(t, map, &nmap);
}
/*--------------------------------------------------------------------------------------------------------------------*/
#define RESOLVE_SYNTH_TENV_MAX 64
static tree_t *resolve_synth_new(tree_e k) {
    tree_t *e = (tree_t *)calloc(1, sizeof(tree_t));
    e->t = k;
    return e;
}
/*--------------------------------------------------------------------------------------------------------------------*/
static void resolve_synth_add_child(tree_t *e, tree_t *child) {
    ast_push(e, child);
}
/*--------------------------------------------------------------------------------------------------------------------*/
static void resolve_synth_free(tree_t *e) {
    if (!e) return;
    for (int i = 0; i < e->n; i++) resolve_synth_free(e->c[i]);
    if (e->c) free((char *)e->c - sizeof(size_t));
    if ((e->t == TT_FNC || e->t == TT_QLIT) && e->v.sval) free(e->v.sval);
    free(e);
}
/*--------------------------------------------------------------------------------------------------------------------*/
static int resolve_tenv_add_dedup(Term **tenv, int *pn, Term *v) {
    for (int i = 0; i < *pn; i++) if (tenv[i] == v) return i;
    if (*pn >= RESOLVE_SYNTH_TENV_MAX) return -1;
    tenv[*pn] = v;
    return (*pn)++;
}
/*--------------------------------------------------------------------------------------------------------------------*/
static tree_t *resolve_term_to_synth_expr(Term *t, Term **tenv, int *pn) {
    t = term_deref(t);
    if (!t) {
        tree_t *e = resolve_synth_new(TT_FNC);
        e->v.sval = strdup("[]");
        return e;
    }
    switch (t->tag) {
        case TERM_VAR: {
            int slot = resolve_tenv_add_dedup(tenv, pn, t);
            tree_t *e = resolve_synth_new(TT_VAR);
            e->v.ival = slot >= 0 ? slot : 0;
            return e;
        }
        case TERM_INT: {
            tree_t *e = resolve_synth_new(TT_ILIT);
            e->v.ival = t->ival;
            return e;
        }
        case TERM_FLOAT: {
            tree_t *e = resolve_synth_new(TT_FLIT);
            e->v.dval = t->fval;
            return e;
        }
        case TERM_ATOM: {
        ...
        case TERM_COMPOUND: {
            ...
            if (arity == 2 && strcmp(fn, "=") == 0) {
                tree_t *e = resolve_synth_new(TT_UNIFY);
                resolve_synth_add_child(e, resolve_term_to_synth_expr(t->compound.args[0], tenv, pn));
                resolve_synth_add_child(e, resolve_term_to_synth_expr(t->compound.args[1], tenv, pn));
                return e;
            }
            if (arity == 2) {
            ...
            for (int i = 0; i < arity; i++)
                resolve_synth_add_child(e, resolve_term_to_synth_expr(t->compound.args[i], tenv, pn));
            return e;
        }
    }
    return resolve_synth_new(TT_FNC);
}
/*--------------------------------------------------------------------------------------------------------------------*/
static int resolve_invoke_var_goal(Term *gt, Term **caller_env);
int resolve_call_term(Term *gt) { return resolve_invoke_var_goal(gt, NULL); }
int resolve_call_term_n(Term *gt, int n_extra, Term **extras) {
    ...
    if (n_extra <= 0)          return resolve_invoke_var_goal(gt, NULL);
    const char *cfn = NULL; int carity_base = 0; Term **cargs_base = NULL;
    if (gt->tag == TERM_ATOM) {
        cfn = prolog_atom_name(gt->atom_id);
        carity_base = 0; cargs_base = NULL;
    } else if (gt->tag == TERM_COMPOUND) {
        cfn = prolog_atom_name(gt->compound.functor);
        carity_base = gt->compound.arity;
        cargs_base = gt->compound.args;
    } else {
        return 0;
    }
    if (!cfn) return 0;
    int total_arity = carity_base + n_extra;
    Term **rargs = (Term **)malloc((size_t)total_arity * sizeof(Term *));
    for (int i = 0; i < carity_base; i++) rargs[i] = term_deref(cargs_base[i]);
    for (int i = 0; i < n_extra;     i++) rargs[carity_base + i] = term_deref(extras[i]);
    int fn_id = prolog_atom_intern(cfn);
    Term *reconstructed = term_new_compound(fn_id, total_arity, rargs);
    free(rargs);
    return resolve_invoke_var_goal(reconstructed, NULL);
}
/*--------------------------------------------------------------------------------------------------------------------*/
static int resolve_invoke_var_goal(Term *gt, Term **caller_env) {
    (void)caller_env;
    ...
    if (gt->tag == TERM_INT)   return 0;
    if (gt->tag == TERM_FLOAT) return 0;
    Term *tenv[RESOLVE_SYNTH_TENV_MAX];
...
int interp_exec_builtin(tree_t *goal, Term **env) {
    ...
    switch (goal->t) {
        case TT_UNIFY: {
            Term *t1=resolve_unified_term_from_expr(goal->c[0],env);
            Term *t2=resolve_unified_term_from_expr(goal->c[1],env);
        ...
        case TT_FNC: {
            ...
            if (is_user_call(goal)) {
                ...
                int nslots = arity + 16;
                Term **callee_env = (Term **)calloc((size_t)nslots, sizeof(Term *));
                int mark = trail_mark(&g_resolve_trail);
                for (int ai = 0; ai < arity; ai++) {
                    Term *at = term_new_var(ai);
                    callee_env[ai] = at;
                    Term *caller_term = resolve_unified_term_from_expr(goal->c[ai], env);
            ...
            if (strcmp(fn,"halt")==0&&arity==1){Term *t=term_deref(resolve_unified_term_from_expr(goal->c[0],env));exit(t&&t->tag==TERM_INT?(int)t->ival:0);}
            ...
            if (strcmp(fn,"write")==0&&arity==1){pl_write(resolve_unified_term_from_expr(goal->c[0],env));return 1;}
            if (strcmp(fn,"writeln")==0&&arity==1){pl_write(resolve_unified_term_from_expr(goal->c[0],env));putchar('\n');return 1;}
            if (strcmp(fn,"print")==0&&arity==1){pl_write(resolve_unified_term_from_expr(goal->c[0],env));return 1;}
            if (strcmp(fn,"writeq")==0&&arity==1){pl_writeq(resolve_unified_term_from_expr(goal->c[0],env));return 1;}
            if (strcmp(fn,"write_canonical")==0&&arity==1){pl_write_canonical(resolve_unified_term_from_expr(goal->c[0],env));return 1;}
            if (strcmp(fn,"tab")==0&&arity==1){
                Term *t=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                long n=(t&&t->tag==TERM_INT)?t->ival:0;
                for(long i=0;i<n;i++) putchar(' ');
                return 1;
            }
            if (strcmp(fn,"is")==0&&arity==2){
                Term *val=resolve_unified_eval_arith_term(goal->c[1],env);
                Term *lhs=resolve_unified_term_from_expr(goal->c[0],env);
                int mark=trail_mark(trail);
            ...
            { struct{const char *n;int op;}cmps[]={{"<",0},{">",1},{"=<",2},{">=",3},{"=:=",4},{"=\\=",5},{NULL,0}};
              for(int ci=0;cmps[ci].n;ci++) if(strcmp(fn,cmps[ci].n)==0&&arity==2){
                  long a=resolve_unified_eval_arith(goal->c[0],env),b=resolve_unified_eval_arith(goal->c[1],env);
            ...
            if (strcmp(fn,"=")==0&&arity==2){
                int mark=trail_mark(trail);
                if(!unify(resolve_unified_term_from_expr(goal->c[0],env),resolve_unified_term_from_expr(goal->c[1],env),trail)){trail_unwind(trail,mark);return 0;}
                return 1;
            }
            if (strcmp(fn,"\\=")==0&&arity==2){
                int mark=trail_mark(trail);
                int ok=unify(resolve_unified_term_from_expr(goal->c[0],env),resolve_unified_term_from_expr(goal->c[1],env),trail);
                trail_unwind(trail,mark);return !ok;
            }
            if (strcmp(fn,"==")==0&&arity==2){
                Term *t1=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *t2=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                ...
                for(int i=0;i<goal->n;i++){
                    ...
                    int ok = is_user_call(g) ? ({
                        ...
                        if(ch){ int ca=g->n; Term **cargs=ca?malloc(ca*sizeof(Term*)):NULL;
                                 for(int a=0;a<ca;a++) cargs[a]=resolve_unified_term_from_expr(g->c[a],env);
            ...
            if (strcmp(fn,"call")==0 && arity>=1) {
                ...
                Term *g_term = resolve_unified_term_from_expr(g_expr, env);
                g_term = term_deref(g_term);
                if (!g_term) return 0;
                int n_extra = arity - 1;
                if (n_extra == 0) {
                    return resolve_invoke_var_goal(g_term, env);
                }
                const char *cfn = NULL;
                int carity_base = 0;
                ...
                for (int i = 0; i < n_extra; i++)
                    all_args[carity_base + i] =
                        resolve_unified_term_from_expr(goal->c[1 + i], env);
                int fn_id = prolog_atom_intern(cfn);
                ...
                free(rargs2);
                return resolve_invoke_var_goal(reconstructed, env);
            }
            if (strcmp(fn,"functor")==0&&arity==3){
                int mark=trail_mark(trail);
                if(!pl_functor(resolve_unified_term_from_expr(goal->c[0],env),resolve_unified_term_from_expr(goal->c[1],env),resolve_unified_term_from_expr(goal->c[2],env),trail)){trail_unwind(trail,mark);return 0;}
                return 1;
            }
            if (strcmp(fn,"arg")==0&&arity==3){
                int mark=trail_mark(trail);
                if(!pl_arg(resolve_unified_term_from_expr(goal->c[0],env),resolve_unified_term_from_expr(goal->c[1],env),resolve_unified_term_from_expr(goal->c[2],env),trail)){trail_unwind(trail,mark);return 0;}
                return 1;
            }
            if (strcmp(fn,"=..")==0&&arity==2){
                int mark=trail_mark(trail);
                if(!pl_univ(resolve_unified_term_from_expr(goal->c[0],env),resolve_unified_term_from_expr(goal->c[1],env),trail)){trail_unwind(trail,mark);return 0;}
                return 1;
            }
            if ((strcmp(fn,"assert")==0||strcmp(fn,"assertz")==0)&&arity==1) {
                Term *arg=resolve_unified_term_from_expr(goal->c[0],env);
                return resolve_assert_clause(arg,1);
            }
            if (strcmp(fn,"asserta")==0&&arity==1) {
                Term *arg=resolve_unified_term_from_expr(goal->c[0],env);
                return resolve_assert_clause(arg,0);
            }
            if ((strcmp(fn,"retract")==0||strcmp(fn,"retractall")==0)&&arity==1) {
                Term *arg=resolve_unified_term_from_expr(goal->c[0],env);
                return resolve_retract_clause(arg);
            }
            if (strcmp(fn,"abolish")==0&&arity==1) {
                Term *arg=resolve_unified_term_from_expr(goal->c[0],env);
                return resolve_abolish_pred(arg);
            }
            if (strcmp(fn,"atom_length")==0&&arity==2) {
                Term *a=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *l=resolve_unified_term_from_expr(goal->c[1],env);
                const char *s=NULL;
            ...
            if (strcmp(fn,"atom_concat")==0&&arity==3) {
                Term *a1=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *a2=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                Term *a3=resolve_unified_term_from_expr(goal->c[2],env);
            ...
            if (strcmp(fn,"atom_chars")==0&&arity==2) {
                Term *a=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *cl=resolve_unified_term_from_expr(goal->c[1],env);
            ...
            if (arity==2&&(strcmp(fn,"@<")==0||strcmp(fn,"@>")==0||strcmp(fn,"@=<")==0||strcmp(fn,"@>=")==0)) {
                Term *a=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *b=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
            ...
            if (strcmp(fn,"compare")==0&&arity==3) {
                Term *order=resolve_unified_term_from_expr(goal->c[0],env);
                Term *a=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                Term *b=term_deref(resolve_unified_term_from_expr(goal->c[2],env));
                int c=term_order_cmp(a,b);
            ...
            if ((strcmp(fn,"sort")==0||strcmp(fn,"msort")==0)&&arity==2) {
                ...
                Term *lst=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *out=resolve_unified_term_from_expr(goal->c[1],env);
            ...
            if (strcmp(fn,"plus")==0&&arity==3) {
                Term *a=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *b=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                Term *c=term_deref(resolve_unified_term_from_expr(goal->c[2],env));
                int mark=trail_mark(trail);
            ...
            if (strcmp(fn,"format")==0&&(arity==1||arity==2)) {
                Term *fmt_t=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                const char *fmt=NULL;
                if (fmt_t&&fmt_t->tag==TERM_ATOM) fmt=prolog_atom_name(fmt_t->atom_id);
                if (!fmt) return 0;
                Term *args_list=(arity==2)?term_deref(resolve_unified_term_from_expr(goal->c[1],env)):NULL;
            ...
            if (strcmp(fn,"numbervars")==0&&arity==3) {
                Term *t   = resolve_unified_term_from_expr(goal->c[0],env);
                Term *s   = term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                Term *end = resolve_unified_term_from_expr(goal->c[2],env);
            ...
            if (strcmp(fn,"char_type")==0&&arity==2) {
                Term *ch=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *ty=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
            ...
            if (strcmp(fn,"nv_get")==0&&arity==2) {
                Term *nm=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                ...
                Term *lhs=resolve_unified_term_from_expr(goal->c[1],env);
                int mark=trail_mark(trail);
            ...
            if (strcmp(fn,"nv_set")==0&&arity==2) {
                Term *nm=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *vl=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                if (!nm || nm->tag != TERM_ATOM) return 0;
                const char *nm_str = prolog_atom_name(nm->atom_id);
            ...
            if (strcmp(fn,"term_string")==0&&arity==2) {
                Term *ta=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *ts=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
            ...
            if (strcmp(fn,"number_codes")==0&&arity==2) {
                Term *tn=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *tc=resolve_unified_term_from_expr(goal->c[1],env);
                int nil_id=prolog_atom_intern("[]"), dot_id=prolog_atom_intern(".");
            ...
            if (strcmp(fn,"char_code")==0&&arity==2) {
                Term *tch=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *tco=term_deref(resolve_unified_term_from_expr(goal->c[1],env));
            ...
            if (strcmp(fn,"upcase_atom")==0&&arity==2) {
                Term *ta=term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *tr=resolve_unified_term_from_expr(goal->c[1],env);
            ...
            if (strcmp(fn,"sub_atom")==0&&arity==5) {
                Term *ta  = term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *tb  = resolve_unified_term_from_expr(goal->c[1],env);
                Term *tl  = resolve_unified_term_from_expr(goal->c[2],env);
                Term *taf = resolve_unified_term_from_expr(goal->c[3],env);
                Term *ts  = resolve_unified_term_from_expr(goal->c[4],env);
                if (!ta || ta->tag != TERM_ATOM) return 0;
                const char *s = prolog_atom_name(ta->atom_id); if (!s) return 0;
            ...
            if (strcmp(fn,"atom_number")==0&&arity==2) {
                Term *ta = term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *tn = resolve_unified_term_from_expr(goal->c[1],env);
            ...
            if (strcmp(fn,"atom_to_term")==0&&arity==3) {
                Term *ta   = term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *tt   = resolve_unified_term_from_expr(goal->c[1],env);
                Term *tbnd = resolve_unified_term_from_expr(goal->c[2],env);
                if (ta && ta->tag == TERM_ATOM) {
            ...
            if ((strcmp(fn,"phrase")==0) && (arity==2||arity==3)) {
                Term *rule = term_deref(resolve_unified_term_from_expr(goal->c[0], env));
                Term *s0   = resolve_unified_term_from_expr(goal->c[1], env);
                Term *s1   = (arity==3) ? resolve_unified_term_from_expr(goal->c[2], env)
                ...
                tree_t *uch = resolve_pred_table_lookup(&g_stage2.resolve_pred_table, ukey);
                if (!uch) return 0;
                Term **uargs = resolve_env_new(call_arity);
            ...
            if (strcmp(fn,"findall")==0&&arity==3){
                ...
                Trail saved_global_trail=g_resolve_trail;
                g_resolve_trail=fa_trail;
                tree_t *fa_synth = NULL; Term **fa_tenv = NULL;
                Term **outer_env = env;
                if (goal_expr && goal_expr->t == TERM_VAR) {
                    Term *gt = term_deref(resolve_unified_term_from_expr(goal_expr, env));
                    fa_tenv = (Term **)calloc(RESOLVE_SYNTH_TENV_MAX, sizeof(Term *));
                    int fa_tn = 0;
                    fa_synth = resolve_term_to_synth_expr(gt, fa_tenv, &fa_tn);
                    goal_expr = fa_synth;
                    env = fa_tenv;
                }
                ...
                free(solutions);
                Term *list_term=resolve_unified_term_from_expr(list_expr,outer_env);
            ...
            }
            {
                ...
                tree_t *uch = resolve_pred_table_lookup(&g_stage2.resolve_pred_table, ukey);
                if (uch) {
                    Term **uargs = (arity > 0) ? resolve_env_new(arity) : NULL;
                    Trail *utrail = &g_resolve_trail;
                    int umark = trail_mark(utrail);
                    int uok = 1;
                    for (int ui = 0; ui < arity && uok; ui++) {
                        Term *actual = resolve_unified_term_from_expr(goal->c[ui], env);
            ...
            if (strcmp(fn,"copy_term")==0&&arity==2) {
                Term *orig = resolve_unified_term_from_expr(goal->c[0],env);
                Term *dest = resolve_unified_term_from_expr(goal->c[1],env);
                Term *copy = resolve_copy_term(orig);
                int mark = trail_mark(trail);
            ...
            if ((strcmp(fn,"atomic_list_concat")==0||strcmp(fn,"concat_atom")==0)&&(arity==2||arity==3)) {
                ...
                if (arity==3) {
                    Term *sv = term_deref(resolve_unified_term_from_expr(goal->c[1],env));
                ...
                buf[pos]='\0';
                Term *res = term_new_atom(prolog_atom_intern(buf));
                Term *out = resolve_unified_term_from_expr(goal->c[arity==3?2:1],env);
                int mark = trail_mark(trail);
                if (!unify(out,res,trail)){trail_unwind(trail,mark);return 0;}
                return 1;
            }
            if (strcmp(fn,"string_to_atom")==0&&arity==2) {
                Term *a0 = term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *a1 = resolve_unified_term_from_expr(goal->c[1],env);
            ...
            if (strcmp(fn,"nb_setval")==0&&arity==2) {
                Term *key = term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                Term *val = resolve_unified_term_from_expr(goal->c[1],env);
                if (!key || key->tag!=TERM_ATOM) return 0;
                resolve_nb_setval(prolog_atom_name(key->atom_id), val);
                return 1;
            }
            if (strcmp(fn,"nb_getval")==0&&arity==2) {
                ...
                Term *out = resolve_unified_term_from_expr(goal->c[1],env);
                if (!key || key->tag!=TERM_ATOM) return 0;
                Term *val = resolve_nb_getval(prolog_atom_name(key->atom_id));
                if (!val) return 0;
                int mark=trail_mark(trail);
            ...
            if (strcmp(fn,"aggregate_all")==0&&arity==3) {
                Term   *tmpl_t    = term_deref(resolve_unified_term_from_expr(goal->c[0],env));
                tree_t *goal_expr = goal->c[1];
                Term   *result_out= resolve_unified_term_from_expr(goal->c[2],env);
            ...
            if (strcmp(fn,"throw")==0&&arity==1) {
                Term *exc = resolve_unified_term_from_expr(goal->c[0],env);
                g_resolve_exception = exc;
                while (g_resolve_catch_top > 0) {
                    int ci = g_resolve_catch_top - 1;
                    Resolve_CatchFrame *cf = &g_resolve_catch_stack[ci];
                    Trail tmptrail; trail_init(&tmptrail);
            ...
            if (strcmp(fn,"catch")==0&&arity==3) {
                ...
                Term   *catcher  = resolve_unified_term_from_expr(goal->c[1],env);
                tree_t *recovery = goal->c[2];
                if (g_resolve_catch_top >= RESOLVE_CATCH_STACK_MAX) return 0;
                Resolve_CatchFrame *cf = &g_resolve_catch_stack[g_resolve_catch_top];
                cf->catcher    = catcher;
                cf->env        = env;
                ...
                if (!threw) {
                    ...
                    if (goal_e && goal_e->t == TERM_VAR) {
                        ...
                        if (uch && uch->n > 0) {
                            int ua=goal_e->n;
                            Term **uenv=ua?resolve_env_new(ua):NULL;
                            for(int ui=0;ui<ua;ui++)
                                uenv[ui]=resolve_unified_term_from_expr(goal_e->c[ui],env);
                            Term **sv=g_resolve_env; g_resolve_env=uenv;
                            DESCR_t rd=FAILDESCR; g_resolve_env=sv; fprintf(stderr, "[NO-AST] interp_eval stub\n");
                            if(uenv)free(uenv);
                            ok=!IS_FAIL_fn(rd);
                        } else {
                            ok = resolve_throw_existence_error_procedure(
                                goal_e->v.sval ? goal_e->v.sval : "", goal_e->n);
                        }
                    } else {
            ...
            if (strcmp(fn,"setup_call_cleanup")==0&&arity==3) {
                ...
                if (setup_e && setup_e->t==TERM_VAR) {
                    Term *gt=term_deref(resolve_unified_term_from_expr(setup_e,env));
                    s_tenv=calloc(RESOLVE_SYNTH_TENV_MAX,sizeof(Term*)); int n=0;
                    s_synth=resolve_term_to_synth_expr(gt,s_tenv,&n); setup_e=s_synth;
                }
                if (scc_goal_e && scc_goal_e->t==TERM_VAR) {
                    Term *gt=term_deref(resolve_unified_term_from_expr(scc_goal_e,env));
                    g_tenv=calloc(RESOLVE_SYNTH_TENV_MAX,sizeof(Term*)); int n=0;
                    g_synth=resolve_term_to_synth_expr(gt,g_tenv,&n); scc_goal_e=g_synth;
                }
                if (cleanup_e && cleanup_e->t==TERM_VAR) {
                    Term *gt=term_deref(resolve_unified_term_from_expr(cleanup_e,env));
