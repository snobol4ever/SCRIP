...
_Static_assert(__builtin_offsetof(pl_trail_t, top) == 32 && sizeof(pl_area_t) == 32, "rtx_plcall.S bakes PL_TRAIL_TOP to inline rt_value_trail_mark for the RTX-1-PL open port; pl_area_t grew or shrank -- the asm would stamp the pcall record's vtmark from the WRONG WORD, which links fine and makes the epilogue landings tidy the wrong span of the value trail");
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
