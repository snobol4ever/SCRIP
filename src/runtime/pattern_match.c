...
uint64_t g_zctx[66] = {0};   /* ZCTX BASE STACK (Fable s11, Lon-in-chat: delete g_blob_ctx): per-activation replacement for the CONVICTED single global (MECH s36 DEPTH; manual Ch.18 p.204 pushdown).  [0]=depth · [1]=CURRENT blob carve base (the sole field readers touch) · [2+i]=spilled bases.  PUSH(b): spill[depth]=current; depth++; current=b.  POP: depth--; current=spill[depth].  Wires/flag/δ0 stay in each activation CARVE at [base+kt-24/16/32/40] -- the stack holds ONLY bases; every reader is one indirection, depth-correct by construction.  Capacity 64; overflow policy = M-1b. */
