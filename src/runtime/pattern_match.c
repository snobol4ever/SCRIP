...
uint64_t g_blob_ctx[5] = {0,0,0,0,0};   /* ⛔ DEL-T1 D-2 (s9): PASS-THRU blob context — the depth-immune anchor replacing the DELETED rbp pin for PAT$ blobs.  [0]=cell base (post-carve rsp at α) · [1]=γ wire · [2]=ω wire · [3]=scan flag · [4]=attempt δ0.  α writes it; scanhit/scanfail/CLASS-D γ/ω read it; the β res-stub REFILLS it from the suspended activation's carve header (the record's [+8] payload is now the CELL BASE, not rbp), which is what makes two interleaved blob activations in one statement (P1 suspend → P2 run → P1 β) come back with the right wires.  NON-NESTING assumption: T1 invariant blobs make no interior defer calls (TT_VAR members are snapshots; TT_DEFER is excluded from invariance), so a single cell suffices; variant blobs with nested defers remain the deferred class pending ZD admission (PT-4 PREREQ). */
void *rt_blob_ctx_ptr(void) { return (void *)g_blob_ctx; }   /* ZW-4 accessor precedent (rt_anchor_ptr): BINARY arms embed the returned address as movabs; TEXT arms name the symbol via GOTPCREL */
