...
static const char * sgoto(const tree_t * s, tree_e kind) {
    for (int i = 0; i < s->n; i++) {
        ...
        if (a->n > 0 && a->c[0] && a->c[0]->t == TT_INDIRECT && a->c[0]->n > 0 && a->c[0]->c[0] && a->c[0]->c[0]->t == TT_VAR && a->c[0]->c[0]->v.sval) { const char * v = a->c[0]->c[0]->v.sval; sno_reg_var(v); size_t ln = strlen(v); char * o = (char *) rt_wsb_alloc(ln + 2); o[0] = '$'; memcpy(o + 1, v, ln); o[ln + 1] = 0; return o; }
        return NULL;
    }
    return NULL;
}
...
static IR_t * sno_goto_special_chain(IR_graph_t * g, const char * dn, IR_t * tail) {
    ...
    for (int k = 0; k < 3; k++) {
        ...
        char * te = (char *) rt_wsb_alloc(dl + 3); te[0] = '^'; te[1] = sc[k]; memcpy(te + 2, dn, dl + 1);
...
static IR_t * sno_goto_computed_target(IR_graph_t * g, scx_t * cx, const tree_t * expr, IR_t * exitnd) {
    ...
    char * tmpn = lp_strdup(nmb); sno_reg_var(tmpn);
    size_t ln = strlen(tmpn); char * dn = (char *) rt_wsb_alloc(ln + 2); dn[0] = by_name ? '@' : '$'; memcpy(dn + 1, tmpn, ln); dn[ln + 1] = 0;
...
static IR_t * sno_goto_direct_target(IR_graph_t * g, scx_t * cx, const tree_t * expr, IR_t * exitnd) {
    ...
    char * tmpn = lp_strdup(nmb); sno_reg_var(tmpn);
    size_t ln = strlen(tmpn); char * dn = (char *) rt_wsb_alloc(ln + 2); dn[0] = '<'; memcpy(dn + 1, tmpn, ln); dn[ln + 1] = 0;
    IR_t * gd = lc_build(g, IR_GOTO_DEFERRED, exitnd, NULL); IR_LIT(gd).sval = dn;
...
static const char * sno_qlit_fold(const tree_t * t) {
    ...
    if ((t->t == TT_CAT || t->t == TT_SEQ) && t->n >= 2) {
        ...
        char * o = (char *) rt_wsb_alloc(la + lb + 1);
