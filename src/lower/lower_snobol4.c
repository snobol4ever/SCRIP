...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zw5_on(void) { const char * e = getenv("SCRIP_ZW5"); return (e && *e == '1') ? 1 : 0; }   /* ⛔ ZW-5 KILLSWITCH, DEFAULT OFF, AND THE POLARITY IS DELIBERATE.  The goal file writes the switch as `SCRIP_ZW5=0 reverts` (i.e. default ON); it CANNOT default on yet and the reason is a HARD CROSS-FRONT DEPENDENCY measured this session: IR_STATEMENT is in zd_k (slice 0) but is NOT in zd_wl_kind, so a minted box is a first-blocker that declines its ENTIRE run under the all-or-nothing gate -- every statement in every program would fall to the UCLAIM arm.  zd_wl_kind is ALPHA's file region under the s23p concurrency contract (item 1, ADMISSION CLUSTER) and item 9's semantic wall forbids OMEGA changing an admission verdict, so the whitelist entry is a CROSS-FRONT REQUEST, not an edit made here.  Flip this helper to `=0`-reverts polarity in the SAME commit that proves the lit rung green at the merged head.  No cached static: the s21x "process-scope flag driving a graph-scope regime" defect signature is a CACHED getenv inside a graph-taking function, and this regime is process-uniform by construction, so the read stays live and stateless rather than pretending a cache is safe. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
