...
static int sno_seq_tail(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_SEQ_TAIL"); v = (e && *e == '0') ? 0 : 1; } return v; }   /* ⭐⭐⭐ SEQ-TAIL-RESUME (s187, THE CAMPAIGN'S ONLY FALSE ACCEPT): an element's RESUME SURFACE is its RIGHTMOST box, not its first-allocated one.  res[i] fed the right neighbour's ω (SEQ-RESUME-GATE's prv) the element HEAD, so every element to the RIGHT of a MULTI-BOX element receded into that element's FIRST box and skipped the β of every interior box with it -- both the interior CHOICE POINTS and, for the class-0 leaves whose β "reverses arithmetically" (ARCH-PASSTHRU), the interior CURSOR UNDO.  Latent while every element was a single box; PAT-INLINE (Lon 2026-08-09) made a bare stored-pattern reference lower its whole tree inline as ONE element, which is exactly a multi-box element -- so `E = ARBNO('a') LEN(1)` / `POS(0) E RPOS(0)` receded past LEN's cursor undo into ARBNO's β, ARBNO resumed one char past its own yield, every retry advanced TWO characters instead of one, and 'aa+aa' MATCHED where the oracle refuses (manual p.121: ARBNO(PAT) == ( "" | PAT | PAT PAT | ... ), so instance k+1 must start where instance k started).  The same skip in the other direction is the false REJECT: `E = ANY('ab') (LEN(1) | '')` matched `*E 'b'` never retreats into the ALT's null arm because the ALT is the element TAIL.  ti = the last node whose γ exits the element rightward (the σ tag this very loop assigns) == the element's rightmost box by construction; a single-box element has ti == the head, so the whole pre-PAT-INLINE corpus is byte-identical.  SCRIP_SEQ_TAIL=0 restores the head aim verbatim. */
static int sno_alt_tail(void) { const char * e = getenv("SCRIP_ALT_TAIL"); return (e && *e == '1') ? 1 : 0; }   /* ⭐ ALT-TAIL-RESUME (s190, measured by seat4 s188/s189, row alt-tail-resume-surface): the SAME false accept sno_seq_tail() names, one construct over -- an ALTERNATIVE's RESUME SURFACE is its RIGHTMOST box, not its first-allocated one. A.β dispatches on alt_i to resume_i's β (the SN4-NARY-ALT comment above: "the alternative's OWN inner resume"), but resume_i was aimed at g->all[before], the arm HEAD; for a MULTI-BOX arm that recedes into the head and skips the β of every interior box, so an inner generator never re-yields and the right context resumes on a box that never carved. Witness crosscheck/patterns/160_pat_alt_inner_gen_resume ('aXb' ? ('a' ARB . V | 'q') 'b') is rc=139 at the head aim and rc=0 V=[X], the live oracle answer, at the tail aim. ti == the last node whose γ exits the arm rightward (the σ tag this very loop assigns) == the arm's rightmost box by construction; a single-box arm has ti == the head, so a single-box corpus is byte-identical. ⛔ DEFAULT OFF pending the blast-radius sweep + passthru board the row prices in: SCRIP_ALT_TAIL=1 arms it. */
static int sno_fence_rtail(void) { const char * e = getenv("SCRIP_FENCE_RTAIL"); return (e && *e == '0') ? 0 : 1; }   /* ⭐⭐⭐ FENCE-RTAIL (s192, row rty-fence-arbno-stored; =0 restores the s182 first-allocated scan and therefore the whole fenced arm byte-identically).  THE s182 FENCE-RESUME ARM LOOKED AT THE WRONG NODE: it inspected the blob's FIRST-ALLOCATED real body node, and sno_seq_nary allocates its elements LEFT TO RIGHT (S sentinel, then element 0, then element 1 …), so "the first real body node" is ELEMENT 0 -- the very thing sno_seq_nary's own out_rtail doc forbids ("a righter construct's exhaust must resume the run's rightmost generator, not element 0").  A blob is entered at β FROM THE RIGHT, so its resume surface is its RIGHTMOST element's carrier, which is exactly what the fence-free road already publishes (brt = out_rtail).  Witness rty_fence_arbno_stored: `expr = LEN(1) ARBNO(FENCE('+') LEN(1))` published fb=IR_MATCH_LEN (the LEADING LEN, tier 2), refused, and emitted `PAT$0_β: jmp PAT$0_ω` where the fence-free twin emits `jmp n1_match_arbno_β`.  ⛔ THE DIVERSION KEYS ON A TOP-LEVEL FENCE ELEMENT, NOT ON pfenced: sno_pat_node's TT_SEQ arm takes the identical flatten→nary route whenever no top-level element is a fence (first_fence == ne), so capturing out_rtail there is byte-identical BY CONSTRUCTION; a top-level fence keeps the FENCE-PASS-THROUGH split verbatim and publishes nothing new.  The tier gate is UNCHANGED (tier 1 or 3 only, FENCE1 still tier 0 and still refused), so the s182 narrowing and the cut semantics are untouched. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static IR_t * sno_seq_nary(scx_t * cx, const tree_t ** elems, int ne, IR_t * succ, IR_t * fail, IR_t ** out_rtail) {   /* ⭐ FENCE-PASS-THROUGH: out_rtail (nullable) receives res[ne-1], the LAST element's resume carrier — the run's EXTERNAL right-side resume surface.  Under the deleted IR_MATCH_SEQUENCE the construct's own β dispatched this internally; SEQ-ERAD removed the construct and no plain caller needed the surface until the fence pass-through seam (a righter construct's exhaust must resume the run's rightmost generator, not element 0). */
    ...
    for (int i = 0; i < ne && ne < 128; i++) {
        ...
        if (sno_seq_tail() && ti) ri = ti;   /* ⭐ SEQ-TAIL-RESUME: aim the element's resume surface at its RIGHTMOST box (the σ-tagged rightward-success exit just assigned above), not at its first-allocated one -- see sno_seq_tail(). */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int sno_kw_nest_ok(const char * nm) { static int _nn = -1; if (_nn < 0) { const char * e = getenv("SCRIP_CONST_NEST"); _nn = (e && *e == '0') ? 0 : 1; } return _nn ? !sno_kw_chase(nm, 0) : !sno_kw_chase((const char *)0, 3); }   /* ⭐⭐⭐ CN-15 -- THE SUBSTITUTION-DEPTH SELECTOR.  ⭐ FLIPPED DEFAULT ON s191 (seat6, queue row `const-nest-flip`, the span-frame/fz3-flip precedent): a keyword member INSIDE an already-substituted tree now substitutes too, and only a GENUINE self-reference refuses -- the honest cycle test (op 0) the chase stack was built for.  =0 reverts VERBATIM to the s161 TOP-LEVEL-ONLY limit (substitute only when the chase stack is EMPTY, op 3).  ⛔ THE ARMING WAS BLOCKED ON SCRIP_SPAN_FRAME, NOT ON THIS SWITCH, AND THAT BLOCKER IS GONE: lifting the limit turns an ALT arm's IR_MATCH_DEFER into a scratch-cell leaf, which re-armed the s130/s131 LEAF-SUSPENSION class; SCRIP_SPAN_FRAME went default-ON at d3251f23 and the class is cured.  RE-MEASURED at pristine 28e2122a, RT_OPT -O0, one build, arms selected by env -- SCRIP_SPAN_FRAME=0 still reproduces the SIG11 in m4 on cn_nest_alt_defer / cn_alt_leaf_flat_red / cn_alt_leaf_lit_red under this switch, so the dependency is live, not historical.  RECEIPTS: .s sweep 2 movers / 2136 comparable across EVERY frontend this lowerer serves (.sno 1648 + .sc/.reb 488; corpus/programs/lon excluded BY CONSTRUCTION), both movers in probe/cn and both a defer-to-staged-tree substitution that SHRINKS the emission (match_defer -> match_arbno / match_span, PATV$ symbols 6->0 and 12->0, everything else a renumber); m3 behavioural A/B 2360 programs, 1 real mover after subtracting a 38-row same-arm noise floor, and it is a CURE.  ⛔ THE BRIEF'S DEFAULT 527-PROGRAM SWEEP REPORTS 0 MOVERS HERE AND IS BLIND: it contains no `&`-constant program, so it cannot express a lowerer rung whose reach is a source construct -- sweep the CONSTRUCT, not the habit.  cn_const_compose_all is the correctness half: `&P2 = &A &B` answered `nomatch` at the old default and `match` here, both media, against a pinned .ref -- the manual's own rule (v3.7, Recursive Patterns: a pattern-valued variable composes BY VALUE; `*X` is the sole deferred form) says the substituted tree is the faithful reading.  ⛔ STILL RED AND OUT OF REACH OF THIS SWITCH: probe/cn/cn_const_compose_leaf (`P2 = &A &B` -- ordinary variable, constant members) is DIFF in BOTH arms; sno_kw_nest_ok gates keyword members inside a KEYWORD tree only, so that seam needs its own rung. */
...
static IR_t * sno_pat_node(scx_t * cx, const tree_t * t, IR_t * succ, IR_t * fail) {
    ...
    case TT_ALT: {
        ...
        for (int i = 0; i < na; i++) {
            ...
            if (sno_alt_tail() && ti) ri = ti;   /* ⭐ ALT-TAIL-RESUME: aim the alternative's resume surface at its RIGHTMOST box (the σ-tagged rightward-success exit just assigned above), not at its first-allocated one -- see sno_alt_tail(). */
...
static IR_t * sno_pat_carrier_build(scx_t * px, const tree_t * pat, IR_t * ok, IR_t * no, IR_t ** out_brt, int * out_pfenced) {   /* ⭐⭐⭐ THE ONE AUTHORITY for "build a stored-pattern body and capture its rightmost resume carrier" (s183, HQ Fable — extracted VERBATIM from sno_pat_thunks_build so the two roads cannot drift; the extraction is the whole point, see sno_pat_tree_graph_rt).  Half B1 of SN4-DEFER-RESUME (s121): a fence-free multi-element top is built through the IDENTICAL flatten→nary path sno_pat_node's TT_SEQ arm takes (same flatten, same args, byte-identical emission) — one level lower ONLY to capture out_rtail, the run's rightmost resume carrier (beauty's `Parse` is a 4-element SEQ; resume must land its rightmost generator, never element 0).  Fenced or single-element tops keep the sno_pat_node route verbatim. */
    ...
    { const tree_t * fel[128]; int fne = 0; sno_seq_flatten_pat(pat, fel, &fne); int topf = 0; for (int i = 0; i < fne; i++) if (sno_is_fence(fel[i])) { topf = 1; break; }   /* ⭐ FENCE-RTAIL (s192): topf mirrors the TT_SEQ arm's own `first_fence == ne` test verbatim -- it is the exact condition under which sno_pat_node ROUTES HERE ANYWAY (`return ne == 1 ? sno_pat_node(...) : sno_seq_nary(cx, elems, ne, succ, fail, NULL)`), so taking the call one level up to capture out_rtail moves ZERO bytes; a TOP-LEVEL fence element keeps the pass-through splitter untouched. */
...
static void sno_pat_publish_body_root(IR_graph_t * gp, int before_pat, const tree_t * pat, IR_t * brt, int pfenced, const char * dbgname) {   /* ⭐⭐⭐ THE ONE AUTHORITY for "which node is this blob's published resume surface" (s183, HQ Fable — extracted VERBATIM from sno_pat_thunks_build; sno_pat_tree_graph_rt spelled this fact a SECOND time and got it wrong, which was THE M1 WALL).  body_root IS the resume surface: fence-free SEQ top → nary's out_rtail (rightmost element's carrier); else the first REAL body node — "R first-allocated ⇒ R.β IS resume" per the TT_ARB/TT_ARBNO arms' own doc, and a deterministic carrier's β is a harmless fail-through, so this is uniform.  Leading argument-less GOTO relays (nested-nary sentinels) are skipped exactly as SEQ-ERAD SE-6's own warning prescribes.  right-sealed blobs keep body_root=NULL — a sealed blob exposes NO resume surface (manual Ch.19, backup through the rightmost fence must not re-enter). */
    ...
    if (sno_defer_resume() && !pfenced) {
        ...
        if (!(_fre && *_fre == '0')) { IR_t * _c = (sno_fence_rtail() && brt) ? brt : NULL; if (!_c) for (int k2 = before_pat; k2 < gp->n; k2++) { IR_t * x2 = gp->all[k2]; if (x2 && !(x2->op == IR_GOTO && x2->n_operands == 0)) { _c = x2; break; } } { int _t2 = zdp_seam_tier(_c); if (_t2 == 1 || _t2 == 3) rn = _c; } }   /* ⭐⭐⭐ FENCE-RTAIL (s192, row rty-fence-arbno-stored): the candidate is the RIGHTMOST element's carrier (brt = sno_seq_nary's out_rtail) whenever the build road could hand one over, and only falls back to the s182 first-allocated scan when it could not (single-element top, or a top-level fence keeping the pass-through splitter).  A blob's β is entered FROM THE RIGHT: publishing element 0's carrier there is the same "resume the run's rightmost generator, not element 0" error out_rtail was minted to prevent, and it is why `expr = LEN(1) ARBNO(FENCE('+') LEN(1))` refused on fb=IR_MATCH_LEN (the LEADING LEN) while its fence-free twin published the ARBNO.  The tier gate is untouched -- tier 1 or 3 only -- so FENCE1 (tier 0) is still refused as the carrier and the fence template's own β still concedes if reached. */   /* ⭐ s189 (row alt-seam-tier, ruling HQ-73): tier 3 -- an ALTERNATE whose beta re-yields at the same cursor -- is a live resume surface here for the same reason tier 1 is; SCRIP_ALT_SEAM_TIER=0 collapses it to 0 and this reads == 1 again. */
    }
