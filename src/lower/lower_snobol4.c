...
static int sno_const_static_on(void) { static int _cs = -1; if (_cs < 0) { const char * e = getenv("SCRIP_CONST_STATIC"); _cs = (e && *e == '0') ? 0 : 1; } return _cs; }   /* ⭐ SN4-CONSTANTS CN-3 killswitch, ONE AUTHORITY (RULES.md): read once, consulted by ALL THREE CN-3 sites -- the seal-table registration, the sno_pat_dfree `&` arm, and the defer-site stamp.  Every site must be gated or SCRIP_CONST_STATIC=0 is not byte-identical to pre-CN-3 and the A/B is vacuous (the s68 vacuous-gate conviction).  The dfree arm is the one that would bite hardest if left ungated: its `!spine` leg answers 1 for an argument-position keyword (`LEN(&MAXLNGTH)`) where the pre-CN-3 default answered 0, which is reachable from CLASSIC programs that contain no user constant at all. */
...
static IR_t * sno_pat_node(scx_t * cx, const tree_t * t, IR_t * succ, IR_t * fail) {
    ...
    case TT_DEFER: {
        ...
        if (in && in->t == TT_KEYWORD && in->v.sval) { static int _cn = -1; if (_cn < 0) { const char * e = getenv("SCRIP_CONST"); _cn = (e && *e == '0') ? 0 : 1; }   /* ⭐ SN4-CONSTANTS CN-2 (s145, default ON; =0 restores the EXPR$ thunk arm = the measured Error-22 dead end): *&Name in pattern position is a DEFERRED CONSTANT READ -- same IR_MATCH_DEFER by-name machinery as *X (manual p.122), NV-keyed "&Name" to match the keyword read/write canonicalization in keywords.c.  seal 0 (plain resume record) and pat_static 0 are the conservative MVP; CN-3 upgrades declared constants to static staging. */
          if (_cn) { char cb[130]; snprintf(cb, sizeof cb, "&%s", in->v.sval[0] == '&' ? in->v.sval + 1 : in->v.sval); IR_t * nd = lc_build(g, IR_MATCH_DEFER, succ, NULL); IR_LIT(nd).sval = lp_strdup(cb);
            { const tree_t * cp = sno_const_pat(cb); if (cp) nd->pat_static = sno_pat_dfree(cp, 1, 0); }   /* ⭐⭐⭐ SN4-CONSTANTS CN-3 -- GUARANTEE-PROPAGATION, the rung CN-2's own comment here promised.  The `*X` arm below must earn pat_static through sno_name_static, whose sno_seal_pat leg can fail for reasons that have NOTHING to do with this pattern (another name's second write, or one EVAL/`$(...)` anywhere in the program poisoning g_sno_fz_unsafe for all names).  A DECLARED constant needs no such proof, so the wrcount/fz legs are dropped and only the half that is still a genuine property of THIS tree is evaluated: sno_pat_dfree, transitive defer-freedom (manual p.122 -- only `*` recurses).  Declaration replaces inference for the part it can; it does NOT excuse the part it cannot, so a constant whose value still contains a dynamic `*Y` correctly stays 0.  Killswitch SCRIP_CONST_STATIC=0 restores the CN-2 conservative stamp for A/B.  BLAST RADIUS IS ZERO ON THE CLASSIC CORPUS BY CONSTRUCTION: reaching this site at all requires a tier-3 `&Name`, which the stock oracle rejects with error 251, so no classic program can carry one.  seal is deliberately LEFT at 0 -- s142 seal==2 (defer-site entry-cell) is a DIFFERENT mechanism from static staging and gets its own rung and its own measurement rather than being confounded with this one. */
