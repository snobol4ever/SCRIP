...
static const char * sno_cset_fold(const tree_t * a) {
    ...
    if (a->t == TT_KEYWORD && a->v.sval) {
        ...
        { char cb[130]; snprintf(cb, sizeof cb, "&%s", a->v.sval[0] == '&' ? a->v.sval + 1 : a->v.sval); const tree_t * cv = sno_const_val(cb); if (cv) return sno_cset_fold(cv); const tree_t * cp = sno_const_pat(cb); if (cp && cp != a) return sno_cset_fold(cp); }   /* ⭐⭐⭐ CN-12 -- A DECLARED CONSTANT'S CSET ARGUMENT FOLDS AT COMPILE TIME (Lon's order, s161-2: the declaration is a guarantee and no consumer may decline it). Scalar table first (T1's literal: `&V = "aeiou"` · SPAN("xyz" &V)), then the pattern table for CAT-of-literal chains the registration classified pattern-shaped. Both resolvers carry their own killswitch gates (STATIC/T1) so the =0 arms restore the old bytes; recursion terminates because stored trees are finite and a cyclic chain bottoms out in the resolvers' own miss (a chain member not yet registered resolves NULL -- statement order, CN-3b). Before this arm the fold's keyword knowledge was a HARDCODED TWO-NAME TABLE (lcase/ucase) -- measured s161: `&T = "\t"` · SPAN(" " &T) declined for exactly that reason. */
    }
...
static void sno_fz_build_table(const tree_t ** st, int nst) {
    ...
    for (int i = 0; i < nst; i++) {
        ...
        if (subj && subj->t == TT_VAR && subj->v.sval) {
            ...
            if (repl && repl->t == TT_QLIT && subj->v.sval && subj->v.sval[0] != '&') sno_const_note_val(subj->v.sval, repl);   /* ⭐⭐⭐ CN-12 -- BARE-VAR SCALAR STRINGS ENTER THE SAME TABLE (the T1 widening precedent: never a second table). A plain `digits = '0123456789'` is a seal CANDIDATE exactly as the pattern line above -- candidacy is free and the GUARDS LIVE IN THE RESOLVER (sno_var_val: wrcount==1, !g_sno_fz_unsafe, killswitches), mirroring sno_seal_pat's own division of labor. This is the arm that lets sno_cset_fold see through beauty's SPAN('.' digits &UCASE '_' &LCASE) -- measured s161-2: the DIGITS leaf alone was the whole reason that shape declined. Key is the bare name (no '&'), so it can never collide with a canonical "&Name" constant lookup. */
...
void sno_pat_thunks_build(int p0) {
    ...
    for (int pi2 = p0; pi2 < g_sno_npat; pi2++) {
        ...
        int pfenced = sno_pat_contains_fence(g_sno_pats[pi2].pat, 0);   /* ⭐ SN4-DEFER-RESUME FENCE DECLINE: one verdict feeds both the nary diversion and the carrier publication below */
