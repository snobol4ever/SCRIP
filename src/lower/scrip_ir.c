...
void bb_reset(IR_graph_t * bbg) {
    ...
    for (int i = 0; i < bbg->n; i++) {
        ...
        if (!bb) continue;
        bb->value   = FAILDESCR;
        if (bb->t != IR_PAT_ARBNO && bb->t != IR_SCAN && bb->t != IR_GOTO && bb->t != IR_GATHER && bb->t != IR_MAP && bb->t != IR_GREP && bb->t != IR_SNO_PROG && bb->t != IR_SUSPEND && !(bb->t == IR_SEQ && bb->dval == 1.0) && !(bb->t == IR_CALL && (bb->dval == 2.0 || bb->dval == 3.0))) bb->counter = 0;
        bb->state   = 0;
    }
    ag_ring_clear(bbg);
}
