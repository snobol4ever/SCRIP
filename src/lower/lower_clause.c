...
static IR_t *lower_pl_new_Alt(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    ...
    IR_t *bb = BB_node_alloc(bbg, BB_DISJ); if (!bb) return NULL;
    bb->α = aα; bb->β = bα;
    bb->γ = γ_in; bb->ω = ω_in;
...
static IR_t *lower_pl_new_Ite(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    ...
    IR_t *ite = BB_node_alloc(bbg, BB_ITE); if (!ite) return NULL;
...
static IR_t *lower_pl_new_Unify(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    ...
    IR_t *bb = BB_node_alloc(bbg, BB_UNIFY); if (!bb) return NULL;
...
static IR_t *lower_pl_new_Compare(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    ...
    IR_t *lhs = lower_pl_term(bbg,e->c[0],γ_in,ω_in,&lα,&lβ); if(!lhs) return NULL;
    IR_t *rhs = lower_pl_term(bbg,e->c[1],γ_in,ω_in,&rα,&rβ); if(!rhs) return NULL;
    IR_t *bb = BB_node_alloc(bbg, BB_BUILTIN); if (!bb) return NULL;
...
static IR_t *lower_pl_new_Conj(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    ...
    IR_t *seq = BB_node_alloc(bbg, BB_GCONJ); if (!seq) return NULL;
...
static IR_t *lower_pl_new_Call(IR_graph_t *bbg, const char *fn, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    int n = e ? e->n : 0;
    IR_t *bb = BB_node_alloc(bbg, BB_GOAL); if (!bb) return NULL;
...
static IR_t *lower_pl_term(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    if (!e) return NULL;
    switch (e->t) {
    case TT_ILIT: { IR_t *bb = BB_node_alloc(bbg, BB_LIT_I); if (!bb) return NULL; bb->ival = e->v.ival; return resolve_leaf(bb, γ_in, ω_in, α_out, β_out); }
    case TT_FLIT: { IR_t *bb = BB_node_alloc(bbg, BB_LIT_F); if (!bb) return NULL; bb->dval = e->v.dval; return resolve_leaf(bb, γ_in, ω_in, α_out, β_out); }
    case TT_QLIT: case TT_NAME: { IR_t *bb = BB_node_alloc(bbg, BB_ATOM); if (!bb) return NULL; bb->sval = e->v.sval; return resolve_leaf(bb, γ_in, ω_in, α_out, β_out); }
    case TT_VAR: { IR_t *bb = BB_node_alloc(bbg, BB_LOGICVAR); if (!bb) return NULL; bb->ival = e->v.ival; bb->sval = NULL; return resolve_leaf(bb, γ_in, ω_in, α_out, β_out); }
    case TT_MAKELIST: {
        ...
        if (has_tail && e->n >= 1) {
            ...
            IR_t *nil = BB_node_alloc(bbg, BB_ATOM); if (!nil) return NULL; nil->sval = "[]"; rest = nil;
        }
        for (int i = n_elem - 1; i >= 0; i--) {
            IR_t *hα=NULL,*hβ=NULL; IR_t *head = lower_pl_term(bbg, e->c[i], NULL, NULL, &hα, &hβ); if (!head) return NULL;
            IR_t *cons = BB_node_alloc(bbg, BB_STRUCT); if (!cons) return NULL;
    ...
    case TT_ADD: case TT_SUB: case TT_MUL: case TT_DIV: case TT_MOD: {
        ...
        IR_t *bb = BB_node_alloc(bbg, BB_ARITH); if (!bb) return NULL;
    ...
    case TT_FNC: {
        ...
        if (e->n == 0) { IR_t *bb = BB_node_alloc(bbg, BB_ATOM); if (!bb) return NULL; bb->sval = fn; return resolve_leaf(bb, γ_in, ω_in, α_out, β_out); }
        {
            IR_t *st = BB_node_alloc(bbg, BB_STRUCT); if (!st) return NULL;
...
static IR_t *lower_pl_goal(IR_graph_t *bbg, const tree_t *e, IR_t *γ_in, IR_t *ω_in, IR_t **α_out, IR_t **β_out) {
    ...
    if (e->t == TT_QLIT && e->v.sval) {
        ...
        if (strcmp(fn,"nl")==0)   { IR_t *bb=BB_node_alloc(bbg,BB_BUILTIN); if(!bb) return NULL; bb->sval=fn; return resolve_leaf(bb,γ_in,ω_in,α_out,β_out); }
    ...
    if (strcmp(fn,"phrase")==0 && (e->n==2 || e->n==3)) {
        ...
        else { IR_t *nil = BB_node_alloc(bbg, BB_ATOM); if (!nil) return NULL; nil->sval="[]"; nil->γ=γ_in; nil->ω=ω_in; zc->args[zc->nargs++]=nil; }
    ...
    if (strcmp(fn,"throw")==0 && e->n==1) {
        IR_t *bb = BB_node_alloc(bbg, BB_BUILTIN); if (!bb) return NULL; bb->sval="throw";
    ...
    if (strcmp(fn,"catch")==0 && e->n==3) {
        IR_t *bb = BB_node_alloc(bbg, BB_CATCH); if (!bb) return NULL;
    ...
    if (strcmp(fn,"findall")==0 && e->n==3) {
        IR_t *bb = BB_node_alloc(bbg, BB_BUILTIN); if (!bb) return NULL; bb->sval="findall"; bb->ival=0;
...
static IR_graph_t *lower_pl_clause_body(const tree_t *clause, int n_args) {
    ...
    if (n_stmts == 0) {
        IR_t *bb = BB_node_alloc(bbg, BB_SUCCEED); if (!bb) { free(stmts); BB_free(bbg); return NULL; }
        bb->γ = NULL; bb->ω = NULL;
    ...
    for (int i = n_stmts-1; i >= 0; i--) {
        ...
        if (i < n_args) {
            IR_t *lα=NULL,*lβ=NULL,*rα=NULL,*rβ=NULL;
            IR_t *slot_var = BB_node_alloc(bbg, BB_LOGICVAR); if (!slot_var) goto fail;
            ...
            IR_t *uni = BB_node_alloc(bbg, BB_UNIFY); if (!uni) goto fail;
    ...
    {
        IR_t *seq = BB_node_alloc(bbg, BB_GCONJ);
        if (!seq) goto fail;
...
IR_graph_t *lower_pl_predicate(tree_t *choice) {
    ...
    IR_t *bb = BB_node_alloc(bbg, BB_CHOICE); if (!bb) { BB_free(bbg); return NULL; }
