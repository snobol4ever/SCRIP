...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static IR_t * lower_binop(lcx_t * cx, const tree_t * t, const char * opn, IR_t * γ, IR_t * ω) {
    IR_t * nd = build(cx, IR_STRUCT, γ, ω); IR_LIT(nd).sval = opn;
    ir_operand_push(nd, lower(cx, t->c[0], NULL, NULL));
    ir_operand_push(nd, lower(cx, t->c[1], NULL, NULL)); return nd;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static IR_t * lower_unop(lcx_t * cx, const tree_t * t, const char * opn, IR_t * γ, IR_t * ω) {
    IR_t * nd = build(cx, IR_STRUCT, γ, ω); IR_LIT(nd).sval = opn;
