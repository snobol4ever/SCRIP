...
static IR_t * icn_assign(lcx_t cx, const tree_t * e, IR_t * γ_in, IR_t * ω_in, IR_t ** α_out, IR_t ** β_out) {
    ...
    if (lhs_t->t == TT_IDX && lhs_t->n >= 2 && lhs_t->c[0] && lhs_t->c[1]) {
        ...
        IR_t * base = lower2(icn_bounded(cx), lhs_t->c[0], idx, ω_in, &bα, &bβ);
        if (!base) return NULL;
        idx->α = bα ? bα : base;
        IR_t * kα = NULL, * kβ = NULL;
        IR_t * key = lower2(icn_bounded(cx), lhs_t->c[1], idx, ω_in, &kα, &kβ);
        if (!key) return NULL;
        idx->β = kα ? kα : key;
        IR_t * rα = NULL, * rβ = NULL;
        IR_t * rhs = lower2(icn_bounded(cx), rhs_t, idx, ω_in, &rα, &rβ);
        if (!rhs) return NULL;
        idx->β->γ = rα ? rα : rhs;
    ...
    if (lhs_t->t == TT_FIELD && lhs_t->n >= 2 && lhs_t->c[0] && lhs_t->c[1] && lhs_t->c[1]->v.sval) {
        ...
        if (!fs) return NULL;
        fs->sval = lhs_t->c[1]->v.sval;
        IR_t * aα = NULL, * aβ = NULL;
        IR_t * obj = lower2(icn_bounded(cx), lhs_t->c[0], fs, ω_in, &aα, &aβ);
        if (!obj) return NULL;
        fs->α = aα ? aα : obj;
        IR_t * rα = NULL, * rβ = NULL;
        IR_t * rhs = lower2(icn_bounded(cx), rhs_t, fs, ω_in, &rα, &rβ);
        if (!rhs) return NULL;
        fs->β = rα ? rα : rhs;
...
static IR_t * icn_swap(lcx_t cx, const tree_t * e, IR_t * γ_in, IR_t * ω_in, IR_t ** α_out, IR_t ** β_out) {
    ...
    if (!lv) return NULL;
    lv->sval = (e->c[0]->t == TT_VAR && e->c[0]->v.sval) ? e->c[0]->v.sval : "";
    lv->γ = sw; lv->ω = ω_in;
    IR_t * rv = nalloc(cx, IR_VAR);
    if (!rv) return NULL;
    rv->sval = (e->c[1]->t == TT_VAR && e->c[1]->v.sval) ? e->c[1]->v.sval : "";
    rv->γ = sw; rv->ω = ω_in;
    sw->α = lv;
    sw->β = rv;
...
static IR_t * icn_field_get(lcx_t cx, const tree_t * e, IR_t * γ_in, IR_t * ω_in, IR_t ** α_out, IR_t ** β_out) {
    ...
    fg->sval = fname;
    IR_t * aα = NULL, * aβ = NULL;
    IR_t * obj = lower2(icn_bounded(cx), obj_t, fg, ω_in, &aα, &aβ);
    if (!obj) return NULL;
    fg->α = aα ? aα : obj;
...
static IR_t * icn_section(lcx_t cx, const tree_t * e, IR_t * γ_in, IR_t * ω_in, IR_t ** α_out, IR_t ** β_out) {
    ...
    IR_t * aα = NULL, * aβ = NULL;
    IR_t * obj = lower2(icn_bounded(cx), e->c[0], sec, ω_in, &aα, &aβ);
    if (!obj) return NULL;
    sec->α = aα ? aα : obj;
    IR_t * bα = NULL, * bβ = NULL;
    IR_t * i1 = lower2(icn_bounded(cx), e->c[1], sec, ω_in, &bα, &bβ);
    if (!i1) return NULL;
    sec->β = bα ? bα : i1;
    IR_t * cα = NULL, * cβ = NULL;
    IR_t * i2 = lower2(icn_bounded(cx), e->c[2], sec, ω_in, &cα, &cβ);
    if (!i2) return NULL;
    sec->β->γ = cα ? cα : i2;
