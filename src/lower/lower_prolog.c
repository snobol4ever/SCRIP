...
static int pl_conj_top_cut(const lc_vec * v) {
    for (int i = 0; i < v->n; i++) { const tree_t * g = ((const tree_t * const *) v->data)[i];
...
static tree_t * pl_cc_fnc4(const char * f, tree_t * a0, tree_t * a1, tree_t * a2, tree_t * a3) { tree_t * n = ast_node_new(TT_FNC); n->v.sval = (char *) f; ast_push(n, a0); ast_push(n, a1); ast_push(n, a2); ast_push(n, a3); return n; }
...
static tree_t * pl_cc_gen2(const char * count_leaf, const char * nth_leaf, tree_t * a0, tree_t * a1, tree_t * a2) {
    tree_t * cv = pl_cc_freshvar(); tree_t * cv2 = pl_cc_freshvar(); tree_t * iv = pl_cc_freshvar(); tree_t * iv2 = pl_cc_freshvar();
    cv2->v.ival = cv->v.ival; iv2->v.ival = iv->v.ival;
...
static IR_t * pl_lower_ite(lcx_t * cx, const tree_t * C, const tree_t * T, const tree_t * E, IR_t * γnext, IR_t * ωfail, IR_t ** entry_out) {
    ...
    for (int j = 0; j < nb; j++) {
        ...
        ml->seal = (rt != ig) ? 1 : 0; IR_LIT(ml).ival = ig->n_operands - 1;
    ...
    { int before = cx->g->n;
      ...
      IR_t * saveω = cx->cutω; IR_t * ccut = build(cx, IR_GOTO, unmk_f, unmk_f); cx->cutω = ccut;
      IR_t * savecutω = cx->cutω; cx->cutω = ccut; int save_scope = cx->cut_scope; cx->cut_scope = ++cx->scope_seq;
...
static IR_t * pl_lower_softcut(lcx_t * cx, const tree_t * C, const tree_t * T, const tree_t * E, IR_t * γnext, IR_t * ωfail, IR_t ** entry_out) {
    ...
    { lc_vec cv; lc_vec_init(&cv, (int) sizeof(const tree_t *));
      collect_conj(C, &cv);
      IR_t * ce = NULL;
      IR_t * savecutω = cx->cutω; cx->cutω = build(cx, IR_GOTO, unmk_f, unmk_f); int save_scope = cx->cut_scope; cx->cut_scope = ++cx->scope_seq;
    ...
    IR_t * tredo = NULL; int sc_cut_in_then = 0; int sc_cut_in_else = 0; IR_t * sc_then_cut = NULL; IR_t * sc_else_cut = NULL;
    { lc_vec tv; lc_vec_init(&tv, (int) sizeof(const tree_t *));
      ...
      sc_cut_in_then = !tredo && pl_conj_top_cut(&tv); if (sc_cut_in_then) sc_then_cut = pl_cut_redo(cx);
    ...
    if (E) {
        ...
        sc_cut_in_else = !eredo && pl_conj_top_cut(&ev); if (sc_cut_in_else) sc_else_cut = pl_cut_redo(cx);
        ir_operand_push(ml_ee, sc_cut_in_else ? sc_else_cut : (eredo ? eredo : ig)); ir_operand_push(ml_ee, ig); ir_operand_push(ig, ml_ee); IR_LIT(ig).ival = ig->n_operands; ml_ee->seal = (eredo || sc_cut_in_else) ? 1 : 0; IR_LIT(ml_ee).ival = ig->n_operands - 1;
    ...
    ir_operand_push(ml_t, sc_cut_in_then ? sc_then_cut : (tredo ? tredo : (credo ? credo : ig))); ir_operand_push(ml_t, ig); ir_operand_push(ig, ml_t); IR_LIT(ig).ival = ig->n_operands; ml_t->seal = (tredo || credo || sc_cut_in_then) ? 1 : 0; IR_LIT(ml_t).ival = ig->n_operands - 1;
    lc_γ_to(mark, ml_e);
    if (entry_out) *entry_out = mark;
    return ig;
}
...
static IR_t * pl_lower_catch(lcx_t * cx, const tree_t * G, const tree_t * C, const tree_t * R, IR_t * γnext, IR_t * ωfail, IR_t ** entry_out) {
    ...
    for (int j = 0; j < 2; j++) {
        ...
        IR_t * saveω = cx->cutω; int save_scope = cx->cut_scope; cx->cutω = build(cx, IR_GOTO, ig, ig); cx->cut_scope = ++cx->scope_seq;
        lc_vec av; lc_vec_init(&av, (int) sizeof(const tree_t *));
        collect_conj(arms[j], &av);
        IR_t * ae = NULL; IR_t * redo = NULL;
        ...
        ir_operand_push(ml, ig);
        ir_operand_push(ig, ml); IR_LIT(ig).ival = ig->n_operands;
        ml->seal = redo ? 1 : 0; IR_LIT(ml).ival = ig->n_operands - 1;
        arm_entry[j] = ae ? ae : (first ? first : ml);
    }
...
static const pl_det_leaf_t pl_det_leaves[] = {
    ...
    { "put_code", 1, "$put_code" }, { "put_byte", 1, "$put_byte" }, { "unget_char", 1, "$unget_char" }, { "at_end_of_stream", 0, "$at_end_of_stream" }, { "current_prolog_flag", 2, "$current_prolog_flag" }, { "set_prolog_flag", 2, "$set_prolog_flag" }, { "telling", 1, "$telling" }, { "seeing", 1, "$seeing" }, { "tell", 1, "$tell" }, { "append", 1, "$append1" }, { "see", 1, "$see" }, { "told", 0, "$told" }, { "seen", 0, "$seen" }, { "at_end_of_stream", 1, "$at_end_of_stream_s" },
    ...
    { "writeq", 1, "$writeq" }, { "print", 1, "$write" }, { "write_term", 2, "$write_term" }, { "write_term", 3, "$write_term_s" }, { "write_canonical", 1, "$write_canonical" }, { "writeln", 1, "$writeln" },
    { "display", 1, "$display" }, { "display", 2, "$display_s" }, { "unify_with_occurs_check", 2, "$unify_oc" }, { "$aggregate_reduce", 3, "$aggregate_reduce" },
...
static const struct { const char * nm; int ar; const char * gsym; } pl_anum_guards[] = {
    ...
    { "number_chars", 2, "$pl_anum_guard2" }, { "number_codes", 2, "$pl_anum_guard2" }, { "number_string", 2, "$pl_anum_guard2" }, { "atom_concat", 3, "$pl_anum_guard3" }, { "atomic_concat", 3, "$pl_anum_guard3" }, { "atomic_list_concat", 2, "$pl_anum_guard2" }, { "atomic_list_concat", 3, "$pl_anum_guard3" },
...
static IR_t * pl_db_leaf2_tree(lcx_t * cx, const char * sym, const tree_t * a0, const tree_t * a1, IR_t * γnext, IR_t * ωfail, IR_t ** entry_out) {
    IR_t * nd = build(cx, IR_CALL, γnext, ωfail); IR_LIT(nd).sval = (char *) sym;
    IR_t * e0 = NULL; IR_t * v0 = term_e(cx, a0, &e0);
    IR_t * e1 = NULL; IR_t * v1 = term_e(cx, a1, &e1);
    lc_γ_to(v0, e1 ? e1 : v1); lc_ω_to(v0, ωfail);
    lc_γ_to(v1, nd); lc_ω_to(v1, ωfail);
    ir_operand_push(nd, v0); ir_operand_push(nd, v1);
...
static IR_t * goal_inner(lcx_t * cx, const tree_t * t, IR_t * γnext, IR_t * ωfail, IR_t ** entry_out) {
    ...
    case TT_FNC: {
        ...
        if (!strcmp(nm, ",")) {
            lc_vec glv; lc_vec_init(&glv, (int) sizeof(const tree_t *));
            collect_conj(t, &glv);
            { IR_t * cω = build(cx, IR_GOTO, ωfail, ωfail); IR_t * ientry = NULL; IR_t * iredo = NULL;
        ...
        if (!strcmp(nm, "aggregate_all") && t->n == 3 && !pl_file_defines(nm, 3)) {
            const tree_t * spec = t->c[0]; const char * kind = NULL; const tree_t * tmpl = NULL;
            if (spec && spec->t == TT_FNC && spec->n == 1 && spec->v.sval) {
                if (!strcmp(spec->v.sval, "count")) { kind = "count"; tmpl = spec->c[0]; }
                else if (!strcmp(spec->v.sval, "max")) { kind = "max"; tmpl = spec->c[0]; }
                else if (!strcmp(spec->v.sval, "min")) { kind = "min"; tmpl = spec->c[0]; }
                else if (!strcmp(spec->v.sval, "sum")) { kind = "sum"; tmpl = spec->c[0]; }
                else if (!strcmp(spec->v.sval, "bag")) { kind = "bag"; tmpl = spec->c[0]; }
                else if (!strcmp(spec->v.sval, "set")) { kind = "set"; tmpl = spec->c[0]; }
            } else if (spec && (spec->t == TT_NAME || spec->t == TT_QLIT) && spec->v.sval && !strcmp(spec->v.sval, "count")) {
                kind = "count"; tmpl = pl_cc_ilit(0);
            }
        ...
        if (((!strcmp(nm, "findall") || !strcmp(nm, "bagof") || !strcmp(nm, "setof")) && t->n == 3) || (!strcmp(nm, "findall") && t->n == 4)) {
            ...
            IR_t * unmk = build(cx, IR_UNMARK, nd, ωfail); ir_operand_push(unmk, mark); IR_LIT(unmk).ival = pl_fence_on() ? 5 : 1;
            IR_t * gentry = NULL; IR_t * gredo = NULL;
            lc_vec glv; lc_vec_init(&glv, (int) sizeof(const tree_t *));
            collect_conj(pl_isfind ? (wantg ? gt : t->c[1]) : gt, &glv);
            IR_t * te = NULL; IR_t * tv = term_e(cx, t->c[0], &te);
            IR_t * bc = build(cx, IR_CALL, unmk, ωfail); IR_LIT(bc).sval = "$ball_pending";
            ...
            lc_γ_to(tv, add); lc_ω_to(tv, unmk);
            ir_operand_push(add, acc); ir_operand_push(add, tv);
            if (gredo) lc_γ_to_β(add, gredo); else lc_γ_to(add, unmk);
            lc_ω_to(add, unmk);
        ...
        if ((!strcmp(nm, "assertz") || !strcmp(nm, "assert") || !strcmp(nm, "asserta")) && t->n == 1) {
            ...
            if (!pn) return goal(cx, pl_cc_fnc2(",", pl_cc_fnc2("$db_t_guard", (tree_t *) t->c[0], (tree_t *) pl_atom_goal("assert")),
        ...
        if (!strcmp(nm, "retract") && t->n == 1) {
            ...
            { const tree_t * bad = pl_clause_ill_typed(t->c[0]); if (bad) return goal(cx, pl_cc_type_error("callable", bad, nm, 1), γnext, ωfail, entry_out); }
            pn = pl_head_key(t->c[0], &ar);
            if (!pn) { IR_t * le = NULL; IR_t * nd = pl_db_enum(cx, -1, pl_clause_target(t->c[0], NULL), 1, γnext, ωfail, &le);
              IR_t * ge = NULL; pl_db_leaf2_tree(cx, "$db_t_guard", t->c[0], pl_atom_goal("retract"), le, ωfail, &ge);
              if (entry_out) *entry_out = ge ? ge : le; return nd; }
            if (pl_pi_is_static_builtin(pn, ar) || !pl_db_owned(pn, ar)) return goal(cx, pl_cc_perm_static(pn, ar), γnext, ωfail, entry_out);
        ...
        if (!strcmp(nm, "retractall") && t->n == 1) {
            ...
            pn = pl_head_key(t->c[0], &ar);
            if (!pn) return goal(cx, pl_cc_fnc2(",", pl_cc_fnc2("$db_t_guard", (tree_t *) t->c[0], (tree_t *) pl_atom_goal("retractall")),
                                                pl_cc_fnc1("$db_retractall_t", (tree_t *) t->c[0])), γnext, ωfail, entry_out);
            if (pl_pi_is_static_builtin(pn, ar) || !pl_db_owned(pn, ar)) return goal(cx, pl_cc_perm_static(pn, ar), γnext, ωfail, entry_out);
        ...
        if (!strcmp(nm, "abolish") && t->n == 1) {
            ...
            if (!pn) return goal(cx, pl_cc_fnc2(",", pl_cc_fnc2("$db_t_guard", (tree_t *) t->c[0], (tree_t *) pl_atom_goal("abolish")),
                                                pl_cc_fnc1("$db_abolish_t", (tree_t *) t->c[0])), γnext, ωfail, entry_out);
        ...
        if (!strcmp(nm, "clause") && t->n == 2) {
            ...
            pn = pl_head_key(t->c[0], &ar);
            if (!pn) { IR_t * le = NULL; IR_t * nd = pl_db_enum(cx, -1, pl_clause_target(t->c[0], t->c[1]), 0, γnext, ωfail, &le);
        ...
        if (!strcmp(nm, "stream_property") && t->n == 2 && !pl_file_defines(nm, 2))
            return goal(cx, pl_cc_fnc2(",", pl_cc_fnc2("$pl_sp_check", (tree_t *) t->c[0], (tree_t *) t->c[1]),
                pl_cc_gen2("$pl_sp_count", "$pl_sp_nth", (tree_t *) t->c[0], (tree_t *) t->c[1], (tree_t *) 0)), γnext, ωfail, entry_out);
        ...
        if (!strcmp(nm, "current_predicate") && t->n == 1 && !pl_file_defines(nm, 1)) {
            ...
            if (s && s->t == TT_FNC && s->v.sval && !strcmp(s->v.sval, "/") && s->n == 2) {
                n = (tree_t *) s->c[0]; a = (tree_t *) s->c[1];
                body = pl_cc_gen2("$pl_cp_count", "$pl_cp_nth", n, a, (tree_t *) 0); }
            else { tree_t * n2; tree_t * a2;
