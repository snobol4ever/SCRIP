...
std::string bb_lit_scalar() {
    return !PLATFORM_X86 ? std::string()
         : _.op_node_kind == (int)IR_LIT_INTEGER && _.op_off >= 0
         ...
         : _.op_node_kind == (int)IR_LIT_STRING && _.op_off >= 0
         ...
         : _.op_node_kind == (int)IR_LIT_CHARSET && _.op_off >= 0
         ? x86("comment", "IR_LIT_CHARSET")
             ...
             + x86("label",  LS(0))
             + x86(".string", _.op_sval ? _.op_sval : "")
         : _.op_node_kind == (int)IR_LIT_REAL && _.op_off >= 0
