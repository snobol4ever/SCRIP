...
static std::string fence_mark_save(int off) {
    if (x86_port_cstack() && _.op_fence_frame_off != -1) return x86("mov", RDQ("rbp", _.op_fence_frame_off), "rsp");   /* ⭐ R-4(f) SLICE 3 (s96): dynamic-depth body (container in P) -- the watermark rides its ACTIVATION-FRAME slot [rbp+off] (shared registry, emit.cpp fence_frame_slot); every release below reads the same slot, so no RSP-relative offset is involved at any depth. K=0/static-K bodies keep the RSP quads below byte-identical. */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static std::string fence_release(int off, int kk = 0) {   /* kk = committed body ΣK, nonzero ONLY on the σ-commit edge (fence_whack_commit); cstack/FORTH arms only -- the arena ports address a slot, not a live frontier */
    if (x86_port_cstack() && _.op_fence_frame_off != -1) return x86("mov", "rsp", RDQ("rbp", _.op_fence_frame_off));   /* ⭐ R-4(f) SLICE 3 (s96): frame-slot arm -- σ commit, PAIR(3) and β all restore from [rbp+off]; kk is a static-K notion and does not apply here */
...
static std::string fence_whack_commit(int off) {   /* ⭐ R-4(a) SLICE 2 (s95): the σ-COMMIT release is the ONE edge on which the body's COMMITTED cells are still live above α's watermark quad -- a STANDING-slotted body-interior capture SAVE carves 16B at its α and, non-popping by THE MODEL, leaves it for the bracket whack; so the quad α stored at FRQ(off+32) sits at FRQ(off+32+ΣK_body) here.  PAIR(3)/β arrive AFTER every member β freed its own K (unwind law) and keep off+32.  ΣK staged ONCE at the FENCE1 dispatch (emit.cpp fence_body_kk, THE ONE K AUTHORITY zd_k); 0 for the K=0 body class = byte-identical to HEAD.  H14 witness: FENCE(LEN(2) . W) -- `mov rsp,[rsp+176]` read 16B under the quad, RSP whacked to garbage, r12/r13 trashed at MATCH_END, movaps fault in libc. */
    return fence_release(off, _.op_fence_body_kk);   /* ⭐ FENCE-WHACK-UCLAIM (this rung): the s137 non-zw floor whack `mov rsp,___` is RETIRED — under UCLAIM the statement claim (`sub rsp,K` at the statement head) lives BELOW the graph ___ floor, so whacking to ___ released the claim the statement's staged `add rsp,K` fail exits still owned: DOUBLE-RELEASE, measured as SEGV-after-correct-output (FENCE(P)+failing-defer witness f6d) and as the H26/G23 hang class (rc=124), all cured by SCRIP_FENCE_WHACK=0 — this line makes the cured path the default.  The commit now frees the fence's OWN extent only (watermark restore = P's growth), per WHACK CONTRACT clause 5: statement-accumulated ζ release is STATEMENT_END's authority, not the fence's.  Success paths were self-healing only because match_end's absolute `mov rsp,[___+64]` forgave the over-free; the seal-fail route had no absolute restore, hence the crash class.  Known cost accepted: the s137 json-match retention win narrows to P's own span (memory, not correctness).  The op_zw arm above is the W ladder's own protocol — untouched. */
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
