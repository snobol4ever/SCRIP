...
static std::string fence_mark_save(int off) {
    if (x86_port_cstack() && _.op_fence_frame_off != -1) return x86("mov", FFCQ(0), "rsp");   /* ⭐ R-4(f) SLICE 3 (s96): dynamic-depth body (container in P) -- the watermark rides its ACTIVATION-FRAME slot [rbp+off] (shared registry, emit.cpp fence_frame_slot); every release below reads the same slot, so no RSP-relative offset is involved at any depth. K=0/static-K bodies keep the RSP quads below byte-identical. */
...
static std::string fence_release(int off, int kk = 0) {   /* kk = committed body ΣK, nonzero ONLY on the σ-commit edge (fence_whack_commit); cstack/FORTH arms only -- the arena ports address a slot, not a live frontier */
    if (x86_port_cstack() && _.op_fence_frame_off != -1) return x86("mov", "rsp", FFCQ(0));   /* ⭐ R-4(f) SLICE 3 (s96): frame-slot arm -- σ commit, PAIR(3) and β all restore from [rbp+off]; kk is a static-K notion and does not apply here */
