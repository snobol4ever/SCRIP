...
std::string bb_var_global() {
    if (PLATFORM_X86)
        return !_.op_zres && _.op_off < 0 ? x86_alpha() + x86_bomb("bb_var_global: unhandled (needs descr flat-chain + own slot)")
             ...
             + x86("note", gva_name(_.op_gva_k)) + x86("mov",    vg_res(0), "rax")
             + x86("note", gva_name(_.op_gva_k)) + x86("mov",    vg_res(8), "rdx")
             + x86_gamma()
             + x86_beta_trampoline()
             : x86("comment", "IR_VAR")
             + x86_alpha()
             ...
             + x86_omega("je")
             + x86("note", ZRESN()) + x86("mov",    vg_res(0), "rax")
             + x86("note", ZRESN()) + x86("mov",    vg_res(8), "rdx")
             + x86_gamma()
             + x86_beta_trampoline()
             + x86("def",    L(0))
