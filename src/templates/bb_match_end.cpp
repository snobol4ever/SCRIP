...
static int has_replace_l(void) { if (!g_emit_cfg) return 0; for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _nd = g_emit_cfg->all[_i]; if (_nd && (_nd->op == IR_MATCH_REPLACE || _nd->op == IR_MATCH_FENCE0 || _nd->op == IR_MATCH_FENCE1 || _nd->op == IR_MATCH_ABORT || _nd->op == IR_MATCH_ARBNO)) return 1; } return 0; }   /* ⭐ FENCE1/ABORT joined (measured, H01/H04/H06/H18/H28): both whack rsp to the ___ floor mid-match, discarding the zclaim(48) that backs HKQ -- subsequent quartet traffic then aliases re-carved spine cells (SEGV).  Same law as the slot authority refuses: whack-hazard graphs keep legacy homes.  ARBNO joined (measured, X10): the ARB-LON-K16 mechanism-2 statement release reads old____ through the POSITIVE [___+pin] slot (x86_asm.h HEAD-PIN restore), which the widened head abandons for HKQ(0) -- correct output then exit SEGV on the stale base.  ARBNO graphs keep the legacy quartet home until that restore reads through the one macro too (later rung). */   /* ⭐ FENCE-SPLIT (this rung): IR_MATCH_FENCE0 listed ALONGSIDE FENCE1 purely to keep this predicate BYTE-IDENTICAL across the kind split — before the split both forms arrived here as IR_MATCH_FENCE1 and both were caught.  ⚠ REVIEW OWED, NOT ASSUMED CORRECT: the stated reason for membership is "whacks rsp to the ___ floor mid-match", and FENCE0 has not whacked since ZWS-FENCE0-UCLAIM (MECH s6) — so its membership is probably now VACUOUS and droppable.  That is a behaviour change with its own blast radius and does not ride this structural commit. */
...
static int cap_name_strict(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_CAP_NAME_STRICT"); v = (e && *e == '0') ? 0 : 1; } return v; }   /* ⭐ SN4-CAP-NAME-STRICT (s170, row b1c-retreat) emitter half -- same env name as the lower + runtime halves so all three flip together.  DEFAULT ON (s178 flip, Lon greenlight; =0 reverts to the pre-s170 emission byte-identically): the armed arm adds one test/jz pair plus a 4-instruction ω stub after the pump call. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
