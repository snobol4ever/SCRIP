...
static int has_replace_l(void) { if (!g_emit_cfg) return 0; for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _nd = g_emit_cfg->all[_i]; if (_nd && (_nd->op == IR_MATCH_REPLACE || _nd->op == IR_MATCH_FENCE1 || _nd->op == IR_MATCH_ABORT || _nd->op == IR_MATCH_ARBNO)) return 1; } return 0; }   /* ⭐ FENCE1/ABORT joined (measured, H01/H04/H06/H18/H28): both whack rsp to the rbp floor mid-match, discarding the zclaim(48) that backs HKQ -- subsequent quartet traffic then aliases re-carved spine cells (SEGV).  Same law as the slot authority declines: whack-hazard graphs keep legacy homes.  ARBNO joined (measured, X10): the ARB-LON-K16 mechanism-2 statement release reads old_rbp through the POSITIVE [rbp+pin] slot (x86_asm.h HEAD-PIN restore), which the widened head abandons for HKQ(0) -- correct output then exit SEGV on the stale base.  ARBNO graphs keep the legacy quartet home until that restore reads through the one macro too (later rung). */
#define stfh() (_.flat_stmt_frame || (oscap_l() && _.flat_deep_arrival && !_.flat_jmp_entry && !_.flat_lcl_proc && !_.zframe_graph && !_.flat_pat && !_.flat_gen && !has_replace_l()))   /* ⭐ OS-2·SLICE-1 QUARTET RE-HOME: the PATCTX/old_rbp quartet's FRQ(op_off+40..72) positive-off spelling on carve-less legacy deep MAINS rides finite CRT slack and was CAUGHT (hardware watchpoint) smashing environ[1] via the [rbp+312] old_rbp store when the head's ZLS off grew past ~336 (dc_recur m4) -- the s22r/s23a class, quartet family.  The stfh arm is the head's OWN correct negative-home protocol (HKQ [rbp-48..-8] backed by its zclaim(48)); widening its predicate to the GLUE-O legacy-deep-main class re-homes stores AND restores through the ONE macro in both files -- no split ends possible.  REPLACE graphs excluded: bb_match_replace.cpp:33 restores rbp from FRQ(op_off+40) directly and would read the abandoned slot; their quartet keeps legacy slack behavior (pre-existing risk, unchanged).  oscap_l: SCRIP_OS_CAP=0 restores the bare flat_stmt_frame predicate -- byte-identical legacy per the killswitch law.  Capture δ slots live at [rbp-64-16k], strictly BELOW this zone -- no overlap by construction. */
static const char * HKQ(int k) { static char b[8][40]; static int i; i = (i + 1) & 7; snprintf(b[i], 40, "qword ptr [rbp + %d]", -48 + 8 * k); return b[i]; }
static const char * HKM() { return "qword ptr [rbp + -56]"; }   /* ⭐ SLACK-RIDER RE-HOME (this session): the bb_match_begin twin, verbatim -- zls2_mark's negative home.  DELIBERATELY DUPLICATED rather than shared, exactly as HKQ already is across these two files: the pair is the head's ABI with itself and the existing file-local spelling is the established house shape here.  A drift between these two lines is the split-ends failure HKN(k) exists to catch, so both files name the slot through HKN(5) at every use. */
static const char * HKD() { return "dword ptr [rbp + -64]"; }   /* ⭐ SLACK-RIDER RE-HOME (this session): the bb_match_begin twin, verbatim -- start_δ's negative home.  RELEASE runs inside the SAME bracket as the head's α, so [rbp-64] is depth-free at the post-unwind write exactly as it was at the write that seeded it -- the same argument HKQ's own comment makes for the quartet, and the reason this cell can move homes without any change to WHEN it is read. */   /* SUBJ-ARM-2: the head's statement-bracket rbp housekeeping slots (bb_match_begin.cpp twin, same k map: 0=deep-rbp 1=r13 2=r14 3=r15 4=capgen) -- RELEASE runs inside the SAME bracket, so [rbp-48+8k] is depth-free at the post-unwind read exactly as at the head's alpha write */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
