...
static int apin(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ARMED_PIN"); v = (e && *e == '0') ? 0 : 1; } return v; }
static int stmt_has_proc_invoke(void) { if (!g_emit_cfg) return 0; for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _nd = g_emit_cfg->all[_i]; if (!_nd) continue; if (_nd->op == IR_MATCH_DEFER && !(_nd->pat_static && IR_LIT(_nd).sval && !strncmp(IR_LIT(_nd).sval, "PATV$", 5))) return 1; } return 0; }
...
std::string bb_match_begin() {
    ...
    if (emit_match_rbp()) {
        ...
        return x86("comment", "IR_MATCH_BEGIN (MATCH-RBP frame; mark=[rbp-8])")
             ...
             + x86("note", "retry_whack") + x86("lea", "rsp", RDQ("rbp", -56 - extra))
    ...
    return ((_.op_sa < 0 && !subjc() && !_.op_zres) || _.op_off < 0)
         ...
         + x86("mov", "r15", "rdx")
         + x86("rtcc_rl")
         + x86("note", "cas_top") + x86("mov", RDQ("r12", 0), (long)0) + x86("note", "cas_rsp_mark") + x86("mov", RDQ("r12", 8), "rsp") + x86("note", "cas_top") + x86("add", "r12", (long)24)
         ...
         + x86("note", "start_δ") + x86("mov", stfh() ? HKD() : FR(_.op_off), (long)0)
         + x86("def", L(0))
         + x86("note", "start_δ") + x86("mov", "r14d", stfh() ? HKD() : FR(_.op_off))
         + x86_gamma()
         + x86_beta()
         + IF(hfc() && x86_zc_frame() == ZC_FRAME_RSP, x86("note", "rsp_restore") + x86("mov", "rsp", RDQ("r12", -16)) + x86("sub", "rsp", (long)32))
         + x86("note", "start_δ") + x86("add", stfh() ? HKD() : FR(_.op_off), (long)1)
         + x86("note", "start_δ") + x86("mov", "eax", stfh() ? HKD() : FR(_.op_off))
         + x86("cmp", "eax", "r15d")
         + x86("jg",  L(1))
         + x86("mov", "rcx", "[rip@got + __]", (uint64_t)(uintptr_t)(const void *)rt_anchor_ptr(), "rt_anchor_g")
         + x86("mov", "rax", "[rcx]")
         ...
         + (hfc() && x86_zc_frame() == ZC_FRAME_RSP
             ? x86("note", "cas_mark") + x86("sub", "r12", (long)24)
             + IF(emit_arbno_rbp_unwind(), x86_arbno_rbp_unwind("r12", 8, 11, 12))
             + x86("note", "cas_rsp_mark") + x86("mov", "rsp", RDQ("r12", 8))
             : IF(x86_zc_frame() != ZC_FRAME_RSP, x86("note", "cas_mark") + x86("sub", "r12", (long)24))
             + (x86_zc_frame() == ZC_FRAME_RSP
                 ? x86("note", "cas_mark") + x86("def", L(2)) + x86("sub", "r12", (long)24) + x86("mov", "rax", RDQ("r12", 0)) + x86("test", "rax", "rax") + x86("jne", L(2))
                 + IF(emit_arbno_rbp_unwind(), x86_arbno_rbp_unwind("r12", 8, 11, 12))
                 + x86("note", "cas_rsp_mark") + x86("mov", "rsp", RDQ("r12", 8))
                 : ( x86_align_enter()
