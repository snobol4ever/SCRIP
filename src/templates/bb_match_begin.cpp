...
std::string bb_match_begin() {
    ...
    return ((_.op_sa < 0 && !subjc() && !_.op_zres) || _.op_off < 0)
         ...
         + x86_gamma()
         + x86_beta()
         + IF(hfc() && x86_zc_frame() == ZC_FRAME_RSP, x86("note", "rsp_restore") + x86("mov", "rsp", RDQ("r12", -16)) + x86("sub", "rsp", (long)32))   /* CLIMB-RSP-FIX: pattern nodes (LIT_INTEGER etc.) push ζ-cells onto RSP during the match body; when they fail back to β the RSP is shifted, making FR(_.op_off) address wrong memory -- start_δ reads/writes a stale slot → corrupted counter → infinite retry → stack overflow (measured: 'zzzz' ? POS(0) 'ab' → SIGSEGV). Restore RSP to match_begin's 32B frame base before the start_δ access: cas_rsp_mark=[r12-16] is the pre-sub-rsp-32 RSP (r12 advanced by 24 at push; FORTH spine has no nested CAS entries in this regime). Gate: hfc() = FORTH spine with an FC window (the only arm that allocates the 32B frame). */
