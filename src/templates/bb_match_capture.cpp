 ...
 * path VERBATIM (degrade never die); the pend/___-dcap machinery is identical on both paths (F2: by-value). */
...
std::string bb_match_capture() {
    ...
    return (_.op_off < 0)
         ...
         ? ( x86("comment", "IR_MATCH_CAPTURE_SAVE push")
           ...
           + x86_omega() )
         : (int)_.op_phase == 1
         /* REG-7 pend shapes (s80; was REG-6 rdi-staged, was REG-2 cell, was ___-dcap).  γ: the pend top IS
