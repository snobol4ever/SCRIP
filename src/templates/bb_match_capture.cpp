...
std::string bb_match_capture() {
    ...
    return (_.op_off < 0)
         ...
         ? ( x86("comment", "IR_MATCH_CAPTURE_COND (pend-park inline pend)")
           ...
           + IF(_.op_zw,
               x86("mov",  RDQ("r12", 0), "rcx")   /* ⭐ ZW-3 R12-FREE-1 REVERSAL (s23l): r12 IS the live CAS top under the canonical frame. */
             + x86("mov",  "esi", "eax")
             ...
             + x86("add",  "r12", (long)24))
           + IF(!_.op_zw,
               x86("mov",  "r10", ABSQ(RT_CAS_TOP))   /* old regime: cell-resident top */
           ...
           + x86_gamma()
           + x86_beta()
           + IF(_.op_zw,  x86("sub",  "r12", (long)24))
           + IF(!_.op_zw, x86("mov",  "rax", ABSQ(RT_CAS_TOP))
                        + x86("sub",  "rax", (long)24)
