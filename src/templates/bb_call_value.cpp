...
std::string bb_call_value() {
    ...
    if (_.op_off < 0 || _.op_sa < 0) return x86("def", "α") + x86_bomb("bb_call_value: needs own slot + callee operand slot");
    int n = _.op_arg_slot_n;
    for (int i = 0; i < n; i++)
        if (_.op_arg_slot[i] < 0)
            return x86("def", "α") + x86_bomb("bb_call_value: argument slot unfilled");
