...
std::string bb_assign_global() {
    ...
    return IF(g_gva_active && _.op_gva_k >= 0,
            ...
            + IF(vfc(),  x86("mov", "rax", ZTOS(_.op_fc_disp)) + x86("mov", "rdx", ZTOS(_.op_fc_disp + 8)) + IF(!stf(), x86("add", "rsp", (long)16)))
         ...
         + IF(!(g_gva_active && _.op_gva_k >= 0),
            ...
            + IF(vfc(),  x86("mov", "rsi", ZTOS(_.op_fc_disp)) + x86("mov", "rdx", ZTOS(_.op_fc_disp + 8)) + IF(!stf(), x86("add", "rsp", (long)16)))
            + x86("mov",    "rdi", ROQ(0))
