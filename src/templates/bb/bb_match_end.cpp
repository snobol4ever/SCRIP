...
static std::string release_pump() {
    return std::string()
         ...
         + x86("cmp",  "rax", 1L)
         + x86("jbe",  L(2))
         + x86("comment", "IR_MATCH_END capture pump: the box enters the target through the tree's ONE entry glue, bb_glue_enter_c2bb -- the pushed wire pair, or the alpha face's five-quad tiny record {nargs=0, gamma, omega, 0, 16} at rcx with rsp restored by the face")
