...
#include "x86_asm.h"
static int sub_val_on(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_SUB_VAL"); v = (e && *e == '0') ? 0 : 1; } return v; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
std::string bb_subscript() {
    ...
    const char * sub_sym = vctx ? "rt_subscript_val" : (conly ? (_.op_strict ? "rt_subscript_var_container_only_strict" : "rt_subscript_var_container_only") : (_.op_strict ? "rt_subscript_var_strict" : "rt_subscript_var"));
    if (_.op_zres)
