        ...
        return stage_arg_inline(i, slot, stage_fp);
         ...
         + (bcps_pl()
            ? x86("comment", "PL omega-LANDING (rung 2): the callee always releases on concede, so rsp is back at the wire pair -- pop pair plus pad, clear the re-entry token so a later beta cannot re-enter a dead frame, and fail forward.")
              ...
              + x86("jmp", L(2))
            : bcps_wire_land(_.op_sval)
         + x86("comment", "CEO-483 (hq_U): pop the L(7) word, and the PL-CALL-ALIGN pad above it ONLY where one was pushed -- the generator regime no longer pushes one (its alignment word moved across the call into the callee`s carve), so this landing pops 8 there and 16 everywhere else. A landing that pops a word its own call site never pushed walks rsp over the caller`s live spine.")
