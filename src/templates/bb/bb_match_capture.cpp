...
std::string bb_match_capture() {
    ...
    return (_.op_off < 0)
         ...
         ? ( x86("comment", "IR_MATCH_CAPTURE_IMM")
           ...
           + x86("test", "rax", "rax")
           + x86("je",   L(1))
           + x86("comment", "IR_MATCH_CAPTURE_IMM computed-name target: rt_cap_open parked the matched slice and returned rax = the target's entry (0 = assigned already) and rdx = its protocol; the box enters it through the tree's ONE entry glue and lands on rt_cap_land_γ/ω, which finish the assignment (Lon 2026-09-19: no BB is entered from C after the original invocation)")
