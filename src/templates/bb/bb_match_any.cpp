...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void an_ranges() { unsigned char in[256]; memset(in, 0, 256);
    for (const unsigned char * p = (const unsigned char *)(_.op_sval ? _.op_sval : ""); *p; p++) in[*p] = 1;
    an_rn = 0;
    for (int b = 0; b < 256; b++)
        if (in[b]) {
            if (an_rn && an_rhi[an_rn - 1] == b - 1) an_rhi[an_rn - 1] = (unsigned char)b;
            else { an_rlo[an_rn] = an_rhi[an_rn] = (unsigned char)b; an_rn++; }
        }
}
static std::string an_rtest(long i) {
    return an_rlo[i] == an_rhi[i]
         ? x86("cmp", "esi", (long)an_rlo[i])
         + x86("je",  L(0))
         : x86("mov", "eax", "esi")
         + x86("sub", "eax", (long)an_rlo[i])
         + x86("cmp", "eax", (long)(an_rhi[i] - an_rlo[i]))
         + x86("jbe", L(0));
}
...
static std::string an_ndl_rsi() {
    return _.op_sa >= 0
         ? x86("mov", "rsi", XSAQ(8))
         + x86("mov", "edx", XSAD(4))
         : x86("lea", "rsi", "[rip + __]", (uint64_t)(uintptr_t)(_.op_sval ? _.op_sval : ""), an_nlb)
         + x86("mov32", "edx", CSK());
}
