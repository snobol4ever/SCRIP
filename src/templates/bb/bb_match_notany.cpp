...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void na_ranges() { unsigned char in[256]; memset(in, 0, 256);
    for (const unsigned char * p = (const unsigned char *)(_.op_sval ? _.op_sval : ""); *p; p++) in[*p] = 1;
    na_rn = 0;
    for (int b = 0; b < 256; b++)
        if (in[b]) {
            if (na_rn && na_rhi[na_rn - 1] == b - 1) na_rhi[na_rn - 1] = (unsigned char)b;
            else { na_rlo[na_rn] = na_rhi[na_rn] = (unsigned char)b; na_rn++; }
        }
}
static std::string na_rtest(long i) {
    return na_rlo[i] == na_rhi[i]
         ? x86("cmp", "esi", (long)na_rlo[i]) + x86_omega("je")
         : x86("mov", "eax", "esi")
         + x86("sub", "eax", (long)na_rlo[i])
         + x86("cmp", "eax", (long)(na_rhi[i] - na_rlo[i]))
         + x86_omega("jbe");
}
...
static std::string na_ndl_rsi() {
    return _.op_sa >= 0
         ? x86("mov", "rsi", XSAQ(8))
         + x86("mov", "edx", XSAD(4))
         : x86("lea", "rsi", "[rip + __]", (uint64_t)(uintptr_t)(_.op_sval ? _.op_sval : ""), na_nlb)
         + x86("mov32", "edx", CSK());
}
