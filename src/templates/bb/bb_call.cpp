...
static std::string bb_call_byname_str(IR_t * pBB) {
    ...
    if (_.op_zres) {
        uint64_t fptr_bl; { DESCR_t (*fp)(const char *, DESCR_t *, int, int) = _.op_sn4 ? rt_call_arr_bl_sn4 : _.op_strict ? rt_call_arr_bl_strict : rt_call_arr_bl; fptr_bl = (uint64_t)(uintptr_t)(void*)fp; }
        ...
        s += x86("call", (_.op_sn4 ? "rt_call_arr_bl_sn4" : _.op_strict ? "rt_call_arr_bl_strict" : "rt_call_arr_bl"), fptr_bl);
    ...
    if (dfp) {
        ...
        s += x86("rtcc_wb");
        s += x86("mov32", "ecx", bid_bake_of(fn));
        s += x86("call_bare", (_.op_sn4 ? "rt_call_arr_bl_sn4" : _.op_strict ? "rt_call_arr_bl_strict" : "rt_call_arr_bl"), fptr_bl);
        s += x86("rtcc_rl");
    }
