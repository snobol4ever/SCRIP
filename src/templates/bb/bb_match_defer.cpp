...
std::string bb_match_defer() {
    ...
    return x86("comment", "IR_MATCH_DEFER (ZS-2 jmp-entry)")
         ...
         + IF(merged,
             ...
             + x86("mov", "ecx", (long)DT_I) + x86("test", "rax", "rax") + x86("jz", L(241)) + x86("mov", "ecx", (long)DT_P) + x86("def", L(241)) + x86_rt_gc_poll_rec_defer("ecx", 0)
             + x86_anchor_leave()
             + x86_xfer_leave())
         ...
         + x86("comment", "ARCH-GC section 3: a poll at the EMITTED return of this allocating runtime call, in the spelling the primitive road already uses (bb_glue_flat.cpp) -- the shield record carries r13:r15d, the pattern subject, as ONE tagged cell, and the packed spine word in rdx is saved across the poll because the poll speaks the argument registers")
         + x86("mov", "ecx", (long)DT_I) + x86("cmp", "rdx", 4L) + x86("jne", L(242)) + x86("mov", "ecx", (long)DT_P) + x86("def", L(242)) + x86_rt_gc_poll_rec_defer("ecx", 1)
         + x86("def",  "L2")
         + x86("test", "rax", "rax")
         + x86("je",   "L3")
         + x86("cmp",  "rdx", 4L)
         ...
         + x86("comment", "ARCH-GC section 3: a poll at the EMITTED return of this allocating runtime call, in the spelling the primitive road already uses (bb_glue_flat.cpp) -- the shield record carries r13:r15d, the pattern subject, as ONE tagged cell, and the packed spine word in rdx is saved across the poll because the poll speaks the argument registers")
         + x86("mov", "ecx", (long)DT_I) + x86("cmp", "rdx", 4L) + x86("jne", L(243)) + x86("mov", "ecx", (long)DT_P) + x86("def", L(243)) + x86_rt_gc_poll_rec_defer("ecx", 1)
         + x86("jmp",  "L2")
         + x86("def",  L(47))
         + x86("call", "rt_defer_land_ω", (uint64_t)(uintptr_t)(void *)(rt_dcap_next_t (*)(long))rt_defer_land_ω)
         + x86("comment", "ARCH-GC section 3: a poll at the EMITTED return of this allocating runtime call, in the spelling the primitive road already uses (bb_glue_flat.cpp) -- the shield record carries r13:r15d, the pattern subject, as ONE tagged cell, and the packed spine word in rdx is saved across the poll because the poll speaks the argument registers")
         + x86("mov", "ecx", (long)DT_I) + x86("cmp", "rdx", 4L) + x86("jne", L(244)) + x86("mov", "ecx", (long)DT_P) + x86("def", L(244)) + x86_rt_gc_poll_rec_defer("ecx", 1)
         + x86("jmp",  "L2")
         + x86("def",  L(40))
         + x86_xfer_leave()
         + x86("mov",  "rdx", "rax")
