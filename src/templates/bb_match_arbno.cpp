...
#include "x86_asm.h"
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ZB-ITER-3 (s85): zv() = the chain arm's ELEMENT VIEW register — the s83-measured rbp borrow, statement-bracketed by the head's gated +40 save (bb_match_begin.cpp:24, flat_deep_arrival).  Decoupling the
...
std::string bb_match_arbno() {
    ...
    if (_.op_zw && _.op_zw_blob_base >= 0 && !_.op_tail && !_.op_arbno_dt && !_.op_arbno_chain && _.op_off >= 0 && _.op_sa >= 0 && _.op_sb > 0)   /* ⭐ r9/wire (OMEGA s35): canonical-frame NARY arm.  ARBNO header (op_off+{0,4,8,16}) is a RUN MEMBER slot -- r9-relative with no blob_base subtraction (ZWR/ZWRQ).  View-register slots (op_sa-N) are heap-element-relative via pinned rbp -- keep FR/FRQ.  r9 is SysV caller-saved: re-establish after every C call (x86_align_enter/leave wrappers clobber it). */
    return x86("comment", "IR_MATCH_ARBNO_NARY (r9/wire ZW)")
         + x86_alpha()
         + x86("lea", "r9", RDQ("rbp", -(long)ZW_FRAME_K))
         + x86("mov", ZWR(_.op_off), "r14d")
         + x86("mov", ZWR(_.op_off + 4), "r14d")
         + x86("mov", ZWR(_.op_off + 8), 0L)
         + x86_gamma()
         + x86_beta()
         + x86("lea", "r9", RDQ("rbp", -(long)ZW_FRAME_K))
         + x86("mov", "r14d", ZWR(_.op_off + 4))
         + x86("lea", "rdi", ZWRQ(_.op_off + 16))
         + x86("lea", "rsi", ZWRQ(_.op_off + 12))
         + x86("mov", "edx", ZWR(_.op_off + 8))
         ...
         + x86("def", PAIR(2))
         + x86("lea", "r9", RDQ("rbp", -(long)ZW_FRAME_K))
         + x86("mov", "rax", ZWRQ(_.op_off + 16))
         + x86("mov", "ecx", ZWR(_.op_off + 8))
         ...
         + x86("lea", "r9", RDQ("rbp", -(long)ZW_FRAME_K))
         + x86("mov", "eax", ZWR(_.op_off + 8))
         + x86("add", "eax", 1L)
         + x86("mov", ZWR(_.op_off + 8), "eax")
         + x86("mov", ZWR(_.op_off + 4), "r14d")
         + x86_gamma()
         + x86("def", PAIR(3))
         + x86("lea", "r9", RDQ("rbp", -(long)ZW_FRAME_K))
