...
static int rpin(void) { return _.flat_deep_arrival && hpin() && (subjc() || _.op_zres || !_.flat_jmp_entry); }   /* REPL-PIN predicate (this session), ONE authority for pin/publication/save-complement/subject-respell: the s22z population (subjc||zres) pins EXACTLY as before, and the legacy-subject population -- replacement statements, fence-denied by their second subject consumer -- joins ONLY on !flat_jmp_entry graphs.  jmp-entry chains (DEFINE bodies via rt_goto_transfer, EVAL fragments, blobs) arrive at runtime-variable depth where hoff's compile-time rsp spelling is invalid -- measured on roman: the recursive body crashed at 3 iterations under the unconditioned widening, LP jmp=1 vs the fixed population's jmp=0 -- so they keep the pre-session regime verbatim until the r9/wire-carried claim base (CARRIED-OPEN) retires the compile-time spelling itself.  Degrade never die. */
