...
std::string bb_match_head() {
    ...
    return ((_.op_sa < 0 && !subjc()) || _.op_off < 0)
         ...
         + IF(stfh(), x86("sub", "rsp", (long)48))   /* SUBJ-ARM-2: the head's LOCAL STORAGE, one instruction (Lon: "each BB allocates its LOCAL STORAGE... by one instruction, decrement RSP"); the mark below captures rsp AFTER this carve, so the S10e unwind returns here and the rbp-slot fields survive to RELEASE and the failure exit */
