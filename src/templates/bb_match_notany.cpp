...
static std::string na_ndl_r8()  { return _.op_sa >= 0 ? x86("mov", "r8",  XSAQ(8)) + x86("mov", "ecx", XSAD(4)) : x86("lea", "r8",  "[rip + __]", (uint64_t)(uintptr_t)(_.op_sval ? _.op_sval : ""), na_nlb) + x86("mov32", "ecx", CSK()); }   /* len in ecx: r9 is the live GVA claim (RC-5), never template scratch; rcx is dead here (subject byte already loaded) */
static std::string na_ndl_rsi() { return _.op_sa >= 0 ? x86("mov", "rsi", XSAQ(8)) + x86("mov", "edx", XSAD(4)) : x86("lea", "rsi", "[rip + __]", (uint64_t)(uintptr_t)(_.op_sval ? _.op_sval : ""), na_nlb) + x86("mov32", "edx", CSK()); }
...
std::string bb_match_notany() {
    ...
    return x86("comment", "IR_MATCH_NOTANY")
         ...
         + IF(na_tablep(),
           ...
           + (sn4_cset32() ? x86("bt", "[rdi]", "esi") + x86_omega("jc") : x86("cmpb0", "[rdi+rsi]", "0") + x86_omega("jne")))
         + x86("add",    "r14d", (long)1)
         + x86_gamma()
         + x86_beta()
         + x86("sub",    "r14d", (long)1)
