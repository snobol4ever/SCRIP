...
std::string bb_var_ref() {
    ...
    if (_.op_off < 0) return x86_alpha() + x86_bomb("bb_var_ref: needs own slot");
    if (_.op_gva_k >= 0)
    ...
    if (_.op_sa >= 0)
        return x86("comment", "IR_VAR_REF local")
             ...
             + x86_beta()
             + x86_omega();
    return x86_alpha() + x86_bomb("bb_var_ref: variable has no addressable cell (NV-fallback global?) — tvsubs needs a GVA/frame cell");
}
