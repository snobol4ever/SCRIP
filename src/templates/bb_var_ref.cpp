...
std::string bb_var_ref() {
    ...
    if (PLATFORM_X86)
        return _.op_off < 0 ? x86_alpha() + x86_bomb("bb_var_ref: needs own slot")
             : _.op_gva_k < 0 && _.op_sa < 0 ? x86_alpha() + x86_bomb(strdup((std::string("bb_var_ref: variable '") + (_.op_sval ? _.op_sval : "?") + "' has no addressable cell (NV-fallback global?) — needs a GVA/frame cell").c_str()))
