...
inline const char * x86_fr32_prefix() { return x86_fb_data() ? "dword ptr [rbp + " : "dword ptr [rsp + "; }   /* ZW-0 stage 2: island r12 arm deleted */
inline const char * x86_fr64_prefix() { return x86_fb_data() ? "qword ptr [rbp + " : "qword ptr [rsp + "; }   /* ZW-0 stage 2: island r12 arm deleted */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
inline const char * ZWB(int off) { static char b[8][48]; static int i; i=(i+1)&7; snprintf(b[i],48,"dword ptr [r9 + %d]", off - _.op_zw_blob_base); return b[i]; }   /* ⭐ r9/wire (OMEGA s35): blob-template op_zw arm dword accessor.  r9 = blob claim base = rbp - ZW_FRAME_K (depth-immune under canonical frame; blob template sets via `lea r9,[rbp-ZW_FRAME_K]` at its alpha).  off - op_zw_blob_base = slot offset from the base of the blob's claim region.  Replaces FR(off) spellings that were shift-fragile under the canonical frame's ZW_FRAME_K insertion.  Only valid when op_zw && op_zw_blob_base >= 0. */
inline const char * ZWBQ(int off) { static char b[8][48]; static int i; i=(i+1)&7; snprintf(b[i],48,"qword ptr [r9 + %d]", off - _.op_zw_blob_base); return b[i]; }   /* ⭐ r9/wire (OMEGA s35): qword twin of ZWB.  Replaces FRQ(off) spellings in blob-template op_zw arms. */
inline const char * ZWR(int off) { static char b[8][48]; static int i; i=(i+1)&7; snprintf(b[i],48,"dword ptr [r9 + %d]", off); return b[i]; }   /* ⭐ r9/wire (OMEGA s35): run-member slot dword accessor.  Unlike ZWB, NO blob_base subtraction -- op_off for run members (e.g. ARBNO header) is already relative to the claim base (= r9).  Use for FR(op_off+N) accesses where the node is a run member, not a blob closure member. */
inline const char * ZWRQ(int off) { static char b[8][48]; static int i; i=(i+1)&7; snprintf(b[i],48,"qword ptr [r9 + %d]", off); return b[i]; }   /* ⭐ r9/wire (OMEGA s35): qword twin of ZWR. */
