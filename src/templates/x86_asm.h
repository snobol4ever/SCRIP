...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
inline const char * FR(int off) { static char b[8][40]; static int i; i = (i + 1) & 7; if (x86_fc_hit(off)) snprintf(b[i], 40, "dword ptr [rsp + %d]", off - _.op_fc_base); else snprintf(b[i], 40, "%s%d]", x86_fr32_prefix(), off); return b[i]; }   /* FLATDISP-9 (2026-07-29): RAW off -- the operand STRING must not pre-add op_flat_disp because x86_parse re-derives the offset and the frame encoders (x86_frame_modrm / x86_frame_text_mem, the 363 single-owner) add the compensation; FR adding it too made cross-depth flat reads carry 2x disp on unpinned graphs (f3 witness: coerce wrote flat 208 at disp 0, span read 216+64+64=344).  Per-box pairs self-cancelled, which is how it stayed latent.  REG-7 U3: op_flat_disp compensation DELETED — rbp is depth-immune (seeded U1/U2/s79); the fc_hit granted-window rebase stays rsp BY DESIGN (element LIFO arithmetic, R12-EXIT-1) */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
inline const char * FRQ(int off) { static char b[8][40]; static int i; i = (i + 1) & 7; if (x86_fc_hit(off)) snprintf(b[i], 40, "qword ptr [rsp + %d]", off - _.op_fc_base); else snprintf(b[i], 40, "%s%d]", x86_fr64_prefix(), off); return b[i]; }   /* FLATDISP-9: same as FR -- raw off, encoders are the one adder.  REG-7 U3: same as FR — compensation deleted, fc_hit stays rsp */
inline const char * FRQB(int off, int bump) { static char b[8][40]; static int i; i = (i + 1) & 7; if (x86_fc_hit(off)) snprintf(b[i], 40, "qword ptr [rsp + %d]", off - _.op_fc_base + bump); else snprintf(b[i], 40, "%s%d]", x86_fr64_prefix(), off + (x86_fb_data() ? 0 : bump)); return b[i]; }   /* FLATDISP-LIVE-BUMP (the 083 define-cluster root): FRQ read while a TEMPLATE-LOCAL rsp bump is live (the scc save block's sub rsp,K) -- the rsp-relative spellings add the live bump; a PINNED rbp does NOT move under that sub, so FLATDISP-8 (s197) suppresses the bump there or the read would be displaced by a carve it never felt.  This is precisely the "under the old always-seeded rbp frame the non-window read was depth-immune" case the original note recorded, now live again per-graph rather than as deleted history.  The offset rides the STRING, so x86_parse's XK_FR64 re-derivation keeps BINARY==TEXT with zero encoder-side change (R10). */
