 ...
 * choice unchanged. */
inline const char * x86_zr()         { return x86_zc_frame() == ZC_FRAME_RSP ? "rsp" : "rbp"; }   /* ZW-0 stage 2: island "r12" arm deleted; REG-7 U5 SEAL: ONE stream — under RSP zr IS rsp */
inline int          x86_zr_num()     { return x86_zc_frame() == ZC_FRAME_RSP ? 4 : 5; }   /* ZW-0 stage 2: island 12 arm deleted; ζ basis set CLOSED at rsp(4)/rbp(5) */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
inline int x86_zop_regime(int off) { if (x86_zstorage() == ZC_STORAGE_FRAME_R12) return 1; if (x86_fc_hit(off)) return 2; return x86_fb_data() ? 3 : 4; }   /* ZOP-1 (ZW-0 stage 2: x86_isle() conjunct deleted -- arm 1 reachable only via ZC_STORAGE_FRAME_R12 now); regime: 1=r12 island, 2=own per-BB cell window on rsp, 3=pinned rbp, 4=rsp+op_flat_disp */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
inline const char * RBPRAWQ(int off) { static char b[8][48]; static int i; i = (i + 1) & 7; snprintf(b[i], 48, "qword ptr [rbp# + %d]", off); return b[i]; }   /* ⭐ ARB-LON-K16: RAW machine-rbp qword operand, runtime offset -- the sanctioned [rbp# + N] escape (x86_parse s23o rule), NEVER the plain [rbp + N] spelling which the fr-prefix classifier would capture and re-canonicalize under op_stmt_dyn.  Consumers: the mechanism-2 dyn whack + old_rbp restore at the γ-jmp zgpop hook. */
...
inline const char * FRQB(int off, int bump) { return x86_zop(off, 1, bump); }   /* ZOP-1: qword + LIVE TEMPLATE BUMP (the scc save-block case).  The bump suppression under a pinned rbp (FLATDISP-8) now lives once, inside x86_zop, instead of being spelled only here -- which is why FRQB and FRQ could previously disagree about depth without anything naming the disagreement. */
...
inline std::string x86_port_hook(int site, int port) {
    ...
    if (site == X86H_DEF && port == X86P_ALPHA && _.op_uclaim > 0 && x86_port_mode() == ZC_PORT_FORTH) {   /* ZW-0 stage 2: !x86_isle() deleted -- island never live at ZC_FRAME_RSP default */
        s += x86_sub("rsp", _.op_uclaim);   /* UCLAIM (wholesale flip, Lon 2026-08-01): the statement's flat-extent claim, carved ONCE at the single-entry run head's alpha, BEFORE the box's own fc cell so the cell stays at TOS as every template expects.  Deliberately NOT op_fc_bytes: the claim must not feed op_zdepth (it backs the legacy FRQ spellings, resolved by zvo_resolve, not intra-box depth) and must not ride the hook's omega auto-release (statement-scope: the staged op_zgpop/op_wpop terminal cuts below are the sole release).  Beta re-entries carve nothing -- the claim is live through every backtrack by design. */
        { static int _cz = -1; if (_cz < 0) { const char * e = getenv("SCRIP_CLAIM_ZERO"); _cz = (e && *e == '0') ? 0 : 1; }
          if (_cz) for (long _zi = 0; _zi < _.op_uclaim; _zi += 8) s += x86("note", "stmt_claim") + x86("mov", x86_zref((int)_zi, 1), (long)0); } }   /* ⭐ ON-5 FIX (s23d): the destination is spelled RAW via x86_zref -- the sanctioned [rsp# + N] escape -- because plain RDQ("rsp",N) text is RE-RESOLVED at encode time by x86_frame_off->zvo_resolve against the UCLAIM owner table, which rebases the raw claim offsets as if they were flat offsets and COLLAPSES the upper ones onto the lower cells.  MEASURED at HEAD before the fix: a 240-byte claim emitted 30 stores over only 26 distinct cells (dups 0/8/16/24, max 200) so the TOP 32 BYTES were never written, and larger claims were worse -- 62 stores/40 distinct and 78/56, 22 collapsed cells each.  This is the ARGREAD hazard documented at x86_asm.h:874, same remedy.  CLAIM-ZERO now actually discharges the zero-fresh contract rt_cap_push depends on.  ⭐ CLAIM-ZERO (s23a, ZC_INIT_ZERO made live for the claim class): the whole-graph carve this claim replaces was ZERO-FRESH at program start (main-stack virgin pages), and rt_cap_push's slot contract is BUILT on that -- "gen 0 != any live gen" validates ZC_INIT_ZERO-fresh cells, buf==0 takes the alloc path.  A bare sub rsp hands the slots PRIOR STATEMENTS' RESIDUE: the moment PIN-REBASE moved the 066 cap slot from envp-luck into the claim, s->buf read nonzero garbage, skipped the alloc, and dereferenced it (rt_cap_push+130 SEGV).  ONE zero pass per statement EXECUTION at the single-entry head, so within an execution every alpha re-entry (cursor loop, backtrack beta cycles) sees persistent slots exactly as the carve world did, and across executions the gen stamp does its lazy kill against a DETERMINISTIC zero instead of nondeterministic residue.  Unrolled qword stores (K/8, statement claims are small); ZC_INIT is the ratified default this finally implements for a live class. */
    if (x86_fc_on()) {
    ...
    if (site == X86H_JMP && port == X86P_GAMMA && _.op_zgpop > 0) s += (_.op_stmt_dyn && _.op_stmt_pin > 0) ? x86("note", "dyn_whack") + x86("lea", "rsp", RBPRAWQ((int)_.op_zgpop)) + x86("note", HKN(0)) + x86("mov", "rbp", RBPRAWQ(_.op_stmt_pin)) : IF(_.op_stmt_pin > 0, x86("note", HKN(0)) + x86("mov", "rbp", RDQ("rbp", _.op_stmt_pin))) + x86_add("rsp", (long)_.op_zgpop);   /* ⭐ ARB-LON-K16 (op_stmt_dyn): a statement whose extent is indeterminable (FRAMELESS_K ARBNO growth) releases via mechanism-2 off the stmt_base -- lea rsp,[rbp+K] == pre-claim frontier at ANY depth (rbp==claim base, the [rbp+pin] old_rbp reads are the standing proof); old_rbp restore ORDERED AFTER the lea (lea reads rbp, does not clobber it).  Legacy add arm verbatim otherwise -- killswitch-inert byte identity. */   /* ZD-1 STATEMENT-TERMINAL GAMMA RELEASE (Lon s21x-v "ONLY statement level scoping"): the success-path twin of the op_wpop arm below -- when this gamma edge crosses a statement boundary, every ZD cell the statement suspended is dead, and this ONE add returns rsp to statement entry before the jmp lands on the next head.  Fires only at jmp-gamma (flags dead before an unconditional transfer; conditional gammas that need it arrive through the x86_jcc invert synth exactly as conditional omegas do), staged by zd_plan, zero when the edge stays inside the statement.  ⭐ HEAD-PIN (s22z): when the statement's match head pinned rbp to the region base (op_stmt_pin = the old-rbp slot's region offset), the restore rides HERE -- the sole release authority -- read depth-free through the still-pinned rbp BEFORE the pop; the exhaust/release tails' own +40 restores are gated off under the pin so exactly one authority restores on every exit path, including the direct af/β cuts that bypass those tails entirely (the 061 m3 spin witness). */
    ...
    if (site == X86H_JMP && port == X86P_OMEGA && _.op_wpop > 0) s += IF(_.op_stmt_pin > 0 && _.op_wterm, x86("note", HKN(0)) + x86("mov", "rbp", RDQ("rbp", _.op_stmt_pin))) + x86_add("rsp", (long)_.op_wpop);   /* ⭐ HEAD-PIN (s22z): restore gated on op_wterm -- ONLY the planner's statement-terminal wpop (zd_wp>0 at the choke, "restores to statement entry" by ZD-1's definition) carries the pin restore; a trampoline-ΣK-only fold is mid-statement and restoring there would corrupt the live pin for every later [rbp+off] reader of the same statement. */
