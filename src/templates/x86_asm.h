...
inline int x86_rsp_slide_known() { return !(emit_rec_pin() || _.op_stmt_dyn); }   /* SLIDING-RSP AUTHORITY: the offset is knowable iff no dynamic-depth class */
...
inline const char * x86_fr32_prefix() { return _.op_stmt_dyn ? "dword ptr [fb5# + " : x86_fb_data() ? "dword ptr [fb5 + " : "dword ptr [rsp + "; }   /* ZW-0 stage 2: island r12 arm deleted */
inline const char * x86_fr64_prefix() { return _.op_stmt_dyn ? "qword ptr [fb5# + " : x86_fb_data() ? "qword ptr [fb5 + " : "qword ptr [rsp + "; }   /* ZW-0 stage 2: island r12 arm deleted */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
inline const char * FB5RAWQ(int off) { static char b[8][48]; static int i; i = (i + 1) & 7; snprintf(b[i], 48, "qword ptr [fb5# + %d]", off); return b[i]; }   /* ⭐ ARB-LON-K16: RAW machine-___ qword operand, runtime offset -- the sanctioned [___# + N] escape (x86_parse s23o rule), NEVER the plain [___ + N] spelling which the fr-prefix classifier would capture and re-canonicalize under op_stmt_dyn.  Consumers: the mechanism-2 dyn whack + old____ restore at the γ-jmp zgpop hook. */
...
inline void x86_parse(const xop & x, opnd & o) {
    ...
    if (!strncmp(s, "dword ptr [fb5# + ", 18)) { o.kind = XK_REGDISP32; o.base[0] = 'r'; o.base[1] = 'b'; o.base[2] = 'p'; o.base[3] = 0; o.off = atoi(s + 18); return; }   /* s23o ___ TWIN of the rsp# escape: on a PINNED graph the fr prefixes are the plain "[___ + " spellings, so a RAW machine-___ slot (the SPD-2 retry protocol's kt-32/kt-40) parses XK_FR* and routes the frame encoders -- and a mnemonic WITHOUT an FR arm (cmp) then silently emitted NOTHING, the ZB-FC-1 drop class, caught only by the .s region diff.  [___# + N] names the raw register; both encoders re-canonicalize to standard [___ + N] in both media (R10). */
    if (!strncmp(s, "qword ptr [fb5# + ", 18)) { o.kind = XK_REGDISP; o.base[0] = 'r'; o.base[1] = 'b'; o.base[2] = 'p'; o.base[3] = 0; o.off = atoi(s + 18); return; }
