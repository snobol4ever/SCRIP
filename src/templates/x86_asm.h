...
inline const char * ZREF(int reg_off, int d, int w) {   /* ⭐⭐⭐⭐⭐ ζ-ONE U-1 — THE ONE ZETA ADDRESS.  Lon s136: "properly calculate and use offset from RSP and RBP based on standing, activation frames, and spine cells."  THE PLANNER DECIDES, NOT THE TEMPLATE: tier and offset come from zzone_tier_of/zzone_off_of (lattice verdict + registry offset, one traversal), so a template can no longer hold an opinion about WHICH BASE its operand lives on -- which is precisely how the planner and the template came to disagree about one operand with nothing able to detect it.  ⛔ STANDING and ACTIVATION are BOTH rbp (Lon's s81 ruling: "RBP for ZETA ACTIVATION FRAMES, and RBP for ZETA STANDING AREA") -- they differ in LIFETIME, not in base, so they share this arm and the tier is carried for the census, not for the encoding.  SPINE is rsp through x86_zop, the existing ONE OPERAND ADDRESS AUTHORITY, untouched. */
    ...
    if (zzone_on()) { int po = zzone_off_of_cur(); if (po != -1) { off = po; tier = zzone_tier_of_cur(); } }   /* the planner OVERRIDES only where it has a slot; -1 means no rbp home and the spine arm below is the answer, which is also the legacy answer -- that is what makes OFF and ON coincide until a real disagreement exists to expose. */
    (void)tier;
