...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
inline int x86_zop_regime(int off) { if (x86_zstorage() == ZC_STORAGE_FRAME_R12 || x86_isle()) return 1; if (x86_fc_hit(off)) return 2; return x86_fb_data() ? 3 : 4; }   /* ZOP-1: the operand regime this reference resolves to -- 1=r12 island whole-graph frame, 2=own per-BB cell window on rsp, 3=pinned rbp whole-graph frame, 4=rsp whole-graph frame + op_flat_disp compensation.  NAMING the arms is the point: before this rung the same four outcomes were the cartesian product of FIVE independent booleans (x86_fc_hit / x86_isle / x86_fb_pinned / flat_fb_refine+op_fb_rbp / the encoder-side op_flat_disp gate), i.e. 32 representable states per node of which four are coherent, decided PER NODE inside one graph.  That combinatorial space -- not any single arm -- is the "struggling for unknown reasons" of s21x-j..m: every depth change re-routes some node into a different arm and the compensations mis-fire in a new place. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
inline const char * ZRESN() { static char b[8][48]; static int i; i = (i + 1) & 7; const char * n = bb_kind_name(_.op_node_kind); snprintf(b[i], 48, "%s", n ? n : ""); return b[i]; }   /* OBJ-NOTE ON-3 (Lon s23c): the SELF name -- the box's OWN result cell, spelled exactly as its n<uid>_<kind> label, so a ZRES store reading '[rsp+112]  # deref' names the object it lands in.  op_node_kind is staged at emit.cpp's SINGLE dispatch point for EVERY node, so this is universal with zero per-template plumbing.  ROTATING buffer (the RDQ/ABSQ idiom) because bb_kind_name's own buffer is a SINGLE static -- two differing kind names alive in one '+' chain would otherwise collapse onto the last writer.  NULL-safe: x86("note",...) renders empty on a null/empty name. */
...
inline std::string x86_port_hook(int site, int port) {
    ...
    if (site == X86H_DEF && port == X86P_ALPHA && _.op_uclaim > 0 && x86_port_mode() == ZC_PORT_FORTH && !x86_isle()) {
    ...
    if (site == X86H_JMP && port == X86P_OMEGA && _.op_wpop > 0) s += IF(_.op_stmt_pin > 0 && _.op_wterm, x86("mov", "rbp", RDQ("rbp", _.op_stmt_pin))) + x86_add("rsp", (long)_.op_wpop);   /* ⭐ HEAD-PIN (s22z): restore gated on op_wterm -- ONLY the planner's statement-terminal wpop (zd_wp>0 at the choke, "restores to statement entry" by ZD-1's definition) carries the pin restore; a trampoline-ΣK-only fold is mid-statement and restoring there would corrupt the live pin for every later [rbp+off] reader of the same statement. */
