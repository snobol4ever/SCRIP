...
inline const char * FRQB(int off, int bump) { static char b[8][40]; static int i; i = (i + 1) & 7; if (x86_fc_hit(off)) snprintf(b[i], 40, "qword ptr [rsp + %d]", off - _.op_fc_base + bump); else snprintf(b[i], 40, "%s%d]", x86_fr64_prefix(), x86_frame_off(off) + bump); return b[i]; }   /* FLATDISP-LIVE-BUMP (this session, the 083 define-cluster root): FRQ read while a TEMPLATE-LOCAL rsp bump is live (the scc save block's sub rsp,K) -- both spellings are rsp-relative since the depth-static regime, so BOTH branches add the live bump; under the old always-seeded rbp frame the non-window read was depth-immune and the fallback never needed it, which is exactly why only the fc_hit arm carried the compensation.  The offset rides the STRING, so x86_parse's XK_FR64 re-derivation keeps BINARY==TEXT with zero encoder-side change (R10). */
