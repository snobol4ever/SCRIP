...
static inline int x86_fb_stmt_on() { static int m = -1; if (m < 0) { const char * e = getenv("SCRIP_FB_STMT"); m = (e && *e == '1') ? 1 : 0; } return m; }   /* FB-STMT OPT-IN (SCRIP_FB_STMT=1): default OFF after same-session falsification — the "statement exits rebalance rsp" premise (058 law) is FALSE on two measured paths: (a) ARBNO hit-path element retention (interior carves survive γ by design, t1.s; 180_pat_arbno_defer_nonrecursive SEGVs on the statement AFTER a successful arbno — its refined rsp+disp reads land below the retained carves; the free-on-final-success rung banked at 3ea60fc2 is this same fact from the other side); (b) DEFER/VALUE-alt backtrack re-pumping a retained blob (072_pat_star_var_alt_backtrack, same session).  RE-ARM LICENSE: monitor-first on 180 and 072, then either narrow the allowed deep-kind set per emit_fb_stmt_scan or extend the bit map to everything downstream of a retaining statement until its release_to provably sweeps.  Under the flag, the four pinned SN4 benchmarks' value refs drop 1015→220 (survivors = match-family housekeeping only) with all four outputs correct — the win is real, the license is not yet. */
