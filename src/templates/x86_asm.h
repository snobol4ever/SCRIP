...
inline const char * x86_zop(int off, int q, int bump) {
    ...
    else { snprintf(b[i], 48, "%s%d]", q ? x86_fr64_prefix() : x86_fr32_prefix(), off + ((x86_fb_data() || _.op_stmt_dyn) ? 0 : bump)); }   /* ⭐ C-4-FIX (FINDING-08-07h, s6): mech2 blob non-fc_on nodes: push_rbp shifts rsp by -8 before sub rsp,Kc, so blob slots designed at [rsp+8+off]=[α_base-Kc+off] need +8. Gate: regime-4 (rsp base, !fb_data), mech2 active (op_mech2_kc>0), no own FORTH cell (!x86_fc_on — own-cell nodes use regime-2 window or TOS, not this arm). Byte-identical when op_mech2_kc==0 (_m2=0). */
    return b[i];
