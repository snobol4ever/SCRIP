...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ⛔⭐ RTCC CALLEE CLASS (s65, Lon directive: "DO NOT do the dance when the rt_* routine is an ASM optimized by RTX GOAL ... categorize and do not do what is unnecessary").                             */
/* The veneer exists to carry the CLAIMED tier {r8,r9,r10,r11} across a crossing that would otherwise destroy it.  A callee that PROVABLY CANNOT disturb a slot needs no round trip for that slot, and a  */
/* callee that cannot disturb ANY of them needs no veneer at all.  The mask below names, per symbol, which claimed registers the callee may disturb.  UNLISTED => RTCC_C_ALL: an unknown callee is a C     */
/* body by assumption, so the default is the full dance and a symbol that is renamed or added simply keeps today's behaviour instead of silently losing its guard.                                        */
/* ⛔ THE ENTRIES ARE VALID ONLY BECAUSE THEIR GATES ARE GONE.  Every listed symbol was an RTX_GATE'd entry whose gate-off arm tail-jumped into a C twin; while that arm existed the register discipline   */
/* was a RUNTIME property (SCRIP_RTX_<FAM>=0 flipped it) and no compile-time claim about it could be sound.  The gate + twin deletion in this same rung is what makes these masks STATIC TRUTH.  Do NOT    */
/* add a row for a symbol that still carries a gate or still delegates to C on any path -- that is the H2 class rebuilt in a new place, and it fails as a wild jump, not as a wrong answer.                */
/* MEASURED, not asserted: destination-position writes to r8/r9/r10/r11 across every RTX_FUNC body, plus every call/jmp leaving the body.  ⛔ Scan the .S with a UNICODE-aware pattern -- two entries are  */
/* spelled with Greek port names (rt_gen_spine_pass_γ/ω) and an [A-Za-z0-9_] census cannot see them.                                                                                                      */
#define RTCC_C_R8   1u
#define RTCC_C_R9   2u
...
static inline std::string x86_rtcc_wb_bin(uint64_t block, unsigned m = RTCC_C_ALL) {
    ...
    if (m & RTCC_C_R8)  { wb += (char)0x4C; wb += (char)0x89; wb += (char)0x40; wb += (char)40; }   /* mov [rax+40], r8   (R8  slot 5) */
    if ((m & RTCC_C_R9) && !RTCC_GLOBAL_R9_GVA) { wb += (char)0x4C; wb += (char)0x89; wb += (char)0x48; wb += (char)48; }   /* mov [rax+48], r9 (R9 slot 6) -- SKIPPED under the GVA claim: see H2 note above x86_rtcc_wb_bin */
    if (m & RTCC_C_R10) { wb += (char)0x4C; wb += (char)0x89; wb += (char)0x50; wb += (char)56; }   /* mov [rax+56], r10  (R10 slot 7) */
    if (m & RTCC_C_R11) { wb += (char)0x4C; wb += (char)0x89; wb += (char)0x58; wb += (char)64; }   /* mov [rax+64], r11  (R11 slot 8) */
...
static inline std::string x86_rtcc_rl_bin(uint64_t block, unsigned m = RTCC_C_ALL) {
    ...
    if (m & RTCC_C_R8)  { rl += (char)0x4D; rl += (char)0x8B; rl += (char)0x43; rl += (char)40; }   /* mov r8,  [r11+40]  */
    if (m & RTCC_C_R9)  { rl += (char)0x4D; rl += (char)0x8B; rl += (char)0x4B; rl += (char)48; }   /* mov r9,  [r11+48]  */
    if (m & RTCC_C_R10) { rl += (char)0x4D; rl += (char)0x8B; rl += (char)0x53; rl += (char)56; }   /* mov r10, [r11+56]  */
    if (m & RTCC_C_R11) { rl += (char)0x4D; rl += (char)0x8B; rl += (char)0x5B; rl += (char)64; }   /* mov r11, [r11+64]  -- r11 LAST: it is the block base above */
    return rl;
}
...
inline const char * XSAQ(int d) { return _.op_zread_xf[0] != -1 ? RDQ("rbp", _.op_zread_xf[0] + d) : FRQ(_.op_sa + d); }   /* ⭐⭐⭐ PF-1d (s178-b, FINDING s178-FW) THE ONE OPERAND-SEAT ACCESSOR for the leaf family's op_sa road (SPAN/BREAK/BREAKX/ANY/NOTANY/TAB/RTAB -- ALL members identical, the no-per-op-filter law; the LFC/s131 one-accessor precedent).  THE DEFECT IT CLOSES: an ARM-INTERIOR leaf is ungranted by zd_plan (per-RUN grants; nested-ALT arms unclaimed by the descent) and falls to FRQ(op_sa) -- a slot in the whole-graph flat frame CARVE-KILL deleted and, since the s97 dead-spill deletion, A SLOT NOBODY WRITES (measured fw_min_inline: SPAN's cset ptr read [rsp+392], written nowhere, garbage r8 deref = the SEGV/nomatch layout-wild pair).  When the producer is frame-homed (op_zread_xf[0] staged from xop_frame_slot, the PF-1c authority) the seat reads the SAME rbp slot the producer's ZRES wrote; -1 = the legacy spelling byte-identically. */
inline const char * XSAD(int d) { return _.op_zread_xf[0] != -1 ? RDD("rbp", _.op_zread_xf[0] + d) : FR(_.op_sa + d); }
inline const char * zone_ref(int rbp_off, int spine_base, int d, int w) { return (rbp_off != -1) ? ((w == 8) ? RDQ("rbp", rbp_off + d) : RDD("rbp", rbp_off + d)) : ((w == 8) ? FRQ(spine_base + d) : FR(spine_base + d)); }   /* ⭐⭐⭐⭐ ζ-ONE U-1 — THE ONE RSP/RBP TIER DISPATCHER (Lon ruling s132: "one function, one data structure, one pass" to end the whack-a-mole).  THE SOLE PLACE A TIER DECISION IS SPELLED: rbp_off != -1 ⇒ an RBP tier (STANDING / ACTIVATION), depth-IMMUNE, the encoder's ONE compensation term stays and no pricing delta can double-add (the s129 re-home-never-a-delta law); rbp_off == -1 ⇒ the ζ-SPINE arm through FR/FRQ, i.e. x86_frame_off, which remains THE ONE OFFSET FUNCTION for rsp (x86_asm.h:583) and is NOT bypassed here but DELEGATED TO.  ⛔ THIS IS A GENERALISATION OF A MEASURED PROTOTYPE, NOT A NEW AUTHORITY: LFC/LFCQ (s131) already had exactly these two arms for ONE of the four frame_slot_scan customers (ARBNO / CAPTURE-SAVE / FENCE1 / leaf), which ALREADY share one slot numbering -- so the registry was unified in NUMBERING while the ACCESSOR stayed per-family, and a fact spelled four times is the s68/s70 spelled-twice disease with three chances to drift.  U-1 lands the dispatcher with LFC/LFCQ re-expressed on top and EVERY caller byte-identical by construction (same callee, same order, same RDD/RDQ rotating-buffer index); U-3 folds the remaining three customers plus blob_frame_bytes and sn4_choice_rbp_off through it, U-2 adds the RECORD-FIELD tier that ends per-template record layout assumptions (the s132 deferclob corpse: IR_MATCH_DEFER β = bare `jmp [rsp]` three lines from IR_MATCH_ALTERNATE β = `[rsp+8]`, two private layouts on one shared region).  ⛔ THE 8-BYTE USABLE-WINDOW LAW STILL BINDS the leaf family's callers (d ∈ {0,4}); it is a per-CLASS property enforced in the planner, not a property of this dispatcher, and it RETIRES at U-3 when the standing carve is re-derived. */
inline int LFC_ON() { return _.op_leaf_frame_off != -1; }   /* ⭐⭐⭐ THE ONE LEAF-SUSPENSION CELL ARM PREDICATE (s131, generalising s130's file-static spf()).  Staged by leaf_frame_slot() at the leaf drive cases; -1 == every legacy path, byte-identical. */
...
enum { ZSP_SCRATCH = 0, ZSP_RAW = 1 };   /* ζ-ONE U-4 (s139): THE REFUSE-SPELLING AXIS.  ZSP_SCRATCH = FR/FRQ(x86_scratch_off+d), the leaf family's flat ZLS coordinate; ZSP_RAW = bare [rsp+d], the ALT choice record's own-carved spelling.  Orthogonal to both the TIER (which base the planner picked) and the CUSTOMER (which cell of that base) -- it answers only \"how does THIS customer address the spine when it has no rbp home\", which was the one fact ZREFC hardwired and therefore the one fact that kept a whole customer outside the instrument. */
inline const char * ZREFS(int reg_off, int d, int w, int customer, int spine);
enum { ZC_LEAF = 0, ZC_ARBNO = 1, ZC_CAPTURE = 2, ZC_FENCE = 3, ZC_CHOICE = 4 };   /* ζ-ONE U-3 (s138): the template-side spelling of emit.h's ZCUS_* customer axis.  Kept numerically identical BY CONSTRUCTION and never re-ordered -- the two enums are ONE fact crossing the emit.h:635 no-IR_t-in-templates boundary, and the bridge passes the raw int. */
inline int zzone_on() { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZONE"); v = (e && *e == '1') ? 1 : 0; } return v; }   /* ⭐⭐⭐⭐⭐ ζ-ONE U-1 (s136) killswitch, DEFAULT OFF and byte-identical: OFF, ZREF falls through to the caller-supplied offset exactly as the four hand-wired call sites did, so the whole rung lands inert and flips under one env var after its own sweep (the R-7/s124 land-OFF protocol). */
inline const char * ZREFC(int reg_off, int d, int w, int customer) {   /* ⭐⭐⭐⭐⭐ ζ-ONE U-3 — THE ONE ZETA ADDRESS, NOW (NODE, CUSTOMER) KEYED (s138).  Lon s136 and again s138: "properly calculate and use offsets from RSP and RBP based on standing, activation frames, and spine cells."  U-1 landed the dispatcher but the planner could supply only the TIER, so the OFFSET still came from whichever staged field the call site happened to pass -- five customers, five private spellings, and no way for the planner and the template to be checked against each other.  THIS CLOSES IT: the planner resolves (current node, customer) through zzone_off_cur_for -> zzone_off_for -> the five registry authorities, at emit time where g_emit_cfg is live (s137: the blocker was SEQUENCING, never purity).  ⛔ STANDING and ACTIVATION are BOTH rbp (Lon s81: "RBP for ZETA ACTIVATION FRAMES, and RBP for ZETA STANDING AREA") -- they differ in LIFETIME, not base, so they share this arm and the tier is carried for the census, not the encoding.  SPINE is rsp through FR/FRQ, i.e. x86_frame_off, THE ONE OFFSET FUNCTION for rsp, delegated to and never bypassed. */
    return ZREFS(reg_off, d, w, customer, ZSP_SCRATCH);
}
inline const char * ZREFS(int reg_off, int d, int w, int customer, int spine) {   /* ⭐⭐⭐⭐⭐ ζ-ONE U-4 (s139) — THE SPINE-SPELLING AXIS, and the reason the checker was BLIND to the one class this goal calls the M1 root cause.  U-3 routed five customers through ONE dispatcher, but a customer whose registry REFUSES (-1) could only be spelled ONE way on the spine: FR/FRQ(x86_scratch_off+d), the leaf family's coordinate.  The ALT choice record's refused arm is a DIFFERENT spelling -- raw [rsp+d] -- so bb_match_alternate could not call the accessor at all on that arm and guarded OUTSIDE it (`cro ? CROQ(..) : RSP(..)`).  MEASURED CONSEQUENCE: sn4_choice_rbp_off() returns 0 for every blob in beauty, so ALL 100+ choice-record accesses took the unrouted arm and the s138 census reported ZERO CHOICE entries -- an instrument silent on the class named R-0 (\"ALT-arm-interior capture has no home\").  ⛔ THE AXIS IS THE REFUSE SPELLING, NOT A TIER: the tier says WHICH BASE the planner chose, this says HOW THIS CUSTOMER SPELLS THE SPINE when there is no rbp home -- and conflating them is what would re-base a raw [rsp] record onto the leaf scratch coordinate, which is a DIFFERENT ADDRESS. */
    int off = reg_off, tier = 2;
    if (zzone_on()) {
        tier = zzone_tier_of_cur();
        int planned = zzone_off_cur_for(customer);
        zzone_disagree(customer, off, planned);   /* CALLED AT EVERY RESOLUTION, NOT ONLY ON MISMATCH -- the callee decides what to report.  ⭐ THIS IS THE NON-VACUITY FIX, and it is the s135 lesson made structural: that seat read fourteen zeros as agreement from an instrument whose breakpoint never resolved.  An instrument that is silent both when it agrees and when it is never reached cannot be told apart from a dead one, so the census mode (SCRIP_ZONE_BOMB=2) exists to make "reached and agreed" a POSITIVE observation. */   /* ⭐⭐⭐ THE DISPLACEMENT CHECK THE s136 PROBE COULD NOT PERFORM, and the reason this rung is a bug-finding instrument and not merely cleanup.  The every-port probe checks the BASE and is blind to an operand banked at one offset and read at another; a planner/template disagreement about the SAME (node, customer) is exactly that class, caught statically at every address site in both media.  Logs only under SCRIP_ZONE_BOMB=1. */
        if (planned != -1) off = planned;   /* THE PLANNER WINS WHERE IT HAS A SLOT.  -1 = no rbp home, and the spine arm below is then the answer -- which is also the legacy answer, so ON and OFF coincide until a genuine disagreement exists to expose.  This is what makes the flip measurable instead of a leap. */
    }
    (void)tier;
    if (off != -1) return (w == 8) ? RDQ("rbp", off + d) : RDD("rbp", off + d);
    if (spine == ZSP_RAW) return (w == 8) ? RDQ("rsp", d) : RDD("rsp", d);   /* ⛔ BYTE-IDENTICAL BY CONSTRUCTION AND THAT IS THE POINT: RSP(d) is `RDQ("rsp", d)` verbatim (x86_asm.h:1424) and the dword site already spelled `RDD("rsp", d)` by hand, so routing the refused choice record here changes NOTHING it emits -- it only makes the access VISIBLE to zzone_disagree.  The carve/release pair (`sub rsp,32` / `add rsp,32`) is NOT an address and stays guarded on cro in the template, because that one IS a behavioural difference between the arms. */
    return (w == 8) ? FRQ(_.x86_scratch_off + d) : FR(_.x86_scratch_off + d);
}
inline const char * ZREF(int reg_off, int d, int w) { return ZREFC(reg_off, d, w, ZC_LEAF); }   /* ζ-ONE U-1's signature preserved so its two existing callers (LFC/LFCQ) stay byte-identical by construction; the leaf customer is what they always meant. */
inline const char * LFC(int d)  { return ZREF(_.op_leaf_frame_off, d, 4); }   /* ζ-ONE U-1: re-expressed on the ONE dispatcher; both arms unchanged (RDD/rbp | FR/spine), byte-identical by construction. */    /* ⭐⭐⭐ THE ONE LEAF-SUSPENSION CELL ACCESSOR, dword (s131; s130 proved it on SPAN alone as the file-static SPC).  ⛔ SITED HERE, NOT PER TEMPLATE, UNDER THE ONE-AUTHORITY LAW: the scratch-cell leaf family is EIGHT boxes (SPAN/BREAK/BREAKX/TAB/RTAB/REM/ARB/BAL) in eight files, and a fact spelled eight times is the s68/s70 spelled-twice disease with seven chances to drift -- one file left on the legacy arm is a HALF-HOMED family, which is exactly the cross-owner overwrite this rung exists to kill.  LEGACY ARM: FR(x86_scratch_off + d), a RAW FLAT ZLS COORDINATE resolved against whatever rsp happens to be, because these boxes carve NOTHING at their own α and x86_frame_off's single op_zdepth term therefore has no own-carve to compensate.  On the ordinary spine that address has a granted owner and is correct; INSIDE AN ALT ARM it has no owner in EITHER medium (zd_plan grants per RUN; the arm interior is the s66/s71 ungranted-arm denial class).  FRAME ARM: rbp-relative and therefore DEPTH-IMMUNE, so the encoder keeps its ONE compensation term and no pricing delta exists to double-add -- a re-home, never a delta (the s129 warning).  ⛔ THE 8-BYTE USABLE-WINDOW LAW BINDS EVERY CALLER: a slot offers d ∈ {0,4} and NOTHING MORE -- d=8 is the NEXT granule's floor, i.e. the neighbouring ARBNO/CAPTURE-SAVE/FENCE1 cell.  Any box needing 12B or 16B (measured s131: BAL spends +0/+4/+8; SPAN's `*var` arm spends the {ptr,len} pair) is REFUSED IN THE PLANNER by leaf_frame_member() and keeps its legacy spelling until the two-consecutive-slot rung grants it a wider granule. */
inline const char * LFCQ(int d) { return ZREF(_.op_leaf_frame_off, d, 8); }   /* ζ-ONE U-1: qword twin through the same dispatcher; same two arms, same law, same 8-byte window. */   /* ⭐ qword twin of LFC -- same two arms, same law, same 8-byte window. */
inline const char * AFC(int d)  { return ZREFC(_.op_arbno_frame_off, d, 4, ZC_ARBNO); }   /* ⭐⭐⭐ ζ-ONE U-3 (s138): THE ARBNO ζ-CELL, dword.  Was five hand-spelled RDD("rbp", off+0|+4) in bb_match_arbno.cpp reading a staged field the planner could not see; now the {Δ0@+0, yield@+4} pair addresses through the ONE dispatcher and the planner's own answer for (this ARBNO, ZC_ARBNO) is checked against it at every site. */
inline const char * AFCQ(int d) { return ZREFC(_.op_arbno_frame_off, d, 8, ZC_ARBNO); }   /* ζ-ONE U-3: qword twin, same customer, same law. */
inline const char * CFC(int d)  { return ZREFC(_.op_cap_frame_off, d, 4, ZC_CAPTURE); }   /* ⭐⭐⭐ ζ-ONE U-3 (s138): THE CAPTURE-SAVE ζ-CELL, dword.  SAVE writes and COND/IMM read the SAME assigned slot (capture_frame_slot resolves both to the SAVE), so routing all three through one customer key is what makes producer and consumer checkable against each other rather than merely conventionally equal. */
inline const char * CFCQ(int d) { return ZREFC(_.op_cap_frame_off, d, 8, ZC_CAPTURE); }   /* ζ-ONE U-3: qword twin. */
inline const char * FFC(int d)  { return ZREFC(_.op_fence_frame_off, d, 4, ZC_FENCE); }   /* ⭐⭐⭐ ζ-ONE U-3 (s138): THE FENCE1 WATERMARK ζ-CELL, dword.  SPITBOL Ch.18 p.204: FENCE "succeeds when the scanner is moving left to right in the pattern, but fails if the scanner has to back up through it" -- the watermark it compares on the way back is precisely a cell whose address must survive an arbitrary interior carve, which is why it was a registry customer before it was an accessor. */
inline const char * FFCQ(int d) { return ZREFC(_.op_fence_frame_off, d, 8, ZC_FENCE); }   /* ζ-ONE U-3: qword twin -- the watermark quad's own spelling. */
inline const char * CROQ(int cro, int d) { return ZREFS(cro ? cro : -1, d, 8, ZC_CHOICE, ZSP_RAW); }   /* ⭐⭐⭐ ζ-ONE U-3 (s138): THE ALT CHOICE-RECORD ζ-CELL, qword.  ⛔ THE SENTINEL NORMALISATION IS THE POINT: sn4_choice_rbp_off spells "no rbp home" as 0 while the other four producers spell it -1, and a fact with two spellings is the s68/s70 disease -- one of them was always going to be tested with the wrong comparison.  Normalised here at the single point of consumption so ZREFC has exactly ONE refuse test for all five customers. */
inline const char * CROD(int cro, int d) { return ZREFS(cro ? cro : -1, d, 4, ZC_CHOICE, ZSP_RAW); }   /* ζ-ONE U-3: dword twin -- the record's Δ dword at +0. */
...
inline std::string x86_4col(const std::string & s) {
    ...
    while (i < n) {
        ...
        if (join) { int pw = x86_disp_w(o.data() + pls, o.size() - pls);   /* ⛔ FIT TEST (s63, same discipline as s62's GOTO 68→78: a column that is overrun is a MEASURED DEFECT, not taste).  A join that cannot */
            if (pend == 1) { if (pw >= 24) join = 0; }                     /* reach its column lands ragged one-or-two past it, so REFUSE it: an over-long label keeps its own line and the instruction beneath   */
...
inline std::string x86_arbno_rbp_unwind_at(const std::string & mark_mem, int l_loop, int l_done) { return x86("note", "arbno_rbp_unwind") + x86("mov", "rax", mark_mem) + x86("def", L(l_loop)) + x86("cmp", "rbp", "rax") + x86("jae", L(l_done)) + x86("test", "rbp", "rbp") + x86("je", L(l_done)) + x86("mov", "rbp", RDQ("rbp", 24)) + x86("jmp", L(l_loop)) + x86("def", L(l_done)); }   /* ⭐ FENCE-COMMIT WALK (Lon 2026-08-14, in-chat: gamma FENCE'd success has the same problem as final teardown): the CORE of the unwind, taking the MARK as a pre-formed memory operand so exits whose banked mark is frame-relative (FENCE1's FRQ watermark) can call THE SAME loop — the (base,off) wrapper below forwards here, so the walk stays spelled ONCE. */
inline std::string x86_arbno_rbp_unwind(const char * mark_base, int mark_off, int l_loop, int l_done) { return x86_arbno_rbp_unwind_at(RDQ(mark_base, mark_off), l_loop, l_done); }   /* ⭐ ARBNO-RBP EPILOGUE (E-1, Lon's correction 2026-08-14: "At the end of pattern match ALL RBP are put back in place"): THE ONE AUTHORITY for walking the ARBNO rbp chain home at a match exit.  s71 pushed rbp at every ARBNO α but restored it on ONE exit only (ARBNO's own exhaustion ω, lea rsp,[rbp+24]/pop rbp), so a match that SUCCEEDS or FAILS with an interior ARBNO still suspended leaves rbp pointing into a frame whose memory the exit's rsp whack has just released — and the next legitimate rbp reader (the FN_RBP RETURN floater, mov rsp,rbp/pop rbp) tears down through it.  MEASURED at this HEAD on probe/arbdef1.sno: m4 rc139, gdb rip=0x0 rbp=0x0 with n19_match_end_α live on the stack — the floater set rsp from the dead frame, popped a NULL γ and jmp'd to it.  THE INVARIANT THAT MAKES THE WALK SOUND: every rbp move inside a match is a push at an ARBNO α, and the pushed value lives at [rbp+24] of the frame that pushed it (exactly what ω's lea rsp,[rbp+24] reads), so the chain is complete and LIFO by construction; ARBNO frames are carved DURING the match hence always BELOW the match's banked entry rsp, so the first link at/above MARK is the pre-match rbp.  CALL IT BEFORE THE EXIT'S rsp WHACK, never after: after the whack the frames sit below rsp where a signal handler may smash them mid-walk.  MARK is whatever the exit already holds and is both >= every ARBNO frame and <= the pre-match rbp — [r8+8] at MATCH_END (the value about to become rsp), [r12+8] at the MATCH_BEGIN fail tail.  rax is the scratch: every caller sits immediately after a cas sentinel scan that already leaves rax dead (it exits holding the tag 0).  ⛔ THE NULL TERMINATOR IS LOAD-BEARING, NOT BELT-AND-BRACES: x86_main_prologue() is EMPTY (s52 RBP-EMISSION-EVAPORATION), so a top-level match inherits whatever rbp the CRT left — and the x86-64 ABI leaves rbp=0 at process entry, which is BELOW every MARK, so a jae-only loop would dereference [0+24] and fault.  Stopping at 0 also RESTORES the correct value there, since 0 was the true pre-match rbp.  Zero ARBNO frames live ⇒ the loop body never runs (2 insns + a not-taken branch), so non-ARBNO matches on qualifying graphs pay only the compare.  Composed ENTIRELY of pre-verified encoders per TEMPLATE-ONLY EMISSION; the note is the grep detector (x86("comment") does NOT survive into the .s — the s71 false-exoneration trap). */
...
inline std::string x86_port_hook(int site, int port) {
     ...
     * op-filtered trivial-β trampoline the driver's ω-wire chase inlined past (flat_trivial_beta, emit.cpp)
...
inline std::string x86_zsm_ev(int kind) {   /* ⭐ THE ONE EMITTED FORM for all three ports.  rdx must carry the TRUE rsp at the port -- the value BEFORE this instrumentation touched the stack -- so the addend is 8*(pushes above it); the pushes and the `add rdx,64` ARE ONE FACT AND MOVE TOGETHER, the same law bb_zdp_anchor.cpp already states for its own three.  ⛔ s179 REENTRANCY CURE (measured, ZSM-ALL audit): the old form used x86("call") = the RTCC veneer, which banks r10/r11 in the GLOBAL rtccb block -- non-reentrant, so an event firing inside emitted code that a C road re-entered (the fn/EVAL/CODE fragment roads) overwrote the OUTER road's banked wires and the instrument FLIPPED VERDICTS (52/106 ptc board under ZSM_ALL; fwctx ctl match->nomatch).  New form: bank the caller-saved VM tier r8..r11 on the STACK (reentrant by construction -- the PF-2 stack-pair argument in miniature) and call BARE.  Eight pushes keep the old form's rsp mod-16 parity.  ⛔⭐ s184 RAX-BANK, THE OTHER HALF OF THE s179 TRADE (measured, ZSM-ALL perturbation row): the s179 cure swapped x86("call") for x86("call_bare") to escape the GLOBAL rtccb, and in doing so escaped the veneer's PROTECTION TOO.  x86_rtcc_call's BINARY stub is `movabs r10,ptr / call r10` and it COMPENSATES for its own trampoline (`m |= RTCC_C_R10`), and an unknown callee gets RTCC_C_ALL, so the veneer form banked all nine GPRs -- rax included.  x86_call_ro's bare BINARY stub is `movabs rax,ptr / call rax` (48 B8 imm64 / FF D0) and NOTHING compensates: rax is destroyed AT THE CALL SITE, before rt_zdp_ev's own `push rax` can bank it.  Every other call_bare customer is a REAL call whose result comes back in rax, so the clobber is invisible to them; the ZSM event is the only one contracted to be INVISIBLE to the program, which is why it alone was convicted.  MEDIUM-ASYMMETRIC and therefore invisible to an asm diff taken in TEXT: the TEXT arm emits `call rt_zdp_ev@PLT` and touches no register, so mode 4 never showed it.  Measured: 87/896 programs answered differently under SCRIP_ZSM_ALL=1 before this bank, beauty among them (`Parse Error` where the shipped default SIGSEGVs); rax banked TWICE so the mod-16 parity above is unchanged, and the rdx addend moves with the pushes as this header already requires. */
