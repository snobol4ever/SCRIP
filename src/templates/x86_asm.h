...
inline const char * ZREFS(int reg_off, int d, int w, int customer, int spine) {   /* ⭐⭐⭐⭐⭐ ζ-ONE U-4 (s139) — THE SPINE-SPELLING AXIS, and the reason the checker was BLIND to the one class this goal calls the M1 root cause.  U-3 routed five customers through ONE dispatcher, but a customer whose registry DECLINES (-1) could only be spelled ONE way on the spine: FR/FRQ(x86_scratch_off+d), the leaf family's coordinate.  The ALT choice record's declined arm is a DIFFERENT spelling -- raw [rsp+d] -- so bb_match_alternate could not call the accessor at all on that arm and guarded OUTSIDE it (`cro ? CROQ(..) : RSP(..)`).  MEASURED CONSEQUENCE: sn4_choice_rbp_off() returns 0 for every blob in beauty, so ALL 100+ choice-record accesses took the unrouted arm and the s138 census reported ZERO CHOICE entries -- an instrument silent on the class named R-0 (\"ALT-arm-interior capture has no home\").  ⛔ THE AXIS IS THE DECLINE SPELLING, NOT A TIER: the tier says WHICH BASE the planner chose, this says HOW THIS CUSTOMER SPELLS THE SPINE when there is no rbp home -- and conflating them is what would re-base a raw [rsp] record onto the leaf scratch coordinate, which is a DIFFERENT ADDRESS. */
    ...
    if (spine == ZSP_RAW) return (w == 8) ? RDQ("rsp", d) : RDD("rsp", d);   /* ⛔ BYTE-IDENTICAL BY CONSTRUCTION AND THAT IS THE POINT: RSP(d) is `RDQ("rsp", d)` verbatim (x86_asm.h:1424) and the dword site already spelled `RDD("rsp", d)` by hand, so routing the declined choice record here changes NOTHING it emits -- it only makes the access VISIBLE to zzone_disagree.  The carve/release pair (`sub rsp,32` / `add rsp,32`) is NOT an address and stays guarded on cro in the template, because that one IS a behavioural difference between the arms. */
