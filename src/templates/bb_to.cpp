    ...
    if (PLATFORM_X86)
        if (_.op_zres)
            ...
            return x86("comment", "IR_TO zd")
                 ...
                 + x86("mov",   FRQ(_.op_off + 16), "rax")   /* counter init: FRQ=[rbp+op_off+16], depth-immune at α and β */
                 + x86("note",  ZOPN(1)) + x86("mov", "rdi", ZOPQ(1, 0))
                 + x86("note",  ZOPN(1)) + x86("mov", "rsi", ZOPQ(1, 8))
                 + x86("call",  "to_int", (uint64_t)(uintptr_t)(void*)to_int)
                 + x86("mov",   FRQ(_.op_off + 24), "rax")   /* limit init: FRQ=[rbp+op_off+24], depth-immune at α and β; mirrors flat arm layout */
                 + x86("def",   L(0))
