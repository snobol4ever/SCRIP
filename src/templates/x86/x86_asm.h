...
inline std::string x86_port_hook(int site, int port) {
    ...
    if ((site == X86H_JMP || site == X86H_JCC) && g_emit.flat_lcl_proc && g_emit.flat_lbl_γ && x86_portname(port) && strcmp(x86_portname(port), g_emit.flat_lbl_γ) == 0 && getenv("SCRIP_PORT_EXIT_AUDIT"))
