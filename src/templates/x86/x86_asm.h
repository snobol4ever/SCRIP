...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
inline int icn_gen_host_reserved(void) {   /* ⛔⭐ N-2 ITEM 3 (hq_P s282): DID **THIS** HOST ACTUALLY RESERVE? -- the question icn_gen_host_reserve_offset() was answering by assumption. icn_gen_host_reserve() is called from exactly ONE place (emit.cpp:2852) and that place sits inside the THIRD arm of a three-arm prologue chain: `if (zframe_graph)` :2829 / `else if (icn_genframe2() && flat_gen)` :2832 / `else if (flat_lcl_proc)` :2845. A graph taking either of the first two arms carves NOTHING for its generator callees -- and a graph can be flat_gen AND flat_lcl_proc at once (emit.cpp:3535-3536 sets flat_lcl_proc under `_gfr && icn_cells_graph`), in which case the flat_gen arm wins because it is tested first. ⛔ SO `flat_lcl_proc` ALONE IS NOT THE PREDICATE, AND USING IT ALONE WOULD BE WRONG FOR EXACTLY THE GRAPHS THIS ROW IS ABOUT. ⛔⛔ THIS MIRRORS AN else-if CHAIN IN ANOTHER TRANSLATION UNIT AND THERE IS NO COMPILER CHECK THAT IT STILL DOES: if a fourth arm is added before :2845, or the reserve call moves, THIS PREDICATE MUST MOVE WITH IT. test_icn_n2_host_reserved_agrees.sh is the canary -- it compares this predicate against the carve actually emitted, and REFUSES rather than passes when it observes no generator host at all. */
    if (g_emit.zframe_graph) return 0;
    if (icn_genframe2() && g_emit.flat_gen) return 0;
