...
static std::string release_pump() {
    return std::string()
         ...
         + IF(ZC_FRAME != ZC_FRAME_RSP, x86("push", x86_zr())
                                      + x86("sub",  "rsp", 8L))
         + x86_lea_id("rcx", 3)
         + x86_lea_id("rdx", 4)
...
std::string bb_match_release() {
    ...
    return !PLATFORM_X86 ? std::string()
         ...
         + x86_alpha()
         + IF(ZC_FRAME == ZC_FRAME_RSP, (rfc() ? x86("mov", "rax", RSP((int)_.op_fc_disp + 8))
                                               : x86("mov", "rax", FRQ(_.op_off + 8)))
             + x86("lea", "rcx", "[rip + __]", (uint64_t)(uintptr_t)(const void *)&g_patstk_sp, "g_patstk_sp")
             + x86("mov", RDQ("rcx", 0), "rax"))
         + (_.op_dval != 0.0
                ? IF(rfc(), x86("mov", "eax", RDD("rsp", (int)_.op_fc_disp))
                          + (ZC_FRAME == ZC_FRAME_RSP ? x86("mov", RDD("rsp", (int)(_.op_off + _.op_fc_disp + 32)), "eax")
                                                      : x86("mov", FR(_.op_off), "eax")))
                + (ZC_FRAME == ZC_FRAME_RSP && rfc() ? x86("mov", RSP((int)(_.op_off + 24 + _.op_fc_disp + 32)), "r14")
                                                     : x86("mov", FRQ(_.op_off + 24), "r14"))
                : std::string())
         + IF(ZC_FRAME != ZC_FRAME_RSP, IF(rfc(),  x86("mov",  "rdi", RSP((int)_.op_fc_disp + 8)))
             + x86_align_enter()
