 ...
 * slot = x86_scratch_off+8, K = 16): x86_alpha() bumps a per-activation arena frame (push+prev-chain+
 * save-slot store), x86_beta() restores the arena cursor to this activation's frame (release_to(block) —
 * backtrack arrival wholesale-frees every failed successor's frame, the fail-direction release), and
 * x86_omega() releases the frame itself when op_omega_is_death marks it a true chain-exit death (a
