        ...
        return x86_alpha()
             ...
             + ((scc_z || (_.op_sval && bb_tiny_shim_ok(_.op_sval, (int)_.op_ival)))
                ...
                ? [&]() -> std::string {
                    ...
                    static int _b1cz = -1; if (_b1cz < 0) { const char * _e = getenv("SCRIP_B1C_PARITY"); _b1cz = (_e && *_e == '1') ? 1 : 0; }   /* B1c (s168): D-18a seals alpha$ for every fragment proc, so the D-18b blanket decline is OVER-BROAD -- the slim/legacy fallback's pushed-landing protocol mismatches wire-exit callees (FINDING s168: rsp climbs past the stack top); DEFAULT OFF until the b1c-flip seat's A/B */
    ...
    return x86_alpha()
         ...
         + ((scc || (_.op_sval && bb_tiny_shim_ok(_.op_sval, (int)_.op_ival))) && !c2
            ...
            ? [&]() -> std::string {
                ...
                static int _b1ct = -1; if (_b1ct < 0) { const char * _e = getenv("SCRIP_B1C_PARITY"); _b1ct = (_e && *_e == '1') ? 1 : 0; }   /* B1c (s168): D-18a seals alpha$ for every fragment proc, so the D-18b blanket decline is OVER-BROAD -- the slim/legacy fallback's pushed-landing protocol mismatches wire-exit callees (FINDING s168: rsp climbs past the stack top); DEFAULT OFF until the b1c-flip seat's A/B */
