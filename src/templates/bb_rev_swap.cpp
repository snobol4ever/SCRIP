...
std::string bb_rev_swap() {
    return [&](long lk, long rk) {
        return !PLATFORM_X86 ? std::string()
             ...
             + x86_alpha()
             + IF(g_scan_regs_live != 0, x86("note", "scan_δ") + x86("mov", FRQ(_.op_off + 48), "r14")
                                       + x86("note", "scan_Δ") + x86("mov", FRQ(_.op_off + 56), "r15"))
             + x86("mov", "rdi", (long)lk)
             ...
             + (g_scan_regs_live != 0 ? x86("note", "scan_δ") + x86("lea", "r9", FRQ(_.op_off + 48))
                                      : x86("mov", "r9", (long)0))
