...
static std::string alt_entry_stubs(long N, int cro) {
    ...
    for (long j = 1; j < N; j++)
        r += x86("def", L((int)(20 + j)))
           ...
           + (cro ? x86("mov", RDQ("rbp", cro + 16), "rax") : x86("mov", RSP(16), "rax"))
           + x86("jmp", PAIR((int)j));
    return r;
}
static std::string alt_sigma_stubs(long N, int cro) {
    ...
    for (long j = 0; j < N; j++)
        r += x86("def", PAIR((int)(2 * N + 2 + j)))
           ...
           + (cro ? x86("mov", RDQ("rbp", cro + 8), "rax") : x86("mov", RSP(8), "rax"))
...
std::string bb_match_alternate() {
    ...
    return x86("comment", cro ? "IR_MATCH_ALT_NARY (ALT-RBP s128, frame-resident choice record)" : "IR_MATCH_ALT_NARY (ALT-FLAT, s61 own-carved record)")
             ...
             + x86_gamma()
             + x86_beta()
             + (cro ? x86("mov", "rax", RDQ("rbp", cro + 8)) : x86("mov", "rax", RSP(8)))
             + x86_jmp_reg("rax")
             + x86("def", PAIR((int)(2 * _.op_ival + 1)))
             + (cro ? x86("mov", "r14d", RDD("rbp", cro)) : x86("mov", "r14d", RDD("rsp", 0)))
             + (cro ? x86("mov", "rax", RDQ("rbp", cro + 16)) : x86("mov", "rax", RSP(16)))
             + x86_jmp_reg("rax")
             + x86("def", L(19))
