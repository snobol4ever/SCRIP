...
#line 1275 "pascal.y"
                                       { if ((yyvsp[0].node)) { if (pas_trace_enabled()) pnl_push((yyvsp[-2].list), mk_fnc1("__trace_stmt", ilit(pascal_get_lineno()))); pnl_push((yyvsp[-2].list), (yyvsp[0].node)); } (yyval.list) = (yyvsp[-2].list); }
#line 2887 "pascal.tab.c"
    break;

  case 82: /* statement_list: statement  */
#line 1276 "pascal.y"
                { PNodeList *l = pnl_new(); if ((yyvsp[0].node)) { if (pas_trace_enabled()) pnl_push(l, mk_fnc1("__trace_stmt", ilit(pascal_get_lineno()))); pnl_push(l, (yyvsp[0].node)); } (yyval.list) = l; }
...
#line 1318 "pascal.y"
        { if ((yyvsp[-2].node) && (yyvsp[-2].node)->t == TT_FNC && (yyvsp[-2].node)->n == 2 && (yyvsp[-2].node)->c[0] && (yyvsp[-2].node)->c[0]->v.sval && (!strcmp((yyvsp[-2].node)->c[0]->v.sval, "__pas_fbuf_get") || !strcmp((yyvsp[-2].node)->c[0]->v.sval, "__pas_tbuf_get"))) {
              ...
              if (pas_trace_enabled() && (yyvsp[-2].node) && (yyvsp[-2].node)->t == TT_VAR && (yyvsp[-2].node)->v.sval) {
                  tree_t *_tv = mk_fnc2("__trace_value", leaf_s(TT_QLIT, (yyvsp[-2].node)->v.sval), pas_trace_wrap_value(leaf_s(TT_VAR, (yyvsp[-2].node)->v.sval)));
