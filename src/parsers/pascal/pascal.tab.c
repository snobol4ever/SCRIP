...
{
       0,  1511,  1511,  1532,  1533,  1536,  1539,  1540,  1543,  1544,
    1545,  1546,  1547,  1550,  1551,  1554,  1555,  1557,  1558,  1559,
    1560,  1561,  1563,  1563,  1563,  1564,  1564,  1564,  1564,  1566,
    1567,  1569,  1571,  1572,  1573,  1574,  1575,  1575,  1576,  1577,
    1578,  1586,  1586,  1588,  1595,  1596,  1599,  1602,  1603,  1606,
    1607,  1610,  1611,  1612,  1615,  1616,  1619,  1620,  1623,  1624,
    1626,  1628,  1629,  1630,  1630,  1634,  1634,  1638,  1638,  1644,
    1647,  1648,  1651,  1652,  1655,  1656,  1657,  1658,  1661,  1662,
    1665,  1668,  1668,  1669,  1669,  1672,  1673,  1678,  1679,  1680,
    1681,  1682,  1683,  1684,  1685,  1686,  1687,  1688,  1691,  1692,
    1695,  1702,  1703,  1706,  1707,  1708,  1711,  1747,  1755,  1759,
    1760,  1763,  1764,  1767,  1770,  1775,  1776,  1779,  1779,  1790,
    1791,  1794,  1795,  1798,  1799,  1802,  1805,  1808,  1813,  1820,
    1823,  1824,  1831,  1832,  1833,  1834,  1835,  1836,  1837,  1838,
    1841,  1842,  1843,  1844,  1845,  1846,  1849,  1850,  1851,  1852,
    1853,  1854,  1857,  1863,  1864,  1865,  1866,  1867,  1868,  1869,
    1870,  1871,  1874,  1875,  1878,  1879
};
#endif
...
#line 1595 "pascal.y"
            { g_pas_pend_typename = ct_strdup((yyvsp[0].str)); g_pas_pend_isbool = pas_is_booltype((yyvsp[0].str)); g_pas_pend_istfile = pas_is_tfiletype((yyvsp[0].str)); g_pas_pend_ischar = !strcmp((yyvsp[0].str), "char") || !strcmp((yyvsp[0].str), "widechar"); const char *_pt = pas_ptrtype_target((yyvsp[0].str)); if (_pt) { g_pas_pend_ptrtarget = ct_strdup(_pt); (yyval.ival) = -3; } else { if (!strcmp((yyvsp[0].str), "char") || !strcmp((yyvsp[0].str), "widechar")) { (yyval.ival) = 255; } else if (pas_is_settype((yyvsp[0].str))) { (yyval.ival) = -2; } else { long long _eh = pas_enumtype_high((yyvsp[0].str)); long long _sh = pas_subtype_high((yyvsp[0].str)); long long _ah = pas_arrtype_high((yyvsp[0].str)); if (_eh >= 0) { (yyval.ival) = _eh; } else if (_sh >= 0) { (yyval.ival) = _sh; } else if (_ah >= 0) { if (g_pas_recbody_depth == 0 && pas_rectype_nf((yyvsp[0].str)) > 0) { pas_rectype_to_pend((yyvsp[0].str)); g_pas_pend_typename = ct_strdup((yyvsp[0].str)); } (yyval.ival) = _ah; } else { if (g_pas_recbody_depth == 0) pas_rectype_to_pend((yyvsp[0].str)); g_pas_pend_typename = ct_strdup((yyvsp[0].str)); (yyval.ival) = -1; } } } }
