    ...
    if (name && !strcmp(name, "fillchar") && args && args->count >= 5) {
        ...
        if (dst && dst->t == TT_VAR && dst->v.sval) {
            ...
            if (pas_array_high_get(dst->v.sval, &fhi)) {
                long long flo = pas_array_low(dst->v.sval); if (flo <= 0) flo = 1;
...
static struct { const char *name; int nvp; int vp[16]; } g_pas_funcs[256]; static int g_pas_nfunc;
static void pas_func_add(const char *name) { if (g_pas_nfunc < 256 && name) { g_pas_funcs[g_pas_nfunc].name = ct_strdup(name); g_pas_funcs[g_pas_nfunc].nvp = 0; g_pas_nfunc++; } }
...
static struct { const char *name; int nvp; int vp[16]; } g_pas_procs[256]; static int g_pas_nproc;
static void pas_proc_add(const char *name) { if (g_pas_nproc < 256 && name) { g_pas_procs[g_pas_nproc].name = ct_strdup(name); g_pas_procs[g_pas_nproc].nvp = 0; g_pas_nproc++; } }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static void pas_proc_vparams(const char *name, PNodeList *params) { if (!name || !params) return;
    for (int i = g_pas_nproc - 1; i >= 0; i--) if (g_pas_procs[i].name && !strcmp(g_pas_procs[i].name, name)) { g_pas_procs[i].nvp = pas_vparam_scan(params, g_pas_procs[i].vp); return; }
    for (int i = g_pas_nfunc - 1; i >= 0; i--) if (g_pas_funcs[i].name && !strcmp(g_pas_funcs[i].name, name)) { g_pas_funcs[i].nvp = pas_vparam_scan(params, g_pas_funcs[i].vp); return; } }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static void pas_enumtype_add(const char *n, long long h) { if (g_pas_nenum < 64 && n) { g_pas_enumtypes[g_pas_nenum].name = ct_strdup(n); g_pas_enumtypes[g_pas_nenum].high = h; g_pas_nenum++; } }
...
static void pas_settype_add(const char *name) { if (g_pas_nsettype < 64 && name) { g_pas_settypes[g_pas_nsettype++].name = ct_strdup(name); } }
...
{
       0,   955,   955,   971,   972,   975,   978,   979,   982,   983,
     984,   985,   986,   989,   990,   993,   994,   996,   997,   998,
     999,  1000,  1002,  1002,  1002,  1003,  1003,  1003,  1003,  1005,
    1006,  1008,  1010,  1011,  1012,  1013,  1014,  1014,  1015,  1016,
    1017,  1025,  1025,  1027,  1034,  1035,  1038,  1041,  1042,  1045,
    1046,  1049,  1050,  1051,  1054,  1055,  1058,  1059,  1062,  1063,
    1065,  1067,  1068,  1069,  1069,  1073,  1073,  1077,  1077,  1083,
    1086,  1087,  1090,  1091,  1094,  1095,  1096,  1097,  1100,  1101,
    1104,  1107,  1108,  1111,  1112,  1117,  1118,  1119,  1120,  1121,
    1122,  1123,  1124,  1125,  1126,  1127,  1130,  1131,  1134,  1140,
    1141,  1144,  1145,  1146,  1149,  1173,  1181,  1185,  1186,  1189,
    1190,  1193,  1196,  1201,  1202,  1205,  1205,  1215,  1216,  1219,
    1220,  1223,  1224,  1227,  1230,  1233,  1235,  1239,  1242,  1243,
    1250,  1251,  1252,  1253,  1254,  1255,  1256,  1257,  1260,  1261,
    1262,  1263,  1264,  1265,  1268,  1269,  1270,  1271,  1272,  1273,
    1276,  1282,  1283,  1284,  1285,  1286,  1287,  1288,  1289,  1290,
    1293,  1294,  1297,  1298
};
#endif
...
#line 1068 "pascal.y"
                                                                                            { pas_func_add((yyvsp[-7].str)); pas_proc_vparams((yyvsp[-7].str), (yyvsp[-5].list)); if ((yyvsp[-3].str) && !strcmp((yyvsp[-3].str), "char")) pas_charvar_add((yyvsp[-7].str)); if (pas_is_booltype((yyvsp[-3].str))) pas_boolvar_add((yyvsp[-7].str)); pas_fwd_save((yyvsp[-7].str), (yyvsp[-5].list)); pas_ptrvar_release(); pas_recvar_release(); }
...
#line 2613 "pascal.tab.c"
  ...
  case 65: /* $@3: %empty  */
#line 1073 "pascal.y"
                                                                        { pas_func_add((yyvsp[-5].str)); pas_proc_vparams((yyvsp[-5].str), (yyvsp[-3].list)); if ((yyvsp[-1].str) && !strcmp((yyvsp[-1].str), "char")) pas_charvar_add((yyvsp[-5].str)); if (pas_is_booltype((yyvsp[-1].str))) pas_boolvar_add((yyvsp[-5].str)); pas_proc_enter(); pas_fwd_restore((yyvsp[-5].str)); }
