...
static inline int zw_rb_on(void)    { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_RB");    v = (e && *e == '1') ? 1 : 0; } return v; }   /* MECHANISM-2 killswitch: DEFAULT OFF (=0).  SCRIP_ZW_RB=1 enables RBP boundary arm for blob-clause programs.  Next session: extend STF to flat_pat=1 (boundary at statement head); remove !flat_pat guard at emit.cpp ~2847. */
static inline int zw_nblob_ok(int nblob_real, int has_blob) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_NBLOB"); v = (e && *e == '0') ? 0 : 1; } return v ? (nblob_real == 0) : (!has_blob); }   /* ⭐ RUNG ZW-14 (s33 OMEGA): nblob_real==0 guards the armed population. ⭐ ZW-15 (OMEGA s36): rbp=claim_base makes FRQ(blob_off)=[claim_base+blob_off] correct depth-free for any nblob_real; BUT blob-member templates (CAPTURE_COND, CAPTURE_SAVE) see op_zw=0 at their invocation site (they are OFF-RUN nodes, not staged at the run choke) and write to RT_DCAP_TOP rather than r12.  MATCH_END's op_zw arm reads r12 as cas_top.  Mismatch: blob writes to cell, match_end reads register.  Consequence: programs with nblob_real>0 AND blob captures are armed but lose captures (V set empty instead of matched string).  The gate therefore stays at nblob_real==0 until blob-context op_zw staging is designed.  Default ON (=1); killswitch =0 reverts to !has_blob (pre-ZW-13). */
#define REX_W            0x48
#define REX_WR           0x4C
#define REX_B            0x41
#define REX_WB           0x49
...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    int                          op_zw2;           /* ⭐ MECHANISM-2 (HQ ruling 2026-08-03, OMEGA s36): 1 = mechanism-2 RBP boundary arm -- blob-clause programs (nblob_real>0) that ZW-12 cannot arm.  push rbp; mov rbp,rsp at MATCH_BEGIN; whack mov rsp,rbp; pop rbp at MATCH_END/head-ω.  DEFAULT OFF (SCRIP_ZW_RB=0); next session: extend STF to flat_pat=1 graphs (boundary must be at statement head, pre-all-cells).  APPENDED AT STRUCT END per s141 ABI law. */
    ...
    long                         op_mech2_kc;      /* ⭐ ZW-RB-1 Part 2: mechanism-2 blob-carve size (bytes). Set at MATCH_BEGIN choke from g_zd_udout (=Kc); blob interior cm[] nodes with op_zw2=1 apply op_zdepth=8+op_mech2_kc so [rsp+off+8+Kc]=[α_base+off]. NOT cleared at per-node reset (line 832) -- persists across whole match extent. Cleared at MATCH_END and emit_jmp_entry_clear only. APPENDED AT STRUCT END per s141 ABI law. */
