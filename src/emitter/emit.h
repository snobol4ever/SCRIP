...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    /* ⭐⭐⭐ ζ-TIER NAMING OF RECORD (Lon in-chat, GOAL-RBP-EARN s77/s78/s80/s81; formalized ARCH-SCRIP.md s81 — READ THAT SECTION FIRST): ζ-STANDING (RBP, one instance, never moves, established once at MATCH_BEGIN) / ζ-FRAME=ζ-ACTIVATION (RBP, N chained, comes and goes per *VAR deref or per-ARBNO-site) / ζ-SPINE (RSP, slides — the `spine` house word already in this file). op_frame_need below decides STANDING-or-FRAME (need=1) vs SPINE (need=0) for the capture family; its own comment predates that naming and used to say DORMANT — it is not: bb_match_capture.cpp has read it live since s81/s83, and the CAP-NEST additive-scan gap that comment named is now LANDED (SCRIP 1995b038, this session) and killswitch-parity re-verified clean. The s81 FINAL RULING (STANDING+FRAME unified as one RBP chain, every activation carrying a copied-down pointer to STANDING, no dedicated register) is NOT yet implemented in bb_match_begin.cpp/bb_match_arbno.cpp — emit_match_rbp()/emit_arbno_rbp() (emit.cpp) still unconditionally return 0 per the s79 retirement; giving them new bodies implementing the s81 chain is the next rung of THIS goal, separate from and not blocked by the capture-family work above. */
