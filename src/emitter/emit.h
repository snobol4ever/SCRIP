...
typedef struct {
    ...
    int                          op_scan;          /* SPD-2 RETRY-INTERNAL: this IR_MATCH_DEFER is the statement's sole frozen-pattern root (head-γ successor, ω back to head-β) -- its fast-arm transfer sets r8=1 so the flat_pat blob runs the positional retry internally */
    int                          op_seal;          /* s137 OVER-SEAL: this IR_MATCH_DEFER's target is a statically right-sealed stored pattern (IR_t.seal) -- α saves an rsp watermark in the defer.pad quad (op_off), the γ/ω glues bulk-restore it (whacking the callee subtree at the fence-demarked sync point), β is an unconditional exhaust */
...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    int                          flat_stmt_frame;  /* STMT-FRAME REGIME (s21x-c design of record, Lon: per-BB one-instruction carves; STATEMENT bracket = the rbp dance; NO graph carve, NO zero-fill, NO anchor): 1 = this graph passed emit_stmt_frame_scan under SCRIP_STMT_FRAME=1 -- the ALL-OR-NOTHING per-graph switch (mixed regimes inside one graph are the s188 disease).  Consumers: xa_flat prologue/epilogue (pad-only 8B arms), the drive loop's per-statement head stubs (x86_stmt_enter at every bb_src_of head) and the two chain exit cuts (x86_stmt_leave glue in front of lbl_γ/lbl_ω), plus the head/port retargets beside the SPD-2 scanhit redirect.  0 = legacy regime verbatim.  APPENDED AT STRUCT END per the s141 ABI law. */
} sm_emit_t;
extern sm_emit_t g_emit;
static inline int emit_jmp_pin_rbp(void) { return g_emit.flat_deep_arrival || g_emit.flat_pat || g_emit.flat_gen; }
static inline int emit_rec_pin(void) { extern int g_gen_proc_active; extern int g_resumable_callable_active; return emit_jmp_pin_rbp() || g_gen_proc_active || g_resumable_callable_active; }   /* REG-7 U5 RECORD-PROTOCOL PIN (s158): emit_jmp_pin_rbp covers the FLAT jmp-entry regime only (deep arrival / pat / gen blobs).  A gen-proc or resumable-callable activation carves NO rsp frame at all -- its base is the HEAP frame in rbp (heap-fb-as-rbp, GOAL-PROLOG-BB JOB #1) -- so it needs the record protocol on rbp for exactly the same reason, and the beta dispatch below ALREADY gates on this pair.  Keeping the two predicates apart is what let the store and the load pick different base registers. */
