...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    int                          op_fb_fb5;        /* FB-STMT per-node twin, set at both promotion sites from the emit_chain map: 1 = this node's FR/FRQ/FRQB refs speak the pinned ___ base; 0 = statically-arriving node, speak rsp+op_flat_disp.  Consulted only when flat_fb_refine=1 and SCRIP_FB_STMT != 0.  Unmapped nodes (foreign drivers) default 1 — conservative.  APPENDED AT STRUCT END per the s141 ABI law. */
