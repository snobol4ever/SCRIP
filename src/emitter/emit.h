...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    int                          op_xf_off;        /* ⭐⭐⭐ PF-1c CROSSED-OPERAND FRAME SLOT, producer half (s178, FINDING s178-L2, witness corpus/probe/bfn/): the FIFTH customer of the ONE activation-frame slot registry.  THE DEFECT IT CLOSES: a ζ-value producer (COERCE_STRING/VAR chain feeding a dynamic needle) whose consumer's op_zread span crosses a VARIABLE-RETENTION box (MATCH_DEFER/ARBNO/VALUE -- earn_hazard_in's family) has NO flat rsp price: the defer's γ road retains the inner blob's 32B resume record PLUS whatever that blob suspended (unbounded), the C road retains 16, so the staged difference-of-two-depths is wrong by a road-dependent amount (measured: bfn_break_var's [rsp+40] needle read garbage; four layout faces from one wild mechanism).  != -1 ⇒ this node's OWN result cell homes at [rbp+op_xf_off] in the blob activation frame (ZRES/ZRESD render the rbp spelling); K stays 16 so the depth model and every other offset are UNTOUCHED -- the spine cell is carved and simply unused (re-home, never a delta, the s129 law).  -1 = every legacy/refused path, byte-identical.  Staged at the ONE choke from xop_frame_slot(nd).  APPENDED AT STRUCT END per the s141 ABI law. */
    int                          op_zread_xf[6];   /* ⭐⭐⭐ PF-1c CROSSED-OPERAND FRAME SLOT, consumer half (s178): operand k's frame home, or -1 for the spine road.  != -1 ⇒ ZOPQ/ZOPD(k,·) render [rbp+op_zread_xf[k]+w] instead of the staged flat difference -- the SAME xop_frame_slot(P) the producer staged its write from, so read and write cannot disagree (ONE AUTHORITY; the carve widens through frame_slot_is_candidate -> frame_slot_scan -> blob_frame_bytes, same one-scan law as the leaf class).  Staged at the choke from nd->operands[k] via the zd_nops mapping.  APPENDED AT STRUCT END per the s141 ABI law. */
} sm_emit_t;
...
int xop_frame_slot(const IR_t * nd);   /* ⭐⭐⭐ PF-1c CROSSED-OPERAND FRAME SLOT (s178): THE ONE AUTHORITY assigning a ζ-value producer whose consumer span crosses a variable-retention box (DEFER/ARBNO/VALUE) a stable rbp-relative slot in the SAME shared frame_slot_scan numbering as the four existing customers. -1 refuses (not a member, killswitch off, no scope) and every spelling stays spine byte-identically. Both halves (op_xf_off producer write, op_zread_xf[k] consumer read) stage from THIS function only. */
int leaf_frame_slot(const IR_t * leaf_nd);
int cap_fail_retreat(void);   /* ⭐⭐⭐ s193 CAPTURE-TARGET FAILURE EDGE (SCRIP_CAP_FAIL_RETREAT, DEFAULT ON): in the IMMEDIATE ($) capture road the transfer's ω landing -- the arm reached when the deferred target proc FAILED -- fell through into the box's common continuation, the same label the γ arm jumps to, so a failing target matched anyway. Armed, that arm leaves the alignment anchor and CONCEDES. Oracle-ruled as a RETREAT, not an abort. THE ONE AUTHORITY + full note at emit.cpp. */   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s130): THE ONE AUTHORITY assigning a scratch-cell leaf matcher on an ALT arm a stable rbp-relative slot in the SAME shared frame_slot_scan numbering as arbno_frame_slot()/capture_frame_slot()/fence_frame_slot(), so the four classes can never collide however they nest. -1 under the same conditions those refuse under, plus: the node is not a scratch-cell leaf, or is not on an ALT arm (leaf_frame_member()==0), or the SCRIP_SPAN_FRAME killswitch is off. Staged into g_emit.op_leaf_frame_off at the leaf drive cases -- see the field note for the defect it closes. */
