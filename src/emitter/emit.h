...
static inline int emit_jmp_pin_rbp(void) { static int _zw = -1; if (_zw < 0) { const char * e = getenv("SCRIP_BB_ALLOC"); _zw = (e && *e == '1') ? 1 : 0; } return _zw || g_emit.flat_deep_arrival || g_emit.flat_pat || g_emit.flat_gen; }   /* ZW-1 (Lon s21x-m): under the universal carve every graph pins rbp -- the 023_arith_add witness measured the intra-box disease (a carve-only box's OWN flat [rsp+off] reads are displaced by its OWN K on unpinned graphs; OUTPUT = 1+2 emitted 1).  rbp-flat is depth-immune, so carve-only cells cost zero addressing anywhere; the crawl then migrates refs rbp->cell per family, which is law 3's RSP-until-brick-wall read as the MIGRATION ORDER.  Gate-off: _zw=0, predicate byte-identical to FLATDISP-8. */
