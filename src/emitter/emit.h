...
static inline int zw_nblob_ok(int nblob, int has_blob) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_NBLOB"); v = (e && *e == '0') ? 0 : 1; } return v ? (nblob == 0) : (!has_blob); }   /* ⭐ RUNG ZW-13 (s32 OMEGA): the ZW-12 verdict's clause (b) is "no PAT$ BLOB members" == nblob==0, and it read exactly that until ALPHA's s24a bd08c7a3 substituted !has_blob on this OMEGA-owned planner line.  has_blob is ALPHA's ZD-5b claim-span predicate: TRUE whenever the run holds ANY ordinary pattern element (LIT/LEN/ANY/SPAN/TAB/POS/REM/BREAK/ARB/BAL/ALTERNATE/SEQUENCE/ASSIGN_*), which every real match run does -- so the canonical frame has armed ZERO times corpus-wide ever since (measured s32: 0/318 programs emit match_frame/cas_base/anchor_snapshot, while 175/318 emit the legacy marker arm).  DEFAULT ON (=1, killswitch =0): restores the documented ZW-12 clause.  Measured s32: 23 programs arm corpus-wide; all 23 PASS m3 (161 m3=PASS rc=139 = pre-existing post-output SEGV, harness-invisible); 22 PASS m4 (161 m4=FAIL pre-existing at bracket).  DEFER/PATREF sealed at graph scope (measured: run-scope seal insufficient when deferral lowers into a sibling chain).  =0 reverts to ALPHA's has_blob predicate, arms nothing.  has_blob at the Kc span line UNTOUCHED. */
#define REX_W            0x48
#define REX_WR           0x4C
#define REX_B            0x41
#define REX_WB           0x49
