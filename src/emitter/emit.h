#include <stdlib.h>
#pragma once
#define ZW_FRAME_K       56L                        /* RUNG ZW-12: the canonical MATCH frame's own sub rsp,K -- ≤64 per the WHACK CONTRACT §6 range; slots [rbp-8..-56] = {Σ,δ,Δ,cas_base,anchor,start_δ,cap_gen} */
#define ZW_FRAME_TOTAL   64L                        /* ZW_FRAME_K + the push rbp qword -- the EXACT bytes zd_plan's depth model inserts between head and END; emitted side and model side read THIS one name */
static inline int zw_frame_on(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_FRAME"); v = (e && *e == '0') ? 0 : 1; } return v; }
static inline int zw_rb_on(void)    { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_RB");    v = (e && *e == '1') ? 1 : 0; } return v; }   /* MECHANISM-2 killswitch: DEFAULT OFF (=0) -- mechanism-2 is under construction (next session).  SCRIP_ZW_RB=1 enables the RBP boundary arm for blob-clause programs (nblob_real>0).  When enabled: push rbp; mov rbp,rsp at MATCH_BEGIN establishes the indeterminacy boundary; mov rsp,rbp; pop rbp at MATCH_END/MATCH_BEGIN-ω is the depth-independent whack.  DESIGN CORRECTION (this session end): the boundary must be at the STATEMENT HEAD (pre-all-cells), not at MATCH_BEGIN (post-VAR-claim), so the whack correctly returns to statement entry rsp.  This is essentially extending STF to flat_pat graphs; next session: modify emit_stmt_frame_scan to admit match-bearing graphs and remove the !flat_pat guard at line 2847 when mechanism-2 is active. */   /* RUNG ZW-12 killswitch: =0 restores the pre-frame regime byte-identically (op_zw never stages).  PROCESS-SCOPE IS SOUND per the CAP-SYM precedent: this latch feeds zd_plan's per-run predicate, and the SAME staged op_zw drives both model and template -- the s21x-r defect (flag vs planner disagreement) is impossible by construction. */
static inline int zw_nblob_ok(int nblob_real, int has_blob) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_NBLOB"); v = (e && *e == '0') ? 0 : 1; } return v ? (nblob_real == 0) : (!has_blob); }   /* ⭐ RUNG ZW-13 (s32 OMEGA): restored nblob==0 after ALPHA's s24a substituted !has_blob.  ⭐ RUNG ZW-14 (s33 OMEGA): input is now nblob_real (blob members with non-sentinel zls_node_off) rather than nblob (total closure count).  nblob_real==0 when all blob members are K=0 elided scanner-register-only nodes (LIT/LEN/ANY/POS/etc.) which carry no FRQ claim slots and are therefore not subject to the s23i CAP-SLOT shift-fragile hazard (canonical frame inserts ZW_FRAME_K=56B between claim carve and expected rsp, shifting FRQ reads; elided nodes have no such reads).  The hazard only applies when nblob_real>0 (blob members with real registered claim slots whose FRQ spellings are baked relative to the old claim base).  DEFAULT ON (=1, killswitch =0 reverts to !has_blob as before ZW-13).  Measured s33: widens armed population to include programs whose only off-run blob members are pure scanner-register-only kinds (FENCE(LIT), FENCE(POS), FENCE(SPAN), etc.). */
#define REX_W            0x48
#define REX_WR           0x4C
#define REX_B            0x41
#define REX_WB           0x49
...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    long                         op_uclaim;        /* UCLAIM (wholesale flip, Lon 2026-08-01 "FLIP them ALL on ... ONE FUNCTION to calculate the offsets based on the execution order"): THIS node's statement-scope flat-plan claim in bytes, carved by the port hook at alpha IN ADDITION to op_fc_bytes and deliberately OUTSIDE op_zdepth (the claim is the slot backing the legacy FRQ spellings, not intra-box depth) and OUTSIDE the hook's omega auto-release (statement-scope: the staged op_zgpop/op_wpop terminal cuts are the sole release).  Staged by zd_plan's declined-run extension at the drive loop, applied at the choke, cleared unconditionally beside op_zres.  APPENDED AT STRUCT END per the s141 ABI law. */
    int                          op_stmt_pin;      /* ⭐ HEAD-PIN slot carry (s22z): NONZERO = the current statement's match head PINNED rbp to the region base, and this value IS the region offset of the slot holding the OLD rbp (head's op_off+40 -- planner-reserved, HEAD is slot-elide EXCLUDED).  Set by bb_match_begin at its template top (bb_slot_claim emit-time staging precedent), consumed by the X86H_JMP terminal-cut arms (x86_asm.h) which prepend `mov rbp,[rbp+off]` to the statement release -- the restore rides the SOLE release authority, per the STF-UNFLIP ruling.  Cleared at the choke when the NEXT statement's uclaim stages and at chain start; deliberately NOT in the per-node unconditional clear (it is STATEMENT-scoped state, the one lifetime the per-node clears must not touch).  0 = no pin, cuts stay byte-identical.  APPENDED AT STRUCT END. */
    int                          op_wterm;         /* HEAD-PIN discriminator (s22z): 1 = THIS node's op_wpop contains the PLANNER's statement-terminal under-cells term (zd_wp>0 at the choke), i.e. its omega cut RETURNS RSP TO STATEMENT ENTRY and must also restore the pinned rbp.  0 = op_wpop is trampoline ΣK fold only (mid-statement, BP-9) -- restoring there would corrupt the live pin.  Per-node, cleared unconditionally at the choke.  APPENDED AT STRUCT END. */
    long                         op_udout;         /* UCLAIM depth-out: cumulative claim depth (armed K + uclaims, execution order) AFTER this node within its statement run -- the reader-side term of the ONE offset function: zvo_resolve maps a flat off to (off - owner_base) + (op_udout - owner_dout), then x86_frame_off adds the legacy op_flat_disp/op_zdepth intra-claim compensation on top.  APPENDED AT STRUCT END per the s141 ABI law. */
    int                          op_uhead;         /* UCLAIM run-head id (nodes[] index of the statement head): disambiguates owner-table entries when statements share a jcon_value_region offset range -- the resolver only honors entries minted by THIS node's own run.  APPENDED AT STRUCT END per the s141 ABI law. */
    int                          op_zw;            /* ⭐ RUNG ZW-12 (s23l): 1 = zd_plan armed THIS statement's run for the CANONICAL MATCH FRAME (WHACK CONTRACT §7): MATCH_BEGIN emits `push rbp; mov rbp,rsp; sub rsp,ZW_FRAME_K`, the head housekeeping quintet {outer Σ/δ/Δ, cas_base, anchor_snapshot, start_δ, cap_gen} lives at [rbp-8..-56] (depth-free raw spellings, never zvo), and MATCH_END/head-ω whack via `mov rsp,rbp; pop rbp`.  Staged by zd_plan for run members when zw_frame_on() && claim staged (Kc>0) && nblob==0 && no seal member (ABORT/FENCE1) && an IR_MATCH_END closes the run -- the SAME predicate that biased the depth model by ZW_FRAME_TOTAL between head and END, so template and model CANNOT drift (one authority, the zd_k law's shape).  0 = old regime byte-identical (killswitch SCRIP_ZW_FRAME=0 forces 0 everywhere).  APPENDED AT STRUCT END per the s141 ABI law. */
    int                          op_zw2;           /* ⭐ MECHANISM-2 RBP BOUNDARY (HQ ruling 2026-08-03, Lon): 1 = zd_plan armed this statement's run for the INDETERMINABLE-EXTENT RBP FRAME (WHACK CONTRACT mechanism 2) for blob-clause programs (nblob_real>0) where the shift-fragile CAP-SLOT hazard prevents the ZW-12 canonical frame's sub rsp,ZW_FRAME_K -- the blob members' FRQ spellings are baked relative to the flat claim base and survive a push+mov only (8B pushed, no sub).  MATCH_BEGIN emits `push rbp; mov rbp,rsp` only (no sub rsp,K), then the legacy flat arm for housekeeping (FRQ saves for Σ/δ/Δ/cas_base/anchor/start_δ/cap_gen) -- these remain at FRQ(op_off+N) relative to the claim.  MATCH_END: legacy pump body then `mov rsp,rbp; pop rbp` whack (depth-independent, reclaims all blob cells + the 8B push + all FORTH spine cells above the frame).  Depth model: 8B (push rbp only) added between head and END at the head boundary.  Staged when zw_frame_on() && Kc>0 && hpos>=0 && nblob_real>0 && !seal && has_END && !flat_stmt_frame.  Killswitch SCRIP_ZW_RB=0.  0 = old regime byte-identical.  APPENDED AT STRUCT END per the s141 ABI law. */
