...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    int                          op_omega_depths[8]; /* ZW-5 SLICE 3 (OMEGA O-2): the SET of distinct non-zero zwpop values over the admitted statement run members whose omega edge exits the statement boundary -- the depth ladder for the per-depth omega stubs emitted by bb_statement.cpp.  Entry k carries the K-byte release amount; the stub label is s<op_stno>_omega_dK.  Populated by zd_plan when op is IR_STATEMENT; cleared at the choke unconditionally.  APPENDED AT STRUCT END per the s141 ABI law. */
    int                          op_n_omega_depths; /* ZW-5 SLICE 3 (OMEGA O-2): count of valid entries in op_omega_depths[].  0 = no stubs needed (all fail edges arrive at depth 0, no release required before fT).  Cleared at the choke.  APPENDED AT STRUCT END per the s141 ABI law. */
} sm_emit_t;
