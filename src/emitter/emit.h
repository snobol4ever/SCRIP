...
typedef struct {
    ...
    int                          op_anchored;      /* ANCHOR-WINDOW s66: this box belongs to a grant-DECLINED statement's match window under ZC_FRAME_RSP — its ζ view is r12 (materialized from rsp at the anchored HEAD's α), the proven s64 configuration per-statement; FR/FRQ emit r12-based refs with NO disp; 0 = the granted pure-rsp fast path */
    int                          op_anchor_head;   /* ANCHOR-WINDOW s66: this IR_MATCH_HEAD opens an anchored window — its α materializes the view (mov r12, rsp) before the self-cell push */
