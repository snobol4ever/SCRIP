...
static inline int zw_frame_on(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_FRAME"); v = (e && *e == '0') ? 0 : 1; } return v; }   /* RUNG ZW-12 killswitch: =0 restores the pre-frame regime byte-identically (op_zw never stages).  PROCESS-SCOPE IS SOUND per the CAP-SYM precedent: this latch feeds zd_plan's per-run predicate, and the SAME staged op_zw drives both model and template -- the s21x-r defect (flag vs planner disagreement) is impossible by construction. */
static inline int zw_r9_on(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_R9"); v = (e && *e == '1') ? 1 : 0; } return v; }   /* ⭐ RUNG r9/wire (OMEGA s35): default OFF until all blob templates have correct op_zw arms.  =1 widens the gate to nblob_real>0; each blob template's op_zw arm must be correct before flipping. */
static inline int zw_nblob_ok(int nblob_real, int has_blob) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZW_NBLOB"); v = (e && *e == '0') ? 0 : 1; } return v ? (nblob_real == 0 || zw_r9_on()) : (!has_blob); }   /* ⭐ RUNG ZW-13 + r9/wire: nblob_real>0 arms when SCRIP_ZW_R9=1 (opt-in) -- blob templates gain op_zw arms using [rbp-ZW_FRAME_K + off - blob_base] spellings that are depth-immune even with the canonical frame's ZW_FRAME_K insertion. */   /* ⭐ RUNG ZW-13 (s32 OMEGA): restored nblob==0 after ALPHA's s24a substituted !has_blob.  ⭐ RUNG ZW-14 (s33 OMEGA): input is now nblob_real (blob members with non-sentinel zls_node_off) rather than nblob (total closure count).  nblob_real==0 when all blob members are K=0 elided scanner-register-only nodes (LIT/LEN/ANY/POS/etc.) which carry no FRQ claim slots and are therefore not subject to the s23i CAP-SLOT shift-fragile hazard (canonical frame inserts ZW_FRAME_K=56B between claim carve and expected rsp, shifting FRQ reads; elided nodes have no such reads).  The hazard only applies when nblob_real>0 (blob members with real registered claim slots whose FRQ spellings are baked relative to the old claim base).  DEFAULT ON (=1, killswitch =0 reverts to !has_blob as before ZW-13).  Measured s33: widens armed population to include programs whose only off-run blob members are pure scanner-register-only kinds (FENCE(LIT), FENCE(POS), FENCE(SPAN), etc.). */
#define REX_W            0x48
#define REX_WR           0x4C
#define REX_B            0x41
#define REX_WB           0x49
...
#define XA_BB_EMIT_PAIR_MAX 1024
    ...
    int                          op_zw_blob_base;  /* ⭐ RUNG r9/wire (OMEGA s35): the minimum zls_node_off among all blob-closure members that have live FRQ-slot spellings (nblob_real>0 runs).  Staged by zd_plan at the same site as zzw[hpos]=1, from umin computed across blob closure.  Blob templates in op_zw arms compute `lea r9,[rbp-ZW_FRAME_K]` (depth-immune: rbp is pinned by the canonical frame) and address their slots as `[r9 + (op_off - op_zw_blob_base)]` via the ZWB() helper, replacing FR()/FRQ() spellings that are shift-fragile under the canonical frame's ZW_FRAME_K insertion.  -1 = no blob (op_zw=0 or nblob_real==0); matches the canonical frame's zw_nblob_ok=1 arm.  APPENDED AT STRUCT END per the s141 ABI law. */
