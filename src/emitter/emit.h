...
typedef struct {
    ...
    int                          flat_gen;         /* GENP slice-2 (Lon 2026-07-17 per-instance-stack directive): this jmp-entry graph is a GENERATOR proc — its body runs on a per-instance coexpr stack (rt_genp_* in rt.c); bb_suspend swaps jmp-γ for the rt_genp_yield call (value in rdi:rsi, resume = fall-through into the suspend's β chain); real return/fail flow unchanged through rt_proc_enter's γ/ω landings into the instance thread's C entry.  Set by emit_jmp_entry_for_proc, cleared with flat_jmp_entry. */
