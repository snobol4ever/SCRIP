/* bb_var.cpp — x86() self-encoding (TEMPLATE-REVAMP, 2026-06-02). IR_VAR — variable read.
   ...
   Two live arms + one fallback bomb:
     SNO flat-chain (g_sno_flat_chain): pass-through — α: jmp γ ; def β ; jmp ω (10 bytes).
       The ASSIGN consumer reads the src variable NAME via _.op_a_sval / rt_sno_assign_var; the IR_VAR box
