...
static std::string bb_scan_bal_str() {
    ...
    return IF(MEDIUM_TEXT, x86("label", _.lbl_α)
                         + x86("comment", "BOX ICN IR_SCAN_BAL bal(c1) [ICN-SCAN-11 fstranl.r function{*} wave-1 c2='(' c3=')': cursor[r12+cur]<-δ cnt[r12+cnt]<-0; L0: cursor>=Δ->ω;"
         ...
         + x86("inc", FRQ(cur))
         + x86("jmp", L(0))
         + x86_ro_seal_str(4, cs);
}
/*--------------------------------------------------------------------------------------------------------------------*/
extern "C" void bb_scan_bal(IR_t * pBB) {
    (void)pBB;
    x86_begin();
    std::string s = bb_scan_bal_str();
    if (s.empty()) { bb_emit_x86(x86_bomb("bb_scan_bal: unhandled (needs nonempty bracket-free literal c1 + descr flat-chain slot)")); return; }
    bb_emit_x86(s);
}
