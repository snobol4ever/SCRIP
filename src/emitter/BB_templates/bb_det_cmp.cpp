...
static int  dcm_l_var()       { return dcm_la()->op == IR_LOGICVAR; }
static int  dcm_r_var()       { return dcm_ra()->op == IR_LOGICVAR; }
...
static long dcm_l_ival()      { return (!dcm_l_var() && dcm_la()->op == IR_LIT_I) ? (long)IR_LIT(dcm_la()).ival : 0L; }
static long dcm_r_ival()      { return (!dcm_r_var() && dcm_ra()->op == IR_LIT_I) ? (long)IR_LIT(dcm_ra()).ival : 0L; }
