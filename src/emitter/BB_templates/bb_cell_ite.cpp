...
static std::string bb_cell_ite_str() {
    ...
    const pl_gz_ite_state_t *st = nd ? (const pl_gz_ite_state_t *)(intptr_t)nd->ival : (const pl_gz_ite_state_t *)0;
    if (!st || st->gate_slot < 0) return x86_bomb("bb_cell_ite: unadmitted ifstmt shape reached the emitter");
    int gate_off = GZ_CELL_OFF(st->gate_slot);
    if (_.op_sa == 0)
