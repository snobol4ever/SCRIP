...
static std::string emit_build_conj_chain(IR_t **goals, int i, int n) {
    ...
    std::string out = x86("ins2", "sub rsp,", emit_fmt("%d", 16));
    out += emit_build_compound_term(goals[i]);
    out += x86("ins2", "mov", emit_fmt("qword ptr [rsp + %d], rax", 0));
    out += emit_build_conj_chain(goals, i + 1, n);
    out += x86("ins2", "mov", emit_fmt("qword ptr [rsp + %d], rax", 8));
    out += x86("ins2", "lea rdi,", emit_fmt("[rip + %s]", clbl))
         + x86("ins2", "mov esi,", emit_fmt("%d", 2))
         + x86("ins2", "mov", "rdx, rsp")
         + x86("ins2", "call", "rt_compound_build_n@PLT")
         + x86("ins2", "add rsp,", emit_fmt("%d", 16));
    return out;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
std::string emit_build_compound_term(const IR_t *nd) {
    ...
    if (nd->t == IR_LIT_I || nd->t == IR_LIT_F || nd->t == IR_ATOM || nd->t == IR_LOGICVAR) {
        ...
        return x86("ins2", "mov edi,",  emit_fmt("%d",  (int)nd->t))
             + x86("ins2", "mov rsi,",  emit_fmt("%ld", (long)nd->ival))
             + (slbl[0] ? x86("ins2", "lea rdx,", emit_fmt("[rip + %s]", slbl)) : x86("ins2", "xor", "edx, edx"))
             + x86("ins2", "xor", "ecx, ecx")
             + x86("ins2", "call", "rt_node_to_term@PLT");
    }
    if (nd->t == IR_STRUCT) {
        ...
        if (arity <= 0 || !nd->α) {
            ...
            return x86("ins2", "mov edi,",  emit_fmt("%d", (int)IR_ATOM))
                 + x86("ins2", "xor", "rsi, rsi")
                 + (fnlbl[0] ? x86("ins2", "lea rdx,", emit_fmt("[rip + %s]", fnlbl)) : x86("ins2", "xor", "edx, edx"))
                 + x86("ins2", "xor", "ecx, ecx")
                 + x86("ins2", "call", "rt_node_to_term@PLT");
        }
        int slots_bytes = arity * 8;
        int frame = (slots_bytes + 15) & ~15;
        std::string out = x86("ins2", "sub rsp,", emit_fmt("%d", frame));
        const IR_t *child = nd->α;
        for (int i = 0; i < arity && child; i++) {
            out += emit_build_compound_term(child);
            out += x86("ins2", "mov", emit_fmt("qword ptr [rsp + %d], rax", i * 8));
            child = child->γ;
        }
        char fnlbl[64]; fnlbl[0] = 0;
        if (nd->sval) strtab_label(fnlbl, sizeof fnlbl, nd->sval);
        out += (fnlbl[0] ? x86("ins2", "lea rdi,", emit_fmt("[rip + %s]", fnlbl)) : x86("ins2", "xor", "edi, edi"))
             + x86("ins2", "mov esi,", emit_fmt("%d", arity))
             + x86("ins2", "mov", "rdx, rsp")
             + x86("ins2", "call", "rt_compound_build_n@PLT")
             + x86("ins2", "add rsp,", emit_fmt("%d", frame));
        return out;
    }
    if (nd->t == IR_GOAL) {
    ...
    if (nd->t == IR_BUILTIN && nd->sval) {
        ...
        if (hit && nd->α && nd->β) {
            ...
            out += x86("ins2", "mov", emit_fmt("qword ptr [rsp + %d], rax", 8));
            out += x86("ins2", "lea rdi,", emit_fmt("[rip + %s]", fnlbl))
                 + x86("ins2", "mov esi,", emit_fmt("%d", 2))
                 + x86("ins2", "mov", "rdx, rsp")
                 + x86("ins2", "call", "rt_compound_build_n@PLT")
                 + x86("ins2", "add rsp,", emit_fmt("%d", 16));
    ...
    return x86("comment", emit_fmt("build_compound_term: unhandled kind %d", (int)nd->t));
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static std::string bb_builtin_str(IR_t * pBB) {
    ...
    if (MEDIUM_MACRO_DEF) return x86("comment", "no macro form — RESOLVE_BUILTIN");
    std::string hdr = x86("label", _.lbl_α)
                    + x86("ins1", emit_fmt("# BOX RESOLVE_BUILTIN(%s/%d)", fn, _.op_ival));
    std::string r;
    ...
    (void)succ_back;
        return hdr + x86("ins1", emit_fmt("# RESOLVE_BUILTIN: unknown '%s' — stub", fn)) + succ_back;
    }
    return std::string();
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
