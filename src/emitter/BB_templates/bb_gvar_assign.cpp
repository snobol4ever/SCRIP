...
static const char * bga_plbl(const char * ps) { const char * l = emit_intern_str(ps); if (l) return l; static char b[64]; strtab_label(b, sizeof b, ps); return b; }
static std::string bga_seq_parts(int off) {
    return x86("label", _.lbl_α)
         + x86("comment", std::string("BOX IR_ASSIGN(concat) ") + std::to_string(_.op_parts_n) + " staged parts -> rt_gvar_assign_concat_parts [zeta region, @PLT]")
         + FOR(0, _.op_parts_n, [&](int i) {
               return x86("mov", FR(off + 16 * i), (long)_.op_parts_tag[i])
                    + x86("lea", "rax", "[rip + __]", (uint64_t)(uintptr_t)(_.op_parts_str[i] ? _.op_parts_str[i] : ""), bga_plbl(_.op_parts_str[i] ? _.op_parts_str[i] : ""))
    ...
    if (PLATFORM_X86) {
        if (g_descr_flat_chain) {
            ...
            return IF(MEDIUM_TEXT, x86("label", _.lbl_α)
                                  + x86("comment", std::string("BOX IR_ASSIGN global write(\"") + dst_name() + "\") [GN-4 nv x86() stackless: NV_SET_fn(name, rhs slot " + std::to_string(_.op_sa) + ") -> own slot " + std::to_string(_.op_off) + "; name sealed RO [rip+disp]]"))
                 + x86_frame_load64("rsi", _.op_sa)
                 + x86_frame_load64("rdx", _.op_sa + 8)
                 + x86_ro_load_q("rdi", 0)
                 + x86("call", "NV_SET_fn", (uint64_t)(uintptr_t)(void *)NV_SET_fn)
        ...
        if (_.op_a_node_kind == (int)IR_BINOP) {
            if (_.op_a_slot < 0) return x86_bomb("bb_gvar_assign int-binop: op_a_slot==-1 (binop slot not promoted)");
            return IF(MEDIUM_TEXT, x86("label", _.lbl_α)
        ...
        if (_.op_a_node_kind == (int)IR_SEQ) {
            if (MEDIUM_TEXT) {
                if (_.op_parts_n <= 0) return x86_bomb("bb_gvar_assign concat: TEXT(mode-4) value shape not flattenable to lit/var parts (pending)");
                return bga_seq_parts(bb_slot_claim(16 * _.op_parts_n));
            }
        ...
        if (_.op_a_node_kind == (int)IR_CALL) {
            if (_.op_a_slot < 0) return x86_bomb("bb_gvar_assign call-result: op_a_slot==-1 (call result slot not promoted)");
            if (MEDIUM_TEXT)
                return x86("label", _.lbl_α)
                     + x86("comment", "BOX IR_ASSIGN(call-result) store = DESCR from call zeta-slot [RO dst ptr, @PLT]")
                     + x86("lea",  "rdi", "[rip + __]", dst_addr(), dst_label())
                     + x86_frame_load64("rsi", _.op_a_slot)
