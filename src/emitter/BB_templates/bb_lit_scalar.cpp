/* bb_lit_scalar.cpp — grouped BB template for the scalar-literal family.
   ...
   (BB_LIT_I / BB_LIT_S / BB_LIT_F / BB_LIT_NUL) share an identical emit shape.
   ...
   BB_LIT_I diverges from the pass-through. In MEDIUM_BINARY it pushes its
   ...
   GZ-2 / R-HW-2 (2026-05-30, GROUND ZERO 3, RO-IP-relative FACT RULE): BB_LIT_I
   and BB_LIT_S are READ-ONLY constants — pure four-port pass-throughs here. Their
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static std::string bb_lit_scalar_str(BB_t * pBB, bb_bin_t & bin) {
    (void)pBB;
    bin = {};
    ...
    if (MEDIUM_BINARY) {
        if (pBB && pBB->t == BB_LIT_I) {
            ...
            /* blob and reads it `[rip+disp]`. So BB_LIT_I is a pure four-port pass-through (like the     */
        ...
        if (pBB && pBB->t == BB_LIT_S && pBB->sval) {
            /* R-HW-2 (GROUND ZERO 3, 2026-05-30): a literal string is a READ-ONLY constant, exactly  */
            /* like the BB_LIT_I case above. Per the RO-IP-relative FACT RULE it is NOT pushed onto a   */
            /* value stack here; the consumer box (the write strlit arm in bb_call.cpp) seals the bytes */
            /* next to its OWN blob and reads them `lea rdi,[rip+disp]`. So BB_LIT_S is a pure four-port */
            /* pass-through (α: jmp γ ; β: jmp ω). The prior form pushed via rt_push_str onto the        */
            /* now-excised value stack. Identical 10-byte shape to BB_LIT_I and the generic fall-through.*/
            bin = { {1, 5, 6}, {_.lbl_γ_p, _.lbl_β_p, _.lbl_ω_p}, {false, true, false} };
            return bytes(1, "\xE9") + u32le(0) + bytes(1, "\xE9") + u32le(0);
        }
        if (pBB && pBB->t == BB_LIT_F) {
            ...
            /* Total: 32 bytes. Identical layout to BB_LIT_I.                                       */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
extern "C" void bb_lit_scalar(BB_t * pBB) { bb_bin_t bin; bb_emit_asm_result(bb_lit_scalar_str(pBB, bin), bin); }
