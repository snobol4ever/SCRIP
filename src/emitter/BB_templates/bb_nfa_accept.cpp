/* bb_nfa_accept.cpp — TEMPLATE-REVAMP bomb stub (Lon directive 2026-06-02).  The original box body used the
   ABOLISHED bb_bin_t / bb_emit_asm_result offset-table path.  It is replaced with a LOUD x86_bomb() stub so
   SCRIP BUILDS + LINKS (green) and ABORTS beautifully when this box is reached — the Raku
...
extern "C" void bb_nfa_accept(IR_t * pBB) {
    (void)pBB;
    bb_emit_x86(x86_bomb("bb_nfa_accept: TEMPLATE-REVAMP not yet converted (was offset-table)"));
}
