...
std::string bb_cell_call() {
    ...
    if (PLATFORM_X86) return IF(!bcc_sh(), x86_bomb("bb_cell_call: unadmitted call shape reached the emitter"))
                           ...
                           + x86("mov", "rdi", "rax")
                           + FOR(4, (int)_.op_parts_ival[1], [&](int i) { return x86("mov", "r11", FRQ(GZ_CELL_OFF((int)_.gz_arg_slots[i]))) + x86("mov", bcc_rdiq(GZ_CELL_OFF(i)), "r11"); })
                           + FOR(0, ((int)_.op_parts_ival[1] < 4 ? (int)_.op_parts_ival[1] : 4), [&](int i) { return x86("mov", bcc_areg(i), FRQ(GZ_CELL_OFF((int)_.gz_arg_slots[i]))); })
