/* bb_swap.cpp — BB template for IR_SWAP (Icon `x :=: y` swap).
   IBB (2026-05-29, Opus 4.8). Composite: α = lhs (IR_VAR), β = rhs (IR_VAR).
...
static std::string bb_swap_str(IR_t * pBB, bb_bin_t & bin) {
    ...
    if (MEDIUM_MACRO_DEF) return s_comment("# no macro form — IR_SWAP");
    IR_t *lhs = pBB ? pBB->α : NULL;
    IR_t *rhs = pBB ? pBB->β : NULL;
    if (!lhs || lhs->t != IR_VAR || !lhs->sval || !rhs || rhs->t != IR_VAR || !rhs->sval) {
        fprintf(stderr, "[IBB] FATAL bb_swap: x:=:y requires two IR_VAR operands with sval "
    ...
    if (MEDIUM_TEXT) {
        return s_1asm(emit_fmt("%s:", _.lbl_α))
             + s_comment(emit_fmt("# BOX IR_SWAP %s :=: %s (pop y->x, pop x->y)", xname, yname))
