...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int gen_is_numarith(int64_t op) { return op == BINOP_ADD || op == BINOP_SUB || op == BINOP_MUL || op == BINOP_DIV || op == BINOP_MOD; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int gen_rel_fail_jcc(int64_t op, const char **mnem) {
    switch (op) {
...
static std::string bb_binop_str(IR_t * pBB, bb_bin_t & bin) {
    ...
    if (g_icn_flat_chain && pBB && (gen_is_numrel(op) || gen_is_strrel(op)) && pBB->α && pBB->β) {
    ...
    if (g_icn_flat_chain && pBB && gen_is_numrel(op) && pBB->α && pBB->β) {
        int sa = bb_slot_get(pBB->α);
        int sb = bb_slot_get(pBB->β);
        if (sa >= 0 && sb >= 0) {
            const char *mnem = "jmp";
            int jcc2 = gen_rel_fail_jcc(op, &mnem);
            if (MEDIUM_BINARY) {
                /*   off  bytes                       asm                                                    */
    ...
    if (g_icn_flat_chain && pBB && gen_is_numarith(op) && pBB->α && pBB->β) {
        int sa = bb_slot_get(pBB->α);
        int sb = bb_slot_get(pBB->β);
        if (sa >= 0 && sb >= 0) {
