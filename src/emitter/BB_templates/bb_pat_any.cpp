...
static std::string bb_pat_any_str(IR_t * pBB, bb_bin_t & bin) {
    ...
    if (PLATFORM_X86) {
         ...
         * &Σ / &Σlen movabs bakes are GONE — δ is r14d, the subject base is r13, the length r15d. ANY(S)
         * matches exactly ONE subject character that appears in S (SPITBOL Manual ch.18: "ANY(S) matches the
         * next subject character if it appears in the string S, and fails otherwise"); on success δ += 1, on
         * β it undoes (δ -= 1) and fails. strchr(cs,ch) != NULL ⇒ char in set. r10 is caller-saved so it is
         * still push/pop'd around call (r13/r14/r15 are callee-saved and survive). LITERAL byte map,
         * hand-coded offsets (FACT RULE TWO LITERAL FORMS — no b.size()); 74 bytes:
         *   0  : 44 89 F0                 mov eax, r14d            ; eax = δ
         *   3  : 44 39 F8                 cmp eax, r15d            ; cmp δ, Δ
         *   6  : 0F 8D + ω_rel32          jge ω                    ; rel32 @8   (δ >= Δ → fail)
         *   12 : 49 63 CE                 movsxd rcx, r14d         ; rcx = δ
         *   15 : 41 0F B6 74 0D 00        movzx esi, byte [r13+rcx]; esi = Σ[δ]
         ...
         *   50 : 0F 84 + ω_rel32          je ω                     ; rel32 @52  (ANY: char NOT in set ⇒ fail)
         *   56 : 41 83 C6 01              add r14d, 1              ; δ += 1
         *   60 : E9 + γ_rel32             jmp γ                    ; rel32 @61
         *   65 : 41 83 EE 01              β: sub r14d, 1           ; δ -= 1 (undo)   [β-def @65]
        ...
        return IF(MEDIUM_MACRO_DEF, s_comment("# no macro form — ANY"))
             ...
             + IF(MEDIUM_TEXT,
                 ...
                 + s_comment("# BOX ANY()  [REG-2 Σ=r13 δ=r14 Δ=r15]")
