...
std::string bb_choice() {
    ...
    return IF(MEDIUM_TEXT,
         ...
         + IF(_.resolve_choice_n >  0, x86("label", _.lbl_α)
                                     ...
                                     + x86("call", "rt_cp_get_cursor@PLT")
                                     + x86("cmp", "eax", (long)_.resolve_choice_n)
                                     + x86("jge",  plch("exhausted"))
                                     + FOR(0, _.resolve_choice_n, [](int i) { return x86("cmp", "eax", std::to_string(i))
                                                                                    + x86("je",  plchi(i, "pre")); })
                                     + x86("jmp", plch("exhausted"))
                                     + FOR(0, _.resolve_choice_n, [](int i) { return x86("label", plchi(i, "pre"))
                                                                                    + IF(i > 0, x86("call", "rt_cp_trail_unwind@PLT"))
                                                                                    + x86("call", "rt_cp_inc_cursor@PLT")
                                     ...
                                     + x86("mov", "rdi", "rax")
                                     + x86("call", "rt_choice_cut_exit@PLT")
                                     + x86("call", "rt_cp_trail_unwind@PLT")
