...
std::string bb_unify() {
    ...
    auto u_const_kind  = [](int k){ return k == IR_ATOM || k == IR_LIT_I; };
    auto u_compound_kind = [](int k){ return k == IR_STRUCT || k == IR_ARITH; };
    auto u_build_scalar = [&](int kind, long ival, double dval, const char *lbl) {
