/* bb_sno_assign.cpp — x86() self-encoding (TEMPLATE-REVAMP, 2026-06-02). SNOBOL4 IR_ASSIGN.
   ...
   x86_load_ro / x86_call_ro encoders):
       lit_s : `DST = "literal"`  → rt_sno_assign_lit_s(dst, str)
       var   : `DST = SRC`         → rt_sno_assign_var(dst, src)
...
static inline uint64_t     fn_lit_s()  { void (*f)(const char *, const char *) = rt_sno_assign_lit_s; return (uint64_t)(uintptr_t)(void *)f; }
static inline uint64_t     fn_var()    { void (*f)(const char *, const char *) = rt_sno_assign_var;    return (uint64_t)(uintptr_t)(void *)f; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static std::string bb_sno_assign_str() {
    if (PLATFORM_X86) {
        if (_.op_a_node_kind == (int)IR_LIT_S)
            return IF(MEDIUM_TEXT, s_1asm(std::string(_.lbl_α) + ":")
                 ...
                 + x86("call", "rt_sno_assign_lit_s", fn_lit_s())
                 + x86("jmp",  PORT_GAMMA)
                 + x86("def",  PORT_BETA)
                 + x86("jmp",  PORT_OMEGA);
        if (_.op_a_node_kind == (int)IR_VAR)
            return IF(MEDIUM_TEXT, s_1asm(std::string(_.lbl_α) + ":")
                 ...
                 + x86("lea",  "rdi", "[rip + __]", dst_addr(), dst_label())
                 + x86("lea",  "rsi", "[rip + __]", rhs_addr(), rhs_label())
                 + x86("call", "rt_sno_assign_var", fn_var())
                 + x86("jmp",  PORT_GAMMA)
                 + x86("def",  PORT_BETA)
                 + x86("jmp",  PORT_OMEGA);
        if (_.op_a_node_kind == (int)IR_BINOP)
            return x86_bomb("bb_sno_assign int-binop: needs the binop's ζ-slot offset promoted onto _ (op_a_slot)");
        return x86_bomb("bb_sno_assign concat/other: relocatable form is STITCH_SEQ (PB-RB-4), not a box rewrite");
    }
    return std::string();
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
extern "C" void bb_sno_assign(IR_t * pBB) { (void)pBB; bb_emit_x86(bb_sno_assign_str()); }
