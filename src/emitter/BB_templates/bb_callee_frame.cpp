...
static std::string bb_callee_frame_str() {
    if (PLATFORM_X86)
        return (_.op_parts_n == 0 || _.op_parts_ival[0] < 0 || _.op_parts_ival[0] > 2 || _.op_parts_ival[1] < 0) ? x86_bomb("bb_callee_frame: unadmitted callee shape reached the emitter")
             : IF(_.op_sa == 0,
