...
static void flat_drive_capture(IR_t *pBB, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    EMIT_PAIR_FILL(pBB, cap_γ, lbl_ω, lbl_β);
    {
        ...
        if (catn == 0) { IR_t *sc = ch; while (sc && sc != pBB && is_pat_chain_elem(sc->op) && stop_n < 64) { if (sc->op == IR_MATCH_ARBNO) stop_has_arbno = 1; stop_arms[stop_n++] = sc; sc = sc->γ.node; } }
        if (catn >= 2) {
        ...
        } else if (ch->γ.node && ch->γ.node->op == IR_MATCH_ALT) {
...
static void flat_drive_gz_query(IR_t *pBB, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    gz_collect_callees(hd, &cv);
    if (twoseg) gz_collect_callees(hdB, &cv);
    g_emit.op_sa = 0;
    g_emit.op_sb = 0;
    ...
    for (IR_t *g = hd; g; g = g->γ.node) {
        ...
        gz_emit_cell(g, next_γ, gw, gb[i], NULL, &cv);
        g_emit.op_bounded = 0;
        i++;
    }
    if (twoseg) {
        ...
        for (IR_t *g = hdB; g; g = g->γ.node) {
            ...
            g_emit.op_bounded = gz_node_bounded(g) ? 1 : 0;
            gz_emit_cell(g, next_γ, gw, hb[i], NULL, &cv);
            g_emit.op_bounded = 0;
            i++;
        }
    }
    ...
    for (int k = 0; k < cv.n; k++) gz_emit_callee(cv.v[k], &cv);
...
static void flat_drive_gvar_concat(IR_t *nd, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    EMIT_PAIR_RESET();
    EMIT_PAIR_DEF_JMP(lbl_β, lbl_ω);
    { IR_e _sk = nd->op; nd->op = IR_BINOP_GVAR_CONCAT; EMIT_PAIR_FILL(nd, lbl_γ, lbl_ω, lbl_β); nd->op = _sk; }
}
/*--------------------------------------------------------------------------------------------------------------------*/
...
static void flat_drive_match(IR_t *pBB, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    if (!elem) {
        fprintf(stderr, "[SBB] FATAL flat_drive_match: IR_MATCH has no element in operands\n");
        abort();
    }
    if (g_subject_slot < 0) {
    ...
    if (catn == 0 && elem->γ.node && elem->γ.node->op == IR_MATCH_ALT) {
    ...
    if (catn == 0 && inline_alt_n < 2 && elem->γ.node) {
        ...
        int real_sibling = (gt != IR_SUCCEED && gt != IR_FAIL) &&
                           (is_pat_chain_elem(gt) || (gt == IR_MATCH_CAT && bb_match_nkids(elem->γ.node) == 0));
        if (real_sibling) {
...
void walk_bb_flat(IR_t *nd, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    switch (nd->op) {
    case IR_MATCH_LIT:    FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_ARB:    g_emit.x86_scratch_off = bb_slot_claim(8); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_REM:    FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_SPAN:   g_emit.op_name1 = IR_LIT(nd).sval ? IR_LIT(nd).sval : ""; g_emit.op_name2 = "bb_span";   g_emit.op_kind = "SPAN";   g_emit.x86_scratch_off = bb_slot_claim(8); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_SPAN_VAR: g_emit.op_name1 = IR_LIT(nd).sval ? IR_LIT(nd).sval : ""; g_emit.op_name2 = "bb_spanv"; g_emit.op_kind = "SPANV"; g_emit.x86_scratch_off = bb_slot_claim(16); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_ANY:    g_emit.op_name1 = IR_LIT(nd).sval ? IR_LIT(nd).sval : ""; g_emit.op_name2 = "bb_any";    g_emit.op_kind = "ANY";    FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_BREAK:  g_emit.op_name1 = IR_LIT(nd).sval ? IR_LIT(nd).sval : ""; g_emit.op_name2 = "bb_brk";    g_emit.op_kind = "BREAK";  g_emit.x86_scratch_off = bb_slot_claim(4); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_BREAKX: g_emit.op_name1 = IR_LIT(nd).sval ? IR_LIT(nd).sval : ""; g_emit.op_name2 = "bb_brkx";   g_emit.op_kind = "BREAKX"; g_emit.x86_scratch_off = bb_slot_claim(8); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_NOTANY: g_emit.op_name1 = IR_LIT(nd).sval ? IR_LIT(nd).sval : ""; g_emit.op_name2 = "bb_notany"; g_emit.op_kind = "NOTANY"; FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    ...
    case IR_MATCH_ABORT:  { bb_label_t *seal = g_scan_seal_lbl ? g_scan_seal_lbl : lbl_ω; FILL(nd, lbl_γ, seal, lbl_β); break; }
    ...
    case IR_MATCH_ASSIGN_IMM:  flat_drive_capture(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_MATCH_ASSIGN_COND: flat_drive_capture(nd, lbl_γ, lbl_ω, lbl_β); break;
    ...
    case IR_BINOP: {
        ...
        if (g_gvar_flat_chain && op_is_pow && bb_child0(nd) && bb_child1(nd) && nd->γ.node && nd->γ.node->op == IR_ASSIGN && IR_LIT(nd->γ.node).sval
            ...
            EMIT_PAIR_DEF_JMP(lbl_β, lbl_ω);
            { IR_e _sk = nd->op; nd->op = IR_BINOP_GVAR_ARITH; EMIT_PAIR_FILL(nd, lbl_γ, lbl_ω, lbl_β); nd->op = _sk; }
        } else if (g_gvar_flat_chain && op_is_arith && bb_arith_is_dynamic(nd) && (bb_child0(nd)->op == IR_IDX || bb_child1(nd)->op == IR_IDX)) {
            IR_t *c0 = bb_child0(nd), *c1 = bb_child1(nd);
            int sa = (c0->op == IR_IDX) ? gvar_prewalk_idx_operand(c0, lbl_ω) : bb_slot_get(c0);
            int sb = (c1->op == IR_IDX) ? gvar_prewalk_idx_operand(c1, lbl_ω) : bb_slot_get(c1);
            if (sa < 0) sa = bb_slot_claim(16);
            if (sb < 0) sb = bb_slot_claim(16);
            g_emit.bb_lk    = (int)ir_norm_call_kind(c0->op);
            g_emit.bb_rk    = (int)ir_norm_call_kind(c1->op);
            g_emit.bb_li    = (c0->op == IR_LIT_I) ? IR_LIT(c0).ival : 0;
            g_emit.bb_ri    = (c1->op == IR_LIT_I) ? IR_LIT(c1).ival : 0;
            g_emit.op_name1 = (c0->op == IR_VAR) ? IR_LIT(c0).sval : (const char *)0;
            g_emit.op_name2 = (c1->op == IR_VAR) ? IR_LIT(c1).sval : (const char *)0;
            g_emit.op_sa    = sa;
            g_emit.op_sb    = sb;
            g_emit.op_arith_descr = 1;
            g_emit.op_off   = bb_slot_alloc16(nd);
            EMIT_PAIR_RESET();
            EMIT_PAIR_DEF_JMP(lbl_β, lbl_ω);
        ...
        } else if (g_gvar_flat_chain && op_is_concat && bb_child0(nd) && bb_child1(nd)) {
            flat_drive_gvar_concat(nd, lbl_γ, lbl_ω, lbl_β);
    ...
    case IR_INDIRECT_ASSIGN_VAR:   flat_drive_indirect_assign(nd, lbl_γ, lbl_ω, lbl_β); break;
    ...
    case IR_MATCH:  flat_drive_match(nd, lbl_γ, lbl_ω, lbl_β); break;
    ...
    case IR_PATTERN_ANY:    g_emit.op_kind = "ANY";    g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_dval = IR_LIT(nd).dval; g_emit.op_off = bb_slot_alloc24(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_PATTERN_NOTANY: g_emit.op_kind = "NOTANY"; g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_dval = IR_LIT(nd).dval; g_emit.op_off = bb_slot_alloc24(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_PATTERN_SPAN:   g_emit.op_kind = "SPAN";   g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_dval = IR_LIT(nd).dval; g_emit.op_off = bb_slot_alloc24(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_PATTERN_BREAK:  g_emit.op_kind = "BREAK";  g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_dval = IR_LIT(nd).dval; g_emit.op_off = bb_slot_alloc24(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_PATTERN_BREAKX: g_emit.op_kind = "BREAKX"; g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_dval = IR_LIT(nd).dval; g_emit.op_off = bb_slot_alloc24(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    ...
    case IR_PATTERN_CAPTURE: g_emit.op_kind = "CAPTURE"; g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_ival = IR_LIT(nd).ival; g_emit.op_off = bb_slot_alloc24(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
...
void emit_jcon_node(IR_t *nd, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_KEYWORD:    g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_off = jcon_value_slot(nd); FILL(nd, lbl_γ, lbl_ω, lbl_β); break;
    case IR_VAR: {
