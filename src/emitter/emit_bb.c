...
static void flat_drive_every(IR_t *pBB, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    if (((IR_t*)0)) {
        if (IR_LIT(pBB).ival == 0 && ((IR_t*)0) &&
            ...
            walk_bb_flat(((IR_t*)0), gen_resume, gen_resume, body_βw);
            EMIT_PAIR_RESET();
            EMIT_PAIR_DEF_JMP(lbl_β, lbl_ω);
            EMIT_PAIR_FILL(pBB, lbl_γ, lbl_ω, lbl_β);
            return;
        ...
        if (IR_LIT(pBB).ival == 2 && bb_child0(pBB) && bb_child0(pBB)->op == IR_ASSIGN &&
            ...
            (void)gen;
            walk_bb_flat(((IR_t*)0), store_α, lbl_γ, gen_resume);
            emit_label_define_bb(store_α);
        ...
        if (((IR_t*)0) && ((IR_t*)0)->op == IR_ASSIGN && IR_LIT(((IR_t*)0)).ival == 1 && ((IR_t*)0)->γ.node) {
            ...
            walk_bb_flat(((IR_t*)0)->γ.node, β, β, body_β);
        } else {
            walk_bb_flat(((IR_t*)0), β, β, body_β);
        }
        EMIT_PAIR_RESET();
        EMIT_PAIR_DEF_JMP(lbl_β, lbl_ω);
        EMIT_PAIR_FILL(pBB, lbl_γ, lbl_ω, lbl_β);
...
void walk_bb_flat(IR_t *nd, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_BINOP: {
        ...
        if (g_gvar_flat_chain && op_is_pow && bb_child0(nd) && bb_child1(nd) && bb_child0(nd)->op == IR_LIT_I && bb_child1(nd)->op == IR_LIT_I && nd->γ.node && nd->γ.node->op == IR_ASSIGN && IR_LIT(nd->γ.node).sval) {
            ...
            if (_c0 && _c1 &&
                ((_c0->op == IR_LIT_I || _c0->op == IR_LIT_NUL) || (_c0->op == IR_VAR && IR_LIT(_c0).sval) || bb_slot_get(_c0) >= 0) &&
    ...
    case IR_ASSIGN:     { IR_t *ac0 = bb_child0(nd);
        ...
        else if (IR_LIT(nd).sval && ac0 && (ac0->op == IR_LIT_S || ac0->op == IR_LIT_I || ac0->op == IR_LIT_F || ac0->op == IR_VAR || ac0->op == IR_SEQ || ac0->op == IR_SEQ_EXPR || ac0->op == IR_CALL)) flat_drive_gvar_assign(nd, lbl_γ, lbl_ω, lbl_β);
...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    while (qh < qt) {
        ...
        if (c->γ.node && qt < CH_MAX) queue[qt++] = c->γ.node;
        if ((c->op == IR_BINOP || c->op == IR_BINOP_GEN) && c->ω.node && qt < CH_MAX) queue[qt++] = c->ω.node;
        if (c->op == IR_CALL && c->ω.node && qt < CH_MAX) queue[qt++] = c->ω.node;
...
static void descr_chain_operand_refs(IR_t *entry) {
    ...
    while (sv > 0 && nc < 512) {
        ...
        if (c->op == IR_CALL && c->ω.node && sv < 512) stkv[sv++] = c->ω.node;
