...
void walk_bb_flat(IR_t *nd, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_BINOP: {
        ...
        if (g_gvar_flat_chain && op_is_pow && bb_child0(nd) && bb_child1(nd) && nd->γ.node && nd->γ.node->op == IR_ASSIGN && IR_LIT(nd->γ.node).sval
            ...
            { int _eqne = (IR_LIT(nd).ival == BINOP_EQ || IR_LIT(nd).ival == BINOP_NE); int _lok = (_c0->op == IR_CALL) || (_c0->op == IR_VAR && IR_LIT(_c0).sval) || (_c0->op == IR_LIT_S && IR_LIT(_c0).sval); int _rok = (_c1->op == IR_CALL) || (_c1->op == IR_VAR && IR_LIT(_c1).sval) || (_c1->op == IR_LIT_S && IR_LIT(_c1).sval); int _trig = (_c0->op == IR_CALL) || (_c1->op == IR_CALL) || (_c0->op == IR_LIT_S) || (_c1->op == IR_LIT_S) || (IR_LIT(nd).dval == 1.0); g_emit.op_relop_descr = (_eqne && _lok && _rok && _trig) ? 1 : 0; }
