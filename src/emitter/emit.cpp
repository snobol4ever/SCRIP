...
void emit_drive(IR_t *nd, bb_label_t *lbl_α, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_MATCH_REM: case IR_MATCH_ARB: case IR_MATCH_BAL: {
        g_emit.x86_scratch_off = drive_value_slot(nd);
        g_emit.op_leaf_frame_off = leaf_frame_slot(nd);   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s131 SLICE 2): REM and ARB are admitted; ⛔ BAL SELF-DECLINES INSIDE leaf_frame_candidate() (it spends +0/+4/+8 and d=8 is the neighbour's granule floor) and so is staged -1 here, keeping bb_match_bal.cpp byte-identical on its legacy spelling.  The decline lives in the PLANNER, never in this staging line and never in the template, so the carve and the address can never disagree about which ops are framed. */
        DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    }
    case IR_MATCH_VALUE: {
        g_emit.op_sa = -1; g_emit.op_off = -1;
...
static int leaf_frame_candidate(const IR_t * nd) {   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME CANDIDATE (s130, witness corpus/probe/clobarm/): a SCRATCH-CELL LEAF MATCHER that lies ON AN ALT ARM.  THE TWO CONJUNCTS, each measured:
    ...
    switch (nd->op) { case IR_MATCH_SPAN: case IR_MATCH_BREAK: case IR_MATCH_BREAKX: case IR_MATCH_TAB: case IR_MATCH_RTAB: case IR_MATCH_REM: break; default: return 0; }   /* ⭐⭐⭐ SLICE 2 (s131) — THE ADMITTED SIX, EACH ADMITTED BY A MEASURED GREEN AND NOT BY FAMILY MEMBERSHIP.  s130 proved the re-home on SPAN alone and named the widening "mechanical"; IT IS NOT, and this whitelist is where that is spelled.  WITNESSED RED, DEFAULT ARM, BOTH MEDIA (corpus/probe/leafsib/, one witness per op, each modelled exactly on the cured clobarm/clob_altarm_arm2direct_red, each oracle-pinned to `id=iffoo`): the family measured 0/8 in m3 AND m4 — SPAN/TAB/RTAB/REM/BREAK/BREAKX rc=139, ARB/BAL rc=0 with a SILENT WRONG ANSWER.  The six admitted here measured 6/6 GREEN on the framed arm in BOTH media.  ⛔ TWO ARE DELIBERATELY ABSENT, EACH FOR ITS OWN MEASURED REASON, AND NEITHER IS AN OVERSIGHT: (1) BAL is the ONE sibling that spends +0/+4/AND +8 (bb_match_bal.cpp, 3 refs at d=8) — d=8 is the NEXT GRANULE'S FLOOR, i.e. a neighbouring ARBNO/CAPTURE-SAVE/FENCE1 cell, so admitting it would hand a neighbour's slot to BAL's third word: the SAME cross-owner overwrite this rung exists to kill, moved indoors, and INVISIBLE IN ITS OWN WITNESS (BAL would go green while corrupting a sibling class).  It joins SPAN's `*var` arm in the named 12B/16B wide class, whose two-consecutive-slot grant moves every other customer's offsets and so earns its own rung and its own blast radius.  (2) ARB is a DIFFERENT DEFECT CLASS ENTIRELY, convicted by the IPC sync-step MONITOR (PARTICIPANTS="spl scr", s131): at DIVERGE step 6 the oracle assigns I=STRING(5)='iffoo' while SCRIP branches to stno=5 — the match FAILS OUTRIGHT, so ARB never resumes into the arm interior to extend from null and ITS CELL IS NEVER RE-ENTERED.  Re-homing an address that is never reached is an unproven admission that widens every frame for no measured green; ARB belongs to the s120/s122/s123 arm-interior RESUME class and waits for that rung.  Its template already routes through LFC/LFCQ, which on the declined arm returns the legacy FR() spelling byte-identically, so that rung inherits the plumbing and changes one word here.  An unlisted op refuses by construction (the s125 whitelist principle). */
    if (nd->op == IR_MATCH_SPAN) { const char * sv = IR_LIT(nd).sval; if (sv && sv[0] == '*') return 0; }   /* ⛔⭐⭐⭐ THE 8-BYTE USABLE-WINDOW LAW, MEASURED s130 -- DO NOT DROP THIS CONJUNCT.  The registry tiles its region as ONE 16B GRANULE PER SLOT with the base 8 BYTES ABOVE THE GRANULE FLOOR: for a blob, blob_frame_bytes()=24+16*count carves [-(24+16*count), -24) below the r10/r11/rdx entry wires, and frame_slot_off()=-(32+16*idx) places idx k's granule at [base-8, base+8) -- the three-candidate case tiles [-72,-24) EXACTLY, floor to ceiling, with no slack anywhere.  So a slot offers d ∈ {0,4} above its base and NOTHING MORE: d=8 is the NEXT granule's floor, i.e. its neighbour's cell.  bb_match_span.cpp's `SPAN(*var)` deferred-by-name arm (sval[0]=='*') is the ONE arm that spends the full 16B ZLS cell -- lea rsi,SPC(0) / lea rdx,SPC(8) / mov r8,SPCQ(0) / mov eax,SPC(8), the {ptr,len} pair rt_pat_prim_str fills -- so re-homing it would hand the neighbouring capture SAVE's cell to a runtime writer.  That is the SAME cross-owner overwrite this whole rung exists to kill, just moved indoors, and it would have been INVISIBLE in the witness (which is a literal charset and never reaches this arm).  Declining leaves that arm on its legacy spelling, byte-identical.  THE FOLLOW-UP IS NAMED, NOT FORGOTTEN: grant the 16B class TWO CONSECUTIVE slots (or widen the granule) -- a registry change that moves ARBNO/CAPTURE/FENCE1 offsets too, hence its own rung with its own blast radius, never a quiet widening of this one. */
...
static int frame_slot_scan(const IR_t * query, int * out_index, int * out_count) {   /* ⭐⭐⭐ ACTIVATION-FRAME SLOT REGISTRY (THREE ZETAS s87): THE ONE SCAN every public authority below calls -- walks
     ...
     * MATCH_BEGIN was found (0 declines everything -- template and carve both fall back to legacy/bomb, the same graceful-decline shape arbno_frame_scan already had for the PAT$N stored-pattern-blob class,
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void sn4_blob_choice_scan(int * n_choice, int * leaf_ok, int * has_fence) {   /* ⭐ s127 ONE SCAN, TWO READERS (β-dispatch admission + blob_frame_bytes widening -- never spell the classification twice, the s68/s70 disease): walks THIS blob graph counting unsealed choice nodes, flagging FENCE1, and verifying every non-choice member is a pure scanner-register leaf (the same whitelist as the :1143 span scan; an unrecognised op refuses by construction, the s125 whitelist principle). */
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0; int arm_zd = 0; int zdh_match = -1;   /* M-2 BUG-2 FIX: zdh_match = zd at MATCH_BEGIN entry (= depth of pre-match producer cells only); used as gpop at STATEMENT_END when hfc because the cas_rsp_mark restore collapses all in-bracket cells (pattern-arg cells like LIT_INTEGER), leaving only the pre-match producers for STATEMENT_END to release.  -1 = no hfc MATCH_BEGIN seen yet. */
            ...
            for (int r = 0; r < rl; r++) rpos[run[r]] = -1;
               ...
               { static int _cd = -1; if (_cd < 0) { const char *_e = getenv("SCRIP_CALL_DIAG"); _cd = (_e && _e[0] == '1') ? 1 : 0; }   /* ZD-7 TAG-SAFE CALLEE CENSUS: partition declined-run first blockers by kind; tag-safe: IR_CALL_VALUE excluded (no sval); IR_DEFINE excluded (ival=role, not a pointer); all other ir_norm_call_kind==IR_CALL sub-kinds write sval in every lowerer. */
    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_β);
    {
        ...
        { extern int sn4_defer_resume(void); const int _dres = sn4_defer_resume();   /* ⭐ SN4-DEFER-RESUME (s121) half B2: the SNOBOL4 arm of the resume dispatch, mirroring the two Prolog arms above.  A PAT$ blob (flat_pat graph) β-arrival — the invoker's `jmp [rsp+0]` through the CLASS D suspension record, res stub restored wires+frame, interior state retained on the spine below — lands the published resume carrier's β: a generator's β EXTENDS (ARBNO one more instance, manual p.121; ARB one more char), a deterministic carrier's β fail-throughs to its ω and rides r11 home through the absolute unwind.  Without this arm the dispatch defaulted to lbl_ω: every stored ARB/ARBNO answered exactly once and took its shy null match forever — the s120 FINDING's closed SUCCEED/FAIL pair, beauty's `Parse Error`.  Same switch as the lowering half; =0 restores the default byte-identically. */
          if (_dres && g_emit.flat_pat && g_emit_cfg && g_emit_cfg->body_root) {
            int _f1 = 0; for (int i = 0; i < n; i++) if (nodes[i]->op == IR_MATCH_FENCE1) { _f1 = 1; break; }   /* ⭐ SN4-DEFER-RESUME FENCE DECLINE (s121): belt to the builder's suspenders — a FENCE1 node anywhere in the blob graph declines the resume arm even if a carrier slipped through (FENCE0 is node-free and only the builder's tree check can see it; FENCE1 leaves this node and both ends catch it).  Declined = the pre-s121 lbl_ω default verbatim. */
