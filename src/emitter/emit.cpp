...
void emit_drive(IR_t *nd, bb_label_t *lbl_α, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_MATCH_REM: case IR_MATCH_ARB: case IR_MATCH_BAL: {
        g_emit.x86_scratch_off = drive_value_slot(nd);
        g_emit.op_leaf_frame_off = leaf_frame_slot(nd);   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s131 SLICE 2): REM and ARB are admitted; ⛔ BAL SELF-REFUSES INSIDE leaf_frame_member() (it spends +0/+4/+8 and d=8 is the neighbour's granule floor) and so is staged -1 here, keeping bb_match_bal.cpp byte-identical on its legacy spelling.  The refuse lives in the PLANNER, never in this staging line and never in the template, so the carve and the address can never disagree about which ops are framed. */
        DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    }
    case IR_MATCH_VALUE: {
        g_emit.op_sa = -1; g_emit.op_off = -1;
