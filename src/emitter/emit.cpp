...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    case IR_MATCH_FENCE1:          { bb_prepare(nd); bb_emit_x86(bb_match_fence1()); } return 0;     /* SYNC-POINT ζ RELEASE sync point 2: NO fc_geom BY DESIGN — the watermark quad must stay [rbp+off] (depth-immune) because the σ glue reads it at the dynamic post-P depth */
