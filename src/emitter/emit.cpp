...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_patref_on(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZD_PATREF"); v = (e && *e == '1') ? 1 : 0; } return v; }   /* ⭐ ZD-PATREF gate (OMEGA s39): ONE reader for BOTH authorities -- zd_k's K verdict and zd_wl_kind's admission verdict -- so the switch cannot be half-flipped.  Two spellings of one gate is the s22k skew disease in miniature; this helper is why there is only one. */
