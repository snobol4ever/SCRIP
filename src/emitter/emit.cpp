...
static long g_zd_uclaim, g_zd_udout; static int g_zd_uhead = -1; static int g_zd_uzw;   /* ZW-12: staged beside the UCLAIM twins, applied at the same choke */   /* UCLAIM staging twins (wholesale flip): filled beside the ZD set at the drive loop, applied at the choke, cleared with the stage flag */   /* ZD-1 driver-handoff staging (R5b): the drive loop fills these from zd_plan's arrays immediately before emit_drive; the choke below applies them to g_emit exactly once and CLEARS the stage flag, so every other walk_bb_node entry path (FILL / EMIT_PAIR_FILL glue) sees zeros and stays byte-identical. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_uclaim = 0; g_emit.op_udout = 0; g_emit.op_uhead = -1; g_emit.op_zw = 0;   /* UCLAIM: cleared at the choke UNCONDITIONALLY, same law as the ZD set above */
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead; g_emit.op_zw = g_zd_uzw;   /* UCLAIM applied regardless of g_zd_arm -- declined runs are its whole constituency */
...
#undef RPO_VISITED
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0 || zd_zw[i]) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = (_zw5_wpop_stolen || _endj_stolen) ? 0 : zd_wp[i]; g_zd_wsteal = _endj_stolen; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i]; g_zd_uzw = zd_zw[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; ZW-5 O-2: g_zd_wpop=0 when stub owns release */
