...
static int zd_wl_kind(IR_t * nd) {
    ...
    if (op == IR_VAR_REF) { static int _plvr = -1; if (_plvr < 0) { const char * e = getenv("SCRIP_ZD_PL_VR"); _plvr = (e && *e == '0') ? 0 : 1; } return (_plvr && g_emit_cfg && g_emit_cfg->pl_cells_graph) ? 1 : 0; }   /* PL-ZK-1 arm 2/5: IR_VAR_REF admitted on the cells arm.  Census (s3): second-most-frequent blocker (13/6/35/9 occurrences).  K=16 default; nops=0 (no inline run-predecessor -- VAR_REF reads a named variable slot, not a spine cell; template ZD arm will read from slot).  Killswitch SCRIP_ZD_PL_VR=0.  ⛔ ZK-2 FINDING (s220): Icon VAR_REF ZD arm REVERTED -- bb_subscript ZD arm reads ZOPQ(0,0/8) as {DT_TABLE,table_ptr} directly, but VAR_REF writes {DT_N=1,&slot} (a reference cell); the subscript template must dereference operand-0 before calling rt_subscript.  All consumers of VAR_REF on the cells arm need matching dereference arms before admission is sound.  MEASURED: 6 regressions (rung23/rung35 table tests + jcon_kross), all silent-wrong-output rc=0.  Deferred to the consumer-update rung. */
