    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                { if (!gin && gt && gt->op == IR_SUCCEED && g_emit_cfg && g_emit_cfg->icn_cells_graph) gin = 1; }   /* ZK-3 ADDITIVE (SUCCEED-GIN): on the cells arm, a gamma arc terminating at IR_SUCCEED is intra-statement by the non-popping FORTH spine law. IR_SUCCEED is the value-produced relay; the drive loop routes it to the consumer beta dynamically (e.g. CALL_BUILTIN(write) gamma->SUCCEED->IR_TO.beta in every-write-1-to-3). Setting zgpop on this arc fires add rsp,K_total inside the generator loop, freeing TO counter and limits -- IR_TO.beta then reads freed stack (SEGV). SUCCEED gamma always routes to a consumer beta, never to the statement terminal, so cells are live and must not be released here. Release authority: the statement terminal via the run-head-staged op_zgpop. GATED icn_cells_graph: SN4/Prolog byte-identical. ONE-LINE ONLY per s22k law. */
...
#define RPO_PUSH_SUCCS(c) \
        ...
        RPO_PUSH((c)->γ.node); \
        if (((c)->op == IR_MATCH_ALTERNATE || (c)->op == IR_MATCH_ARBNO || (c)->op == IR_MATCH_FENCE1 || (c)->op == IR_SCAN_SEQUENCE || (c)->op == IR_SCAN_ALTERNATE || (c)->op == IR_DISJUNCTION) && (c)->n_operands > 0) for (int _oi = (c)->n_operands - 1; _oi >= 0; _oi--) RPO_PUSH((c)->operands[_oi]); \
