    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (r == hpos) zdh = zd;
                if ((zws || zwr) && nodes[i]->op == IR_MATCH_END) zd -= (zws ? (int)ZW_FRAME_TOTAL : 8);   /* ⭐ ZW-12: END's zout (and the terminal-cut pops computed below it) are POST-WHACK coordinates -- `mov rsp,rbp; pop rbp` lands rsp exactly at the head-α claim base, so END's own claim-slot stores (end_δ +24) and every downstream reader resolve through the unchanged machinery.  The subtraction PRECEDES staging so op_udout carries the restored depth; the whack is the emitted twin of this line.  ⭐ MECHANISM-2: same law; 8B instead of ZW_FRAME_TOTAL (push only). */
