...
#undef RPO_VISITED
    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_ω); bb_emit_x86(bb_glue_outer_ω());   /* OUTER-EXIT-1 (Lon s22p): the outermost box owns its OWN two ports, as glue -- this is the whole of what the deleted epilogue did in the ordinary case.  lbl_ω was left UNDEFINED by the s22o deletion while every fail edge still jumped to it, which is the single reason `--compile` produced no assemblable .s for 317/317 programs: one undefined symbol, not 317 defects.  The landings are RETURNS (main CALLs the chain) carrying the eax code the caller reads; no stack motion, because with no prologue the outermost box carved nothing -- see the whack note in bb_glue_flat.cpp for the one line that changes when the spine goes non-popping. */
