...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        { if (ok && g_emit_cfg && g_emit_cfg->icn_cells_graph) { for (int _rb = 0; _rb < rl && ok; _rb++) { IR_t *_gt = zd_chase(nodes[run[_rb]]->γ.node); if (!_gt) continue; for (int _rg = 0; _rg < _rb && ok; _rg++) { if (nodes[run[_rg]] == _gt && ir_is_generator_kind(nodes[run[_rg]]->op)) { ok = 0; why = "gen-loop-body"; badi = run[_rg]; rgood = _rg; } } } } }   /* ⭐ ZK-2 ZD-GEN-LOOP-BODY DECLINE (this rung, measured): runs where a downstream armed node has a backward γ→generator-kind-member (the every-loop-body topology: IR_ASSIGN.γ→IR_TO.β) must DECLINE the cells arm.  ROOT CAUSE: gpop at the downstream terminal (IR_ASSIGN) fires add rsp,K_total before β, restoring RSP to statement_entry.  The generator's ZRES(0/8) lives on the RSP spine and is freed by this release.  On the next β pass, downstream nodes re-execute (they are deterministic, not suspended) and re-carve fresh cells above TO's block — but ZRES has been freed, so any write to it corrupts the rebuilt spine.  The counter/limit can survive in FRQ (depth-immune), but ZRES cannot survive the gpop release.  ZK-6 (suspended-state mechanism) is the correct rung for this topology.  GATED icn_cells_graph: SN4/Prolog byte-identical.  KILLSWITCH: none needed (additive decline; CELLS=0 flat arm handles it correctly).  s22k law: no K constant spelled here. */
