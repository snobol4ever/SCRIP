...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        int zdyn = 0; for (int k = 0; k < n && !zdyn; k++) { int o = (int)nodes[k]->op; if (o == IR_MATCH_DEFER || o == IR_MATCH_PATREF || o == IR_MATCH_FENCE1) zdyn = 1;   /* PATREF measured both ways (this session): narrowed out, 117/142 arm GREEN but 135/136_pat_balanced_parens_* crash (recursive stored patterns re-enter the blob at depths the armed geometry does not yet model) -- so PATREF stays declined, degrade never die.  The 117/142 green is the named signal for the NEXT rung: the static-shape patref population can arm once the recursive-re-entry class is bracketed. */ }   /* ⭐ ZD-5 DYNAMIC-BOX DECLINE (mirror of s22y's subject-cell rule, same three kinds, same reason): graphs bearing DEFER/PATREF/FENCE1 re-enter blobs through the stored-pattern/fence protocol whose geometry (ARBNO deep repoint static constant, blob wire glue) predates cells above the claim -- arming the quartet there re-created the s22y casualty classes, MEASURED this session as the m3 flip set {135,136,164,165,183} + m4 {117}.  Degrade never die: these statements keep the declined claim regime verbatim. */
