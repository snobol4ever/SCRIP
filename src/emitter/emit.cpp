...
int emit_arbno_rbp(void) { static int _env = -1; if (_env < 0) { const char * e = getenv("SCRIP_ARBNO_RBP"); _env = (e && *e == '0') ? 0 : 1; } return _env && !g_emit.flat_layout_unknown && !g_emit.flat_jmp_entry; }   /* ⭐ ARBNO-RBP (Lon in-chat 2026-08-14, "For ARBNO, ALWAYS create an RBP on ALPHA to carve out RESULT and ONE LOCAL, an integer counter of # instances which when goes negative then causes exhaustion thru OMEGA ... at BETA, the BB LOCAL is directly accessed thru RBP+16 or +0 whichever"): THE ONE AUTHORITY for the rbp-framed ARBNO -- env read HERE only (default ON per the directive), consulted by zd_k's ARBNO arm below (16 -> 32) and staged into g_emit.op_arbno_rbp at the IR_MATCH_ARBNO drive case, so planner accounting and template routing flip on ONE predicate (the s66 two-coherent-worlds law; same discipline as emit_match_rbp above).  SCRIP_ARBNO_RBP=0 restores legacy byte-identically on BOTH sides (the env short-circuits BEFORE the graph flags, so OFF never reads them).  The two graph conjuncts keep the arm on the charted main class: layout-unknown and jmp-entry graphs (the HEAD-PIN rpin() population, plus blob-regime suspend machinery whose interleavings with a live cross-γ rbp are unmeasured) DECLINE to legacy -- conservative by construction, costing only legacy bytes on rare classes.  ⚠ KNOWN-OPEN (measured, probe/arbdef1.sno, 2026-08-14): ARBNO γ-suspended with count>=1 inside a DEFINE body's SUCCESSFUL match leaves rbp pointed at the whacked ARBNO frame past MATCH_END, and the FN_RBP RETURN floater (mov rsp,rbp; pop rbp) tears down through the dead frame -- m4 rc139 zero-output under ON, PASS under OFF; SNOBOL4 DEFINE is runtime so NO static qualification can scope it (function bodies are main-graph statements; a flat_jmp_entry exclusion was tried and is vacuous).  BRACKET POSTMORTEM (attempted + reverted this rung, do not redo blindly): the natural cure is save-rbp-at-MATCH_BEGIN / restore-at-the-three-BRACKET-GATE-stubs (begin fail tail, end release, replace γ -- all empty since the rbp eradication), but BOTH candidate save homes are unsound TODAY: the +40 old-base slot's saved rsp is consumed by the ARB-LON-K16 mechanism-2 statement release (x86_asm.h HEAD-PIN restore, live on op_stmt_dyn statements -- see has_replace_l() field notes), and the +72 capgen slot is NOT granted (the IR_MATCH_BEGIN zls grant is 16B -- head.cursor + zls_mark; the whole +40..+72 quartet is the s22r UNGRANTED-overhang zone riding CRT slack, the exact class the 181-m4 watchpoint conviction caught at movl [rsp+0x170]).  SOUND CURE PATHS for the next rung: (A) re-home the quartet (the OS-2 stfh widening) THEN extend the head grant by one quad for the rbp save, or (B) ~~FUNCTION-floater restores rsp from a per-call activation record~~ **VOID, DISPROVED BY READING s63 (bb_save_restore.cpp:253-267) THE SAME SESSION IT WAS WRITTEN**: under fnrbp()==1 there IS no rbp-independent activation home -- RBP *IS* the innermost activation pin (the shim writes it; [rbp+0]=enclosing rbp, [rbp+8]=gamma, [rbp+16]=omega) and the floater discovers the callee DYNAMICALLY through it, which is the whole point of the frame.  So the collision is structural: ARBNO and the function protocol want the SAME register to mean two different things, and a live ARBNO frame makes rbp mean 'innermost ARBNO' instead of 'innermost activation'.  The 2x2 probe (arbdef1 m4, FN_RBP x ARBNO_RBP, measured 2026-08-14) could not isolate it further because fnrbp()==2 (the rsp-only readers) is INDEPENDENTLY RED on this program under BOTH arbno arms -- only FN_RBP=1/ARBNO_RBP=0 is clean -- so the rsp-only protocol is not a usable control, let alone a cure, until its own red is chased.  SURVIVING CURES, LON'S RULING WANTED: (A) above (bracket restore off a GRANTED save home, quartet re-home first) -- the only one that keeps both meanings and needs no new register; (C) give ARBNO a base register that is not rbp (contradicts the directive as literally worded, so it needs Lon's word, not mine); (D) make MATCH_END drive the teardown of any still-suspended ARBNO frame so rbp is restored by the frame's OWN owner rather than by a bracket that saved a copy -- structurally the cleanest, and it is (A) with the lifetime question asked at the right end.  Until then the leak class is: user-defined function whose body match SUCCEEDS with an interior ARBNO still suspended (count>=1) then RETURNs; corpus incidence measured zero beyond the synthetic witness (claws5-match, the deepest DEFINE+ARBNO program, PASSES both media under ON). */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
