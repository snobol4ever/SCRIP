    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            for (int r = 0; r < rl; r++) { int i = run[r];
                int K = ((int)nodes[i]->op == IR_ASSIGN || (int)nodes[i]->op == IR_GOTO_DEFERRED) ? 0 : 16;   /* ZD-8 (b1) sink */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = zd_wp[i];
              g_zd_k = zd_on[i] ? (((int)nodes[i]->op == IR_ASSIGN || (int)nodes[i]->op == IR_GOTO_DEFERRED) ? 0 : 16) : 0;   /* ZD-8 (b2) THE SECOND K AUTHORITY.  This expression and zd_plan's at :1899 are the SAME RULE SPELLED TWICE; editing one alone makes the planner depth model and the emitted alpha carve disagree by exactly K, which is a silent 16-byte stack skew on every node of the admitted kind (MEASURED: segfault, zero output, on 1020_code_label_transfer).  The tree already carries this law for zd_nops ("THIS IS THE ONE AUTHORITY ... the moment those two disagreed every operand rendered [rsp+0] and 29 programs silently returned wrong answers"); K does NOT yet have it.  Until these two are collapsed to one function, EVERY change to the sink set must edit BOTH. */
