...
static long zvo_uclaim_k(IR_t * nd) {   /* UCLAIM K: the node's statement-scope claim = its flat-plan extent (zw_node_k, liveness-elided) minus any fc_geom cell it self-carves (that 16 is template-carved and legacy-compensated).  Protocol boxes that manage their own frames or transfer control claim nothing -- the four-construct family minus MATCH_HEAD, which is exactly the box whose unclaimed quartet/subject region IS the s22r envp corruption. */
    int op = (int)nd->op;
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        for (int r = 0; r < rl && ok; r++) { int i = run[r];
            { int _a = zd_wl_kind(nodes[i]); int _o = (int)nodes[i]->op; if (_a && zdyn && (_o == IR_MATCH_HEAD || _o == IR_MATCH_SEQUENCE || _o == IR_MATCH_RELEASE || _o == IR_MATCH_REPLACE)) _a = 0;
        ...
        if (ok) { int zd = 0;
            ...
            long Kc = 0; int hpos = -1; IR_t * hnode = 0; for (int r = 0; r < rl; r++) if (nodes[run[r]]->op == IR_MATCH_HEAD) { hpos = r; hnode = nodes[run[r]]; break; }
