    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                { int oib = port_sz_beta(nodes[i]->ω.sz); if (!oin && oib && K == 0 && !beta_is_stmt_land(ot)) oin = 1; }   /* ZD-5b (s24b):
                { if (!oin && zws && ot) { for (int k = 0; k <= r; k++) { if (nodes[run[k]] == ot) { oin = 1; break; } } } }   /* ⭐ M-1 FIX 3 (C-4 CLIMB recovery): ZWS backward-edge oin guard.  The k>r forward-only test on line 2093 sets oin=0 for any node whose ω target appears EARLIER in the run (k <= r) — a backward edge.  Under ZWS (op_zw=1 canonical frame) this wrongly stages wpop = zd - K + kc at that node, releasing the subject VAR cell (and the statement claim Kc) on every scan retry, BEFORE match_end (the sole release authority) fires.  Measured: MATCH_LIT whose ω is MATCH_BEGIN's retry beta, k=2 < r=5 in the G06 graph, wpop=224 on each '+' probe step, use-after-free on the VAR cell; F03/F05/G06/G22/G23 m4 crash.  m3 survives because the VAR read precedes the ω add rsp; m4's compiled binary enforces stack discipline strictly.  THE FIX: when zws is armed, scan the run for the ω target at positions 0..r inclusive; if found, treat as interior (oin=1), suppressing wpop.  GATED zws: non-ZWS runs use the original k>r test unchanged (byte-identical).  ONE LINE per s22k law. */ blob-interior K=0 leaf kinds (LIT/LEN/ANY/NOTANY/SPAN/TAB/RTAB/POS/RPOS/REM) wire omega to MATCH_BEGIN's BETA (the scanner retry path, port tag 0xce 0xb2).  The k>r forward-only guard above misses this backward beta target (MATCH_BEGIN is at r=1, LIT at r=2, so k=1 < r=2 and oin stays 0).  CONSEQUENCE: wpop = zd - K + kc releases the subject VAR cell BEFORE the beta retry reads it -- use-after-free on the scanner loop.  THE CORRECT RULE: a blob-interior K=0 leaf whose omega IS a beta re-entry frees NOTHING at omega (MATCH_END is the sole release authority for the whole match extent); the beta-tag sniff is the canonical detection -- same tag the drive loop reads at emit.cpp:2368 to route the omega wire.  GATED K==0: K>0 leaf kinds (if any are ever admitted) may have different topology; this fix is strictly for the zero-cell scanner-register class. */
