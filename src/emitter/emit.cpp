...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_nops(IR_t * nd) { int op = (int)nd->op; return (op == IR_UNOP || op == IR_ASSIGN || op == IR_DEREF || op == IR_COERCE_STRING || op == IR_COERCE_INTEGER || op == IR_FIELD_VAR) ? 1 : (op == IR_BINOP || op == IR_COERCE_NUMERIC || op == IR_CMP_TEST || op == IR_SUBSCRIPT || op == IR_ASSIGN_VAR) ? 2 : 0; }   /* ZD-2d/2e/2f: COERCE_NUMERIC reads operand 0 as the value and operand 1 as the partner type-peek; CMP_TEST takes both coerced operands by address; SUBSCRIPT takes base and index by value.  All stage two op_zread differences.  THIS IS THE ONE AUTHORITY -- the drive-loop staging site calls it rather than re-deriving, since the moment those two disagreed every operand rendered [rsp+0] and 29 programs silently returned wrong answers. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    { static int _stf = -1; if (_stf < 0) { const char * e = getenv("SCRIP_STMT_FRAME"); _stf = (e && *e == '0') ? 0 : 1; }   /* ⭐ STF-FLIP (s21x-y): DEFAULT FLIPPED OFF -> ON, and `SCRIP_STMT_FRAME=0` is now the killswitch.  This is the moment the "C-style RBP used occasionally" of the design of record stops being dormant scaffolding and becomes the shipped regime: under it a STATEMENT is bracketed by the rbp dance (bb_glue_framed_enter/leave) and bb_match_head's five POST-UNWIND-lifetime fields move from the whole-graph flat slots FRQ(op_off+40..72) onto HKQ = [rbp-48+8k], which is DEPTH-FREE BY CONSTRUCTION -- the value spine keeps riding RSP FORTH-style while exactly the housekeeping that must survive an unwind rides a stable base.  LICENSED BY THE CONDITION THIS LINE ITSELF NAMED ("default OFF until the watermark license ... fail-set diffed programmatically"), now satisfied and MEASURED AT THIS HEAD, not inherited: crosscheck 317 m3 232/85 - m4 229/86/2 - DIV=1 and broad 336 m3 244/92 - m4 238/90/8, and in BOTH corpora BOTH modes the fail set is BYTE-IDENTICAL to the regime-off run.  ⚠ NOT VACUOUS -- the trap that makes an identical watermark meaningless: the regime ARMS 31 GRAPHS IN 31 PROGRAMS (SCRIP_STF_DEBUG=1, live=1 census), so the neutrality is 31 armed customers agreeing, not a classifier declining everything.  s21x-r's 41-program regression is NOT being re-litigated here: it was ZLEAK-1, a process-scope `static int` from getenv applied to all 316 programs while emit armed 31, cured at SCRIP `efb72d69`; the armed set and that regression set were measured DISJOINT then, and this re-measurement at a later HEAD confirms the cure held. */
