...
static long g_zd_uclaim, g_zd_udout; static int g_zd_uhead = -1; static int g_zd_uzw; static int g_zd_uzwr; static long g_zd_suspend_uclaim = 0;   /* ICN-FR-5: staging twin for op_suspend_stmt_uclaim; applied at the choke alongside the other ZD twins */   /* MECHANISM-2 staging twin: set from zd_zwr[i], applied at choke as op_zw2 */   /* ZW-12: staged beside the UCLAIM twins, applied at the same choke */   /* UCLAIM staging twins (wholesale flip): filled beside the ZD set at the drive loop, applied at the choke, cleared with the stage flag */   /* ZD-1 driver-handoff staging (R5b): the drive loop fills these from zd_plan's arrays immediately before emit_drive; the choke below applies them to g_emit exactly once and CLEARS the stage flag, so every other walk_bb_node entry path (FILL / EMIT_PAIR_FILL glue) sees zeros and stays byte-identical. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_uclaim = 0; g_emit.op_udout = 0; g_emit.op_uhead = -1; g_emit.op_zw = 0; g_emit.op_zw2 = 0; g_emit.op_suspend_stmt_uclaim = 0;   /* UCLAIM: cleared at the choke UNCONDITIONALLY; op_zw2 (mechanism-2) follows same discipline; ICN-FR-5: op_suspend_stmt_uclaim cleared per-node */
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead; g_emit.op_zw = g_zd_uzw; g_emit.op_zw2 = g_zd_uzwr; g_emit.op_suspend_stmt_uclaim = g_zd_suspend_uclaim; g_zd_suspend_uclaim = 0;   /* ICN-FR-5: apply suspend uclaim staging twin at the same choke; clear twin so it does not bleed to the next node */   /* UCLAIM applied regardless of g_zd_arm; op_zw2 mechanism-2 flag applied beside op_zw */
    ...
    case IR_MATCH_ASSIGN_COND:    { g_emit.op_fc_disp = fc_cond_fp(nd); if (getenv("SCRIP_CAP_DIAG")) fprintf(stderr, "[CAP] COND nd=%p fc_disp=%d port=%d zres=%d zread0=%d\n", (void*)nd, g_emit.op_fc_disp, (int)x86_port_mode(), g_emit.op_zres, g_emit.op_zread[0]); { IR_t *_dfr = (nd->n_operands > 0 && nd->operands[0] && nd->operands[0]->op == IR_MATCH_DEFER) ? nd->operands[0] : (IR_t*)0; if (_dfr && _dfr->n_operands > 0 && _dfr->operands[0]) { const char *_pn = IR_LIT(_dfr->operands[0]).sval; if (_pn) { extern void *rt_proc_get_fn(const char *name); extern long rt_fn_frame_bytes_known(void *fn); void *_fn = rt_proc_get_fn(_pn); long _fb = rt_fn_frame_bytes_known(_fn); if (_fb > 0) { int _adj = ((48 + (int)_fb + 15) & ~15) + 16; /* flat_frame_bytes=(48+jcon_value_region+15)&~15 + 16 from proc_γ push-pair */ if (g_emit.op_zres) { g_emit.op_zread[1] += _adj; /* ZD arm: COND stages SAVE at zj=1, template reads ZOPD(1,0)=op_zread[1] */ } else if (g_emit.op_fc_disp < 0) { g_emit.op_zdepth += _adj; } if (getenv("SCRIP_CAP_DIAG")) fprintf(stderr, "[CAP] COND defer-blob-fix pat=%s fb=%ld adj=%d zres=%d zread1-after=%d zdepth-after=%d\n", _pn, _fb, _adj, g_emit.op_zres, g_emit.op_zread[1], g_emit.op_zdepth); } } } } bb_emit_x86(bb_match_capture()); } return 0;               /* SN4-PAT-2 + ZB-FC-3c cross-box read grant; D-FAMILY (s18 C-6): COND after DEFER -- op_zdepth/op_zread[0] does not include blob runtime overhead.  ZD arm (op_zres=1): op_zread[0]+=fb+16 so ZOPD(1,0) reads SAVE's cell.  Non-ZD non-cfc arm: op_zdepth+=fb+16 so FR(op_off) resolves to SAVE's slot.  Gate: DEFER predecessor with named PAT$ and known frame size. */
    case IR_MATCH_ASSIGN_IMM:     { g_emit.op_fc_disp = fc_cond_fp(nd); { IR_t *_dfr = (nd->n_operands > 0 && nd->operands[0] && nd->operands[0]->op == IR_MATCH_DEFER) ? nd->operands[0] : (IR_t*)0; if (_dfr && _dfr->n_operands > 0 && _dfr->operands[0]) { const char *_pn = IR_LIT(_dfr->operands[0]).sval; if (_pn) { extern void *rt_proc_get_fn(const char *name); extern long rt_fn_frame_bytes_known(void *fn); void *_fn = rt_proc_get_fn(_pn); long _fb = rt_fn_frame_bytes_known(_fn); if (_fb > 0) { int _adj = ((48 + (int)_fb + 15) & ~15) + 16; if (g_emit.op_zres) { g_emit.op_zread[1] += _adj; } else if (g_emit.op_fc_disp < 0) { g_emit.op_zdepth += _adj; } } } } } bb_emit_x86(bb_match_capture()); } return 0;               /* $ immediate capture + ZB-FC-3c (identical mechanism, phase 2); D-FAMILY blob-overhead fix mirrors COND */
