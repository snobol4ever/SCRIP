...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int g_zd_stage, g_zd_arm, g_zd_k, g_zd_gpop, g_zd_wpop, g_zd_wsteal, g_zd_read[6], g_zd_kind[6], g_zd_ztail, g_zd_zunder, g_zd_zpat;   /* ⭐ C-9 (s35): g_zd_zunder = bytes of ζ-cells the REPLACEMENT EXPRESSION stacked between MATCH_END and IR_MATCH_REPLACE -- staged at the choke for IR_MATCH_REPLACE only; 0 for all other kinds.  ONE AUTHORITY: only the drive-loop staging site writes this; the choke consumes it into op_zdepth so bb_match_replace's FRQ/FR reads clear the still-live replacement cells.  s22k law: a kind whose under-displacement differs from 0 changes THAT LINE ONLY. */   /* ⭐ ZK-2 (this rung): g_zd_ztail = bytes of cells stacked above IR_TO's K=32 block by downstream run members -- staged at the choke for IR_TO only; 0 for all other kinds (op_ztail cleared at the per-node unconditional clear).  ONE AUTHORITY: only the choke (line ~838) writes this; bb_to.cpp reads op_ztail via ZLOC_B.  s22k law: a kind whose tail displacement differs from 0 changes THIS LINE ONLY. */
...
static int cap_anchor_of(IR_t * nd) { static int _on = -1; if (_on < 0) { const char * _e = getenv("SCRIP_OS_CAP"); _on = (_e && *_e == '0') ? 0 : 1; } if (!_on || !g_emit_cfg || !nd) return 0; { int _ldm = g_emit.flat_deep_arrival && !g_emit.flat_jmp_entry && !g_emit.flat_lcl_proc && !g_emit.zframe_graph && !g_emit.flat_pat && !g_emit.flat_gen; if (!_ldm && !g_emit.flat_jmp_entry) return 0; }   /* ⭐ CLASS GATE (measured, L07): the anchor arm is sound only where the δ home is REAL -- legacy deep MAIN (region slots [rbp-64-16k] backed by the GLUE-O carve) or jmp-entry frames (FR raw against a carve-then-pin activation).  On UNPINNED graphs FR's rsp arm compensates with the CONSUMER's own zdepth only, so a cross-box read spanning intervening carves lands shallow -- the depth-diff disease reborn (L07: COND read LEN's cell, begin=0, OUTPUT='abcd' for 'cd').  Those graphs cannot contain DEFER (defer forces deep) and keep ZB-FC-3c/ZD legacy verbatim -- which was already correct for them. */ IR_t * base = ((nd->op == IR_MATCH_ASSIGN_IMM || nd->op == IR_MATCH_ASSIGN_COND) && nd->n_operands > 1 && nd->operands[1]) ? nd->operands[1] : nd; int _ni = -1; for (int _j = 0; _j < g_emit_cfg->n; _j++) if (g_emit_cfg->all[_j] == base) { _ni = _j; break; } if (_ni < 0) return 0; for (int _j = 0; _j < g_emit_cfg->n; _j++) { IR_t * _w = g_emit_cfg->all[_j]; if (_w && (_w->op == IR_MATCH_FENCE1 || _w->op == IR_MATCH_ABORT)) return 0; } if (!cap_ok_at(base, _ni)) return 0; int _k = 1; for (int _j = 0; _j < _ni; _j++) { IR_t * _p = g_emit_cfg->all[_j]; if (_p && _p->op == IR_MATCH_ASSIGN_SAVE) { int _pi = _j; if (cap_ok_at(_p, _pi)) _k++; } } return _k; }   /* ⭐ OS-2·SLICE-1 (GOAL-SN4-ZETA-MECH ONE-SYSTEM): THE ONE ROUTING AUTHORITY, now returning the 1-BASED REGION SLOT k (0 = declined/killswitched) -- env read HERE only (SCRIP_OS_CAP=0 = byte-identical legacy, killswitch law); the template keys purely on the staged op_cap_anchor so the ends cannot half-flip (ARBNO-K16 precedent verbatim).  k = 1 + count of prior anchored SAVEs in cfg order; IMM/COND resolve through operands[1] (the SAVE twin) so the pair shares one slot BY CONSTRUCTION -- producer symmetric, same bracket, same k.  Slot HOME by graph class (template caphome()): legacy deep MAIN = [rbp - 64 - 16k] inside the GLUE-O enter carve (64 clears the stfh HKQ zone at rbp-48..-8); carve-then-pin classes (blob/lcl_proc/zframe/gen) = FR(op_off) raw against their REAL frames.  GRAPH DECLINES: any IR_MATCH_FENCE1/IR_MATCH_ABORT present -- both whack rsp to the rbp floor mid-graph (law-4 users), which would free the region while slots are live; fence+capture composition is a later slice.  The m4-SEGV that forced this shape: raw graph-flat FR offs (272/320/400) against the single-bracket main rbp overshot the 8-byte token carve into CRT territory -- slot 400 landed INSIDE environ[8], a dword cursor store smashed the pointer array, and getenv walked it to a SEGV (measured in gdb; the s22r/s23a envp-corruption class verbatim, extending the PIN-REBASE evidence: the LP-1 flat_all_zd=8 "region unused" premise is FALSIFIED by pinned-FR region readers -- the match-head quartet at [rbp+160..232] rides the same slack and is a pre-existing latent of the same class). */
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_zres = 0; g_emit.op_zgpop = 0; g_emit.op_wsteal = 0; g_emit.op_ztail = 0; g_emit.op_zpat = 0; for (int _zi = 0; _zi < 6; _zi++) { g_emit.op_zread[_zi] = 0; g_emit.op_zkind[_zi] = -1; }   /* ZD-1: cleared at the choke UNCONDITIONALLY so glue and non-drive fills can never inherit a stale regime.  op_ztail (ZK-2): 0 for all non-IR_TO nodes; staged for IR_TO in codegen_flat_chain_body before the staging globals are consumed here. */
    ...
    g_emit.op_fb_rbp = g_emit.flat_fb_refine ? emit_fb_bit_of(nd) : 1; g_emit.op_cap_anchor = 0;   /* FB-STMT per-node bit (Lon 2026-07-29) — twin lives in DRIVE_FILL; OS-2 anchor bit cleared per node, staged only at the capture-family dispatch */
    ...
    case IR_MATCH_BEGIN:           { extern int fc_tail_head(const IR_t *); int _th = (x86_zc_frame() == ZC_FRAME_RSP && fc_tail_head(nd)); if (fc_head_fp(nd) >= 0 || _th) { g_emit.op_fc_wbytes = 80; g_emit.op_fc_base = g_emit.op_off; } if (_th) g_emit.op_tail = 1;    /* ⭐ ZW-RB-1: set op_mech2_kc from op_udout (=Kc); zero zpop -- mech2_whack handles cleanup; fc-hook wbytes skipped under op_zw2 -- mech-2 push rbp subsumes the CAS sentinel, no separate sub rsp,24 */  /* ⭐ M-2 HFC-WINDOW: op_fc_wbytes 24→80 -- covers all 80B of MATCH_BEGIN's ZLS fields (5 quads: cursor+zeta_mark+lls2_mark at +0..+23 were FC-live; +24..+79 = head.end+dcap_mark+incoming_rbp+sigma_save+delta_save+Delta_save+capgen_save were FLAT-BYDESIGN under UCLAIM; M-3 deleted the UCLAIM head claim so FLAT-BYDESIGN now resolves to [rbp+off] in the ambient CRT frame (SIGSEGV). Widening the window to 80 FC-rebases ALL fields to [rsp + off - op_fc_base] inside the enlarged sub rsp,80 region. Window extension is safe: the FC rebase formula [rsp+off-base] gives the same bytes for +0..+23 as before (rebase=0 for those, unchanged); for +24..+79 the rebase steers into the newly-allocated 80B stack region. ONE SIDE EFFECT: x86_zclaim in the template must grow to 80B to match (see bb_match_begin.cpp hfc arm). */ bb_emit_x86(bb_match_begin()); } return 0;                  /* SN4-PAT-2 + ZB-FC-3d self-cell WINDOW + R12-EXIT-1 tail statements self-push too (bracket source) */
...
#undef RPO_VISITED
    ...
    if (blob_act) {
        ...
        /* ⭐ DEL-T1 D-2 PASS-THRU α (s9): the blob ADOPTS the caller's frame — rbp arrives pinned to the invoking
         * statement's MATCH_BEGIN frame (the licensed keeper) and is NEVER WRITTEN here or anywhere in the blob.
         * The activation's own memory is a pin-free CELL CARVE on the FORTH spine: sub rsp,kt (planner layout kept;
         * [kt-8] is now dead pad where the caller-rbp save lived) + the four live header fills ([kt-24]=γ wire rcx ·
         * [kt-16]=ω wire rdx · [kt-32]=scan flag r8 · [kt-40]=attempt δ0 r14d) + publication of the same quad plus
         * the cell base into g_zctx — the depth-immune anchor that replaces the pin (manual p.204: on total
         * failure the pushdown DRAINS to entry depth, so ω can re-base off ctx; γ SUSPENDS with stack content at
         * dynamic depth, which is exactly why a register-free base is needed).  Interior readers speak rsp+op_flat_disp
         * via the ONE selector now that flat_pat left emit_jmp_pin_rbp() (emit.h:630, same commit). */
        if (g_emit.flat_jmp_entry && !wreg_on()) {   /* ⭐⭐⭐ LADDER WREG-2 (Lon s15: "remove the stupid PROC shim around patterns"): THE SHIM IS THIS BLOCK, and under SCRIP_WREG=1 it emits NOTHING.  Deleted with it: the sub rsp,kt carve, the γ/ω wire stores [kt-24]/[kt-16] (the wires stay LIVE in r10/r11 -- there is nothing to store and nothing to reload), the SCANBASE fills [kt-32]/[kt-40], the own-base save [kt-8], and the entire 6-instruction g_zctx push.  ⭐ THE CONVERGENCE THAT MAKES THIS SAFE: that g_zctx push is itself the sharpest r10/r11 SCRATCH user in the product (mov r10,[rax] depth · mov r11,[rax+8] cur · lea r11,[rsp+kt-40]) -- the "WREG landmine" MECH s37 flagged as live-on-arrival.  The rung that CLAIMS the registers DELETES the code that was clobbering them, so this pair of edits is one mechanism, not two racing ones.  op_zgpop at the statement remains the SOLE release authority (no carve here means no carve to dispose of). */
    ...
    } else if (g_suspend_resume_slot >= 0 && g_resumable_callable_active && !wreg_on()) {   /* ⭐⭐⭐ WREG-3 SLICE 1 (s19): SUPPRESS THE RESUME-SLOT STORE UNDER THE SWITCH.  Under WREG the slot has NO FUNCTION: γ is a bare `jmp r10` and never pushes a res landing, so proc_PAT$N_res is unreferenced (grep-verified on both artifacts) and the slot is only ever read back by β, where it holds ω’s address.  Meanwhile the STORE is actively harmful: `mov [rbp+144],rax` with the carve deleted puts rbp at the ADOPTED INVOKER’s frame, so +144 — which used to sit inside the blob’s own 208B carve — now lands in the CALLER’S frame at a slot the blob does not own.  Caller-frame corruption on every blob entry.  Killing the store costs nothing because β’s guard below flips in the same slice and its else-arm already targets ω / the IR_SUSPEND β directly. */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          { static int _uw = -1; if (_uw < 0) { const char *_e = getenv("SCRIP_UNWIND"); _uw = (_e && *_e == '0') ? 0 : 1; }
            int _h3 = i;
            if (_uw && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_BEGIN && nodes[i]->op != IR_STATEMENT_END && zd_on[i] && zd_wp[i] > 0 && g_emit.op_wpop == 0 && !omega_is_retry && !omega_is_phi && !scan_live && !g_emit.flat_stmt_frame) {
          ...
          { static int _ej = -1; if (_ej < 0) { const char *_e = getenv("SCRIP_ZD_ENDJMP"); _ej = (_e && *_e == '0') ? 0 : 1; }
            int _h2 = i;
            if (_ej && !_uw_stolen && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_BEGIN && nodes[i]->op != IR_STATEMENT_END && zd_on[i] && zd_wp[i] > 0 && g_emit.op_wpop == 0 && !omega_is_retry && !omega_is_phi && !scan_live && !g_emit.flat_stmt_frame) {
    ...
    if (scan_live) {   /* SPD-2 RETRY-INTERNAL blocks.  scanhit: publish the winning start for the statement-side head-slot write-back, fall to γ.  scanfail: flag off → γ.. no, → ω; else start+1, bound (r15d=Δ), &ANCHOR test — any exit → ω; otherwise write back start, r14d=start, rsp=rbp (post-carve frontier: every element grant sits below), jmp attempt. */
        ...
        x86_begin();
        if (!wreg_on())   /* ⭐⭐⭐ LADDER WREG-4 SLICE 1 (s19, gdb-convicted): under SCRIP_WREG=1 the α shim at :2373 is suppressed, and that shim was the SOLE WRITER of g_zctx.cur (lea r11,[rsp+kt-40]; mov [rax+8],r11).  Both scan blocks OPEN by dereferencing that cell — `mov rdx,[g_zctx]; mov rdx,[rdx+8]; cmp [rdx+8],1` — so with the publisher gone every reader takes a NULL base.  CONVICTED AT THE INSTRUCTION, not inferred: gdb on 075_pat_arbno_star_backtrack under WREG=1 faults at `cmpq $0x1,0x8(%rdx)` with rdx=0, and the surrounding six instructions disassemble byte-for-byte to this scanhit chain.  BY SET on crosscheck/patterns (122 programs, same container, same binary): OFF 100 PASS / ON 64 PASS, 36 BROKEN 0 REPAIRED, and blob presence is NECESSARY in all 36 (though NOT sufficient — 7 ON-PASSING programs also carry a blob and simply never enter it, the PT-2 dead-blob class).  ⛔ THIS IS THE LADDER'S OWN STATED INTENT, NOT AN IMPROVISATION: \"The entire in-blob scanhit/scanfail protocol dies with the carve, in the same slice.\"  The blob under WREG knows NOTHING about retry — it has no ctx, no carve, no δ0 mirror and no base to whack back to.  ⛔ WHAT THIS SLICE DOES **NOT** PAY: the unanchored-scan RETRY LOOP itself, which WREG-4's remaining half re-homes to the SITE as a statement-side ω retry-advance stub (attempt cursor in the statement frame's own licensed slots), plus the scanhit head-slot publication to g_scan_hit_start.  Anchored patterns are COMPLETE under this arm (manual p.204 step 6: with &ANCHOR nonzero an empty stack IS total failure — there is no advance-and-retry to lose); unanchored ones still owe the stub.  ⛔⛔ MEASURED RESULT, AND IT FALSIFIES THE OBVIOUS EXPECTATION — DO NOT READ THIS SLICE AS THE CRASH CURE: ON goes 64→68 PASS (BROKEN-vs-OFF 36→32), and SIGSEGVs PERSIST across the residual set.  Killing the scan blocks' null-ctx read was NECESSARY BUT NOT SUFFICIENT because TWO further blob-side readers survive it, both visible in the WREG=1 artifact: (1) ⛔⭐ THE RESUME SLOT SCRIBBLES THE INVOKER'S FRAME — α_body emits `lea rax,[rip+ω]; mov [rbp+144],rax` and β emits `jmp qword ptr [rbp+144]`.  Under the old regime rbp was the blob's OWN base and +144 sat INSIDE its 208B carve; WREG deletes the carve, so rbp is the ADOPTED invoker's and the store lands in the CALLER'S frame at a slot the blob does not own — caller-frame corruption on every blob entry, and the leading SEGV candidate for the residual set.  β reads that slot back and finds ω, so a backtrack into a suspended blob transfers to FAIL instead of resuming, losing the pending alternatives (manual p.204 step 6 is POP-and-retry, never abandon).  (2) proc_PAT$N_res re-pushes g_zctx using r10/r11 as scratch — ⛔ BUT IT IS UNREACHABLE UNDER THIS ARM AND CLOBBERS NOTHING TODAY: γ is a bare `jmp r10` and never emits `lea rcx,[rip+res]`, so no reference to the label exists (verified by grep over BOTH artifacts: the `lea` of _res appears ONLY in the WREG=0 text).  It is a LATENT hazard that arms the moment WREG-3 makes γ push a record and res reachable again, so whoever writes that record MUST NOT let res take r10/r11 as scratch.  ⛔ THE s19 CURSOR AS FIRST WRITTEN CALLED THIS A LIVE WIRE-DESTROYER; THAT WAS INFERRED FROM READING EMITTED TEXT WITHOUT TRACING REACHABILITY, AND IS CORRECTED HERE AND IN THE GOAL FILE — the same read-and-guess class this file convicts elsewhere, caught on myself.  This is the predicted "WREG landmine": located, but dormant rather than live.  Both are WREG-3 (suspension capture), which is therefore a HARD PREREQUISITE of any default flip — the s15 ordering (WREG-1→4→3→2) stands, re-proved independently here.  Registration (6 rt_proc_set_* calls + .globl proc_PAT$N_α) is likewise untouched by this slice. */
        ...
        x86_begin();
        if (!wreg_on())   /* ⭐ WREG-4 SLICE 1 (companion of the scanhit guard above — ONE mechanism, both arms flip together): same null-ctx conviction.  Under the switch scanfail degenerates to the bare ω transfer that follows, which is the correct terminal shape for an anchored blob and the honest placeholder for an unanchored one until the site-side stub lands. */
    ...
    if (g_emit.flat_jmp_entry) {
        ...
        if (g_emit.flat_pat) {   /* ⛔ DEL-T1 D-2 β res-stub PASS-THRU arm (s9): record[+8] is now the CELL BASE, not rbp.  Drop the landing word, pop the base, REFILL g_zctx from the suspended activation's carve header [base+kt-24/16/32/40] plus the base itself — the interleaving repair: P1 suspend-γ → P2 defer runs (ctx now P2's) → statement fails back into P1's β → this refill re-arms P1's wires/scan cells before the resume dispatch.  Zero rbp touches (rbp = statement's, adopted, never written). */
            int _rkt = g_emit.flat_frame_bytes;
            extern uint64_t g_zctx[66];
            if (g_is_text) {
                char _res[320];
                snprintf(_res, sizeof _res, "%s",
                ...
                ef_b4(0x48, 0x83, 0xC4, 0x08);                                                         /* add rsp,8       */
                ef_b1(0x58);                                                                           /* pop rax (base)  */
                { uint64_t _cx = (uint64_t)(uintptr_t)g_zctx; ef_b2(0x48, 0xBA); bb_emit_u64(_cx); }
                ef_b3(0x4C, 0x8B, 0x12);                          /* mov r10,[rdx] depth  */
                ef_b4(0x4C, 0x8B, 0x5A, 0x08);                    /* mov r11,[rdx+8] cur  */
                ef_b4(0x4E, 0x89, 0x5C, 0xD2); ef_b1(0x10);       /* [rdx+r10*8+16]=r11   */
                ef_b3(0x48, 0xFF, 0x02);                          /* inc qword [rdx]      */
                ef_b4(0x48, 0x89, 0x42, 0x08);                    /* mov [rdx+8],rax      */
            }
        } else if (g_is_text) {
            char _res[96];
    ...
    if (g_suspend_resume_slot >= 0 && emit_heap_fb_adopt() && !wreg_on()) {   /* ⭐⭐⭐ WREG-3 SLICE 1 companion (s19) — ONE mechanism with the store guard above, both arms flip together: with no store there is nothing to dispatch through, so β takes the ELSE arm (resume_tgt = ω, or the IR_SUSPEND β when the graph has one).  That is EXACTLY the control flow the slot produced anyway — it held ω — so this is behaviour-preserving minus the corrupting write, which makes it a clean test of whether the caller-frame scribble is the residual SEGV mechanism. ⛔ It does NOT give suspended blobs a real resume path: that is W-MAP (3)’s {res,r10,r11} spine record, still owed, and until it lands a backtrack into a suspended blob still transfers to FAIL rather than resuming (manual p.204 step 6 is POP-and-retry, never abandon). */   /* ZETA-FB-3 (s161): the SEVENTH copy of the adopt condition, now named — this is the β frame-slot dispatch the emit_rec_pin comment calls out as "ALREADY gates on this pair".  ⚠ OBSERVED, NOT CHANGED: the GUARD is the NARROW adopt-only arm while the body spells emit_rec_fb()/emit_rec_fb_num(), the WIDE union — sound today because adopt ⟹ emit_rec_pin, so the register the body names is right whenever this arm is taken.  A graph that is emit_jmp_pin_rbp() but NOT adopt, carrying a resume slot, therefore takes the ELSE arm by construction; whether it SHOULD is a separate measurable question, and blind widening of exactly this kind is what s158 measured as a regression.  Named here, deliberately NOT widened. */
        if (g_is_text) {
            char _ind_jmp[64];
    ...
    if (g_emit.zframe_graph || (g_emit_cfg && g_emit_cfg->icn_cells_graph && g_emit.flat_lcl_proc && g_emit.flat_jmp_entry)) { extern void xa_flat_zframe_epilogue_γ(void); xa_flat_zframe_epilogue_γ(); }   /* ZK-4 SLICE 1 (s215): CLASS ZF widened to the CELLS arm. Predicate mirrors xa_flat_class_zf() in xa_flat.cpp (the template-side ONE AUTHORITY); the header base differs per regime and is computed there by xa_flat_wire_hdr_base().  ZK-2 (s226): flat_jmp_entry conjunct added to cells-arm condition. The outer main graph has flat_jmp_entry=0 (entered from C via bare jmp, no wire installed in rcx/rdx); its γ exit is call exit@PLT, not wire-jmp. Procedures (noop, fib, etc.) have flat_jmp_entry=1 and exit via CLASS ZF wire-jmp. Without this guard, cells-arm main's γ would jmp through garbage rcx = SEGV. */   /* ICN-FR-2 CLASS ZF γ: direct wire-header read, depth-immune unwind (lea rsp,[rbp+kt]; load rcx=[rbp+kt-24]; restore rbp=[rbp+kt-8]; jmp rcx).  BOTH MEDIUM via x86() in xa_flat.cpp.  Precedence over _blob_wire/_wire_stub: those are 0 for ζ-frame graphs by construction (floor=0, flat_lcl_proc=0, flat_pat=0). */
    else if (_blob_wire && wreg_on()) bb_emit_x86(x86_jmp_reg("r10"));   /* ⭐⭐⭐ LADDER WREG-2 CLASS D γ: SUCCESS = `jmp r10`, one instruction, and that is the WHOLE exit.  What dies here: the g_zctx load, the cur-base read, the γ wire read out of the frame header (the wire never left its register), the zctx depth POP, and the {res,base} resume-record push.  NO UNWIND -- there is no carve to dispose of, because WREG-2's α carved nothing.  ⛔ MEASUREMENT OWED, NOT ASSUMED: the resume-record half served β RE-ENTRY for suspended blobs (deep-arrival backtrack).  The ladder names dc_nest_bt/dc_sib_bt as the deciding witnesses -- if they ride the wires, the class is WREG; if they do not, deep-arrival is FRAMED-licensed and this arm needs a narrower predicate.  Decide by measurement and RECORD WHICH, per the ladder's own instruction. */
        ...
        if (g_is_text) { char _bs[384]; snprintf(_bs, sizeof _bs, "mov rdx, qword ptr [rip + g_zctx@GOTPCREL]\nmov rax, qword ptr [rdx + 8]\npush rax\nlea rcx, [rip + %s]\npush rcx\nmov rcx, qword ptr [rax + %d]\nmov r10, qword ptr [rdx]\ndec r10\nmov qword ptr [rdx], r10\nmov r11, qword ptr [rdx + r10*8 + 16]\nmov qword ptr [rdx + 8], r11\njmp rcx\n", lbl_res.name, 16); emit_text_n(_bs, strlen(_bs)); }
               ...
               ef_b2(0xFF, 0xE1); } }
    else if (g_emit_cfg && g_emit_cfg->icn_cells_graph && g_emit.flat_lcl_proc && !g_emit.flat_jmp_entry) { int _bk = g_emit.flat_frame_bytes;   /* ZK-2 (s226): CELLS-ARM OUTER MAIN γ EPILOGUE. flat_lcl_proc prologue: sub rsp,kt; ...; mov rbp,rsp — rbp = entry_rsp - kt. GLUE-O leave (mov rsp,rbp; pop rbp) would restore rsp to entry_rsp-kt and pop from [rbp+0]=slot0=0, leaving rsp = entry_rsp-kt+8. Correct: lea rsp,[rbp+kt] = entry_rsp; pop rbp from [rbp+kt-8] = saved caller rbp; then exit(0). */
        if (g_is_text) { char _seg[128]; snprintf(_seg, sizeof _seg, "lea rsp, [rbp + %d]\npop rbp\nxor edi, edi\ncall exit@PLT\n", _bk); emit_text_n(_seg, strlen(_seg)); }
        else { ef_b3(0x48, 0x8D, 0xA5); bb_emit_u32((uint32_t)_bk); ef_b1(0x5D); ef_b3(0x31, 0xFF, 0x90); { uint64_t _ex = (uint64_t)(uintptr_t)(void *)exit; ef_b2(0x48, 0xB8); bb_emit_u64(_ex); ef_b2(0xFF, 0xD0); } } }
...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    { static int _stf = -1; if (_stf < 0) { const char * e = getenv("SCRIP_STMT_FRAME"); _stf = (e && *e == '1') ? 1 : 0; }   /* ⛔⭐⭐ STF-UNFLIP (s22b, LON DIRECTIVE, and it is a MODEL correction not a tuning knob): DEFAULT FLIPPED BACK ON -> OFF; `SCRIP_STMT_FRAME=1` is now the OPT-IN and the regime ships DORMANT.  THE MODEL, restated in Lon's words because two sessions in a row encoded the opposite: NO BB EVER CONSUMES ANYTHING, EVER.  A BB allocates EXACTLY at alpha -- one `sub rsp, K`, its OWN cell -- never at beta (ARBNO-class excepted); it frees at omega; and the whole accumulation is freed AT THE END OF THE STATEMENT.  Cells stack up through the statement and ONE statement-terminal release drops all of them.  That release is op_zgpop, and it is the SOLE release authority.  ⛔ THE ERROR s21x-y AND s22a ENCODED: the per-statement rbp dance (bb_glue_framed_enter/leave) was made to stand in for that release, and s22a's ZGPOP-STF then ZEROED op_zgpop on armed graphs to stop the two colliding -- i.e. the bracket did not SUPPLEMENT the statement-terminal free, it REPLACED it, and a C-style frame pointer became per-statement ceremony on the value spine.  Law 4's "C-style RBP used occasionally" licenses rbp for CONSTRUCT housekeeping that must survive an unwind (ARBNO/FENCE1/CALL) -- it never licensed one bracket per statement.  MEASURED AT THIS HEAD, not inherited: crosscheck 318 m3 232/85 - m4 229/86/2 - DIV=1 {W04_arbno_basic}, IDENTICAL to the regime-on run, and the fail set is IDENTICAL BY SET in BOTH modes (diffed programmatically, not by count -- the test_string flake makes counts lie).  ⚠ NOT VACUOUS, and this is the instrument that proves it: an rbp-INSTRUCTION census over all 318 crosscheck programs moves 15709 -> 15459 (250 instructions gone) and the count of programs containing ANY rbp moves 158 -> 127, i.e. EXACTLY 31 PROGRAMS GO RBP-FREE -- the same 31 the SCRIP_STF_DEBUG=1 armed census names.  Witness `arithmetic.sno`: 26 rbp -> 0, statement `OUTPUT = A + B` now three alpha carves (`sub rsp,16` x3) and one terminal `add rsp,48`.  ⭐ THE REMAINING RBP DEBT IS A DIFFERENT CITIZEN AND IS THE NEXT RUNG: 15459 instructions across 127 programs is the flat_pat/flat_gen WHOLE-GRAPH PIN, not a statement bracket -- do not confuse the two (the s22a CENSUS-CORRECTED instrument law).  HISTORICAL, kept because the next session will otherwise re-derive it: s21x-y's flip was licensed by a watermark-neutrality argument that was TRUE and INSUFFICIENT -- neutrality proved the bracket broke nothing, never that it belonged. */   /* ⭐ STF-FLIP (s21x-y), SUPERSEDED BY THE ABOVE, retained for provenance: DEFAULT FLIPPED OFF -> ON, and `SCRIP_STMT_FRAME=0` is now the killswitch.  This is the moment the "C-style RBP used occasionally" of the design of record stops being dormant scaffolding and becomes the shipped regime: under it a STATEMENT is bracketed by the rbp dance (bb_glue_framed_enter/leave) and bb_match_begin's five POST-UNWIND-lifetime fields move from the whole-graph flat slots FRQ(op_off+40..72) onto HKQ = [rbp-48+8k], which is DEPTH-FREE BY CONSTRUCTION -- the value spine keeps riding RSP FORTH-style while exactly the housekeeping that must survive an unwind rides a stable base.  LICENSED BY THE CONDITION THIS LINE ITSELF NAMED ("default OFF until the watermark license ... fail-set diffed programmatically"), now satisfied and MEASURED AT THIS HEAD, not inherited: crosscheck 317 m3 232/85 - m4 229/86/2 - DIV=1 and broad 336 m3 244/92 - m4 238/90/8, and in BOTH corpora BOTH modes the fail set is BYTE-IDENTICAL to the regime-off run.  ⚠ NOT VACUOUS -- the trap that makes an identical watermark meaningless: the regime ARMS 31 GRAPHS IN 31 PROGRAMS (SCRIP_STF_DEBUG=1, live=1 census), so the neutrality is 31 armed customers agreeing, not a classifier declining everything.  s21x-r's 41-program regression is NOT being re-litigated here: it was ZLEAK-1, a process-scope `static int` from getenv applied to all 316 programs while emit armed 31, cured at SCRIP `efb72d69`; the armed set and that regression set were measured DISJOINT then, and this re-measurement at a later HEAD confirms the cure held. */
      ...
      g_emit.flat_lcl_proc = (!g_emit.flat_pat && !g_emit.flat_gen && g_emit_cfg && ((g_emit.flat_jmp_entry && (g_emit_cfg->nparams > 0 || g_emit_cfg->nlocals > 0)) || g_emit_cfg->icn_cells_graph)) ? 1 : 0;   /* ICN-PROC-FRAME (s211): jmp-entry graph with lexical locals or params requiring rbp seed + g_call_args install at α. Guards: NOT flat_pat (blob has its own BLOB-GRANT protocol); NOT flat_gen (generator keeps suspended activation, separate rung). Named for WHAT it decides. SNOBOL4: dyn_scope procs have no lexical locals (graph_has_local is always 0 there), so this fires only for Icon/Raku lex procs with params/locals.  ZK-2 FIX (s225): icn_cells_graph procs ALWAYS need flat_lcl_proc=1 — they exit CLASS ZF (wire-header read) which requires the flat_lcl_proc prologue to carve the wire-header frame (sub rsp,flat_frame_bytes + pin rbp=rsp + install args).  Without this, a 0-param/0-local cells-arm proc (e.g. inc() in rung21) got flat_lcl_proc=0, bypassed the frame carve, and fell through to CLASS P / the Prolog DC stub (rt_pl_dc_prep) path — which pushes no pcall record and crashes.  ZK-2 FIX (s226): LIFT flat_jmp_entry REQUIREMENT FOR CELLS ARM. The s225 fix worked for jmp-entry procs (noop(), fib() — all have flat_jmp_entry=1). The outer main graph has flat_jmp_entry=0 and fell through to GLUE-O (push rbp; sub rsp,8) — a 16-byte frame while FRQ slots extend to [rbp+184+], corrupting the C caller's frame and rbp. icn_cells_graph alone is sufficient to require flat_lcl_proc: the cells arm's CLASS ZF epilogue reads wire header at [rbp+kt-24/-16/-8], which requires the flat_lcl_proc prologue's sub+save+pin regardless of whether the graph was entered via jmp-entry or as the outer main. The flat_jmp_entry conjunct is retained ONLY for the SN4/Raku nparams/nlocals arm (non-cells jmp-entry procs still require it). TWO CONDITIONS: (1) jmp-entry with params/locals as before; (2) icn_cells_graph regardless of jmp-entry. SN4/Prolog/Raku/Pascal: icn_cells_graph=0 — byte-identical. CELLS=0: icn_cells_graph=0 — byte-identical. ONE AUTHORITY. */
      ...
      int _pat_ok = (!g_emit.flat_pat) ? 1 : 0;   /* ⭐ MECHANISM-2 STF EXTENSION (SCRIP_ZW_RB): when mechanism-2 is enabled, flat_pat=1 graphs are admitted.  STF fires push rbp at statement head (pre-all-cells); mechanism-2's push rbp in MATCH_BEGIN nests inside it (nested indeterminacy = nested frames, HQ ruling 2026-08-03).  Default OFF (zw_rb_on()=0) preserves !flat_pat exclusion byte-identical. */
