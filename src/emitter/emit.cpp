...
static int zd_wl_kind(IR_t * nd) {
    ...
    if (op == IR_CALL_PROC_STAGED) { static int _zk2cps = -1; if (_zk2cps < 0) { const char * e = getenv("SCRIP_ZD_ICN_CPS"); _zk2cps = (e && *e == '0') ? 0 : 1; } return (_zk2cps && g_emit_cfg && g_emit_cfg->icn_cells_graph) ? 1 : 0; }   /* ZK-2 ADDITIVE: IR_CALL_PROC_STAGED admitted on the cells arm -- K=16 (result cell); ZD template arm already exists in bb_call_proc_staged.cpp bc_det_arm op_zres path: LEA args from ZOPQ cells, x86_call_dc, L(2) writes ZRES(0/8). ROOT CAUSE THIS CLOSES: without admission op_zres=0 non-ZD arm uses FRQ(off)=[rsp+off+op_zdepth] with off=flat-frame-slot putting result write at [stmt_claim_base+48] above main entry rsp corrupting CRT frame and SEGVing in exit() atexit after correct output. zd_nops already returns n_operands for IR_CALL_PROC_STAGED ONE AUTHORITY. K=16 default. SN4 watermark: icn_cells_graph=0 for all SN4 graphs byte-identical. ZFRAME watermark: cells arm exclusive with zframe_graph. Killswitch SCRIP_ZD_ICN_CPS=0. ONE AUTHORITY per s22k law. */
