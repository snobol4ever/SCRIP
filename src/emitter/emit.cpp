...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    { static int _ba = -1; static const char * _bo; static const char * _bs; if (_ba < 0) { const char * e = getenv("SCRIP_BB_ALLOC"); _ba = (e && *e == '0') ? 0 : 1; _bo = getenv("SCRIP_BB_ONLY"); _bs = getenv("SCRIP_BB_SKIP"); }
      ...
      { extern long zw_carve_k(const IR_t *); extern int zc_nofc(void); long _k = zc_nofc() ? 0 : zw_carve_k(nd);   /* NOFC-SYM: the universal carve is the PRODUCER half of the killswitch's missing symmetry -- suppressing the fc_geom grant alone would leave every declined node still emitting its `sub rsp, K` with nothing to release it, which is the 16B-per-statement leak measured on the loop benchmarks.  ZD-armed nodes are unaffected: their K arrives from the ZD staging block below, not from here. */   /* CARVE-ERAD step 1: the predicate that used to live here is now zw_carve_k (zeta_storage.c) so the LOWER-side prefix walk spends the SAME K this hook does -- one authority, per the zw_node_k precedent. */
