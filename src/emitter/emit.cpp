...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int g_zd_stage, g_zd_arm, g_zd_k, g_zd_gpop, g_zd_wpop, g_zd_read[6];
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_zres = 0; g_emit.op_zgpop = 0; g_emit.op_wterm = 0; for (int _zi = 0; _zi < 6; _zi++) g_emit.op_zread[_zi] = 0;   /* ZD-1: cleared at the choke UNCONDITIONALLY so glue and non-drive fills can never inherit a stale regime */
    ...
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead;   /* UCLAIM applied regardless of g_zd_arm -- declined runs are its whole constituency */
        ...
        if (g_zd_arm) { g_emit.op_zres = 1; g_emit.op_fc_bytes = g_zd_k; g_emit.op_fc_base = -1; g_emit.op_zdepth = g_zd_k; for (int _zi = 0; _zi < 6; _zi++) g_emit.op_zread[_zi] = g_zd_read[_zi]; }
