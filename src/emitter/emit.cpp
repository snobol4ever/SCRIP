...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_nops(IR_t * nd) { int op = (int)nd->op; return (op == IR_UNOP || op == IR_ASSIGN) ? 1 : (op == IR_BINOP || op == IR_COERCE_NUMERIC || op == IR_CMP_TEST) ? 2 : 0; }   /* ZD-2d/2e: COERCE_NUMERIC reads operand 0 as the value and operand 1 as the partner type-peek; CMP_TEST takes both coerced operands by address.  Both stage two op_zread differences. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
