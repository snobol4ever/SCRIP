...
static int arbno_frameless_fpr_rsp(IR_t * nd) {   /* ⭐ N02-FIX FRAMELESS: right-spine RSP depth for ARBNO-LON K0/K16 arms. Walk nd->\xce\xb3.node (the ARBNO's own subsequent) summing zd_k() until IR_MATCH_END/IR_MATCH_BEGIN/IR_MATCH_ARBNO(=entered enclosing ARBNO's territory)/NULL. Same law as fct_rsp_range in zeta_storage.c. Zero when right-spine is empty (byte-identical). */
    int fpr = 0;
    IR_t *rs = nd->γ.node;
    for (int g = 0; rs && g < 32 && rs->op != IR_MATCH_END && rs->op != IR_MATCH_BEGIN && rs->op != IR_MATCH_ARBNO; rs = rs->γ.node, g++) { int mk = zd_k(rs); if (mk) fpr += mk; }
    return fpr;
}
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    case IR_MATCH_ARBNO:          { extern int fc_tail_arbno(const IR_t *, int *, int *, int *, int *); extern int fc_tail_ncap(const IR_t *); int _fpb = 0, _fpl = 0, _osb = 0, _hdr = 0;
                                    ...
                                    if (!_k16r && _tailc) { g_emit.op_tail = 1; g_emit.op_sb = _osb; g_emit.op_sa = _hdr; g_emit.op_tail_fpb = _fpb; g_emit.op_tail_fpl = _fpl; g_emit.op_tail_ncap = fc_tail_ncap(nd); g_emit.op_tail_seal = (nd->n_operands > 3 && nd->operands[3] == nd) ? 1 : 0; g_emit.op_tail_dfr = fc_tail_dfr(nd); { extern int fc_tail_fpr_rsp(const IR_t *); g_emit.op_tail_fpr_rsp = fc_tail_fpr_rsp(nd); } }
                                           ...
                                           if (_chain && g_emit_cfg) { int _ni=-1; for(int _j=0;_j<g_emit_cfg->n;_j++) if(g_emit_cfg->all[_j]==nd){_ni=_j;break;} int _fr=0; if(_ni>=0) for(int _j=0;_j<g_emit_cfg->n&&!_fr;_j++){IR_t*_o=g_emit_cfg->all[_j]; if(_o&&_o!=nd&&_o->op==IR_MATCH_ARBNO&&_o->n_operands>=3){int _a=-1,_b=-1; for(int _k=0;_k<g_emit_cfg->n;_k++){if(g_emit_cfg->all[_k]==_o->operands[1])_a=_k;if(g_emit_cfg->all[_k]==_o->operands[2])_b=_k;} if(_a>_b){int _t=_a;_a=_b;_b=_t;} if(_a>=0&&_ni>=_a&&_ni<=_b)_fr=1;}} g_emit.op_arbno_framed=_fr; int _k0=1; { int _s0=-1,_s1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_s0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_s1=_j;} if(_s0>_s1){int _t=_s0;_s0=_s1;_s1=_t;} if(_s0<0)_k0=0; else for(int _j=_s0;_j<=_s1&&_k0;_j++){IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m!=nd&&zd_k(_m)!=0)_k0=0;} } g_emit.op_arbno_body_k0=_k0; if (_k0) g_emit.op_tail_fpr_rsp = arbno_frameless_fpr_rsp(nd); }   /* ⭐ N02-FIX FRAMELESS call (N21) */  /* ⭐ ARBNO-FRAME containment dual of the op_body_has_arbno scan above: THIS node inside any OTHER ARBNO's [operands[1]..operands[2]] span = the nested/unbounded tier -> per-activation header frame (see emit.h field note; Lon directive 2026-08-06) */ } else { g_emit.op_sa = -1; g_emit.op_sb = -1; g_emit.op_arbno_chain = 0; } }
...
void emit_drive(IR_t *nd, bb_label_t *lbl_α, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_MATCH_POS: case IR_MATCH_RPOS: {
        ...
        g_emit.op_off = -1;
        if (!a0) { drive_unowned(nd); break; }
        if (a0->op == IR_LIT_INTEGER) { g_emit.op_sb = (int)IR_LIT(a0).ival; g_emit.op_sa = -1; { extern int fc_geom(const IR_t *, long *); long _fk = 0; if (x86_port_mode() == ZC_PORT_FORTH && g_zd_arm && !fc_geom(a0, &_fk)) g_emit.op_wpop += 16; } }   /* ⭐ ZD-5b POS/RPOS CONST-WPOP (s31): const arg is ZD-armed in FORTH mode (universal arm zd_wl_kind:1907; fc_vlit_active=0 so fc_geom=0 → sub rsp,16 emitted, no ZLS slot). Template uses op_sa=-1 (reads op_sb), NEVER reads ZOPQ — LIT_INT's 16B orphaned on failure. op_wpop+=16 arms x86_fc_jcc_omega (x86_asm.h:402): je Lγ / add rsp,16 / jmp ω / Lγ:. N21 witness: two-sibling ARBNO; ARBNO(b) frameless [rsp+0/4] displaced → spurious =F. Guards: ZC_PORT_FORTH (no push in non-FORTH) + g_zd_arm (run armed → LIT_INT armed) + !fc_geom (no ZLS slot → rsp push). All three required — missing g_zd_arm regresses X09/X10. ONE AUTHORITY. */
        else { int sl = bb_slot_get(a0); if (sl < 0) { drive_unowned(nd); break; } g_emit.op_sa = sl; g_emit.op_sb = 0; }
        DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    }
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0; int zdh_match = -1;   /* M-2 BUG-2 FIX: zdh_match = zd at MATCH_BEGIN entry (= depth of pre-match producer cells only); used as gpop at STATEMENT_END when hfc because the cas_rsp_mark restore collapses all in-bracket cells (pattern-arg cells like LIT_INTEGER), leaving only the pre-match producers for STATEMENT_END to release.  -1 = no hfc MATCH_BEGIN seen yet. */
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (!gin) zgpop[i] = ((zdh_match >= 0 && (nodes[i]->op == IR_STATEMENT_END || nodes[i]->op == IR_STATEMENT)) ? zdh_match : _wzdepth) + (int)kc;   /* M-2 BUG-2: for STATEMENT_END/STATEMENT in an hfc match run, use zdh_match (depth at MATCH_BEGIN entry = only pre-match producers) instead of _wzdepth (full accumulated depth including in-bracket cells like LIT_INTEGER). cas_rsp_mark restore collapses all in-bracket cells; STATEMENT_END must release only the pre-match producers. */
