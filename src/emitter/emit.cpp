...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int g_zd_stage, g_zd_arm, g_zd_k, g_zd_gpop, g_zd_wpop, g_zd_wsteal, g_zd_read[6], g_zd_kind[6];
...
static long g_zd_uclaim, g_zd_udout; static int g_zd_uhead = -1; static int g_zd_uzw; static int g_zd_uzwr;   /* MECHANISM-2 staging twin: set from zd_zwr[i], applied at choke as op_zw2 */   /* ZW-12: staged beside the UCLAIM twins, applied at the same choke */   /* UCLAIM staging twins (wholesale flip): filled beside the ZD set at the drive loop, applied at the choke, cleared with the stage flag */   /* ZD-1 driver-handoff staging (R5b): the drive loop fills these from zd_plan's arrays immediately before emit_drive; the choke below applies them to g_emit exactly once and CLEARS the stage flag, so every other walk_bb_node entry path (FILL / EMIT_PAIR_FILL glue) sees zeros and stays byte-identical. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_sr_role(const IR_t * nd) { int64_t v = IR_LIT(nd).ival; return (v == 1 || v == 2 || v == 3) ? (int)v : 0; }   /* ⭐ ONE AUTHORITY for the IR_SAVE_RESTORE role, per the s22k ZD-K law ("grep the RULE'S SHAPE, not the variable's name" -- that law was written after a THIRD spelling of K survived a two-site collapse, so this rule gets one site the first time it is needed twice).  IR.h's sval/ival/dval are ONE union: roles 1-3 (RETURN floater / FRETURN floater / WIRE-ADOPT) write ival, role 0 (the CALL2BB call-site OPEN box) writes sval, an lp_strdup heap pointer that is never 1..3 -- so the value read back IS the role.  Consumers: the dispatch's op_ival normalization below, and zd_wl_kind's ZD-SR admission. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    case IR_MATCH_ARBNO:          { extern int fc_tail_arbno(const IR_t *, int *, int *, int *, int *); extern int fc_tail_ncap(const IR_t *); int _fpb = 0, _fpl = 0, _osb = 0, _hdr = 0;
                                    ...
                                    if (!_k16r && _tailc) { g_emit.op_tail = 1; g_emit.op_sb = _osb; g_emit.op_sa = _hdr; g_emit.op_tail_fpb = _fpb; g_emit.op_tail_fpl = _fpl; g_emit.op_tail_ncap = fc_tail_ncap(nd); g_emit.op_tail_seal = (nd->n_operands > 3 && nd->operands[3] == nd) ? 1 : 0; g_emit.op_tail_dfr = fc_tail_dfr(nd); }
                                           ...
                                           if (_chain && g_emit_cfg) { int _ni=-1; for(int _j=0;_j<g_emit_cfg->n;_j++) if(g_emit_cfg->all[_j]==nd){_ni=_j;break;} int _fr=0; if(_ni>=0) for(int _j=0;_j<g_emit_cfg->n&&!_fr;_j++){IR_t*_o=g_emit_cfg->all[_j]; if(_o&&_o!=nd&&_o->op==IR_MATCH_ARBNO&&_o->n_operands>=3){int _a=-1,_b=-1; for(int _k=0;_k<g_emit_cfg->n;_k++){if(g_emit_cfg->all[_k]==_o->operands[1])_a=_k;if(g_emit_cfg->all[_k]==_o->operands[2])_b=_k;} if(_a>_b){int _t=_a;_a=_b;_b=_t;} if(_a>=0&&_ni>=_a&&_ni<=_b)_fr=1;}} g_emit.op_arbno_framed=_fr; int _k0=1; { int _s0=-1,_s1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_s0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_s1=_j;} if(_s0>_s1){int _t=_s0;_s0=_s1;_s1=_t;} if(_s0<0)_k0=0; else for(int _j=_s0;_j<=_s1&&_k0;_j++){IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m!=nd&&zd_k(_m)!=0)_k0=0;} } g_emit.op_arbno_body_k0=_k0; }   /* ⭐ ARBNO-FRAME containment dual of the op_body_has_arbno scan above: THIS node inside any OTHER ARBNO's [operands[1]..operands[2]] span = the nested/unbounded tier -> per-activation header frame (see emit.h field note; Lon directive 2026-08-06) */ } else { g_emit.op_sa = -1; g_emit.op_sb = -1; g_emit.op_arbno_chain = 0; } }
    ...
    case IR_MATCH_BEGIN:           { extern int fc_tail_head(const IR_t *); int _th = (x86_zc_frame() == ZC_FRAME_RSP && fc_tail_head(nd)); if (!g_emit.op_zw2 && (fc_head_fp(nd) >= 0 || _th)) { g_emit.op_fc_wbytes = 24; g_emit.op_fc_base = g_emit.op_off; } if (_th) g_emit.op_tail = 1; if (g_emit.op_zw2) { g_emit.op_mech2_kc = g_emit.op_udout; g_emit.op_zgpop = 0; g_emit.op_wpop = 0; }   /* ⭐ ZW-RB-1: set op_mech2_kc from op_udout (=Kc); zero zpop -- mech2_whack handles cleanup; fc-hook wbytes skipped under op_zw2 -- mech-2 push rbp subsumes the CAS sentinel, no separate sub rsp,24 */ bb_emit_x86(bb_match_begin()); } return 0;                  /* SN4-PAT-2 + ZB-FC-3d self-cell WINDOW + R12-EXIT-1 tail statements self-push too (bracket source) */
    case IR_MATCH_END:        { IR_t * _hd = nd->n_operands > 0 ? nd->operands[0] : (IR_t *)0; extern int fc_tail_release(const IR_t *, int *); int _br = -1;
                                    ...
                                    if (g_emit.op_zw2) { g_emit.op_zgpop = 0; g_emit.op_wpop = 0; }   /* ⭐ ZW-RB-1: zero zpop for MATCH_END mech2 -- whack restores rsp; ADD RSP,N over-releases */
    ...
    case IR_MATCH_ASSIGN_COND:    { g_emit.op_fc_disp = fc_cond_fp(nd); if (getenv("SCRIP_CAP_DIAG")) fprintf(stderr, "[CAP] COND nd=%p fc_disp=%d port=%d\n", (void*)nd, g_emit.op_fc_disp, (int)x86_port_mode()); bb_emit_x86(bb_match_capture()); } return 0;               /* SN4-PAT-2 + ZB-FC-3c cross-box read grant */
    ...
    case IR_MATCH_ASSIGN_SAVE:    { { long fck; if (fc_geom(nd, &fck)) { g_emit.op_fc_bytes = fck; g_emit.op_fc_base = g_emit.op_off; } } { extern int fc_save_active(const IR_t *); if (getenv("SCRIP_CAP_DIAG")) fprintf(stderr, "[CAP] SAVE nd=%p save_active=%d fc_bytes=%ld port=%d\n", (void*)nd, fc_save_active(nd), (long)g_emit.op_fc_bytes, (int)x86_port_mode()); } bb_emit_x86(bb_match_capture()); } return 0;               /* SN4-PAT-3h phase-0 SAVE + ZB-FC-3c fixed-cell grant (δ at cell+0) */
    ...
    case IR_GALT:                 { g_emit.op_off = 0; bb_emit_x86(bb_rk_galt()); }                 return 0;   /* RK-GRAM-3d grammar alternation: delta-save slot at [rbp+0]; lbl_t1 pre-resolved in pre-pass above */
    case IR_RETURN: {
...
void emit_drive(IR_t *nd, bb_label_t *lbl_α, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_VAR: {
        ...
        else if (vn) { int voff = bb_varslot_peek(vn); g_emit.op_sa = voff; g_emit.op_off = (voff != -1) ? drive_value_slot(nd) : -1; }   /* ICN-PROC-FRAME (s211): voff may be negative for rbp-relative lexical locals; -1 is the unresolved sentinel.  op_off is the result cell (always needed when voff resolved). PL-FR-2: changed >= 0 to != -1 so negative rbp-relative zframe slots are recognized as valid (not absent). */
...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
typedef struct { int base; int end; long dout; int head; } zvo_ent_t;   /* UCLAIM OWNER TABLE (wholesale flip, Lon 2026-08-01 "FLIP them ALL on ... This is just ONE FUNCTION basically to calculate the offsets based on the execution order"): one entry per claimed node -- its flat-plan range [base,end) and its claim-depth-out in the execution-order walk, tagged by run head so region-shared offsets across statements cannot alias.  Built by zd_plan's declined-run extension, reset per chain, consumed only by zvo_resolve below. */
static zvo_ent_t g_zvo[8192]; static int g_zvo_n = 0;
static void zvo_reset(void) { g_zvo_n = 0; }
static void zvo_add(int base, long bytes, long dout, int head) { if (bytes <= 0 || base <= -0x30000000 || g_zvo_n >= 8192) return; g_zvo[g_zvo_n].base = base; g_zvo[g_zvo_n].end = base + (int)bytes; g_zvo[g_zvo_n].dout = dout; g_zvo[g_zvo_n].head = head; g_zvo_n++; }
extern "C" int zvo_resolve(int off, long cur_dout, int cur_head, long * out) {   /* THE ONE OFFSET FUNCTION's table half: a legacy flat spelling off resolves to (off - owner_base) + (delta_out(reader) - delta_out(owner)) -- the s21x-u difference-of-two-depths law generalized from ZD operands to EVERY flat slot, which is what converts the ~1054 FR/FRQ/FRQB readers wholesale with zero template edits.  x86_frame_off adds the legacy op_flat_disp/op_zdepth on top (intra-claim fc-cell/window motion, unchanged machinery).  Miss = not a claimed slot in this run = legacy path, byte-identical. */
    for (int i = 0; i < g_zvo_n; i++) if (g_zvo[i].head == cur_head && off >= g_zvo[i].base && off < g_zvo[i].end) { *out = (long)(off - g_zvo[i].base) + (cur_dout - g_zvo[i].dout); return 1; }
    return 0;
}
extern "C" int zvo_resolve_base(int off, int cur_head, long * out) {   /* ⭐ ZD-5 MATCH-SPINE: the PIN coordinate half -- pure (off - owner_base), NO dout terms.  The fb_data+op_stmt_pin arm's own doc says "THE REBASE IS NOT DEPTH COMPENSATION: zvo's dout terms are K-K==0 on the UCLAIM path"; the hybrid staging broke that identity (armed members carry Kc + depth-below-claim so RSP readers rebase correctly), so the pin arm gets its own resolver that keeps the identity BY CONSTRUCTION for every reader depth -- the pinned rbp is the claim base and a slot is base + (off - umin), full stop.  Declined runs: dout==Kc for every member, so base-only == the old expression exactly; byte-identical there. */
    for (int i = 0; i < g_zvo_n; i++) if (g_zvo[i].head == cur_head && off >= g_zvo[i].base && off < g_zvo[i].end) { *out = (long)(off - g_zvo[i].base); return 1; }
    return 0;
}
extern "C" long zvo_owner_dout(int cur_head) {   /* ZD-5 MATCH-SPINE: the owner's staged depth-out (== Kc on hybrid runs) -- the head template subtracts this from its own op_udout to learn how far its alpha rsp sits BELOW the claim base, so the pin lands ON the base (lea rbp,[rsp+delta]) instead of delta below it. */
    for (int i = 0; i < g_zvo_n; i++) if (g_zvo[i].head == cur_head) return g_zvo[i].dout;
    return 0;
}
static long zvo_uclaim_k(IR_t * nd) {   /* UCLAIM K: the node's statement-scope claim = its flat-plan extent (zw_node_k, liveness-elided) minus any fc_geom cell it self-carves (that 16 is template-carved and legacy-compensated).  Protocol boxes that manage their own frames or transfer control claim nothing -- the four-construct family minus MATCH_BEGIN, which is exactly the box whose unclaimed quartet/subject region IS the s22r envp corruption. */
    int op = (int)nd->op;
    if (ir_norm_call_kind(nd->op) == IR_CALL || op == IR_SAVE_RESTORE || op == IR_GOTO_DEFERRED || op == IR_GOTO || op == IR_SUCCEED) return 0;
    { extern long zw_node_k(const IR_t *); long b = zw_node_k(nd); if (b <= 0) return 0;
      { long fck = 0; extern int fc_geom(const IR_t *, long *); if (fc_geom(nd, &fck) && fck > 0) b -= fck; }
      return b > 0 ? ((b + 15) & ~15L) : 0; }
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_patref_on(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZD_PATREF"); v = (e && (*e == '1' || *e == '2')) ? 1 : 0; } return v; }
static int zd_patref_k16(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ZD_PATREF"); v = (e && *e == '2') ? 1 : 0; } return v; }   /* ⭐ ZD-PATREF K A/B (s39): =1 arms with K=0 (pure transfer box), =2 arms with K=16 (SELF-CARVED locals).  WHY THE KNOB EXISTS: emit.cpp SPAN SHRINK excludes an armed member from the claim extent only when zd_k>0 -- so a K=0 PATREF leaves its zls watermark slot INSIDE the claim and INFLATES it (measured any.sno 256->496B, roman 240->416B).  PATREF genuinely owns storage (the s137 defer.pad watermark read as FRQ(op_off)), so under Lon's "have each BB allocate its LOCAL STORAGE needs, IF it has any" it must self-carve rather than ride the statement claim.  This knob measures which reading the emitted code agrees with. */   /* ⭐ ZD-PATREF gate (OMEGA s39): ONE reader for BOTH authorities -- zd_k's K verdict and zd_wl_kind's admission verdict -- so the switch cannot be half-flipped.  Two spellings of one gate is the s22k skew disease in miniature; this helper is why there is only one. */
static int zd_wl_kind(IR_t * nd) {
    int op = (int)nd->op;
    extern int is_global(const char *);
    if (g_emit.zframe_graph && !(g_emit_cfg && (g_emit_cfg->icn_cells_graph || g_emit_cfg->pl_cells_graph))) {
        /* ICN-FR-2: zframe graphs pre-allocate flat_frame_bytes on entry (sub rsp,kt; mov rbp,rsp); ZD spine-relative reads (ZOPQ = [rsp+op_zread[k]]) compute depth offsets WITHOUT accounting for this pre-allocation.  Result: n1's ZOPQ(0,0) reads from the FORTH spine cell [rbp-16] while n0 wrote to the ZLS frame slot [rbp+80] — 96 bytes apart.  Excluding zframe forces all nodes to the non-ZD (FRQ = [rbp+N]) path, which reads the frame slots correctly at any spine depth.  R-ICN-D: g_emit.zframe_graph=0 for all non-Icon graphs (SN4/Prolog/Raku/Pascal) so this return is never taken for them.  ZK-3 ADDITIVE: icn_cells_graph is the CELLS track (R-ZK-A routing law: mutually exclusive with zframe_graph at lower_icon.c — one graph is never in both arms); the cells arm uses FORTH-spine ZOPQ addressing which is correct regardless of the frame pre-allocation.  The conjunct lets the cells arm reach the per-kind whitelist below even when the graph's zframe_graph bit is 0 (cells arm sets icn_cells_graph, not zframe_graph).  PL-ZK-1b ADDITIVE: pl_cells_graph is the Prolog cells track; PL-FR-2 stamps zframe_graph=1 on all Prolog graphs (lower_prolog.c:1395), so both bits are simultaneously 1 when SCRIP_PL_CELLS=1 + SCRIP_PL_ZFRAME=1.  The pl_cells arm uses RSP-relative ZOPQ addressing (no rbp frame reads) and is correct under the zframe prologue — exempt it from the early-exit identically to icn_cells_graph.  R-PL-ZK-A mutual exclusion is now enforced HERE at the guard (not at lower_prolog.c, which cannot prevent PL-FR-2's wholesale zframe_graph stamp). */
    ...
    if (op == IR_VAR_REF && g_emit_cfg && g_emit_cfg->icn_cells_graph) { static int _icnvr = -1; if (_icnvr < 0) { const char * e = getenv("SCRIP_ZD_ICN_VR"); _icnvr = (e && *e == '0') ? 0 : 1; } return (_icnvr && x86_fb_pinned() && IR_LIT(nd).sval && !is_global(IR_LIT(nd).sval)) ? 1 : 0; }   /* ⭐ ZK-2 ADDITIVE: IR_VAR_REF admitted on the cells arm for PINNED, LOCAL-ONLY graphs.  WHAT IT DOES: produces {DT_N, &rbp_frame_slot} in ZRES -- a reference-DESCR pointing at the variable's depth-immune rbp-relative cell.  Downstream IR_DEREF (already ZD-admitted line 1931) reads ZOPQ(0,0/8) via its ZD arm, calls rt_deref, writes the dereferenced value to its own ZRES.  PINNED GUARD (x86_fb_pinned()): unpinned rsp-based graphs carry a LEA whose absolute address drifts with sub rsp,K depth accumulation -- the stored &frame_slot pointer reaches different memory at each depth.  Pinned rbp is depth-immune (xa_flat hdr seed, gen-adopt; x86_fb_pinned = emit_rec_pin).  GLOBAL GUARD (!is_global): globals route via GVA (ZK-5, ABSQ addressing) -- no rbp frame cell to point at; they remain on the non-ZD arm which writes FRQ(op_off) correctly.  SNOBOL4/Prolog watermark: icn_cells_graph=0 for non-Icon graphs -- byte-identical by construction.  CONSUMER CHAIN: run gate ensures VAR_REF+DEREF (and any other consumer) arm atomically or not at all -- the all-or-nothing guarantee prevents a partial-arm NAMETRAP-in-ZRES landing at a legacy rvalue consumer (the jcon_kross s221 measurement).  ONE AUTHORITY per s22k law -- only zd_wl_kind + zd_k (K=16 default) name this kind for the ZD regime.  Killswitch SCRIP_ZD_ICN_VR=0. */
    /* ⛔ ZK-2 FINDING (s220/s221): IR_VAR_REF Icon cells arm DEFERRED.  Two independent blockers measured this session:
       (1) UNPINNED GRAPH LEA: `lea rdx, [rsp + op_sa + depth]` at VAR_REF alpha captures an RSP-relative address;
           as subsequent sub rsp,16 carves accumulate the absolute address drifts -- rung23_table_basic measured
           [rsp+288] at n3 vs [rsp+288] at n8 with 80B of carves between them, reaching different memory.
           PIN GUARD (x86_fb_pinned()) prevents this, but...
       (2) LVALUE/RVALUE CONSUMER SPLIT (pinned graphs): rt_subscript_var ALWAYS returns a NAMETRAP (lvalue);
           when the NAMETRAP lands in a ZRES cell, lvalue consumers (rt_assign_var via IR_ASSIGN_VAR) handle it
           correctly, but rvalue consumers (right(), write() etc via IR_CALL_BUILTIN_ICON) receive the NAMETRAP
           where they expect a string/integer VALUE -- IR_DEREF (the rvalue-extraction node) is not yet admitted
           on the cells arm, so subscript+deref runs cannot arm atomically.  MEASURED (s221): xprint proc in
           jcon_kross (pinned flat_gen graph) armed 13/28 nodes with VAR_REF enabled, producing truncated output
           because the right() builtin received a NAMETRAP instead of the deref'd string value.
       NEXT ATTEMPT: admit IR_DEREF on the cells arm FIRST (K=16, nops=1, template ZD arm reads ZOPQ(0,0/8),
       calls rt_deref, writes ZRES(0/8)); then IR_VAR_REF can be re-admitted knowing the deref consumer is armed.
       The all-or-nothing run gate ensures that a run with VAR_REF+SUBSCRIPT+DEREF either arms completely or
       declines as a unit, preventing the partial-arm NAMETRAP-in-ZRES hazard.  Pl_cells_graph arm below stays. */
    if (op == IR_VAR_REF) { static int _plvr = -1; if (_plvr < 0) { const char * e = getenv("SCRIP_ZD_PL_VR"); _plvr = (e && *e == '0') ? 0 : 1; } return (_plvr && g_emit_cfg && g_emit_cfg->pl_cells_graph) ? 1 : 0; }   /* PL-ZK-1 arm 2/5: IR_VAR_REF admitted on the cells arm.  Census (s3): second-most-frequent blocker (13/6/35/9 occurrences).  K=16 default; nops=0 (no inline run-predecessor -- VAR_REF reads a named variable slot, not a spine cell; template ZD arm will read from slot).  Killswitch SCRIP_ZD_PL_VR=0. */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh, int *zzw, int *zzwr) {
    ...
    for (int i = 0; i < n; i++) { zon[i] = 0; zout[i] = -1; zgpop[i] = 0; zwpop[i] = 0; zuk[i] = 0; zud[i] = 0; zuh[i] = -1; zzw[i] = 0; zzwr[i] = 0; }
    ...
    { /* _ng gate deleted, CARVE-DATA-ERAD s23a */
      ...
      if (!_zd || x86_port_mode() != ZC_PORT_FORTH || n <= 0) return; }   /* ⭐ CARVE-DATA-ERAD (s23a): the jmp-entry WHOLE-GRAPH veto is DELETED -- blob statements enter the claim regime unconditionally.  zd_stub_ok is caller-less pending janitorial deletion. */
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        int nblob = 0; long umin = 0x7FFFFFFF, umax = -0x7FFFFFFF; extern int zls_node_off(const IR_t *); extern int zls_node_bytes(const IR_t *);
        { int wn = 0;
        ...
        int nblob_real = 0; { extern int zls_result_live(const IR_t *); for (int k = 0; k < n; k++) { if (!cm[k] || rpos[k] >= 0) continue; int b = zls_node_off(nodes[k]); if (b > -0x30000000 && zls_result_live(nodes[k])) nblob_real++; } }   /* ⭐ ZW-14 (OMEGA s33): REAL blob count -- blob closure members that have a non-sentinel zls_node_off AND live=1 (i.e. own a genuine live claim slot with shift-fragile FRQ spelling).  Two exclusions: (1) K=0 sentinel nodes (IR_MATCH_LIT/LEN/ANY/POS/etc. that are elided-and-absent from the plan: zls_node_off returns -0x40000000); (2) dead-result scratch nodes (live=0 in the zls entry: zls_grant_elide stamps the first dead leaf as the shared scratch at offset 0 via zls_entry()+live=0; subsequent dead leaves alias to the same scratch offset; zls_result_live returns e->live = 0 for these).  Both classes produce no FRQ emission in their templates -- the shift-fragile raw-claim-spelling hazard (s23i CAP-SLOT: canonical frame inserts ZW_FRAME_K=56B between claim carve and blob-relative FRQ reads, shifting them by 56B) does NOT apply because there are no such reads.  The canonical ZW-12 frame is therefore safe to arm for programs whose only off-run blob members fall into these two classes (e.g. FENCE('AB'), FENCE(POS(0)), FENCE(SPAN('A')), etc. where the argument is a pure scanner-register-only pattern).  nblob_real is the correct predicate for zw_nblob_ok; nblob (total closure count) remains available for the veto and the window-integrity scan.  rpos[k]>=0 excludes admitted run members.  Measured s33: widens armed population to include programs where blob is K=0 dead-result only. */   /* ⭐ ZW-14 (OMEGA s33): REAL blob count -- blob closure members that have a non-sentinel zls_node_off (i.e. own a genuine claim slot with shift-fragile FRQ spelling).  Elided K=0 scanner-register-only blob nodes (IR_MATCH_LIT, IR_MATCH_LEN, IR_MATCH_ANY/NOTANY/POS/RPOS/REM/etc.) return the -0x40000000 sentinel from zls_node_off (zls_grant_elide dead-branch, no zls_entry call) and are skipped harmlessly in the Kc span walk at :2016.  When nblob_real==0 all blob closure members are pure scanner-register-only with zero FRQ slots -- the shift-fragile raw-claim-spelling hazard (s23i CAP-SLOT: canonical frame inserts ZW_FRAME_K=56 bytes between claim carve and blob-relative FRQ reads, shifting them by 56 bytes) does NOT apply because there are no such reads.  The canonical ZW-12 frame is therefore safe to arm for these programs.  nblob_real is the correct predicate for zw_nblob_ok; nblob (total closure count) remains available for the veto and the window-integrity scan.  rpos[k]>=0 excludes admitted run members (not blob members even if cm[k] was set transitively). */
        int ok = (rl > 0); const char * why = ""; int badi = -1; int rgood = rl;
        int zdyn = 0; { static int _da = -2; if (_da == -2) { const char * e = getenv("SCRIP_ZD_DYNARM"); _da = e ? atoi(e) : -1; }   /* ⭐ ZD-5 PATREF/DEFER STATIC-SHAPE ARMING (s23i, the s23h-named next rung): a DEFER/PATREF whose pat_static=1 (lower's transitive defer-free proof over the seal table) no longer vetoes the quartet -- the stored pattern provably enters, scans, and returns with no blob re-entry, so the armed claim geometry is never crossed at an unmodeled depth.  The recursive class (135/136's B = '(' FENCE(*B|eps) ')') stamps 0 and keeps the declined regime VERBATIM, as do composite `*expr` sites and every non-eligible name -- degrade never die.  FENCE1 in the STATEMENT graph still declines unconditionally: its wire-glue geometry predates cells-above-claim and has no per-target proof to ride (142's lone in-statement green is one witness, not a bracket).  SCRIP_ZD_DYNARM is the MEASUREMENT MASK, unset = classifier: bit0 forces DEFER armed, bit1 PATREF, bit2 FENCE1 (=7 reproduces the s23h full-arm flip set {135,136,164,165,183}+{117}); =0 forces all three declined = the s23h HEAD regime byte-identical, the escape hatch every narrowing keeps. */
          static int _ds = -1; if (_ds < 0) { const char * s = getenv("SCRIP_ZD_DYNSCOPE"); _ds = (s && *s == '0') ? 0 : 1; }   /* ⭐ ZD-4 (s23q, ALPHA A-1) RUN-SCOPED DYNAMIC VETO -- default ON (flip s23s after A-2 gated 318 BY SET IDENTICAL).  =0 reverts to GRAPH scope byte-for-byte.  =1 scopes it to the run's own closure. */
          for (int k = 0; k < n && !zdyn; k++) { int o = (int)nodes[k]->op; if (_ds && !cm[k]) continue;   /* ⛔⭐ ZD-4 (s23q): THE VETO WAS FUNCTION-SCOPED AND THAT IS THE DEFECT.  `for k < n` scans EVERY node in the graph, so ONE dynamic box anywhere declines EVERY match statement in that graph -- the exact shape of the whole-graph jmp-entry veto that ZD-NOGRAPH deleted in s23a under Lon's s21x-w law ("NO FUNCTION-level processing for ZETA whatsoever.  ONLY statement level scoping"), reintroduced by kind instead of by entry.  MEASURED s23q on roman.sno: proc_LBL__ROMAN armed=0/30 with its per-node verdicts ALL admit=1 and its per-run reasons naming IR_MATCH_BEGIN -- i.e. the quartet veto at the member test below, not a capability gap; SCRIP_ZD_DYNARM=2 (PATREF force-arm) alone lifts it to armed=10, so the whole graph was being declined by ONE PATREF (`T` in spine position, statement 2's `'0,1I,..' ? T BREAK(',') . T`) sitting in a DIFFERENT statement.  cm[] is the run's own closure, so a dynamic box now vetoes the statement that CONTAINS it and no other; the hazard the veto guards (blob re-entry at an unmodeled claim depth, casualties {135,136,164,165,183}+{117}) is intra-statement by construction -- re-entry arrives through the bearing statement's own blob, never a sibling's.  Degrade never die: the bearing statement keeps its declined regime VERBATIM. */
            if (o == IR_MATCH_FENCE1) { static int _zdf1 = -1; if (_zdf1 < 0) { const char *_e = getenv("SCRIP_ZD_FENCE1"); _zdf1 = (_e && *_e == '0') ? 0 : 1; } if (!(_da >= 0 && (_da & 4) && !g_emit.flat_pat) && !(_zdf1 && !g_emit.flat_pat)) zdyn = 1; }   /* ⭐ O-7 FENCE1 zdyn veto: respects SCRIP_ZD_FENCE1=1 (default ON) AND !g_emit.flat_pat. Inside PAT$ blobs (flat_pat=1) the blob geometry predates cells-above-claim; blob-interior runs keep the veto even if their own run-closure contains no FENCE1. Outside blobs (flat_pat=0) the ZW canonical frame provides the correct depth model for FENCE1. Measured: 066_pat_fence_fn_nested segvd when blob runs armed via the veto lift. DYNARM override blocked inside blobs: (_da & 4) only lifts the veto when !flat_pat; inside blobs DYNARM=4 keeps the veto. */
            else if (o == IR_MATCH_DEFER) { int st = nodes[k]->pat_static; if (_da == 0) st = 0; else if (_da > 0 && (_da & ((o == IR_MATCH_DEFER) ? 1 : 2))) st = 1; if (!st) zdyn = 1; }
            if (zdyn && _dg) fprintf(stderr, "[ZD-DYN] h=%d vetoed by i=%d %s pat_static=%d in_run_closure=%d scope=%s\n", hi, k, bb_op_name(nodes[k]->op), nodes[k]->pat_static, (int)cm[k], _ds ? "run" : "graph"); } }   /* ⭐ ZD-4 (s23q): the veto's OWN reason line.  [ZD-GAP] prints only zd_wl_kind's per-node capability verdict, so a run declined by the quartet veto (or by nops/opnd) reads as "every node admits" -- which is exactly how the inherited A-1 rung text came to name a jmp-entry gate that s23a had already deleted.  in_run_closure names whether the offender is even in this statement. */   /* ⭐ ZD-5 DYNAMIC-BOX DECLINE (mirror of s22y's subject-cell rule, same three kinds, same reason): graphs bearing DEFER/PATREF/FENCE1 re-enter blobs through the stored-pattern/fence protocol whose geometry (ARBNO deep repoint static constant, blob wire glue) predates cells above the claim -- arming the quartet there re-created the s22y casualty classes, MEASURED this session as the m3 flip set {135,136,164,165,183} + m4 {117}.  Degrade never die: these statements keep the declined claim regime verbatim. */
        for (int r = 0; r < rl && ok; r++) { int i = run[r];
            { int _a = zd_wl_kind(nodes[i]); int _o = (int)nodes[i]->op; if (_a && zdyn && (_o == IR_MATCH_BEGIN || _o == IR_MATCH_END || _o == IR_MATCH_REPLACE || _o == IR_MATCH_ALTERNATE)) _a = 0;   /* IR_MATCH_ALTERNATE joins the zdyn veto arm (s29): FENCE1/DEFER/PATREF in the run closure must also decline ALTERNATE -- ALTERNATE's subwalked arms can contain FENCE1, whose wire-glue geometry predates cells-above-claim (same hazard class as BEGIN/SEQ).  Without this, a FENCE(ALTERNATE(...)) run admits ALTERNATE while being vetoed for BEGIN, corrupting the claim depth. */
        ...
        if (ok) { int zd = 0;
            ...
            long Kc = 0; int hpos = -1; IR_t * hnode = 0; for (int r = 0; r < rl; r++) if (nodes[run[r]]->op == IR_MATCH_BEGIN) { hpos = r; hnode = nodes[run[r]]; break; }
            IR_t * subjkeep = 0; { extern int fc_vread_fp(const IR_t *); if (hnode && hnode->n_operands > 0 && fc_vread_fp(hnode) < 0) subjkeep = hnode->operands[0]; }   /* !subjc graphs: the mirror/REPLACE read op_sa = this producer's slot -- keep its extent in the span */
            int has_blob = (nblob > 0); if (!has_blob) { for (int _r = 0; _r < rl; _r++) { int _ro = (int)nodes[run[_r]]->op; if (_ro == IR_MATCH_LIT || _ro == IR_MATCH_LEN || _ro == IR_MATCH_ANY || _ro == IR_MATCH_NOTANY || _ro == IR_MATCH_SPAN || _ro == IR_MATCH_TAB || _ro == IR_MATCH_RTAB || _ro == IR_MATCH_POS || _ro == IR_MATCH_RPOS || _ro == IR_MATCH_REM || _ro == IR_MATCH_BREAK || _ro == IR_MATCH_BREAKX || _ro == IR_MATCH_ARB || _ro == IR_MATCH_BAL || _ro == IR_MATCH_ALTERNATE || _ro == IR_MATCH_ASSIGN_SAVE || _ro == IR_MATCH_ASSIGN_COND || _ro == IR_MATCH_ASSIGN_IMM) { has_blob = 1; break; } } }   /* ⭐ ZD-5b (A-7, s24a/s24b): hoisted BEFORE Kc so the has_blob OR nblob gate can use it.  When blob-interior K=0 kinds enter run[], nblob drops to 0 (those nodes are no longer off-run operand-closure members), but the PATCTX claim is still needed and the canonical ZW frame must still be blocked.  has_blob captures both cases. */
            if (nblob > 0 || has_blob) { for (int k = 0; k < n; k++) { if (!cm[k] && !(rpos[k] >= 0 && (!zd_patref_on() || claim[k] == hi) && zd_k(nodes[k]) == 0)) continue;   /* ⭐ ZD-PATREF [RECON MERGE]: claim[k]==hi guard added (gated on zd_patref_on()): stale rpos[] from prior ARMED statements (which do NOT reset rpos[]) could include K=0 nodes (e.g. MATCH_BEGIN from h=0) in h=6's span walk, driving umin=0 and inflating Kc. Guard accepts only nodes claimed by THIS statement. Gated to preserve gate-off byte-identity. OMEGA's unconditional PATREF exclusion (line below) handles the defer.pad blob inflation; this guard handles residual cross-stmt K=0 contamination. */   /* ZD-5b (s24b): has_blob path (nblob=0 but blob-interior kinds in run[]) also needs Kc -- PATCTX/marks still live in the claim.  cm[k] covers blob closure; rpos[k]>=0 && K==0 adds admitted K=0 run members that have zls slots (scanner-register-only kinds like LIT return the sentinel from zls_node_off and are skipped harmlessly below). */
                    if (ir_norm_call_kind(nodes[k]->op) == IR_CALL || nodes[k]->op == IR_SAVE_RESTORE || nodes[k]->op == IR_GOTO_DEFERRED) continue;
                    if (rpos[k] >= 0 && zd_k(nodes[k]) > 0 && nodes[k] != subjkeep) continue;   /* ⭐ SPAN SHRINK (Lon "a var just needs a little space"): an ARMED value member's zls slot is DEAD -- its value lives in its cell -- so its extent must not inflate the claim.  The one exception is the !subjc subject producer above (its slot is the live op_sa dataflow).  The K=0 quartet's quads (PATCTX, marks, start_delta) stay: they are the claim's whole point. */
                    { int _op = (int)nodes[k]->op; if (_op == (int)IR_MATCH_DEFER) continue; }   /* ⭐ SPAN SHRINK K=0 TRANSFER (O-PB-2): PATREF/DEFER defer.pad is always addressed FRQ (rbp-relative, depth-immune under ZW canonical frame) -- NOT a claim-relative RSP cell -- whether the node is an armed run member (rpos>=0) or a blob-closure-only member (rpos=-1, cm=1).  Including their zls offset in umax inflates Kc: measured any.sno stmt2 NOTANY, PATREF at b=480 e=496 (rpos=-1 cm=1) expanded umax from 368->496 adding 128B to Kc.  Excluded from the span regardless of run/closure membership. */
                    { int b = zls_node_off(nodes[k]); if (b <= -0x30000000) continue; int e = b + zls_node_bytes(nodes[k]); if (b < umin) umin = b; if (e > umax) umax = e; } }
                if (umax > umin) Kc = ((long)(umax - umin) + 15L) & ~15L; }
            int zdh = 0;   /* zd at head entry == bytes of pre-head cells, which sit ABOVE the claim (carved before it) */
            int zws = 0; if (zw_frame_on() && Kc > 0 && hpos >= 0 && zw_nblob_ok(nblob_real, has_blob) && !g_emit.flat_stmt_frame) {   /* STF exclusion: flat_stmt_frame is its OWN statement rbp bracket (st_x leave stubs whack via mov rsp,rbp) -- the zw match frame inside it would nest two rbp disciplines; STF keeps the old regime */ int _end = 0, _seal = 0; for (int r = 0; r < rl; r++) { int _o = (int)nodes[run[r]]->op; if (_o == IR_MATCH_END) _end = 1; if (_o == IR_MATCH_ABORT) _seal = 1; (void)0;   /* ⭐ ZW-13 (s32 OMEGA), MEASURED not assumed: with the blob clause restored, 22 of the 23 armed programs pass BOTH modes; the sole regression is 161_pat_defer_fn_nested_match (m3 PASS -> SIGSEGV).  DEFER/PATREF are the WHACK CONTRACT clause-1 mechanism-(2) citizens -- a deferred `*P` re-enters the matcher at runtime-variable extent (manual p.122: only `*` permits the self-reference that recurses), so the head's `mov rsp,rbp; pop rbp` can land against a nested activation the planner never modelled.  Sealing them is the ABORT precedent and the LAWS' DEFER-DEEP-LOAD-BEARING warning applied verbatim: degrade never die.  Their admission is its own rung once the nested-frame protocol is designed, NOT a widening of this one. */ { static int _zdf = -1; if (_zdf < 0) { const char *_e = getenv("SCRIP_ZD_FENCE1"); _zdf = (_e && *_e == '0') ? 0 : 1; } if (_zdf && _o == IR_MATCH_FENCE1) {} else if (_o == IR_MATCH_FENCE1) _seal = 1; } } for (int _g = 0; !_seal && _g < n; _g++) { int _go = (int)nodes[_g]->op; if (_go == IR_MATCH_DEFER) _seal = 1; /* PB-3: IR_MATCH_PATREF(DELETED) removed from ZW-13 graph-scope seal — PATREF is being deleted (PB-5); DEFER stays (sole recursion vehicle, manual p.122) */ }   /* ⭐ ZW-13 seal is GRAPH-SCOPE, not run-scope (measured s32): sealing only run MEMBERS left 161_pat_defer_fn_nested_match armed and still SEGV-ing, because its `*P` deferral is invoked from a pattern-function body that lowers into a DIFFERENT run -- the outer run's members are all ordinary, so a member scan sees nothing to seal.  The hazard is re-entrancy of the MATCHER, not membership: any graph holding a deferral can re-enter the head whose frame is live, so the whole graph declines.  This is a decline-widening only (degrade never die) and is the run-scope/graph-scope distinction the s21x-r law names -- graph-scope evidence driving a run-scope verdict is sound; the reverse is what cost 41 programs. */ zws = _end && !_seal;   /* ⭐ O-7 ZD-5b FENCE1 ZD ARM (s30): FENCE1 removed from the seal exclusion under SCRIP_ZD_FENCE1=1 (default ON). ABORT stays sealed: it bypasses the head's ω-choke with pattern cells still carved at dynamic depth, so the ZW frame's `mov rsp,rbp; pop rbp` whack would land with live cells above it. FENCE1 is safe under the ZW canonical frame because fence_whack_commit is updated to use `mov rsp,[rbp+0]` (= activation floor = old rbp pushed by match_begin) instead of `mov rsp,rbp` (= match frame base) -- the ZW frame's push rbp saves the pre-match rbp at [rbp+0], so the activation ζ bulk-free whacks to the right floor. Killswitch SCRIP_ZD_FENCE1=0 restores the sealed regime byte-identical. */ if (zws) { int epos = -1; for (int r = 0; r < rl; r++) if (nodes[run[r]]->op == IR_MATCH_END) { epos = r; break; }
                for (int r = hpos + 1; zws && r < epos; r++) { IR_t * gt = zd_chase(nodes[run[r]]->γ.node); IR_t * ot = zd_chase(nodes[run[r]]->ω.node); int gin = 0, oin = 0;
                    for (int k = 0; k < rl; k++) { if (nodes[run[k]] == gt) gin = 1; if (nodes[run[k]] == ot) oin = 1; }
                    if ((gt && !gin) || (ot && !oin)) zws = 0; } } }   { static int _zwsd = -1; if (_zwsd < 0) { const char *_e = getenv("SCRIP_ZWS_DIAG"); _zwsd = (_e && *_e == '1') ? 1 : 0; } if (_zwsd && hpos >= 0) fprintf(stderr, "[ZWS] %s: Kc=%ld rl=%d nblob=%d nblob_real=%d has_blob=%d hpos=%d stf=%d why=%s\n", zws ? "ARMED" : "DECLINED", (long)Kc, rl, nblob, nblob_real, has_blob, hpos, (int)g_emit.flat_stmt_frame, zws ? "-" : (!zw_frame_on() ? "killswitch" : Kc <= 0 ? "Kc<=0" : !zw_nblob_ok(nblob_real, has_blob) ? "blob-clause" : g_emit.flat_stmt_frame ? "stf" : "seal/no-END/window")); }   /* ⭐ ZW-13 (s32): the diag printed ONLY on arm, so a predicate that armed NOTHING printed NOTHING and read as "no news" for eight sessions while O-3/O-4/O-5s2/O-7 all landed into the dead op_zw arm and reported BY-SET-IDENTICAL gates -- which they were, being no-ops on emitted output.  Attribution on the DECLINE side is the instrument that catches that class; same lesson as the playbook's ZD-GAP-is-blind note, applied to the frame verdict. */   /* ⭐ ZW-12 window-integrity pre-scan: a member STRICTLY inside (head, END) whose γ/ω escapes the RUN ENTIRELY would cross the statement cut with the frame still pushed -- rsp arithmetic balances (its staged pop carries the +TOTAL) but old rbp is stranded inside the released region, so such a run DECLINES the frame wholesale (byte-identical, degrade never die).  Membership here is ANY-position (not the k>r forward test the pop staging uses): a backward edge into the head is ABSORBED -- the head's β/ω machinery unwinds ABSOLUTELY off rbp (the frame's whole point), so a premature relative add on the way in is overwritten, exactly as today's marker-carried rsp restore already tolerates.  The head itself (γ into pattern elements, ω to statement fail) is exempt by construction: its ω tail IS the whack site and its staged zwpop was computed at α coordinates (the +TOTAL enters the model only after the head's own staging). */   /* ⭐ RUNG ZW-12 verdict (s23l), the ONE predicate: canonical frame arms only where (a) the claim world is coherent (Kc>0 -- jmp-entry/EVAL fragments decline claims and keep the old regime verbatim, sidestepping the s23j roman hazard by construction), (b) no blob members (nblob==0 -- the shift-fragile raw-claim-spelling class, s23i CAP-SLOT; their fix is the r9/wire rung, not this one), (c) no seal member (ABORT/FENCE1 jump past the head's omega-choke -- zeta_storage's KNOWN BYPASS -- and would leave the frame's rbp live across a statement cut; excluded until the fJ route lands), (d) an IR_MATCH_END closes the run (the frame MUST be popped).  Degrade never die: a declined run is byte-identical to today. */
            int zwr = 0; if (zw_rb_on() && !zws && zw_frame_on() && Kc > 0 && hpos >= 0 && nblob_real > 0) { int _end = 0, _seal = 0; for (int r = 0; r < rl; r++) { int _o = (int)nodes[run[r]]->op; if (_o == IR_MATCH_END) _end = 1; if (_o == IR_MATCH_ABORT) _seal = 1; } for (int _g = 0; !_seal && _g < n; _g++) { int _go = (int)nodes[_g]->op; if (_go == IR_MATCH_DEFER) _seal = 1; /* PB-3: IR_MATCH_PATREF(DELETED) removed from ZW-13 graph-scope seal — PATREF is being deleted (PB-5); DEFER stays (sole recursion vehicle, manual p.122) */ } zwr = _end && !_seal; }
            /* ⭐ W-1 MECH-2 LAYOUT: header = 8 fixed slots × 8B = 64B at [rbp-8]..[rbp-64] (outer_Σ/δ/Δ,capgen,start_δ,patstk,zls2) + 8B alignment pad = 72B header. Blob flat cells via FR [rsp+8+off]. Total Kc_zwr = Kc_cells + 72. (Kc_cells≡0 mod16 → Kc_zwr≡72≡8 mod16 → after push_rbp(−8) + sub_rsp,Kc_zwr: rsp ≡ 0 mod16, correct for C ABI movaps.) Only fed to op_udout (mech-2 head); ZW-12/UCLAIM paths keep plain Kc. */
            long Kc_zwr = (zwr && Kc > 0) ? Kc + 72L : Kc;   /* ⭐ MECHANISM-2 (HQ ruling 2026-08-03): arms blob-clause programs (nblob_real>0) that ZW-12 cannot arm.  Predicate mirrors ZW-12 but with nblob_real>0 instead of ==0.  Seal logic identical.  DEFAULT OFF via zw_rb_on().  ⭐ MECHANISM-2 STF EXTENSION: !flat_stmt_frame guard REMOVED.  Under the STF+mechanism-2 nested model (HQ ruling: nested indeterminacy = nested frames), flat_stmt_frame=1 is the OUTER frame; zwr fires the INNER push rbp at MATCH_BEGIN.  The original guard prevented nesting two rbp disciplines -- that concern is resolved by the ruling: nesting is correct and the two whacks unwind in order (MATCH_END pops inner, statement-exit cut pops outer). */   { static int _zwsd = -1; if (_zwsd < 0) { const char *_e = getenv("SCRIP_ZWS_DIAG"); _zwsd = (_e && *_e == '1') ? 1 : 0; } if (_zwsd && hpos >= 0 && !zws) fprintf(stderr, "[ZWR] %s: Kc=%ld rl=%d nblob=%d nblob_real=%d has_blob=%d hpos=%d stf=%d\n", zwr ? "ARMED" : "DECLINED", (long)Kc, rl, nblob, nblob_real, has_blob, hpos, (int)g_emit.flat_stmt_frame); }
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (r == hpos) zdh = zd;
                if ((zws || zwr) && nodes[i]->op == IR_MATCH_END) { int _fci = 0; if (zws || zwr) { for (int _r = hpos + 1; _r < r; _r++) _fci += zd_k(nodes[run[_r]]); } zd -= (zws ? (int)ZW_FRAME_TOTAL + _fci : 8 + _fci); }   /* ⭐ ZW-12: END's zout (and the terminal-cut pops computed below it) are POST-WHACK coordinates -- `lea rsp,[rbp-8]; pop rbp` lands rsp exactly at the head-α claim base.  The whack removes ZW_FRAME_TOTAL (push rbp + sub rsp,ZW_FRAME_K) PLUS any fc_geom FORTH cells (sub rsp,K each) that run members between head and END emitted inside the frame -- the sum of their zd_k values is the exact extra rsp movement the whack also undoes.  Without this correction, post-whack zd remains inflated by _fci and the terminal gpop over-releases by _fci, corrupting the caller frame (the Z4 span segfault root cause).  ⭐ MECHANISM-2 W-1 FIX: same law applied to mech-2 -- whack removes push_rbp(8) PLUS blob-member Ks (SAVE cells' sub rsp,16 inside the frame); 8+_fci instead of 8 alone.  Without _fci the STATEMENT_END's zgpop was inflated by _fci, making the post-whack stack release over-count, corrupting RSP at program exit (SEGV at ret). */
                zon[i] = 1; zout[i] = zd + K; if (Kc > 0 && hpos >= 0 && r >= hpos) { long _kbase = (zwr && r == hpos) ? Kc_zwr : Kc; zud[i] = _kbase + (zd - zdh) + K; zuh[i] = hi; } if (zws && r >= hpos) zzw[i] = 1; if (zwr && r >= hpos) zzwr[i] = 1;   /* ⭐ MECHANISM-2: zzwr[] is the parallel mechanism-2 member array staged into g_zd_uzwr → op_zw2 at the same choke as zzw → op_zw. */   zd = zout[i];
                if (zws && r == hpos) zd += (int)ZW_FRAME_TOTAL;   /* ⭐ ZW-12: the canonical frame's push rbp + sub rsp,ZW_FRAME_K enters the model HERE, after the head's own staging (the head's zref subject read executes at α depth, BEFORE the push) and before every member between head and END -- their claim-slot reads gain exactly the bytes the emitted frame really costs, from the SAME named constant, so the two sides cannot drift (the zd_k one-authority law applied to the frame). */   /* ⭐ CLAIM-AT-HEAD (v2, supersedes the fold): the claim is carved at the HEAD's alpha -- the match owns its scanner storage -- so pre-head producer cells sit ABOVE it and the head's own rsp IS the claim base with ZERO instruction cost: no pop, no fold, PURE NON-POPPING (the directive verbatim), and the blob's raw claim spellings hold at OFF-world distances by construction.  zud for at/after-head members = Kc + their cells BELOW the claim (zd - zdh, plus own K since the FRQ spellings execute post-own-carve). */
                if (zwr && r == hpos) zd += 8;   /* ⭐ MECHANISM-2: only push rbp (8B), no sub rsp,ZW_FRAME_K -- FRQ spellings unshifted relative to ZW-12 */
                IR_t * gt = zd_chase(nodes[i]->γ.node); IR_t * ot = zd_chase(nodes[i]->ω.node);
                int gin = 0; int oin = 0;
                if (nblob > 0) { for (int k = 0; k < n; k++) if (cm[k]) { if (nodes[k] == gt) gin = 1; if (nodes[k] == ot) oin = 1; } }
                else for (int k = 0; k < rl; k++) { if (nodes[run[k]] == gt && k > r) gin = 1; if (nodes[run[k]] == ot) oin = 1; }   /* ⭐ ZD-5b-SAVE-OIN (s14): drop k>r on oin — scan-retry back-edge (ω→MATCH_BEGIN, k<r) is intra-match; forward-only guard was correct for gin but wrong for oin. */
                ...
                { if (!oin && (zws || zwr) && K == 0) { int _io = !ot; if (!_io) { for (int _ik = 0; _ik <= r; _ik++) { if (nodes[run[_ik]] == ot) { _io = 1; break; } } } if (_io) oin = 1; } }   /* ⭐ M-1 FIX 3 + ZWR EXTENSION (C-5 CLIMB, ZK-3/ZK-4 regression fix): backward-edge oin guard for ZWS and ZWR.  !ot arm: ω.node=NULL means template-routed fail (e.g. MATCH_LIT abort-fail path) — MATCH_END is sole release authority, suppress wpop.  ot-found arm: ω points to an earlier run member (k<=r backward edge) — same suppression law as ZWS M-1 FIX 3.  GATED K==0 && (zws||zwr): non-match, non-window, K>0 runs are byte-identical.  ONE LINE per s22k law. */
                { long kc = (hpos >= 0 && r >= hpos && !zwr) ? Kc : 0;   /* claim is live only from the head onward; a pre-head exit (subject-eval failure) frees cells alone. ⭐ W-1: !zwr -- mech-2 has NO UCLAIM head claim (push_rbp is the boundary, no sub rsp,Kc); kc=0 for mech-2 members or their zgpop/wpop is inflated by Kc (256-SEGV class). */
                int _wzdepth = ((zws && r == hpos) || (!zws && !zwr && Kc > 0 && r > hpos)) ? zdh : (int)zd;   /* ⭐ ZW-12 HEAD-WPOP FIX: MATCH_BEGIN's ω exit is the pattern-exhausted fail — it fires AFTER the whack restores rsp to the claim base, so it sees only pre-frame depth (zdh), NOT the post-ZW_FRAME_TOTAL depth (zd which has been incremented by 64).  Using post-ZW zd here inflated wpop by ZW_FRAME_TOTAL=64, causing add rsp,N to release 64 bytes into the caller's frame on each scan retry (rip=0x1/0x3 class, Z4 span crash).  BUG-2-FIX (s17): extended to non-framed (!zws && !zwr) post-head (r>hpos) runs with Kc>0 -- MATCH_END's mov rsp,[zls2_mark] collapses all interior cells back to claim_base; STATEMENT_END zgpop = zdh+Kc (not zd+Kc where zd carries accumulated post-head member Ks that are already freed by the zls2_mark restore, causing over-release by SAVE.K=16 in pb_snapshot_imm: 192->176). */
                ...
                if (!gin) zgpop[i] = _wzdepth + (int)kc;
            ...
            if (Kc > 0 && hpos >= 0 && !zwr) { zuk[run[hpos]] = Kc; zvo_add((int)umin, Kc, Kc, hi);   /* ⭐ !zwr: mechanism-2 suppresses the UCLAIM head-claim (push rbp is the boundary, no sub rsp,K); blob FRQ spellings use raw off+op_zdepth.  CLAIM-AT-HEAD: zuk staged on the MATCH_BEGIN member -- the port hook carves it at the head's alpha, once per statement execution. */
                for (int k = 0; !zwr && k < n; k++) if (cm[k] && zuh[k] < 0 && claim[k] != hi) { zud[k] = Kc; zuh[k] = hi; }   /* ⭐ !zwr: under mechanism-2, blob members use raw FRQ spellings (no claim-relative rebase).  blob members resolve DECLINED-IDENTICAL: dout delta ZERO, not Kc+hdd.  ⭐ ZD-5b-LEN claim[k]!=hi GUARD: exclude current-run members from the blob-closure Kc assignment.  Pre-head run members (r<hpos) have cm[k]=1 (seeded by the closure walk's run[] seed loop) and zuh[k]=-1 (the r>=hpos branch never fires for them), so without this guard they would get zud[k]=Kc and become indistinguishable from post-head members at the staging site -- killing the _xh cross-head detection (which needs zd_ud[pre-head-producer]==0 to fire).  Post-head run members already have zuh[k]=hi (set at line 2087 r>=hpos branch) so zuh[k]<0 is false for them; this guard only adds exclusion for pre-head members.  claim[k]==hi is the current-run-membership predicate (set by the inner walk loop for all gamma-chain members of run hi, not reset between runs -- unlike rpos[] which is stale across runs); off-run closure members keep claim=-1.  Byte-identical when no pre-head run member exists (only post-head runs, the prior non-LEN population). */
            if (ok && Kc > 0 && hpos >= 0 && hi >= 0 && hi < n) zud[hi] = (long)zdh;   /* ⭐ ZD-5b-LEN: store zdh in the run-head's zud slot (STATEMENT_BEGIN, always pre-head so zuh=-1; the rpos[k]<0 guard above now correctly skips it because rpos[hi]=0>=0, protecting this store from being overwritten by the blob-closure Kc assignment -- previously hi=STATEMENT_BEGIN had rpos[hi]=0 >= 0 but the original guard was zuh[k]<0 only, which let the blob loop stomp zdh=48 with Kc=256, making _xh detect 256!=0 and fail).  After this line: zud[hi]=zdh=48, used at staging site by _xh formula: correct_ZOPQ = zd_ud[consumer] - K(consumer) + zd_ud[hi] - zd_out[producer]. */
                if (_dg) fprintf(stderr, "[ZD-H] h=%d Kc=%ld span=[%ld,%ld) hpos=%d zdh=%d nblob=%d\n", hi, Kc, umin, umax, hpos, zdh, nblob); }
            if (Kc > 0 && hpos >= 0 && zwr) { for (int k = 0; k < n; k++) if (cm[k] && !zzwr[k]) zzwr[k] = 1; }   /* ⭐ ZW-RB-1 Part 1: mark cm[] blob-closure members with zzwr[k]=1 when mechanism-2 is armed so op_zw2=1 fires and op_zdepth=8+Kc compensation applies: [rsp+off+8+Kc]=[α_base+off]. */
        }
        else { /* UCLAIM (wholesale flip, Lon 2026-08-01 "FLIP them ALL on ... ONE FUNCTION to calculate the offsets based on the execution order"): a DECLINED run is exactly the population whose flat readers address the DELETED whole-graph frame -- the s22r envp-corruption class, and SCRIP_M4_HEADROOM=65536 flipping 15 programs green proved those readers are INTERNALLY CONSISTENT with only the RESERVATION missing.  So the RUN HEAD claims the statement's whole flat extent (the union of member ranges) at its OWN alpha -- one sub, once per statement execution, single-entry by construction (bb_src_of statement roots) -- and every terminal exit releases it through the EXISTING op_zgpop/op_wpop channels (the head's exhaust already ends in x86_omega(), so the release rides the proven hook arms and the x86_jcc invert synth).  PER-NODE claims were designed first and REJECTED before shipping: match-blob interiors re-enter alpha on every backtrack retry (alternation beta cycling), so a static per-node claim there re-carves per retry and leaks; and a head-exhaust release staged at the head's run position would under-free every claim minted downstream of it.  ONE claim at the single-entry head has neither disease: interior alphas re-fire against a claim that is already live, depth is the CONSTANT K for every member (so mid-run exits free exactly K, correct on every path), and loops re-entering the statement label re-claim only what their previous iteration's terminal cut freed.  The owner-table entry spans [min,max) tagged by run head, so region-shared offsets across statements cannot alias; members' claims resolve to (off - min) + (K - K) = off - min plus the untouched op_flat_disp/op_zdepth intra-statement compensation.  Statement-level scoping per Lon s21x-w ("ONLY statement level scoping"); ARBNO/REPALT per-iteration cells ride their existing linked-frame machinery BELOW the claim, unclaimed and unchanged. */
            long umin = 0x7FFFFFFF, umax = -0x7FFFFFFF; extern int zls_node_off(const IR_t *); extern int zls_node_bytes(const IR_t *);
            unsigned char * mem = (unsigned char *)alloca((size_t)(n > 0 ? n : 1)); for (int k = 0; k < n; k++) mem[k] = 0;
            { int * wl = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int wn = 0;
              for (int r = 0; r < rl; r++) { mem[run[r]] = 1; wl[wn++] = run[r]; }
              while (wn > 0) { int i = wl[--wn];   /* OPERAND CLOSURE (the t7 frame-header smash, this session): match blobs hang off run members as OPERAND TREES, not gamma links -- the pos/lit leaves write their zls slots inside the statement span but were invisible to the chase, so they kept the legacy ghost spelling while the claim moved rsp down by K and dropped those ghosts onto the saved-rbp/return slots.  Every transitive operand is a statement member: it gets the staged resolution (zud/zuh) so its flat spellings resolve into the claim, and it counts for the span and for the exit test below (a spine edge landing on a blob beta is intra-statement, never a release site -- which also retires the latent capture-reject mid-match cut the 4-member test would have staged on the release's omega). */
                  for (int a = 0; a < nodes[i]->n_operands; a++) { IR_t * p = nodes[i]->operands[a]; if (!p) continue;
            ...
            if (umax > umin) { long K = ((long)(umax - umin) + 15L) & ~15L;
                ...
                zvo_add((int)umin, K, K, hi);
                for (int r = 0; r < rl; r++) { int i = run[r];
                    if (nodes[i]->op == IR_STATEMENT_BEGIN) continue;   /* s30: BEGIN is a pure relay in UCLAIM runs -- its gamma always targets the first content node (inside the run, gin=1 guaranteed), so zgpop=0 is already correct; its omega exits the run to the fail label, but END at run[rl-1] already owns the sole release authority (gpop=K there) -- allowing wpop=K on BEGIN too creates a double-release: K added twice => rsp zeroed => segfault at main_gamma (measured 058_pat_fence_keyword, gdb: rbp/rsp==0x0). */
                    IR_t * gt = zd_chase(nodes[i]->γ.node); IR_t * ot = zd_chase(nodes[i]->ω.node);
                    int gin = 0, oin = 0;
                    for (int k = 0; k < n; k++) if (mem[k]) { if (nodes[k] == gt) gin = 1; if (nodes[k] == ot) oin = 1; }   /* membership = the FULL statement closure, any position -- backward backtrack edges and operand-tree landings are intra-statement */
                    if (!gin) zgpop[i] = (int)K;
                    if (!oin) zwpop[i] = (int)K;
                    if (_dg && (zgpop[i] || zwpop[i])) fprintf(stderr, "[ZD-U] h=%d r=%d i=%d %s K=%ld gpop=%d wpop=%d span=[%ld,%d)\n", hi, r, i, bb_op_name(nodes[i]->op), K, zgpop[i], zwpop[i], umin, umax); } }
            if (rl > 0 && zzwr[run[0]]) { for (int r = 0; r < rl; r++) { zgpop[run[r]] = 0; zwpop[run[r]] = 0; } }   /* ⭐ MECHANISM-2: whack is sole release authority -- zero zgpop/zwpop (UCLAIM computed them above; mechanism-2 supersedes). Use zzwr[run[0]] since zwr is in a different scope. */
...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    g_emit.op_stmt_pin        = 0;   /* HEAD-PIN (s22z): statement-scoped, must not leak across chains (a blob emitted after a pinned main statement would otherwise inherit a live pin into its cuts) */
...
#undef RPO_VISITED
    ...
    long *zd_uk = (long *)alloca(sizeof(long) * (size_t)(n > 0 ? n : 1)); long *zd_ud = (long *)alloca(sizeof(long) * (size_t)(n > 0 ? n : 1)); int *zd_uh = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int *zd_zw = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int *zd_zwr = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1));   /* UCLAIM: per-node staged claim / depth-out / run-head for the ONE offset function; zd_zwr = mechanism-2 RBP boundary arm */
    ...
    if (scan_live) {
        ...
        if (g_emit_cfg && g_emit_cfg->pl_cells_graph && g_emit.flat_all_zd) { int _plk = 0; for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _nd = g_emit_cfg->all[_i]; if (_nd && zd_wl_kind(_nd)) _plk += zd_k(_nd); } if (_plk > 0) { bb_emit_x86(x86("sub", "rsp", (long)_plk)); } }   /* PL-ZK-3 COMPLETE (s6): PROC-ENTRY CELLS CARVE gated on flat_all_zd (Option A, Lon ruling).  flat_all_zd=1 iff EVERY node armed by zd_plan -- no UCLAIM path possible by construction (LP-2 sets flat_all_zd from the SAME zd_on[] array the drive loop uses; ONE AUTHORITY, cannot disagree).  When flat_all_zd=0 (any declined node -- UCLAIM fires), the carve is suppressed and UCLAIM path runs byte-identically to cells=0 -- RSP_stmt_entry = RBP - flat_frame_bytes as zvo_resolve expects.  Gate makes the two regimes mutually exclusive: flat_all_zd=1 => carve fires, UCLAIM cannot fire (no declined nodes); flat_all_zd=0 => carve suppressed, UCLAIM path correct.  TIMING: flat_all_zd set by LP-2 before prologue dispatch; this carve runs inside the prologue arm -- correct ordering, no circularity.  The zframe prologue (xa_flat_zframe_prologue) carves flat_frame_bytes for the ZLS frame and pins rbp=rsp.  When pl_cells_graph=1 AND flat_all_zd=1 the planner additionally stages K_total=sum_zd_k(admitted) bytes of per-BB cells on the FORTH spine BELOW the ZLS frame.  FIX: sum zd_k over all admitted nodes -- THE ONE AUTHORITY (zd_k, same function the planner calls; this scan calls it directly to avoid a second spelling) -- and emit sub rsp,_plk immediately after the zframe prologue while rbp is pinned.  CORRECTNESS: _plk is the STATIC maximum cells depth (sum over all arms admitted, non-popping -- they stack); the epilogue lea rsp,[rbp+flat_frame_bytes] unwinds from ANY in-body rsp depth (depth-immune by rbp pin).  GATED pl_cells_graph: SN4 never sets pl_cells_graph; Icon uses icn_cells_graph on a mutually exclusive path (R-ZK-A); byte-identical for both by construction.  KILLSWITCH: SCRIP_PL_CELLS=0 leaves pl_cells_graph=0.  BOTH-MEDIUM: bb_emit_x86(x86("sub",...)) routes through the x86() encoder.  ONE AUTHORITY: this site is the only proc-entry cells carve; zd_plan K_total and this scan agree by construction (both call zd_k).  s22k law: _plk is computed by the ONE K function, never re-spelled. */
    } else if (g_emit.flat_lcl_proc) {   /* ICN-PROC-FRAME (s211): jmp-entry lexical proc with params/locals.  Prologue: sub rsp by kt+(np+nl)*16 to carve wire-header+param+local slots; save γ/ω wires and caller-rbp in the header; pin rbp=rsp; call rt_lcl_proc_args_install(rbp,np,nl) to copy g_call_args into [rbp-(i+1)*16] and zero locals.  BOTH MEDIUM: TEXT and BINARY both emit a C call so no medium-specific address arithmetic is needed.  Params address: [rbp-16] .. [rbp-np*16]; locals: [rbp-(np+1)*16] .. [rbp-(np+nl)*16].  Wire-header layout (same as BLOB-GRANT): [kt-24]=γ [kt-16]=ω [kt-8]=caller-rbp; at entry rsp==rbp, so [rsp+kt-24]==[rbp+kt-24]. */
        ...
        if (g_is_text) {
            ...
            ef_b3(0x48, 0x89, 0xE5);   /* mov rbp,rsp */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          { extern const char * bb_src_of(const IR_t *); if (i == 0 || bb_src_of(nodes[i])) g_emit.op_stmt_pin = 0; }   /* ⭐ HEAD-PIN lifetime (s22z, ONE AUTHORITY): the pin dies at the NEXT STATEMENT HEAD, cleared here in the drive walk -- NOT at uclaim staging, which was the first spelling and SEGV'd every armed program: unclaimed statements (a bare `NO OUTPUT = ...` assign) stage no uclaim, so the stale pin rode into THEIR terminal cuts, whose restore then loaded [old_rbp+88] garbage into rbp and fed it to the chain-γ whack.  bb_src_of heads are exactly the statement roots the whole zd/uclaim machinery is already segmented by (ZD-1 "segmented into STATEMENTS at bb_src_of heads"); the match statement's own run head clears a not-yet-set flag (harmless), bb_match_begin sets it nodes later, every subsequent node of the SAME statement reads it, and the next statement's head retires it. */
          ...
          { static int _uw = -1; if (_uw < 0) { const char *_e = getenv("SCRIP_UNWIND"); _uw = (_e && *_e == '0') ? 0 : 1; }
            int _h3 = (zd_uh[i] >= 0 && zd_uh[i] < n) ? zd_uh[i] : i;
            if (_uw && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_BEGIN && nodes[i]->op != IR_STATEMENT_END && zd_uk[_h3] == 0 && zd_on[i] && zd_wp[i] > 0 && g_emit.op_wpop == 0 && !omega_is_retry && !omega_is_phi && !scan_live && !g_emit.flat_stmt_frame) {
          ...
          { static int _ej = -1; if (_ej < 0) { const char *_e = getenv("SCRIP_ZD_ENDJMP"); _ej = (_e && *_e == '0') ? 0 : 1; }
            ...
            if (_ej && !_uw_stolen && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_BEGIN && nodes[i]->op != IR_STATEMENT_END && zd_uk[_h2] == 0 && zd_on[i] && zd_wp[i] > 0 && g_emit.op_wpop == 0 && !omega_is_retry && !omega_is_phi && !scan_live && !g_emit.flat_stmt_frame) {
          ...
          { int _h = (zd_uh[i] >= 0 && zd_uh[i] < n) ? zd_uh[i] : i;
          if (_zw5_on && !_endj_stolen && !_uw_stolen && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_END && zd_uk[_h] == 0 && zd_on[i] && zd_wp[i] > 0 && !omega_is_retry && !omega_is_phi) { for (int _si = 0; _si < zw5_pool_stmts; _si++) { int _si_base = 0; for (int _sj = 0; _sj < _si; _sj++) _si_base += zw5_stmt_cnt[_sj]; if (zw5_stmt_idx[_si] > i) { for (int _d = 0; _d < zw5_stmt_cnt[_si]; _d++) { if (zw5_stmt_depths[_si][_d] == zd_wp[i]) { _zw5_saved_omega = node_ω; node_ω = &zw5_pool[_si_base + _d]; _zw5_wpop_stolen = zd_wp[i]; break; } } break; } } } }
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0 || zd_zw[i] || zd_zwr[i]) { g_zd_stage = 1; g_zd_arm = (zd_on[i] && !(g_emit_cfg && g_emit_cfg->pl_cells_graph && !g_emit.flat_all_zd)) ? 1 : 0;   /* PL-ZK-3 ARM SUPPRESS: when pl_cells_graph=1 and flat_all_zd=0, the graph has mixed armed+declined nodes -- the carve is suppressed (no sub rsp,K_total at prologue) so ZOPQ([rsp+N]) reads from armed-node templates would address wrong memory. Force g_zd_arm=0 for ALL nodes in this graph so every node falls through to the legacy FRQ/UCLAIM path byte-identically. flat_all_zd=1 => pure cells arm (carve present, ZOPQ correct); flat_all_zd=0 => pure UCLAIM arm (carve absent, FRQ correct). Gate: icn_cells_graph graphs never take this path (mutually exclusive, R-ZK-A); SN4/Raku/Pascal have pl_cells_graph=0; byte-identical by construction for all three. ONE AUTHORITY: this is the only site that sets g_zd_arm; the carve suppressor at the prologue and this suppressor are the SAME predicate (pl_cells_graph && !flat_all_zd) applied at two points. */ int _pl_suppress = (g_emit_cfg && g_emit_cfg->pl_cells_graph && !g_emit.flat_all_zd) ? 1 : 0; g_zd_gpop = _pl_suppress ? 0 : zd_gp[i]; g_zd_wpop = _pl_suppress ? 0 : (_uw_stolen ? (int)_uw_pop : ((_zw5_wpop_stolen || _endj_stolen) ? 0 : zd_wp[i]));   /* PL-ZK-3 RELEASE SUPPRESS: same predicate as the arm suppress above; when mixed-armed/declined (flat_all_zd=0), no cells carve was emitted so gpop/wpop releases must also be zeroed to prevent unpaired add-rsp corruption. */ g_zd_wsteal = _endj_stolen || _uw_stolen; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i]; g_zd_uzw = zd_zw[i]; g_zd_uzwr = zd_zwr[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; mechanism-2 g_zd_uzwr staged alongside g_zd_uzw */
              ...
              if (zd_on[i]) { int _no = zd_nops(nodes[i]);
                  ...
                  IR_t * _p = nodes[i]->operands[_zj]; for (int _k = 0; _k < n; _k++) if (nodes[_k] == _p) { int _xh_zdh = (zd_uh[i] >= 0 && zd_uh[i] < n) ? (int)zd_ud[zd_uh[i]] : -1; int _xh = (zd_ud[i] > 0 && zd_ud[_k] == 0 && _xh_zdh > 0 && (zd_out[i] - _xh_zdh + zd_k(nodes[i])) > 0) ? 1 : 0; g_zd_read[_zj] = _xh ? ((int)zd_ud[i] - zd_k(nodes[i]) + _xh_zdh - zd_out[_k]) : (zd_out[i] - zd_out[_k]);   /* ⭐ ZD-5b-LEN cross-head correction: when producer is pre-head (zd_ud[_k]==0, no Kc) and consumer is post-head (zd_ud[i]>0, has Kc), the raw zout difference misses the PATCTX sub_rsp,Kc that MATCH_BEGIN emits between them.  Correct offset = zd_ud[consumer] - K(consumer) + zdh - zd_out[producer], where zdh=zd_ud[hi] was stored by zd_plan at the run-head slot (always 0 for STATEMENT_BEGIN before this fix).  Byte-identical when no cross-head read (_xh=0): same formula reduces to the existing zd_out[i]-zd_out[_k].  Gate: consumer must be post-head (zd_ud[i]>0 i.e. in an armed match statement with Kc>0) and producer must be pre-head (zd_ud[_k]==0).  MEASURED root cause: LEN(*N)/LEN(N) with N pre-constructed; ZOPQ(0,8) read [rsp+104] instead of [rsp+360] (off by Kc=256); N was an integer but the slot was inside MATCH_BEGIN's zeroed PATCTX so rcx=0, LEN matched 0 chars, conditional assignment captured empty string (D07 probe). */ g_zd_kind[_zj] = (int)ir_norm_call_kind(_p->op); if ((int)nodes[i]->op == (int)IR_TO) fprintf(stderr, "[ZD_TO_STAGE] i=%d _zj=%d _k=%d zd_out[i]=%d zd_out[k]=%d g_zd_read=%d\n", i, _zj, _k, zd_out[i], zd_out[_k], g_zd_read[_zj]); break; } } } }   /* ZD-1 staging (R5b driver-handoff): op_zread[k] = the DIFFERENCE OF TWO DEPTHS (s21x-u), zout[consumer] - zout[producer], both from the ONE walk.  ⛔ ZD-2d/2e FIX: this site RE-DERIVED the operand count inline (UNOP/ASSIGN?1:BINOP?2:0) instead of calling zd_nops, so the planner and the driver held TWO OPINIONS of how many operands a kind has -- the moment zd_nops grew COERCE_NUMERIC/CMP_TEST the planner ARMED their statements while this loop still computed _no=0, left g_zd_read all zero, and every ZOPQ/ZOPD rendered [rsp+0] = the box's OWN cell.  29 programs, every one silently wrong rather than crashing (rt_cmp_d comparing an uninitialized cell against itself).  ONE authority now; a kind added to zd_nops is staged here automatically.  K is no longer a second copy: s22k folded it into zd_k beside zd_nops, so BOTH one-authority laws now hold on this staging path. */
...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    { static int _stf = -1; if (_stf < 0) { const char * e = getenv("SCRIP_STMT_FRAME"); _stf = (e && *e == '1') ? 1 : 0; }   /* ⛔⭐⭐ STF-UNFLIP (s22b, LON DIRECTIVE, and it is a MODEL correction not a tuning knob): DEFAULT FLIPPED BACK ON -> OFF; `SCRIP_STMT_FRAME=1` is now the OPT-IN and the regime ships DORMANT.  THE MODEL, restated in Lon's words because two sessions in a row encoded the opposite: NO BB EVER CONSUMES ANYTHING, EVER.  A BB allocates EXACTLY at alpha -- one `sub rsp, K`, its OWN cell -- never at beta (ARBNO-class excepted); it frees at omega; and the whole accumulation is freed AT THE END OF THE STATEMENT.  Cells stack up through the statement and ONE statement-terminal release drops all of them.  That release is op_zgpop, and it is the SOLE release authority.  ⛔ THE ERROR s21x-y AND s22a ENCODED: the per-statement rbp dance (bb_glue_framed_enter/leave) was made to stand in for that release, and s22a's ZGPOP-STF then ZEROED op_zgpop on armed graphs to stop the two colliding -- i.e. the bracket did not SUPPLEMENT the statement-terminal free, it REPLACED it, and a C-style frame pointer became per-statement ceremony on the value spine.  Law 4's "C-style RBP used occasionally" licenses rbp for CONSTRUCT housekeeping that must survive an unwind (ARBNO/FENCE1/CALL) -- it never licensed one bracket per statement.  MEASURED AT THIS HEAD, not inherited: crosscheck 318 m3 232/85 - m4 229/86/2 - DIV=1 {W04_arbno_basic}, IDENTICAL to the regime-on run, and the fail set is IDENTICAL BY SET in BOTH modes (diffed programmatically, not by count -- the test_string flake makes counts lie).  ⚠ NOT VACUOUS, and this is the instrument that proves it: an rbp-INSTRUCTION census over all 318 crosscheck programs moves 15709 -> 15459 (250 instructions gone) and the count of programs containing ANY rbp moves 158 -> 127, i.e. EXACTLY 31 PROGRAMS GO RBP-FREE -- the same 31 the SCRIP_STF_DEBUG=1 armed census names.  Witness `arithmetic.sno`: 26 rbp -> 0, statement `OUTPUT = A + B` now three alpha carves (`sub rsp,16` x3) and one terminal `add rsp,48`.  ⭐ THE REMAINING RBP DEBT IS A DIFFERENT CITIZEN AND IS THE NEXT RUNG: 15459 instructions across 127 programs is the flat_pat/flat_gen WHOLE-GRAPH PIN, not a statement bracket -- do not confuse the two (the s22a CENSUS-CORRECTED instrument law).  HISTORICAL, kept because the next session will otherwise re-derive it: s21x-y's flip was licensed by a watermark-neutrality argument that was TRUE and INSUFFICIENT -- neutrality proved the bracket broke nothing, never that it belonged. */   /* ⭐ STF-FLIP (s21x-y), SUPERSEDED BY THE ABOVE, retained for provenance: DEFAULT FLIPPED OFF -> ON, and `SCRIP_STMT_FRAME=0` is now the killswitch.  This is the moment the "C-style RBP used occasionally" of the design of record stops being dormant scaffolding and becomes the shipped regime: under it a STATEMENT is bracketed by the rbp dance (bb_glue_framed_enter/leave) and bb_match_begin's five POST-UNWIND-lifetime fields move from the whole-graph flat slots FRQ(op_off+40..72) onto HKQ = [rbp-48+8k], which is DEPTH-FREE BY CONSTRUCTION -- the value spine keeps riding RSP FORTH-style while exactly the housekeeping that must survive an unwind rides a stable base.  LICENSED BY THE CONDITION THIS LINE ITSELF NAMED ("default OFF until the watermark license ... fail-set diffed programmatically"), now satisfied and MEASURED AT THIS HEAD, not inherited: crosscheck 317 m3 232/85 - m4 229/86/2 - DIV=1 and broad 336 m3 244/92 - m4 238/90/8, and in BOTH corpora BOTH modes the fail set is BYTE-IDENTICAL to the regime-off run.  ⚠ NOT VACUOUS -- the trap that makes an identical watermark meaningless: the regime ARMS 31 GRAPHS IN 31 PROGRAMS (SCRIP_STF_DEBUG=1, live=1 census), so the neutrality is 31 armed customers agreeing, not a classifier declining everything.  s21x-r's 41-program regression is NOT being re-litigated here: it was ZLEAK-1, a process-scope `static int` from getenv applied to all 316 programs while emit armed 31, cured at SCRIP `efb72d69`; the armed set and that regression set were measured DISJOINT then, and this re-measurement at a later HEAD confirms the cure held. */
      ...
      int _pat_ok = (!g_emit.flat_pat || zw_rb_on()) ? 1 : 0;   /* ⭐ MECHANISM-2 STF EXTENSION (SCRIP_ZW_RB): when mechanism-2 is enabled, flat_pat=1 graphs are admitted.  STF fires push rbp at statement head (pre-all-cells); mechanism-2's push rbp in MATCH_BEGIN nests inside it (nested indeterminacy = nested frames, HQ ruling 2026-08-03).  Default OFF (zw_rb_on()=0) preserves !flat_pat exclusion byte-identical. */
