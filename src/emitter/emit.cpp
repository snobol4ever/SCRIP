...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    else if (strncmp(prefix, "proc_", 5) == 0) emit_label_initf(&lbl_α, "FN__%s", prefix + 5);   /* s112 (Lon in-chat): the DEFINE wrapper face is FN__<FN>, unexported — proc_<FN>_α removed (supersedes the s62 keep); m3's proc_flat/pat_flat diagnostic prefixes ride the same arm, inert (binary label pool, never looked up by these names) */
