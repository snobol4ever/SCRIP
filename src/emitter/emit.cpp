...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    if (nd->op == IR_GOTO_DEFERRED && nd->seal == 1) g_emit.lbl_t0 = (nd->n_operands >= 1 && nd->operands[0] && nd->operands[0]->op == IR_LIT_NAME) ? IR_LIT(nd->operands[0]).sval : (const char *)0;
   ...
   { int _du=0; int _ds0=-1,_ds1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_ds0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_ds1=_j;} if(_ds0>_ds1){int _t=_ds0;_ds0=_ds1;_ds1=_t;} int _alt=0,_ref=0,_dfr=0,_arb=0,_altnest=0; if(_ds0>=0) for(int _j=_ds0;_j<=_ds1;_j++){IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m!=nd&&(int)_m->op==IR_MATCH_DEFER&&!_m->pat_static)_du=1; if(!_m||_m==nd)continue; int _o=(int)_m->op; if(_o==IR_MATCH_ALTERNATE||_o==IR_DISJUNCTION){ if(_m->seal)_ref=1; else { _alt++; for(int _q=0;_q<_m->n_operands;_q++){IR_t*_a=_m->operands[_q]; if(_a&&((int)_a->op==IR_MATCH_ALTERNATE||(int)_a->op==IR_DISJUNCTION))_altnest=1;} } continue;} if(_o==IR_MATCH_DEFER){ extern int sn4_alt_carrier(void); if(sn4_alt_carrier()&&_m->seal!=1)_dfr++; else _ref=1; continue;} if(_o==IR_MATCH_ARBNO){ extern int sn4_arbno_tailbeta(void); if(sn4_arbno_tailbeta()&&_m->γ.node==nd)_arb++; else _ref=1; continue;}       switch(_o){case IR_MATCH_LIT:case IR_MATCH_LEN:case IR_MATCH_ANY:case IR_MATCH_NOTANY:case IR_MATCH_POS:case IR_MATCH_RPOS:case IR_MATCH_TAB:case IR_MATCH_RTAB:case IR_MATCH_SPAN:case IR_MATCH_BREAK:case IR_MATCH_REM:break;default:_ref=1;break;} } int _fseal=(nd->n_operands>3&&nd->operands[3]==nd)?1:0; g_emit.op_tail_seal=_fseal;    g_emit.op_arbno_body_defer_unsafe=_du||arbno_reentry_hazard(nd); { extern int sn4_arbno_altsib(void); int _altok=sn4_arbno_altsib()?(_alt>=1&&!_altnest):(_alt==1); g_emit.op_arbno_body_actframe=(((_altok&&_dfr==0&&_arb==0)||(_alt==0&&_dfr==1&&_arb==0)||(_alt==0&&_dfr==0&&_arb==1))&&!_ref&&!_fseal)?0:1; } }                   } else { g_emit.op_sa = -1; g_emit.op_sb = -1; g_emit.op_arbno_chain = 0; } }
    ...
    case IR_DEFINE: {
        if (ir_define_sr_citizen(nd)) { extern int fc_call_active(const IR_t *); if (g_emit.op_ival == 4 && nd->n_operands >= 2 && nd->operands[0] && nd->operands[1]) { g_emit.op_sval = IR_LIT(nd->operands[0]).sval; g_emit.lbl_t0 = IR_LIT(nd->operands[1]).sval; g_emit.op_seal = (nd->n_operands >= 3 && nd->operands[2] && nd->operands[2]->op == IR_LIT_NAME) ? 1 : 0; }    if (g_emit.op_ival == 3 && nd->γ.node && sr3_gamma_label(nd->γ.node)) { const char *_e = sr3_gamma_label(nd->γ.node); g_emit.op_sval = strncmp(_e, "LBL__", 5) ? _e : _e + 5; }    if (g_emit.op_ival == 0 && nd->γ.node && fc_call_active(nd->γ.node) && nd->n_operands == 1 && nd->operands[0]) { extern int zls_off(const IR_t *); int _as = bb_slot_get(nd->operands[0]); if (_as < 0) _as = zls_off(nd->operands[0]); if (_as >= 0) { g_emit.op_fc_wbytes = 16; g_emit.op_fc_base = _as; } } g_emit.op_define_role = (int)g_emit.op_ival; extern std::string bb_define(); bb_emit_x86(bb_define()); return 0; }
        if (ir_define_is_bind(nd)) { g_emit.op_sval = IR_LIT(nd).sval;    g_emit.lbl_t0 = (const char *)0; int _realstub = 0; if (g_emit_cfg && g_emit_cfg->n_dentry > 0) for (int _dq = 0; _dq < g_emit_cfg->n_dentry; _dq++) if (g_emit_cfg->dentry_node[_dq] == nd) { g_emit.lbl_t0 = g_emit_cfg->dentry_name[_dq]; _realstub = g_emit_cfg->dentry_entry[_dq] ? 1 : 0; break; }    { int _bc = 0; if (g_emit_cfg && g_emit.op_sval) for (int _bi = 0; _bi < g_emit_cfg->n; _bi++) { const IR_t *_bn = g_emit_cfg->all[_bi]; if (ir_define_is_bind(_bn) && IR_LIT(_bn).sval && !strcmp(IR_LIT(_bn).sval, g_emit.op_sval)) _bc++; } g_emit.op_seal = (_bc > 1) ? 1 : 0; }    int _m3fb = 0; if (!g_emit.lbl_t0 && !g_is_text && g_emit.op_seal) { const char *_be = ir_define_bind_entry(nd); if (_be) { char _bl[300]; snprintf(_bl, sizeof _bl, "LBL__%s", _be); g_emit.lbl_t0 = strdup(_bl); _m3fb = 1; } }    g_emit.op_define_role = 6; extern std::string bb_define(); bb_emit_x86(bb_define()); if (_m3fb) g_emit.lbl_t0 = (const char *)0; int _d1st = 1; if (g_emit_cfg && g_emit.op_sval) for (int _dr = 0; _dr < g_emit_cfg->n_dentry; _dr++) { IR_t *_dp = g_emit_cfg->dentry_node[_dr]; if (_dp == nd) break; if (_dp && ir_define_is_bind(_dp) && IR_LIT(_dp).sval && !strcmp(IR_LIT(_dp).sval, g_emit.op_sval)) { _d1st = 0; break; } }    if (g_is_text && g_emit.lbl_t0 && _d1st && _realstub) { long _sv5 = g_emit.op_ival; g_emit.op_ival = 5; g_emit.op_define_role = 5; bb_emit_x86(bb_define()); g_emit.op_ival = _sv5; }    return 0; }
...
extern "C" int fc_seq_active(const IR_t *);
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int fc_seq_on(const IR_t *nd) { return nd && fc_seq_active(nd); }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
void emit_drive(IR_t *nd, bb_label_t *lbl_α, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_MATCH_BEGIN: {
        ...
        { extern int fc_vread_fp(const IR_t *); g_emit.op_subj_cell = (subj && fc_vread_fp(nd) >= 0) ? 1 : 0; }
    ...
    case IR_GOTO_DEFERRED: {
        g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_ival = nd->seal; DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); if (nd->seal == 1) g_emit.lbl_t0 = (nd->n_operands >= 1 && nd->operands[0] && nd->operands[0]->op == IR_LIT_NAME) ? IR_LIT(nd->operands[0]).sval : (const char *)0; break;
    }
...
static int zd_k(IR_t * nd);
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int choice_body_member(const IR_t * q) {
...
static int xop_frame_member(const IR_t * nd) {
    ...
    for (int j = 0; j < g_emit_cfg->n; j++) { const IR_t * c = g_emit_cfg->all[j]; if (!c || c == nd) continue;
        ...
        if (!reached && choice_body_member(c)) return 1; }
    return 0;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int blob_frame_scope(void);
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, int *zarm) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = (vd_tidx >= 0) ? zd_omega_seed(nodes, n, nodes[hi], zon, zout) : 0; int arm_zd = 0; int zdh_match = -1; int zdh_stmt = -1;
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (_zvd) { zgt[i] = gt; zot[i] = ot; zgin[i] = (unsigned char)gin; zoin[i] = (unsigned char)oin; zmatch[i] = zdh_match; zstmt[i] = zdh_stmt; }
                                { long kc = 0;
                ...
                {
                if (!gin) zgpop[i] = (gback >= 0) ? (_wzdepth - _gbpre) : (zd_exit_pop_s(nodes[i]->op, zdh_match, _wzdepth, zdh_stmt) + (int)kc);
                if (!oin) zwpop[i] = (oback >= 0) ? ((_wzdepth - K) - _obpre) : (((nodes[i]->op == IR_GOTO_DEFERRED) ? zd_exit_pop_s(nodes[i]->op, zdh_match, _wzdepth, zdh_stmt) : (_wzdepth - K)) + (int)kc); } }
    ...
    if (_zvd) { for (int i = 0; i < n; i++) { if (!zon[i] || !(zvd_ok[i] || _zn)) continue; int K = zd_k(nodes[i]); int gback = -1, oback = -1;
        ...
        if (!zoin[i]) zwpop[i] = (oback >= 0) ? ((zout[i] - K) - _obpre) : ((nodes[i]->op == IR_GOTO_DEFERRED) ? zd_exit_pop_s(nodes[i]->op, zmatch[i], zout[i], zstmt[i]) : (zout[i] - K));
