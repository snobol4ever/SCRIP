...
int walk_bb_node(IR_t * nd, FILE * out) {
    ...
    g_emit.op_sval = (nd->op == IR_VAR || nd->op == IR_VAR_REF || nd->op == IR_ASSIGN || nd->op == IR_LIT_STRING || nd->op == IR_LIT_CHARSET || nd->op == IR_KEYWORD || nd->op == IR_FIELD_GET || nd->op == IR_FIELD_SET || nd->op == IR_SUBSCRIPT || nd->op == IR_PROC_GEN || ir_norm_call_kind(nd->op) == IR_CALL) ? IR_LIT(nd).sval : (const char *)0;
...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    while (qh < qt) {
        ...
        if ((c->op == IR_SUBSCRIPT || c->op == IR_DEREF || c->op == IR_ASSIGN_VAR || c->op == IR_REV_ASSIGN_VAR) && c->ω.node && qt < CH_MAX) queue[qt++] = c->ω.node;   /* IDX-UNIFY: failure edges (je ω) must reach the walk — else/fail blocks were unemitted (rung16_subscript_sub_fail) */
...
static void descr_chain_operand_refs(IR_t *entry) {
    ...
    while (sv > 0 && nc < 512) {
        ...
        if ((c->op == IR_SUBSCRIPT || c->op == IR_DEREF || c->op == IR_ASSIGN_VAR || c->op == IR_REV_ASSIGN_VAR) && c->ω.node && sv < 512) stkv[sv++] = c->ω.node;
