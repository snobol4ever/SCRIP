...
static int zd_wl_kind(IR_t * nd) {
    ...
    /* ⭐ ZD-5b BLOB-INTERIOR ADMISSION (A-7, s24a): leaf kinds (LIT/LEN/ANY/NOTANY/SPAN/TAB/RTAB/POS/RPOS/REM/ARB/BAL) + ALTERNATE/SEQUENCE envelopes + ASSIGN_SAVE/COND/IMM admitted here KIND BY KIND as OMEGA delivers their template ZD arms.  The planner walk extension (SCRIP_ZD_5B, phase 2 above) is already live and correctly enqueues arm-interior nodes; but zd_wl_kind is the CAPABILITY REGISTRY -- a kind must NOT appear here until its template has a ZD arm reading ZOPQ(k) for its cell operands.  MEASURED s24a: admitting leaf kinds before their template arms causes K=16 cell carves with templates reading scanner registers instead of cells -- double-release, wrong match results, segfaults.  Cross-front request to OMEGA filed s24a: add ZD arm to each bb_match_*.cpp in the order below, then ALPHA adds the kind here and runs byte-identity sweep.  ARBNO stays out (INDETERMINABLE class, body depth non-static, OMEGA's RBP terrain).  No kinds admitted yet -- this comment is the placeholder. */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh, int *zzw) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            int has_blob = (nblob > 0); if (!has_blob) { for (int _r = 0; _r < rl; _r++) { int _ro = (int)nodes[run[_r]]->op; if (_ro == IR_MATCH_LIT || _ro == IR_MATCH_LEN || _ro == IR_MATCH_ANY || _ro == IR_MATCH_NOTANY || _ro == IR_MATCH_SPAN || _ro == IR_MATCH_TAB || _ro == IR_MATCH_RTAB || _ro == IR_MATCH_POS || _ro == IR_MATCH_RPOS || _ro == IR_MATCH_REM || _ro == IR_MATCH_ARB || _ro == IR_MATCH_BAL || _ro == IR_MATCH_ALTERNATE || _ro == IR_MATCH_SEQUENCE || _ro == IR_MATCH_ASSIGN_SAVE || _ro == IR_MATCH_ASSIGN_COND || _ro == IR_MATCH_ASSIGN_IMM) { has_blob = 1; break; } } }   /* ⭐ ZD-5b (A-7, s24a): when ZD-5b descends blob-interior kinds into run[], nblob drops to 0 (those nodes are no longer off-run operand-closure members), but the shift-fragile raw-claim-spelling hazard (s23i CAP-SLOT) still applies -- the canonical ZW frame cannot coexist with blob-interior members IN OR OUT of the run.  has_blob captures both cases; the !has_blob gate on zws stays correct under both the old linear walk and the new subtree descent. */
