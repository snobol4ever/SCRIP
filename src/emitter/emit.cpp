...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_zres = 0; g_emit.op_zgpop = 0; g_emit.op_wsteal = 0; g_emit.op_ztail = 0; g_emit.op_zpat = 0; g_emit.op_zfc = 0; g_emit.op_xf_off = -1; for (int _zi = 0; _zi < 6; _zi++) { g_emit.op_zread[_zi] = 0; g_emit.op_zkind[_zi] = -1; g_emit.op_zread_xf[_zi] = -1; }   /* ZD-1: cleared at the choke UNCONDITIONALLY so glue and non-drive fills can never inherit a stale regime.  op_ztail (ZK-2): 0 for all non-IR_TO nodes; staged for IR_TO in codegen_flat_chain_body before the staging globals are consumed here.  op_zfc (L-3b, this session): 0 for all non-IR_MATCH_REPLACE nodes, mirrors op_zpat's clear. */
