...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    { static int _ba = -1; static const char * _bo; static const char * _bs; if (_ba < 0) { const char * e = getenv("SCRIP_BB_ALLOC"); _ba = (e && *e == '1') ? 1 : 0; _bo = getenv("SCRIP_BB_ONLY"); _bs = getenv("SCRIP_BB_SKIP"); }
      int _spine = (nd->op == IR_BINOP || nd->op == IR_ASSIGN || nd->op == IR_LIT_INTEGER || nd->op == IR_LIT_STRING || nd->op == IR_LIT_REAL || nd->op == IR_LIT_CHARSET || nd->op == IR_VAR || nd->op == IR_CMP_TEST || nd->op == IR_COERCE_NUMERIC);   /* ZW-1 v0.1 (023 witness, armed .s read): the ZB-VAL TOS value-spine family is ALREADY the cell machine -- producers push their own 16B DESCR cells, consumers read [rsp+0..31] and net the result IN PLACE (RESULT-IS-THE-CELL).  A zw carve-only cell inserted mid-spine displaces every TOS read by K (OUTPUT = 1+2 emitted 1).  These kinds are living the design; zw arms the UNCONVERTED population around them. */
...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    int st_first_seen = 0;
    bb_label_t **st_x   = (bb_label_t **)alloca(sizeof(bb_label_t *) * n);   /* STMT-FRAME: the head's LEAVE half -- a statement-crossing edge must cut the SOURCE bracket before the dest's enter (the seed's stmt_*_end/_fail law: leave THEN jmp), so st_x[k] emits x86_stmt_leave and FALLS THROUGH into st_pre[k].  NULL for the first-emitted head: its only entrant is the prologue fallthrough, which has no bracket to cut (and cutting there would load the CRT caller's rbp into rsp -- the 058 disease). */
    ...
    for (int i = 0; i < n; i++) {
        if (g_emit.flat_stmt_frame && st_pre[i]) { g_emit.xa_bb_emit_pair_n = 2; g_emit.xa_bb_emit_pair_define[0] = st_x[i]; g_emit.xa_bb_emit_pair_jmp[0] = NULL; g_emit.xa_bb_emit_pair_define[1] = st_pre[i]; g_emit.xa_bb_emit_pair_jmp[1] = NULL; bb_emit_x86((st_x[i] ? x86_deflabel_pair(0) + x86_stmt_leave() : std::string()) + x86_deflabel_pair(1) + x86_stmt_enter()); }   /* STMT-FRAME head stub: [st_x: cut the SOURCE bracket (mov rsp,rbp; pop rbp) --] st_pre: rbp dance forward, fall through into the node's own α.  Cross-statement edges land st_x (leave-then-enter, the seed's stmt_*_end/_fail law); the prologue and nothing else falls into the first head's bare st_pre.  Both media via the pair 'E' records + the s21x-c encoders. */
    ...
    if (g_emit.flat_stmt_frame) { emit_sep_rule('-'); emit_label_define_bb(&lbl_stcγ); bb_emit_x86(x86_stmt_leave()); emit_jmp_label(&lbl_γ, JMP_JMP); emit_label_define_bb(&lbl_stcω); bb_emit_x86(x86_stmt_leave()); emit_jmp_label(&lbl_ω, JMP_JMP); }   /* STMT-FRAME chain-exit cuts: leave (mov rsp,rbp; pop rbp) then jmp the port -- every retargeted γ/ω edge lands here; the cut normalizes rsp from ANY statement depth, which is what licenses leaving every template's inline pop untouched */
