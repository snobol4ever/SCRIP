...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead; g_emit.op_zw = g_zd_uzw; g_emit.op_zw2 = g_zd_uzwr;   /* UCLAIM applied regardless of g_zd_arm; op_zw2 mechanism-2 flag applied beside op_zw */
        ...
        if (g_zd_arm) { g_emit.op_zres = 1; g_emit.op_fc_bytes = g_zd_k; g_emit.op_fc_base = -1; g_emit.op_zdepth = g_zd_k; for (int _zi = 0; _zi < 6; _zi++) { g_emit.op_zread[_zi] = g_zd_read[_zi]; g_emit.op_zkind[_zi] = g_zd_kind[_zi]; } }
    ...
    case IR_MATCH_BEGIN:           { extern int fc_tail_head(const IR_t *); int _th = (x86_zc_frame() == ZC_FRAME_RSP && fc_tail_head(nd)); if (fc_head_fp(nd) >= 0 || _th) { g_emit.op_fc_wbytes = 24; g_emit.op_fc_base = g_emit.op_off; } if (_th) g_emit.op_tail = 1; if (g_emit.op_zw2) { g_emit.op_mech2_kc = g_emit.op_udout; g_emit.op_zgpop = 0; g_emit.op_wpop = 0; }   /* ⭐ ZW-RB-1: set op_mech2_kc from op_udout (=Kc); zero zpop -- mech2_whack handles cleanup */ bb_emit_x86(bb_match_begin()); } return 0;                  /* SN4-PAT-2 + ZB-FC-3d self-cell WINDOW + R12-EXIT-1 tail statements self-push too (bracket source) */
