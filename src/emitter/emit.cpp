...
static int zd_wl_kind(IR_t * nd) {
    ...
    if (op == IR_MATCH_LEN) return 1;   /* ZD-5b (s25): bb_match_len arm.  K=0 (zd_k), nops=nd->n_operands (0 for static LEN(literal), 1 for dynamic LEN(var) — the COERCE_INTEGER predecessor), scanner-register-only body; dynamic arm reads ZOPQ(0,8) instead of FRQ(op_sa+8). */
