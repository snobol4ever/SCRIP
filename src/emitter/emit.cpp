...
static int g_zd_omega_depths[8]; static int g_zd_n_omega_depths;   /* ZW-5 SLICE 3 (OMEGA O-2): staged by zd_plan for IR_STATEMENT nodes; applied at the choke into g_emit.op_omega_depths/op_n_omega_depths; cleared with the stage flag */
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_uclaim = 0; g_emit.op_udout = 0; g_emit.op_uhead = -1; g_emit.op_zw = 0; g_emit.op_n_omega_depths = 0; for (int _zw = 0; _zw < 8; _zw++) g_emit.op_omega_depths[_zw] = 0;   /* ZW-5 SLICE 3 (OMEGA O-2): cleared unconditionally at the choke so glue/non-drive fills cannot inherit a stale depth set */   /* UCLAIM: cleared at the choke UNCONDITIONALLY, same law as the ZD set above */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh, int *zzw) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        g_zd_n_omega_depths = 0; for (int _zw = 0; _zw < 8; _zw++) g_zd_omega_depths[_zw] = 0;   /* ZW-5 SLICE 3 (OMEGA O-2): clear at run start so non-statement runs carry zero; re-collected inside the ok=1 path */
        ...
        int zdyn = 0; { static int _da = -2; if (_da == -2) { const char * e = getenv("SCRIP_ZD_DYNARM"); _da = e ? atoi(e) : -1; }   /* ⭐ ZD-5 PATREF/DEFER STATIC-SHAPE ARMING (s23i, the s23h-named next rung): a DEFER/PATREF whose pat_static=1 (lower's transitive defer-free proof over the seal table) no longer vetoes the quartet -- the stored pattern provably enters, scans, and returns with no blob re-entry, so the armed claim geometry is never crossed at an unmodeled depth.  The recursive class (135/136's B = '(' FENCE(*B|eps) ')') stamps 0 and keeps the declined regime VERBATIM, as do composite `*expr` sites and every non-eligible name -- degrade never die.  FENCE1 in the STATEMENT graph still declines unconditionally: its wire-glue geometry predates cells-above-claim and has no per-target proof to ride (142's lone in-statement green is one witness, not a bracket).  SCRIP_ZD_DYNARM is the MEASUREMENT MASK, unset = classifier: bit0 forces DEFER armed, bit1 PATREF, bit2 FENCE1 (=7 reproduces the s23h full-arm flip set {135,136,164,165,183}+{117}); =0 forces all three declined = the s23h HEAD regime byte-identical, the escape hatch every narrowing keeps. */
          ...
          for (int k = 0; k < n && !zdyn; k++) { int o = (int)nodes[k]->op; if (_ds && !cm[k]) continue;   /* ⛔⭐ ZD-4 (s23q): THE VETO WAS FUNCTION-SCOPED AND THAT IS THE DEFECT.  `for k < n` scans EVERY node in the graph, so ONE dynamic box anywhere declines EVERY match statement in that graph -- the exact shape of the whole-graph jmp-entry veto that ZD-NOGRAPH deleted in s23a under Lon's s21x-w law ("NO FUNCTION-level processing for ZETA whatsoever.  ONLY statement level scoping"), reintroduced by kind instead of by entry.  MEASURED s23q on roman.sno: proc_LBL__ROMAN armed=0/30 with its per-node verdicts ALL admit=1 and its per-run reasons naming IR_MATCH_BEGIN -- i.e. the quartet veto at the member test below, not a capability gap; SCRIP_ZD_DYNARM=2 (PATREF force-arm) alone lifts it to armed=10, so the whole graph was being declined by ONE PATREF (`T` in spine position, statement 2's `'0,1I,..' ? T BREAK(',') . T`) sitting in a DIFFERENT statement.  cm[] is the run's own closure, so a dynamic box now vetoes the statement that CONTAINS it and no other; the hazard the veto guards (blob re-entry at an unmodeled claim depth, casualties {135,136,164,165,183}+{117}) is intra-statement by construction -- re-entry arrives through the bearing statement's own blob, never a sibling's.  Degrade never die: the bearing statement keeps its declined regime VERBATIM. */
            if (o == IR_MATCH_FENCE1) { if (!(_da >= 0 && (_da & 4))) zdyn = 1; }
        ...
        for (int r = 0; r < rl && ok; r++) { int i = run[r];
            { int _a = zd_wl_kind(nodes[i]); int _o = (int)nodes[i]->op; if (_a && zdyn && (_o == IR_MATCH_BEGIN || _o == IR_MATCH_SEQUENCE || _o == IR_MATCH_END || _o == IR_MATCH_REPLACE)) _a = 0;
        ...
        if (ok) { int zd = 0;
            ...
            if (nblob > 0) { for (int k = 0; k < n; k++) { if (!cm[k]) continue;
            ...
            int zws = 0; if (zw_frame_on() && Kc > 0 && hpos >= 0 && nblob == 0 && !g_emit.flat_stmt_frame) {   /* STF exclusion: flat_stmt_frame is its OWN statement rbp bracket (st_x leave stubs whack via mov rsp,rbp) -- the zw match frame inside it would nest two rbp disciplines; STF keeps the old regime */ int _end = 0, _seal = 0; for (int r = 0; r < rl; r++) { int _o = (int)nodes[run[r]]->op; if (_o == IR_MATCH_END) _end = 1; if (_o == IR_MATCH_ABORT || _o == IR_MATCH_FENCE1) _seal = 1; } zws = _end && !_seal; if (zws) { int epos = -1; for (int r = 0; r < rl; r++) if (nodes[run[r]]->op == IR_MATCH_END) { epos = r; break; }
            ...
            /* ZW-5 SLICE 3 (OMEGA O-2): collect distinct non-zero zwpop values over run members (excluding the IR_STATEMENT trailer whose own zpop IS the terminal release) into g_zd_omega_depths; keyed on IR_STATEMENT presence in the run so only statement runs get a depth set.  The STATEMENT node is always last (lower mints it as the success wire's target); members at r<rl-1 whose ω crosses the statement boundary carry zwpop>0 and need a stub at that depth.  Distinct: if two members fail at the same depth, one stub covers both.  Max 8 slots; overflow logged and truncated (design anomaly). */
            { int _stmtrun = (rl > 0 && (int)nodes[run[rl-1]]->op == (int)IR_STATEMENT) ? 1 : 0;
              g_zd_n_omega_depths = 0; for (int _zw = 0; _zw < 8; _zw++) g_zd_omega_depths[_zw] = 0;
              if (_stmtrun) { for (int _r = 0; _r < rl - 1; _r++) { int _wp = zwpop[run[_r]]; if (_wp <= 0) continue;
                  int _dup = 0; for (int _d = 0; _d < g_zd_n_omega_depths; _d++) if (g_zd_omega_depths[_d] == _wp) { _dup = 1; break; }
                  if (!_dup) { if (g_zd_n_omega_depths < 8) g_zd_omega_depths[g_zd_n_omega_depths++] = _wp;
                              else if (_dg) fprintf(stderr, "[ZD-OMEGA] WARN h=%d more than 8 distinct omega depths -- truncated\n", hi); } } }
              if (_dg && g_zd_n_omega_depths > 0) { fprintf(stderr, "[ZD-OMEGA] h=%d depths=%d:", hi, g_zd_n_omega_depths); for (int _d=0;_d<g_zd_n_omega_depths;_d++) fprintf(stderr," %d",g_zd_omega_depths[_d]); fprintf(stderr,"\n"); } } } }
            ...
            if (umax > umin) { long K = ((long)(umax - umin) + 15L) & ~15L;
                zuk[run[0]] = K; for (int k = 0; k < n; k++) if (mem[k]) { zud[k] = K; zuh[k] = hi; }
...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    /* ZW-5 SLICE 3 (OMEGA O-2): per-depth omega stub labels for IR_STATEMENT runs.  At most 8 depths per
     * statement.  Label names are n<uid>_statement_omega_d<K>; allocated here so the chain body's
     * forward-reference resolution can patch member jumps to the stub defined in bb_statement. */
    /* ZW-5 SLICE 3 (OMEGA O-2): stub label pool.  Each statement gets up to 8 stubs;
     * use a running base index so consecutive statements do not overwrite each other's labels.
     * 128 slots = 16 statements × 8 depths max per chain (generous; most chains have <10 stmts). */
    bb_label_t zw5_stub_lbls[128]; int zw5_pool_base = 0;
    int zw5_n_stubs = 0; int zw5_stub_K[8]; int zw5_stmtnode = -1;
...
#undef RPO_VISITED
    ...
    /* ZW-5 SLICE 3 (OMEGA O-2): stub labels allocated PER STATEMENT in the drive loop (see node_ω redirection block below) */
    zw5_stmtnode = -1; zw5_n_stubs = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0 || zd_zw[i]) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = zd_wp[i]; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i]; g_zd_uzw = zd_zw[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke */
          ...
          /* ZW-5 SLICE 3 (OMEGA O-2): at each run head (bb_src_of or node 0), look ahead to find
           * this run's IR_STATEMENT trailer (the last admitted node in the run) and pre-allocate
           * per-depth stubs BEFORE any member is driven, so node_ω redirection can fire correctly
           * for every member.  The look-ahead scans forward until it hits a node that is EITHER:
           * IR_STATEMENT with zd_on (the stmt trailer of this run), OR a new bb_src_of head (a
           * different statement -- stubs not needed for non-stmt runs). */
          { extern const char * bb_src_of(const IR_t *);
            /* Look ahead only when we are at a new run head AND past the previous statement's stmtnode.
             * This ensures stubs allocated at head i are still valid when we reach the stmtnode at i+k. */
            if ((i == 0 || bb_src_of(nodes[i])) && i > zw5_stmtnode) {
              zw5_stmtnode = -1; zw5_n_stubs = 0;
              for (int _f = i; _f < n; _f++) {
                  if (_f > i && bb_src_of(nodes[_f])) break;
                  if ((int)nodes[_f]->op == (int)IR_STATEMENT && zd_on[_f]) {
                      zw5_stmtnode = _f;
                      zw5_n_stubs = 0;
                      for (int _j = i; _j < _f; _j++) {
                          int _wp = zd_wp[_j]; if (_wp <= 0) continue;
                          int _dup = 0; for (int _d = 0; _d < zw5_n_stubs; _d++) if (zw5_stub_K[_d] == _wp) { _dup = 1; break; }
                          if (!_dup && zw5_n_stubs < 8 && zw5_pool_base + zw5_n_stubs < 128) {
                              zw5_stub_K[zw5_n_stubs] = _wp;
                              emit_label_initf(&zw5_stub_lbls[zw5_pool_base + zw5_n_stubs], "n%d_statement_omega_d%d", bb_node_id(nodes[_f]), _wp);
                              zw5_n_stubs++; } }
                      break; } } } }
          bb_label_t *_saved_node_ω = node_ω;
          /* ZW-5 SLICE 3 (OMEGA O-2): redirect node_ω ONLY for true cross-statement fail edges.
           * Guard !omega_is_beta: the BETA trampoline emits `x86_deflabel(BETA)+x86_jmp(OMEGA)`, so
           * a redirected node_ω would corrupt the beta resume label (n1_assign_β->stub instead of fT).
           * Guard !omega_is_phi: inside-fail sigma-phi edges are internal, not cross-statement.
           * The zd_wp>0 guard already limits to cross-boundary exits; the beta guard makes it safe. */
          if (zw5_n_stubs > 0 && i != zw5_stmtnode && zd_on[i] && zd_wp[i] > 0 && !omega_is_beta && !omega_is_phi) {
              for (int _d = 0; _d < zw5_n_stubs; _d++) if (zw5_stub_K[_d] == zd_wp[i]) { node_ω = &zw5_stub_lbls[zw5_pool_base + _d]; break; } }
          g_emit.op_n_omega_depths = zw5_n_stubs;
          for (int _d = 0; _d < zw5_n_stubs; _d++) g_emit.op_omega_depths[_d] = zw5_stub_K[_d];
          emit_drive(nodes[i], lbls[i], node_γ, node_ω, betas[i]);
          node_ω = _saved_node_ω;   /* restore after emit_drive so subsequent iterations use the real lbl_ω */
          /* ZW-5 SLICE 3 (OMEGA O-2): if this was the IR_STATEMENT trailer, emit the per-depth omega stubs
           * immediately after the template body.  Each stub: define label, add rsp K, jmp outer lbl_ω.
           * lbl_ω (= &lbl_ω chain local) is fT -- the statement's real fail continuation.  BOTH media:
           * emit_label_define_bb writes the position (resolves forward X-records from members); then
           * x86("add","rsp",K) + x86_jmp_ext(&lbl_ω) as a normal bb_emit_x86 call, medium-invisible. */
          if (i == zw5_stmtnode && zw5_n_stubs > 0) {
              static int _zot = -1; if (_zot < 0) { const char *_e = getenv("SCRIP_ZETA_OMEGA_TRACE"); _zot = (_e && _e[0]) ? 1 : 0; }
              for (int _d = 0; _d < zw5_n_stubs; _d++) {
                  emit_label_define_bb(&zw5_stub_lbls[zw5_pool_base + _d]);
                  if (_zot) fprintf(stderr, "[OMEGA-TRACE] stub stmtnode=%d depth=%d label=%s\n", i, zw5_stub_K[_d], zw5_stub_lbls[zw5_pool_base + _d].name ? zw5_stub_lbls[zw5_pool_base + _d].name : "?");
                  /* NO x86_begin() here -- stubs use no internal L(n) labels so x86_uid increment would
                   * shift all subsequent template .Lx<uid>_N references, corrupting .quad .string rodata links */
                  bb_emit_x86(x86("add", "rsp", (long)zw5_stub_K[_d]) + x86_jmp_ext(node_ω)); }   /* node_ω = fT for this statement */
              zw5_pool_base += zw5_n_stubs; zw5_n_stubs = 0; }
