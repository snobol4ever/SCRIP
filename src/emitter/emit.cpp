    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                zon[i] = 1; zout[i] = zd + K; if (Kc > 0 && hpos >= 0 && r >= hpos) { zud[i] = Kc + (zd - zdh) + K; zuh[i] = hi; } if (zws && r >= hpos) zzw[i] = 1; if (zwr && r >= hpos) zzwr[i] = 1;   /* ⭐ MECHANISM-2: zzwr[] is the parallel mechanism-2 member array staged into g_zd_uzwr → op_zw2 at the same choke as zzw → op_zw. */   zd = zout[i];
