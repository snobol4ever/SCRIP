...
int sn4_span_frame(void) {   /* ⭐⭐⭐ s130 THE ONE AUTHORITY for the LEAF-SUSPENSION-FRAME killswitch (SCRIP_SPAN_FRAME).  TWO READERS in lockstep -- leaf_frame_member() (which feeds the registry scan, hence the carve width at BOTH
     ...
     * and cannot drift (the s124 two-reader law, the s66 coherent-worlds law).  DEFAULT OFF: =0 is byte-identical to pre-s130 by construction, since a refused candidate never enters the scan (count unchanged,
