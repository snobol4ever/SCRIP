...
static long g_zd_uclaim, g_zd_udout; static int g_zd_uhead = -1; static int g_zd_uzw; static int g_zd_uzw_blob_base = -1;   /* ZW-12: staged beside the UCLAIM twins, applied at the same choke.  g_zd_uzw_blob_base: r9/wire rung -- the umin blob_base for the run's blob closure, -1 when no live blob slots. */   /* UCLAIM staging twins (wholesale flip): filled beside the ZD set at the drive loop, applied at the choke, cleared with the stage flag */   /* ZD-1 driver-handoff staging (R5b): the drive loop fills these from zd_plan's arrays immediately before emit_drive; the choke below applies them to g_emit exactly once and CLEARS the stage flag, so every other walk_bb_node entry path (FILL / EMIT_PAIR_FILL glue) sees zeros and stays byte-identical. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_uclaim = 0; g_emit.op_udout = 0; g_emit.op_uhead = -1; g_emit.op_zw = 0; g_emit.op_zw_blob_base = -1;   /* UCLAIM: cleared at the choke UNCONDITIONALLY, same law as the ZD set above */
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead; g_emit.op_zw = g_zd_uzw; g_emit.op_zw_blob_base = g_zd_uzw_blob_base;   /* r9/wire: apply blob_base alongside op_zw */   /* UCLAIM applied regardless of g_zd_arm -- declined runs are its whole constituency */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh, int *zzw) {
    ...
    for (int i = 0; i < n; i++) { zon[i] = 0; zout[i] = -1; zgpop[i] = 0; zwpop[i] = 0; zuk[i] = 0; zud[i] = 0; zuh[i] = -1; zzw[i] = 0; }
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            if (zws && nblob_real > 0 && umin < 0x7FFFFFFF) g_zd_uzw_blob_base = (int)umin; else g_zd_uzw_blob_base = -1;   /* ⭐ r9/wire (OMEGA s35): stage blob_base = umin when the canonical frame arms a run with live blob slots; blob templates read via op_zw_blob_base at dispatch.  -1 = no live blob (nblob_real==0 clean arm, or declined). */
            ...
            if (Kc > 0 && hpos >= 0) { zuk[run[hpos]] = Kc; zvo_add((int)umin, Kc, Kc, hi);   /* ⭐ CLAIM-AT-HEAD: zuk staged on the MATCH_BEGIN member -- the port hook carves it at the head's alpha, once per statement execution (the head's alpha is entered once; the unanchored retry loops inside it), so the var carves ONLY its own 16 -- Lon's roman.s complaint verbatim.  Pre-head members never see the claim: their exits free cells alone (kc gate above). */
                for (int k = 0; k < n; k++) if (cm[k] && zuh[k] < 0) { zud[k] = Kc; zuh[k] = hi; }   /* ⭐ blob members resolve DECLINED-IDENTICAL: dout delta ZERO, not Kc+hdd.  MEASURED (174_pat_bal_manual_example, this session): bb_match_bal spells its counter at raw offsets that CO-INCIDE with claim-relative resolution only when the delta is 0 (umin==0 graphs render [rsp+0] both ways); staging Kc+hdd shifted those spellings +16 into the neighboring assign_save cell -- corrupted paren counter, BAL accepted '-2)'.  Blob boxes execute at match-machinery depths the fc/op_zdepth machinery already compensates; the claim's coordinate is the OFF-world one, full stop.  Run members keep Kc+zd_before (they carry real cells above the claim). */
...
#undef RPO_VISITED
    ...
    long *zd_uk = (long *)alloca(sizeof(long) * (size_t)(n > 0 ? n : 1)); long *zd_ud = (long *)alloca(sizeof(long) * (size_t)(n > 0 ? n : 1)); int *zd_uh = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int *zd_zw = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1));   /* UCLAIM: per-node staged claim / depth-out / run-head for the ONE offset function */
    ...
    zd_plan(nodes, n, zd_on, zd_out, zd_gp, zd_wp, zd_uk, zd_ud, zd_uh, zd_zw);   /* ZD-1: the ONE execution-order walk runs ONCE per chain, before any emission, over the same nodes[] the loop below lays down */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0 || zd_zw[i]) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = _zw5_wpop_stolen ? 0 : zd_wp[i]; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i]; g_zd_uzw = zd_zw[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; ZW-5 O-2: g_zd_wpop=0 when stub owns release */
