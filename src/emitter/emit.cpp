...
static int zd_wl_kind(IR_t * nd) {
    ...
    { static int _zm = -1; if (_zm < 0) { const char * e = getenv("SCRIP_ZD_MATCH"); _zm = (e && *e == '0') ? 0 : 1; }   /* ⭐ DEFAULT FLIPPED ON (this session): the match quartet ARMS by default.  Evidence: full crosscheck-318 identical BY SET to the declined regime in both modes (m3 280/27/10, m4 266/39/10/2C == the s23g record), pattern-family 131 identical by set, zero P->F flips any arm, the one T->F (061_capture_in_arbno) closed by the general subject read.  SCRIP_ZD_MATCH=0 restores the declined regime byte-identically -- the escape hatch every wholesale flip keeps. */
      if (_zm && (op == IR_MATCH_BEGIN || op == IR_MATCH_SEQUENCE || op == IR_MATCH_END || op == IR_MATCH_REPLACE)) return 1; }   /* ⭐ ZD-5 MATCH-SPINE admission (this session): the STATEMENT-SPINE match quartet only -- the blob interior kinds stay out (their storage is the head's claim, per the hybrid staging above).  HEAD reads its subject via ZOPQ(0) non-popping (template arm this session); SEQ/RELEASE are claim-backed transparent boxes; REPLACE reads the replacement value via ZOPQ(1).  Killswitch-shaped env, default OFF until the watermark bracket proves it. */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh, int *zzw, int *zzwr) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        for (int r = 0; r < rl && ok; r++) { int i = run[r];
            { int _a = zd_wl_kind(nodes[i]); int _o = (int)nodes[i]->op; if (_a && zdyn && (_o == IR_MATCH_BEGIN || _o == IR_MATCH_SEQUENCE || _o == IR_MATCH_END || _o == IR_MATCH_REPLACE || _o == IR_MATCH_ALTERNATE)) _a = 0;   /* IR_MATCH_ALTERNATE joins the zdyn veto arm (s29): FENCE1/DEFER/PATREF in the run closure must also decline ALTERNATE -- ALTERNATE's subwalked arms can contain FENCE1, whose wire-glue geometry predates cells-above-claim (same hazard class as BEGIN/SEQ).  Without this, a FENCE(ALTERNATE(...)) run admits ALTERNATE while being vetoed for BEGIN, corrupting the claim depth. */
