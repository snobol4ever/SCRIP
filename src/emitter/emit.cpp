        ...
        { extern int sn4_defer_resume(void); const int _dres = sn4_defer_resume();
          ...
          if (_dres && g_emit.flat_pat && g_emit_cfg && g_emit_cfg->body_root) {
            ...
            if (getenv("SCRIP_RESUME_WHY")) { extern int zdp_seam_tier(const IR_t *); const IR_t * _br = g_emit_cfg->body_root; int _found = 0; for (int i = 0; i < n; i++) if (nodes[i] == _br) _found = 1;
                fprintf(stderr, "[RESUME-WHY] flat_pat=%d dres=%d f1=%d f1r=%d body_root_op=%d tier=%d in_nodes=%d n=%d\n", g_emit.flat_pat, _dres, _f1, _f1r, _br ? (int)_br->op : -1, _br ? zdp_seam_tier(_br) : -1, _found, n); }   /* ⛔ DIAGNOSTIC ONLY (s182): prints WHY the blob β-dispatch chose its resume target.  No verdict, no behaviour change; getenv at the site so nothing can be left half-armed. */
            if (!_f1r) for (int i = 0; i < n; i++) if (nodes[i] == g_emit_cfg->body_root && resume_carrier_ok(nodes[i])) { resume_tgt = betas[i]; break; }
