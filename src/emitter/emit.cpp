...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_k(IR_t * nd) { int op = (int)nd->op; return (op == IR_ASSIGN || op == IR_GOTO_DEFERRED || op == IR_SAVE_RESTORE) ? 0 : 16; }   /* ZD-SR joins the K=0 class: a transparent protocol box yields no cell, exactly like the IR_ASSIGN sink and the IR_GOTO_DEFERRED transfer already listed here.  THE EDIT IS THIS ONE LINE, which is the promise ZD-K1/K2 made. */   /* ZD-8 (b3) THIS IS THE ONE AUTHORITY for K, the alpha-carve byte count of an admitted node: a value-spine sink that yields no cell (IR_ASSIGN, IR_GOTO_DEFERRED) carves 0; every other admitted kind carves 16.  Until s22k this rule was SPELLED TWICE -- zd_plan's depth model and the drive loop's g_zd_k staging each held a copy -- and editing one alone made them disagree by exactly K, a silent stack skew on every node of the admitted kind (MEASURED: segfault, zero output, on 1020_code_label_transfer; it also cost s22j a misdiagnosis of the innocent by-name ZD arm).  Same disease zd_nops carries its own law for, and the s22i note at the staging site predicted this exact fold.  A kind whose K is neither 0 nor 16 changes THIS LINE ONLY. */
static int zd_stub_ok(void) { static int on = -1; if (on < 0) { const char * e = getenv("SCRIP_ZD_STUB"); on = (e && *e == '0') ? 0 : 1; } if (!on) return 0; { extern int g_flat_frame_floor; extern int g_gen_proc_active; if (getenv("SCRIP_ZD_STUB_DIAG")) fprintf(stderr, "[STUB] pat=%d gen=%d deep=%d genproc=%d resum=%d floor=%d\n", g_emit.flat_pat, g_emit.flat_gen, g_emit.flat_deep_arrival, g_gen_proc_active, (g_emit_cfg && g_emit_cfg->resumable_callable)?1:0, g_flat_frame_floor); return (!g_emit.flat_pat && !g_emit.flat_gen && !g_emit.flat_deep_arrival && !g_gen_proc_active && !(g_emit_cfg && g_emit_cfg->resumable_callable) && g_flat_frame_floor > 0) ? 1 : 0; } }   /* ZD-9 (s22k) DEFINE-STUB ADMISSION.  zd_plan's jmp-entry decline below was BLANKET, and that conflated two populations its own comment already distinguishes: runtime EVAL/CODE chains, which speak their own enter/exit protocol and genuinely must stay flat, versus DEFINE-stub blobs, whose body statements are ordinary value-spine work.  MEASURED s22k: a DEFINE'd proc body reports jmp=1 armed=0 under SCRIP_LP_DIAG, so EVERY statement in EVERY user proc was declined from the zeta spine -- which is why the SCRIP_NOFC break set is dominated by DEFINE programs and why it looked like an arg-marshalling ("Family A five-arm") problem: the arg side is DOWNSTREAM of a graph-level decline, not the cause.  THE PREDICATE IS NOT NEW -- it is the DEFINE-STUB DISCRIMINATOR already live and measured at the emit_chain choke, minus its two non-default env gates: g_flat_frame_floor > 0 is the DRIVER's own stub verdict (all four proc-emission loops in scrip.c set it from the role-3-entry predicate exactly and only around DEFINE-stub emissions), and every other jmp-entry citizen -- PAT$ patprocs, EVAL/CODE chains, rt pattern builds -- leaves it 0.  Killswitch SCRIP_ZD_STUB=0 restores the blanket decline. */
