...
static int emit_graph_has_deep_arrival(IR_graph_t *g) { if (!g) return 1; for (int i = 0; i < g->n; i++) { IR_t *c = g->all[i]; if (!c) continue; switch (c->op) { case IR_SUSPEND: case IR_SCAN: case IR_SCAN_ENTER: case IR_SCAN_ALTERNATE: case IR_SCAN_SEQUENCE: case IR_SCAN_UPTO: case IR_SCAN_FIND: case IR_SCAN_BAL: case IR_SCAN_MATCH: case IR_SCAN_MOVE: case IR_SCAN_TAB: case IR_TO: case IR_TO_BY: case IR_LIMIT: case IR_REPALT: case IR_PROC_GEN: case IR_CREATE: case IR_ITERATE: case IR_DISJUNCTION: case IR_CALL_BUILTIN_GEN: case IR_KEYWORD_ICON_GEN: case IR_MATCH_FENCE1: case IR_MATCH_ABORT: case IR_MATCH_HEAD: case IR_MATCH_ARBNO: case IR_MATCH_DEFER: case IR_MATCH_CALLOUT: case IR_MATCH_VALUE: return 1; default: break; } } return 0; }   /* FLATDISP-6 (s193, Lon's FOUR-CONSTRUCT ruling: STATEMENT/FUNCTION/ARBNO/FENCE1): the list is the rbp-READER set established by template census — FENCE1 reads rbp as the dynamic-ζ floor (mov rsp,rbp + watermark := rbp); ARBNO BORROWS rbp as zv() the element-view register (bb_match_arbno 222/229/242 re-point it — Lon's "housekeeping about variable length children based off of RBP", and WHY head/release/replace carry the +40 save/restore bracket); DEFER/VALUE σ-transfer into γ-retaining PAT$ blobs (post-transfer depth dynamic, the FORTH-ledger depth-immune watermark fact); CALLOUT conservative.  HEAD is Lon's STATEMENT construct and stays FOR A MEASURED REASON, not ceremony: TT_ABORT lowers to a BARE IR_GOTO at the seal edge (lower_snobol4.c ~1167) — no IR_MATCH_ABORT node ever exists in the graph (bb_match_abort.cpp + the enum + flat_trivial_beta all reference a kind nothing constructs; the constructing lower is in the gz5 PARKED file), so an abort-bearing statement is CLASSIFIER-INVISIBLE, and its kill jump lands at exhaustion with pattern cells still carved = dynamic depth.  Dropping HEAD was tried THIS session: m4 183→181, the two casualties exactly 170/171_pat_abort_* — the tripwire worked.  NAMED FOLLOW-ON (ABORT-NODE): relower TT_ABORT to the existing IR_MATCH_ABORT + add the dispatch case (mirror IR_MATCH_FENCE1 at ~938); the box's β→ω is MORE manual-faithful than the goto (Ch.9: backing up into ABORT kills the match — the reason FENCE(…|ABORT) exists); THEN HEAD can leave the list and fence/arbno/defer/abort-free match statements go depth-static.  DROPPED s193 and staying dropped (subsumed by HEAD, recorded for the census truth): ALTERNATE, SEQUENCE, ARB, RETRY, RELEASE — comment-only or save/restore-of-caller-value, and the match protocol rebalances rsp through the head snapshot at BOTH exits (058 .s ground truth).  FUNCTION is covered by the jmp-entry arm (seeds unconditionally, both media).  ⚠ COMMENT CORRECTION (prose-rot, RULES.md (a)-shape): the s192 text here claimed "every unlisted-but-present kind falls through to the SAFE answer (1)" — FALSE: default: break falls to return 0; the conservatism lives in the LIST being complete, which is why the census had to be by-template-read.  The crosscheck watermark is the tripwire, twice proven this session. */
