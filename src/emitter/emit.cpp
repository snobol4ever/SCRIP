...
#undef RPO_VISITED
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = _uw_stolen ? (int)_uw_pop : ((_zw5_wpop_stolen || _endj_stolen) ? 0 : zd_wp[i]); g_zd_wsteal = _endj_stolen || _uw_stolen;   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; mechanism-2 g_zd_uzwr staged alongside g_zd_uzw */
