    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                { long kc = (hpos >= 0 && r >= hpos) ? Kc : 0;   /* claim is live only from the head onward; a pre-head exit (subject-eval failure) frees cells alone */
                ...
                { int _ms = (zwr && ((int)nodes[i]->op == (int)IR_STATEMENT_END || (int)nodes[i]->op == (int)IR_STATEMENT));
                if (!gin) zgpop[i] = _ms ? 0 : _wzdepth + (int)kc;   /* ⭐ ZW-RB-1: mech2 (zwr) zeros STATEMENT_END/STATEMENT zgpop -- mech2_whack already restored rsp to α_base; add rsp,N here over-releases. Gate-OFF byte-identical. */
                if (!oin) zwpop[i] = _ms ? 0 : _wzdepth - K + (int)kc; } }   /* ZD-K2; ZW-RB-1 same zero for wpop; two closing braces: _ms block + kc/_wzdepth block. */
