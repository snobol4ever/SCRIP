...
#undef RPO_VISITED
    ...
    { extern std::string bb_zdp_origin(long); extern int x86_zdp_on_c(void); if (x86_zdp_on_c()) bb_emit_x86(bb_zdp_origin((long)0)); }   /* ⭐⭐⭐ LON'S EVERY-PORT PROBE (s136), THE DATUM.  Lon in-chat: "Instrument each BB graph's FIRST BB to save RSP (RSP0)".  Placed at the graph body label and BEFORE the R-4(b) blob activation carve on the next line, so RSP0 is this graph's TRUE entry frontier -- capturing it after any carve would bake the carve into the datum and every port would then agree by construction, which is the s134 defect (normalising at the point of measurement) in its purest form. */
