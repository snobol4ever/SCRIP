...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    case IR_MATCH_DEFER: { bb_prepare(nd); g_emit.op_seal = nd->seal; g_emit.op_off = drive_value_slot(nd); g_emit.op_defer_leaf_susp = g_emit_cfg ? fc_tail_defer_susp_g(g_emit_cfg, nd) : -1; bb_emit_x86(bb_match_defer()); } return 0;  /* SN4-PAT-FOLD deferred/stored pattern var; s137 OVER-SEAL: seal state written HERE (post-prologue, both drive paths) from IR_t.seal — op_off points at the defer.pad quad (the α rsp watermark) when FULL-sealed (==1); s142: seal==2 = write-once class, entry-cell only, no pad; s153: op_defer_leaf_susp arms the ZERO-GUARDED β for priced tail-candidate leaves (the ε-resume phantom-pad convention). RBP-EARN s44 WIRE-SAVE: op_off is now ALWAYS staged (not gated on seal==1) — the zeta_storage.c ZLS grant for IR_MATCH_DEFER is unconditional (16B every node, seal==1 merely repurposes +0 as the fence watermark), so the quad is a safe, always-valid home; bb_match_defer.cpp's own wire-save arm additionally gates on seal!=1 (so it never fights the fence watermark for +0) and x86_fb_pinned() (so FRQ is depth-immune — see s43d's rsp-arm misread caveat) before actually using it. */
