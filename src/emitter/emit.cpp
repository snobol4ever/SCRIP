    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_β);
    {
        ...
        { extern int sn4_defer_resume(void); const int _dres = sn4_defer_resume();   /* ⭐ SN4-DEFER-RESUME (s121) half B2: the SNOBOL4 arm of the resume dispatch, mirroring the two Prolog arms above.  A PAT$ blob (flat_pat graph) β-arrival — the invoker's `jmp [rsp+0]` through the CLASS D suspension record, res stub restored wires+frame, interior state retained on the spine below — lands the published resume carrier's β: a generator's β EXTENDS (ARBNO one more instance, manual p.121; ARB one more char), a deterministic carrier's β fail-throughs to its ω and rides r11 home through the absolute unwind.  Without this arm the dispatch defaulted to lbl_ω: every stored ARB/ARBNO answered exactly once and took its shy null match forever — the s120 FINDING's closed SUCCEED/FAIL pair, beauty's `Parse Error`.  Same switch as the lowering half; =0 restores the default byte-identically. */
          if (_dres && g_emit.flat_pat && g_emit_cfg && g_emit_cfg->body_root) {
            ...
            if (!_f1) for (int i = 0; i < n; i++) if (nodes[i] == g_emit_cfg->body_root && resume_carrier_ok(nodes[i])) { resume_tgt = betas[i]; break; }
