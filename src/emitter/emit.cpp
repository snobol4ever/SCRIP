...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, int *zarm) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = (vd_tidx >= 0) ? zd_omega_seed(nodes, n, nodes[hi], zon, zout) : 0; int arm_zd = 0; int zdh_match = -1;
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (_zvd) { zgt[i] = gt; zot[i] = ot; zgin[i] = (unsigned char)gin; zoin[i] = (unsigned char)oin; zmatch[i] = zdh_match; }
                                { long kc = 0;
...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    g_last_flat_fp = (g_emit_cfg && !g_emit.flat_layout_unknown) ? (g_emit.flat_gen ? (g_emit_cfg->nparams + g_emit_cfg->nlocals) * 16 : zls_g_fp_total(g_emit_cfg)) : 0;   /* ⛔⛔ flat_gen arm (ceo s283d): the registry pairing align16(32+fb)+fp+16 must equal the callee alpha's frame_total = flat_frame_bytes + (np+nl)*16, and zls_g_fp_total knows zls FIELDS, not Icon params/locals -- so every generator WITH a local was re-registered 16-per-slot short by THIS line the moment it was emitted, silently overwriting the pre-pass's correct value (scrip.c:1411/:1824 call through here after each proc). The caller's landing then read the yielded value low and the host slice was carved short: seat15's one-empty-key concord table, the w1 concat witness's empty yields. np=nl=0 on every prior proof witness made 0 the right answer six times running -- the plausible-zero class, again. Keyed on flat_gen: only Icon generator graphs carry the term, SNOBOL4 pattern registration is byte-unchanged. ⛔⛔ CORRECTED SAME DAY (s283e): the first cut ADDED the term on top of zls_g_fp_total -- zero for plain generators (six lucky witnesses AGAIN) but NONZERO for SCAN graphs, whose zls fields already live inside flat_frame_bytes, so the registry then OVERSHOT frame_total by the zls amount and the landing read 32 HIGH on the seat15 scan witness (alpha H at +576, landing at -608, measured). For a flat_gen graph the term REPLACES zls -- frame_total is kt2+(np+nl)*16, period, ONE formula -- and the regression was invisible to every board because the armed D2 set had no scan witness: suspend_scan added to test_icn_d2_suspend_witness.sh in this commit. */
