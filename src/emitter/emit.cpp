...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    while (qh < qt) {
        ...
        if (((c->op >= IR_MATCH_LIT && c->op <= IR_MATCH_ASSIGN_SAVE) || c->op == IR_MATCH_DEFER || c->op == IR_MATCH_VALUE) && c->ω.node && qt < Q_MAX) queue[qt++] = c->ω.node;  /* SN4-PAT: match-family ω is a real control edge (next alternative / fail handler; DEFER folded in for SN4-PAT-FOLD) */
...
static int emit_stmt_frame_scan(IR_graph_t * g, IR_t * entry) {   /* STMT-FRAME classifier, SLICE 1 (s21x-c NEXT EXECUTABLE: "straight-line int-only statement graphs first"): ADMIT iff every node REACHABLE from entry is in the closed whitelist {IR_LIT_INTEGER, IR_VAR, IR_BINOP, IR_ASSIGN}.  Reachability, not g->all: lower seeds inert per-graph companions (two IR_SAVE_RESTORE floaters + an IR_CALL/IR_LIT_STRING NRETURN pair, measured on the sl0 witness) that never reach the emitted chain -- name-testing them here would be a language sentinel (BANNED past first dispatch), so the license is scoped to what will EMIT.  The walk mirrors the codegen chain BFS restricted to whitelist ops (γ always; ω for the BINOP/VAR classes, the real BFS's rule for exactly these kinds); the mirror is a safe superset BY CONSTRUCTION because a reached non-whitelist op rejects before its unknown edge classes matter.  No gotos admitted => every edge into a bb_src_of head is a genuine statement entry (retargets need no membership map).  The list GROWS one construct per rung, CELL-1a discipline; whitelist completeness IS the safety (the FLATDISP-6 conservatism rule). */
    ...
    while (qh < qt) { IR_t * c = q[qh++];
        ...
        seen[sn++] = c;
        switch (c->op) { case IR_LIT_INTEGER: case IR_ASSIGN: break;
            case IR_VAR: case IR_BINOP: if (c->ω.node) q[qt++] = c->ω.node; break;
            default: { if (_sd) { extern const char * bb_op_name(IR_e); const char * nm = bb_op_name(c->op); fprintf(stderr, "[STF] reject op %s(%d) reachable sn=%d\n", nm ? nm : "?", (int)c->op, sn); } return 0; } }
