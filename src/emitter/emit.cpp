...
int sn4_defer_resume(void) {   /* ⭐ s123 THE ONE AUTHORITY for the SN4-DEFER-RESUME killswitch (was a static local at the emit.cpp beta-dispatch, s121 half B2; hoisted so bb_match_arbno_frame's af edge reads the SAME fact -- the beta route and the af unwind are ONE mechanism and must never disagree, the s68/s70 spelled-twice disease). =1 admits the leaf-generator resume carrier AND drops the af->defer_beta over-pop edge; =0 restores pre-s121 byte-identically. */
    static int _dres = -1; if (_dres < 0) { const char * e = getenv("SCRIP_DEFER_RESUME"); _dres = (e && *e == '1') ? 1 : 0; } return _dres;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_seam_walk(void) {   /* ⭐ s124 THE ONE AUTHORITY for the LEFTWARD-SEAM-WALK killswitch. =1 widens the blob beta-dispatch carrier set from leaf generators to ANY deterministic fail-through element, so a blob whose RIGHTMOST element is deterministic still ENTERS its interior seam instead of dead-ending at lbl_omega; =0 restores s123 byte-identically. */
    static int _sw = -1; if (_sw < 0) { const char * e = getenv("SCRIP_SEAM_WALK"); _sw = (e && *e == '1') ? 1 : 0; } return _sw;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
