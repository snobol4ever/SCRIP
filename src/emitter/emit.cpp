...
static int fence0_whack_on() { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_FENCE0_WHACK"); v = (e && *e && *e != '0') ? 1 : 0; } return v; }   /* ⭐ FZ-3 (s168): MOVED HERE VERBATIM FROM bb_match_fence0.cpp -- NOT a new global, the SAME one function-static getenv cache relocated, and the template's copy is deleted in the same commit so the count of arm spellings stays exactly one.  IT HAS TO LIVE ON THIS SIDE because the ζ DEPTH PLANNER must ask the question too: FZ-3 threads the release through zd_plan's depth accumulator, and a template-side gate is invisible there, so an armed planner would meet a disarmed template (or the reverse) and every staged [rsp+off] in the statement would be skewed by exactly the release -- the planner/template disagreement the s66 coherent-worlds law convicts and the s22k ONE-AUTHORITY law forbids.  DEFAULT OFF is unchanged: absent or 0 => 0 bytes => byte-identical to the no-whack box. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
