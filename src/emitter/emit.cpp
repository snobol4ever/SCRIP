...
static long g_zd_uclaim, g_zd_udout; static int g_zd_uhead = -1;   /* UCLAIM staging twins (wholesale flip): filled beside the ZD set at the drive loop, applied at the choke, cleared with the stage flag */   /* ZD-1 driver-handoff staging (R5b): the drive loop fills these from zd_plan's arrays immediately before emit_drive; the choke below applies them to g_emit exactly once and CLEARS the stage flag, so every other walk_bb_node entry path (FILL / EMIT_PAIR_FILL glue) sees zeros and stays byte-identical. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_uclaim = 0; g_emit.op_udout = 0; g_emit.op_uhead = -1;   /* UCLAIM: cleared at the choke UNCONDITIONALLY, same law as the ZD set above */
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead;   /* UCLAIM applied regardless of g_zd_arm -- declined runs are its whole constituency */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh) {
    ...
    for (int i = 0; i < n; i++) { zon[i] = 0; zout[i] = -1; zgpop[i] = 0; zwpop[i] = 0; zuk[i] = 0; zud[i] = 0; zuh[i] = -1; }
...
#undef RPO_VISITED
    ...
    long *zd_uk = (long *)alloca(sizeof(long) * (size_t)(n > 0 ? n : 1)); long *zd_ud = (long *)alloca(sizeof(long) * (size_t)(n > 0 ? n : 1)); int *zd_uh = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1));   /* UCLAIM: per-node staged claim / depth-out / run-head for the ONE offset function */
    zvo_reset();   /* UCLAIM owner table is per-chain, rebuilt by the plan below */
    zd_plan(nodes, n, zd_on, zd_out, zd_gp, zd_wp, zd_uk, zd_ud, zd_uh);   /* ZD-1: the ONE execution-order walk runs ONCE per chain, before any emission, over the same nodes[] the loop below lays down */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = zd_wp[i]; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke */
