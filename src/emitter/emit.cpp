...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    { static int _nc = -1; if (_nc < 0) { const char * e = getenv("SCRIP_NO_GRAPH_CARVE"); _nc = (e && *e == '1') ? 1 : 0; } if (_nc && g_emit.flat_frame_bytes > 48) g_emit.flat_frame_bytes = 48; }   /* s21x-r EXPERIMENT ONLY: clamp the whole-graph carve to the xa_flat >=48 floor to measure whether it is still load-bearing.  NOT a fix -- the 1057 FR/FRQ/FRQB reference sites across 108 templates read slots this carve allocates. */
