...
#undef RPO_VISITED
    ...
    if (g_emit.flat_jmp_entry) {
        ...
        if (g_emit.flat_pat) {
            bb_emit_x86(x86_rsp_load64("r10", 8) + x86_rsp_load64("r11", 16) + IF(blob_frame_bytes() > 0, x86_rsp_load64("rbp", 24)) + x86("add", "rsp", 32L));
        } else {
        if (g_is_text) {
            char _res[96];
