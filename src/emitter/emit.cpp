...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    case IR_MATCH_ARBNO:          { bb_prepare(nd); g_emit.op_arbno_rbp = emit_arbno_rbp();   /* ⭐ ARBNO-RBP (Lon 2026-08-14): staged from THE ONE AUTHORITY (beside emit_match_rbp); the template keys purely on this field, zd_k reads the same predicate -- the killswitch cannot half-flip (s66). */ extern int fc_tail_arbno(const IR_t *, int *, int *, int *, int *); extern int fc_tail_ncap(const IR_t *); int _fpb = 0, _fpl = 0, _osb = 0, _hdr = 0;
