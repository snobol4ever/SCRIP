...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int zd_nops(IR_t * nd) { int op = (int)nd->op; return (op == IR_UNOP || op == IR_ASSIGN || op == IR_DEREF || op == IR_COERCE_STRING || op == IR_COERCE_INTEGER) ? 1 : (op == IR_BINOP || op == IR_COERCE_NUMERIC || op == IR_CMP_TEST || op == IR_SUBSCRIPT || op == IR_ASSIGN_VAR) ? 2 : 0; }   /* ZD-2d/2e/2f: COERCE_NUMERIC reads operand 0 as the value and operand 1 as the partner type-peek; CMP_TEST takes both coerced operands by address; SUBSCRIPT takes base and index by value.  All stage two op_zread differences.  THIS IS THE ONE AUTHORITY -- the drive-loop staging site calls it rather than re-deriving, since the moment those two disagreed every operand rendered [rsp+0] and 29 programs silently returned wrong answers. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
