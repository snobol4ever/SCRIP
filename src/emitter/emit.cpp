...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_zres = 0; g_emit.op_zgpop = 0; g_emit.op_wsteal = 0; for (int _zi = 0; _zi < 6; _zi++) { g_emit.op_zread[_zi] = 0; g_emit.op_zkind[_zi] = -1; }   /* ZD-1: cleared at the choke UNCONDITIONALLY so glue and non-drive fills can never inherit a stale regime */
    if (g_zd_stage) {    /* UCLAIM applied regardless of g_zd_arm; op_zw2 mechanism-2 flag applied beside op_zw */
...
static int zd_wl_kind(IR_t * nd) {
    ...
    if (op == IR_VAR_REF) { static int _plvr = -1; if (_plvr < 0) { const char * e = getenv("SCRIP_ZD_PL_VR"); _plvr = (e && *e == '0') ? 0 : 1; } if (_plvr && g_emit_cfg && g_emit_cfg->pl_cells_graph) return 1; static int _icnvr = -1; if (_icnvr < 0) { const char * e = getenv("SCRIP_ZD_ICN_VR"); _icnvr = (e && *e == '0') ? 0 : 1; } if (_icnvr && g_emit_cfg && g_emit_cfg->icn_cells_graph && x86_fb_pinned()) { const char *vn = IR_LIT(nd).sval; return (vn && !is_global(vn)) ? 1 : 0; } return 0; }   /* ZK-2 ICN-CELLS ADDITIVE (s222, landed): IR_VAR_REF admitted on icn_cells_graph for pinned graphs with local (non-global) variables.  THREE GUARDS: (1) icn_cells_graph -- cells arm only, not zframe or default; (2) x86_fb_pinned() -- pinned graphs (flat_lcl_proc prologue sets rbp; FRQ(op_sa)=[rbp+op_sa], depth-immune across subsequent RSP carves -- BLOCKER 1 from s221); (3) !is_global(vn) -- globals route to GVA slots, not the FORTH spine (ZK-5 law).  BLOCKER 2 FROM s221 RESOLVED BY WL-DELETE (s12 merge): IR_DEREF is now unconditionally admitted at line 1906 (IR_DEREF||IR_ASSIGN_VAR), so VAR_REF+SUBSCRIPT+DEREF arms atomically on every pinned graph -- the partial-arm NAMETRAP hazard cannot occur.  TEMPLATE: bb_var_ref.cpp ZD arm (op_zres && op_sa>=0) writes {DT_N,slen=1,&rbp_slot} to ZRES(0/8); bb_deref.cpp ZD arm reads ZOPQ(0,0/8)=NAMETRAP, calls rt_deref, writes plain value to ZRES(0/8).  SN4 watermark: icn_cells_graph=0 for all SN4 graphs -- byte-identical by construction.  Killswitch SCRIP_ZD_ICN_VR=0.  ONE AUTHORITY per s22k law -- only this line and zd_k (K=16 default) name IR_VAR_REF for the ZD regime on icn_cells_graph.  PL-ZK-1 arm 2/5: IR_VAR_REF admitted on pl_cells_graph.  K=16 default; nops=0 (no inline run-predecessor -- VAR_REF reads a named variable slot, not a spine cell).  Killswitch SCRIP_ZD_PL_VR=0. */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            /* ⭐ WL-DELETE MATCH-HEAD FIX (CLIMB s19, 2026-08-08): WL-DELETE admitted MATCH_BEGIN into the ZD run, causing blob-interior K>0 nodes (e.g. LIT_INTEGER inside the pattern body) to
             * accumulate zd past the MATCH_BEGIN-α entry floor.  MATCH_END's CAS-restore (mov rsp,[cas_rsp_mark]) resets RSP to MATCH_BEGIN-α entry regardless of how deep blob-interior carves went,
             * so _wzdepth at STATEMENT_END (the node after MATCH_END) must be capped at the depth seen at MATCH_BEGIN entry, not the total accumulated zd.  Before WL-DELETE MATCH_BEGIN was declined
             * so the whole run declined and zgpop was always 0; now the run arms and the W-1 FIX invariant ("zd at STATEMENT_END correctly reflects only the pre-match producer cells") is broken.
             * Fix: zd_mhead records zd at MATCH_BEGIN entry; past_mend flips at MATCH_END; _wzdepth is capped at zd_mhead for nodes after MATCH_END.  GATED: zd_mhead<0 = no MATCH_BEGIN in run,
             * byte-identical to prior behavior.  ONE AUTHORITY: this is the sole site that corrects the post-mend depth floor.  SCRIP_ZD_DIAG=1 verifies gpop before and after. */
            int zd_mhead = -1; int past_mend = 0; int past_arbno = 0;
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (nodes[i]->op == IR_MATCH_BEGIN && zd_mhead < 0) zd_mhead = zd;   /* record depth AT MATCH_BEGIN entry (before its K=0 is added, so zd is the pre-head floor) */
                if (nodes[i]->op == IR_MATCH_END) past_mend = 1;   /* flip AFTER MATCH_END processes; STATEMENT_END follows and sees capped _wzdepth */
                if (past_arbno && K > 0) zon[i] = 2;   /* ⭐ WL-DELETE ARBNO POST-BODY SUPPRESS (CLIMB s19, 2026-08-08): K>0 nodes after MATCH_ARBNO in the run execute on ARBNO's sigma path (every iteration) without any RSP restore between iterations.  Arming them emits sub rsp,K at each sigma pass; these accumulate without release because ARBNO-beta does NOT do add rsp,K before retrying, and MATCH_END's CAS-restore only fires on final success.  Tag zon[i]=2 (sentinel: armed for depth accounting but NOT for emission arm): the staging choke reads zon[i]==1 as armed, zon[i]==2 as depth-only (suppress g_zd_arm).  gin/oin tests (which use cm[] or run[]) are unaffected since they do not read zon[].  MECH cross-request: post-ARBNO K>0 nodes need per-iteration ZD cells re-homed inside ARBNO's frame before re-enabling.  GATED past_arbno && K>0: K=0 nodes (MATCH_RPOS, MATCH_END, STATEMENT_END) need no carve anyway; pre-ARBNO K>0 nodes (VAR, LIT_INT-POS) are correctly armed with zon[i]=1. */
                else zon[i] = 1;
                if (nodes[i]->op == IR_MATCH_ARBNO) past_arbno = 1;   /* flip AFTER ARBNO so ARBNO itself takes the staging choke arm-suppress (zon[ARBNO]=1 but g_zd_arm suppressed by _arbno_arm_suppress in the choke) */
                zout[i] = zd + K; zd = zd + K;
                ...
                { if (!oin && K == 0) { int _io = !ot; if (!_io) { for (int _ik = 0; _ik <= r; _ik++) { if (nodes[run[_ik]] == ot) { _io = 1; break; } } } if (_io) oin = 1; } }   /* ⭐ M-1 FIX 3 + ZWR EXTENSION (C-5 CLIMB, ZK-3/ZK-4 regression fix): backward-edge oin guard for ZWS and ZWR.  !ot arm: ω.node=NULL means template-routed fail (e.g. MATCH_LIT abort-fail path) — MATCH_END is sole release authority, suppress wpop.  ot-found arm: ω points to an earlier run member (k<=r backward edge) — same suppression law as ZWS M-1 FIX 3.  GATED K==0 && (zws||zwr): non-match, non-window, K>0 runs are byte-identical.  ONE LINE per s22k law. */
                                { long kc = 0;
                int _wzdepth = (past_mend && zd_mhead >= 0) ? zd_mhead : (int)zd;   /* ⭐ ZW-12 HEAD-WPOP FIX: MATCH_BEGIN's ω exit is the pattern-exhausted fail — it fires AFTER the whack restores rsp to the claim base, so it sees only pre-frame depth (zdh), NOT the post-ZW_FRAME_TOTAL depth (zd which has been incremented by 64).  Using post-ZW zd here inflated wpop by ZW_FRAME_TOTAL=64, causing add rsp,N to release 64 bytes into the caller's frame on each scan retry (rip=0x1/0x3 class, Z4 span crash). */   /* ⭐ WL-DELETE MATCH-HEAD FIX (CLIMB s19): cap at zd_mhead for post-MATCH_END nodes -- MATCH_END's CAS-restore already freed blob-interior K>0 carves; STATEMENT_END must release only the pre-head floor (zd_mhead = depth at MATCH_BEGIN entry).  Byte-identical when zd_mhead<0 (no MATCH_BEGIN in run). */
...
#undef RPO_VISITED
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0) { g_zd_stage = 1; int _arbno_arm_suppress = (nodes[i]->op == IR_MATCH_ARBNO) || (zd_on[i] == 2);   /* ⭐ WL-DELETE ARBNO ARM SUPPRESS (CLIMB s19, 2026-08-08): ARBNO is admitted to the ZD run by the universal arm (zd_wl_kind returns 1) so depth accounting (gpop/wpop) is correct — MATCH_BEGIN.wpop and STATEMENT_END.gpop both need the accumulated K.  But the ZD arm (op_zres=1 + sub rsp,16 hook + ZOPQ reads) is WRONG for ARBNO: the template does sub rsp,288 at alpha BEFORE any ZD arm code reads predecessors, so ZOPQ(0,op_zread[0]) resolves to inside the 288B zero-init frame rather than the predecessor's K=16 cell.  Result: ARBNO-sigma reads r14d from [rsp+260] which is the zero-init cursor slot → r14d=0 every retry → RPOS(0) never satisfied → unbounded ARBNO retry loop → stack overflow SIGSEGV.  zon[i]==2 sentinel: post-ARBNO K>0 nodes (e.g. LIT_INT-RPOS) are tagged by zd_plan to suppress emission arm while preserving depth accounting.  Fix: suppress g_zd_arm for ARBNO (op check) and post-ARBNO K>0 nodes (zon==2 sentinel); g_zd_gpop/wpop propagate unchanged so all depth releases in the run stay correct.  MECH cross-request: ARBNO needs a 288B-aware ZD arm (ZOPQ offset + op_zread[0]) before re-enabling.  SN4 watermark: every K>0 node that was formerly armed for ARBNO-containing runs is now non-ZD — byte-identical to prior behavior for those nodes since ARBNO was not in the old whitelist.  ONE AUTHORITY: this site and the zd_plan depth model now agree: zd_plan arms zon[ARBNO]=1 for depth accounting; the choke alone suppresses the emission arm. */ g_zd_arm = zd_on[i] && !_arbno_arm_suppress; g_zd_gpop = zd_gp[i]; g_zd_wpop = _uw_stolen ? (int)_uw_pop : ((_zw5_wpop_stolen || _endj_stolen) ? 0 : zd_wp[i]); g_zd_wsteal = _endj_stolen || _uw_stolen;   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; mechanism-2 g_zd_uzwr staged alongside g_zd_uzw */
