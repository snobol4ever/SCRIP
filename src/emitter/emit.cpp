...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int emit_graph_reads_pinned_frame(void) { return g_emit.flat_jmp_entry || g_emit.flat_pat || g_emit.flat_gen; }   /* CARVE-KILL RE-LAND (s165): THE ONE PREDICATE naming the graph class whose protocol READS the pinned whole-graph frame, and which therefore still requires the entry/exit pair that establishes and releases it.  This is emit.h:601 flat_all_zd's own stated rule quoted back verbatim -- "jmp-entry blobs, flat_pat, flat_gen keep legacy verbatim (their protocols read the pinned whole-graph frame)" -- and it is the separation ef9a7d2c's own message demanded before any re-land ("the deleted function was the SINGLE AUTHORITY for EVERY graph entry shape ... any re-land has to separate those three before it can keep the good one").  MEASURED s165: with the pair deleted outright, nrev.s carried 1372 [rbp+off] references against ONE surviving mov rbp,rsp -- first consumer line 8, establishment line 9972 -- so every predicate/procedure entry stored through a base register the caller owned.  The carve is NOT dead: all 1414 [rbp+N] offsets in the last-green artifact land inside it (max offset 1296 == max carve 1296); the s164 instrument read 685 [rsp+..] refs and was blind to the addressing mode that actually consumes the region.  Behavioural, not a language sentinel: these are graph-shape flags set by LOWER-independent emit-time analysis, so NO-LANGUAGE-SENTINEL holds and test_gate_emit_no_lang.sh stays green.  Self-retiring: as ZD kinds arm, flat_gen/flat_pat/flat_jmp_entry graphs convert to per-BB self-allocation and this returns 0 with no edit here -- the carve then vanishes BY CONSTRUCTION, which is what CARVE-KILL wanted and deletion could not deliver. */
...
#undef RPO_VISITED
    ...
    if (emit_graph_reads_pinned_frame()) xa_dispatch(XA_FLAT_PROLOGUE);   /* CARVE-KILL RE-LAND (s165): the prologue is emitted ONLY for the graph class that reads the pinned frame.  Non-jmp-entry RSP main graphs keep s22n's carve-free entry unchanged -- that half of CARVE-KILL is CORRECT and is preserved. */
    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_γ);   /* CARVE-KILL RE-LAND (s165): lbl_γ is defined for EVERY graph class -- both owners need it and every success edge targets it.  Only the BODY and the lbl_ω define below are class-specific.  Gating this line too (first cut, s165) left `proc_flat_γ` as an unresolved forward reference at bb_emit_end: a label every edge jumps to and nobody defines. */
    if (!emit_graph_reads_pinned_frame()) {   /* CARVE-KILL RE-LAND (s165): ONE OWNER PER GRAPH CLASS for the γ/ω exit BODIES.  OUTER-EXIT-1 (s22p) made the outermost box own its own ports as glue and landed AFTER the s22o epilogue deletion -- it IS the replacement for the deleted epilogue's exit duties, and it is correct for carve-free main graphs.  A pinned-frame graph's exits are instead owned by the flat EPILOGUE, which defines lbl_ω itself while ALSO releasing the frame (lea rsp,[rbp+kt]; reload of the saved rcx/rdx/rbp wire at kt-24/-16/-8).  Emitting both defines lbl_ω twice: MEASURED as `symbol proc_reverse$2F2_ω is already defined` x4 from `as`, a graph that cannot assemble at all.  This restores the LAST-KNOWN-GREEN shape verbatim for that class -- 548c5637 emit.cpp:2443-2444 is `emit_label_define_bb(&lbl_γ); xa_dispatch(XA_FLAT_EPILOGUE);` and nothing else. */
        bb_emit_x86(bb_glue_outer_γ());
    emit_sep_rule('-'); emit_label_define_bb(&lbl_ω); bb_emit_x86(bb_glue_outer_ω());   /* OUTER-EXIT-1 (Lon s22p): the outermost box owns its OWN two ports, as glue -- this is the whole of what the deleted epilogue did in the ordinary case.  lbl_ω was left UNDEFINED by the s22o deletion while every fail edge still jumped to it, which is the single reason `--compile` produced no assemblable .s for 317/317 programs: one undefined symbol, not 317 defects.  ONE-SHOT BRIDGE (s22p): lbl_gamma and lbl_omega are jmp targets inside main -- no ret, no eax. bb_glue_outer_whack=true; framed_leave is the depth-immune exit; bb_glue_flat.cpp owns the protocol. */
    } else { xa_dispatch(XA_FLAT_EPILOGUE); }   /* CARVE-KILL RE-LAND (s165): the epilogue is the prologue's matched half -- re-landed under the SAME predicate or not at all, because a carve without its release leaks rsp per activation.  s22o was right that the epilogue could not survive the prologue's death; it is right for the same reason that it must return with it. */
