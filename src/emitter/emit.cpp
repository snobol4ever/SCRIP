...
int emit_arbno_rbp(void) { static int _env = -1; if (_env < 0) { const char * e = getenv("SCRIP_ARBNO_RBP"); _env = (e && *e == '0') ? 0 : (e && *e == '2') ? 2 : 1; } return (_env == 2) ? 1 : (_env && !g_emit.flat_layout_unknown && !g_emit.flat_jmp_entry); }   /* ⛔⭐ E-4 WIDENING ARM (Lon in-chat 2026-08-14, on being shown claws5-match's frameless n1: "Does the ARBNO in claws5-match NEED an RBP?" — it DOES: its β/as/af read [rsp+0]/[rsp+4] under an UNVERIFIED net-zero-interior invariant, so it works by discipline-luck, not by knowledge; only an rbp base makes those reads sound at ANY depth).  THREE ARMS on the SCRIP_FN_RBP=0/1/2 precedent: 0 = legacy frameless (killswitch, byte-identical); 1 = framed-with-conjuncts (the s71 default, PRESERVED so the widening is measurable against it); 2 = ALWAYS, the directive as literally worded — the two graph conjuncts are BYPASSED and every ARBNO on every graph gets its frame.  ⛔ THE CONJUNCTS WERE THIS SEAT'S UNAUTHORIZED NARROWING OF A LON RULING: "ALWAYS" was the word, and flat_layout_unknown/flat_jmp_entry were added out of caution about unmeasured HEAD-PIN rpin()/blob-suspend interleavings with a live cross-γ rbp — recorded in a comment instead of raised as the banner ask the FACT RULE requires for a deviation.  Arm 2 exists to MEASURE that caution rather than keep inheriting it; when its radius and witnesses are green the conjuncts are deleted and arm 1 goes with them.  E-1's epilogue (emit_arbno_rbp_unwind, below) is the enabling precondition: widening without it would widen the stale-rbp leak class too. */   /* ⭐ ARBNO-RBP (Lon in-chat 2026-08-14, "For ARBNO, ALWAYS create an RBP on ALPHA to carve out RESULT and ONE LOCAL, an integer counter of # instances which when goes negative then causes exhaustion thru OMEGA ... at BETA, the BB LOCAL is directly accessed thru RBP+16 or +0 whichever"): THE ONE AUTHORITY for the rbp-framed ARBNO -- env read HERE only (default ON per the directive), consulted by zd_k's ARBNO arm below (16 -> 32) and staged into g_emit.op_arbno_rbp at the IR_MATCH_ARBNO drive case, so planner accounting and template routing flip on ONE predicate (the s66 two-coherent-worlds law; same discipline as emit_match_rbp above).  SCRIP_ARBNO_RBP=0 restores legacy byte-identically on BOTH sides (the env short-circuits BEFORE the graph flags, so OFF never reads them).  The two graph conjuncts keep the arm on the charted main class: layout-unknown and jmp-entry graphs (the HEAD-PIN rpin() population, plus blob-regime suspend machinery whose interleavings with a live cross-γ rbp are unmeasured) DECLINE to legacy -- conservative by construction, costing only legacy bytes on rare classes.  ⭐ THE arbdef1 LEAK IS FIXED (E-1, 2026-08-14) AND THE OLD "STRUCTURAL COLLISION" FRAMING HERE WAS WRONG — Lon's correction, verbatim substance: "At the end of pattern match ALL RBP are put back in place."  WHAT THIS COMMENT USED TO SAY (a KNOWN-OPEN leak, a bracket postmortem, and a four-option cure menu A/B/C/D) described the defect as ARBNO and the FUNCTION protocol fighting over one register.  IT WAS NOT: the rung pushed rbp at every ARBNO α and restored it on ONE exit only (ARBNO's own exhaustion ω), so any match that ended with an interior ARBNO still suspended walked away with rbp pointing into a released frame.  A MISSING EPILOGUE, not an architecture problem — the FUNCTION was merely the first legitimate downstream rbp reader, which is why arbdef1 is where it surfaced and why top-level mains survived on luck.  THE FIX: every match exit walks the chain home before its rsp whack — x86_arbno_rbp_unwind() (x86_asm.h, THE ONE AUTHORITY for the walk) gated by emit_arbno_rbp_unwind() (below, THE ONE AUTHORITY for when).  Wired at the three exits that release rsp from a banked mark: MATCH_END success (both the tail and non-tail arms, MARK=[r8+8]) and the MATCH_BEGIN fail tail (both the hfc and non-hfc arms, MARK=[r12+8]).  NO new storage, NO grant, NO quartet dependency — the chain already banks every value at [rbp+24], so the exits only had to WALK it; the s71 blocker analysis ("+40 collides with mechanism-2, +72 is ungranted") was solving a problem that did not exist, because nothing needs SAVING.  MEASURED at the landing HEAD: probe/arbdef1.sno m4 rc139 zero-output → rc=0 output byte-identical to the live sbl oracle; gdb before the fix pinned rip=0x0 rbp=0x0 with n19_match_end_α live (the RETURN floater set rsp from the dead frame, popped a NULL γ and jmp'd to it).  Killswitch OFF byte-identical 318/318; ON radius 86, every mover ARBNO-census, ZERO non-ARBNO movement; the 25 ARBNO-carrying non-movers all attributed (17 bomb-defer-unsafe, 5 FRAMELESS conjunct-declines, 2 no ARBNO box, 1 compile-fail) — all populations that push no frame, so the walk correctly does not fire.  Witness suite held: arb1 PASS/PASS, N22–N33 12/12 both media, poisoned control PASS.  STILL OPEN, AND NOT THIS DEFECT: 181 m4 (the s22r ungranted +40..+72 quartet on carve-less deep mains, PC-pinned by s71 to an IDENTICAL instruction in the BASELINE binary — OS-2's rung) and earn0_disc_arbno_star_fence_positive (osv=1 mb=-1, the *cmd fence family — its own mechanism rung).  The two graph conjuncts below (layout-unknown / jmp-entry) remain as s71 set them, conservative by construction; they were scoped when the leak was believed structural, so E-4 owes a re-measurement of whether they still earn their place now that the epilogue exists. */
int emit_arbno_rbp_unwind(void) { if (!emit_arbno_rbp() || emit_match_rbp() || !g_emit_cfg) return 0; for (int i = 0; i < g_emit_cfg->n; i++) { IR_t * nd = g_emit_cfg->all[i]; if (nd && (int)nd->op == IR_MATCH_ARBNO) return 1; } return 0; }   /* ⭐ ARBNO-RBP EPILOGUE (E-1, Lon 2026-08-14 "At the end of pattern match ALL RBP are put back in place"): THE ONE AUTHORITY for whether a match exit emits the rbp chain-walk (x86_arbno_rbp_unwind, x86_asm.h -- the walk and this gate are a pair, neither spelled twice).  Three conjuncts, each load-bearing: (1) emit_arbno_rbp() -- no frames are pushed under the killswitch, so the walk would be pure cost and OFF must stay byte-identical; (2) !emit_match_rbp() -- mrbp owns rbp for the MATCH frame itself and releases it with its own mov rsp,rbp/pop rbp whack, so a chain-walk there would fight the frame owner (the two never coexist by qualification; this STATES it rather than assuming it); (3) the graph actually CONTAINS an IR_MATCH_ARBNO -- without this the radius widens from the ARBNO census to every deep-arrival graph, which E-2 requires be either avoided or SAID.  The scan reads g_emit_cfg->all[] (the whole node list) so a declined or bombed ARBNO still counts: those graphs push no frame, the walk is a no-op on them, and counting them keeps this predicate INDEPENDENT of which arm each ARBNO node took -- a gate that tracked arm selection could disagree with the template per node, the s66 half-flip disease. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
