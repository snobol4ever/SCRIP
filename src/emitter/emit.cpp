...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int leaf_zeta_family(int op) { return op == IR_MATCH_SPAN || op == IR_MATCH_BREAK || op == IR_MATCH_BREAKX || op == IR_MATCH_TAB || op == IR_MATCH_RTAB || op == IR_MATCH_REM || op == IR_MATCH_ARB || op == IR_MATCH_BAL; }   /* THE SCRATCH-CELL LEAF FAMILY DEFINITION (Lon 2026-08-20 in-chat: ALL LEAF BBs ARE THE SAME) -- the boxes whose template owns a scratch suspension cell, spelled through LFC. ONE family, EVERY member treated identically: no per-op admission, no per-arm carve-outs; a per-op filter hides a class defect behind box names and is forbidden. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int leaf_frame_member(const IR_t * nd) { return sn4_span_frame() && nd && leaf_zeta_family((int)nd->op) && alt_arm_member(nd, (const IR_t *)0); }   /* EVERY leaf is a member; the ONE condition is LOCATION, never identity. Inside an ALT arm the flat rsp coordinate has no granted owner in either medium (s66/s71 ungranted-arm class), so the cell homes on rbp. On the ordinary spine the offset calculation is exact and rsp stays the home (Lon HQ-60: spine-resident leaves keep RSP -- re-homing them wholesale was measured broken, s127: 120/131/165/181/182). Registry claim width for every leaf is TWO consecutive 16B slots, so the full 32B region has one owner and every spelling the family uses -- LFC(8) included -- is inside its own claim: the old 8-byte usable-window carve-out is DEAD (Lon 2026-08-20). PURE and plan-independent. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int frame_slot_scan(const IR_t * query, int * out_index, int * out_count) {   /* ⭐⭐⭐ ACTIVATION-FRAME SLOT REGISTRY (THREE ZETAS s87): THE ONE SCAN every public authority below calls -- walks
    ...
    for (int j = mb + 1; j < hi; j++) { IR_t * m = g_emit_cfg->all[j]; if (!m || !frame_slot_is_candidate(m)) continue;
        if (j == qi) found = k; k += leaf_zeta_family((int)m->op) ? 2 : 1; }   /* a leaf claims TWO consecutive slots -- one 32B owner -- so k advances by claim width and the interleaved ARBNO/CAPTURE/FENCE1/LEAF numbering stays collision-free by construction */
...
void sn4_blob_choice_scan(int * n_choice, int * leaf_ok, int * has_fence) {   /* ⭐ s127 ONE SCAN, TWO READERS (β-dispatch admission + blob_frame_bytes widening -- never spell the classification twice, the s68/s70 disease): walks THIS blob graph counting unsealed choice nodes, flagging FENCE1, and verifying every non-choice member is a pure scanner-register leaf (the same op-filter as the :1143 span scan; an unrecognised op refuses by construction, the s125 op-filter principle). */
    ...
    if (g_emit_cfg) for (int j = 0; j < g_emit_cfg->n; j++) { IR_t * m = g_emit_cfg->all[j]; if (!m) continue; int o = (int)m->op;
        ...
        switch (o) { case IR_MATCH_LIT: case IR_MATCH_LEN: case IR_MATCH_ANY: case IR_MATCH_NOTANY: case IR_MATCH_POS: case IR_MATCH_RPOS: case IR_MATCH_TAB: case IR_MATCH_RTAB: case IR_MATCH_SPAN: case IR_MATCH_BREAK: case IR_MATCH_REM: break; default: _lf = 0; break; } }
    if (n_choice) *n_choice = _nc; if (leaf_ok) *leaf_ok = _lf; if (has_fence) *has_fence = _fn;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int resume_carrier_ok(int op) {   /* ⭐ s124 CARRIER OP-FILTER, stated BEHAVIOURALLY (no language name, FACT RULE NO-LANGUAGE-IDENTITY): tier 1 = leaf GENERATORS whose beta EXTENDS (one more instance/char) -- the s121 class, admitted unconditionally.  tier 2 (seam walk) = DETERMINISTIC elements whose beta is a self-undo fail-through that jumps LEFTWARD to the previous element's beta; entering the seam at its right end therefore walks the whole chain to the leftmost beta, which alone routes to omega.  The seam is ALREADY BUILT by the element templates (measured s124: `n1_match_len_beta: sub r14d,0; jmp n0_match_arbno_beta`) -- this predicate only stops REFUSING to enter it.  REFUSED and left at the lbl_omega default: DISJUNCTION/ALTERNATE (s121 board conviction -- an ALT carrier reads its 32B arm record at a depth the winning arm shifted; needs its own depth story), DEFER, FENCE1, ASSIGN_* capture wrappers, BREAKX, and every op not named here -- an unlisted op keeps s123 behaviour EXACTLY, so the widening is safe by construction. */
    if (op == IR_MATCH_ARBNO || op == IR_MATCH_ARB || op == IR_MATCH_BAL) return 1;
    if (!sn4_seam_walk()) return 0;
    if (op == IR_MATCH_DEFER) return 1;   /* ⭐ s124 TIER 3, folded in after measurement REFUTED this seat's own first hypothesis: a defer's beta is `jmp qword ptr [rsp]`, which LOOKS unsafe to enter (resume-THROUGH-RECORD, not a self-undo) -- but the record IS live at seam arrival, because the res stub restores rsp to exactly the frontier where the defer's own gamma left it.  Assumption refuted by experiment, not by argument.  EVIDENCE BASE DIFFERS FROM TIER 2 AND IS RECORDED HONESTLY: tier 2 is safe BY CONSTRUCTION (read the element templates -- the beta is a self-undo), tier 3 is safe BY MEASUREMENT (zero row-level movers over 318 crosscheck programs against BOTH baselines, arbnostore 8/9 and seam 4/4 unmoved).  A seat needing to bisect the two tiers separates them by moving this one line below the tier-2 switch. */
    switch (op) { case IR_MATCH_LEN: case IR_MATCH_POS: case IR_MATCH_RPOS: case IR_MATCH_TAB: case IR_MATCH_RTAB: case IR_MATCH_LIT: case IR_MATCH_ANY: case IR_MATCH_NOTANY: case IR_MATCH_SPAN: case IR_MATCH_SPAN_VAR: case IR_MATCH_BREAK: case IR_MATCH_REM: return 1; default: return 0; }
}
...
extern "C" int sn4_choice_rbp(void) {   /* ⭐ s128 THE ONE AUTHORITY for the SCRIP_CHOICE_RBP killswitch (GOAL-SNOBOL4-100 next-rung: THE NON-LEAF ALT-CARRIER DEPTH STORY, witnesses probe/altdepth/).  Design of record (s127 cursor): the ALT's arm record moves off the rsp frontier into the blob ACTIVATION frame -- per-entry = per-ARBNO-iteration (D12 recursion already requires per-activation frames) -- and res re-seats rbp from record[+24] (already built, :3139), making the arm-resume read DEPTH-IMMUNE regardless of what the winning arm carved.  =1 opt-in; default OFF byte-identical (land OFF, sweep before any flip -- the R-7/s124 protocol). */
    static int _cr = -1; if (_cr < 0) { const char * e = getenv("SCRIP_CHOICE_RBP"); _cr = (e && *e == '1') ? 1 : 0; } return _cr;
}
static int blob_choice_rbp_scan(void) {   /* ⭐ s128 ADMISSION for the rbp-resident choice record, ONE SCAN consumed by blob_frame_bytes (+32), sn4_choice_rbp_off (the template/dispatch offset) -- lockstep by construction.  Admits EXACTLY the refused-leaf single-choice blob (_nc==1, !_lf, !_fn: the captarm/beauty-Command class) and ONLY when the blob is ALREADY FRAMED by existing law (registry demand or wire-clobber): the s127 zd conviction stands VERBATIM -- this predicate NEVER frames a zero-demand blob, so no ZD-priced caller-territory cell ever meets a frame it was not priced against.  Leaf blobs (_lf) keep the proven s127 contiguity arm; _nc>1 stays the refused two-record-composition class (s126); _fn stays the FENCE semantic refusal (manual p.125/p.204). */
    ...
    if (_nc != 1 || _fn || _lf) return 0;
    if (blob_wire_clobber_scan()) return 0;   /* ⛔ s128 SAME-SESSION TIGHTENING, MEASURED BEFORE LANDING: the wide cut (clobber blobs admitted) cured captarm but MOVED TWO -- 150_pat_star_var_fence_alts_no_arbno PASS->FAIL and beauty ON rc=0@10/622 -> rc=139@7/622.  A wire-crossing arm interior (DEFER/CALL/VALUE/CALLOUT) suspends through ITS OWN records and re-seats r10/r11 for the INNER crossing; the rbp choice record makes only the OUTER arm-resume depth-immune -- resuming such an arm needs the inner story too (the named residue).  Registry-demand-framed carving arms (captures: the captarm/cap_in_alt_arm class) are the proven slice. */
    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_β);
    {
        ...
        { extern int sn4_defer_resume(void); const int _dres = sn4_defer_resume();   /* ⭐ SN4-DEFER-RESUME (s121) half B2: the SNOBOL4 arm of the resume dispatch, mirroring the two Prolog arms above.  A PAT$ blob (flat_pat graph) β-arrival — the invoker's `jmp [rsp+0]` through the CLASS D suspension record, res stub restored wires+frame, interior state retained on the spine below — lands the published resume carrier's β: a generator's β EXTENDS (ARBNO one more instance, manual p.121; ARB one more char), a deterministic carrier's β fail-throughs to its ω and rides r11 home through the absolute unwind.  Without this arm the dispatch defaulted to lbl_ω: every stored ARB/ARBNO answered exactly once and took its shy null match forever — the s120 FINDING's closed SUCCEED/FAIL pair, beauty's `Parse Error`.  Same switch as the lowering half; =0 restores the default byte-identically. */
          if (_dres && g_emit.flat_pat && g_emit_cfg && g_emit_cfg->body_root) {
            ...
            if (!_f1) for (int i = 0; i < n; i++) if (nodes[i] == g_emit_cfg->body_root && resume_carrier_ok(nodes[i]->op)) { resume_tgt = betas[i]; break; }
            if (!_f1 && resume_tgt == &lbl_ω && sn4_alt_carrier() && g_emit_cfg->body_root && (g_emit_cfg->body_root->op == IR_MATCH_ALTERNATE || g_emit_cfg->body_root->op == IR_DISJUNCTION) && !g_emit_cfg->body_root->seal) {
                int _nc = 0, _lf = 0, _fn = 0; sn4_blob_choice_scan(&_nc, &_lf, &_fn);
