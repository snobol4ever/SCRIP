...
#undef RPO_VISITED
    ...
    if (scan_live) {
        ...
        if (g_emit_cfg && g_emit_cfg->pl_cells_graph) { int _plk = 0; for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _nd = g_emit_cfg->all[_i]; if (_nd && zd_wl_kind(_nd)) _plk += zd_k(_nd); } if (_plk > 0) { bb_emit_x86(x86("sub", "rsp", (long)_plk)); } }   /* ⭐ PL-ZK-3: PROC-ENTRY CELLS CARVE (s5).  The zframe prologue (xa_flat_zframe_prologue) carves flat_frame_bytes for the ZLS frame and pins rbp=rsp.  When pl_cells_graph=1 the planner additionally stages K_total=Σzd_k(admitted) bytes of per-BB cells on the FORTH spine BELOW the ZLS frame.  Without this sub rsp,K_total the planner's staged K_total is accurate but RSP at the first ZOPQ(0,0)=[rsp+0] read points into the CALLER'S frame (K_total bytes above the ZLS floor), producing the SEGV confirmed on queens.pl census (rc=139 with armed=21/12/17).  FIX: sum zd_k over all admitted nodes — THE ONE AUTHORITY (zd_k, same function the planner calls; this scan calls it directly to avoid a second spelling) — and emit sub rsp,_plk immediately after the zframe prologue while rbp is pinned.  CORRECTNESS: _plk is the STATIC maximum cells depth (Σ over all arms admitted, non-popping — they stack); the epilogue's lea rsp,[rbp+flat_frame_bytes] unwinds to EXACTLY before these cells from ANY in-body rsp depth (depth-immune by rbp pin).  GATED pl_cells_graph: SN4 never sets pl_cells_graph (lower_snobol4.c has no pl_cells_stamp call); Icon uses icn_cells_graph on a mutually exclusive path (R-ZK-A); byte-identical for both by construction.  KILLSWITCH: SCRIP_PL_CELLS=0 leaves pl_cells_graph=0 (pl_cells_stamp nops) — this arm not reached.  BOTH-MEDIUM: bb_emit_x86(x86("sub",...)) routes through the x86() encoder (BINARY: inline ef_b3+bb_emit_u32; TEXT: asm string) — TEMPLATE-ONLY per RULES.md.  ONE AUTHORITY: this site is the only proc-entry cells carve; zd_plan's K_total and this scan agree by construction (both call zd_k, the ONE K AUTHORITY).  s22k law: _plk is computed by the ONE K function, never re-spelled. */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0 || zd_zw[i] || zd_zwr[i]) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = _uw_stolen ? (int)_uw_pop : ((_zw5_wpop_stolen || _endj_stolen) ? 0 : zd_wp[i]); g_zd_wsteal = _endj_stolen || _uw_stolen; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i]; g_zd_uzw = zd_zw[i]; g_zd_uzwr = zd_zwr[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; mechanism-2 g_zd_uzwr staged alongside g_zd_uzw */
