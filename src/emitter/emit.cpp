...
#undef RPO_VISITED
    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_γ); bb_emit_x86(_wire_stub ? bb_glue_wire_γ() : bb_glue_outer_γ());
    emit_sep_rule('-'); emit_label_define_bb(&lbl_ω); bb_emit_x86(_wire_stub ? bb_glue_wire_ω() : bb_glue_outer_ω()); }   /* OUTER-EXIT-1 (Lon s22p): the outermost box owns its OWN two ports, as glue -- this is the whole of what the deleted epilogue did in the ordinary case.  lbl_ω was left UNDEFINED by the s22o deletion while every fail edge still jumped to it, which is the single reason `--compile` produced no assemblable .s for 317/317 programs: one undefined symbol, not 317 defects.  ONE-SHOT BRIDGE (s22p): lbl_gamma and lbl_omega are jmp targets inside main -- no ret, no eax. bb_glue_outer_whack=true; framed_leave is the depth-immune exit; bb_glue_flat.cpp owns the protocol. */
