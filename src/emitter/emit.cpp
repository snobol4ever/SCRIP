...
static int zd_wl_kind(IR_t * nd) {
    ...
    if (op == IR_MAKE_LIST) { static int _zkml = -1; if (_zkml < 0) { const char * e = getenv("SCRIP_ZD_ML"); _zkml = (e && *e == '0') ? 0 : 1; } return (_zkml && g_emit_cfg && g_emit_cfg->icn_cells_graph) ? 1 : 0; }   /* ⭐ ZK-2 ADDITIVE (s228): IR_MAKE_LIST admitted on the cells arm -- deterministic value producer (no β resume port), lowered as straight γ chain from bounded element evaluations (ir_a_ListConstructor ir_MakeList ir_Goto success; canonical: refs/jcon-master/tran/irgen.icn:1346).  K=16 default (zd_k; result DESCR only -- staging area reuses ZLS-granted flat frame at FRQ(op_off+16+i*16) which is rbp-relative and depth-immune under flat_lcl_proc's pinned rbp).  nops=n->n_operands (zd_nops line 2812 already correct -- elements are inline run-predecessors staged via ZOPQ).  ZD arm in bb_make_list.cpp reads each element from ZOPQ(i,0/8) into the staging area, calls rt_make_list, writes result to ZRES(0/8).  Guard x86_fb_pinned() in template: staging area address FRQ(op_off+16) is rbp-relative, depth-immune only when rbp is pinned (flat_lcl_proc prologue pins it; unpinned rsp arm would need a different staging strategy).  SN4 watermark: lower_snobol4.c never emits IR_MAKE_LIST; byte-identical by construction.  Killswitch SCRIP_ZD_ML=0.  ONE AUTHORITY per s22k law -- only zd_wl_kind and zd_k name this kind for the ZD regime. */
