 ...
 * including the tiny call-stub the SAME DEFINE also mints -- sno_build_call_stub, IR_DEFINE ->
...
static int zd_nops(IR_t * nd);   /* forward: defined below (ZD one-authority operand count); the PF-1c choke staging maps op_zread_xf[k] over the same count the generic staging loop uses */
static int alt_arm_member(const IR_t * a, const IR_t * b) {   /* ⭐⭐⭐ THE ONE ALT-ARM CONTAINMENT AUTHORITY (s130): is either node ON some ALT arm's γ-chain in this run?  Every IR_MATCH_ALTERNATE in the statement is walked arm by arm, arm j = the γ-chain from operands[2j]
     ...
     * spelled ONCE and its two customers -- the CAPTURE half (cap_in_alt_arm) and the LEAF-SUSPENSION half (leaf_frame_member, s130) -- cannot drift apart: they are the same ζ-SPINE denial, so they must
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int arbno_reentry_hazard(const IR_t * nd);   /* ⭐ defined beside the ARBNO-FRAME registry below; forward-declared here because the IR_MATCH_ARBNO drive case stages the SAME fact (op_arbno_body_defer_unsafe) and both readers must derive from ONE authority (s189, the s124 two-reader law) */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int zd_k(IR_t * nd);   /* forward: THE ONE K AUTHORITY (s22k law), consulted by fence_body_kk so the committed-body ΣK is never spelled twice */
static int fence_body_kk(const IR_t * nd) {   /* ⭐ R-4(a) SLICE 2 (s95): IR_MATCH_FENCE1 body-span ΣK over [operands[0]..operands[1]] (all[]-order, lower_snobol4.c:1360 pe/p_tail) -- the twin of the ARB-LON-K16 prelude's _kk loop, same container/transfer exclusions (a body holding one of those is not the static-K class and stays K=0, i.e. byte-identical to HEAD). Consumed ONLY by bb_match_fence1's σ-commit release (FRQ(off+32+kk)); see emit.h op_fence_body_kk. Pure, plan-independent, no new global. */
...
static int fence0_whack_on() { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_FENCE0_WHACK"); v = (e && *e == '0') ? 0 : 1; } return v; }   /* ⭐ FZ-3 (s168): MOVED HERE VERBATIM FROM bb_match_fence0.cpp -- NOT a new global, the SAME one function-static getenv cache relocated, and the template's copy is deleted in the same commit so the count of arm spellings stays exactly one.  IT HAS TO LIVE ON THIS SIDE because the ζ DEPTH PLANNER must ask the question too: FZ-3 threads the release through zd_plan's depth accumulator, and a template-side gate is invisible there, so an armed planner would meet a disarmed template (or the reverse) and every staged [rsp+off] in the statement would be skewed by exactly the release -- the planner/template disagreement the s66 coherent-worlds law convicts and the s22k ONE-AUTHORITY law forbids.  DEFAULT ON as of the s172 HQ desk ruling (Lon delegated the desk): the arm was measured before it was flipped -- 8 of 8 fz witnesses oracle-green ARMED in BOTH modes and on BOTH depth spellings, and a 1034-program corpus differential whose entire m4 blast radius is those same 8 programs (m3: zero movers).  SCRIP_FENCE0_WHACK=0 IS THE KILLSWITCH AND STAYS: it restores the pre-flip box byte-for-byte (0 bytes => byte-identical to the no-whack box), which is the BASELINE-ARM law's whole point -- the retreat is one env var, not a revert. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int fence0_release_bytes(const IR_t * nd) {   /* ⭐ FZ-1 THE FENCE0 RELEASE PLAN, WALKED IN EXECUTED ORDER (FZ-2, s166 -- this is the reconciliation the s154 cursor demanded, and the answer was neither of its two hypotheses).  THE DEFECT IT RETIRES: the first cut walked g_emit_cfg->all[] by descending index and CALLED THAT CARVE ORDER.  It is not.  all[] is ALLOCATION order and SNOBOL4 lowering is RIGHT-FIRST, so on the s154 witness `S ? SPAN('a') FENCE SPAN('b') SPAN('c')` the two SPANs sitting immediately left of the fence in all[] (25, 26) are 'b' and 'c' -- both carved AFTER the fence runs -- while 'a', the ONE genuinely dead cell, sits at 28, to the RIGHT, and was never counted.  Measured with SCRIP_FZ_DIAG: the walk billed 16+16=32 where the emitted carve holds exactly one releasable 16 (`n7_match_span_α: sub rsp,16`) standing on MATCH_BEGIN's LIVE 24 -- so `add rsp,32` popped 16 bytes of the bracket's own cell.  THAT IS THE SAME OVER-RELEASE-INTO-LIVE-STORAGE CLASS THAT CORED THE FLOOR WHACK, reached by a different road and quiet only because no witness read the clobbered half before MATCH_END rebuilt it.  THE FIX IS TO STOP GUESSING AT ORDER AND READ THE ONE THAT EXISTS: the emitter lays cells down in the RPO nodes[] order zd_plan consumes (its own ZK-2 note: "nodes[] is execution order"), and that order is recovered from the graph by chasing γ -- so this walks γ from the enclosing MATCH_BEGIN to this fence through zd_chase(), THE SAME edge-following authority the planner walk uses, and only then steps back down the recovered prefix.  Nothing else changes: the op-filter is still the seven leaves whose 16B cell is their own cnt/cur retry state (dead once the cut forbids re-entry, result delivered in r13/r14/r15 and not through the cell), ASSIGN_SAVE is still excluded BY NAME (its cell is the SAVE cursor the paired COND/IMM reads back), contiguity is still the LIFO law rather than a simplification, and the bracket is still the wall.  ONE RULE ADDED: the control kinds fence_body_kk and fence_frame_candidate already exclude by name (ALT/ARBNO/FENCE/DEFER/VALUE/CALL/DISJUNCTION/ABORT) now STOP the walk even at K=0, because a suspension or choice record with no ζ cell of its own still owns spine below it -- the old "K=0 neither contributes nor breaks contiguity" leniency is kept only for genuinely inert kinds.  A fence not reachable on any bracket's forward chain bills 0, which is byte-identical to the disarmed box.  Pure, plan-independent, no new global, no new IR_t field (PEERS RULE). */
    if (!fence0_whack_on() || !x86_port_cstack()) return 0;   /* ⭐ FZ-3 (s168) THE ARM GATE IS PART OF THE COUNT, NOT PART OF THE TEMPLATE.  This function is now THE ONE AUTHORITY on "how many bytes does this cut actually spend", and it answers 0 whenever the cut will not spend any -- disarmed, or a non-cstack port whose FENCE0 box emits no release.  BOTH customers read the same answer: the FENCE0 dispatch stages it into op_fence0_release (what the template spends) and zd_plan subtracts it from the depth accumulator (what every [rsp+off] right of the cut is staged against).  The template's own fence0_whack_on()/x86_port_cstack() test is DELETED, not duplicated: with the gate here, rel==0 disarmed makes the emitted box byte-identical to the no-whack box exactly as before, and the two worlds cannot drift apart. */
    ...
    for (int j = ns - 1; j >= 0; j--) { IR_t * m = seq[j]; int mo = (int)m->op;
        ...
        break;                        /* anything that CARVES and is not op-filtered pins the region — stop */ }
    if (_fzd) fprintf(stderr, "[FZ-DIAG] fence %p RELEASE=%d\n", (const void *)nd, total);
    return total;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int fence_frame_candidate(const IR_t * nd) {   /* ⭐ R-4(f) SLICE 3 (s96): IR_MATCH_FENCE1 whose body span [operands[0]..operands[1]] holds a container/transfer member (the SAME exclusion list fence_body_kk refuses on -- a body that is not the static-K class): its watermark cannot be read RSP-relative at any static offset (H08 FENCE(ARBNO(LEN(1))): α stores the quad, ARBNO's K16 commit leaves the frontier 16B/instance deeper at σ; H29 FENCE(TAB(2) $ OUTPUT | ABORT): the taken ALT arm carves 32B), so it rides a shared-registry slot in the CURRENT ACTIVATION FRAME. Pure, plan-independent, no new global. */
    if (!nd || !g_emit_cfg || nd->op != IR_MATCH_FENCE1 || nd->n_operands < 2) return 0;
    if (IR_LIT(nd).ival == 2) return 1;   /* ⭐ DYNAMIC ENTRY DEPTH (this rung): the scan below asks whether MY BODY moves the frontier, but a FENCE1 in an ARBNO body (ival=2, stamped lower_snobol4.c:1695) is ENTERED at a different rsp every iteration, so no static RSP offset is correct for it either -- measured: the +176 quad lands on the blob head's [rbp-24] DTP cell (blob_head_bytes) at iteration 2 and DEFER then reads the watermark as its pattern operand. */
    int lo = -1, hi = -1; for (int k = 0; k < g_emit_cfg->n; k++) { if (g_emit_cfg->all[k] == nd->operands[0]) lo = k; if (g_emit_cfg->all[k] == nd->operands[1]) hi = k; }
    if (lo < 0 || hi < 0) return 0; if (lo > hi) { int t = lo; lo = hi; hi = t; }
...
static int frame_need_of(const IR_t * nd) {                                                      /* ⭐ EARN-1 SLICE 1 -- THE ONE AUTHORITY.  THE LAW: a cell needs a frame iff its byte distance to RSP is not a compile-time constant at some reading site.  Verdict map + placeholder caveats documented on op_frame_need (emit.h). */
    ...
    case IR_MATCH_FENCE0:       return 1;   /* ⭐ FENCE-SPLIT: preserves the pre-split answer — a bare FENCE arrived here as IR_MATCH_FENCE1 and returned 1 */
    case IR_MATCH_FENCE1:       return 1;
    case IR_MATCH_ASSIGN_SAVE:  {                                                                /* ⭐ s83 SAVE-PROPAGATION FIX: earn_hazard_in walks a node's OWN operand subtree, but SAVE is a pure producer with no relevant operands of its own -- the hazard (a DEFER/VALUE between SAVE and its reader) lives entirely in the PAIRED COND/IMM's operand[0], which SAVE has no pointer to (the link runs the other way: COND/IMM.operands[1] == this SAVE, per lower_snobol4.c:1509/1546).  s82 MEASURED this asymmetry directly (gdb: cur_delta correct, saved_delta garbage) after IMM alone reported need=1 and SAVE's own frame_need_of() unconditionally read 0.  FIX: scan g_emit_cfg->all[] (the SAME whole-graph accessor stmt_has_proc_invoke in bb_match_begin.cpp already uses for an analogous forward-reference walk -- no new global, no new IR_t field, PEERS RULE respected) for the COND/IMM whose operands[1] IS this SAVE, and borrow ITS verdict.  Bounded scan: g_emit_cfg->n is one statement/chain's node count, not the whole program. */
        if (!g_emit_cfg) return 0;
        for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _c = g_emit_cfg->all[_i];
            if (_c && (_c->op == IR_MATCH_ASSIGN_COND || _c->op == IR_MATCH_ASSIGN_IMM) && _c->n_operands > 1 && _c->operands[1] == nd) { int _old = earn_hazard_in(_c, 0); int _new = frame_need_of(_c); if (getenv("SCRIP_CAP_NEST_DIAG") && _old != _new) fprintf(stderr, "[CAPNEST-SAVE-DIVERGE] c=%p old_haz=%d new_fno=%d\n", (void*)_c, _old, _new); return _new; } }   /* EARN-CAP-NEST FIX: borrow the PAIRED COND/IMM's FULL verdict (frame_need_of, which now includes the nested-span scan below), not just earn_hazard_in -- calling the raw hazard walk here bypassed the fix entirely, since SAVE's own need is defined as "whatever my reader needs". s83c re-derivation (this session): the killswitch-off non-parity this diag was added to chase was RE-INVESTIGATED and FALSIFIED as a real bug -- see the commit message / cursor entry. Root cause was a CONTAMINATED COMPARISON BASELINE (a .s snapshot accidentally captured with the fix's default arm already active, mistaken for pristine HEAD), not a g_emit_cfg scoping defect. Re-verified against a genuinely clean, freshly-stashed-and-rebuilt unpatched binary: all 6 originally-named files (fn_call_in_pat/Qize/Qize_driver/omega_driver/XDump_driver/beauty) plus dv_defer_nested and dv_value_imm are BYTE-IDENTICAL between the unpatched binary and this patched binary run with SCRIP_CAP_NEST=0, on every one of 676 swept files (corpus/programs/snobol4, probe/dv, crosscheck, SCRIP/test/snobol4, SCRIP/demo/snobol4). The one non-matching sweep entry, unary_not.sno, is confirmed run-to-run non-deterministic (3 consecutive same-binary same-arm compiles gave 3 different md5s) -- a pre-existing ASLR-class artifact, unrelated, exactly as s81 already flagged it. Diag line kept as a cheap future tripwire. */
        return 0; }
    case IR_MATCH_ASSIGN_IMM:
    case IR_MATCH_ASSIGN_COND:  {                                                               /* EARN-CAP-NEST FIX (s81, dv_defer_nested witness): earn_hazard_in(nd,0) walks nd->operands[] -- for COND/IMM that is [inner-pattern ENTRY node, own SAVE] -- so it sees the entry node's OWN operand subtree but NOT the box-chain BETWEEN entry and this node, reachable only by graph ALLOCATION order, not by operand recursion. A capture nested INSIDE another capture's span (e.g. `(*P $ X 'D') . Y`: Y's COND wraps a group whose interior is X's own SAVE/DEFER/IMM/LIT chain) has its hazard invisible to the outer COND's operand walk -- op_frame_need's own doc comment (emit.h:628) named this exact gap ("conditional rows under-detect toward 0") as a known placeholder limitation, unwitnessed until now. MEASURED (printed-value class per s50): dv_defer_nested's oracle is `M BC BCD`; pre-fix HEAD prints `M BC ABCD` -- Y's outer COND read its start-cursor from BEFORE 'A' matched instead of from its own SAVE's delta, because it silently took the ordinary ZRES spine path (need=0) while an activation frame was actually required. FIRST ATTEMPT (superseded, left as history): a raw index-order scan between this node's own index and its SAVE's index in g_emit_cfg->all[] -- FALSIFIED by SCRIP_CAP_NEST_DIAG: COND is allocated BEFORE its own SAVE (lower_snobol4.c's TT_CAPT_COND_ASGN builds `nd` first, `save` second, per its own comment "COND is allocated FIRST"), so the naive [si,ci) range was empty by construction (si=35 > ci=34 on the witness) and the scan silently found nothing. FIX: use fc_pair_extent(nd) -- already registered at lowering time (lower_snobol4.c:1532/1560, "inner allocation ends here") as the exclusive upper bound of THIS capture's own inner span -- as the scan's upper bound, with max(own-index, SAVE-index)+1 as the lower bound (both COND and SAVE precede the inner body in allocation order, whichever of the two is allocated second still precedes the body). ADDITIVE ONLY: h starts as the existing verdict and the scan can only raise 0->1, never lower 1->0. NO NEW GLOBAL (g_emit_cfg and fc_pair_extent's side table both already exist, PEERS RULE respected). Killswitch SCRIP_CAP_NEST=0 restores the pre-fix operand-walk-only verdict byte-identically -- RE-VERIFIED s83c (see IR_MATCH_ASSIGN_SAVE comment above): the earlier "unresolved parity" note was itself a measurement error, not a defect in this scan. */
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    { extern int zdp_alpha(const IR_t *); extern int zdp_beta(const IR_t *); extern int zdp_known_alpha(const IR_t *); extern int zdp_known_beta(const IR_t *); extern int zdp_mode(void);   /* ⭐⭐⭐ LON'S EVERY-PORT PROBE (s136), STAGING HALF.  Lon in-chat: "Instrument each BB graph's FIRST BB to save RSP (RSP0), then instrument every ALPHA and BETA to check the diff of RSP to this RSP0 and report if different than SCRIP's CALCULATED expected value."  THE CALCULATED VALUE IS THE LATTICE'S -- zdp_alpha/zdp_beta, read here and NEVER re-derived (the s133 conviction: a transfer function that re-spells a number another authority owns is a SECOND OPINION, which is how the carve bug got in).  Staged at THIS choke, beside op_zdepth, because it is the one place that already knows the node and the templates do not. */
      ...
      g_emit.op_zdp_rbp = (nd && (nd->op == IR_MATCH_BEGIN || nd->op == IR_MATCH_DEFER)) ? 1 : 0; }   /* the RBP-CREATING SET, MEASURED not assumed (s136): the only live `push rbp` producers reachable from a SNOBOL4 graph are bb_match_begin.cpp:46 and bb_match_defer.cpp:55 -- bb_define.cpp's two are behind fnrbp()==1 which the :494 clamp aliases to 2 (VERIFIED: SCRIP_FN_RBP=1 output byte-identical to default on fn_plain/fn_recur), and bb_match_arbno.cpp:256 is never called.  ⛔ NO RBP FRAME IS EVER CREATED FOR A FUNCTION (Lon's ruling s136, then measured: 0 push-rbp in a plain DEFINE and 0 in a RECURSIVE one; the single hit in a function containing `S ? P` is the MATCH's own ζ-STANDING).  The R-4(b) blob activation is emitter-level (emit.cpp:2891) and joins this set at the graph-entry origin hook, not here. */
    ...
    if (nd) { g_emit.op_xf_off = xop_frame_slot(nd); int _xn = zd_nops(nd); if (_xn > 6) _xn = 6; for (int _zi = 0; _zi < _xn && _zi < nd->n_operands; _zi++) g_emit.op_zread_xf[_zi] = xop_frame_slot(nd->operands[_zi]); }   /* ⭐ PF-1c/PF-1d (s178) staged at the ONE choke UNCONDITIONALLY (PF-1d moved it out of the armed guard: the ungranted arm-interior consumer is precisely NOT zd-armed, and its XSAQ/XSAD seat needs op_zread_xf staged), BOTH halves from THE ONE AUTHORITY xop_frame_slot: the producer's own home (op_xf_off, ZRES renders rbp when != -1) and each operand's producer home (op_zread_xf[k]; ZOPQ on the armed road, XSAQ/XSAD on the legacy seat) -- computed pure from nd, no staging channel; non-positional seats (COND/IMM's SAVE, REPLACE's subject) refuse via the member predicate and stay -1 = spine. */   /* ZD-1: cleared at the choke UNCONDITIONALLY so glue and non-drive fills can never inherit a stale regime.  op_ztail (ZK-2): 0 for all non-IR_TO nodes; staged for IR_TO in codegen_flat_chain_body before the staging globals are consumed here.  op_zfc (L-3b, this session): 0 for all non-IR_MATCH_REPLACE nodes, mirrors op_zpat's clear. */
    ...
    case IR_MATCH_ARBNO:          { bb_prepare(nd); g_emit.op_arbno_rbp = emit_arbno_rbp();   /* ⭐ ARBNO-RBP (Lon 2026-08-14): staged from THE ONE AUTHORITY (beside emit_match_rbp); the template keys purely on this field, zd_k reads the same predicate -- the killswitch cannot half-flip (s66). */
                                    g_emit.op_arbno_frame_off = arbno_frame_slot(nd);   /* ⭐ ARBNO-FRAME SLOT (THREE ZETAS s86): staged from THE ONE AUTHORITY beside emit_match_rbp/emit_arbno_rbp -- -1 unless this ARBNO's body is DEFER-unsafe AND emit_match_rbp() is on, in which case bb_match_arbno.cpp's W-7/K0-DEFER bombs step aside for the ARBNO-FRAME arm instead. */
                                    extern int fc_tail_arbno(const IR_t *, int *, int *, int *, int *); extern int fc_tail_ncap(const IR_t *); int _fpb = 0, _fpl = 0, _osb = 0, _hdr = 0;
                                    int _tailc = (x86_zc_frame() == ZC_FRAME_RSP && fc_tail_arbno(nd, &_fpb, &_fpl, &_osb, &_hdr)) ? 1 : 0;   /* ⭐ hoisted ONCE (const-pure analysis, no staging): the prelude below needs _fpb -- outer pending SAVE cells riding the frontier across this ARBNO.  K16's non-popping commits would interpose between such a cell and its COND's static-distance read (166_pat_arbno_cond_assign_commit witnesses this), so _fpb>0 refuses to the carry-the-tail arm which exists precisely to relocate those cells. */
                                    int _k16r = 0; { static int _k16e = -1; if (_k16e < 0) { const char * _e1 = getenv("SCRIP_ARBNO_K16"); const char * _e2 = getenv("SCRIP_ARBNO_FRAMELESS"); _k16e = ((_e1 ? (atoi(_e1) != 0) : 1) && (_e2 ? (atoi(_e2) != 0) : 1)) ? 1 : 0; } if (_k16e && x86_port_mode() == ZC_PORT_FORTH && g_emit_cfg && nd->n_operands >= 3) { int _ni=-1; for(int _j=0;_j<g_emit_cfg->n;_j++) if(g_emit_cfg->all[_j]==nd){_ni=_j;break;} int _fr=0; if(_ni>=0) for(int _j=0;_j<g_emit_cfg->n&&!_fr;_j++){IR_t*_o=g_emit_cfg->all[_j]; if(_o&&_o!=nd&&_o->op==IR_MATCH_ARBNO&&_o->n_operands>=3){int _a=-1,_b=-1; for(int _k=0;_k<g_emit_cfg->n;_k++){if(g_emit_cfg->all[_k]==_o->operands[1])_a=_k;if(g_emit_cfg->all[_k]==_o->operands[2])_b=_k;} if(_a>_b){int _t=_a;_a=_b;_b=_t;} if(_a>=0&&_ni>=_a&&_ni<=_b)_fr=1;}} int _k0=1,_kk=0,_sq=1; { int _s0=-1,_s1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_s0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_s1=_j;} if(_s0>_s1){int _t=_s0;_s0=_s1;_s1=_t;} if(_s0<0){_k0=0;_sq=0;} else for(int _j=_s0;_j<=_s1;_j++){IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m!=nd){int _mo2=(int)_m->op; if(_mo2==IR_MATCH_ALTERNATE||_mo2==IR_MATCH_ARBNO||(_mo2==IR_MATCH_FENCE1 || _mo2==IR_MATCH_FENCE0)||_mo2==IR_MATCH_DEFER||_mo2==IR_MATCH_VALUE||_mo2==IR_CALL||_mo2==IR_CALL_VALUE||_mo2==IR_DISJUNCTION||_mo2==IR_MATCH_ABORT)_sq=0; int _mk=zd_k(_m); if(_mk!=0)_k0=0; _kk+=_mk;}} } int _osv=0,_dmb=-2,_dme=-2; { int _mb=-1,_hi=g_emit_cfg->n; for(int _j=_ni;_j>=0;_j--){if(g_emit_cfg->all[_j]&&g_emit_cfg->all[_j]->op==IR_MATCH_BEGIN){_mb=_j;break;}} if(_mb>=0) for(int _j=_mb+1;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]&&g_emit_cfg->all[_j]->op==IR_MATCH_BEGIN){_hi=_j;break;}} if(_mb>=0){ int _s0=-1,_s1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_s0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_s1=_j;} if(_s0>_s1){int _t=_s0;_s0=_s1;_s1=_t;} for(int _j=_mb+1;_j<_hi;_j++){ if(_j>=_s0&&_j<=_s1) continue; IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m->op==IR_MATCH_ASSIGN_SAVE)_osv=1; } } else _osv=1; _dmb=_mb; _dme=_hi; }   /* ⭐ OUTER-pending scan: a SAVE inside this match bracket but OUTSIDE the body span parks a cursor cell on the frontier ACROSS this ARBNO -- K16 commits would interpose between that cell and its COND static-distance read (166 witnesses).  Body-interior SAVEs travel WITH the instance and are exactly what the arm re-homes, so only the outer kind refuses. */ if (_sq && !_k0 && _kk > 0 && !_fr && !_osv) { _k16r = 1; g_arbk16_stmt = 1; g_emit.op_arbno_framed = 0; g_emit.op_arbno_body_k0 = 0; g_emit.op_arbno_body_kk = _kk; }   /* framed (nested-in-another-ARBNO) context REFUSES: under a legacy chain outer, an inner activation must leave net-zero stack delta (the chain's per-iteration block arithmetic cannot represent a variable count of surviving committed cells) -- K16's non-popping commits violate that by design, so depth>0 keeps legacy until the outer chain itself is replaced (X03/X05 witness this law) */   /* ⭐⭐⭐ ZETA-ACTIVATION UNIFICATION -- DEFER ADMISSION TRIED AND REVERTED THIS SESSION, RECORDED SO THE NEXT ATTEMPT DOES NOT REDO THE SAME MEASUREMENT: admitting pat_static==1 IR_MATCH_DEFER members into _sq (matching W-7's own !pat_static unsafe test) routes arb1.sno-class bodies
                                     * to FRAMELESS_K correctly ON THE ADMISSION SIDE (sq=1 confirmed, arm correctly selected) but the RESULT IS STILL A SIGSEGV, gdb/asm-confirmed: zd_k(IR_MATCH_DEFER)==16 (the general non-PATV$ fallback K, NOT the ZD-PATREF K=0 class) describes the
                                     * DEFER NODE's OWN static alpha-carve -- it does NOT and CANNOT describe what the DEFER's TARGET PATTERN (P in ARBNO(*P)) carves once control transfers into its separately-compiled proc blob at runtime.  frameless_k's kkN()-relative addressing
                                     * needs the TOTAL transitively-reachable carve to be a compile-time constant; zd_k gives only the DEFER call-site's own footprint, one level short.  The codebase already has the right primitive for this (g_last_flat_uniform / zls_g_fp_total, PS-3
                                     * s152's "uniform suspension footprint... align16(32+fb)+fp+16" for a zstatic/uniform target), but it is captured as a "just-emitted graph" snapshot at the TARGET's own emission time, not indexed by pattern name for later query from the ARBNO's
                                     * host-graph classification pass -- wiring that lookup is real, separate, cross-cutting work (a footprint registry keyed by DEFER target name, available before the ARBNO body scan runs), not a two-line predicate fix.  REVERTED to the pre-attempt
                                     * _sq test (DEFER excluded unconditionally) so this admission cannot silently re-open the aliasing class arb1.sno's header already named; the K0-defer compile-time bomb in bb_match_arbno.cpp is the correct, honest state for this class until that
                                     * footprint plumbing lands as its own measured rung. */ { static int _ad=-1; if(_ad<0){const char*_e=getenv("SCRIP_ARBNO_DIAG"); _ad=(_e&&*_e=='1')?1:0;} if(_ad) fprintf(stderr,"[ARBNO-K16] framed=%d k0=%d sq=%d kk=%d osv=%d mb=%d me=%d route=%s\n", _fr, _k0, _sq, _kk, _osv, _dmb, _dme, _k16r?"FRAMELESS_K":"legacy"); } } }   /* ⭐ ARB-LON-K16 PRELUDE (this rung): the containment+ΣK scan runs BEFORE the tail claim so the sequence-only static-K body class (SAVE cells; kk = Σ zd_k over the span, THE ONE K AUTHORITY) routes to the re-homed frameless arm INSTEAD of fc_tail -- this is the ONE routing authority (env read HERE only; the template keys purely on the staged op_arbno_body_kk field, so the killswitches cannot half-flip).  SCRIP_ARBNO_K16=0 (or SCRIP_ARBNO_FRAMELESS=0) makes this prelude inert and every byte downstream identical to HEAD.  Container/transfer kinds in the span (path-dependent or unbounded frontier at body-γ) refuse to the legacy routes verbatim. */
                                    if (!_k16r && 0 && _tailc) { g_emit.op_tail = 1;   /* ⛔ D-1 DELETE: carry-the-tail claim RETIRED -- the tail arm is gone, so its geometry must not stage; zls_arbno_geom below is now the sole ARBNO geometry authority */ g_emit.op_sb = _osb; g_emit.op_sa = _hdr; g_emit.op_tail_fpb = _fpb; g_emit.op_tail_fpl = _fpl; g_emit.op_tail_ncap = fc_tail_ncap(nd); g_emit.op_tail_seal = (nd->n_operands > 3 && nd->operands[3] == nd) ? 1 : 0; g_emit.op_tail_dfr = fc_tail_dfr(nd); { extern int fc_tail_fpr_rsp(const IR_t *); g_emit.op_tail_fpr_rsp = fc_tail_fpr_rsp(nd); } }
                                           ...
                                           if (_chain && g_emit_cfg) { int _ni=-1; for(int _j=0;_j<g_emit_cfg->n;_j++) if(g_emit_cfg->all[_j]==nd){_ni=_j;break;} int _fr=0; if(_ni>=0) for(int _j=0;_j<g_emit_cfg->n&&!_fr;_j++){IR_t*_o=g_emit_cfg->all[_j]; if(_o&&_o!=nd&&_o->op==IR_MATCH_ARBNO&&_o->n_operands>=3){int _a=-1,_b=-1; for(int _k=0;_k<g_emit_cfg->n;_k++){if(g_emit_cfg->all[_k]==_o->operands[1])_a=_k;if(g_emit_cfg->all[_k]==_o->operands[2])_b=_k;} if(_a>_b){int _t=_a;_a=_b;_b=_t;} if(_a>=0&&_ni>=_a&&_ni<=_b)_fr=1;}} g_emit.op_arbno_framed=_fr; int _k0=1; { int _s0=-1,_s1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_s0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_s1=_j;} if(_s0>_s1){int _t=_s0;_s0=_s1;_s1=_t;} if(_s0<0)_k0=0; else for(int _j=_s0;_j<=_s1&&_k0;_j++){IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m!=nd){int _mo3=(int)_m->op; if(zd_k(_m)!=0||_mo3==IR_MATCH_DEFER||_mo3==IR_MATCH_ALTERNATE||_mo3==IR_MATCH_ARBNO||(_mo3==IR_MATCH_FENCE1 || _mo3==IR_MATCH_FENCE0)||_mo3==IR_MATCH_VALUE||_mo3==IR_CALL||_mo3==IR_CALL_VALUE||_mo3==IR_DISJUNCTION||_mo3==IR_MATCH_ABORT)_k0=0;}} } g_emit.op_arbno_body_k0=_k0; }   /* ⭐⭐⭐ ZETA-ACTIVATION UNIFICATION K0 CONTAINER/TRANSFER EXCLUSION (this session, WIDENED from DEFER-only after t1.sno surfaced the identical defect through IR_MATCH_ALTERNATE): zd_k()==0 for a container/transfer
                                            * kind (DEFER, ALTERNATE, nested ARBNO, FENCE1, VALUE, CALL/CALL_VALUE, DISJUNCTION, ABORT -- the SAME list _sq already excludes from K16 admission a few lines below, for the identical reason) is a STATIC BYTE-COUNT fact about that node's OWN
                                            * alpha carve; it is NOT a claim that the ζ frontier stays put while the node's OWN interior executes (ALTERNATE selects and drives a sub-pattern at runtime; DEFER invokes a separately-compiled proc blob; both have real, non-static interior
                                            * behavior their zd_k()==0 does not describe -- "zero-cell envelope" in ALTERNATE's own zd_k comment means "no OWN result cell", not "no interior frontier movement").  bb_match_arbno_frameless()'s soundness claim ("frontier never moves inside the
                                            * activation, so [rsp+0]/[rsp+4] name the same cell from every port site") requires EVERY body member to hold that property, not just have K=0.  arb1.sno (DEFER) and t1.sno (ALTERNATE, `ARBNO('Bird'|'Blue'|LEN(1))`) are the SAME mechanism through
                                            * two different container kinds -- t1.sno was ALREADY producing wrong output (m3=DIFF, m4=DIFF) at the pre-edit baseline (this was never RBP-protected: op_arbno_rbp's own qualification never covered ALTERNATE-bearing bodies, only the plain K0
                                            * arm did, silently), and this session's ARBNO-RBP removal changed its FAILURE MODE (silent DIFF -> m3 SIGSEGV) without being its root cause.  The bomb this exclusion feeds (bb_match_arbno.cpp's op_arbno_body_k0 check) converts BOTH classes from
                                            * silent-wrong-answer or crash into a loud, honest compile-time refusal -- matching the codebase's own established discipline (W-7) rather than leaving either as an unbombed correctness hole. */
   { int _du=0; int _ds0=-1,_ds1=-1; for(int _j=0;_j<g_emit_cfg->n;_j++){if(g_emit_cfg->all[_j]==nd->operands[1])_ds0=_j;if(g_emit_cfg->all[_j]==nd->operands[2])_ds1=_j;} if(_ds0>_ds1){int _t=_ds0;_ds0=_ds1;_ds1=_t;} int _alt=0,_ref=0,_dfr=0,_arb=0,_altnest=0; if(_ds0>=0) for(int _j=_ds0;_j<=_ds1;_j++){IR_t*_m=g_emit_cfg->all[_j]; if(_m&&_m!=nd&&(int)_m->op==IR_MATCH_DEFER&&!_m->pat_static)_du=1; if(!_m||_m==nd)continue; int _o=(int)_m->op; if(_o==IR_MATCH_ALTERNATE||_o==IR_DISJUNCTION){ if(_m->seal)_ref=1; else { _alt++; for(int _q=0;_q<_m->n_operands;_q++){IR_t*_a=_m->operands[_q]; if(_a&&((int)_a->op==IR_MATCH_ALTERNATE||(int)_a->op==IR_DISJUNCTION))_altnest=1;} } continue;} if(_o==IR_MATCH_DEFER){ extern int sn4_alt_carrier(void); if(sn4_alt_carrier()&&_m->seal!=1)_dfr++; else _ref=1; continue;} if(_o==IR_MATCH_ARBNO){ extern int sn4_arbno_tailbeta(void); if(sn4_arbno_tailbeta()&&_m->γ.node==nd)_arb++; else _ref=1; continue;}   /* ⭐ ARBNO-TAIL-BETA (s145) NESTED-ARBNO CARRIER: an inner IR_MATCH_ARBNO sitting at the span TAIL is an interior choice point of exactly the record-resuming class the s125 edge was written for -- its β is `jmp <body α>` (extend by one instance) and its af pops its OWN 16B cell, never an activation unwind, so the s123 rbp-climb hazard cannot arise.  Restricted to the TAIL because that is the node the admitted edge actually lands (PAIR(4)); an inner ARBNO anywhere else in the span still refuses via _ref=1.  OFF is byte-identical: _arb stays 0 and this takes the _ref=1 exit the old `default:` gave it. */   /* s127: seal DISCRIMINATES BY VALUE on DEFER -- 1 = right-sealed fence demarcation (bb_match_defer.cpp's op_seal==1 arm, the β activation-unwind s123 SEGV class, REFUSED), 2 = s142 WRITE-ONCE cache eligibility (β stays the plain `jmp [rsp]` record-resume, ADMITTED; measured here: the witness's *cmd stages seal==2 and the first cut's !seal gate refused it) */ switch(_o){case IR_MATCH_LIT:case IR_MATCH_LEN:case IR_MATCH_ANY:case IR_MATCH_NOTANY:case IR_MATCH_POS:case IR_MATCH_RPOS:case IR_MATCH_TAB:case IR_MATCH_RTAB:case IR_MATCH_SPAN:case IR_MATCH_BREAK:case IR_MATCH_REM:break;default:_ref=1;break;} } int _fseal=(nd->n_operands>3&&nd->operands[3]==nd)?1:0; g_emit.op_tail_seal=_fseal;   /* ⭐ ARBNO-SEAL-OMEGA (s189): stage the FENCE-rooted-body fact on the LIVE path -- op_tail_seal was assigned ONLY inside the `!_k16r && 0 && _tailc` dead tail block, so it read 0 for every node while flat_drive_match_alt (:1436) still re-aimed the body resume to na_f.  The FRAME arm has its own seal guard (op_arbno_body_actframe); the FRAMELESS arm had none and self-looped. */ g_emit.op_arbno_body_defer_unsafe=_du||arbno_reentry_hazard(nd); { extern int sn4_arbno_altsib(void); int _altok=sn4_arbno_altsib()?(_alt>=1&&!_altnest):(_alt==1); g_emit.op_arbno_body_actframe=(((_altok&&_dfr==0&&_arb==0)||(_alt==0&&_dfr==1&&_arb==0)||(_alt==0&&_dfr==0&&_arb==1))&&!_ref&&!_fseal)?0:1; } }   /* ⭐⭐⭐ s127 R-4(a) DEFER CLASS: an UNSEALED IR_MATCH_DEFER span member counts as THE choice carrier (exactly one, exclusive with an inline ALT) under sn4_alt_carrier() -- its β is the plain resume-through-record `jmp [rsp]` (bb_match_defer.cpp non-seal arm, never an activation unwind), so the admitted af->PAIR(1) edge lands the LAST PLACED INSTANCE's suspension record, whose blob res+β then re-enter the interior choice point (manual p.121 ARBNO retry = manual Ch.18 pop-the-stack; the depth story is the blob-side sn4_blob_choice_scan gate at the β-dispatch).  A SEALED defer keeps _ref=1: its β is the `mov rsp,rbp; pop rbp` unwind -- the s123 rbp-climb SEGV class -- and FENCE semantics forbid re-entry anyway (p.125/p.127 FENCE(P): alternatives within P invisible when backing up).  OFF is byte-identical: _dfr stays 0 and the defer takes _ref=1 exactly as the old switch default did. */   /* ⭐⭐ s126 SECOND TIGHTENING, MEASURED BEFORE LANDING: _alt is a COUNT and the edge is admitted for EXACTLY ONE choice node in the span.  Witness pair nf3/nf4 (probe/arbnofence/): ARBNO(FENCE('a'|'ab')|'c') vs ARBNO(('a'|'ab')|'c') on 'ab' -- the UNFENCED body FLATTENS to one 3-arm ALTERNATE (works under the edge, oracle-equal match), while the FENCE BLOCKS THE FLATTEN leaving two NESTED ALTERNATEs, and the admitted edge then LIVELOCKS (af->outer-ALT-beta->...->af, rc=124) where pre-patch default passed -- a regression this count refuses.  Nested-ALT-in-span IS s121's refused ALT-carrier-depth class (s124: "IR_DISJUNCTION still REFUSED -- s121's ALT-depth conviction untouched"); one-deep records compose by contiguity, two-deep do not yet.  The refusal is STRUCTURAL (choice-node count), never fence-named -- the fence's only material effect here is preventing the flatten. */   /* ⭐⭐⭐ s126 THE ANSWER TO s125's ONE REMAINING QUESTION: FENCE(P) as ARBNO's direct body is NODE-FREE IN THE SPAN -- its seal is the EDGE PROPERTY operands[3]==nd on THIS ARBNO node (written lower_snobol4.c:1473 "operands[3]=R marks resume≡fail-glue"; read as the Fence-seal marker at emit.cpp:1133/:1149/:1386), which is why all three span-side discriminators s125 tried (FENCE1-in-span, IR_t.seal on the ALT, container enums) MEASURED WRONG: the span for 151 is byte-identical to c2's (ALTERNATE + two LITs).  _fseal refuses the edge for the SEMANTIC reason the manual states (p.125/p.204: FENCE makes interior alternatives invisible to a backing-up matcher); mechanically it also breaks the s125 rc=124 LIVELOCK -- with the seal present, emit.cpp:1386 retargets body resumes to the fail glue, so an admitted af->PAIR(1) edge loops af->fail-glue->af forever.  Exhaustion under seal IS ω. */   /* ⭐⭐⭐ s125 R-4(a) ARBNO INTERIOR CHOICE POINT: op_arbno_body_actframe==0 (af back-out edge ADMITTED) iff the body span carries an ALTERNATE/DISJUNCTION resume record AND EVERY OTHER span member is a pure scanner-register leaf matcher.  STRICT OP-FILTER, safe BY CONSTRUCTION -- an unrecognised op ANYWHERE in the span refuses and keeps s124 behaviour exactly, so no future container has to be remembered here.  DELIBERATELY NOT A BLACKLIST: the first cut enumerated refused containers and MEASURED WRONG -- 151_pat_arbno_inline_fence_backtrack (ARBNO(FENCE('a'|'ab'))) still hung at rc=124 because FENCE() does not reach this span as IR_MATCH_FENCE1, so the blacklist never saw it.  FENCE must refuse for a SEMANTIC reason (manual p.125/p.204: FENCE makes the alternatives inside it invisible to a matcher that is backing up, which is the one thing the construct exists to forbid), and 151 is the exact FENCE-sealed twin of the witness this rung fixes -- ARBNO(FENCE('a'|'ab')) vs ARBNO('a'|'ab'), opposite required verdicts on the same shape. */   /* W-7 INTERIM GUARD (HOME-WIRES s38): SAME body span as op_arbno_body_k0 above, SAME containment idiom -- flags an IR_MATCH_DEFER member whose pat_static==0 (can transitively recurse, manual p.122's *X rule).  Independent of op_arbno_body_k0.  Consumed by bb_match_arbno() to refuse the unconditional plain-frameless fallback with x86_bomb.  See emit.h field note for the full record. */   /* ⭐ ARBNO-FRAME containment dual of the op_body_has_arbno scan above: THIS node inside any OTHER ARBNO's [operands[1]..operands[2]] span = the nested/unbounded tier -> per-activation header frame (see emit.h field note; Lon directive 2026-08-06) */ } else { g_emit.op_sa = -1; g_emit.op_sb = -1; g_emit.op_arbno_chain = 0; } }
                                    /* PS-3 (s152) DEFER-TAIL candidacy (decided ENTIRELY here, emit drive time -- LOWER untouched, so a refuse is byte-identical chain): body bracket must be EXACTLY one
                                     ...
                                     * refuses v1.  SCRIP_ARBNO_LATCH=1 arms (opt-in this slice until monitor-proven); the chain staging above stays live -- the template selects dt first. */
                                    if (g_emit.op_arbno_chain && g_emit.op_off >= 0 && !(nd->n_operands > 3 && nd->operands[3] == nd) && g_emit_cfg && nd->n_operands >= 3) {
                                        ...
                                        if (_dtl) {
                                            ...
                                            for (int _j = _i0; _ok && _j <= _i1; _j++) { IR_t * _x = g_emit_cfg->all[_j]; if (!_x) continue;
                                                ...
                                                else if (_x->op == IR_LIT_STRING || _x->op == IR_GOTO) { }   /* inert wiring: the fz procname literal + gotos; a granted quad on them is dead weight (never read per-iteration -- the SLOT-ELIDE census class), so fields do NOT refuse (s152 fix: the op op-filter is the gate) */
                                                else _ok = 0; }
                                            const char * _pn = NULL;
    ...
    case IR_MATCH_FENCE0:          { bb_prepare(nd); g_emit.op_fence0_release = fence0_release_bytes(nd); bb_emit_x86(bb_match_fence0()); } return 0;     /* ⭐ FENCE-SPLIT: the bare-FENCE cut box — operand-free, no body span, so none of FENCE1's body-span staging (fence_body_kk / fence_frame_slot) applies or is computed */
    ...
    case IR_GOTO_DEFERRED:             bb_emit_x86(bb_goto_deferred());       return 0;               /* EVAL/CODE runtime label transfer */
    case IR_DEFINE: {   /* AB-1: fill op_sval=fname, op_ival=nsave; operands carry GVA indices of {fname,formals,locals} minted by the lowerer */
        if (ir_define_sr_citizen(nd)) { extern int fc_call_active(const IR_t *); if (g_emit.op_ival == 4 && nd->n_operands >= 2 && nd->operands[0] && nd->operands[1]) { g_emit.op_sval = IR_LIT(nd->operands[0]).sval; g_emit.lbl_t0 = IR_LIT(nd->operands[1]).sval; }   /* TINY-REAL s58 role-4 SHIM: operands = {fname, entry-label-name} as IR_LIT_STRING nodes (gather-then-deposit per the FACT RULE; ival=4 aliases sval so the names ride operands) */ if (g_emit.op_ival == 3 && nd->γ.node && sr3_gamma_label(nd->γ.node)) { const char *_e = sr3_gamma_label(nd->γ.node); g_emit.op_sval = strncmp(_e, "LBL__", 5) ? _e : _e + 5; }   /* TINY-SITE s57: role-3 shim needs fname; γ = the stub's goto_deferred carrying entry_label "LBL__<FN>" — gather here per the FACT RULE (templates never read node neighbors) */ if (g_emit.op_ival == 0 && nd->γ.node && fc_call_active(nd->γ.node) && nd->n_operands == 1 && nd->operands[0]) { extern int zls_off(const IR_t *); int _as = bb_slot_get(nd->operands[0]); if (_as < 0) _as = zls_off(nd->operands[0]); if (_as >= 0) { g_emit.op_fc_wbytes = 16; g_emit.op_fc_base = _as; } } g_emit.op_define_role = (int)g_emit.op_ival; extern std::string bb_define(); bb_emit_x86(bb_define()); return 0; }   /* s116 fold: Ch.8 citizens (roles 1..4 in ival; site-0 = sval + g-call) ride this case now -- body verbatim from the deleted IR_SAVE_RESTORE dispatch. */               /* SN4-FLAT-PROC linkage: roles normalized at the UNION-TAG block above (0 CALL2BB site / 1 RETURN / 2 FRETURN / 3 wire-adopt).  CALL2BB 3b: when the gamma-paired call is fc-registered, sr0 gets the ARG window (base = the one arg's quad) so its existing FRQB(slot, sb) staging read self-rebases to [rsp + sb] -- the arg CELL above sr0's own save block -- with ZERO sr0 template edits (the s21x-h "fc arm inside FRQB" window-readiness, now armed); driver-side neighbor resolution is the sanctioned gather-then-deposit pattern. */
        if (nd->n_operands == 0) { g_emit.op_sval = IR_LIT(nd).sval;   /* ROLE DISCRIMINATOR = n_operands: IR_LIT is a UNION (sval clobbers ival — measured this session: the ival==2 guard sank every bind into drive_unowned); a role-1 block ALWAYS carries >=1 operand (fname), the bind carries none. */ g_emit.lbl_t0 = (const char *)0; if (g_emit_cfg && g_emit_cfg->n_dentry > 0) for (int _dq = 0; _dq < g_emit_cfg->n_dentry; _dq++) if (g_emit_cfg->dentry_node[_dq] == nd) { g_emit.lbl_t0 = g_emit_cfg->dentry_name[_dq]; break; }   /* NATURAL-LABEL (Lon s114): deposit the body-entry statement's natural port label name (alloc-time dentry resolution) so bb_define_bind's rt_define_site fn lea and the inline shim target n<uid>_statement_begin_α — no FN__/LBL__ face; gather-then-deposit per the FACT RULE. */ g_emit.op_define_role = 6; extern std::string bb_define(); bb_emit_x86(bb_define()); int _d1st = 1; if (g_emit_cfg && g_emit.op_sval) for (int _dr = 0; _dr < g_emit_cfg->n_dentry; _dr++) { IR_t *_dp = g_emit_cfg->dentry_node[_dr]; if (_dp == nd) break; if (_dp && _dp->n_operands == 0 && IR_LIT(_dp).sval && !strcmp(IR_LIT(_dp).sval, g_emit.op_sval)) { _d1st = 0; break; } }   /* REDEFINE-ONE-FACE (M1-R4b): a function may legally be DEFINE'd more than once (SPITBOL; oracle green), and every DEFINE bind node of one name stamps the SAME dentry_entry -- the dentry loop resolves the body through the by-name proc row, of which the lowerer keeps exactly ONE.  So the per-site role-5 shims of a name are identical modulo their per-instantiation .Lx ids (measured, both witnesses in corpus/probe/redef), while their faces <FN>_α/_γ/_ω are name-derived and therefore collide: `as` rejected `symbol F_α is already defined` and EVERY redefining program was m4-uncompilable.  The face is emitted at the FIRST bind node of each name; later sites keep their role-6 bind (the rt_define_site registration is per-statement and stays) and skip only the duplicate face.  m3 never reached this arm (TEXT-gated), so this is m4-only by construction and m3 =/= m4 cannot open here. */ if (g_is_text && g_emit.lbl_t0 && _d1st) { long _sv5 = g_emit.op_ival; g_emit.op_ival = 5; g_emit.op_define_role = 5; bb_emit_x86(bb_define()); g_emit.op_ival = _sv5; }   /* SHIM-AT-STATEMENT (Lon s114 in-chat: "Move all the SHIM code emission directly after the spot where the DEFINE statement is"): role 5 = the role-4 tiny shim emitted INLINE right here, skip-wrapped (inline flow jumps over it; entered only by name at <FN>_α), body transfer = the natural entry label in this same chain.  m4 TEXT only, the s58/s62 owed-m3 precedent: the m3 world keeps the stub-blob shim + balias verbatim, byte-identical. */ return 0; }   /* AB-3a: role 2 = the DEFINE residual bind, a live main-chain box (anchor->bind->sJ); role 1 (blocks) rides g->ab_nodes[] via bb_ab_emit_nodes and never reaches this dispatch in-chain. */
        g_emit.op_sval = IR_LIT(nd).sval;
        g_emit.op_ival = (long)nd->n_operands;
        g_emit.op_ab_nformals = nd->seal;   /* AB-3b: nformals stored in seal by lowerer (seal is IR_MATCH_DEFER-specific, always 0 on IR_DEFINE nodes) */
    ...
    case IR_STATEMENT_BEGIN:      { static int _mon = -1; if (_mon < 0) _mon = getenv("MONITOR_BIN") ? 1 : 0; extern int g_mon_max_stno; g_emit.op_mon_stmt_tap = ((_mon || x86_zdp_rbp_on()) && g_emit.op_stno > 0) ? 1 : 0;   /* s189: ZSM-armed runs emit the tap too -- mon_emit_label_bin latches the current stno (monitor-off path returns right after), giving the ZSM ring a stable source coordinate; default byte-identical (tap off both ways) */ if (g_emit.op_mon_stmt_tap && g_emit.op_stno > g_mon_max_stno) g_mon_max_stno = g_emit.op_stno; g_emit.op_fc_bytes = 0; bb_emit_x86(bb_statement()); { extern std::string bb_zdp_anchor(long, long); static int _zdpa = -1; if (_zdpa < 0) { const char * e = getenv("SCRIP_ZDP_TEARDOWN"); _zdpa = (e && *e == '1') ? 1 : 0; } if (_zdpa) bb_emit_x86(bb_zdp_anchor((long)nd->op, (long)bb_node_id((IR_t *)nd))); }   /* ⭐ LON'S ANCHOR CHECK (s135): IR_STATEMENT_BEGIN is an anchor — the graph's stack is TOTALLY EMPTY here, so every anchor must observe ONE rsp.  Killswitch SCRIP_ZDP_TEARDOWN=1, DEFAULT OFF and byte-identical BY CONSTRUCTION (OFF emits nothing; manifest over 124 programs hashing COMPILE STDOUT proved 0 movers, null-stable, and non-vacuous at 124/124 under SCRIP_OPT=0).  Placed AFTER bb_statement() so the check sits at the head bracket's own alpha, the point the static half measures.  Instrumentation-tap precedent: op_mon_stmt_tap on this very line. */ g_emit.op_mon_stmt_tap = 0; } return 0;   /* s26 (Lon): statement HEAD bracket -- alpha label = attribution range start; frame arm dormant (K=0 relay) until the FINDING-2026-08-02h STF rung; mon tap mirrors IR_GOTO's so the 2-way monitor syncs on true statement boundaries. */
    case IR_STATEMENT_END:
    case IR_STATEMENT:            { g_emit.op_fc_bytes = 0; bb_emit_x86(bb_statement()); } return 0;   /* ZW-5 (O-1 LIGHTING, s23q): op_zgpop is NOT set here -- the staging choke (grep \"ZD-1: cleared at the choke UNCONDITIONALLY\") is the ONE authority and supplies it from zd_plan's zd_gp[] via g_zd_gpop.  The prior `op_zgpop = nd->ival` was the s22k SECOND-SPELLING disease: nd->ival = lc_build default 0 (lower never stamps it -- K_total comes from the planner, not lower), so the direct read silently overwrote the planner's K_total with 0 and suppressed the release on every refused (UCLAIM) run.  op_fc_bytes=0 stays: K=0 is correct (zd_k), the statement box yields no cell. */
    ...
    case IR_SCAN_TAB:             { long fck; if (fc_geom(nd, &fck) && getenv("SCRIP_ICN_FCCARVE")) { g_emit.op_fc_bytes = fck; g_emit.op_fc_base = g_emit.op_off + 16; if (g_emit.op_sa >= 0 && !getenv("SCRIP_NO_CM_CARVE")) g_emit.op_sa += (int)fck; } bb_emit_x86(bb_scan_tab()); }    return 0;   /* ZB-ICN-FC-1: Icon's first FORTH cell — saved-cursor scratch at op_off+16 rides rsp; grant set HERE (post-DRIVE_FILL reset), result stays flat at op_off+0 */
    case IR_SCAN_MOVE:            { long fck; if (fc_geom(nd, &fck) && getenv("SCRIP_ICN_FCCARVE")) { g_emit.op_fc_bytes = fck; g_emit.op_fc_base = g_emit.op_off + 16; if (g_emit.op_sa >= 0 && !getenv("SCRIP_NO_CM_CARVE")) g_emit.op_sa += (int)fck; } bb_emit_x86(bb_scan_move()); }   return 0;   /* ZB-ICN-FC-2: move is tab's twin — saved-cursor scratch at op_off+16 rides rsp; grant set HERE (post-DRIVE_FILL reset) */
    ...
    case IR_SCAN_MATCH:           { long fck; if (fc_geom(nd, &fck) && getenv("SCRIP_ICN_FCCARVE")) { g_emit.op_fc_bytes = fck; g_emit.op_fc_base = g_emit.op_off + 16; } bb_emit_x86(bb_scan_match()); }  return 0;   /* ZB-ICN-FC-3: {0,1} match's needle scratch at op_off+16/+24 rides an rsp cell (transient alpha->gamma; single-yield keeps LIFO clean even in a scan) */
    case IR_SCAN_POS:             bb_emit_x86(bb_scan_pos());    return 0;
    case IR_SCAN_BAL:             bb_emit_x86(bb_scan_bal());    return 0;
    case IR_GLIT:                 { g_emit.op_name1 = IR_LIT(nd).sval; bb_emit_x86(bb_glit()); } return 0;   /* RK-GRAM-3b grammar literal-match leaf */
    case IR_GCC:                  { g_emit.op_name1 = IR_LIT(nd).sval; bb_emit_x86(bb_gcc());  } return 0;   /* RK-GRAM-3b grammar char-class leaf */
    case IR_GALT:                 { g_emit.op_off = 0; bb_emit_x86(bb_galt()); }                 return 0;   /* RK-GRAM-3d grammar alternation: delta-save slot at [___+0]; lbl_t1 pre-resolved in pre-pass above */
    case IR_RETURN: {
...
static void flat_drive_match_alt(IR_t **nodes, int n, int i, bb_label_t **lbls, bb_label_t **betas, bb_label_t **na_s, bb_label_t **na_f, bb_label_t ***fc_sig, bb_label_t *node_γ, bb_label_t *node_ω, bb_label_t *chain_ω) {
    ...
    if (nd->op == IR_MATCH_ARBNO) { IR_t *_t = (nd->n_operands > 2) ? nd->operands[2] : (IR_t *)0; { IR_t *_s0 = (nd->n_operands > 1) ? nd->operands[1] : (IR_t *)0; int _i0 = -1, _i1 = -1; for (int k = 0; k < n; k++) { if (_s0 && nodes[k] == _s0) _i0 = k; if (_t && nodes[k] == _t) _i1 = k; } if (_i0 > _i1) { int _sw = _i0; _i0 = _i1; _i1 = _sw; } if (_i0 >= 0) for (int k = _i1; k >= _i0; k--) if (nodes[k] && nodes[k] != nd && nodes[k]->γ.node == nd) { _t = nodes[k]; break; } }   /* ⭐ THE TAIL IS THE LAST TOP-LEVEL BODY ELEMENT, NOT THE LAST ALLOCATED NODE: lower pushes operands[2] = g->all[g->n-1], which for a body whose last element is itself a container is that container's OWN last child (measured: ARBNO('a' ARBNO('b')) staged operands[2] = the inner ARBNO's body LIT, whose β RETREATS the inner ARBNO instead of EXTENDING it, so the nested witness stayed nomatch under the first cut of this rung).  A body element is top-level iff its γ lands the owning ARBNO (lower_snobol4.c tags exactly those edges σ); scanning the span RIGHT-to-LEFT for the last such node yields the inner ARBNO itself, and for a single-element body it yields that one element = operands[1] = PAIR(1), preserving byte-identity for the s125 ALT class. */ bb_label_t *_tb = node_ω; for (int k = 0; k < n; k++) if (_t && nodes[k] == _t) { _tb = betas[k]; break; } int _i = g_emit.xa_bb_emit_pair_n++; g_emit.xa_bb_emit_pair_define[_i] = NULL; g_emit.xa_bb_emit_pair_jmp[_i] = _tb; }   /* ⭐ ARBNO-TAIL-BETA (s145): pair slot 4 = β of operands[2] = the body span's LAST node, i.e. the last committed instance's RIGHTMOST interior choice point (manual Ch.18 step 6).  Pushed UNCONDITIONALLY so the slot always resolves; consumed only by bb_match_arbno_frame()'s af edge under sn4_arbno_tailbeta().  operands[1] (PAIR(1), the span HEAD) stays exactly as it was -- the span scan at the ARBNO drive case keys off operands[1]..operands[2] and must keep both ends.  Single-node body => operands[1]==operands[2] => PAIR(4) and PAIR(1) are ONE label and the arm is byte-identical. */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void drive_arg_slots_reserve(int n) {   /* promoted non-static for bb_ab_emit_nodes (bb_define.cpp) — the bb_ab_fn_cell_ptr precedent; ONE allocator authority, declared emit.h */
...
void * bb_ab_fn_cell_ptr(const char * fname) { return (void *)&g_ab_fn_cells[bb_ab_slot_for(fname)]; }   /* AB-3b: the OPERAND face — the cell ADDRESS baked into emitted code (bb_call_proc_staged / bb_goto_deferred x86_jmp_via_cell, bb_define's body$<ENTRY>, and the m3 driver + runtime_eval seals).  BOTH MEDIA and never NULL: the slot exists in the COMPILING process whatever the medium, and TEXT renders the assembler symbol instead of the baked value.  Same bb_ab_slot_for ⇒ same slot as bb_ab_cell_addr: ONE allocator, two faces. */
...
void emit_drive(IR_t *nd, bb_label_t *lbl_α, bb_label_t *lbl_γ, bb_label_t *lbl_ω, bb_label_t *lbl_β) {
    ...
    case IR_MATCH_BEGIN: {
        ...
        g_emit.op_frame_extra = emit_match_begin_frame_extra(nd);   /* ⭐ ARBNO-FRAME SLOT (THREE ZETAS s86): staged HERE (not read by the template as a function call) because templates never hold the raw IR_t* -- see emit.h field note. */
        DRIVE_PAIR_RESET();
    ...
    case IR_GOTO_DEFERRED: {
        g_emit.op_sval = IR_LIT(nd).sval; g_emit.op_ival = nd->seal; DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;   /* ⭐ DEFINE-FOLD (s53): op_ival carries the fold-eligible seal (1 = constant DEFINE entry label, bb_goto_deferred emits the direct chain transfer) */
    }
    ...
    case IR_MATCH_FENCE0: {   /* ⭐ FENCE-SPLIT: the bare-FENCE cut box — operand-free, PURE 4-port (α commits, β abandons), so it rides the GENERIC drive.  Its granted quad is reserved for the
                               * own-extent whack rung and is not read by today's template.  Was `IR_MATCH_FENCE1 && n_operands==0` before the kinds were separated. */
        g_emit.op_off = drive_value_slot(nd); DRIVE_PAIR_RESET(); DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β);
        break;
    }
    case IR_MATCH_FENCE1: {   /* FENCE(P) only — the operand form.  The n-ary constructs keep the flat-path-only contract. */
    ...
    case IR_MATCH_BREAK: case IR_MATCH_BREAKX: {
        ...
        if (a0 && g_emit.op_sa < 0) { drive_unowned(nd); break; }
        g_emit.x86_scratch_off = drive_value_slot(nd);
        g_emit.op_leaf_frame_off = leaf_frame_slot(nd);   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s131 SLICE 2): staged from THE ONE AUTHORITY exactly as the SPAN case above — the planner decides, so the carve (frame_slot_scan) and the address (LFC) are one decision and cannot drift (s124 two-reader law).  -1 on every refuse leaves bb_match_break/breakx.cpp byte-identical. */
        DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    }
    case IR_MATCH_TAB: case IR_MATCH_RTAB: {
        ...
        g_emit.x86_scratch_off = drive_value_slot(nd);
        g_emit.op_leaf_frame_off = leaf_frame_slot(nd);   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s131 SLICE 2): TAB/RTAB take an INTEGER cursor argument, never a needle, so no arm of theirs spends more than the 8-byte usable window — both were measured rc=139 in BOTH media on the default arm (corpus/probe/leafsib/). */
        DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    }
    ...
    case IR_MATCH_REM: case IR_MATCH_ARB: case IR_MATCH_BAL: {
        g_emit.x86_scratch_off = drive_value_slot(nd);
        g_emit.op_leaf_frame_off = leaf_frame_slot(nd);   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s131 SLICE 2): REM, ARB and BAL are all admitted, on the ONE condition leaf_frame_member() states -- LOCATION, never identity.  ⛔ THE OLD TEXT HERE SAID "BAL SELF-REFUSES ... it spends +0/+4/+8 and d=8 is the neighbour's granule floor" AND IT DIED WITH THE 8-BYTE WINDOW LAW at s174 (44b8b82c, Lon's NO-PER-OP-FILTER ruling): every leaf now claims TWO CONSECUTIVE 16B registry slots, so BAL's third word is inside its own claim and there is no neighbour to hand a slot to.  MEASURED s193 (row alt-arb-bal-witness) in the emitted asm, witness probe/leafsib/leafsib_bal_flat_red: SPAN_FRAME=0 spells the cell [rsp+464]/[rsp+468]/[rsp+472] and m4 dies rc=139; =1 -- the shipped default since s188 (d3251f23) -- spells it [rbp-80] and both media pass.  A stale refuse written into a staging line reads as a live decision for as long as nobody re-measures it.  The admission still lives in the PLANNER, never in this staging line and never in the template, so the carve and the address can never disagree about which ops are framed. */
        DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    }
    case IR_MATCH_VALUE: {
        g_emit.op_sa = -1; g_emit.op_off = -1;
    ...
    case IR_CUT:
        DRIVE_PAIR_RESET(); DRIVE_PAIR_JMP(lbl_γ); DRIVE_PAIR_DEF_JMP(lbl_β, lbl_ω); DRIVE_FILL(nd, lbl_α, lbl_γ, lbl_ω, lbl_β); break;
    case IR_DEFINE:   /* AB-3a: role-2 bind in the live chain.  SECOND-DISPATCHER law (the 1560 comment, verbatim mechanism): emit_drive needs its own case or drive_unowned FATALs "op=15 has no template" while the walk case plainly exists — MEASURED on ab_recurse this session, same as ZW-5 SLICE 2 measured on probe1.  Shape = the SUCCEED/CUT trailer contract; the walk case's ival==2 arm supplies bb_define_bind. */
        if (ir_define_sr_citizen(nd)) {   /* s116 fold: Ch.8 citizens drive arm, body verbatim from the deleted IR_SAVE_RESTORE case */
        ...
        g_emit.op_c2 = 0;
        int na = nd->n_operands; drive_arg_slots_reserve(na);
        for (int i = 0; i < na; i++) { IR_t * a = nd->operands[i]; int s = a ? bb_slot_get(a) : -1; if (s < 0 && a) s = zls_off(a); g_emit.op_arg_slot[i] = s; }   /* CALL2BB slice 2: bcps_arg_slot's slot_get→zls precedence, -1 on total miss (role-0 refuses to pass-through) */
        g_emit.op_arg_slot_n = na;
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* BP-9 (ii) TRIVIAL-β OP-FILTER (LIVE CURSOR residual (a); the drift-hazard the gate script backs): an op is
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ⭐⭐⭐ U-1b UNWIND PRED OP-FILTER (Lon HQ directive 2026-08-03c, "get every BB to FREE on OMEGA"): the kinds whose β may be the LANDING of an unwind roll.  THE LAW IS ALREADY IN THE TEMPLATE LAYER and
 ...
 * ONLY reason the unwind could not use it was that flat_trivial_beta named four match kinds -- a op-filter minted for the ΣK POP FOLD, whose question ("may I INLINE this β away") is the mirror-image of
 ...
 * level down): the statement rides ZD iff every node in it is either a op-filtered spine kind whose value operands all resolve to ZD producers inside the same statement, or a passive control node that
 * carves nothing -- one legacy reader or one foreign carve refuses the whole statement, which is exactly the s21x-o reader-frontier law ("a BB may be armed once ALL of its consumers speak the live-depth
...
static int zd_wl_kind(IR_t * nd) {
    ...
    /* ZD-5 NOTE (s22i): IR_MATCH_BEGIN is NOT admitted here yet.  The 247 "MATCH_BEGIN refuses" in the census
     * mean MATCH_BEGIN was the FIRST blocker in the run -- but the ACTUAL blocker keeping those runs refused
     ...
     * function LIE.  zd_wl_kind is not a policy op-filter that someone was being timid with -- it is a CAPABILITY REGISTRY recording which kinds have a ZD
     ...
     * reads an unwritten rsp slot.  The ledger entry "IR_CALL_BUILTIN_ICON -- 68 refuses, absent from the op-filter; likely ONE TEMPLATE ZD ARM" says
...
int emit_match_begin_stfh_k_raw(void) {   /* MATCH-RBP (s65e): the graph-qualification body, unchanged — the wrapper below is the ONE AUTHORITY callers keep using *//* ZD-8·STFH-CARVE (FINDING-2026-08-09g): THE ONE AUTHORITY for the MATCH_BEGIN head-carve byte count.  Returns 64 when the OS-2·SLICE-1 GLUE-O deep-main stfh arm fires AND has no bracket leave to reclaim it; 0 otherwise.  Called at BOTH plan time (from zd_k, before g_emit graph-level fields are populated) AND emit time (from the template, after g_emit is fully staged), so all inputs are derived from g_emit_cfg (the IR graph — available at both times).  FLAT_STMT_FRAME DISJUNCT CONTRIBUTES 0: stfh() in the template fires for flat_stmt_frame too, but the STF bracket leave (mov rsp,___ at emit.cpp:841) already reclaims ALL rsp depth — returning 64 there would double-count the carve.  We only return 64 for the OS-2 arm where no bracket leave exists.  JMP-ENTRY FLAGS (flat_jmp_entry/flat_pat/flat_gen/flat_lcl_proc/zframe_graph) are always 0 for any graph containing IR_MATCH_BEGIN: those flags are set only for blob/proc/gen/zframe graphs, which are jmp-entered and never contain a MATCH_BEGIN node.  The absence of MATCH_BEGIN-incompatible opcodes in g_emit_cfg is therefore the sufficient guard.  SCRIP_OS_CAP=0 restores 0 byte-identically (the killswitch for the OS-2·SLICE-1 widening that introduced the stfh arm). */
    ...
    for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _nd = g_emit_cfg->all[_i]; if (!_nd) continue; if (_nd->op == IR_MATCH_REPLACE || (_nd->op == IR_MATCH_FENCE1 || _nd->op == IR_MATCH_FENCE0) || _nd->op == IR_MATCH_ABORT || _nd->op == IR_MATCH_ARBNO) return 0; }   /* has_replace equivalent — same four opcodes as has_replace_l() in bb_match_begin.cpp */
    return 64;
}
...
int emit_defer_carve_rbp(void) {   /* ⭐⭐⭐⭐⭐ s137 DEFECT C AT THE GATE — the BEAUTY SELF-HOST blocker, landed OFF per the R-7/s124 protocol.  MEASURED CHAIN (FINDING-2026-08-17-s137): bb_match_defer.cpp gates the ζ-ACTIVATION frame on `_.op_seal == 1`, so an UNSEALED defer -- which is what an ARBNO BODY defer always is -- never pushes one and falls to the legacy rsp-watermark arm that emit_defer_rbp()'s own comment above already calls Defect C, "unsound the moment the deferred target carves without self-releasing".  That is EXACTLY the measured discriminator: ARBNO(*C) with C's inner defer -> LEN(0) (carves nothing) PASSES, -> SPAN(' ') (carves) FAILS, and it fails only from ITERATION TWO (manual Ch.9 p.121: each retry supplies another instance -- instance two has no home).  ⛔ THE CATEGORY ERROR THIS FIXES: the seal is a FENCE demarcation (s127, "seal DISCRIMINATES BY VALUE"), never a statement about carve, so gating a CARVE-SOUNDNESS property on a FENCE property is why the class survived every prior sweep.  ⛔ DEFAULT OFF AND BYTE-IDENTICAL: OFF, the widened expression collapses to `_.op_seal == 1` exactly, so the shipped build is unchanged by construction.  ON, every defer in the cstack port manages its own frame at α/γ/ω/β -- the sites are PAIRED, so widening one expression keeps the stack balanced by construction rather than by inspection.  ⛔ THIS FIRST CUT IS THE BROAD FORM (all unsealed defers), deliberately: it is the PROBE that confirms the diagnosis end-to-end.  Narrowing it to "unsealed AND the target carves AND we are re-entrant" is the next seat's job and needs the 150/151 pair and probe/arbnofence/ green before any flip -- those two punished the last two gate widenings. */
...
int emit_arbno_rbp(void) {   /* ⭐⭐⭐ ZETA-ACTIVATION UNIFICATION (Lon in-chat, this session): "remove the RBP used for ARBNO ... in EVERY case where an RBP was needed it now MOVES into the ACTIVATION FRAME."  THE ARBNO-RBP
     * ARM IS RETIRED -- forced OFF unconditionally, same treatment given emit_match_rbp() above this session.  The ELSE arm every caller of this predicate already carries (bb_match_arbno.cpp's
     * bb_match_arbno_frameless_k, "ARB-LON-K16") is not an improvised fallback: RESULT (Δ0/yield dwords) and the LOCAL instance counter both already live at ζ-ACTIVATION-relative offsets inside the pinned
     * MATCH_BEGIN statement bracket -- re-homed to the frontier at each committed instance (FR/FRQ-relative, "one-level-in" reads), never a per-instance chained RBP push.  Same "goes negative ⇒ exhaustion"
     * counter semantics, same RESULT/count roles Lon specified, same manual Ch.9 ARBNO contract (shy, backtrack-retried, one instance per retry) -- only the storage family changes, from a second per-construct
     * RBP frame to the ONE activation frame every other construct already uses.  Env read kept for forensic re-derivation ONLY; SCRIP_ARBNO_RBP no longer has any live effect on codegen. */
    (void)getenv("SCRIP_ARBNO_RBP");
    return 0;
}
int emit_arbno_rbp_unwind(void) { return 0; }   /* ⭐⭐⭐ ZETA-ACTIVATION UNIFICATION (this session): DEAD BY CONSTRUCTION now that emit_arbno_rbp() above is forced 0 -- the original guard's first conjunct (!emit_arbno_rbp()) already
     * forced this to 0 on every call, so collapsing the body to a bare `return 0` changes no caller's observed value; the g_emit_cfg scan and the emit_match_rbp() conjunct are removed as dead code together
     * with the ARBNO-RBP frame arm they existed only to gate.  x86_arbno_rbp_unwind()/x86_arbno_rbp_unwind_at() (x86_asm.h) remain defined but are now permanently unreached through this gate; left in place
     * rather than deleted, same discipline as every prior killswitch-retirement rung in this file, so their own removal stays a separate byte-identity-gated edit. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_arbno_reentry(void);
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int arbno_reentry_hazard(const IR_t * nd) {   /* ⭐⭐⭐ THE ONE AUTHORITY for "can an activation re-enter while this ARBNO instance is still live and RETRYABLE" (s189, row arbno-altsib-residue).  TWO READERS IN LOCKSTEP, which is the whole point: arbno_frame_candidate() (the registry, which ALLOCATES the slot) and the op_arbno_body_defer_unsafe staging at the IR_MATCH_ARBNO drive case (which is what bb_match_arbno.cpp:302 actually STEERS on).  Widening only the registry was measured INERT -- the slot was allocated and the template still took the frameless arm -- the s124 two-reader law / s68 "spelled-twice disease" in one witness. */
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_arbno_reentry(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ARBNO_REENTRY"); v = (e && *e == '1') ? 1 : 0; } return v; }   /* ⭐ THE ONE AUTHORITY for the ARBNO RE-ENTRY FRAME killswitch (s189, row arbno-altsib-residue).  ⛔ DEFAULT **OFF**, DELIBERATELY, AND THE REASON IS THE POINT OF THE ROW: the arm is CORRECT AND NECESSARY BUT NOT SUFFICIENT -- it moves the ARBNO cursor from a flat ζ-SPINE cell to the per-activation blob slot ([rsp+0]/[rsp+4] -> [rbp-48]/[rbp-44], measured), which is exactly what a re-entrant activation requires, and the witness STAYS RED because a SECOND fact is computed over the same wrong span (op_arbno_body_actframe at the drive case, which decides whether the af->retreat edge is emitted at ALL; red emits `cmp r14d,eax; jmp PAT$N_ω`, the s184 absent-edge signature, with the cmp setting flags nothing reads).  Landing this ON would move bytes for ZERO behavioural gain, so it ships ARMED and OFF: default output is byte-identical by construction (a refused candidate never enters the registry, so count, indices and carve are unchanged) and the next seat gets the storage half already built and measured.  Same function-local-static getenv shape every killswitch in this file uses (sn4_span_frame, sn4_pt_frame, sn4_blob_casmark) and the same shape sno_defer_resume (lower_snobol4.c:1245) uses -- no new file-scope state beyond the memoised env read. */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int arbno_frame_candidate(const IR_t * nd) {   /* ⭐ ARBNO-FRAME SLOT (THREE ZETAS s86): 1 iff nd is an IR_MATCH_ARBNO whose body span [operands[1]..operands[2]] is DEFER-unsafe by EITHER of bb_match_arbno.cpp's
     * two existing bomb predicates -- W-7 (a body member is IR_MATCH_DEFER with pat_static==0) OR the K0-DEFER companion (some body member's zd_k()!=0, or is itself a container/transfer kind: DEFER/ALTERNATE/
     * nested ARBNO/FENCE1/VALUE/CALL/CALL_VALUE/DISJUNCTION/ABORT -- the SAME list the emit-drive K0 scan at line ~1061 already excludes, reproduced here standalone so this function can run from EITHER the
     * MATCH_BEGIN counting site or the ARBNO node's own drive site without depending on _chain/op_arbno_chain staging order). A body that is SAFE (plain K0, no container/transfer member) does not need a frame
     * slot at all -- it stays on bb_match_arbno_frameless(), unchanged; ONLY the two bomb classes route here. */
    int blob_frame_scope(void);   /* declared below beside frame_slot_scan; the same inline forward-declaration leaf_frame_member and xop_frame_member already use */
    ...
    for (int j = s0; j <= s1; j++) { IR_t * m = g_emit_cfg->all[j]; if (!m || m == nd) continue;
        if (m->op == IR_MATCH_DEFER && !m->pat_static) return 1;   /* W-7 */
        int mo = (int)m->op;
        if (zd_k(m) != 0 || mo == IR_MATCH_DEFER || mo == IR_MATCH_ALTERNATE || mo == IR_MATCH_ARBNO || (mo == IR_MATCH_FENCE1 || mo == IR_MATCH_FENCE0) || mo == IR_MATCH_VALUE
            || mo == IR_CALL || mo == IR_CALL_VALUE || mo == IR_DISJUNCTION || mo == IR_MATCH_ABORT) return 1;   /* K0-DEFER companion */ }
    if (arbno_reentry_hazard(nd)) return 1;   /* ⭐⭐⭐ RE-ENTRY ARM (s189, row arbno-altsib-residue): the two scans above ask whether this ARBNO's OWN BODY is defer-unsafe, but the hazard the frame slot exists to answer is "can an ACTIVATION re-enter while this ARBNO instance is still live and RETRYABLE", and that is created by everything to the ARBNO's RIGHT, not by its body.  Witness ptw_min_arbno_altrec_falsereject: `E = ARBNO('a') ('+' *E | '')` on 'a+a+a' -- the body is the plain literal 'a', so both scans refuse, the cursor stays a FLAT ζ-SPINE cell (`sub rsp,16` at α, read back `[rsp+0]`/`[rsp+4]` at _as/_af), and the recursive activation between the carve and the reads displaces it, so the outer ARBNO never extends and a matching subject is REJECTED.  PROOF THE PREDICATE'S SPAN IS THE WHOLE DEFECT: adding a SEMANTICALLY INERT `*Z` with `Z = ''` inside the body -- which cannot change what the pattern matches -- flips the witness GREEN, and the asm moves the same cursor to [rbp-48]/[rbp-44] = frame_slot_off(blob,0).  Membership is LOCATION, never identity (leaf_frame_member's own law): inside a blob activation -- which is BY DEFINITION the callee of a `*P` defer, R-4(b) -- any DEFER anywhere in the graph can lead back here directly (a1) or mutually (a20: E -> *G -> *E, measured red), so every ARBNO in a defer-bearing blob claims a per-activation slot.  Deliberately a SUPERSET of the body scans, so no arm above changes answer.  SCRIP_ARBNO_REENTRY=0 reverts verbatim -- a refused candidate never enters the registry, so count, indices and carve are all unchanged. */
    return 0;
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_span_frame(void) {   /* ⭐⭐⭐ s130 THE ONE AUTHORITY for the LEAF-SUSPENSION-FRAME killswitch (SCRIP_SPAN_FRAME).  TWO READERS in lockstep -- leaf_frame_member() (which feeds the registry scan, hence the carve width at BOTH
     ...
     * and cannot drift (the s124 two-reader law, the s66 coherent-worlds law).  ⭐ FLIPPED DEFAULT ON s188 (HQ-59 ruling, Lon desk delegation 2026-08-20, the fz3-flip precedent): the arm cures the leaf-suspension
     * wild-write class -- a leaf on an ALT arm addressing its cell at a RAW FLAT ZLS COORDINATE off rsp and writing above the live frame once earlier statements push that coordinate past it -- and the flip was
     * blocked only by the s184 TDump_driver blocker, itself CURED at sn4_xh_frame_extra (SCRIP 0b75fa5e).  RECEIPTS at pristine 0b75fa5e, RT_OPT -O0, one build, both arms selected by env: .s sweep 13 movers /
     * 527 comparable, every changed line a carve / retry_whack / ALT-arm leaf re-home / registry renumber / flat-spine rebase by exactly the carve delta, and ZERO spine-resident re-homes (the HQ-60 acceptance);
     * 6-suite board 1222 rows x 2 modes, 4 movers ALL CURES ZERO REGRESSIONS (control self-run noise floor 1); corpus m3 332/5 m4 325/11 SKIP 1, fail-set identical by name.  ⛔ HQ-60, SPINE-RESIDENT LEAVES KEEP
     * RSP: leaf_frame_member()'s alt_arm_member conjunct is LOAD-BEARING and this rung did NOT widen it -- re-homing spine leaves wholesale was measured broken (s127: 120/131/165/181/182).  =0 reverts VERBATIM
     * to the pre-flip default, byte-identical to pre-s130 by construction, since a refused candidate never enters the scan (count unchanged, every other class's index unchanged) and every leaf template keeps
     * its legacy FR(x86_scratch_off) spelling; proven by re-sweeping both arms at the flipped default and joining row-for-row against the pre-flip pair. */
    static int _sf = -1; if (_sf < 0) { const char * e = getenv("SCRIP_SPAN_FRAME"); _sf = (e && *e == '0') ? 0 : 1; } return _sf;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_xh_frame_extra(void) {   /* ⭐⭐⭐ s184 THE ONE AUTHORITY for the CROSS-HEAD FRAME-EXTRA killswitch (SCRIP_XH_FRAME_EXTRA, DEFAULT ON = the cure).  The cross-MATCH_BEGIN correction below hardcodes 64 for "MATCH_BEGIN's alpha is push rbp + 4 pushes + sub 24 = 64B of real RSP motion" -- true only when emit_match_begin_frame_extra() is 0.  Every registry candidate (ARBNO-frame, CAPTURE-SAVE, FENCE1, and under SCRIP_SPAN_FRAME every ALT-arm scratch-cell leaf) widens that carve, so the real motion is 64 + extra and every flat ζ-SPINE operand read across the head is short by exactly extra -- caller territory.  Measured on TDump_driver: the SPAN reads its charset 16B past its slot and the match spuriously FAILS, which the program renders as a quoted identifier (FINDING s184).  =0 restores the 64 verbatim. */
    static int _xf = -1; if (_xf < 0) { const char * e = getenv("SCRIP_XH_FRAME_EXTRA"); _xf = (e && *e == '0') ? 0 : 1; } return _xf;
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int cap_fail_retreat(void) {   /* ⭐⭐⭐ s193 THE ONE AUTHORITY for the CAPTURE-TARGET FAILURE EDGE killswitch (SCRIP_CAP_FAIL_RETREAT, DEFAULT ON = the cure).  THE DEFECT IT CLOSES: in the IMMEDIATE ($) capture road the transfer's ω landing -- the arm reached when the deferred target PROC FAILED -- ran its epilogue, called rt_cap_finish, and then FELL THROUGH into the box's common continuation L(1), the very label the γ (succeeded) arm jumps to.  Four ports, three wires: a failing target was indistinguishable from a succeeding one and the element matched anyway (witness corpus/probe/dcap/dcap_freturn_target: oracle nomatch, SCRIP matched).  ARMED, the ω arm leaves the alignment anchor and CONCEDES (x86_jmp(X86P_OMEGA)) -- the same wire x86_beta_trampoline already routes β through, so no new edge is invented.  ⛔ CONCEDE, NOT ABORT, AND THE ORACLE RULED IT: `(SPAN('ABC') $ *chk('NO') | 'ABC')` prints one CHK and then MATCHES on live sbl -bf, so a FAILING target RETREATS and the alternation's next arm runs -- unlike a NOT-A-NAME target, which aborts the statement through IR_MATCH_END's ω (the s170 SN4-CAP-NAME-STRICT edge).  The two are different verdicts on the same road and must not be merged.  ⛔ NOT the dcap/COND road: that one drains through rt_dcap_pump at MATCH_END and reports rc=1 under strict; the IMM road never records an entry at all (SCRIP_DCAP_TRACE prints `end_ok n=0` on the witness), which is why no existing killswitch moved it.  =0 restores the fall-through VERBATIM (the armed arm adds instructions; disarmed emits the empty string).  Both media by construction -- x86_jmp/x86_anchor_leave are medium-complete. */
    static int _cf = -1; if (_cf < 0) { const char * e = getenv("SCRIP_CAP_FAIL_RETREAT"); _cf = (e && *e == '0') ? 0 : 1; } return _cf;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_pt_frame(void) {   /* ⭐⭐⭐ s177 PF-1 THE ONE AUTHORITY for the PASS-THRU FRAME killswitch (SCRIP_PT_FRAME) -- ARCH-PASSTHRU law 0c mechanized: a scratch-cell leaf inside a SEAM-ENTERABLE graph (a blob: no MATCH_BEGIN of its own) may not price its cell as a flat rsp offset, because what sits above it is the CALLER'S ACTIVATION (unknown count, unknown type) -- FINDING s177 measured TAB(3)'s [rsp+64] landing ON the caller's banked γ wire (pc=0x1). Under this switch every zdp_scratch_cell leaf in blob scope becomes a registry candidate and homes in the blob's OWN rbp frame (frame_slot_scan rc-2 arm, 2-slot claim). NARROWER than SCRIP_SPAN_FRAME (which also re-homes ALT-arm leaves in STATEMENT graphs -- the seat7 TDump blocker's territory): blob scope only. DEFAULT OFF = byte-identical (a refused candidate never enters the scan); flip after the ptc-board + corpus receipts, R-7 protocol. */
    static int _pf = -1; if (_pf < 0) { const char * e = getenv("SCRIP_PT_FRAME"); _pf = (e && *e == '0') ? 0 : 1; } return _pf;   /* ⭐ FLIPPED DEFAULT ON same rung (s177): armed receipts strictly dominate -- corpus m3 328/9 vs 327/10 (sole delta: demo_claws5 CURED), m4 fail-set identical, ptc 42/55 vs 40/55 (TAB-SEGV class cured), wordcount/ARB silent-drop encoder gaps closed first.  =0 reverts byte-identically. */
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int leaf_frame_member(const IR_t * nd) { extern int zdp_scratch_cell(const IR_t *); int blob_frame_scope(void); return nd && zdp_scratch_cell(nd) && ((sn4_span_frame() && alt_arm_member(nd, (const IR_t *)0)) || (sn4_pt_frame() && blob_frame_scope())); }   /* EVERY leaf is a member; the ONE condition is LOCATION, never identity. Inside an ALT arm the flat rsp coordinate has no granted owner in either medium (s66/s71 ungranted-arm class), so the cell homes on rbp. On the ordinary spine the offset calculation is exact and rsp stays the home (Lon HQ-60: spine-resident leaves keep RSP -- re-homing them wholesale was measured broken, s127: 120/131/165/181/182). Registry claim width for every leaf is TWO consecutive 16B slots, so the full 32B region has one owner and every spelling the family uses -- LFC(8) included -- is inside its own claim: the old 8-byte usable-window carve-out is DEAD (Lon 2026-08-20). PURE and plan-independent. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_pt_opframe(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_PT_OPFRAME"); v = e ? (*e == '1') : sn4_pt_frame(); } return v; }   /* ⭐⭐⭐ s178 PF-1c THE ONE AUTHORITY for the CROSSED-OPERAND FRAME killswitch (SCRIP_PT_OPFRAME) -- ARCH-PASSTHRU law 0c, tier-2 escalation, applied to the ζ-VALUE-OPERAND plane: a producer whose consumer's op_zread span crosses a VARIABLE-RETENTION box (MATCH_DEFER/ARBNO/VALUE, earn_hazard_in's family) has NO flat rsp price -- the defer's γ road retains the inner blob's 32B resume record plus its own suspended cells (unbounded), the C road 16, measured to the byte in FINDING s178-L2 (bfn_break_var: [rsp+40] needle read garbage, four layout faces of one wild mechanism).  Follows sn4_pt_frame when unset (ensemble, PF-1b precedent: SCRIP_PT_FRAME=0 reverts the whole pass-thru frame family); explicit env wins either way; =0 byte-identical (a refused member never enters the scan). */
static int xop_hazard_kind(int op) { return op == IR_MATCH_DEFER || op == IR_MATCH_ARBNO || op == IR_MATCH_VALUE; }   /* the VARIABLE-RETENTION family, same members as earn_hazard_in (s101: "no compile-time-constant RSP distance") -- stated once here for the crossing test; a member's γ exit leaves road-dependent bytes on the spine. */
static int zd_k(IR_t * nd);
static int xop_frame_member(const IR_t * nd) {   /* ⭐⭐⭐ PF-1c (s178): is nd a ζ-value producer whose result is consumed ACROSS a variable-retention box in this blob graph?  Membership = blob scope (statement graphs keep spine until a witness convicts them -- the s127 do-not-widen conviction) AND nd owns exactly one 16B result cell (zd_k==16, behavioral, excludes the leaf/scratch family and multi-cell kinds) AND some consumer C with nd among its operands sits past a hazard box on nd's γ-chain (zd_chase, THE edge-following authority, the FZ-1 lesson: never guess order from all[]).  PURE and plan-independent like every registry predicate. */
    ...
    if (nd->op == IR_MATCH_ASSIGN_SAVE || nd->op == IR_MATCH_ARBNO || nd->op == IR_MATCH_FENCE1) return 0;   /* already customers of this registry under their OWN classes (capture/ARBNO/FENCE1 slots) -- one node, one home, never two */
    if (!g_emit_cfg) return 0;
    for (int j = 0; j < g_emit_cfg->n; j++) { const IR_t * c = g_emit_cfg->all[j]; if (!c || c == nd) continue;
        ...
        if (!isop) continue;
        if (alt_arm_member(c, (const IR_t *)0)) return 1;   /* ⭐ PF-1d (s178-b, FINDING s178-FW): an ARM-INTERIOR consumer is the ungranted-arm denial class (zd_plan grants per RUN; nested-ALT arms unclaimed by the descent) and its op_sa seat is a slot nobody writes since the s97 dead-spill deletion -- the producer frame-homes and the seat (XSAQ/XSAD or ZOPQ) reads the SAME rbp slot.  Same containment authority as the CAPTURE and LFC customers. */
...
static int frame_slot_is_candidate(const IR_t * nd) {   /* ⭐⭐⭐ ACTIVATION-FRAME SLOT REGISTRY (THREE ZETAS s87): THE ONE PREDICATE unifying ARBNO-FRAME and the CAPTURE family's op_frame_need arm into ONE shared
     ...
     * ARBNO's own retries were done -- the exact crossover class Lon's own prior attempt at per-construct transient frames for ARBNO/CAPTURE hit and reverted). ARBNO-FRAME candidates: arbno_frame_candidate().
     * CAPTURE candidates: IR_MATCH_ASSIGN_SAVE with frame_need_of()==1 (the EXISTING EARN-1 verdict, already correctly propagated to a SAVE from its paired COND/IMM by the s83 CAP-NEST fix) -- SAVE is the
     * textual anchor because it is always allocated at or before its COND/IMM (lower_snobol4.c's "COND is allocated FIRST, SAVE second" note), so scanning for SAVE gives a stable single occurrence per
     * capture pair; COND/IMM resolve to their own SAVE (operands[1]) and read/write the SAME assigned slot -- see capture_frame_slot() below. */
    if (!nd) return 0;
    ...
    if (leaf_frame_member(nd)) return 1;   /* ⭐⭐⭐ LEAF-SUSPENSION (s130): the registry's FOURTH customer -- a scratch-cell leaf matcher on an ALT arm, whose raw-flat suspension cell has no ζ-SPINE home (see leaf_frame_member / emit.h op_leaf_frame_off) */
    if (xop_frame_member(nd)) return 1;   /* ⭐⭐⭐ PF-1c CROSSED-OPERAND (s178): the registry's FIFTH customer -- a ζ-value producer consumed across a variable-retention box; claim width 1 slot (16B result cell), see xop_frame_slot / emit.h op_xf_off */
    return 0;
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int blob_frame_scope(void);
static int frame_slot_scan(const IR_t * query, int * out_index, int * out_count) {   /* ⭐⭐⭐ ACTIVATION-FRAME SLOT REGISTRY (THREE ZETAS s87): THE ONE SCAN every public authority below calls -- walks
     * g_emit_cfg ONCE, finds the MATCH_BEGIN textually enclosing query (the last IR_MATCH_BEGIN at or before its index -- g_emit_cfg is one statement's run, so exactly one is live at any node), then counts
     * every frame_slot_is_candidate() node (ARBNO or CAPTURE-SAVE, interleaved in textual order) between that MATCH_BEGIN and the next IR_MATCH_BEGIN (or run end), assigning each the next available 0-based
     * index -- ONE shared numbering space, so an ARBNO candidate and a CAPTURE-SAVE candidate can never be assigned the same offset regardless of which textually precedes the other. *out_index = query's own
     * index (-1 if query is not itself a candidate, e.g. called from the MATCH_BEGIN counting site with query=NULL). *out_count = total candidates in that MATCH_BEGIN's scope. Returns 1 iff an enclosing
     * MATCH_BEGIN was found (0 refuses everything -- template and carve both fall back to legacy/bomb, the same graceful-refuse shape arbno_frame_scan already had for the PAT$N stored-pattern-blob class,
     * which has no MATCH_BEGIN in its OWN g_emit_cfg and stays out of scope for this rung). */
    ...
    int blob = (mb < 0 && blob_frame_scope()) ? 1 : 0;   /* ⭐⭐⭐ R-4(b) (s97, GOAL-SNOBOL4-100 stored-pattern-blob class, witnesses probe/bb/probes/{D12,D13,H31}): a PAT$ blob graph has NO MATCH_BEGIN of its own -- it is the CALLEE of a `*P` DEFER (manual Ch.7 p.85 deferred evaluation, Ch.9 p.122 recursive patterns), i.e. it IS the ζ-ACTIVATION-FRAME the THREE ZETAS ruling assigns to every `*P` dereference. The scope is the whole run and the slots hang off the blob's OWN rbp frame (blob_frame_bytes(): pushed at α_body, whacked at ω, restored across the γ suspension through the resume record) -- per-activation by construction, which is what recursion (D12: LIST -> ITEM -> *LIST -> the SAME ARBNO node re-entered while the outer instance is mid-iteration) requires and what a single shared STANDING slot at a fixed offset can never give (Lon s95: RESULT slots live in the ACTIVATION frame, not STANDING). Return code 2 tells the three authorities the base is 16 (saved caller rbp at [rbp+0], slots strictly below), not STANDING's 64. */
    ...
    for (int j = mb + 1; j < hi; j++) { IR_t * m = g_emit_cfg->all[j]; if (!m || !frame_slot_is_candidate(m)) continue;
        if (j == qi) found = k; k += zdp_scratch_cell(m) ? 2 : 1; }   /* a leaf claims TWO consecutive slots -- one 32B owner -- so k advances by claim width and the interleaved ARBNO/CAPTURE/FENCE1/LEAF numbering stays collision-free by construction */
    if (out_index) *out_index = found; if (out_count) *out_count = k;
    return blob ? 2 : 1;
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_blob_casmark(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_BLOB_CASMARK"); v = (e && *e == '0') ? 0 : 1; } return v; }   /* ⭐⭐⭐ s178-e THE ONE AUTHORITY for the BLOB CAS-MARK killswitch (witness probe/fw/fw_arbno_double_commit): a blob activation is a match sub-activation, so its ω must restore the capture-pending arena TOP (r12) to its OWN α-entry value -- the MATCH_BEGIN CAS-MARK discipline applied per-activation.  THE DEFECT: the β-dispatch's refused/choiceless road is `PAT$N_β: jmp PAT$N_ω`, which bypasses the interior cond_β chain and its `sub r12,24` pops -- a completed-then-retreated capture blob (ARBNO's abandoned zero-iteration continuation) left its pend record standing and the pump committed it (PC2 fired twice; in beauty the doubled PushCounter corrupts the counter stack).  The same leak exists on FORWARD ω-fail past an already-recorded COND.  =0 reverts head/base/save/restore in lockstep, byte-identically.  FOUR READERS through blob_head_bytes() + the two emission arms -- the s66 coherent-worlds law. */
static int blob_head_bytes(void) { return sn4_blob_casmark() ? 40 : 24; }   /* head below the pushed rbp: -8 r10 · -16 r11 · -24 rdx(DTP) · [-32 r12 CAS bank · -40 pad, casmark arm]; parity: push(8)+head ≡ 0 mod 16 both arms */
static int frame_slot_off(int scan_rc, int idx) { return -((scan_rc == 2 ? (blob_head_bytes() + 8) : 64) + 16 * idx); }   /* ⭐ R-4(b): THE ONE OFFSET RULE for the shared registry -- STANDING (rc 1) keeps its 64B PATCTX/retry/RESULT head above the slots; a blob activation (rc 2) keeps its 32B head {saved caller rbp @+0, entry γ wire r10 @-8, entry ω wire r11 @-16, pad @-24}, so slot 0 is [rbp-32]. Both: 16B per slot, idx in the interleaved ARBNO/CAPTURE-SAVE/FENCE1 numbering. */
...
int zzone_off_for(const IR_t * nd, int customer) {   /* ⭐⭐⭐⭐⭐ ζ-ONE U-3 — THE ONE (NODE, CUSTOMER) ZETA OFFSET RESOLVER (s138).  Lon's ask, verbatim s136 and again s138: "Complete the unified zeta offset calculation graph traversals and the template
     ...
     * at the single point of consumption is what lets ZREF have exactly one refuse test for all five customers. */
    if (!nd) return -1;
    switch (customer) {
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ARBNO-ALTSIB (s184): s126 made `_alt` a COUNT and admitted the af
 * retreat edge for EXACTLY ONE choice node, on a conviction measured
 * against a NESTED pair -- nf3 ARBNO(FENCE('a'|'ab')|'c'), where the
 * fence blocks the flatten, leaving an ALTERNATE inside an ALTERNATE,
 * and the admitted edge livelocks af->outer-beta->af.  SIBLING alts in
 * one span were swept up with them and are a DIFFERENT shape:
 * ARBNO((LEN(1)|LEN(2)) (LEN(1)|LEN(2))) is two DISJOINT 32B records
 * laid contiguously on one spine, the composition s125 relied on.  With
 * the edge refused the generator explores ONLY the all-first-arms path:
 * measured, it matches every EVEN prefix length and NO odd one, where
 * the oracle matches every length >= 2.  =0 restores s126 byte-identically. */
int sn4_arbno_altsib(void) { static int v = -1; if (v < 0) { const char * e = getenv("SCRIP_ARBNO_ALTSIB"); v = (e && *e == '0') ? 0 : 1; } return v; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ⭐ THE ONE AUTHORITY for the ARBNO-SEAL-OMEGA killswitch (s189).  =1 omits bb_match_arbno_frameless()'s φ retract `jne PAIR(1)` when the body is
 * FENCE-rooted, so a sealed ARBNO exhausts straight into ω; =0 restores the pre-s189 emission byte-identically.  WHY: emit.cpp:1436 re-aims a
 * FENCE-rooted body's resume to na_f ("resume a committed iteration ≡ abandon it"), which makes PAIR(1) resolve to the ARBNO's OWN af -- so
 * `jne PAIR(1)` at af is a literal self-loop re-testing an unchanged δ against an unchanged Δ0 forever.  The retract instruction is still emitted
 * and still correct; nothing jumps to it (0 refs, measured).  bb_match_arbno_frame() already omits the same edge under seal via
 * op_arbno_body_actframe, and emit.cpp:1192 states the contract in words: "Exhaustion under seal IS ω." */
int sn4_arbno_seal_omega(void) {
    const char * e = getenv("SCRIP_ARBNO_SEAL_OMEGA"); return (e && *e == '0') ? 0 : 1;   /* no cached file-scope state: compile-time only, one call per ARBNO node */
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_arbno_tailbeta(void) {   /* ⭐ THE ONE AUTHORITY for the ARBNO-TAIL-BETA killswitch (s145).  =1 aims bb_match_arbno_frame()'s af retreat edge at the body span's LAST node's β (PAIR(4)) instead of its FIRST node's β (PAIR(1)), and admits a span-tail nested IR_MATCH_ARBNO as the interior choice carrier; =0 restores s144 byte-identically.  WHY: PAIR(1) is β of operands[1] = the body span HEAD, but manual Ch.18 step 6 pops to the MOST RECENT choice point, which is the last committed instance's RIGHTMOST node -- the β chain then ripples leftward undoing each element's cursor advance.  For a single-node body (the s125 ALT class) head==tail so both spellings resolve to ONE label and every byte is unchanged; they diverge only for a multi-node span, which is exactly the class that shipped wrong answers: ARBNO('a' ('b'|'bc')) on 'abc' and ARBNO('a' ARBNO('b')) on 'ab' both fail in SCRIP and match on the oracle. */
    static int _atb = -1; if (_atb < 0) { const char * e = getenv("SCRIP_ARBNO_TAILBETA"); _atb = (e && *e == '0') ? 0 : 1; } return _atb;   /* ⭐ DEFAULT INVERTED ON under Lon's in-chat grant 2026-08-19 (the s124 words: measured first, flipped on request).  MEASURED BEFORE THE FLIP: killswitch byte-identity 529/529 over demo+crosscheck+probe/bb; blast radius 15 of 529, every one a nested-ARBNO or ARBNO-with-defer program; those 15 graded in BOTH modes against the live oracle 12 PASS/PASS held + calculator-2-match{,-fence} DIFF->PASS + json TIMEOUT unchanged, zero regressions; demos m3 11/22->13/22, m4 10/22->12/22.  KILLSWITCH INVERTED, NEVER DELETED (R-7): SCRIP_ARBNO_TAILBETA=0 restores the pre-rung emission byte-identically. */
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_seam_walk(void) {   /* ⭐ s124 THE ONE AUTHORITY for the LEFTWARD-SEAM-WALK killswitch. =1 widens the blob beta-dispatch carrier set from leaf generators to ANY deterministic fail-through element, so a blob whose RIGHTMOST element is deterministic still ENTERS its interior seam instead of dead-ending at lbl_omega; =0 restores s123 byte-identically. */
    static int _sw = -1; if (_sw < 0) { const char * e = getenv("SCRIP_SEAM_WALK"); _sw = (e && *e == '0') ? 0 : 1; } return _sw;   /* ⭐ s124 DEFAULT INVERTED ON under the same grant: zero row-level movers over 318 crosscheck programs in BOTH arms, probe/seam 1/4 -> 4/4.  SCRIP_SEAM_WALK=0 restores s123 byte-identically. */
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sn4_alt_carrier(void) {   /* ⭐ s127 THE ONE AUTHORITY for the SN4-ALT-CARRIER killswitch (R-4(a) defer class, witness probe/arbnostore/arbno_defer_altarg_red).  TWO READERS, one function, flips in lockstep (the s124 two-reader law): (1) the :1143 ARBNO span scan -- an unsealed DEFER span member becomes the admitted choice carrier for the af->PAIR(1) edge; (2) the blob β-dispatch -- a stored blob whose body_root is a single unsealed ALTERNATE with ALL-LEAF siblings routes β to the ALT's own interior β (the depth story: leaf arms carve nothing, so res's fixed -32 lands rsp EXACTLY on the 32B arm record -- s121's 150 conviction was arms WITH interior carves, refused here by sn4_blob_choice_scan).  A third reader (blob_frame_bytes safety-floor framing for refused-carrier targets) was landed and RETRACTED same-session -- see the ⛔ conviction at blob_frame_bytes: zd cross-blob pricing must learn the frame first.  RESIDUAL HAZARD, opt-in only, honestly stated: ARBNO(*p) where p resolves at match time to a REFUSED-carrier blob that γ-suspends retained interior -- the admitted af edge's second lap can jmp through the retained record's cursor field (rip=0 bomb) where OFF silently wrong-answers; the honest-bomb-over-silent-wrong trade is deliberate (RULES) and the class IS the named next rung.  =1 opt-in; default OFF byte-identical (pending the corpus ON-arm sweep + Lon's flip grant, the R-7/s124 protocol). */
    static int _ac = -1; if (_ac < 0) { const char * e = getenv("SCRIP_ALT_CARRIER"); _ac = (e && *e == '0') ? 0 : 1; } return _ac;   /* ⭐ s127 DEFAULT INVERTED ON under Lon's in-chat grant ("All your choices. I'm with you on this.", given over the full gate table: crosscheck 318 m3 A/B +1/-0 zero regressions, arbnostore ON 10/10 m3+m4, seam 4/4, arbnofence 4/4, named movers 8/8 incl 150/151 both arms, OFF byte-identity 122/122 by git-stash pristine manifest, beauty 10/622 NEUTRAL both arms).  Killswitch INVERTED, NEVER DELETED (R-7): SCRIP_ALT_CARRIER=0 restores the pre-s127 emission byte-identically. */
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void sn4_blob_choice_scan(int * n_choice, int * leaf_ok, int * has_fence) {   /* ⭐ s127 ONE SCAN, TWO READERS (β-dispatch admission + blob_frame_bytes widening -- never spell the classification twice, the s68/s70 disease): walks THIS blob graph counting unsealed choice nodes, flagging FENCE1, and verifying every non-choice member is a pure scanner-register leaf (the same op-filter as the :1143 span scan; an unrecognised op refuses by construction, the s125 op-filter principle). */
    ...
    if (g_emit_cfg) for (int j = 0; j < g_emit_cfg->n; j++) { IR_t * m = g_emit_cfg->all[j]; if (!m) continue; int o = (int)m->op;
        ...
        if (o == IR_SUCCEED || o == IR_FAIL) continue;   /* s127: every blob graph closes with the SUCCEED/FAIL terminal pair (the s120 FINDING's own words) -- exit-port ROUTING, not matchers; they carve nothing on the spine and are neutral to the depth story.  Measured here by gdb: the witness's 3-matcher blob is a 5-node cfg {SUCCEED, FAIL, ALTERNATE, LIT, LIT}. */
...
static int blob_wire_clobber_scan(void) {   /* ⭐ R-4(b) (s97): 1 iff this blob's interior contains a node that OVERWRITES the r10/r11 wires -- a jmp-entry crossing into another blob/proc: IR_MATCH_DEFER (bb_glue_pass_wires_blob), IR_CALL / IR_CALL_VALUE (bb_call_proc_staged / bb_call_value's site-set wires), IR_MATCH_VALUE / IR_MATCH_CALLOUT (σ-transfer into a γ-retaining blob). MEASURED s97 (witness `Q = 'x' P` / `Q = P 'b'`, oracle =S): with the s44 WIRE-SAVE arm dead (x86_fb_pinned()==0, DEL-T1 D-2) the blob's own γ then jumps through its LAST interior DEFER's L(4) instead of the wires it was ENTERED with -- re-entering itself forever, one 32B record per lap: the m3 `[ZHP] heap exhausted` rc134 / SIGSEGV pair, i.e. R-3(a)'s demos SIG6 class. The cure is the activation frame: entry wires ride [rbp-8]/[rbp-16], reloaded at γ/ω. */
    if (!g_emit_cfg) return 0;
...
extern "C" int sn4_choice_rbp(void) {   /* ⭐ s128 THE ONE AUTHORITY for the SCRIP_CHOICE_RBP killswitch (GOAL-SNOBOL4-100 next-rung: THE NON-LEAF ALT-CARRIER DEPTH STORY, witnesses probe/altdepth/).  Design of record (s127 cursor): the ALT's arm record moves off the rsp frontier into the blob ACTIVATION frame -- per-entry = per-ARBNO-iteration (D12 recursion already requires per-activation frames) -- and res re-seats rbp from record[+24] (already built, :3139), making the arm-resume read DEPTH-IMMUNE regardless of what the winning arm carved.  =1 opt-in; default OFF byte-identical (land OFF, sweep before any flip -- the R-7/s124 protocol). */
    static int _cr = -1; if (_cr < 0) { const char * e = getenv("SCRIP_CHOICE_RBP"); extern int sn4_pt_frame(void); _cr = e ? (*e == '1') : sn4_pt_frame(); } return _cr;   /* ⭐ s177 PF-1b: follows sn4_pt_frame when unset (explicit env still wins either way) -- the rbp-resident choice record is the PASS-THRU frame law applied to the choice class, so the two flip together; SCRIP_PT_FRAME=0 reverts the ensemble byte-identically. */
}
static int blob_choice_rbp_scan(void) {   /* ⭐ s128 ADMISSION for the rbp-resident choice record, ONE SCAN consumed by blob_frame_bytes (+32), sn4_choice_rbp_off (the template/dispatch offset) -- lockstep by construction.  Admits EXACTLY the refused-leaf single-choice blob (_nc==1, !_lf, !_fn: the captarm/beauty-Command class) and ONLY when the blob is ALREADY FRAMED by existing law (registry demand or wire-clobber): the s127 zd conviction stands VERBATIM -- this predicate NEVER frames a zero-demand blob, so no ZD-priced caller-territory cell ever meets a frame it was not priced against.  Leaf blobs (_lf) keep the proven s127 contiguity arm; _nc>1 stays the refused two-record-composition class (s126); _fn stays the FENCE semantic refusal (manual p.125/p.204). */
    ...
    { extern int sn4_pt_frame(void); if (!sn4_pt_frame()) {   /* ⭐ s177 PF-1b: under the pass-thru frame law the s128 tightenings RETIRE -- the _lf refusal, the wire-clobber refusal, and the already-framed (count>0) requirement all existed because a frame could meet cells priced against caller territory; PF-1a homes every scratch-cell leaf in the blob's own frame, so the convicted mechanism (s127's five movers, s128's two) is structurally gone and the ONLY remaining refusals are semantic: multi-record composition (_nc>1, the named per-node-slot follow-up) and FENCE (manual p.125/p.204).  The legacy arm below is byte-identical for SCRIP_PT_FRAME=0. */
        if (_lf) return 0;
...
int blob_frame_bytes(void) {   /* ⭐ R-4(b) THE ONE AUTHORITY for the blob activation's carve BELOW the pushed caller rbp: 24 (entry wires r10/r11 + pad, 16B-parity with the push) + 16 * (registry candidates in this blob graph); 0 when the blob has neither slot demand nor a wire-clobbering interior -- and 0 is the byte-identical legacy shape (no push, no whack, the γ record keeps its pad, res its two loads): every such blob is untouched by this rung. Read by the α_body prologue, the CLASS D γ record, res landing and ω whack, so all four agree by construction (one scan, four sites -- the s66 coherent-worlds law).  s128: + 32 for the rbp-resident CHOICE RECORD when blob_choice_rbp_scan() admits -- the record lives at [rbp - blob_frame_bytes() .. +31], strictly BELOW every registry slot (whose rbp offsets do not move), preserving 16B parity (push 8 + carve stays 16-aligned since 32 ≡ 0 mod 16). */
    if (!blob_frame_scope() || !g_emit_cfg) return 0;
    int count = 0; if (frame_slot_scan((const IR_t *)0, (int *)0, &count) != 2) return 0;
    int _pend = 0; if (sn4_blob_casmark()) for (int _i = 0; _i < g_emit_cfg->n; _i++) { IR_t * _m = g_emit_cfg->all[_i]; if (_m && _m->op == IR_MATCH_ASSIGN_COND) { _pend = 1; break; } }   /* ⭐ s178-e: a COND banks pend records, so a COND-bearing blob is ALWAYS framed -- the CAS bank needs its home even when no registry slot demands one */
    if (count == 0 && !_pend && !blob_wire_clobber_scan() && !blob_choice_rbp_scan()) return 0;   /* s177 PF-1b: a zero-scratch blob whose CHOICE record homes on rbp still needs the frame -- without this conjunct sn4_choice_rbp_off() would answer 0 (base = -frame_bytes) while the scan says rbp-resident, two authorities disagreeing about one record.  No recursion: the scan reads frame_slot_scan only. */   /* ⭐⛔ s127 CONVICTION -- DO NOT WIDEN THIS GATE BY BLOB SHAPE (the first cut of R-4(a)'s safety floor did, and MEASURED five OFF=PASS -> ON=SEGV movers: 120/131/165/181/182, all rc=139): a zero-demand blob's interior is ZD-PRICED -- its leaf suspension cells live at static offsets the CALLER's carve reserved (measured gdb, 120_pat_calc_add: ws=SPAN(' ')|LEN(0)'s span cell `mov dword [rsp+52],r14d` = blob-entry+20, CALLER territory by design; the framed shape put the SAME baked spelling at [rbp-4] and shredded the frame head's own r10 wire slot, rip=0x2_00401544).  Wire-clobber blobs tolerate the frame ONLY because dynamic-box graphs REFUSE zd static pricing wholesale (the zdyn veto).  A safety-floor frame for refused-carrier choice blobs therefore needs the pricing to SEE the frame bytes first -- that is part of the named ALT-carrier-depth-story rung (rbp-relative choice records), not a one-line widening here. */
    return blob_head_bytes() + 16 * count + (blob_choice_rbp_scan() ? 32 : 0);
}
extern "C" int sn4_choice_rbp_off(void);
extern "C" int sn4_choice_rbp_off(void) {   /* ⭐ s128 reader authority for the CHOICE RECORD's rbp offset: 0 = rsp arm (legacy, byte-identical), else the negative base of the 32B record in THIS blob's activation frame.  Read by bb_match_alternate (all record spellings) and the β-dispatch admission -- one function, readers in lockstep (the s124 law). */
    return blob_choice_rbp_scan() ? -(int)blob_frame_bytes() : 0;
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int arbno_frame_slot(const IR_t * arbno_nd) {   /* ⭐ ARBNO-FRAME SLOT (THREE ZETAS s86/s87): see emit.h field note. -1 unless emit_match_rbp() AND arbno_frame_candidate(arbno_nd) AND an enclosing MATCH_BEGIN
     * exists in g_emit_cfg -- offset = -(64 + 16*index) in the SHARED frame_slot_scan numbering (ARBNO and CAPTURE-SAVE candidates interleaved), the fixed layout below ζ-STANDING's own 64B PATCTX/retry/
     * RESULT/pad, one 16B cell per site so the {Δ0,yield} dword pair fits exactly as bb_match_arbno_rbp's retired per-construct layout did (Δ0@+0, yield@+4), just rbp-relative to the SHARED frame instead
     * of a frame ARBNO owns itself. */
...
int leaf_frame_slot(const IR_t * leaf_nd) {   /* ⭐⭐⭐ LEAF-SUSPENSION FRAME SLOT (s130): see emit.h op_leaf_frame_off.  SAME shared frame_slot_scan numbering as arbno_frame_slot()/capture_frame_slot()/fence_frame_slot(),
     ...
     * exactly where a cell the CALLER cannot see must live.  -1 refuses and the template keeps its legacy spelling; every refuse path is byte-identical to pre-s130. */
    if (!emit_match_rbp() || !leaf_frame_member(leaf_nd)) return -1;
    int idx = -1, rc = frame_slot_scan(leaf_nd, &idx, NULL); if (!rc || idx < 0) return -1;
    return frame_slot_off(rc, idx + 1);   /* the claim is slots {idx, idx+1}: staging the LOWER slot's offset puts every d in [0,24) inside the leaf's own 32B region -- LFC(8) is in-claim, never a neighbour's cell. -1 = the spine spelling FR(x86_scratch_off+d), used wherever the rsp offset is exactly calculable. */
}
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int xop_frame_slot(const IR_t * nd) {   /* ⭐⭐⭐ PF-1c (s178): THE ONE AUTHORITY for the crossed-operand frame slot -- see emit.h.  Mirrors leaf_frame_slot's shape: refuse -1 everywhere the member predicate refuses; claim width 1 (a 16B value cell, d ∈ {0,8} both in-claim), so frame_slot_off(rc, idx) addresses the whole claim.  Both the producer's ZRES and every consumer's ZOPQ stage from HERE, so write and read cannot disagree (the s66 coherent-worlds law). */
...
int capture_frame_slot(const IR_t * cap_nd) {   /* ⭐⭐⭐ ACTIVATION-FRAME SLOT REGISTRY (THREE ZETAS s87): THE REPLACEMENT for bb_match_capture.cpp's own transient push-rbp/pop-rbp arm (s81/s83) -- SAME shared
     ...
     * fixed offset in the ONE frame that already spans the whole match). cap_nd may be SAVE itself or its paired COND/IMM (operands[1] == the SAVE) -- both resolve to the SAME assigned slot, matching the
     * existing op_off design where SAVE writes and COND/IMM read the identical cell. -1 unless emit_match_rbp() AND the resolved SAVE has frame_need_of()==1 AND an enclosing MATCH_BEGIN exists. */
...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int emit_match_begin_frame_extra(const IR_t * match_begin_nd) {   /* ⭐ ACTIVATION-FRAME SLOT REGISTRY (THREE ZETAS s86/s87): see emit.h field note. 0 unless emit_match_rbp(); otherwise 16 * (count of
     * frame_slot_is_candidate() nodes -- ARBNO and CAPTURE-SAVE together -- in this MATCH_BEGIN's own scope). frame_slot_scan with query=NULL (an unconditional full-scope count, since match_begin_nd IS the
     * MATCH_BEGIN itself and can never BE a candidate) makes the count match exactly what arbno_frame_slot()/capture_frame_slot() will assign each member, by construction (one scan, three call sites, the
     * s66 two-coherent-worlds law widened to three parties instead of two). */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, int *zarm) {
    ...
    { /* _ng gate deleted, CARVE-DATA-ERAD s23a */
       ...
       * WHOLE-GRAPH early return -- one pinned graph refuses every statement in it before any per-node arm is consulted (measured s204, queens:
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        while (cur && guard++ <= n) {   /* LINEAR γ-CHAIN WALK -- unchanged from prior; subtree descent deferred per blocker above */
            ...
            { static int _fl = -1; if (_fl < 0) { const char * e = getenv("SCRIP_ZD_FLOATER_CUT"); _fl = (e && *e == '0') ? 0 : 1; } if (_fl && rl > 0 && emit_floater_kind(nodes[ci])) break; }   /* ⭐⭐⭐ S-1 FLOATER-CUT (RBP-EARN s78): THE RUN WALK MUST OBEY THE FLOATER LAW THE RPO WALK ALREADY OBEYS -- RPO_PUSH refuses emit_floater_kind ("FLOATERS: never pulled into a chain's flow") and this linear γ-chase did not, so a `:(RETURN)` statement's γ admitted the RETURN floater as a chain CONTINUATION.  That set gin=1 at the STATEMENT_END, suppressing its own gpop, and deferred the release to the floater -- which CANNOT perform it: the floater is a shared JOIN whose predecessors arrive at DISAGREEING depths (zd_depth_census on 083_define_simple_return: preds at 0, 48, 48), so no single static add rsp can serve them, and its exit is a register-indirect `jmp rcx` wire that never reaches the X86H_JMP/X86P_GAMMA hook where op_zgpop is spent -- the staged 48 was silently dropped.  MEASURED at HEAD 3672ab9b: n17_statement_end_α emitted a bare `jmp RETURN` over 48 live bytes (n13 lit 16 + n14 var 16 + n15 binop 16) on BOTH arms; SCRIP_FN_RBP=1 masked it because the RETURN floater's `mov rsp,rbp` discards any depth wholesale, and SCRIP_FN_RBP=2 (no rbp to whack to) popped garbage into rcx and jumped to it -- rc=139.  Cutting here restores the invariant that a release belongs to the EDGE, never to a multi-depth join: the STATEMENT_END stages its own gpop and the existing hook emits it, exactly as the non-goto statements at h=11/h=16 already do.  rl>0 conjunct mirrors the bb_src_of guard directly above -- a floater may still HEAD its own run (h=0 unchanged), it may never be swallowed by another.  Killswitch SCRIP_ZD_FLOATER_CUT=0. */
            run[rl] = ci; rpos[ci] = rl; claim[ci] = hi; rl++;
            cur = zd_chase(cur->γ.node);
        }
        { static int _z5bd = -1; if (_z5bd < 0) { const char * e = getenv("SCRIP_ZD_5B"); _z5bd = (e && *e == '0') ? 0 : 1; }   /* ⭐⭐ ZD-5b ARM DESCENT (s102, lands DESIGN-SN4-ZD5B §4 leaf slice; killswitch SCRIP_ZD_5B=0): the linear γ-walk above threads THROUGH an IR_MATCH_ALTERNATE as one admitted K=0 node, so its arm interiors -- hung as (entry,resume) operand PAIRS, never γ-children -- were closure-only: zon=0, no ZOPQ staging, and every evaluated-operand charset primitive inside an arm fell to the FRQ flat read, whose resolution across the live mrbp frame (64B) + the s61 ALT record (32B) is exactly 96 high (witness probe/zd5b/alt_span_concat_segv: emitted [rsp+216], truth [rsp+120] -- the TDump_driver SIG11 signature verbatim).  Descent roots ONLY at ALT nodes already claimed into THIS run, so their operands are this statement's blob by the closure definition -- the s24a cross-statement steal cannot occur (that blocker descended speculatively before closure).  LEAF-ONLY SLICE per design §6: an arm chain admitting anything outside the op-filtered leaf set (DEFER incl. PATV$, nested ALT/SEQ, captures, ARBNO, FENCE1...) is unclaimed WHOLE and stays closure-only, per-ARM not per-ALT, so a (SPAN(...)|*defer) mix still arms the span arm.  Depth model per design §3a amended s61: arm members' zout = zout[ALT] + intra-arm cumulative K (cells-only ledger; frame+record are off-ledger and ride the crossing sum), reset per arm chunk via the private entry bits.  zarm[ci] carries the enclosing ALT's nodes[] index to the +32 conjunct at the staging site. */
                ...
                for (int a = 0; a + 1 < env->n_operands; a += 2) { IR_t * c2 = env->operands[a]; int rl0 = rl; int bad = 0; int g2 = 0; int first = 1;
                    while (c2 && g2++ <= n) {
                        ...
                        { int aop = (int)nodes[ci]->op; int leaf = (aop == IR_MATCH_LIT || aop == IR_MATCH_LEN || aop == IR_MATCH_ANY || aop == IR_MATCH_NOTANY || aop == IR_MATCH_SPAN || aop == IR_MATCH_BREAK || aop == IR_MATCH_BREAKX || aop == IR_MATCH_POS || aop == IR_MATCH_RPOS || aop == IR_MATCH_TAB || aop == IR_MATCH_RTAB || aop == IR_MATCH_ARB || aop == IR_MATCH_REM || aop == IR_MATCH_BAL);
                          if (!leaf) { bad = 1; break; }
                          if (nodes[ci]->n_operands > 0) { run[rl] = ci; rpos[ci] = rl; claim[ci] = hi; zarm[ci] = ei; if (first) { aent[ci] = 1; first = 0; } rl++; } }   /* ⭐ ZD-5b EVALUATED-ARG-ONLY (s102, measured narrowing): claiming const-form leaves (n_operands==0 -- literal LIT/LEN(k)/POS(k) arms) into the armed regime flipped their β-resume protocol and CRASHED the ALT-backtrack family (bb A07/A08/A09/A13/G00/H07/fence_probe, 166→159); they need no ZOPQ (nothing to read) so they stay closure-only and byte-identical, while evaluated-arg leaves -- the whole broken class -- arm and gain the staged read.  A const leaf mid-chain does NOT abort the arm: the walk passes through unclaimed (leaves have no inter-leaf operand edges, so the convex producer/consumer law is not crossed).  RESIDUAL, documented: β-resume INTO an armed evaluated-arg leaf is unexercised by the witnesses; if a backtrack witness later reds, the resume protocol of the armed arm is the suspect, not this admission. */
         ...
         * above is to arm the MAXIMAL PASSING PREFIX of the run instead of refusing the whole statement (the Icon census reads armed=0/refused=271
         * precisely because one unarmable node vetoes every BB in its statement).  The argument for it is that a refused node's result is not
         * cell-resident so later consumers must refuse too, making arming a PREFIX property; and that the boundary node's gamma/omega targets are
        ...
        if (ok) { int zd = 0; int arm_zd = 0; int zdh_match = -1;   /* M-2 BUG-2 FIX: zdh_match = zd at MATCH_BEGIN entry (= depth of pre-match producer cells only); used as gpop at STATEMENT_END when hfc because the cas_rsp_mark restore collapses all in-bracket cells (pattern-arg cells like LIT_INTEGER), leaving only the pre-match producers for STATEMENT_END to release.  -1 = no hfc MATCH_BEGIN seen yet. */
             ...
             * OPERAND TREES exactly as the refused path's OPERAND CLOSURE comment records.  Two consequences, both fatal to the s22s-b trio-transparent attempt and both repaired here at the
             ...
             * (any position, the refused path's own law) whenever the closure is wider than the run; the k>r positional test is kept VERBATIM for closure==run so every currently-armed value
             ...
             * stages a claim over the closure span exactly as the refused arm does; armed members' cells ride BELOW it (the hook carves claim before cell, cell stays TOS), and each member's
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                int REL = fence0_release_bytes(nodes[i]);   /* ⭐⭐⭐ FZ-3 (s168) THE RELEASE IS NOW VISIBLE TO THE DEPTH MODEL -- THE RUNG s166 NAMED AND REFUSED TO ARM WITHOUT.  THE DEFECT: FZ-1/FZ-2 taught the cut to pop a measured, contiguous, provably-dead frontier, but the pop was invisible here -- zd_plan staged every [rsp+off] in the statement at a depth computed WITHOUT it, so the instant the count went above 0 every static offset to the RIGHT of the cut was stale by exactly that count.  MEASURED s166, one diff: armed vs disarmed on corpus/probe/fz/fz3 case D differed in ONLY the two `add rsp,16` lines -- every staged offset byte-identical while RSP had moved -- and `S ? (SPAN('a') FENCE SPAN('b')) . W` printed `abbb` for the oracle's `aaabbb` because the group's ASSIGN_COND read its SAVE cursor at [rsp+32] when the cut had already lifted RSP to where [rsp+16] named it.  THE FIX IS THE ONE THE ζ MODEL ALREADY SUPPORTS: a cut that pops REL bytes is a node of depth contribution K-REL, so subtract it from the SAME accumulator K is added to and every downstream spelling follows for free -- operand reads (g_zd_read = zd_out[reader] - zd_out[producer]) shrink by REL exactly as RSP rose by REL, and the terminal pops (zgpop/zwpop, both read off zd after this update) pop the remainder rather than double-popping what the cut already freed.  β IS CONSISTENT BY THE SAME LIFO ARGUMENT THAT LICENSED THE COUNT: the released cells are exactly the ones whose β the cut skips, so the box the fence's β jumps to expects RSP precisely at the post-release depth.  0 FOR EVERY NON-FENCE0 KIND AND FOR EVERY DISARMED BUILD (the gate now lives inside fence0_release_bytes), so the default build's depth model is byte-identical.  ⭐ WHY THE ARM CASE BELOW DOES NOT SUBTRACT: an IR_MATCH_FENCE0 can never carry zarm[i]>=0 -- the ZD-5b arm admission op-filter above is leaf-only (LIT/LEN/ANY/NOTANY/SPAN/BREAK/BREAKX/POS/RPOS/TAB/RTAB/ARB/REM/BAL) and rejects the whole arm on anything else -- so REL is 0 there by construction, not by omission. */
                ...
                if (zarm && zarm[i] >= 0) { if (aent[i]) arm_zd = zout[zarm[i]]; zon[i] = 1; zout[i] = arm_zd + K; arm_zd = arm_zd + K; }   /* ⭐ ZD-5b arm-member depth (s102): base = zout[enclosing ALT] (a main-chain member, always earlier in run[] by construction), reset at each arm-chunk entry bit, intra-arm cumulative K -- the cells-only ledger; the frame's 64 and the ALT record's 32 are off-ledger and ride the crossing sum at the staging site, mirroring how MATCH_BEGIN's own carve is handled.  Arm cells never advance the main `zd`: they are in-frame, collapsed wholesale by MEND's mov rsp,rbp, invisible to STATEMENT_END's gpop -- exactly the zdh_match law one line above.  `static` here is a loop-carried local of THIS walk (re-seeded at every aent chunk before first read), not program state. */
                else { zon[i] = 1; zout[i] = zd + K - REL; zd = zd + K - REL; }   /* ⭐ FZ-3: K carves, REL pops, the node contributes the difference.  zout[i] is the depth AFTER this node, which for a cut is the post-release depth -- the value the next node's zout, every operand read that crosses the cut, and this node's own zgpop/zwpop are all derived from. */
                IR_t * gt = zd_chase(nodes[i]->γ.node); IR_t * ot = zd_chase(nodes[i]->ω.node);
                int gin = 0; int oin = 0;
                int gib = port_sz_beta(nodes[i]->γ.sz); { int _gg = 0; IR_t * _g = nodes[i]->γ.node; while (_g && _g->op == IR_GOTO && _gg++ < 128) { if (!gib) gib = port_sz_beta(_g->γ.sz); _g = _g->γ.node; } } gib = gib && !beta_is_stmt_land(gt);   /* ⭐⭐ RTY-GAMMA-RETREAT (s192, row rty-fail-inline-retry): THE ɣ PORT CAN CARRY A β TAG, AND WHEN IT DOES THE ARC IS A RETREAT INTO THE RUN, NOT A STATEMENT EXIT.  Read through the GOTO chase because the tag lives on the intervening RELAY, never on the producer: `S ? POS(0) ARB $ OUTPUT FAIL` lowers FAIL to a bare IR_GOTO whose BOTH ports are φ-tagged (lower_snobol4.c TT_FAIL) and sno_seq_nary's fixup repoints them at the left neighbour as β, so ASSIGN_IMM's own ɣ.sz is CLEAR and only the relay says retreat.  Same chase the drive loop already runs for gamma_is_beta (~3163) and the same !beta_is_stmt_land split as omega_is_retry (~3165): a β-edge into STATEMENT_BEGIN is a pure landing pad, not an in-match back-edge. */
                ...
                else for (int k = 0; k < rl; k++) { if (nodes[run[k]] == gt && (k > r || gib)) gin = 1; if (nodes[run[k]] == ot && !(port_sz_beta(nodes[i]->ω.sz) && beta_is_stmt_land(ot))) oin = 1; }   /* ⭐⭐ RTY-GAMMA-RETREAT (s192): `|| gib` is the ɣ TWIN of the oin relaxation this comment already records -- a retreating ɣ arc lands at or BEHIND its own run position (the FAIL relay chases straight back to the node itself, k==r), so the forward-only test read it as leaving the statement and stamped the statement-terminal release on it: `add rsp,48` fired in front of `jmp n6_match_assign_imm_β`, freeing the ARB retry cursor the retreat was on its way to read, so the generator fired ONCE (oracle A/AB/ABC, SCRIP A).  The STORED road is green on the same pattern because the nblob arm above carries NO positional test at all -- it is the specification, and this is the line that made the two roads disagree.  Keyed on the PORT TAG, never on an op: BAL and ARB are one class here and no member is named.  ORIGINAL: */   /* ⭐ ZD-5b-SAVE-OIN (s14): drop k>r on oin — scan-retry back-edge (ω→MATCH_BEGIN, k<r) is intra-match; forward-only guard was correct for gin but wrong for oin. */
 ...
 * so the default build is byte-identical and this can ride any rung without gating it.  REFUSED nodes (zon=0) are SKIPPED, not assumed depth 0: their depth lives in the UCLAIM wholesale claim, which is
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
extern "C" int xa_flat_class_c_pred(void);   /* CLASS C (s114): ONE AUTHORITY lives in xa_flat.cpp beside the prologue it admits */
...
static int codegen_flat_chain_body(IR_t *entry, const char *prefix) {
    ...
    int _top_hoist = 0;   /* TOP-PORTS (s122, Lon in-chat): 1 = this graph's four CLASS-O ports were emitted at the TOP by bb_main (beside main) instead of at the chain tail; the three tail port groups below are suppressed in exchange.  Computed ONCE at the α define and read at the tail — one decision, recorded once, consulted by its own counterpart (the GLUE-SYM law). */
    int _flt_hoisted[4] = {0, 0, 0, 0};   /* FLOATERS (s123): per-KIND twin of _top_hoist — 1 = RETURN(1)/FRETURN(2)/NRETURN(3) already emitted in the top section, so the drive loop skips that node instead of emitting it a second time.  A FUNCTION LOCAL, never a global: the decision is per-graph and dies with the graph (the NO-NEW-GLOBALS FACT RULE — nothing here needs to outlive this call). */
    int _flt_uid_burn[4] = {0, 0, 0, 0};   /* FLOATERS (s123): uids the hoisted stub consumed out of g_flat_node_id, MEASURED at the hoist and re-consumed at the vacated site so the stream below the old position is byte-identical.  Measured, never assumed — a hardcoded count rots the moment a floater body grows an internal label. */
    const char *fam = (strncmp(prefix, "proc_", 5) == 0) ? prefix + 5 : prefix;   /* PROC-PREFIX-DROP (Lon s119 in-chat: "Remove the proc_ from the labels like proc_PAT$0_α_body"): the EMITTED family stem loses the proc_ routing prefix — labels read PAT$0_α_body / range_ω, not proc_PAT$0_α_body.  The prefix ITSELF is unchanged (still "proc_%s" from the driver): every discriminator below (proc_LBL__ / proc_ / bare) keeps reading `prefix`; only the label TEXT is built from `fam`.  ONE AUTHORITY: fam is computed once here and consumed by all four name-mint sites in this function (six-family, zw5, stγ/stω, dcα); the three by-name external constructors (scrip.c dcα startup ref, scrip.c dormant .Lpn α ref, bb_call_proc_staged dc_name ×2) are renamed in the same commit.  m3's proc_flat diagnostic prefix yields fam "flat" — same inert-duplicate class the s112 comment already blesses (binary label pool resolves by struct offset, never by these names). */
    ...
    int bare = g_emit.flat_bare_chain;   /* BARE-CHAIN (Lon s62): the six-label proc_* wrapper family + port stubs are the pre-floater chain protocol — for SN4 flat proc chains the body statements exit through the RETURN/FRETURN floaters and the ports never fire, so the wrapper is dead weight in the .s.  LBL__ body chains bind exactly ONE label whose asm symbol IS the rt key, LBL__<entry> (the shim's fold target, computed-goto landings, and the LBL__ registration all repoint to it); DEFINE stub chains keep only the unexported wrapper face FN__<FN> (the goto-transfer entry rt_define_site registers) — s112 (Lon in-chat 2026-08-16) renames both faces and drops proc_<FN>_α + its .globl, superseding the s62 keep.  Dangling chain-level γ/ω/β/res names alias to the floaters: for a DEFINE body, falling off the end IS a RETURN and a chain-level failure IS an FRETURN (SPITBOL Ch.8), so the aliases are not a hack but the true semantics; every such site is post-unconditional-jmp dead code regardless.  m4-only by the driver's setting (the role-4 shim and fold arm are MEDIUM_TEXT this seat — same owed m3 class as s58 tiny). */
    if (strncmp(prefix, "proc_LBL__", 10) == 0) emit_label_initf(&lbl_α, "%s", prefix + 5);   /* s112 (Lon in-chat): the label-entry face's asm symbol IS the rt key, LBL__<entry> — supersedes the s62 <entry>_body spelling; unconditional on prefix so every emission path's definition matches the ONE reference spelling */
    else if (strncmp(prefix, "proc_", 5) == 0 && !bare) emit_label_initf(&lbl_α, "FN__%s", prefix + 5);   /* s112 (Lon in-chat): FN__<FN> face, unexported — proc_<FN>_α removed; m3's proc_flat/pat_flat diagnostic prefixes ride the same arm, inert (binary label pool, never looked up by these names).  FN-FACE-DELETE (Lon s114 in-chat: "DELETE this label FN__ROMAN ... Is not needed"): the BARE class (kind-1 DEFINE stub blob) gets NO chain face at all — its first node's own port label (n<uid>_save_restore_α) is the only name at that address, and the ONE live consumer (bb_define_bind's define-site lea) now targets LBL__<entry> via the dentry stamp; the α define below is skipped for this class (fn_face_dead).  Kind-2 EXPR$/PAT$ thunks keep the FN__ face: the m4 startup loop (scrip.c rt_proc_set_fn lea) still references it for them. */
    int fn_face_dead = bare && strncmp(prefix, "proc_", 5) == 0 && strncmp(prefix, "proc_LBL__", 10) != 0;   /* FN-FACE-DELETE (Lon s114): the bare stub's chain-α label is never defined — nothing may reference it (link gate proves it); lbl_α keeps its default name for diagnostics only */
    if (bare) { emit_label_initf(&lbl_γ, "main_\xcf\x89"); emit_label_initf(&lbl_ω, "main_\xcf\x89"); emit_label_initf(&lbl_β, "main_\xcf\x89"); emit_label_initf(&lbl_res, "main_\xcf\x89"); }   /* s62b: the dangling-port aliases MUST name a label every program defines — RETURN/FRETURN floaters are seeded only when the graph's own edges reference them (MEASURED: 1014_func_freturn's body touches only FRETURN, so the shim chain's dead `jmp RETURN` fallback was an undefined reference and the link failed).  main_ω is unconditional and is the correct landing for a port that can only fire on protocol violation: exit 1. */
...
#undef RPO_VISITED
    ...
    unsigned char *zd_on = (unsigned char *)alloca((size_t)(n > 0 ? n : 1)); int *zd_out = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int *zd_gp = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int *zd_wp = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1)); int *zd_arm = (int *)alloca(sizeof(int) * (size_t)(n > 0 ? n : 1));   /* ZD-5b (s102): per-node index of the enclosing IR_MATCH_ALTERNATE for arm-interior members claimed by the descent; -1 = main chain.  Consumed by the op_zread crossing sum (+32 ALT-record conjunct beside the s99 +64 mrbp arm). */
    if (!g_emit.flat_jmp_entry) g_emit.flat_pat = 0;   /* [ALPHA] s32 STALE-FLAT-PAT FIX: emit_jmp_entry_for_patproc sets flat_pat=1 for PAT$ blobs; emit_jmp_entry_clear resets it, but only jmp-entry chains call clear before their emit_chain. A main chain that follows a PAT$ blob emission inherits flat_pat=1, making zd_plan's FENCE1 zdyn veto at emit.cpp:1975 believe it is inside a blob and fire unnecessarily -- refusing match-construction statements containing FENCE1 that are provably outside any blob. Guard: jmp-entry chains legitimately have flat_pat=1 (set by their arm function before emit_chain) and must keep it. Non-jmp-entry chains have no mechanism to set flat_pat=1 before their own emit_chain, so any 1 they see is stale from a previous blob. MEASURED: 10 spurious FENCE1 vetoes eliminated; 105->99 refused runs; s32. */
    zd_plan(nodes, n, zd_on, zd_out, zd_gp, zd_wp, zd_arm);   /* ZD-1: the ONE execution-order walk runs ONCE per chain, before any emission, over the same nodes[] the loop below lays down */
    ...
    { /* ZK-0 (s212): per-graph cell census. READ-ONLY: zero g_emit writes, zero emission. Inert unless SCRIP_ZD_CENSUS=1. Accumulates graph-level: total K*bytes of armed nodes, armed node count, refused run first-blocker kind histogram, .bss arena bytes. FALSIFIABILITY: (1) a known-armed run (IR_TO with K=32) moves census_k_total up by 32; (2) a known-refused graph (locals-only, pin=0) shows blocker=IR_VAR in the histogram -- these two injections are the ZK-0 completion criterion before any rung code lands. icn_cells_graph=1 flag (set by lower_icon.c when SCRIP_ICN_CELLS=1) routes this printout; flag=0 graphs are SN4/Prolog and their line is suppressed so the census is not polluted. */
    ...
    if (_zk0_on && g_emit_cfg && g_emit_cfg->icn_cells_graph) {
        ...
        /* blocker histogram: count first-blocker op per refused run (simple: per unrefused node in a run that was not armed) */
        ...
        /* scan refused runs: a run that starts at a non-armed node whose zd_wl_kind returns 1 but zd_plan refused it -- first non-admitted node is the blocker */
        ...
        fprintf(stderr, "[ZK-CENSUS] graph=%s icn_cells=1 n=%d armed=%d K_total=%ld refused_nodes=%d bss_arena=%d globals_on_stack=%d\n",
                prefix ? prefix : "?", n, _armed, _k_total, _unpriced_runs, _bss_bytes, _globals_on_stack);   /* ZK-5: globals_on_stack must be 0 -- asserted by test_gate_icn_zk5_gva.sh */
    ...
    { /* PL-ZK-0 (s3): per-graph Prolog cell census. READ-ONLY: zero g_emit writes, zero emission. Inert unless SCRIP_ZD_CENSUS=1. Same structure as the Icon census above; gated on pl_cells_graph so Icon/SN4 output is not polluted. FALSIFIABILITY: (1) a known-armed run moves armed UP; (2) a known-refused graph names the first blocker. Both injections required before rung declared complete. */
    ...
    { static int _tp = -1; if (_tp < 0) { const char * e = getenv("SCRIP_TOP_PORTS"); _tp = (e && *e == '1') ? 1 : 0; }   /* ⭐⭐⭐ TOP-PORTS (s122, Lon in-chat: "Finish moving the main and return stuff from the bottom to the top of the generated file"): the CLASS-O outer main's four ports are STATIC (β = jmp ω, γ = exit 0, ω = exit 1) and belong beside main, not 900 lines below it.  The admission predicate is the s118 equivalence proof verbatim — every conjunct is a shape in which β is NOT simply `jmp ω` or in which the chain head is not the program entry, and each one refuses rather than mis-hoist.  OPT-IN FIRST per the s203 ZW-1 lesson; default OFF is byte-identical by construction (the tail groups below run exactly as before when _top_hoist==0). */
      ...
      if (_cls_o) for (int _i = 0; _i < n; _i++) if (nodes[_i]->op == IR_SUSPEND) { _cls_o = 0; break; }   /* a suspending graph's β is a RESUME landing, not `jmp ω` — the tail dispatch below is the only authority that can name it */
      _top_hoist = (_tp && _cls_o) ? 1 : 0;
      if (_top_hoist) {
          g_emit.flat_main_body_p = &lbl_α_body; g_emit.flat_main_ω_p = &lbl_ω;   /* STAGE — the xa_strtab_rodata discipline (emit.cpp:447-454): stage, dispatch, clear, all inside one window so no per-graph reset can reach it */
          bb_emit_x86(bb_main_entry_bridge());   /* ⛔ MANDATORY AND FIRST: emit_chain returns (bb_box_fn)buf in BINARY, so the chain's ENTRY POINT IS THE FIRST BYTE EMITTED.  Without this jmp the hoisted β becomes the entry, `jmp ω` fires, and the program exits 1 having run no statement — the exact s118 ports-only fibonacci m3 rc=1 signature. */
          ...
          for (int _k = 1; _k <= 3; _k++) { int _has = 0; for (int _i = 0; _i < n; _i++) if (emit_floater_kind(nodes[_i]) == _k) { _has = 1; break; } if (!_has) continue; _flt_hoisted[_k] = 1;
              ...
              g_emit.op_ival = _k; g_emit.op_define_role = (_k == 3) ? 0 : _k;   /* ⛔ FLOATER KIND ≠ DEFINE ROLE: kind 3 is NRETURN (bb_nreturn_mark), while bb_define's role 3 is wire-adopt — the two numberings are unrelated and only bb_main_floater's own dispatch may cross between them */
              int _uid0 = g_flat_node_id; emit_sep_rule('-'); bb_emit_x86(bb_main_floater(_k)); _flt_uid_burn[_k] = g_flat_node_id - _uid0; g_flat_node_id = _uid0; }   /* ⭐⭐ FLOATERS HOISTED (s123, s122 cursor NEXT RUNG (a)): the singleton landings are STATIC — nothing in their bodies reads the node — so emitting them here and skipping them in the drive loop is a pure MOVE, and it takes them out of the drive loop's emission-order staging accumulators entirely (the zd_sr_role ival 1/2 collision the s122 cursor named as what broke cap_imm_nret under the s118 full variant). */
              /* ⛔⭐⭐⭐ THE UID REWIND IS THE WHOLE CORRECTNESS ARGUMENT, AND THE FIRST BUILD OF THIS RUNG GOT IT WRONG (retracted in-session, before any commit).  x86_begin() is `_.x86_uid = g_flat_node_id++` (x86_asm.h:926) — THE SAME counter the node-label mint loop consumes at line 2812 — so a template emitted UP HERE, ahead of that loop, steals uids from every node below it.  Measured: bb_define_sr calls x86_begin once per floater, bb_nreturn_mark not at all, so hoisting RETURN+FRETURN rebased the ENTIRE downstream stream by +2 (n22→n24, .Lx116→.Lx118, .Lrkfnzd119→.Lrkfnzd121).  That is the s118 FINDING's renumbering mechanism arriving from the direction its author argued it could not, and a consistent rename is exactly what s118 looked like TWICE before it was measured broken.  So we do not ship a rename and call it cosmetic: we BURN AND REWIND.  The burn count is MEASURED, never assumed (a hardcoded 1 would rot the moment a floater body grows an internal label), rewound here so the mint loop sees the stream it always saw, and re-consumed at the vacated site so the nodes BELOW the old position see theirs.  SAFE BECAUSE THE FLOATER BODIES CARRY NO INTERNAL LABELS — verified in the emitted .s: not one .Lx in any of the three — so the rewound uids are pure burn and can never collide with a body that reuses them.  BINARY is untouched throughout: x86_begin is gated !MEDIUM_BINARY, so m3 never had the drift and the rewind is a no-op there. */
          g_emit.flat_main_body_p = (bb_label_t *)0; g_emit.flat_main_ω_p = (bb_label_t *)0;   /* CLEAR */
      } }
    if (text_externalise && g_is_text && !fn_face_dead) emit_label_define_bb(&lbl_α);   /* FN-FACE-DELETE (Lon s114): bare DEFINE stubs define no chain face — n0's own port label is the address's only name */
    ...
    bb_label_t **st_pre = (bb_label_t **)alloca(sizeof(bb_label_t *) * n);   /* STMT-FRAME (s21x-c): per-statement-head enter stub label -- minted for every bb_src_of head when the regime is live, NULL otherwise; every cross-edge into a head lbls[k] retargets here (slice-1 op-filter has no gotos, so EVERY edge into a head is a genuine statement entry -- no membership map needed) */
    int st_first_seen = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (g_emit_cfg && g_emit_cfg->n_dentry > 0) { for (int _dq = 0; _dq < g_emit_cfg->n_dentry; _dq++) if (g_emit_cfg->dentry_entry[_dq] == nodes[i]) g_emit_cfg->dentry_name[_dq] = lbls[i]->name; }   /* NATURAL-LABEL (Lon s114): record the DEFINE body-entry statement's natural port label name (n<uid>_statement_begin_α) — uids are minted HERE, so this is the one moment the name exists before emission; the bind dispatch deposits it into lbl_t0 for bb_define_bind's fn lea and the inline role-5 shim.  Pool-owned string: valid for this chain's whole emission, consumed only within it. */
    ...
    for (int i = 0; i < n; i++) {
        { int _fk = emit_floater_kind(nodes[i]); if (_fk && _flt_hoisted[_fk]) { g_flat_node_id += _flt_uid_burn[_fk]; continue; } }   /* ⭐⭐ FLOATERS (s123): the singleton landing was already emitted, whole, in the top section — driving it here would emit it TWICE and define its one shared label twice.  The `continue` is the rung: the node leaves the loop entirely, so it can no longer touch the emission-order staging accumulators that zd_sr_role's ival 1/2 collide with.  ⛔ THE BURN COMES FIRST AND IS NOT OPTIONAL: the hoisted stub consumed these uids up top and rewound them, so THIS is where the stream must absorb them — skip without burning and every .Lx/.Lrkfnzd/n<uid> below rebases (measured +2 on cap_imm_nret before the rewind landed).  Reached only under SCRIP_TOP_PORTS=1 (_flt_hoisted is all-zero otherwise), so the default arm is byte-identical by construction. */
         ...
         * ω wire resolved to a OP-FILTERED trivial-β trampoline (flat_trivial_beta -- template β is exactly
         ...
         * hop is again a op-filtered β.  node_ω lands the chain's FINAL label; op_wpop carries ΣK; the
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0) { g_zd_stage = 1; g_zd_arm = (zd_on[i] && !(g_emit_cfg && g_emit_cfg->pl_cells_graph && !g_emit.flat_all_zd)) ? 1 : 0; int _pl_suppress = (g_emit_cfg && g_emit_cfg->pl_cells_graph && !g_emit.flat_all_zd) ? 1 : 0; g_zd_gpop = _pl_suppress ? 0 : zd_gp[i]; g_zd_wpop = _pl_suppress ? 0 : (_uw_stolen ? (int)_uw_pop : ((_zw5_wpop_stolen || _endj_stolen) ? 0 : zd_wp[i])); g_zd_wsteal = _endj_stolen || _uw_stolen;   /* PL-ZK-3 ARM+RELEASE SUPPRESS preserved through M-3 merge: mixed armed+refused pl_cells_graph (flat_all_zd=0) has no carve, so arm and gpop/wpop must zero or ZOPQ/add-rsp corrupt. ONE AUTHORITY: same predicate at prologue carve suppressor. */   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; mechanism-2 g_zd_uzwr staged alongside g_zd_uzw */
              ...
              if (zd_on[i]) { int _no = zd_nops(nodes[i]);
                  ...
                  IR_t * _p = nodes[i]->operands[_zj]; for (int _k = 0; _k < n; _k++) if (nodes[_k] == _p) { int _xh = 0; for (int _zm = i - 1; _zm > _k; _zm--) { if (zd_arm[i] >= 0 && _zm == zd_arm[i]) { _xh += 32; continue; }   /* ⭐ ZD-5b ALT-RECORD CROSSING (s102): an ARM-INTERIOR consumer (zd_arm[i] = its enclosing ALT's nodes[] index, staged by the descent) reading a producer from before the ALT crosses the ALT's live 32B α record (bb_match_alternate sub rsp,32, s61) -- off-ledger to zout exactly like the mrbp frame below, same loop, same shape.  Gated on the consumer's OWN enclosing ALT only: post-ALT main-chain members (RPOS, END...) keep zd_arm=-1 and are byte-identical; nested ALTs never reach here (the descent unclaims arms containing them).  Witness probe/zd5b/alt_span_concat_segv: 216-96 = the measured-correct 120 = this 32 + the mrbp 64 below + the producer delta. */ if (nodes[_zm]->op != IR_MATCH_BEGIN) continue; extern int fc_head_fp(const IR_t *); extern int fc_tail_head(const IR_t *); extern int emit_match_rbp(void); if (emit_match_rbp()) { extern int sn4_xh_frame_extra(void); extern int emit_match_begin_frame_extra(const IR_t *); _xh += 64 + (sn4_xh_frame_extra() ? emit_match_begin_frame_extra(nodes[_zm]) : 0); }   /* ⭐⭐⭐ s184 CARVE-EXTRA IS PART OF THE HEAD MOTION: the 64 above is push rbp + 4 pushes + sub 24, i.e. the carve with extra==0.  emit_match_begin_frame_extra() adds 16 per registry candidate ON TOP of that 24, and that motion is just as real to a cross-head read as the 64 is -- omitting it left every flat operand read inside the frame short by exactly extra (s124 two-reader law: the carve authority and the depth authority disagreed, and nothing in src/ read the carve but the prologue template).  Summed per crossing exactly like the 64, so nested heads accumulate correctly. */ else if (x86_zc_frame() == ZC_FRAME_RSP && x86_port_mode() == ZC_PORT_FORTH && (fc_head_fp(nodes[_zm]) >= 0 || fc_tail_head(nodes[_zm]))) _xh += 32; } g_zd_read[_zj] = zd_out[i] - zd_out[_k] + _xh;   /* ⭐ R-3(a) MRBP CROSS-HEAD FRAME (GOAL-SNOBOL4-100 s98 cursor, THE rt_sg_scan_nonmember class -- Gen/Qize/XDump): under emit_match_rbp() MATCH_BEGIN's alpha is push rbp + 4 pushes + sub 24 = 64B of real RSP motion, replacing the legacy hfc 32B carve entirely (witness Gen_driver n2250: no sub rsp,32 exists on the mrbp arm); zd_k(MATCH_BEGIN)=0 by design and emit_match_begin_stfh_k() deliberately reads 0 under mrbp (correct for RELEASE -- the frame self-releases via mov rsp,rbp/pop rbp -- but the in-match ZOPQ spelling of PRE-FRAME cells was left 64 short: producer n2249_coerce_string at [rsp+96..104] actual, read at [rsp+32..40] planned; rt_sg_scan.S:41 needle read then SIGSEGVd on stack garbage).  SAME loop, SAME per-crossing sum as the L-4/061 hfc arm below it -- the mrbp arm takes precedence because the two carves are mutually exclusive branches of bb_match_begin.cpp:34.  In-frame->in-frame reads never cross a MATCH_BEGIN in [_k..i], so they stay byte-identical by construction. */   /* ⭐ L-4/061 CROSS-HEAD MATCH_BEGIN CARVE (this session, gdb-verified on crosscheck/capture/061_capture_in_arbno.sno at N=0 AND N=2, real linked mode-4 binary, not hand-derived): a producer before a run-local MATCH_BEGIN, read by a consumer after it, is short by exactly MATCH_BEGIN's own hfc() runtime carve -- sub rsp,32 at bb_match_begin.cpp:77 -- gated here by the IDENTICAL predicate bb_match_begin.cpp's hfc()/op_fc_wbytes uses (fc_head_fp>=0||fc_tail_head under ZC_FRAME_RSP+ZC_PORT_FORTH; mirrors emit.cpp:2720's g_zd_zfc computation for the same _mb).  That carve is INVISIBLE to zd_out because zd_k(IR_MATCH_BEGIN)=0 by design (no value cell of its own) -- ordinary armed nodes between producer and MATCH_BEGIN are NOT under-counted (K is additive; MATCH_BEGIN's own K=0 does not disturb the running sum), only the off-ledger carve is missing.  CORRECTS the prior ZD-5b-LEN comment at this site: its zd_ud/zdh fields exist nowhere in this tree (grep-confirmed, matches the s45 FINDING) and its worked number (deficit 48=32+16 for witness 061) does not reproduce -- gdb on the real linked binary measured the true deficit as 32 flat, and emitted+32 read back exactly N's coerced value on both witnesses.  Summed across every run-local MATCH_BEGIN found, not just the nearest, so a nested head is not silently under-corrected -- UNVERIFIED against an actual nesting witness, treat that arm as inherited-unverified until one exists.  Byte-identical when no cross-head MATCH_BEGIN exists (_xh=0): reduces to the prior zd_out[i]-zd_out[_k]. */ g_zd_kind[_zj] = (int)ir_norm_call_kind(_p->op); break; } } } }   /* ZD-1 staging (R5b driver-handoff): op_zread[k] = the DIFFERENCE OF TWO DEPTHS (s21x-u), zout[consumer] - zout[producer], both from the ONE walk.  ⛔ ZD-2d/2e FIX: this site RE-DERIVED the operand count inline (UNOP/ASSIGN?1:BINOP?2:0) instead of calling zd_nops, so the planner and the driver held TWO OPINIONS of how many operands a kind has -- the moment zd_nops grew COERCE_NUMERIC/CMP_TEST the planner ARMED their statements while this loop still computed _no=0, left g_zd_read all zero, and every ZOPQ/ZOPD rendered [rsp+0] = the box's OWN cell.  29 programs, every one silently wrong rather than crashing (rt_cmp_d comparing an uninitialized cell against itself).  ONE authority now; a kind added to zd_nops is staged here automatically.  K is no longer a second copy: s22k folded it into zd_k beside zd_nops, so BOTH one-authority laws now hold on this staging path. */
              { if (zd_on[i] && nodes[i]->op == IR_MATCH_REPLACE) { g_zd_read[2] = -1; g_zd_kind[2] = -1; IR_t * _mb2 = (IR_t *)0; int _jh2 = -1; for (int _zj = i - 1; _zj >= 0; _zj--) { if (nodes[_zj]->op == IR_MATCH_BEGIN) { _mb2 = nodes[_zj]; _jh2 = _zj; break; } } IR_t * _sp = (_mb2 && _mb2->n_operands > 0) ? _mb2->operands[0] : (IR_t *)0; if (_sp && _jh2 >= 0) { for (int _js = _jh2 - 1; _js >= 0; _js--) if (nodes[_js] == _sp) { if (zd_on[_js]) { g_zd_read[2] = g_zd_zunder + (zd_out[_jh2] - zd_out[_js]); g_zd_kind[2] = (int)_sp->op; } if (getenv("SCRIP_ZPAT_DIAG")) fprintf(stderr, "[SUBJ2] jh=%d js=%d zout_jh=%d zout_js=%d zon_js=%d staged=%d\n", _jh2, _js, zd_out[_jh2], zd_out[_js], zd_on[_js], g_zd_read[2]); break; } } } }   /* ⭐ R-3(c) SUBJECT SEAT (GOAL-SNOBOL4-100 s99 cursor "fourth seat", REGIME-INDEPENDENT, root cause = s97's DEAD-SPILL deletion removed the head's op_sa mirror from BOTH head arms while bb_match_replace still read FRQ(op_sa) -- the flat slot nobody writes in the armed world; l3 ladder 0/14, tiny `S 'b' = 'X'` -> 'X', SCRIP_REPL_TRACE slen=0 both regimes): the armed splice reads the SUBJECT from its producer's LIVE spine cell (statement-scope, survives the MEND frame whack) via a staged raw-cell displacement, per the ZD-5 MATCH-SPINE ZOPQ(2) design of record.  FORMULA (physically exact at the splice, NOT the generic loop's model -- the whack breaks the pure depth model here): displacement = zunder (replacement subtree K, live above the splice) + (zd_out[head] - zd_out[subj]) (the subject cell's offset below the head-alpha frontier R0, i.e. armed coerce-chain K between producer and head -- the 061 lesson); NO crossing +64 term (the frame is DEAD at the splice -- MEND's mov rsp,rbp restored R0) and NO pattern-carve term (whacked with it), which is exactly why the generic staging loop CANNOT serve this seat: it would add K_pattern + the mrbp 64.  MUST run AFTER the generic loop: its unconditional 6-slot clear zeroes any earlier staging (measured: staged 16 printed, [rsp+0] emitted).  -1 sentinel (producer absent from nodes[] or refused) keeps the legacy FRQ(op_sa) read byte-identical -- degrade never die. */
    ...
    if (g_emit.flat_jmp_entry) {
        ...
        if (g_emit.flat_pat) {   /* W-MAP(3) res landing: the site jumped to [rsp+0]; the record is intact beneath.  Reload BOTH wires and drop 32B so the resumed interior speaks the same r10/r11 it was entered with.  r10/r11 are DESTINATIONS ONLY -- using either as scratch would destroy the wire it must return through. */   /* ⭐ R-4(b) (s97): spelled through x86() -- BOTH-MEDIUM by construction, byte-identical to the raw text/bytes it replaces (TEMPLATE-ONLY law; the ex-R-7 raw-byte item for this glue is discharged here); a framed blob (blob_frame_bytes()>0) also re-seats its OWN activation base rbp from record[+24] (the γ twin pushed it in the pad slot) before the interior resumes. */
            bb_emit_x86(x86_rsp_load64("r10", 8) + x86_rsp_load64("r11", 16) + IF(blob_frame_bytes() > 0, x86_rsp_load64("rbp", 24)) + x86("add", "rsp", 32L));   /* raw rsp displacements: x86_rsp_load64 is the UNCOMPENSATED encoder -- the "qword ptr [rsp + N]" text spelling parses as the FR64 frame prefix and picks up _.op_zdepth (measured s97: +16 under SCRIP_MATCH_RBP=0), which is a template-node notion this glue is outside of. */
        } else {
    ...
    emit_sep_rule('-'); emit_label_define_bb(&lbl_β);
    {
        ...
        { extern int sn4_defer_resume(void); const int _dres = sn4_defer_resume();
          if (getenv("SCRIP_RESUME_WHY")) fprintf(stderr, "[RESUME-GATE] dres=%d flat_pat=%d cfg=%d body_root=%p\n", _dres, g_emit.flat_pat, g_emit_cfg ? 1 : 0, (void *)(g_emit_cfg ? g_emit_cfg->body_root : (IR_t *)0));   /* ⛔ DIAGNOSTIC ONLY (s182): the OUTER gate of the blob β-dispatch, printed even when it refuses. */   /* ⭐ SN4-DEFER-RESUME (s121) half B2: the SNOBOL4 arm of the resume dispatch, mirroring the two Prolog arms above.  A PAT$ blob (flat_pat graph) β-arrival — the invoker's `jmp [rsp+0]` through the CLASS D suspension record, res stub restored wires+frame, interior state retained on the spine below — lands the published resume carrier's β: a generator's β EXTENDS (ARBNO one more instance, manual p.121; ARB one more char), a deterministic carrier's β fail-throughs to its ω and rides r11 home through the absolute unwind.  Without this arm the dispatch defaulted to lbl_ω: every stored ARB/ARBNO answered exactly once and took its shy null match forever — the s120 FINDING's closed SUCCEED/FAIL pair, beauty's `Parse Error`.  Same switch as the lowering half; =0 restores the default byte-identically. */
          if (_dres && g_emit.flat_pat && g_emit_cfg && g_emit_cfg->body_root) {
            int _f1 = 0; for (int i = 0; i < n; i++) if (nodes[i]->op == IR_MATCH_FENCE1) { _f1 = 1; break; }   /* ⭐ SN4-DEFER-RESUME FENCE REFUSE (s121): belt to the builder's suspenders — a FENCE1 node anywhere in the blob graph refuses the resume arm even if a carrier slipped through (FENCE0 is node-free and only the builder's tree check can see it; FENCE1 leaves this node and both ends catch it).  Refused = the pre-s121 lbl_ω default verbatim. */
            int _sd = 0; for (int i = 0; i < n; i++) if (nodes[i]->op == IR_MATCH_DEFER && nodes[i]->seal == 1) { _sd = 1; break; }   /* ⭐ s177 PF-1b: sealed-defer census, consumed ONLY by the ALT-carrier (second) admission below -- a seal==1 DEFER's β is the fence-demarcation frame unwind, correct only in its own activation context; the ALT arm thunks stamp per-ARM β targets and an arm ending in a sealed defer would jump that unwind against the BLOB's rbp (150_pat_star_var_fence_alts SEGV, the s128 residue met head-on).  β→ω for such a blob IS fence semantics (manual p.125/p.127: backing up through FENCE fails the match).  ⛔ Deliberately NOT in _f1: a blob whose resume carrier is tier-1 (ARBNO/ARB extend) re-enters through the CARRIER's β, never the sealed defer's -- folding this into _f1 broke 143_pat_regex_quantified_class (ARBNO(*LP), LP fence-sealed: the extend road is not a back-up through the fence). */
            int _f1r = _f1; { extern int zdp_seam_tier(const IR_t *); const char * _fre = getenv("SCRIP_FENCE_RESUME"); if (!(_fre && *_fre == '0') && g_emit_cfg->body_root && g_emit_cfg->body_root->op != IR_MATCH_FENCE1 && (zdp_seam_tier(g_emit_cfg->body_root) == 1 || zdp_seam_tier(g_emit_cfg->body_root) == 3)) _f1r = 0; }   /* ⭐ s189: tier 3 joins tier 1 at the fence arm on the manual's own ground -- FENCE fails when the scanner BACKS UP THROUGH it, and re-offering an arm at the cursor the ALT was entered with backs up through nothing. */   /* ⭐⭐⭐ FENCE-RESUME (s182, HQ Fable; SCRIP_FENCE_RESUME=0 reverts to the wholesale refusal byte-identically).  THE s121 `_f1` REFUSE IS A SHAPE REFUSAL AND LAW 0d FORBIDS IT: a FENCE1 ANYWHERE in the blob vetoed the resume arm for the WHOLE graph, so a stored `ARBNO(FENCE(X))` emitted `PAT$N_β: jmp PAT$N_ω` and CONCEDED instead of extending -- witness ptw_min_arbno_fence_defer, and beauty's own `Parse = nPush() ARBNO(*Command) ...` where `Command = nInc() FENCE(3-arm ALT)`.  THE MANUAL IS THE AUTHORITY (v3.7 Ch.9/18): FENCE "fails if the scanner has to BACK UP THROUGH it", and ARBNO's β does not back through anything -- it EXTENDS FORWARD, supplying one more instance at the current cursor (`ARBNO(P)` ≡ `( "" | P | P P | ... )`).  Those are different motions, and the wholesale scan could not tell them apart.  Narrowed to the BOX BEING RE-ENTERED: a tier-1 generator body_root (ARBNO/ARB/BAL, the zdp lattice's own "beta EXTENDS" class) is enterable regardless of what its body contains; a body_root that IS the FENCE1 still refuses, and the fence template's own β still concedes if it is ever actually reached, so the cut semantics are untouched.  The ALT-carrier admission below deliberately keeps the wide `_f1` -- one rung, one change. */
            if (getenv("SCRIP_RESUME_WHY")) { extern int zdp_seam_tier(const IR_t *); const IR_t * _br = g_emit_cfg->body_root; int _found = 0; for (int i = 0; i < n; i++) if (nodes[i] == _br) _found = 1;
                ...
                fprintf(stderr, "[RESUME-WHY] flat_pat=%d dres=%d f1=%d f1r=%d sd=%d body_root_op=%d tier=%d seal=%d in_nodes=%d n=%d nc=%d lf=%d fn=%d cro=%d\n", g_emit.flat_pat, _dres, _f1, _f1r, _sd, _br ? (int)_br->op : -1, _br ? zdp_seam_tier(_br) : -1, _br ? (int)_br->seal : -1, _found, n, _wnc, _wlf, _wfn, sn4_choice_rbp_off()); }   /* ⛔ DIAGNOSTIC ONLY (s182): prints WHY the blob β-dispatch chose its resume target.  No verdict, no behaviour change; getenv at the site so nothing can be left half-armed. */
            ...
            if (!_f1 && !_sd && resume_tgt == &lbl_ω && sn4_alt_carrier() && g_emit_cfg->body_root && (g_emit_cfg->body_root->op == IR_MATCH_ALTERNATE || g_emit_cfg->body_root->op == IR_DISJUNCTION) && !g_emit_cfg->body_root->seal) {
                int _nc = 0, _lf = 0, _fn = 0; sn4_blob_choice_scan(&_nc, &_lf, &_fn);
                if (_nc == 1 && !_fn && (_lf || sn4_choice_rbp_off() != 0)) for (int i = 0; i < n; i++) if (nodes[i] == g_emit_cfg->body_root) { resume_tgt = betas[i]; break; }   /* s128: second admission -- a NON-LEAF single-choice framed blob whose arm record is rbp-resident (sn4_choice_rbp_off()!=0) is depth-immune BY CONSTRUCTION: res re-seats rbp from record[+24], alternate_β reads [rbp+cro+8], and the winning arm's interior carve (retained on the spine across γ, unwound by its own β chain) can no longer shift the read.  _lf keeps the s127 contiguity admission verbatim. */ } } }   /* ⭐⭐⭐ s127 R-4(a) reader (2) of sn4_alt_carrier: THE ALT-CARRIER DEPTH STORY, EARNED NOT ASSUMED.  s121 convicted the ALT carrier because 150's winning arm carved interior state, shifting the 32B arm record under res's fixed -32.  The gate here is the missing half: ONE unsealed choice node (== body_root) and EVERY other graph member a pure leaf matcher (sn4_blob_choice_scan) => the winning arm carves NOTHING between alternate_α's record and γ's suspension push, so res's `add rsp,32` lands rsp EXACTLY on the arm record and alternate_β's `mov rax,[rsp+8]; jmp rax` reads the pending arm-resume at the depth it was written (contiguity by construction, the same argument as the s125 inline admission).  Exhaust composes: ALT af pops its own 32B and rides ω->r11 (the record-restored ARBNO af wire) home with r14d = the instance's ENTRY cursor, which the af Δ0 compare then uses to cascade one instance further left or fall to ω -- manual Ch.18's pop-the-stack, stated per-instance.  Guarded behind resume_tgt==&lbl_ω so SUSPEND/DISJ-prolog/GEN/tier-1 arms keep precedence, behind _f1 (FENCE1 refuse, s121's belt), and inside the _dres block so SCRIP_DEFER_RESUME=0 restores legacy wholesale. */   /* ⭐ CARRIER = LEAF GENERATOR ONLY (s121, board-convicted twice this session): an ALTERNATE carrier resumed here read its 32B arm record at a depth the winning arm's interior had shifted (150's negative discriminator, jmp-through-0, rip=0 with LIVE wires) — the ALT/DEFER-carrier resume needs its own depth story and is the NAMED NEXT RUNG, not a widening of this one.  ARB/ARBNO/BAL βs consume exactly the state their own γ left at the frontier, which is exactly where res lands rsp — the class the rung was opened for and the only class with 12-run witnesses. */
    ...
    if (g_emit.zframe_graph || (g_emit_cfg && g_emit_cfg->icn_cells_graph && g_emit.flat_lcl_proc && g_emit.flat_jmp_entry)) { extern void xa_flat_zframe_epilogue_γ(void); xa_flat_zframe_epilogue_γ(); }   /* ZK-4 SLICE 1 (s215): CLASS ZF widened to the CELLS arm. Predicate mirrors xa_flat_class_zf() in xa_flat.cpp (the template-side ONE AUTHORITY); the header base differs per regime and is computed there by xa_flat_wire_hdr_base().  ZK-2 (s226): flat_jmp_entry conjunct added to cells-arm condition. The outer main graph has flat_jmp_entry=0 (entered from C via bare jmp, no wire installed in rcx/rdx); its γ exit is call exit@PLT, not wire-jmp. Procedures (noop, fib, etc.) have flat_jmp_entry=1 and exit via CLASS ZF wire-jmp. Without this guard, cells-arm main's γ would jmp through garbage rcx = SEGV. */   /* ICN-FR-2 CLASS ZF γ: direct wire-header read, depth-immune unwind (lea rsp,[___+kt]; load rcx=[___+kt-24]; restore ___=[___+kt-8]; jmp rcx).  BOTH MEDIUM via x86() in xa_flat.cpp.  Precedence over _blob_wire/_wire_stub: those are 0 for ζ-frame graphs by construction (floor=0, flat_lcl_proc=0, flat_pat=0). */
        ...
        int _bfb = blob_frame_bytes();   /* ⭐ R-4(b) (s97): spelled through x86() -- BOTH-MEDIUM by construction, byte-identical to the raw text/bytes it replaces for the zero-demand blob (TEMPLATE-ONLY law). A FRAMED blob fills the record's pad slot with its own activation base (`push rbp` in place of `sub rsp,8` -- same 32B, same alignment) and then restores the CALLER's rbp from [rbp+0] before riding γ out: the caller's interior continues against ITS activation, and the interior state + frame stay retained on the spine below the record until the invoker's β `jmp [rsp]` lands in res (which re-seats rbp from record[+24]) or a bracket whack discards them. */
        bb_emit_x86(IF(_bfb > 0, x86("mov", "r10", RDQ("rbp", -8)) + x86("mov", "r11", RDQ("rbp", -16))) + (_bfb > 0 ? x86("push", "rbp") : x86("sub", "rsp", 8L)) + x86("push", "r11") + x86("push", "r10") + x86_lea_ext("rax", &lbl_res) + x86("push", "rax") + IF(_bfb > 0, x86("mov", "rbp", RDQ("rbp", 0))) + x86_jmp_reg("r10"));
    }
    ...
    else if (_blob_wire) { extern int sn4_blob_casmark(void); bb_emit_x86(IF(blob_frame_bytes() > 0, x86("mov", "r10", RDQ("rbp", -8)) + x86("mov", "r11", RDQ("rbp", -16)) + IF(sn4_blob_casmark(), x86("mov", "r12", RDQ("rbp", -32))) + x86("mov", "rsp", "rbp") + x86("pop", "rbp")) + x86_jmp_reg("r11")); }   /* ⭐ R-4(b) (s97): a FRAMED blob whacks its own activation on the ω exit -- rsp back to the entry frontier from ANY interior drift, rbp back to the caller's -- exactly the MATCH_BEGIN/DEFER whack shape; zero-demand blobs keep the bare `jmp r11`. */   /* ⭐⭐⭐ LADDER WREG-2 CLASS D ω: FAIL-AFTER-DRAIN = `jmp r11`.  The absolute unwind `lea rsp,[rax+kt]` dies because it existed to rejoin a pre-entry spine the α no longer left -- with no carve there is no depth to re-base from, and the FORTH fail-chain restores entry depth BY CONSTRUCTION (the drain law).  The ω wire read from the header dies for the same reason γ's did: the wire is in the register.  g_zctx POP dies with the push it balanced. */
    else { if (xa_flat_class_c_pred()) xa_flat_chain_epilogue(); bb_emit_x86(_wire_stub ? bb_glue_wire_ω() : bb_glue_outer_ω()); } } }   /* CLASS C (s114): ω twin of the γ release; both edges leave through a plain ret, so both must pair with the carve. */   /* OUTER-EXIT-1 (Lon s22p): the outermost box owns its OWN two ports, as glue -- this is the whole of what the deleted epilogue did in the ordinary case.  lbl_ω was left UNDEFINED by the s22o deletion while every fail edge still jumped to it, which is the single reason `--compile` produced no assemblable .s for 317/317 programs: one undefined symbol, not 317 defects.  ONE-SHOT BRIDGE (s22p): lbl_gamma and lbl_omega are jmp targets inside main -- no ret, no eax. bb_glue_outer_whack=true; framed_leave is the depth-immune exit; bb_glue_flat.cpp owns the protocol. */
...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    if (g_emit_cfg) zls_fct_finalize(g_emit_cfg, 1);   /* PS-3 s153 LATE fct finalize: defer-bearing tail candidates were left PENDING by the early pass (drive_slots_all runs before any proc registers its patzeta ζ terms); HERE every earlier-emitted PAT$ is registered, so the SUSP license resolves -- price or refuse-to-chain -- before this graph's first node emits (fc_tail_head is consulted at IR_MATCH_BEGIN). */
    g_bb_slotmap_n = 0;
