...
static int emit_graph_has_deep_arrival(IR_graph_t *g) { if (!g) return 1; for (int i = 0; i < g->n; i++) { IR_t *c = g->all[i]; if (!c) continue; switch (c->op) { case IR_SUSPEND: case IR_SCAN: case IR_SCAN_ENTER: case IR_SCAN_ALTERNATE: case IR_SCAN_SEQUENCE: case IR_SCAN_UPTO: case IR_SCAN_FIND: case IR_SCAN_BAL: case IR_SCAN_MATCH: case IR_SCAN_MOVE: case IR_SCAN_TAB: case IR_TO: case IR_TO_BY: case IR_LIMIT: case IR_REPALT: case IR_PROC_GEN: case IR_CREATE: case IR_ITERATE: case IR_DISJUNCTION: case IR_CALL_BUILTIN_GEN: case IR_KEYWORD_ICON_GEN: case IR_MATCH_FENCE1: case IR_MATCH_ABORT: case IR_MATCH_HEAD: case IR_MATCH_ALTERNATE: case IR_MATCH_ARBNO: case IR_MATCH_ARB: case IR_MATCH_DEFER: case IR_MATCH_RETRY: case IR_MATCH_RELEASE: case IR_MATCH_SEQUENCE: case IR_MATCH_CALLOUT: case IR_MATCH_VALUE: return 1; default: break; } } return 0; }   /* FLATDISP-5: the CLOSED generator/suspending family — every construct that can hold a live resumption point BELOW the activation base, so a `return` reached from inside it arrives with rsp deeper than the frame base and only a pinned register can name that base.  CONSERVATIVE BY CONSTRUCTION: a null graph and every unlisted-but-present kind fall through to the SAFE answer (1 = deep = today's rbp-absolute behaviour); only a graph proven to contain NONE of these earns the cheap depth-static exit, so a miscategorised new kind degrades to correct-and-slower, never to wrong.  Deliberately NOT keyed on language (RULES.md NO-LANGUAGE-SENTINEL): a SNOBOL4 graph qualifies because it happens to contain none of these, not because it is SNOBOL4, and an Icon graph that likewise contains none would qualify identically. */
...
extern "C" int emit_jmp_entry_for_proc(const char *pname, int dyn_scope, int is_generator, IR_graph_t *g) {
    ...
    g_emit.flat_deep_arrival = emit_graph_has_deep_arrival(g);   /* FLATDISP-5: set BESIDE flat_gen because both answer "what shape is this activation's exit?" — flat_gen says the frame is RETAINED across γ, flat_deep_arrival says the frame base cannot be recovered by arithmetic on rsp.  Cleared with the rest of the jmp-entry arm. */
