...
static int zd_wl_kind(IR_t * nd) {
    ...
    { static int _zc = -1; if (_zc < 0) { const char * _e = getenv("SCRIP_ZD_CAP"); _zc = (_e && *_e == '1') ? 1 : 0; }   /* ZD-5b CAPTURE/VALUE pre-wiring (s27, default OFF -- SCRIP_ZD_CAP=1 enables; OMEGA must first land op_zres-gated arms in bb_match_capture.cpp + staging-loop special-case for COND/IMM operands[1]; armed path segfaults 043_pat_len: rt_cap_push fallback calls FR(op_off) which indexes the flat PATCTX block absent under ZD).  SAVE stores r14d at ZRES(0) (K=16 own cell); COND/IMM read SAVE's cell via ZOPQ(1,8) = staged op_zread[0] from operands[1] (STAGING LOOP CROSS-FRONT to OMEGA s27).  VALUE reads operand[0] via ZOPQ(0,8) replacing FR(op_a_slot). */
      ...
      if (_zc && op == IR_MATCH_VALUE) return 1; }   /* ZD-5b (s27): bb_match_value ZD arm needed (OMEGA-owned).  operand[0] = value-spine pattern DESCR; ZD arm reads ZOPQ(0,8) replacing FR(op_a_slot).  K=0 (no own result cell; cursor advance via r14d only). */
