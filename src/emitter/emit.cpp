...
static int zd_wl_kind(IR_t * nd) {
    ...
    if (op == IR_CALL_BUILTIN_ICON) { extern int icn_builtin_is_generator(const char *); extern int icn_builtin_is_known(const char *); const char * fn = IR_LIT(nd).sval; if (fn && icn_builtin_is_generator(fn)) return 0; if (fn && icn_builtin_is_known(fn)) return 1; return 1; }   /* ZD-ICN-CALL (s208): IR_CALL_BUILTIN_ICON admitted for non-generator builtins.  ROOT CAUSE THIS CLOSES: CARVE-KILL deleted the whole-graph flat frame; the legacy arm of bb_call_fn_str reads FRQ(argbase) offsets into that deleted frame, writing [rsp+16]/[rsp+24] above the FORTH frontier and scribbling caller-saved rdi/rsi -- the exact SEGV in write("hello").  The ZD arm of bb_call_fn_str already exists (ZD-7c shape: sub rsp,nargs*16 / copy ZOPQ predecessors / lea rsi,[rsp+0] / call rt_call_arr / add rsp / ZRES write) -- this admission line is the ONLY missing piece.  Generators excluded: suspended RSP frontier is incompatible with op_zread depth-difference arithmetic, same reason as ZD-7 generators.  bb_call_route_classify routes CALL_BUILTIN_ICON to CALL_ROUTE_FN (known non-gen) or CALL_ROUTE_BYNAME (generator), both of which already carry ZD arms; unknown builtins fall through to CALL_ROUTE_BYNAME via the final else -- also ZD-armed -- so return 1 is safe for the unknown arm too.  SNOBOL4 watermark: SNOBOL4 does not use IR_CALL_BUILTIN_ICON so this line is INVISIBLE to the SN4 crosscheck set by construction. */
    { static int _zm = -1; if (_zm < 0) { const char * e = getenv("SCRIP_ZD_MATCH"); _zm = (e && *e == '1') ? 1 : 0; }   /* ⛔ DEFAULT FLIPPED BACK OFF (SEQ-ERAD s9, MEASURED): SE-6 deleted IR_MATCH_SEQUENCE, which INVALIDATED THE EVIDENCE BASE quoted below.  The crosscheck-318 identity that justified arming was gathered while SEQUENCE was a K=0 spine member whose 2N operand container held its elements OFF the run -- pattern elements (POS/LEN/FENCE1/RPOS/capture pairs) entered zd_plan as blob-interior operand-closure members, never as run members.  With the container deleted the SAME elements sit DIRECTLY on the spine and are admitted individually, so the claim-depth arithmetic prices a partition that no longer exists: RSP is imbalanced at statement exit (crash PC 0x200000002, descriptor-shaped, ret into garbage AFTER correct output).  MEASURED on the 141-probe gate: armed = 81 pass/34 xfail/12 XPASS/14 REGRESSION; declined = 90/11/35/5 -- +9 of the 14 (FENCE0 seven G04 G05 G08 G09 G21 G22 G23 + ALT3-capture pair A05 A06) and +23 XPASS, i.e. 125 programs correct vs 93.  SCRIP_ZD_MATCH=1 RE-ARMS byte-identically -- the killswitch the ladder's law requires, now pointing the other way.  ⚠ RE-ARMING IS A LON/ALPHA-front decision: this is a PERF default, and re-arming needs a fresh evidence base computed against the container-free spine, not the quoted 318 run.  The remaining 5 (nested-ARBNO H24 H25 X02 X06 X11) are a DIFFERENT defect, immune to every ZD switch -- see the sno_seq_nary second-pass catch-all in lower_snobol4.c.  ORIGINAL ARMING NOTE, RETAINED FOR THE RECORD:  Evidence: full crosscheck-318 identical BY SET to the declined regime in both modes (m3 280/27/10, m4 266/39/10/2C == the s23g record), pattern-family 131 identical by set, zero P->F flips any arm, the one T->F (061_capture_in_arbno) closed by the general subject read.  SCRIP_ZD_MATCH=0 restores the declined regime byte-identically -- the escape hatch every wholesale flip keeps. */
      if (_zm && (op == IR_MATCH_BEGIN || op == IR_MATCH_END || op == IR_MATCH_REPLACE)) return 1; }   /* ⭐ ZD-5 MATCH-SPINE admission: BEGIN/END/REPLACE only (SEQ-ERAD SE-5/SE-6: IR_MATCH_SEQUENCE deleted) */
    ...
    if (zd_patref_on() && (op == IR_MATCH_PATREF || op == IR_MATCH_DEFER)) return 1;   /* ⭐ ZD-PATREF (OMEGA s39, DEFAULT OFF -- opt-in until the gate proves it): the stored-pattern INVOKE box admitted as a transparent transfer citizen.  WHY THIS IS THE RUNG THAT MATTERS: [ZD-GAP] on alt1/any/len/match shows IR_MATCH_PATREF admit=0 is the FIRST BLOCKER for the pattern-statement class -- one unadmitted node at the head declines the whole run, which is what routes 20 of 120 measured programs to the UCLAIM wholesale arm (x86_asm.h:2053, `sub rsp,<claim>` + N zero stores; sudoku 4944B, nqueens 1872B, ops 1680B).  This is the Lon s21x-w model's remaining debt: those bytes are a STATEMENT-scope block standing in for per-BB self-carves.  K=0 in zd_k above (transfer box, no spine cell).  nops default 0 is correct: the target is reached by pointer through the flat glue, never staged as a depth-difference operand read.  ⛔ DEFAULT OFF ON PURPOSE: bb_match_defer.cpp carries NO op_zres arm at this HEAD (grep = 0), so arming by kind alone is the 017 falsification shape (staged verdict handed to a template that ignores it) UNTIL the template arm lands -- this gate exists to MEASURE that gap, not to close it.  Killswitch-shaped per the s22h law; flip is its own commit. */
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        int zdyn = 0; { static int _da = -2; if (_da == -2) { const char * e = getenv("SCRIP_ZD_DYNARM"); _da = e ? atoi(e) : -1; }   /* ⭐ ZD-5 PATREF/DEFER STATIC-SHAPE ARMING (s23i, the s23h-named next rung): a DEFER/PATREF whose pat_static=1 (lower's transitive defer-free proof over the seal table) no longer vetoes the quartet -- the stored pattern provably enters, scans, and returns with no blob re-entry, so the armed claim geometry is never crossed at an unmodeled depth.  The recursive class (135/136's B = '(' FENCE(*B|eps) ')') stamps 0 and keeps the declined regime VERBATIM, as do composite `*expr` sites and every non-eligible name -- degrade never die.  FENCE1 in the STATEMENT graph still declines unconditionally: its wire-glue geometry predates cells-above-claim and has no per-target proof to ride (142's lone in-statement green is one witness, not a bracket).  SCRIP_ZD_DYNARM is the MEASUREMENT MASK, unset = classifier: bit0 forces DEFER armed, bit1 PATREF, bit2 FENCE1 (=7 reproduces the s23h full-arm flip set {135,136,164,165,183}+{117}); =0 forces all three declined = the s23h HEAD regime byte-identical, the escape hatch every narrowing keeps. */
          ...
          for (int k = 0; k < n && !zdyn; k++) { int o = (int)nodes[k]->op; if (_ds && !cm[k]) continue;   /* ⛔⭐ ZD-4 (s23q): THE VETO WAS FUNCTION-SCOPED AND THAT IS THE DEFECT.  `for k < n` scans EVERY node in the graph, so ONE dynamic box anywhere declines EVERY match statement in that graph -- the exact shape of the whole-graph jmp-entry veto that ZD-NOGRAPH deleted in s23a under Lon's s21x-w law ("NO FUNCTION-level processing for ZETA whatsoever.  ONLY statement level scoping"), reintroduced by kind instead of by entry.  MEASURED s23q on roman.sno: proc_LBL__ROMAN armed=0/30 with its per-node verdicts ALL admit=1 and its per-run reasons naming IR_MATCH_BEGIN -- i.e. the quartet veto at the member test below, not a capability gap; SCRIP_ZD_DYNARM=2 (PATREF force-arm) alone lifts it to armed=10, so the whole graph was being declined by ONE PATREF (`T` in spine position, statement 2's `'0,1I,..' ? T BREAK(',') . T`) sitting in a DIFFERENT statement.  cm[] is the run's own closure, so a dynamic box now vetoes the statement that CONTAINS it and no other; the hazard the veto guards (blob re-entry at an unmodeled claim depth, casualties {135,136,164,165,183}+{117}) is intra-statement by construction -- re-entry arrives through the bearing statement's own blob, never a sibling's.  Degrade never die: the bearing statement keeps its declined regime VERBATIM. */
            ...
            else if (o == IR_MATCH_DEFER || o == IR_MATCH_PATREF) { int st = nodes[k]->pat_static; if (_da == 0) st = 0; else if (_da > 0 && (_da & ((o == IR_MATCH_DEFER) ? 1 : 2))) st = 1; if (!st) zdyn = 1; }
        ...
        if (ok) { int zd = 0;
            ...
            int zwr = 0; if (zw_rb_on() && !zws && zw_frame_on() && Kc > 0 && hpos >= 0 && nblob_real > 0) { int _end = 0, _seal = 0; for (int r = 0; r < rl; r++) { int _o = (int)nodes[run[r]]->op; if (_o == IR_MATCH_END) _end = 1; if (_o == IR_MATCH_ABORT) _seal = 1; } for (int _g = 0; !_seal && _g < n; _g++) { int _go = (int)nodes[_g]->op; if (_go == IR_MATCH_DEFER || _go == IR_MATCH_PATREF) _seal = 1; } zwr = _end && !_seal; }
            ...
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                { int oib = (nodes[i]->ω.sz[0] == (char)0xce && (unsigned char)nodes[i]->ω.sz[1] == 0xb2); if (!oin && oib && K == 0) oin = 1; }   /* ZD-5b (s24b): blob-interior K=0 leaf kinds (LIT/LEN/ANY/NOTANY/SPAN/TAB/RTAB/POS/RPOS/REM) wire omega to MATCH_BEGIN's BETA (the scanner retry path, port tag 0xce 0xb2).  The k>r forward-only guard above misses this backward beta target (MATCH_BEGIN is at r=1, LIT at r=2, so k=1 < r=2 and oin stays 0).  CONSEQUENCE: wpop = zd - K + kc releases the subject VAR cell BEFORE the beta retry reads it -- use-after-free on the scanner loop.  THE CORRECT RULE: a blob-interior K=0 leaf whose omega IS a beta re-entry frees NOTHING at omega (MATCH_END is the sole release authority for the whole match extent); the beta-tag sniff is the canonical detection -- same tag the drive loop reads at emit.cpp:2368 to route the omega wire.  GATED K==0: K>0 leaf kinds (if any are ever admitted) may have different topology; this fix is strictly for the zero-cell scanner-register class. */
            ...
            if (Kc > 0 && hpos >= 0 && !zwr) { zuk[run[hpos]] = Kc; zvo_add((int)umin, Kc, Kc, hi);   /* ⭐ !zwr: mechanism-2 suppresses the UCLAIM head-claim (push rbp is the boundary, no sub rsp,K); blob FRQ spellings use raw off+op_zdepth.  CLAIM-AT-HEAD: zuk staged on the MATCH_BEGIN member -- the port hook carves it at the head's alpha, once per statement execution. */
                for (int k = 0; !zwr && k < n; k++) if (cm[k] && zuh[k] < 0) { zud[k] = Kc; zuh[k] = hi; }   /* ⭐ !zwr: under mechanism-2, blob members use raw FRQ spellings (no claim-relative rebase).  blob members resolve DECLINED-IDENTICAL: dout delta ZERO, not Kc+hdd. */
...
#undef RPO_VISITED
    ...
    for (int i = 0; i < n; i++) {
        ...
        na_f[i]  = ((nodes[i]->op == IR_MATCH_ALTERNATE || nodes[i]->op == IR_MATCH_ARBNO || nodes[i]->op == IR_MATCH_FENCE1 || nodes[i]->op == IR_SCAN_SEQUENCE || nodes[i]->op == IR_SCAN_ALTERNATE || nodes[i]->op == IR_DISJUNCTION) && nodes[i]->n_operands > 0) ? emit_label_alloc("n%d_%s_af", _uid, _kn) : NULL;   /* SN4-NARY-ALT/-ARBNO fail-glue */
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          { static int _uw = -1; if (_uw < 0) { const char *_e = getenv("SCRIP_UNWIND"); _uw = (_e && *_e == '0') ? 0 : 1; }
            ...
            if (_uw && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_BEGIN && nodes[i]->op != IR_STATEMENT_END && zd_uk[_h3] == 0 && zd_on[i] && zd_wp[i] > 0 && g_emit.op_wpop == 0 && !omega_is_beta && !omega_is_phi && !scan_live && !g_emit.flat_stmt_frame) {
          ...
          { static int _ej = -1; if (_ej < 0) { const char *_e = getenv("SCRIP_ZD_ENDJMP"); _ej = (_e && *_e == '0') ? 0 : 1; }
            ...
            if (_ej && !_uw_stolen && nodes[i]->op != IR_STATEMENT && nodes[i]->op != IR_STATEMENT_BEGIN && nodes[i]->op != IR_STATEMENT_END && zd_uk[_h2] == 0 && zd_on[i] && zd_wp[i] > 0 && g_emit.op_wpop == 0 && !omega_is_beta && !omega_is_phi && !scan_live && !g_emit.flat_stmt_frame) {
