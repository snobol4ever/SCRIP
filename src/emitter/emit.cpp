...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int g_zd_stage, g_zd_arm, g_zd_k, g_zd_gpop, g_zd_wpop, g_zd_read[6], g_zd_kind[6];
static long g_zd_uclaim, g_zd_udout; static int g_zd_uhead = -1; static int g_zd_uzw; static int g_zd_uzwr;   /* MECHANISM-2 staging twin of g_zd_uzw: set from zd_zwr[i], applied at choke as g_emit.op_zw2 */   /* ZW-12: staged beside the UCLAIM twins, applied at the same choke */   /* UCLAIM staging twins (wholesale flip): filled beside the ZD set at the drive loop, applied at the choke, cleared with the stage flag */   /* ZD-1 driver-handoff staging (R5b): the drive loop fills these from zd_plan's arrays immediately before emit_drive; the choke below applies them to g_emit exactly once and CLEARS the stage flag, so every other walk_bb_node entry path (FILL / EMIT_PAIR_FILL glue) sees zeros and stays byte-identical. */
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static int walk_bb_node_inner(IR_t * nd, FILE * out) {
    ...
    g_emit.op_zres = 0; g_emit.op_zgpop = 0; g_emit.op_wterm = 0; for (int _zi = 0; _zi < 6; _zi++) { g_emit.op_zread[_zi] = 0; g_emit.op_zkind[_zi] = -1; }   /* ZD-1: cleared at the choke UNCONDITIONALLY so glue and non-drive fills can never inherit a stale regime */
    g_emit.op_uclaim = 0; g_emit.op_udout = 0; g_emit.op_uhead = -1; g_emit.op_zw = 0; g_emit.op_zw2 = 0;   /* UCLAIM: cleared at the choke UNCONDITIONALLY, same law as the ZD set above; op_zw2 (mechanism-2) follows same discipline */
    if (g_zd_stage) { g_emit.op_uclaim = g_zd_uclaim; g_emit.op_udout = g_zd_udout; g_emit.op_uhead = g_zd_uhead; g_emit.op_zw = g_zd_uzw; g_emit.op_zw2 = g_zd_uzwr;   /* UCLAIM applied regardless of g_zd_arm -- declined runs are its whole constituency; op_zw2 mechanism-2 boundary flag applied beside op_zw */
...
static void zd_plan(IR_t **nodes, int n, unsigned char *zon, int *zout, int *zgpop, int *zwpop, long *zuk, long *zud, int *zuh, int *zzw, int *zzwr) {
    ...
    for (int hi = 0; hi < n; hi++) {
        ...
        if (ok) { int zd = 0;
            ...
            int zwr = 0; if (zw_rb_on() && !zws && zw_frame_on() && Kc > 0 && hpos >= 0 && nblob_real > 0 && !g_emit.flat_stmt_frame) { int _end = 0, _seal = 0; for (int r = 0; r < rl; r++) { int _o = (int)nodes[run[r]]->op; if (_o == IR_MATCH_END) _end = 1; if (_o == IR_MATCH_ABORT) _seal = 1; } for (int _g = 0; !_seal && _g < n; _g++) { int _go = (int)nodes[_g]->op; if (_go == IR_MATCH_DEFER || _go == IR_MATCH_PATREF) _seal = 1; } zwr = _end && !_seal; }   /* ⭐ MECHANISM-2 (HQ ruling 2026-08-03): arms blob-clause programs (nblob_real>0) that ZW-12 cannot arm due to CAP-SLOT hazard.  Predicate: zw_rb_on() && !zws (ZW-12 not armed) && same eligibility as ZW-12 (Kc>0, hpos>=0, no STF) BUT nblob_real>0 instead of nblob_real==0.  Seal logic identical to ZW-12 (ABORT/DEFER/PATREF sealed graph-scope; FENCE1 admitted since the push+mov floor is correctly set for fence_whack_commit).  Only 8B (push rbp) added to the depth model -- no sub rsp,ZW_FRAME_K -- so FRQ spellings remain valid at the flat claim base.  Killswitch SCRIP_ZW_RB=0. */   { static int _zwsd = -1; if (_zwsd < 0) { const char *_e = getenv("SCRIP_ZWS_DIAG"); _zwsd = (_e && *_e == '1') ? 1 : 0; } if (_zwsd && hpos >= 0 && !zws) fprintf(stderr, "[ZWR] %s: Kc=%ld rl=%d nblob=%d nblob_real=%d has_blob=%d hpos=%d stf=%d\n", zwr ? "ARMED" : "DECLINED", (long)Kc, rl, nblob, nblob_real, has_blob, hpos, (int)g_emit.flat_stmt_frame); }
            for (int r = 0; r < rl; r++) { int i = run[r];
                ...
                if (zwr && r == hpos) zd += 8;   /* ⭐ MECHANISM-2: only push rbp (8B) added after head -- no sub rsp,ZW_FRAME_K.  FRQ(off) spellings for blob members remain valid at the flat claim base (no shift introduced).  The whack `mov rsp,rbp; pop rbp` at END subtracts exactly this 8B plus all FORTH cells above the frame. */
            ...
            if (Kc > 0 && hpos >= 0 && !zwr) { zuk[run[hpos]] = Kc; zvo_add((int)umin, Kc, Kc, hi);   /* ⭐ MECHANISM-2 (!zwr): mechanism-2 suppresses the UCLAIM head-claim (no sub rsp,K at alpha) -- the frame boundary (push rbp) IS the statement boundary, and blob members' FRQ spellings resolve to raw off+op_zdepth against the current rsp rather than through the owner table (which would rebase relative to the UCLAIM claim that doesn't exist).  CLAIM-AT-HEAD: zuk staged on the MATCH_BEGIN member -- the port hook carves it at the head's alpha, once per statement execution (the head's alpha is entered once; the unanchored retry loops inside it), so the var carves ONLY its own 16 -- Lon's roman.s complaint verbatim.  Pre-head members never see the claim: their exits free cells alone (kc gate above). */
                for (int k = 0; !zwr && k < n; k++) if (cm[k] && zuh[k] < 0) { zud[k] = Kc; zuh[k] = hi; }   /* ⭐ !zwr: blob members need claim-relative depth only when UCLAIM is active; under mechanism-2 (zwr) they use raw FRQ spellings (off+op_zdepth vs current rsp).  blob members resolve DECLINED-IDENTICAL: dout delta ZERO, not Kc+hdd. */
            ...
            if (rl > 0 && zzwr[run[0]]) { for (int r = 0; r < rl; r++) { zgpop[run[r]] = 0; zwpop[run[r]] = 0; } }   /* ⭐ MECHANISM-2: the whack (mov rsp,rbp; pop rbp) in bb_match_begin/end is the SOLE release authority -- zero zgpop/zwpop so the planner does not also emit add rsp,K at statement exits (UCLAIM computed them above; mechanism-2 supersedes them as ZW-12 does for its population). Use zzwr[run[0]] (the head member flag, set in the depth-model loop) instead of the zwr local which is in the ZW-12 path scope. */
...
#undef RPO_VISITED
    ...
    for (int i = 0; i < n; i++) {
        ...
        { static int _dd = -1; if (_dd < 0) { const char *_e = getenv("SCRIP_DRIVE_DIAG"); _dd = (_e && _e[0] == '1') ? 1 : 0; }
          ...
          if (zd_on[i] || zd_gp[i] > 0 || zd_wp[i] > 0 || zd_uk[i] > 0 || zd_ud[i] > 0 || zd_zw[i] || zd_zwr[i]) { g_zd_stage = 1; g_zd_arm = zd_on[i]; g_zd_gpop = zd_gp[i]; g_zd_wpop = _zw5_wpop_stolen ? 0 : zd_wp[i]; g_zd_uclaim = zd_uk[i]; g_zd_udout = zd_ud[i]; g_zd_uhead = zd_uh[i]; g_zd_uzw = zd_zw[i]; g_zd_uzwr = zd_zwr[i];   /* UCLAIM staged beside the ZD set -- one stage flag, one choke; ZW-5 O-2: g_zd_wpop=0 when stub owns release; g_zd_uzwr = mechanism-2 RBP boundary staging */
...
bb_box_fn emit_chain(IR_t *entry, FILE *out, const char *prefix) {
    ...
    { static int _stf = -1; if (_stf < 0) { const char * e = getenv("SCRIP_STMT_FRAME"); _stf = (e && *e == '1') ? 1 : 0; }   /* ⛔⭐⭐ STF-UNFLIP (s22b, LON DIRECTIVE, and it is a MODEL correction not a tuning knob): DEFAULT FLIPPED BACK ON -> OFF; `SCRIP_STMT_FRAME=1` is now the OPT-IN and the regime ships DORMANT.  THE MODEL, restated in Lon's words because two sessions in a row encoded the opposite: NO BB EVER CONSUMES ANYTHING, EVER.  A BB allocates EXACTLY at alpha -- one `sub rsp, K`, its OWN cell -- never at beta (ARBNO-class excepted); it frees at omega; and the whole accumulation is freed AT THE END OF THE STATEMENT.  Cells stack up through the statement and ONE statement-terminal release drops all of them.  That release is op_zgpop, and it is the SOLE release authority.  ⛔ THE ERROR s21x-y AND s22a ENCODED: the per-statement rbp dance (bb_glue_framed_enter/leave) was made to stand in for that release, and s22a's ZGPOP-STF then ZEROED op_zgpop on armed graphs to stop the two colliding -- i.e. the bracket did not SUPPLEMENT the statement-terminal free, it REPLACED it, and a C-style frame pointer became per-statement ceremony on the value spine.  Law 4's "C-style RBP used occasionally" licenses rbp for CONSTRUCT housekeeping that must survive an unwind (ARBNO/FENCE1/CALL) -- it never licensed one bracket per statement.  MEASURED AT THIS HEAD, not inherited: crosscheck 318 m3 232/85 - m4 229/86/2 - DIV=1 {W04_arbno_basic}, IDENTICAL to the regime-on run, and the fail set is IDENTICAL BY SET in BOTH modes (diffed programmatically, not by count -- the test_string flake makes counts lie).  ⚠ NOT VACUOUS, and this is the instrument that proves it: an rbp-INSTRUCTION census over all 318 crosscheck programs moves 15709 -> 15459 (250 instructions gone) and the count of programs containing ANY rbp moves 158 -> 127, i.e. EXACTLY 31 PROGRAMS GO RBP-FREE -- the same 31 the SCRIP_STF_DEBUG=1 armed census names.  Witness `arithmetic.sno`: 26 rbp -> 0, statement `OUTPUT = A + B` now three alpha carves (`sub rsp,16` x3) and one terminal `add rsp,48`.  ⭐ THE REMAINING RBP DEBT IS A DIFFERENT CITIZEN AND IS THE NEXT RUNG: 15459 instructions across 127 programs is the flat_pat/flat_gen WHOLE-GRAPH PIN, not a statement bracket -- do not confuse the two (the s22a CENSUS-CORRECTED instrument law).  HISTORICAL, kept because the next session will otherwise re-derive it: s21x-y's flip was licensed by a watermark-neutrality argument that was TRUE and INSUFFICIENT -- neutrality proved the bracket broke nothing, never that it belonged. */   /* ⭐ STF-FLIP (s21x-y), SUPERSEDED BY THE ABOVE, retained for provenance: DEFAULT FLIPPED OFF -> ON, and `SCRIP_STMT_FRAME=0` is now the killswitch.  This is the moment the "C-style RBP used occasionally" of the design of record stops being dormant scaffolding and becomes the shipped regime: under it a STATEMENT is bracketed by the rbp dance (bb_glue_framed_enter/leave) and bb_match_begin's five POST-UNWIND-lifetime fields move from the whole-graph flat slots FRQ(op_off+40..72) onto HKQ = [rbp-48+8k], which is DEPTH-FREE BY CONSTRUCTION -- the value spine keeps riding RSP FORTH-style while exactly the housekeeping that must survive an unwind rides a stable base.  LICENSED BY THE CONDITION THIS LINE ITSELF NAMED ("default OFF until the watermark license ... fail-set diffed programmatically"), now satisfied and MEASURED AT THIS HEAD, not inherited: crosscheck 317 m3 232/85 - m4 229/86/2 - DIV=1 and broad 336 m3 244/92 - m4 238/90/8, and in BOTH corpora BOTH modes the fail set is BYTE-IDENTICAL to the regime-off run.  ⚠ NOT VACUOUS -- the trap that makes an identical watermark meaningless: the regime ARMS 31 GRAPHS IN 31 PROGRAMS (SCRIP_STF_DEBUG=1, live=1 census), so the neutrality is 31 armed customers agreeing, not a classifier declining everything.  s21x-r's 41-program regression is NOT being re-litigated here: it was ZLEAK-1, a process-scope `static int` from getenv applied to all 316 programs while emit armed 31, cured at SCRIP `efb72d69`; the armed set and that regression set were measured DISJOINT then, and this re-measurement at a later HEAD confirms the cure held. */
      ...
      g_emit.flat_stmt_frame = ((_stf && !g_emit.flat_jmp_entry && !g_emit.flat_pat && !g_emit.flat_gen && !g_gen_proc_active && !(g_emit_cfg && g_emit_cfg->resumable_callable) && g_emit.flat_outer_nparams == 0) || _stfj) ? emit_stmt_frame_scan(g_emit_cfg, entry) : 0;
