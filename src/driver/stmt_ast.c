...
tree_t *stmt_to_ast(const STMT_t *s)
{
    ...
    { char * ssrc = stmt_src_slice(s);
      if (!s->subject && !s->pattern && !s->replacement && !s->label && !s->goto_s && !s->goto_f && !s->goto_u && !s->goto_s_expr && !s->goto_f_expr && !s->goto_u_expr && !s->is_end) { free(ssrc); ssrc = strdup(""); }   /* EMPTY-STMT src (Lon s116 in-chat, the duplicate DEFINE banner): a blank source line mints an empty record whose lineno is the PREVIOUS statement's (lexer g_stmt_lineno not refreshed), so the slice re-served that statement's text and the banner duplicated.  An empty statement truthfully carries an EMPTY note -- still non-NULL, so zd_plan's per-statement segmentation root (the R-3(f) law below) is preserved, while emit.cpp:1049's banner block prints nothing for it. */
      if (!ssrc && !s->is_end) { char b[160]; snprintf(b, sizeof b, "%s%s<stmt %d, line %d: source not in main file (INCLUDE)>", s->label ? s->label : "", s->label && s->label[0] ? "  " : "        ", s->stno, s->lineno); ssrc = strdup(b); }   /* ⭐ R-3(f) rtx_func_11_include ROOT CAUSE (s97, Fable 5): stmt_src_slice indexes g_src_lines -- the MAIN file's lines -- by s->lineno, and an INCLUDEd statement carries the include file's line numbers, so its slice fails the label/indent guards and the statement got NO :src. bb_src_of is not a comment facility: emit.cpp's zd_plan roots STATEMENT SEGMENTATION on it ("Runs are already rooted at bb_src_of statement heads"), so an INCLUDE became ONE run -- statement_end emitted no release, the beta cascades accumulated (measured: add rsp,96/176/256... across the include), and every INCLUDEd loop leaked per iteration until SIGSEGV (beauty_suite 17/17 via global.sno's SORT+loop tail; the named probe/rtx_func_11_include witness). Every statement now carries a non-empty note -- the truthful text where the slice can serve it, a labelled placeholder where it cannot -- so segmentation is per statement by construction. */
