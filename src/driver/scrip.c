...
static pl_gz_choice_state_t * pl_gz_choice_inline(IR_t *gg) {
    ...
    if (!bc || !bc->bodies || bc->nbodies < 2) return NULL;
...
static int pl_gz_choice_rule_clauses(IR_graph_t *cg, int ar, bb_choice_state_t **bc_out) {
    ...
    bb_choice_state_t *bc = (bb_choice_state_t *)(intptr_t)IR_LIT(cg->entry).ival;
    if (!bc || !bc->bodies || bc->nbodies < 2) return 0;
...
static int pl_gz_rule_body_root_ok(IR_t *root) {
    ...
    if (root->op == IR_GCONJ) {
        ...
        for (int i = 0; i < zs->ngoals; i++) if (!pl_gz_rule_body_goal_ok(zs->goals[i])) return 0;
        return 1;
    }
...
static int pl_gz_rule_clause(IR_graph_t *cg, int ar, bb_conj_state_t **zs_out) {
    ...
    for (int i = 0; i < cg->n; i++) {
        ...
        if (nd->op == IR_ITE) {
            ...
            if (!zi) return 0;
            if (!pl_gz_rule_body_root_ok(zi->cond_root) || !pl_gz_rule_body_root_ok(zi->then_root) || !pl_gz_rule_body_root_ok(zi->else_root)) return 0;
            continue;
        }
        if (nd->op == IR_BUILTIN) {
    ...
    for (int i = 0; i < cg->n; i++) {
        IR_t *cnd = cg->all[i];
        if (!cnd || cnd->op != IR_GCONJ) continue;
        int cl = 0; for (int k = 0; k < pl_nclaimed; k++) if (pl_claimed[k] == cnd) { cl = 1; break; }
        if (cl) continue;
        if (gconj) return 0; gconj = cnd;
    }
    if (!gconj) return 0;
    bb_conj_state_t *zs = (bb_conj_state_t *)(intptr_t)IR_LIT(gconj).ival;
...
static int pl_gz_nsynth_root(IR_t *root, int ar);
static int pl_gz_nsynth_goal(IR_t *gg, int ar) {
    int nsynth = 0;
    if (!gg) return 0;
    if (gg->op == IR_CATCH) {
        bb_catch_state_t *zc = (bb_catch_state_t *)(intptr_t)IR_LIT(gg).ival;
    ...
    if (gg->op == IR_ITE || gg->op == IR_DISJ) {
        ...
        if (!zi) return -1;
        nsynth++;
        int c = pl_gz_nsynth_root(zi->cond_root, ar); if (c < 0) return -1; nsynth += c;
        int t = pl_gz_nsynth_root(zi->then_root, ar); if (t < 0) return -1; nsynth += t;
        int e = pl_gz_nsynth_root(zi->else_root, ar); if (e < 0) return -1; nsynth += e;
        return nsynth;
    }
    ...
    if (gg->op != IR_GOAL) return nsynth;
...
static int pl_gz_nsynth_root(IR_t *root, int ar) {
    if (!root || root->op == IR_SUCCEED || root->op == IR_FAIL) return 0;
    if (root->op == IR_GCONJ) {
        bb_conj_state_t *zs = (bb_conj_state_t *)(intptr_t)IR_LIT(root).ival;
        if (!zs || !zs->goals || zs->ngoals < 0 || zs->ngoals > 64) return -1;
        int n = 0;
        for (int i = 0; i < zs->ngoals; i++) { int a = pl_gz_nsynth_goal(zs->goals[i], ar); if (a < 0) return -1; n += a; }
        return n;
    }
    return pl_gz_nsynth_goal(root, ar);
}
static int pl_gz_nsynth_chain(IR_t *entry, int ar) {
    int nsynth = 0;
    for (IR_t *gg = entry; gg && gg->op != IR_SUCCEED && gg->op != IR_FAIL; gg = gg->γ.node) {
        int a = pl_gz_nsynth_goal(gg, ar); if (a < 0) return -1; nsynth += a;
    }
    return nsynth;
}
static int pl_gz_clause_nsynth(bb_conj_state_t *zs, int ar) {
    ...
    for (int i = ar; i < zs->ngoals; i++) {
        int a = pl_gz_nsynth_goal(zs->goals[i], ar); if (a < 0) return -1; nsynth += a;
    }
    return nsynth;
}
...
static IR_t * pl_gz_arith_slot_map(const IR_t *nd, int ar, int lbase) {
    ...
    if (nd->op == IR_ATOM || nd->op == IR_LIT_F) { return (IR_t *)nd; }
    if (nd->op == IR_STRUCT) { return pl_gz_struct_slot_map(nd, ar, lbase); }
...
static IR_t * pl_gz_struct_slot_map(const IR_t *nd, int ar, int lbase) {
    if (!nd) return NULL;
    if (nd->op == IR_ARITH) { return pl_gz_arith_slot_map(nd, ar, lbase); }
    if (nd->op == IR_LOGICVAR) { return pl_gz_lv(pl_gz_slot_map((int)IR_LIT(nd).ival, ar, lbase)); }
...
static int pl_gz_callee_body_node(IR_t *gg, int ar, int lbase, int *snp, IR_t **headp, IR_t **tailp, pl_gz_callee_t *ce, pl_gz_callee_vec_t *cv) {
    ...
    if (gg->op == IR_GOAL) {
        ...
        if (!nn) return 0;
        IR_LIT(nn).ival = (int64_t)(intptr_t)is;
        ...
        if (!g0 || !g1) return 0;
        IR_t *na = (g0->op == IR_LOGICVAR) ? pl_gz_lv(pl_gz_slot_map((int)IR_LIT(g0).ival, ar, lbase)) : (g0->op == IR_STRUCT) ? pl_gz_struct_slot_map(g0, ar, lbase) : (g0->op == IR_ARITH) ? pl_gz_arith_slot_map(g0, ar, lbase) : g0;
        IR_t *nb = (g1->op == IR_LOGICVAR) ? pl_gz_lv(pl_gz_slot_map((int)IR_LIT(g1).ival, ar, lbase)) : (g1->op == IR_STRUCT) ? pl_gz_struct_slot_map(g1, ar, lbase) : (g1->op == IR_ARITH) ? pl_gz_arith_slot_map(g1, ar, lbase) : g1;
        if (!na || !nb) return 0;
        ...
        ir_operand_push(nn, na); ir_operand_push(nn, nb);
               ...
               (strcmp(IR_LIT(gg).sval,"var")==0||strcmp(IR_LIT(gg).sval,"nonvar")==0||strcmp(IR_LIT(gg).sval,"atom")==0||strcmp(IR_LIT(gg).sval,"atomic")==0||
                strcmp(IR_LIT(gg).sval,"number")==0||strcmp(IR_LIT(gg).sval,"integer")==0||strcmp(IR_LIT(gg).sval,"float")==0||strcmp(IR_LIT(gg).sval,"compound")==0||
                strcmp(IR_LIT(gg).sval,"callable")==0||strcmp(IR_LIT(gg).sval,"is_list")==0||strcmp(IR_LIT(gg).sval,"ground")==0)) {
        const char *fn = IR_LIT(gg).sval;
        IR_t *a0 = ir_call_arg(gg,0);
        IR_t *slot_node = NULL;
        if (a0->op == IR_LOGICVAR) { slot_node = pl_gz_lv(pl_gz_slot_map((int)IR_LIT(a0).ival, ar, lbase)); }
        else {
            int kk = synth_next++;
            IR_t *cu = pl_gz_det_node(IR_CELL_UNIFY); if (!cu) return 0;
            IR_t *ca = pl_gz_lv(kk); if (!ca) return 0;
            IR_t *a0m = (a0->op == IR_STRUCT) ? pl_gz_struct_slot_map(a0, ar, lbase) : (a0->op == IR_ARITH) ? pl_gz_arith_slot_map(a0, ar, lbase) : a0;
            if (!a0m) return 0;
            ir_operand_push(cu, ca); ir_operand_push(cu, a0m);
            if (!head) head = cu; else { tail->γ.node = cu; memcpy(tail->γ.sz, "α", 3); }
            tail = cu; slot_node = pl_gz_lv(kk);
        }
        if (!slot_node) return 0;
        nn = pl_gz_det_node(IR_DET_TYPE_TEST);
        if (!nn) return 0;
        IR_LIT(nn).sval = fn; ir_operand_push(nn, slot_node);
    } else {
...
static int pl_gz_rule_callee_body(bb_conj_state_t *zs, IR_graph_t *cg, pl_gz_callee_t *ce, int clause_idx, int lbase, pl_gz_callee_vec_t *cv) {
    ...
    for (int i = 0; i < ar; i++) {
        ...
        if (u1 && u1->op == IR_LOGICVAR) {
            ...
            if (u1) {
                IR_t *u1m = (u1->op == IR_STRUCT) ? pl_gz_struct_slot_map(u1, ar, lbase) : (u1->op == IR_ARITH) ? pl_gz_arith_slot_map(u1, ar, lbase) : u1;
                if (!u1m) return 0;
...
static pl_gz_callee_t * pl_gz_callee_get_choice(IR_graph_t *cg, int ar, bb_choice_state_t *bc, pl_gz_callee_vec_t *cv) {
    ...
    IR_LIT(ce->frame_node).ival = (int64_t)(intptr_t)ce;
    if (!pl_gz_callees_push(cv, ce)) return (pl_gz_callee_t *)0;
    int nbod = bc->nbodies > 0 ? bc->nbodies : 1;
    bb_conj_state_t **zsk = (bb_conj_state_t **)alloca(sizeof(bb_conj_state_t *) * nbod); int *lbase = (int *)alloca(sizeof(int) * nbod); int total = 0;
    for (int k = 0; k < bc->nbodies; k++) {
        zsk[k] = NULL;
