...
int main(int argc, char **argv)
{
    ...
    while (argi < argc && argv[argi][0] == '-' && argv[argi][1] == '-') {
        ...
        else if (strncmp(argv[argi], "--zeta-port=", 12) == 0) { const char *z = argv[argi] + 12; int pm = strcmp(z, "plain") == 0 ? 0 : strcmp(z, "instrumented") == 0 ? 1 : strcmp(z, "alloc") == 0 ? 2 : strcmp(z, "inline") == 0 ? 3 : strcmp(z, "cstack") == 0 ? 4 : strcmp(z, "forth") == 0 ? 6 : -1; /* ZC_PORT_* (zeta_choices.h): inline = old arena, cstack = ζ on the C stack, forth = cstack superset (per-BB fixed rsp cells + ARBNO linked-frame-chain) = the compiled default; owned(5) stays env-only */ if (pm < 0) { fprintf(stderr, "scrip: bad --zeta-port=%s (valid: plain, instrumented, alloc, inline, cstack, forth)\n", z); return 2; } rt_zeta_port_set_mode(pm); argi++; }
    ...
    if (mode_compile_x86) {
        ...
        if (is_icon || is_raku || is_sno_bb || is_prolog) {
            ...
            for (int _pi = 0; _pi < s2->proc_count; _pi++) {
                ...
                { char _pfx[256]; snprintf(_pfx, sizeof(_pfx), "proc_%s", asm_sym_name(pname)); fprintf(stdout, "  .globl %s_\xce\xb1\n", _pfx); if (scrip_symmap()) fprintf(stdout, "  .type %s_\xce\xb1, @function\n", _pfx); emit_chain(s2->bbp.table[idx]->entry, stdout, _pfx); if (scrip_symmap()) fprintf(stdout, "  .size %s_\xce\xb1, .-%s_\xce\xb1\n", _pfx, _pfx); }
        ...
        }
        {
            ...
            for (int _pi = 0; _pi < s2->proc_count; _pi++) {
                ...
                { char _pfx[256]; snprintf(_pfx, sizeof(_pfx), "proc_%s", asm_sym_name(pname)); fprintf(stdout, "  .globl %s_\xce\xb1\n", _pfx); if (scrip_symmap()) fprintf(stdout, "  .type %s_\xce\xb1, @function\n", _pfx); emit_chain(s2->proc_table[_pi].proc_entry_node, stdout, _pfx); if (scrip_symmap()) fprintf(stdout, "  .size %s_\xce\xb1, .-%s_\xce\xb1\n", _pfx, _pfx); }
                { extern void emit_jmp_entry_clear(void); emit_jmp_entry_clear(); }
