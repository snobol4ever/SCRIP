...
static void emit_compat_bake_data(void) {
    int se = compat_bake_on("SCRIP_SETEXIT_END"), io = compat_bake_on("SCRIP_IO_ASSOC_LEGACY"), rf = compat_bake_on("SCRIP_REAL_FMT_CSNOBOL4"), ip = compat_bake_on("SCRIP_IPOW_CSNOBOL4");
    if (!se && !io && !rf && !ip) return;
...
int main(int argc, char **argv)
{
    ...
    while (argi < argc && argv[argi][0] == '-' && argv[argi][1] == '-') {
        if      (strcmp(argv[argi], "--run")           == 0) { mode_run       = 1; argi++; }
            ...
            else if (strcmp(d, "csnobol4") == 0) { setenv("SCRIP_SETEXIT_END", "1", 1); setenv("SCRIP_IO_ASSOC_LEGACY", "1", 1); setenv("SCRIP_REAL_FMT_CSNOBOL4", "1", 1); setenv("SCRIP_IPOW_CSNOBOL4", "1", 1); }
    ...
    if (mode_run) {
        ...
        if (is_icon || is_raku || is_sno_bb || is_prolog) {
            ...
            ir_delete_all(s2);
            void *mf = NULL;
            { extern void rt_main_args_stage(char **, int); rt_main_args_stage(g_prog_argv, g_prog_argc); } if (_nparams >= 1) { extern void rt_main_args_bind(void); rt_main_args_bind(); }
        ...
        }
        {
            ...
            if (sbbg && sbbg->entry) {
                ...
                if (fn) {
                    void *mf = NULL;
                    { extern void rt_main_args_stage(char **, int); rt_main_args_stage(g_prog_argv, g_prog_argc); } if (sbbg->nparams >= 1) { extern void rt_main_args_bind(void); rt_main_args_bind(); }
                    { extern void bbprof_start(void); bbprof_start(); }
                    { extern void rt_gcheap_warmup(void); rt_gcheap_warmup(); }
