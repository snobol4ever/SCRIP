...
static void m3_seal_entry_cells(const char *pname, void *fnbase, int alpha_face) {   /* R-1 s94 (Fable 5) THE CELL FILL, one authority for both directions of the TINY cross-chain crossing (x86_jmp_via_cell): the just-sealed chain's label pool is live until the next bb_emit_begin, so <FN>_alpha (DEFINE stub seal, alpha_face=1) / <ENTRY>_body (main seal, alpha_face=0) resolve by name to a stable cell of the ONE allocator (bb_ab_fn_cell_ptr) that the sites/shim already jump through -- no new global, no ordering coupling */
    ...
    if (alpha_face) { snprintf(lbl, sizeof lbl, "%s_\xce\xb1", pname); snprintf(cell, sizeof cell, "alpha$%s", pname); }   /* raw: the role-4 shim interns fn4+"_\xce\xb1" verbatim (s112 Greek ports); LBL__ label faces are asm_sym_name'd (BODY-ALIAS build) */
    else            { snprintf(lbl, sizeof lbl, "LBL__%s",  asm_sym_name(pname)); snprintf(cell, sizeof cell, "body$%s",  pname); }   /* s112 rename: the body label's asm symbol IS the rt key spelling, LBL__<entry> */
    ...
    if (mode_compile_x86) {
        ...
        if (is_icon || is_raku || is_sno_bb || is_prolog) {
            ...
            int rc;
            {
                ...
                { int _bd = 0; for (int _s = 0; _s < bbg->n; _s++) { IR_t *_c = bbg->all[_s]; if (_c && _c->op == IR_DEFINE && _c->n_operands == 0) _bd++; }
                  if (_bd > 0 && bbg->n_dentry == 0) { bbg->dentry_node = (IR_t **)calloc((size_t)_bd, sizeof(IR_t *)); bbg->dentry_entry = (IR_t **)calloc((size_t)_bd, sizeof(IR_t *)); bbg->dentry_name = (const char **)calloc((size_t)_bd, sizeof(char *));
                      if (bbg->dentry_node && bbg->dentry_entry && bbg->dentry_name) for (int _s = 0; _s < bbg->n; _s++) { IR_t *_c = bbg->all[_s]; if (!_c || _c->op != IR_DEFINE || _c->n_operands != 0 || !IR_LIT(_c).sval) continue;
                          ...
                          for (int _q = 0; _q < s2->proc_count; _q++) { ProcEntry *_pr = &s2->proc_table[_q]; if (!_pr->name || strcmp(_pr->name, IR_LIT(_c).sval)) continue;
                              ...
                              IR_t *_gd = (_sn && _sn->op == IR_SAVE_RESTORE) ? _sn->γ.node : _sn; if (!_gd || _gd->op != IR_GOTO_DEFERRED || !IR_LIT(_gd).sval) break;
