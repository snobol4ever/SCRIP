...
int main(int argc, char **argv)
{
    ...
    while (argi < argc && argv[argi][0] == '-' && argv[argi][1] == '-') {
        ...
        else if (strncmp(argv[argi], "--zeta-port=", 12) == 0) { const char *z = argv[argi] + 12; int pm = strcmp(z, "plain") == 0 ? 0 : strcmp(z, "instrumented") == 0 ? 1 : strcmp(z, "alloc") == 0 ? 2 : strcmp(z, "inline") == 0 ? 3 : strcmp(z, "cstack") == 0 ? 4 : strcmp(z, "forth") == 0 ? 6 : strcmp(z, "heap") == 0 ? 7 : -1; /* ZC_PORT_* (zeta_choices.h): inline = old arena, cstack = ζ on the C stack, forth = cstack superset (per-BB fixed rsp cells + ARBNO linked-frame-chain) = the compiled default; heap = RUNG ZHEAP target (rbx bump frontier), added s206 — it was reachable only as SCRIP_ZETA_PORT=7 while being the #1 rung's target port; owned(5) stays env-only */ if (pm < 0) { fprintf(stderr, "scrip: bad --zeta-port=%s (valid: plain, instrumented, alloc, inline, cstack, forth, heap)\n", z); return 2; } rt_zeta_port_set_mode(pm); argi++; }
        else if (strncmp(argv[argi], "--zeta-storage=", 15) == 0) { const char *z = argv[argi] + 15; int sm = strcmp(z, "frame-r12") == 0 ? 0 : strcmp(z, "frame-rsp") == 0 ? 1 : strcmp(z, "cell-stack") == 0 ? 2 : strcmp(z, "cell-heap") == 0 ? 3 : -1; /* ZC_STORAGE (GOAL-ZETA-FOUR Z4-4 slice 2): the four-config selector; the setter derives the legacy port tuple (cell-stack->forth, cell-heap->heap, frame-rsp->cstack; frame-r12 none until Z4-7) so zero downstream seams flip this rung */ if (sm < 0) { fprintf(stderr, "scrip: bad --zeta-storage=%s (valid: frame-r12, frame-rsp, cell-stack, cell-heap)\n", z); return 2; } rt_zeta_storage_set(sm); argi++; }
