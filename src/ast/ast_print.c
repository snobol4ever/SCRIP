...
static int flat_length(const tree_t *e, int budget) {
    ...
    total = 1 + klen;
    if (e->v.sval && e->t != TT_QLIT && e->t != TT_CSET && e->t != TT_SUB_DECL && e->t != TT_PROC_DECL && e->t != TT_REGEX_DECL)
...
static void print_node(const tree_t * e, FILE * f, int depth) {
    ...
    if (inline_ok) {
        ...
        if (e->v.sval && e->t != TT_QLIT && e->t != TT_CSET && e->t != TT_SUB_DECL && e->t != TT_PROC_DECL && e->t != TT_REGEX_DECL) { fputc(' ', f); fputs(e->v.sval, f); }
