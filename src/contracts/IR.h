...
struct IR_graph_t {
    ...
    int            ring_head;
    int            ring_depth;
    int            zframe_graph; /* ICN-FR-2 / PL-FR-2: whole-graph ζ frame on rsp — ONE activation frame per graph, wire header above, rbp pinned; set by lower_icon.c (all Icon graphs, killswitch SCRIP_ICN_ZFRAME=0) and lower_prolog.c (all Prolog graphs, killswitch SCRIP_PL_ZFRAME=0); never set by any other lowerer (SN4/Raku/Pascal invariance guaranteed by construction).  Named for WHAT it decides ("this graph uses the ζ-frame storage regime"), never for a language.  Emitter routes on this flag: prologue sub rsp,kt + wire header + pin + seed (lex arm: rt_jmp_frame_lexprep2; non-lex arm: rt_icn_zframe_args_install); epilogue wire-header read + jmp; γ-retain for suspending graphs.  SCRIP_ICN_ZFRAME=0 reproduces pre-FR-2 HEAD for Icon graphs; SCRIP_PL_ZFRAME=0 reproduces pre-FR-2 HEAD for Prolog graphs. */
