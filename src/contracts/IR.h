...
struct IR_t {
    ...
    int           in_arbno;   /* PB-1s FAST-PATH β RECORD: 1 iff this IR_MATCH_DEFER was lowered while sno_in_arbno>0 (inside an ARBNO body). The DEFER fast-path pushes a 16B β record at L(4) so that DEFER_β: jmp [rsp] correctly retires to L(5)=MATCH_BEGIN.β when downstream fails. Inside an ARBNO body, ARBNO owns rsp arithmetic (β/σ/φ iterate a collection frame); the 16B push at L(4) shifts the rsp that ARBNO.α stored as [rbp+N], causing ARBNO.φ to restore the wrong rsp level and corrupt the β chain. Gate: no β push when in_arbno=1 — the ARBNO β machinery and MATCH_BEGIN.β handle backtrack correctly without it. Set at lower_snobol4.c DEFER build site (PB-1s arm); promoted to op_defer_in_arbno by emit_drive DEFER case. */
