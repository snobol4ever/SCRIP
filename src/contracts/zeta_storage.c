...
void zls_build(IR_graph_t * g) {
    ...
    { static int subj_on = -1; if (subj_on < 0) { const char * b = getenv("SCRIP_SUBJ_CELL"); subj_on = (b && *b == '0') ? 0 : 1; }   /* SUBJECT-CELL rung (b), s22y DEFAULT-ON (killswitch SCRIP_SUBJ_CELL=0, the zc_nofc polarity): proven strictly-better-by-set at flip time -- crosscheck 318, m3 220->233 m4 217->229, ZERO broken either mode, decline set carrying the dynamic-box/FENCE1 graphs flat-verbatim.  STMT_FRAME conjunct DROPPED same session: it rode the s21x era when STF was default-ON; one env, one pair. */   /* SUBJECT-CELL rung (a) (Lon s21x directive "NO FRAME RELATIVE addressing for operands" -- the s21x-e measured frontier: LIT_STRING 157 / VAR 85 / LIT_INTEGER 29 first-blockers are subject-position scalars): register the subject producer chain with IR_MATCH_HEAD as the reading consumer, MIRRORING the IR_ASSIGN loop above member for member -- same leaves, same tree scan, same gamma-adjacency fence, same d-simulation, same all-or-nothing capacity discipline.  Two-env gate = the fc_call_ok precedent: gate-off registration is ZERO, so the default path is byte-identical.  The head POPS the subject DESCR from TOS at the very top of alpha (bb_match_head subjc arm) BEFORE its own 32B window carve, so fc_leaf_walk's D=32+prefix math for every downstream pattern box is untouched.  fc_call arms cannot appear in subject trees: fc_vtree_scan's call arm requires g_fcc_gfence, which the first zls_build loop zeroes for every IR_MATCH_HEAD-bearing graph. */
      if (subj_on) for (int vi = 0; vi < g->n; vi++) { IR_t * h = g->all[vi]; if (!(h && h->op == IR_MATCH_HEAD && h->n_operands > 0 && h->operands[0])) continue;
        { int dyn = 0; for (int di = 0; di < g->n && !dyn; di++) { const IR_t * d = g->all[di]; if (d && (d->op == IR_MATCH_DEFER || d->op == IR_MATCH_PATREF || d->op == IR_MATCH_FENCE1)) dyn = 1; } if (dyn) continue; }   /* SUBJECT-CELL rung (b) decline set, s22y MEASURED: graphs bearing the DYNAMIC BOX customers (DEFER/PATREF -- blob re-entry whose deep-repoint rbp=rsp-K static model and wire glue predate the subject grant; 117/142 both-modes casualties) or FENCE1 (m3 exit-path scan spins post-output at a 16-sensitive boundary; 061/107, output-correct rc=124) stay flat-verbatim.  Degrade never die -- blob/fence arming is the named follow-on with the deep-repoint constant as its first audit. */
        IR_t * r = h->operands[0];
...
static const IR_t * fvs[64]; static int fvs_n = 0;   /* SUBJECT-CELL rung (b), s22y: membership of the SUBJECT producer class within fvl -- the one fvl population whose consumer (IR_MATCH_HEAD, ZD's 247-strong first-blocker) the non-popping spine cannot serve, so its grant is exempt from the zc_nofc retirement below */
