...
static int zls_grant_locals(const IR_t * nd, int scope_id, int off) {
    ...
    case IR_MATCH_ARBNO:
        if (IR_LIT(nd).ival == 1) { zls_field(scope_id, off, 16, ZK_RAW, 0, "arbno.owner quad: entry/yield/i/cap (4x4B; SN4-NARY-ARBNO one-node form)", nd); zls_field(scope_id, off + 16, 8, ZK_PTR_GC, 0, "arbno.COLLECTION ptr (rt_zcol_push-grown per-iteration elements: 16B header {prev_view, saved_delta} + body slot window; the rsp flavor = linked frame chain + explicit count in the header — Lon ruling 2026-07-12, lands at ZB-ITER under ZLS_ARBNO_STACK)", nd); zls_field(scope_id, off + 24, 8, ZK_RAW, 0, "arbno.saved_rsp (alpha saves rsp here; exhaust L(2) restores it)", nd); zls_field(scope_id, off + 32, 8, ZK_RAW, 0, "arbno.saved_outer_rbp (W-1c.2: unconditional — chain-beta always saves MATCH_BEGIN rbp into this slot before repointing rbp as element view; exhaust L(2) always restores it; view-restores at sigma/phi re-derive rbp from rsp unconditionally)", nd); return 3; }   /* W-1c.2: SCRIP_U2 gate removed.  The template always writes slot+32 (save rbp at alpha, restore at exhaust).  Symmetry law: grant=3, template reads +32 on every chain arm invocation. */
        return 0;
