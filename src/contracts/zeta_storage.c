...
static int zls_grant_locals(const IR_t * nd, int scope_id, int off) {
    ...
    case IR_MATCH_ARBNO:
        if (IR_LIT(nd).ival == 1) { static int _u2 = -1; if (_u2 < 0) { const char *_e = getenv("SCRIP_U2"); _u2 = (_e && _e[0] == '1') ? 1 : 0; } zls_field(scope_id, off, 16, ZK_RAW, 0, "arbno.owner quad: entry/yield/i/cap (4x4B; SN4-NARY-ARBNO one-node form)", nd); zls_field(scope_id, off + 16, 8, ZK_PTR_GC, 0, "arbno.COLLECTION ptr (rt_zcol_push-grown per-iteration elements: 16B header {prev_view, saved_delta} + body slot window; the rsp flavor = linked frame chain + explicit count in the header — Lon ruling 2026-07-12, lands at ZB-ITER under ZLS_ARBNO_STACK)", nd); zls_field(scope_id, off + 24, 8, ZK_RAW, 0, "arbno.saved_rsp (alpha saves rsp here; exhaust L(2) restores it)", nd); if (_u2) { zls_field(scope_id, off + 32, 8, ZK_RAW, 0, "arbno.saved_outer_rbp (U-2 nested frame: chain-beta saves outer rbp into FRQ(op_off+32); exhaust L(2) restores it before rsp restore — callee-saved ABI discipline across unbounded ARBNO growth)", nd); return 3; } return 2; }   /* ⭐ U-2 STRUCTURAL (s42+1): the +32 slot is ONLY granted when SCRIP_U2=1 (same env read as the template guard) so gate-OFF is byte-identical to baseline — no offset skew on any downstream node.  Symmetry law: zls grant and template read MUST agree on whether the slot exists; a mismatch at op_off+32 reads a neighbour's field.  The static caches the getenv once per compile (same pattern as arbno_nofill / arbno_u2_frame in the template). */
        return 0;
...
static int zls_s4_ok(IR_e op) { return op == IR_MATCH_SPAN || op == IR_MATCH_BREAK || op == IR_MATCH_BREAKX || op == IR_MATCH_TAB || op == IR_MATCH_RTAB || op == IR_MATCH_REM || op == IR_MATCH_BAL || op == IR_MATCH_ALTERNATE || op == IR_MATCH_FENCE1 || op == IR_MATCH_DEFER || op == IR_MATCH_PATREF || op == IR_MATCH_VALUE || op == IR_CALL_BUILTIN_PROLOG; }   /* SEQ-ERAD SE-5: IR_MATCH_SEQUENCE removed — zls_grant_locals returns 0, no dead front quad to elide */
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    { static int subj_on = -1; if (subj_on < 0) { const char * b = getenv("SCRIP_SUBJ_CELL"); subj_on = (b && *b == '0') ? 0 : 1; }   /* SUBJECT-CELL rung (b), s22y DEFAULT-ON (killswitch SCRIP_SUBJ_CELL=0, the zc_nofc polarity): proven strictly-better-by-set at flip time -- crosscheck 318, m3 220->233 m4 217->229, ZERO broken either mode, decline set carrying the dynamic-box/FENCE1 graphs flat-verbatim.  STMT_FRAME conjunct DROPPED same session: it rode the s21x era when STF was default-ON; one env, one pair. */   /* SUBJECT-CELL rung (a) (Lon s21x directive "NO FRAME RELATIVE addressing for operands" -- the s21x-e measured frontier: LIT_STRING 157 / VAR 85 / LIT_INTEGER 29 first-blockers are subject-position scalars): register the subject producer chain with IR_MATCH_BEGIN as the reading consumer, MIRRORING the IR_ASSIGN loop above member for member -- same leaves, same tree scan, same gamma-adjacency fence, same d-simulation, same all-or-nothing capacity discipline.  Two-env gate = the fc_call_ok precedent: gate-off registration is ZERO, so the default path is byte-identical.  The head POPS the subject DESCR from TOS at the very top of alpha (bb_match_begin subjc arm) BEFORE its own 32B window carve, so fc_leaf_walk's D=32+prefix math for every downstream pattern box is untouched.  fc_call arms cannot appear in subject trees: fc_vtree_scan's call arm requires g_fcc_gfence, which the first zls_build loop zeroes for every IR_MATCH_BEGIN-bearing graph. */
      if (subj_on) for (int vi = 0; vi < g->n; vi++) { IR_t * h = g->all[vi]; if (!(h && h->op == IR_MATCH_BEGIN && h->n_operands > 0 && h->operands[0])) continue;
        ...
        if ((r->op == IR_LIT_INTEGER || r->op == IR_LIT_STRING || r->op == IR_LIT_REAL
             || (r->op == IR_VAR && IR_LIT(r).sval && IR_LIT(r).sval[0] != '&' && ((is_global(IR_LIT(r).sval) && !graph_has_local(g, IR_LIT(r).sval)) || !strcmp(IR_LIT(r).sval, "write") || !strcmp(IR_LIT(r).sval, "writes"))))
            && r->γ.node == h && fc_vcap(1, 1, 0, 0)) { fc_vlit_register(r); fc_subj_register(r); fc_vread_register(h, 0); continue; }   /* direct pair: scalar lit or global-routed var -> head, gamma-adjacent; LIT_CHARSET stays flat (a cset subject is the runtime type-error path, not a value-spine citizen).  s22y: fc_subj_register makes the PRODUCER grant survive zc_nofc at the fc_geom vlit line -- the s22s "producer-side gate UNLOCATED" was that line's !zc_nofc() term, landed s22l AFTER this loop was written: it retired the ASSIGN-pair producer half (ZD covers those consumers) but took the subject producer with it while fc_vread_fp armed the head unconditionally -- the exact consumer-armed/producer-flat displacement the bare decouple measured. */
...
long zw_carve_k(const IR_t * nd) {
    ...
    if (!_all && (nd->op == IR_MATCH_BEGIN || nd->op == IR_MATCH_FENCE1 || nd->op == IR_SAVE_RESTORE || ir_norm_call_kind(nd->op) == IR_CALL || nd->op == IR_GOTO_DEFERRED)) return 0;
