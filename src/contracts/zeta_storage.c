...
static int zls_grant_locals(const IR_t * nd, int scope_id, int off) {
    ...
    case IR_MATCH_FENCE0:
        /* ⭐ FENCE-SPLIT (this rung): FENCE0 is its OWN kind now, but it KEEPS the identical quad grant so this commit is byte-identical — dropping it would renumber every
         * later node's offsets and mix a structural split with a layout change.  The quad is RESERVED, not vestigial: it is exactly the home an own-extent FENCE0 whack needs (bank rsp at α, restore
         * at γ — the relative shape bb_match_fence1 proves, which cannot dive under the UCLAIM statement claim the way the retired absolute ___-floor whack did).  Dropping or using it is a separate rung. */
...
static int zls_locals_shifted(IR_e op) { return op == IR_MATCH_BEGIN || op == IR_MATCH_ALTERNATE || op == IR_MATCH_ARB || op == IR_MATCH_BAL || op == IR_MATCH_FENCE0 || op == IR_MATCH_FENCE1 || op == IR_MATCH_ARBNO || op == IR_MATCH_SPAN || op == IR_MATCH_BREAK || op == IR_MATCH_BREAKX || op == IR_MATCH_TAB || op == IR_MATCH_RTAB || op == IR_MATCH_REM || op == IR_MATCH_DEFER || op == IR_MATCH_VALUE || op == IR_MATCH_ASSIGN_SAVE || op == IR_SCAN_ENTER || op == IR_INITIAL; }   /* SEQ-ERAD SE-5: IR_MATCH_SEQUENCE removed — zero locals, no front-quad shift */
 ...
 * shrinks by (dead−1)×16 and the prologue stosb with it (s132: the match phase is STORE-BOUND, 63% stores).  TWO safety fences, both op-filters so drift cannot create a wrong answer: (1) only kinds
 ...
 * OP-FILTER = kinds positively audited to hold ALL runtime state in their zls_grant_locals fields with no cross-box reader of the front quad: SPAN/BREAK/BREAKX/TAB/RTAB/REM/BAL (fc-cell matchers),
 ...
 * the exact over-count the pre-lift fc_tail_walk refuse existed to avoid; fc_geom(granted ALT)=16, so the ALT case MUST run before the fc_geom catch).  Leaf displacements: the ALT itself registers
 ...
 * fpb+span-rmn right); skip = the ARBNO node itself (registers nothing; its tail arm speaks raw [rsp+const]).  An ungranted ALT cannot reach a finalized candidate (fc_tail_walk refused the
...
static int fct_defer_susp(const IR_t * nd) {   /* the licensed defer's compile-time retention: SUSP = align16(32+fb)+fp+16 via the emit_patzeta registry keyed by the node's PAT$ operand literal;
                                                * -1 = not licensed here (no PAT$ operand / unregistered / non-uniform target) -- the caller refuses the candidate wholesale. */
...
static int fc_vbinop_ok(long long v) { return v == 0 || v == 1 || v == 2 || v == 3 || v == 4 || v == 11 || v == 18; }   /* ZB-VAL-4/6a widened s21x-e (Lon ACROSS-THE-BOARD): ADD/SUB/MUL/CONCAT + DIV(3)/MOD(4)/POW(18).  The prior DIV/MOD refuse ("the lean fc arm has no idiv fault path") is STALE vs the collapsed one-per-op-call arm (bb_binop_arith rtop dispatch): non-fast ops take the generic rt_num_arith call whose DT_FAIL lands x86_omega -- the fault path IS the arm's default, measured on 026/027 (the original regime casualties) this session.  Relops stay out (ZB-VAL-7's own zero-or-one-result shape) */
...
void zls_build(IR_graph_t * g) {
     ...
     * emission entry, where every earlier-emitted PAT$ is registered, and the pending candidates resolve (price at SUSP) or refuse wholesale (chain arm; head/arbno nulled + SEQ unregistered). */
...
void zls_fct_finalize(IR_graph_t * g, int late) {
    ...
    { extern int fc_geom(const IR_t *, long *); extern void fc_cond_register(const IR_t *, int);
      for (int c = 0; c < fct_n; c++) {
         ...
         * (fct_defer_susp > 0) or the WHOLE candidate refuses to the chain arm -- head+arbno NULLED so fc_tail_head (fin-blind, consulted by IR_MATCH_BEGIN before this ARBNO emits) and every other
...
int fc_geom(const IR_t * nd, long * k) {
    ...
    { static int _ac = -1; if (_ac < 0) { const char * e = getenv("SCRIP_ALT_CAP"); _ac = (e && *e == '0') ? 0 : 1; }
      if (fc_arm_member(nd) && !(_ac && nd->op == IR_MATCH_ASSIGN_SAVE && fc_save_active(nd))) return 0; }   /* ALT-FLAT (s202): arm residents are flat -- zero cell, zero rsp motion; their zls quads are kept (zls_fc_cell twin guard) */   /* ⭐ ALT-CAP (s66, THE s65c INVISIBLE-RED ROOT CAUSE): s202's "zero rsp motion" premise is FALSE AT EMISSION -- bb_match_alternate.cpp:65 emits sub rsp,32 ("alpha carves its OWN 32B record", its line 25) -- so an arm resident denied its cell here falls to the flat rt_cap array path (next line's own words) whose raw flat coordinate is spelled with op_zdepth = the box's OWN carve = 0 (emit.cpp:907 "Zero when the box carves nothing"), leaving the slot exactly the ALT's 32 too low: rt_cap_push receives unclaimed stack.  MEASURED, four minted variants oracle-checked: capture INSIDE an arm ('ab' | 'xy' . W) SIGSEGVs in m4 while the same capture OUTSIDE (('ab' | 'xy') . W) passes, and hand-patching ONLY the emitted offset 192->224 (+32 = the ALT carve, exactly) makes both crashing variants exit 0 byte-identical to sbl.  m3 "passes" both by landing in mapped slack and corrupting silently -- the s22r envp class the retired cap_anchor_of comment already names, so m3 exit 0 was never exoneration.  THE GRANT IS THE FIX, NOT AN OFFSET PATCH: fc_save_active is ALREADY 1 for these nodes (SCRIP_CAP_DIAG: save_active=1 fc_bytes=0), so this denial was the SOLE suppressor, and granting restores the SAME ZD capture path (SAVE K=16 own cell) that s53 retired cap_anchor_of in favour of and called "the FORTH-correct ONE system" -- the box then compensates for exactly what IT carved and is depth-correct at any ancestor depth.  Narrowed to ASSIGN_SAVE on purpose: the other arm-resident kinds carry no cross-box slot and stay flat.  Killswitch SCRIP_ALT_CAP=0 restores the s202 denial verbatim. */
 ...
 * captures / wiring -- nested ALTERNATE, ARBNO, ARB, DEFER, anything unknown REFUSES: outer stays flat,
 ...
 * both refuse it -- arm residents keep their FLAT zls quads and push NO rsp cell, so every arm's rsp footprint is 0, every arm yields at the ALT's own frontier, and the S10d pad-to-max law is
 ...
 * pre-refuse answers; every LATER reader (statement walk, fc_leaf_walk, fct twins, zls_build via drive_slots_all, emit staging) sees the refuse.  Same node-ptr keying/staleness envelope as fca. */
...
void fc_alt_register(const IR_t * nd, int n, const int * fp, const int * ab, const int * ae) {
    if (!nd || n <= 0 || n > 10 || fca_n >= 256) return;   /* N>10 exceeds the 3N+2 <= XA_BB_EMIT_PAIR_MAX(32) stub budget -- silent refuse */
 ...
 * linear op-filter verbatim): every element must respect the S10c port invariant -- granted leaves, zero-cell
 * leaves, nested SEQ, captures, wiring.  Nested ALTERNATE/ARBNO/ARB/DEFER/ABORT/unknown REFUSE the whole SEQ
...
int fc_arbno_member(const IR_t * nd) { for (int i = 0; i < fcab_n; i++) if (fcab[i] == nd) return 1; return 0; }   /* PS-3 s153: the tail candidate's pat_entry SEQ converts at lower ASSUMING the tail lands; a finalize-refuse (defer target unregistered/non-uniform) must revert it or the SEQ's static re-points assume element depths the chain arm never establishes.  Swap-remove; consulted (fc_seq_active) only at emit, after the layout pass where the refuse runs. */
 ...
 * either direction REFUSES both registrations and the capture keeps the flat rt_cap array path verbatim
...
int fc_cond_fp(const IR_t * nd) {
    ...
    for (int i = 0; i < fcc_n; i++) if (fcc[i].nd == nd) {
        if (fcc[i].save && fc_arm_member(fcc[i].save)) return -1;   /* ALT-FLAT coherence: SAVE inside ALT arm gets no rsp cell (fc_arm_member=1, fc_geom=0); COND must also refuse to rt_cap_top path */
        return fcc[i].fp;
    }
    return -1;
}
 ...
 * only one padded arm is live at yield -- so statements containing ALTERNATE refuse wholesale; the per-ALT 16+fpmax lift is a named follow-on).  Eligibility = the 3c walk verbatim over the PATTERN
 * range only; either-direction failure refuses and the whole statement keeps the flat path byte-verbatim (degrade never die).  Side table keyed by the HEAD node ptr (the fcc precedent). */
 ...
 * file's layout pass FINALIZES geometry (windows need zls offsets, which exist only here) and registers every range leaf's fcl displacement with the element-region formula.  Refuses degrade to the
 ...
 * primitives (pre-chain flat operand slots), and captures allocated left of the ARBNO all refuse v1. */
...
int fc_tail_defer_susp_g(IR_graph_t * g, const IR_t * nd) {   /* PS-3 s153: is nd a PRICED defer leaf inside a FINALIZED tail candidate of graph g?  Returns its SUSP for the zero-guarded β (the
                                             ...
                                             * DT-arm defers, refused/pending candidates all keep the original β byte-verbatim (default md5 identity). */
...
long zw_carve_k(const IR_t * nd) {
    ...
    if ((_bo && *_bo) || (_bs && *_bs)) { int _nid = bb_node_id((IR_t *)nd); if (_bo && *_bo && !zw_nid_listed_c(_bo, _nid)) return 0; if (zw_nid_listed_c(_bs, _nid)) return 0; }   /* ⭐⭐⭐⭐⭐ s137 — THE s136 PERTURBATION, LOCALISED AND CURED AT ITS SOURCE.  `bb_node_id` in dense mode (`g_m4_dense_nid=1`, which mode 4 sets) is a LAZY ALLOCATE-ON-FIRST-CALL counter (`++g_nid_count`, emit.cpp:342), so calling it ASSIGNS a number in CALLER-VISIT ORDER.  zw_carve_k called it UNCONDITIONALLY while consuming it ONLY under the two SCRIP_BB_ONLY/SCRIP_BB_SKIP debug filters -- so `zdp_analyze` (which reads THE ONE CARVE AUTHORITY from scrip_ir.c:274, BEFORE the emitter walks) seeded every nid in LATTICE order instead of EMIT order, and every `.Lbynamefnzd<nid>` label moved.  That is the whole of the 81/656 movers: an ALLOCATION, exactly as the FINDING predicted, in suspect #1 of its ranked list.  ⛔ THE CURE IS NOT TO REPLICATE THE EMITTER'S VISIT ORDER (a second opinion about an order somebody else owns, and it would drift) -- it is to STOP NUMBERING WHEN THE NUMBER IS NOT WANTED.  The default arm is unaffected because the emitter assigns each nid at its own walk choke (emit.cpp:1034) regardless; this call was never the assigning caller on the shipped path, only on the instrumented one. */
    return _k;
}
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
