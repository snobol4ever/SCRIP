...
#error "ZC_FRAME_RBP does not run (MEASURED s202, not assumed): it BUILDS clean and executes trivial programs, then crashes 13 of 22 corpus programs that the RSP default runs green -- SNOBOL4 feat, Icon rung36, and Prolog rung10 alike (matched compiler+runtime pair; the RSP control's only 2 crashes are the pre-existing jcon_args FZ-E1 and the jcon_btrees xfail). ROOT CAUSE, structural: the 17 `ZC_FRAME != ZC_FRAME_RSP` arms across bb_match_release/capture/head, bb_call_proc_staged, xa_flat and zeta_storage.c were written for the R12 basis -- bb_call_proc_staged.cpp:281 says so in its own words, 'configs where r12 IS the zeta frame' -- and s201's ZR-RSPRBP-1 deleted ZC_FRAME_R12's LABEL while leaving that CODE in place, so those arms silently re-pointed at RBP without ever being designed for it. Under RBP, x86_zr() and x86_fb() are BOTH rbp, so arms like bb_match_release.cpp:32's `push x86_zr()` / `mov x86_zr(),rsp` push and repoint the frame base itself; and x86_align_save(), the helper whose whole purpose was making the C-call dance frame-safe under an rbp frame, has been DELETED (x86_asm.h:1435 still describes it as live). The zeta basis is therefore reachable at RSP ONLY. THIS GUARD IS NOT THE FIX -- it converts a silent 13-crash trap into a loud one. The fix is a decision for Lon: either re-establish the RBP arms against the current register contract, or delete all 17 and retire ZC_FRAME entirely, leaving the PER-GRAPH x86_fb_pinned() rsp/rbp selection (FLATDISP-8) as the whole zeta RSP/RBP story -- which is the duality that actually runs and is already complete (FLATDISP-9 census: ~285,000 frame refs, unseeded=0). Do NOT half-land the deletion: several arms sit inside suspend/resume protocols. See FINDING-2026-07-28b."
#endif
