...
void region_report(IR_graph_t * g) {
    if (!g || g->n <= 0) return;
    int * rid = (int *)ct_zalloc((size_t)g->n, sizeof(int)); if (!rid) return;
    int next = 0, cur = 0;
    for (int i = 0; i < g->n; i++) { IR_t * nd = g->all[i]; if (nd && nd->op == IR_MATCH_BEGIN) cur = ++next; rid[i] = cur; if (nd && nd->op == IR_MATCH_END) cur = ++next; }
    int nr = next + 1;
    int * bin = (int *)ct_zalloc((size_t)nr, sizeof(int)); int * bout = (int *)ct_zalloc((size_t)nr, sizeof(int)); int * bxin = (int *)ct_zalloc((size_t)nr, sizeof(int));
    int * lo = (int *)ct_zalloc((size_t)nr, sizeof(int)); int * hi = (int *)ct_zalloc((size_t)nr, sizeof(int)); int * cnt = (int *)ct_zalloc((size_t)nr, sizeof(int)); int * pat = (int *)ct_zalloc((size_t)nr, sizeof(int));
    if (!bin || !bout || !bxin || !lo || !hi || !cnt || !pat) { ct_drop(rid); ct_drop(bin); ct_drop(bout); ct_drop(bxin); ct_drop(lo); ct_drop(hi); ct_drop(cnt); ct_drop(pat); return; }
    ...
    ct_drop(rid); ct_drop(bin); ct_drop(bout); ct_drop(bxin); ct_drop(lo); ct_drop(hi); ct_drop(cnt); ct_drop(pat);
}
