...
void region_report(IR_graph_t * g) {
    ...
    int next = 0, cur = 0;
    for (int i = 0; i < g->n; i++) { IR_t * nd = g->all[i]; if (nd && nd->op == IR_MATCH_HEAD) cur = ++next; rid[i] = cur; if (nd && nd->op == IR_MATCH_RELEASE) cur = ++next; }
    int nr = next + 1;
    ...
    for (int i = 0; i < g->n; i++) { int r = rid[i]; if (lo[r] < 0) lo[r] = i; hi[r] = i; cnt[r]++; IR_t * nd = g->all[i]; if (nd && (nd->op == IR_MATCH_HEAD || (nd->op >= IR_MATCH_LIT && nd->op <= IR_MATCH_ASSIGN_SAVE))) pat[r] = 1; }
    for (int i = 0; i < g->n; i++) {
    ...
    fprintf(stderr, "[REGION-REPORT] graph=%p nodes=%d regions=%d (partition v0: MATCH_HEAD..MATCH_RELEASE pattern intervals; branch-in/out over gamma+omega edges; beta-sigil external landings tracked)\n", (void *)g, g->n, nr);
