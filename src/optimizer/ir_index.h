...
static inline void ir_index_build(ir_index_t * ix, IR_graph_t * g) {
    ...
    ix->key = (IR_t **)ct_zalloc(ix->cap, sizeof(IR_t *)); ix->val = (int *)ct_zalloc(ix->cap, sizeof(int));
    if (!ix->key || !ix->val) { ct_drop(ix->key); ct_drop(ix->val); ix->key = (IR_t **)0; ix->val = (int *)0; return; }
    for (int i = 0; i < g->n; i++) {
        IR_t * p = g->all[i];
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static inline void ir_index_free(ir_index_t * ix) { ct_drop(ix->key); ct_drop(ix->val); ix->key = (IR_t **)0; ix->val = (int *)0; }
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static inline void ir_pset_add(ir_pset_t * s, const IR_t * p) {
    ...
    if (s->cnt * 2u >= s->cap) {
        ...
        const IR_t ** ns = (const IR_t **)ct_zalloc(nc, sizeof(const IR_t *));
        if (!ns) return;
        for (size_t i = 0; i < s->cap; i++) if (s->slot[i]) ns[ir_pset_probe(ns, nc, s->slot[i])] = s->slot[i];
        ct_drop(s->slot); s->slot = ns; s->cap = nc;
    }
    size_t i = ir_pset_probe(s->slot, s->cap, p);
    if (!s->slot[i]) { s->slot[i] = p; s->cnt++; }
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static inline void ir_pset_free(ir_pset_t * s) { ct_drop(s->slot); s->slot = (const IR_t **)0; s->cap = 0; s->cnt = 0; s->has_null = 0; }
#endif
