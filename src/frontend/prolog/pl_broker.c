...
{
    (void)zeta; (void)entry;
    fprintf(stderr, "libscrip: FATAL pl_cat_fn — C BB box DELETED (GROUND ZERO). "
                    "Prolog sequence/concat box must be rebuilt as an emitted wired graph.\n");
    abort();
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static DESCR_t pl_unify_fn(void *zeta, int entry) {
    pl_unify_t *ζ = zeta;
    if (entry == α) {
        ζ->mark  = trail_mark(&g_resolve_trail);
        ζ->fired = 0;
        if (!ζ->node || ζ->node->n < 2) return FAILDESCR;
        Term *t1 = resolve_unified_term_from_expr(ζ->node->c[0], ζ->env);
        Term *t2 = resolve_unified_term_from_expr(ζ->node->c[1], ζ->env);
        if (!unify(t1, t2, &g_resolve_trail)) { trail_unwind(&g_resolve_trail, ζ->mark); return FAILDESCR; }
        ζ->fired = 1;
        return NULVCL;
    }
    if (ζ->fired) { trail_unwind(&g_resolve_trail, ζ->mark); ζ->fired = 0; }
    return FAILDESCR;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
static DESCR_t pl_head_unify_fn(void *zeta, int entry) {
    ...
    if (entry == α) {
        ζ->head_mark = trail_mark(&g_resolve_trail);
        ζ->cenv      = resolve_env_new(ζ->n_vars);
        ζ->fired     = 0;
        for (int i = 0; i < ζ->arity && i < ζ->ec->n; i++) {
            Term *ha = resolve_unified_term_from_expr(ζ->ec->c[i], ζ->cenv);
            Term *ca = (ζ->caller_args && i < ζ->arity) ? ζ->caller_args[i] : term_new_var(i);
            if (!unify(ca, ha, &g_resolve_trail)) {
                trail_unwind(&g_resolve_trail, ζ->head_mark);
...
static DESCR_t pl_choice_fn(void *zeta, int entry) {
    ...
    if (entry == β && ζ->cur_active) {
        ...
        trail_unwind(&g_resolve_trail, ζ->trail_mark);
        ζ->ci++;
        ζ->cur_active = 0;
    } else if (entry == α) {
        ζ->trail_mark = trail_mark(&g_resolve_trail);
        ζ->ci         = 0;
        ζ->cur_active = 0;
        ζ->cut        = 0;
    }
...
bb_node_t pl_box_choice_call(tree_t *goal, Term **env) {
    ...
    tree_t *choice = resolve_pred_table_lookup_global(key);
    ...
    for (int i = 0; i < arity; i++) {
        ...
        if (ch && ch->t == TT_VAR && (int)ch->v.ival == -1)
            ...
            caller_args[i] = resolve_unified_term_from_expr(ch, env);
    }
