...
%type <ival> constant scalar_constant simple_type type dim_list dim_entry
%type <ival> with_open
%start program
%%
program:
...
dim_list:
    dim_entry { $$ = $1; }
    | dim_list COMMA dim_entry { if (g_pas_pend_ndim < 8) g_pas_pend_dims[g_pas_pend_ndim++] = $3; $$ = $1; }
    ;
dim_entry:
    simple_type { if (g_pas_pend_ndim == 0) { g_pas_pend_dims[g_pas_pend_ndim++] = $1; } $$ = $1; }
    ;
packed_opt: PACKEDSY | ;
simple_type:
    LPARENT id_list RPARENT
    ...
    | IDENT { g_pas_pend_typename = strdup($1); g_pas_pend_ischar = !strcmp($1, "char"); const char *_pt = pas_ptrtype_target($1); if (_pt) { g_pas_pend_ptrtarget = strdup(_pt); $$ = -3; } else { long long _eh = pas_enumtype_high($1); long long _sh = pas_subtype_high($1); if (_eh >= 0) { $$ = _eh; } else if (_sh >= 0) { $$ = _sh; } else { pas_rectype_to_pend($1); $$ = -1; } } }
...
assignment:
    ...
    ;
selector:
    selector LBRACK expression_list RBRACK { if ($3 && $3->count == 2 && $1 && $1->t == TT_VAR && $1->v.sval) { long long _nc = pas_array2_ncols($1->v.sval); if (_nc > 0) { tree_t *_idx = bin(TT_ADD, bin(TT_MUL, $3->items[0], ilit(_nc)), $3->items[1]); tree_t *e = ast_node_new(TT_IDX); ast_push(e, $1); ast_push(e, _idx); $$ = e; } else { tree_t *e = ast_node_new(TT_IDX); ast_push(e, $1); if ($3) for (int i = 0; i < $3->count; i++) ast_push(e, $3->items[i]); $$ = e; } } else { tree_t *e = ast_node_new(TT_IDX); ast_push(e, $1); if ($3) for (int i = 0; i < $3->count; i++) ast_push(e, $3->items[i]); $$ = e; } }
